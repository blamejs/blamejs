...
- **Supply-chain compromise via npm transitive deps** — zero npm runtime dependencies. Every external library is vendored under `lib/vendor/` as an unminified, reviewable bundle, with a manifest pinning version + license + provenance. Build reproducibility is verified via the GHCR image's SLSA provenance attestation (see DEPLOY.md → "Release verification").
...
- [ ] Behind an edge that publishes the client address as something other than `X-Forwarded-For` — Cloudflare's `CF-Connecting-IP`, or `X-Real-IP` from the usual nginx recipe — name it: `b.requestHelpers.trustedClientIp({ trustedProxies: [...], forwardedHeaders: ["cf-connecting-ip"] })`. Reading that header directly instead drops the peer gate entirely, and passing `clientIpResolver` hands back the whole trust decision while still reporting `peerGated`. List only headers your proxy sets or overwrites on every request: the gate proves a request arrived through your proxy, not that your proxy authored the header, so a header passed through unchanged can be injected by the client. The same applies to any decision keyed on the client address, including `b.flag.middleware({ trustedProxies: [...] })` — without it every anonymous caller behind the proxy shares one rollout bucket
- [ ] Behind an identity-injecting reverse proxy (Cloudflare Access, oauth2-proxy, Tailscale Serve): resolve its identity headers through `b.requestHelpers.trustedIdentityHeaders({ trustedProxies: [...], headers: {...} })` rather than reading them directly. The family is trusted only when the immediate socket peer is a declared trusted proxy; from any other peer it is stripped from `req.headers`, so a client cannot forge the identity. With no gate it fails closed (always strips). Reading these headers without a peer-gate is a full impersonation bypass
- [ ] For outbound integrations: pin destination hosts via `b.httpClient.request({ allowedHosts: ["api.partner.com", ".internal.example.com"] })` so a compromised process can't reach arbitrary upstreams. The primitives that dial an identity provider on your behalf take the same pin, and the client to dial through, at construction: `b.auth.oauth.create({ ..., http: { client, allowedHosts: ["idp.example.com"] } })`, likewise `b.auth.ciba.client.create` and `b.auth.saml.fetchMdq`. The pin covers discovery and JWKS as well as the token endpoint, which is where a mangled issuer is read; supplying `client` also keeps those dials inside whatever circuit breaker and retry policy your other calls to that upstream already use
...
- [ ] Alert on the `tls.no_ephemeral_key_exchange` audit action alongside `tls.classical_downgrade`. The first says an outbound connection negotiated no ephemeral key exchange at all — no forward secrecy, so recorded traffic is decryptable to anyone who later obtains the peer's long-term key. The second says the key exchange was forward-secret but classical, which is the harvest-now-decrypt-later exposure. Both carry host + port, and together they are the evidence of which dependencies are post-quantum-ready
...
- [ ] Build your own `https.Server` / `https.Agent` through `b.network.tls.applyToContext({ base })` rather than hand-listing TLS options, so it offers the configured key shares and certificate compression and keeps tracking `b.network.tls.preferredGroups.set(...)`. A server assembled by hand negotiates on the runtime's default group order, which is wider than the framework's and outside your control
