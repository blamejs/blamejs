...
- [ ] For routes that emit or accept CSV (operator exports, user-supplied uploads, mail attachments, object-store deliverables): wire `b.guardCsv.gate({ profile: "strict" })` into `b.staticServe.create({ contentSafety: { ".csv": gate } })` and `b.fileUpload.create({ contentSafety: gate })` — strict profile applies the OWASP-recommended `prefix-tab` formula-injection mitigation, the dangerous-function denylist (HYPERLINK / WEBSERVICE / IMAGE / IMPORT* / RTD / DDE / CALL), bidi / homoglyph / control / null-byte / BOM detection, dialect-ambiguity refusal, and CSV-bomb size caps; pick `compliancePosture: "hipaa" | "pci-dss" | "gdpr" | "soc2-cc7"` instead of (or layered over) the profile when the workload is regulated
...
- [ ] Or wire `b.guardAll.byExtension({ profile: "strict", audit: b.audit })` directly into `b.staticServe.create({ contentSafety })` and `b.guardAll.gate({ profile: "strict", audit: b.audit })` into `b.fileUpload.create({ contentSafety })`. Default is every shipped guard ON; opt out per guard with `exceptFor: { name: { reason: "..." } }` (the reason lands in the `guardAll.gate.created` audit row so a security review can reconstruct what this deploy elected NOT to defend against, and why). Future guards added to the family auto-extend the deploy without re-wiring
