...
async function testAppShutdownMiddleware503DuringDrain() {
  ...
  var mw = o.middleware();
  // Before draining: passes through
  var EE = require("node:events").EventEmitter;
  var req1 = new EE();
  req1.method = "GET";
  req1.url = "/";
  req1.headers = {};
  var res1 = _metricsRes();
  var passed = false;
  ...
  // Start shutdown — draining flag flips
  var sp = o.shutdown();
  // New requests now get 503
  var req2 = new EE(); req2.method = "GET"; req2.url = "/"; req2.headers = {};
  var res2 = _metricsRes();
  var nextCalled = false;
  mw(req2, res2, function () { nextCalled = true; });
  check("middleware: draining → 503",           res2._statusCode === 503);
...
async function testAppShutdownInFlightTracking() {
  ...
  var res = _metricsRes();
  // Start a "request" — middleware passes through, increments count.
  mw(req, res, function () {
    check("inFlight: counted while in flight", o.inFlight() === 1);
    // Don't call res.end yet — counter stays at 1.
    resolveReq = function () { res.end(); };
  });
  // Two more parallel requests
  var req2 = new EE(); req2.method = "GET"; req2.url = "/"; req2.headers = {};
  var res2 = _metricsRes();
...
async function testAppShutdownDrainPhaseWaitsForInFlight() {
  ...
  // Add a drain-in-flight phase that uses orchestrator.waitInFlight
  o.addPhase({
  ...
  var res = _metricsRes();
  // Start an in-flight request that won't end immediately.
  mw(req, res, function () {});
...
async function testAppShutdownDrainTimeoutWhenInFlightStuck() {
  ...
  var res = _metricsRes();
  mw(req, res, function () {});
  // Never call res.end — request stuck in flight.
  var result = await o.shutdown();
...
async function testAppShutdownStandardPhasesBuilder() {
  // Build standard phases from a components map
  var stops = [];
  var components = {
  ...
  // Order check: mark-draining → scheduler → jobs → websockets → http-server → cluster → db → external-db
  ...
  // Run them and check execution order matches
...
async function testAppShutdownStandardPhasesOmitsAbsentComponents() {
  // Only db + cluster — others omitted
...
async function testAppShutdownSignalHandlersInstall() {
  ...
  // Verify handlers are registered on the process
  ...
  return {
    ...
    context: {
      ...
      with: function (ctx, fn) {
        // Track active span via our own state; we don't model context API fully.
        var prev = activeSpan;
...
async function testTracingPassthroughSpan() {
  // Force pass-through: no @otel/api installed.
  ...
  var result = await t.span("test-op", function (span) {
    ranWith = span;
    span.setAttribute("k", "v"); // no-op but doesn't throw
    span.addEvent("hello");
    return 42;
  });
...
async function testTracingRealSpan() {
  // Inject a fake OTel API to verify the real-tracer path.
  b.tracing._resetForTest();
...
async function testTracingCurrentSpanInsideFn() {
  ...
  // After span ended, currentSpan returns null
...
async function testTracingTapNoOpWhenNoRegistry() {
  b.tracing._resetForTest();
  // No registry created → tap should still call the fn but not span it.
  var ran = false;
...
async function testTracingPassthroughTap() {
  // Registry active but OTel not installed: tap creates pass-through span.
  b.tracing._resetForTest();
  b.tracing._setOtelForTest(null);
  var t = b.tracing.create();
  var captured = null;
...
function testCspNonceInjectStrictDynamic() {
  ...
  // strict-dynamic only applies to script-src
...
function testCspNonceInjectIdempotent() {
  ...
  // Same nonce shouldn't be added twice if middleware accidentally runs twice
...
async function testCspNonceMiddlewareSkipsWhenNoCspAndNotAlways() {
  // Default behavior: if no CSP header was set by anyone earlier and
  // operator didn't pass always:true, don't create a fresh CSP. The
  // nonce is still on req — operators can use it without the middleware
  // imposing a CSP on routes that don't need one.
...
async function testCspNonceMiddlewareNonceLengthDefault() {
  ...
  var res = _cspRes(null);
  await new Promise(function (resolve) { mw(req, res, function () { resolve(); }); });
  // base64(16 bytes) = 24 chars (with padding) or 22 (without). Default = 16 bytes.
...
async function testCspNonceLayeredOnSecurityHeaders() {
  // End-to-end: securityHeaders runs first, then cspNonce.
  // Verify the final CSP header contains both the security-headers'
  // baseline AND the nonce-injected directives.
...
function testRenderHtmlAutoMergesResLocals() {
  // Verify render.js auto-merges res.locals into template data so
  // operators don't have to thread cspNonce through every render call.
  var stubEngine = {
    render: function (viewName, data) {
      // Capture the data the engine would render with, return as JSON
      // for assertion.
...
function testRenderHtmlOperatorDataOverridesLocals() {
  // Operator-supplied data wins on key collision. Operators sometimes
  // override the request-scoped value (e.g. force a specific CSP nonce
  // in test fixtures).
  var stubEngine = {
...
async function run() {
  ...
  // tracing — OpenTelemetry seam without otel runtime dep (Phase 9.8)
  testTracingSurface();
...
module.exports = {
  ...
  testAppShutdownPhaseFailureContinues:      testAppShutdownPhaseFailureContinues,
  testAppShutdownPhaseTimeout:               testAppShutdownPhaseTimeout,
  testAppShutdownIdempotent:                 testAppShutdownIdempotent,
  testAppShutdownDrainingFlag:               testAppShutdownDrainingFlag,
  testAppShutdownMiddleware503DuringDrain:   testAppShutdownMiddleware503DuringDrain,
  testAppShutdownInFlightTracking:           testAppShutdownInFlightTracking,
  testAppShutdownDrainPhaseWaitsForInFlight: testAppShutdownDrainPhaseWaitsForInFlight,
  testAppShutdownDrainTimeoutWhenInFlightStuck: testAppShutdownDrainTimeoutWhenInFlightStuck,
  testAppShutdownAddPhaseRejectsAfterStart:  testAppShutdownAddPhaseRejectsAfterStart,
  testAppShutdownStandardPhasesBuilder:      testAppShutdownStandardPhasesBuilder,
  testAppShutdownStandardPhasesOmitsAbsentComponents: testAppShutdownStandardPhasesOmitsAbsentComponents,
  testAppShutdownSignalHandlersInstall:      testAppShutdownSignalHandlersInstall,
  testAppShutdownConfigValidation:           testAppShutdownConfigValidation,
  testTracingSurface:                        testTracingSurface,
  testTracingTraceparentParse:               testTracingTraceparentParse,
  testTracingTraceparentFormat:              testTracingTraceparentFormat,
  testTracingNewIds:                         testTracingNewIds,
  testTracingPassthroughSpan:                testTracingPassthroughSpan,
  testTracingPassthroughSpanAsync:           testTracingPassthroughSpanAsync,
  testTracingPassthroughSpanThrows:          testTracingPassthroughSpanThrows,
  testTracingRealSpan:                       testTracingRealSpan,
  testTracingRealSpanThrowRecordsException:  testTracingRealSpanThrowRecordsException,
  testTracingCurrentSpanInsideFn:            testTracingCurrentSpanInsideFn,
  testTracingContextHeaders:                 testTracingContextHeaders,
  testTracingExtractContext:                 testTracingExtractContext,
  testTracingRequestMiddleware:              testTracingRequestMiddleware,
  testTracingTapRoutes:                      testTracingTapRoutes,
  testTracingTapNoOpWhenNoRegistry:          testTracingTapNoOpWhenNoRegistry,
  testTracingPassthroughTap:                 testTracingPassthroughTap,
  testTracingIsReal:                         testTracingIsReal,
  ...
  testCspNonceInjectAddsMissingDirective:    testCspNonceInjectAddsMissingDirective,
  testCspNonceInjectStrictDynamic:           testCspNonceInjectStrictDynamic,
  testCspNonceInjectIdempotent:              testCspNonceInjectIdempotent,
  testCspNonceInjectBuildsFromScratch:       testCspNonceInjectBuildsFromScratch,
  testCspNonceMiddlewareSetsReqAndLocals:    testCspNonceMiddlewareSetsReqAndLocals,
  testCspNonceMiddlewareCustomProperty:      testCspNonceMiddlewareCustomProperty,
  testCspNonceMiddlewareSkipsWhenNoCspAndNotAlways: testCspNonceMiddlewareSkipsWhenNoCspAndNotAlways,
  testCspNonceMiddlewareAlwaysCreatesCsp:    testCspNonceMiddlewareAlwaysCreatesCsp,
  testCspNonceMiddlewareNonceLengthDefault:  testCspNonceMiddlewareNonceLengthDefault,
  testCspNonceMiddlewareCustomNonceLength:   testCspNonceMiddlewareCustomNonceLength,
  testCspNonceUniquenessAcrossRequests:      testCspNonceUniquenessAcrossRequests,
  testCspNonceConfigValidation:              testCspNonceConfigValidation,
  testCspNonceLayeredOnSecurityHeaders:      testCspNonceLayeredOnSecurityHeaders,
  testRenderHtmlAutoMergesResLocals:         testRenderHtmlAutoMergesResLocals,
  testRenderHtmlOperatorDataOverridesLocals: testRenderHtmlOperatorDataOverridesLocals,
  ...
  testEventsOffRemovesListener:              testEventsOffRemovesListener,
  testEventsOnceFiresOnceOnly:               testEventsOnceFiresOnceOnly,
  testEventsEmitSwallowsListenerThrows:      testEventsEmitSwallowsListenerThrows,
  testEventsEmitWithNoListenersIsNoop:       testEventsEmitWithNoListenersIsNoop,
