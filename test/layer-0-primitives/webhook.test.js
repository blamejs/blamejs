...
function testStripeHappyPath() {
  ...
  var r = b.webhook.verify({ alg: "hmac-sha256-stripe", secret: secret, header: fx.header, body: body });
  check("stripe happy path → ok=true",       r.ok === true);
  check("stripe scheme v1",                   r.scheme === "v1");
  check("stripe timestamp echoed",            r.timestamp === ts);
}

function testStripeWrongSecret() {
  var body = '{"x":1}';
  var ts   = Math.floor(Date.now() / 1000);
  var fx   = _stripeFixture(body, ts, "whsec_correct_secret");
  var thrown = false;
  try { b.webhook.verify({ alg: "hmac-sha256-stripe", secret: "whsec_wrong_secret", header: fx.header, body: body }); }
...
function testStripeMultipleV1() {
  ...
  var r = b.webhook.verify({ alg: "hmac-sha256-stripe", secret: "whsec_real", header: header, body: body });
  check("stripe v1-rotation accepts good one", r.ok === true);
}

function testStripeMissingFields() {
  var body = '{}';
  var ts   = Math.floor(Date.now() / 1000);
  var fx   = _stripeFixture(body, ts, "whsec_x");
  var thrownT = false, thrownV1 = false;
  try { b.webhook.verify({ alg: "hmac-sha256-stripe", secret: "whsec_x", header: "v1=" + fx.hex, body: body }); }
  catch (_e) { thrownT = true; }
  try { b.webhook.verify({ alg: "hmac-sha256-stripe", secret: "whsec_x", header: "t=" + ts, body: body }); }
  catch (_e) { thrownV1 = true; }
  check("missing t → throws",                 thrownT);
  check("missing v1 → throws",                thrownV1);
}

function testStripePrefixPreserved() {
  ...
  // Verifying a `whsec_test123`-signed message with the same `whsec_test123`
  // secret MUST succeed (i.e., the verifier doesn't internally strip).
  var r = b.webhook.verify({ alg: "hmac-sha256-stripe", secret: "whsec_test123", header: withPrefix.header, body: body });
  check("verify accepts whsec_-prefixed key",  r.ok === true);
}

function testStripeRoundTrip() {
  var body = '{"id":"evt_42"}';
  ...
  var r = b.webhook.verify({ alg: "hmac-sha256-stripe", secret: secret, header: header, body: body });
  check("round-trip verify ok",                r.ok === true);
}

function testStripeOverlongSignature() {
  var body = '{}';
  var ts   = Math.floor(Date.now() / 1000);
  var longHex = "a".repeat(300);
  var thrown = false;
  try { b.webhook.verify({ alg: "hmac-sha256-stripe", secret: "whsec_x", header: "t=" + ts + ",v1=" + longHex, body: body }); }
  catch (_e) { thrown = true; }
  check("overlong v1 hex → throws",            thrown);
}

function testStripeBadAlg() {
  var thrown = false;
  try { b.webhook.verify({ alg: "wrong-alg", secret: "x", header: "t=1,v1=ab", body: "{}" }); }
  catch (e) { thrown = true; check("bad alg → bad-alg code", e.code === "webhook/bad-alg"); }
  check("bad alg → throws",                    thrown);
}

function testStripeNonceReplay() {
  var body = '{"x":1}';
  var ts   = Math.floor(Date.now() / 1000);
  ...
  var r1 = b.webhook.verify({ alg: "hmac-sha256-stripe", secret: secret, header: fx.header, body: body, nonceStore: store });
  check("first call ok",                       r1.ok === true);
  var thrown = false;
  try { b.webhook.verify({ alg: "hmac-sha256-stripe", secret: secret, header: fx.header, body: body, nonceStore: store }); }
...
async function run() {
  ...
  testStripeHappyPath();
  testStripeWrongSecret();
  testStripeTamperedBody();
  testStripeStaleTimestamp();
  testStripeMultipleV1();
  testStripeMissingFields();
  testStripePrefixPreserved();
  testStripeRoundTrip();
  testStripeOverlongSignature();
  testStripeBadAlg();
  testStripeNonceReplay();
}
