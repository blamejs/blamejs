...
"use strict";
/**
 * b.mail.server.managesieve — command-handler + error-branch coverage driven
 * over a real localhost listener (RFC 5804). The sibling
 * mail-server-managesieve.test.js covers create() opts validation; this file
 * drives the wire-protocol dispatch (CAPABILITY / NOOP / STARTTLS / LOGOUT /
 ...
 * backend-failure / rate-limit / idle-timeout branches that the happy-path
 * suite never reaches. Every assertion drives the public API over a socket;
 * the STARTTLS upgrade trusts the test CA via `ca:` (never
 * rejectUnauthorized:false).
 *
 * Run standalone: node test/layer-0-primitives/mail-server-managesieve-coverage.test.js
 */
...
async function _makeTestTlsContext() {
  var ca = await b.mtlsEngine.generateCa({ name: "managesieve-cov-ca" });
...
if (require.main === module) {
  run().then(
    function () { console.log("mail-server-managesieve-coverage OK — " + helpers.getChecks() + " checks"); },
