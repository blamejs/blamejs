  ...
  var KNOWN_CLUSTERS = [
    ...
    },
    {
      ...
      files: [
        ...
        "lib/mail-server-jmap.js:dispatch",
    ...
    },
    {
      ...
      files: [
        ...
        "lib/guard-mail-query.js:validate",
      ...
      reason: "guard-* family `validate(input, opts)` entry — every member's validate() opens with the SAME profile-resolution + posture-mapping + cap-extraction cascade (profile = opts.profile || DEFAULT_PROFILE; if posture → map; lookup PROFILES[profile]; throw if unknown). The cascade pattern is owned by gate-contract.resolveProfileAndPosture for guards that compose it; the SMTP / IMAP / JMAP / mail-* / posture / saga validators each diverge in the per-input shape they validate next, so they keep an inline opener instead of routing through gate-contract.",
    },
    {
      files: [
      ...
      reason: "Recursive Object.keys-walking visitor — each primitive walks a different tree shape (ACME directory profiles, JMAP back-reference resolution per RFC 8620 §3.7, template AST). The 50-token shingle is the for-keys-with-Object.prototype.hasOwnProperty guard pattern; the per-call-site semantics are structurally distinct.",
    },
    {
      files: [
        "lib/mail-server-mx.js:create",
