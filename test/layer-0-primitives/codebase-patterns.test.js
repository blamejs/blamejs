...
async function testNoDuplicateCodeBlocks() {
  ...
  var KNOWN_CLUSTERS = [
    ...
    },
    {
      ...
      reason: "Per-module audit-emit wrapper + the matching `_validateDomainHardened(d, label)` wrapper around `b.guardDomain.validate(...)` + the listener `create(opts)` opt-normalization shell + the `listen(listenOpts)` lifecycle. Each module's audit calls land on a distinct action-namespace (daemon.* / mail.server.mx.* / mail.server.submission.* / mail.server.pop3.* / self-update.*); the domain validators wrap the SAME b.guardDomain.validate but emit to different audit events; the listener create() + listen() entries normalize structurally-different opt shapes. Mirrors the audit-emit-wrapper pattern that lib/agent-audit.js extracted for the agent-substrate modules.",
    },
    {
      files: [
        "lib/mail-server-mx.js:create",
    ...
    },
    {
      ...
      reason: "RFC 5322 / RFC 1939 header-injection boolean scan (CR/LF/NUL/C0/DEL char-code check, allow TAB). Each guard inlines an 8-line for-loop; the shape IS the contract. Future consolidation candidate via `lib/codepoint-class.js` if the family grows past 5 sites, but the per-guard scan keeps the error-class wrapper (GuardDsnError / GuardListIdError / GuardListUnsubscribeError / GuardPop3CommandError / SafeRedirectError) domain-local and the inlined check has zero call overhead.",
    },
    {
