...
var KNOWN_ANTIPATTERNS = [
  ...
  {
    // v0.15.0 — physically deleting audit-chain rows is a
...

  {
    ...
    skipCommentLines: true,
    allowlist: [],
    reason: "v0.15.0 — b.db whereRaw / WhereBuilder.raw accept an operator SQL fragment plus a bound-params array. Concatenating or template-interpolating a value into the fragment defeats the placeholder binding and reintroduces SQL injection. lib/ has zero such call sites today; the detector keeps it that way (the runtime _assertRawNoStringLiteral gate is the operator-facing backstop, this is the framework-internal one). No-argument `.raw()` mountings (e.g. bodyParser.raw()) don't match — an interpolated/concatenated string argument is required.",
  },

  {
    // v0.15.0 — equality-lookup ("derived") hashes for sealed columns
