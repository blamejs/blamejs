...
"use strict";
/**
 * b.mail.server.submission — command-handler + error-branch coverage driven
 * over a real localhost listener. The sibling mail-server-submission.test.js
 * covers create() opts validation + the BDAT/CHUNKING happy path; this file
 * targets the ERROR / DEFENSIVE / ADVERSARIAL branches the happy-path suite
 * never reaches:
 *
 *   - create() defensive throws (bad opts / tenant-scope / dkim-mode)
 *   - wire dispatch under permissive: every verb + wrong-state refusals +
 *     unknown-verb + malformed args + control-char / NUL / bare-LF refusals
 *   - AUTH: cleartext-accepted, mech-not-advertised, already-authenticated,
 *     multi-step challenge (334), verify-returns-fail, verify-throws,
 *     per-IP AUTH-failure rate-limit (421 + close), cross-tenant refusal
 *   - identity-binding (strict): in-set OK, not-in-set 553, no-mailboxes 553
 *   - strict profile: MAIL-before-STARTTLS 530, AUTH-before-STARTTLS 538,
 *     STARTTLS upgrade, STARTTLS-again 503, DKIM-required 550
 *   - implicit-TLS: STARTTLS-not-available 502, no STARTTLS in EHLO
 *   - DKIM-required gate: any / self / mismatch / no-header-block
 *   - recipientPolicy: accept / refuse 550 / throw 451
 *   - per-IP concurrent-connection rate-limit 421
 *   - line-too-long 500, body-too-large 552, DATA smuggling 554
 *   - idle-timeout 421, close() drain + close-before-listen
 *
 * Every assertion drives the public API over a socket; STARTTLS upgrades
 * trust the test CA via `ca:` (never rejectUnauthorized:false).
 *
 * Run standalone: node test/layer-0-primitives/mail-server-submission-coverage.test.js
 */
...
async function _makeTestTlsContext() {
  var ca = await b.mtlsEngine.generateCa({ name: "submission-cov-ca" });
...
function testCreateValidation(tls) {
  ...
  // guardDomain:false + guardDomain object accepted at create (branch coverage).
...
async function run() {
  var tls;
  try { tls = await _makeTestTlsContext(); }
  catch (_e) { check("mail-server-submission-coverage skipped (no TLS ctx)", true); return; }
...
if (require.main === module) {
  run().then(
    function () { console.log("mail-server-submission-coverage OK — " + helpers.getChecks() + " checks"); },
