...
"use strict";
/**
 * b.auth.oauth — network-driven + error/defensive-branch coverage.
 *
 * Supplements auth-oauth-coverage.test.js (offline config/validation
 * branches) and oauth-callback.test.js (network happy paths) by driving the
 * error, wrong-state, backend-failure and adversarial branches that only
 * resolve with a live IdP round-trip:
 *
 *   - verifyIdToken full signature + claim pipeline (exp / iat / nbf / iss /
 *     aud / azp / nonce / kid / kidless / alg-kty / bad-signature), against a
 *     loopback JWKS server and locally-signed ES256 id_tokens.
 *   - exchangeCode / refreshAccessToken / exchangeToken / nativeSsoExchange
 *     token-endpoint flows incl. _normalizeTokens, refresh rotation, and the
 *     RFC 9396 granted-details cross-check.
 *   - fetchUserInfo / revokeToken / introspectToken / registerClient /
 *     read/update/deleteClient / deviceAuthorization / pollDeviceCode /
 *     verifyBackchannelLogoutToken / parseJarmResponse.
 *   - _fetchJson + _postForm backend-failure branches (non-2xx, non-JSON).
 *   - discovery-driven _discover / _resolveEndpoint / parseCallback branches.
 *   - the no-endpoint refusals (checkSessionIframeUrl / endSessionUrl /
 *     pushAuthorizationRequest / introspectToken / deviceAuthorization).
 *
 * Every fetch targets a 127.0.0.1 loopback stand-in reached with
 * allowHttp + allowInternal (never a real endpoint, never
 * rejectUnauthorized:false); id_tokens are signed by a locally-generated
 * P-256 key whose public JWK the loopback /jwks serves.
 */
...
async function _drainTcpHandles() {
  ...
  }, { timeoutMs: 5000, label: "auth-oauth-coverage2: TCP handle drain after _resetForTest" });
}
...
async function run() {
  ...
  console.log("auth-oauth-coverage2 checks passed");
}
