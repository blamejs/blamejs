...
- **Content-safety gates** — `b.gateContract` uniform composition contract (mode posture / hooks / forensic snapshot / decision cache / runtime cap) with `b.guardCsv` shipping the full v1 catalog (formula injection ASCII + full-width prefixes, dangerous-function denylist, bidi / homoglyph / control / null / BOM / zero-width detection, dialect ambiguity, CSV-bombs, numeric precision, schema-bound serializer, OWASP-aligned strict / balanced / permissive / email-attachment profiles, hipaa / pci-dss / gdpr / soc2-cc7 compliance postures). `b.guardAll` is the registry + aggregator: every shipped guard ON by default; opt-out per guard with audited reason via `exceptFor: { name: { reason } }`; `b.guardAll.byExtension({ profile: "strict" })` drops straight into `b.staticServe.create({ contentSafety })` and `b.fileUpload.create({ contentSafety })` so future guards added to the family auto-extend operator deploys without re-wiring.
