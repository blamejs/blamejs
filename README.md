...
- **Mail server listeners** — RFC 5321 MX inbound with connection-level gate cascade (HELO identity / DNS blocklist / greylisting) and a DATA-phase SPF/DKIM/DMARC gate that refuses policy-failing mail before storage (`b.mail.server.mx`), RFC 6409 submission with SASL + identity-binding (`b.mail.server.submission`), RFC 9051 IMAP4rev2 with CONDSTORE / QRESYNC / NOTIFY / METADATA / CATENATE (`b.mail.server.imap`), whose `LIST` reads its reference and pattern and answers the `SUBSCRIBED` selection option and the `SUBSCRIBED` / `CHILDREN` / `STATUS` return options, and whose `CREATE` / `DELETE` / `RENAME` / `SUBSCRIBE` / `UNSUBSCRIBE` answer `NO` until the operator supplies `mailboxAdmin`, the object whose methods perform them, RFC 8620 + RFC 8621 JMAP Core + Mail over HTTP/SSE/WebSocket (`b.mail.server.jmap`), POP3 (`b.mail.server.pop3`), ManageSieve (`b.mail.server.managesieve`). Every listener takes a `maxConnections` ceiling of its own alongside the per-address rate limit, reports `connectionCount()`, and releases a peer's rate-limit slot and its tracking-set entry together when the socket closes; the two SMTP listeners enforce the `minBytesPerSecond` floor on an arriving DATA body
...
- **Mail crypto** — PQC-first S/MIME via CMS (`b.mail.crypto.smime`) + OpenPGP encrypt/decrypt + WKD key discovery with IDN-homograph defense (`b.mail.crypto.pgp`)
- **Mail-stack agent** — multi-threaded worker pool + queue dispatch + sealed mail-store backed by SQLite FTS5, threading scoped to the account that owns the folder (`b.mail.agent`, `b.mailStore`); folders are created, renamed, removed and subscribed to through the store (`createFolder` / `renameFolder` / `deleteFolder` / `subscribeFolder`), and removing one takes its messages, quota rows and index entries with it in one transaction while refusing a folder holding a message under legal hold
...
- **File uploads** — chunked with per-chunk SHA3-512 verification + atomic finalize + tombstone cleanup, with per-actor ownership, quota and concurrency keyed on the actor's tenant alongside its id (`actorKey` overrides the resolution; `b.requestHelpers.actorIdentityKey` is the shared resolver) (`b.fileUpload`)
### AI / agentic
...
- **Backup + restore** — end-to-end-encrypted bundles with pre-flush fail-closed mode + manifests signed with the audit-sign keypair (SLH-DSA-SHAKE-256f by default, ML-DSA-87 and ML-DSA-65 supported) and trusted only under a pinned fingerprint or an active or rotated audit-sign key + retention that deletes older bundles only after the new one reads back intact + scheduled backup-restore drills (`b.backup`, `b.backup.scheduleTest`, `b.backup.verifyManifestSignature`); restore with pulled-bundle footprint preflight, staging beside the data directory, and a rollback point per restore (`b.restore`, `b.restoreRollback`); disaster-recovery runbook generator (HIPAA / PCI-DSS / GDPR / SOC 2 / DORA postures) (`b.drRunbook`)
...
- **Concurrency + kill-switches** — per-resource lock with cooperative-cancel + audit (`b.resourceAccessLock`); composite account-takeover kill-switch (`b.auth.atoKillSwitch`)
...
| [`@blamejs/pki`](https://github.com/blamejs/pki) | 0.8.17 | [blamejs](https://github.com/blamejs) | Zero-dependency pure-JS X.509 / CRL / PKCS#12 / CSR / CMS toolkit backing `b.mtlsCa` — ML-DSA-87 (FIPS 204) post-quantum + ECDSA-P384 cert signing, PBMAC1 PKCS#12 packaging, chain validation (no openssl CLI) — and the WebAuthn attestation / assertion verification behind `b.auth.passkey` |
