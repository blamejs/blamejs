...
- **SQLite with sealed-by-default columns** — `b.db`, migrations, seeders, atomic-file writes; the db handle constructs with SQLITE_LIMIT_* parse-time caps (a >1 MiB statement is rejected, ATTACH is denied) as a DoS floor on the raw-SQL surface
