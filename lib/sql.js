...
function _ddlType(logical, dialect) {
  ...
  // type position, never identifier position - but a config-time shape check
  // refuses a type carrying a statement terminator or comment marker before it
  // reaches the emitted DDL (no legitimate SQL type contains these; spaces,
  // parens, commas, and BALANCED quotes - which ENUM / SET need - are allowed).
  // createTable / alterTable then route the finished statement through
  // _assertCatalogEmittable, which backstops an UNBALANCED quote, a stacked
  // top-level statement, a NUL, and unbalanced parens.
  if (logical.indexOf(";") !== -1 ||
      logical.indexOf("--") !== -1 ||
      logical.indexOf("/*") !== -1 ||
      logical.indexOf("*/") !== -1) {
    throw _err("column type '" + logical + "' has an unsupported shape " +
      "(statement terminator / comment marker)", "sql-builder/bad-type");
  }
  return logical;
}
