...
function create(opts) {
  ...
  // The next complete line, or null when the buffer holds none. Closes the
  // connection when an unterminated line passes the cap, which is the same
  // answer the loop gave before.
  ...
  // One command at a time. A handler that verifies credentials or reaches the
  // store settles before the next line is read, so a guard that reads state a
  // later handler writes sees what it is guarding against.
  //
  // Draining the whole buffer synchronously did not: the AUTHORIZATION-state
  // guards read `state.stage` and `state.actor`, and PASS moves the stage only
  // after its verify resolves, so a client writing `USER a / PASS a / USER b /
  // PASS b` in ONE segment had all four dispatched while the stage was still
  // "authorization" and the actor still null. Both pairs authenticated.
  //
  // `mail-server-mx.js` already chained its ingestion for the same reason;
  // this is that, per line rather than per chunk, because a pipelined client
  // puts every command in one chunk.
  function _drainBuffer(state, socket) {
    ...
    // behind it than the allowance, and that first event takes the
    // not-running branch. Commands queue while a handler runs, a credential
    // check or opening the maildrop, and a client that sends faster than they
    ...
    // post-STLS reader is the shared upgradeLineProtocol helper, which feeds
    // this function directly: a bound placed on the plaintext handler alone
    // would be missing on the encrypted path, which is the one that matters.
  ...
  function _handleLine(state, socket, line) {
    ...
    // Returned so the reader can wait for it. _dispatch already returns each
    // handler's value.
  ...
  function _handlePass(state, socket, args) {
    ...
    _emit("mail.server.pop3.auth_attempt",
      { connectionId: state.id, verb: "PASS", remoteAddress: state.remoteAddress });
    // Returned, not just started: the reader dispatches the next command only
    // after this settles, and the AUTHORIZATION-state guards read state this
    // sets.
  ...
  function _enterTransaction(state, socket, verb) {
    ...
    // Returned, because the stage moves inside this chain rather than before
    // it. A caller that waits only for the credential check is still waiting
    // on the wrong thing: the session is in AUTHORIZATION until the drop
    // opens, and a second command read in that window sees an unauthenticated
    // session and is answered as one.
    //
  ...
  function _close(socket, state) {
    ...
    // continuation resumed and ran whatever the client had already queued, on
    // a connection the server had refused and closed. Clearing the buffer as
    // well so nothing is left to resume with.
    // Its own field, not a `stage` value: `stage` is the protocol position and
    // handlers assign it -- "transaction" once the maildrop opens, "update" on
    // QUIT -- so a close landing while one was in flight was overwritten by the
    ...
    // from the count while the teardown was still holding it open to flush.
