...
function create(opts) {
  ...
  function _drainBuffer(state, socket) {
    ...
    state.pumping = true;
    function step() {
      if (state.stage === "closed") { state.pumping = false; return; }
  ...
  function _close(socket, state) {
    // The reader's `stage === "closed"` guard is only reachable if closing
    // writes the sentinel. Without it a refusal that destroys the socket left
