...
function create(opts) {
  ...
  function _handleConnection(rawSocket) {
    ...
    var accepted = mailServerNet.acceptConnection(rawSocket, {
      ...
      onClose:      function () { if (state) _fireSessionEnd(state); },
    });
  ...
  function _enterTransaction(state, socket, verb) {
    ...
    // Held on the state so a session ending mid-open is reported after the
    // lease it is about to acquire exists — see _fireSessionEnd.
    state.storeWork = Promise.resolve()
  ...
  function _handleQuit(state, socket) {
    ...
    // pending Promises).
    // Held on the state for the same reason as the open: the next session must
    // not be let in while this commit is still mutating the mailbox.
    state.storeWork = safeAsync.withTimeout(
