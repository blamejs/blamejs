 ...
 *     octet count, the size with CRLF line endings, which is what STAT /
 *     LIST advertise and what RETR / TOP deliver: the listener
 *     canonicalizes a body's line endings to CRLF before dot-stuffing
 *     (RFC 1939 §3), so a backend that stores messages with bare-LF
 *     endings reports the CRLF size, not the stored byte length.
