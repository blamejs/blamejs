 ...
 *
 *   Scope: the guard detects DELIMITED template syntax (including Pug
 *   `#{...}` / `!{...}` interpolation), which is rare in ordinary text.
 *   It does not detect the UN-delimited line-level code of an
 *   indentation-based engine (Pug, HAML): a buffered `=` / `!=` line,
 *   an unbuffered `-` line, a control keyword, an attribute expression,
 *   or a mixin call is compiled as code, yet is syntactically
 *   indistinguishable from ordinary prose (a domain, a Markdown link,
 *   an `x = y` assignment), so a marker scan cannot flag it without
 *   refusing that prose. Untrusted input must therefore never be
 *   compiled as a Pug / HAML template, whatever this guard reports.
 *
