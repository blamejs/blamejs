...
function _throw(errorClass, code, msg, defaultCode, permanent) {
  // Both branches resolve the code the same way. They did not: the factory
  // branch ignored the default the CALLER named, so the same failure was
  // reported as a bare BAD_OPT against a class with a factory and as the
  // named default against one without.
...
// The same assertion for a callback whose ABSENCE removes a check rather than
// selecting a default — a resolver that decides whether a purged range is
// still recoverable, say. `null` is the shape a configuration that failed to
// load arrives in, and treating it as "not supplied" is how a requested check
// becomes a silently skipped one, on the exact path where the answer then
// reads as clean. Only `undefined` means absent here.
...
// check(), rethrown as the caller's own error class and code. `check` throws a
// plain Error, so a primitive that reports typed errors otherwise had to wrap
// it, and five of them wrapped it the same way.
...
// For a module whose convention is a native error (TypeError for a shape
// error, say): the wording lives here, the throw stays the caller's.
