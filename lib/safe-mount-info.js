 ...
 */
// mountinfo's path fields are octal-escaped by the kernel: space, tab, newline
// and backslash become \040, \011, \012 and \134 (Documentation/filesystems/
// proc.rst). Those are the four the kernel emits, and it emits nothing else,
// so this decodes exactly them rather than treating any backslash-digit run as
// an escape — a literal `\1` in a path is not one.
//
// Left encoded, these fields do not compare equal to what realpath() returns
// for the same directory, so a path under such a mount matches no entry and
// bestMatch answers with the filesystem BENEATH it. That reads as a confident
// wrong answer rather than as a failure: b.db decides an encrypted database's
// working copy is on disk and refuses a boot on a perfectly good tmpfs, and
// b.watcher's bind-mount detection misses the mount entirely.
// Scanned rather than matched: this file is a safe-* primitive and carries no
// regex, and the scan is the clearer statement anyway — a backslash followed by
// exactly one of four known three-digit codes, and nothing else.
...
function _unescapePath(s) {
  ...
  while (i < s.length) {
    if (s.charCodeAt(i) === BACKSLASH_CODE && i + 4 <= s.length) {
      ...
      // hasOwnProperty, not a bare index: the codes come off a parsed file, and
      // a bare lookup finds inherited names.
...
function parse(text, opts) {
  ...
  for (var i = 0; i < lines.length; i += 1) {
    ...
    out.push({
      ...
      root:         _unescapePath(preFields[3]),                                     // *** field 4 (0-indexed 3) — bind-mount detection
