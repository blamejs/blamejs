...
var vaultWrap = lazyRequire(function () { return require("./vault-wrap"); });
...
function _fsyncFile(p) {
  try {
    var fd = fs.openSync(p, "r+");
    try { fs.fsyncSync(fd); } finally { fs.closeSync(fd); }
  } catch (_e) { /* best-effort */ }
}
function _fsyncDir(d) {
  try {
    var fd = fs.openSync(d, "r");
    try { fs.fsyncSync(fd); } finally { fs.closeSync(fd); }
  } catch (_e) { /* fsync on dir not supported on all platforms */ }
}

function _copyFile(src, dest) { fs.copyFileSync(src, dest); }
...
async function rotate(opts) {
  ...
  for (var vd = 0; vd < paths.verbatimDirs.length; vd++) {
    ...
    _copyDirRecursive(sdir, path.join(stagingDir, dent.relativePath));
  }
  ...
  for (var as = 0; as < paths.additionalSealed.length; as++) {
    ...
    if (!fs.existsSync(asDestDir)) fs.mkdirSync(asDestDir, { recursive: true, mode: 0o700 });
