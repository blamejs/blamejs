...
function roundMinor(minor, step, mode) {
  ...
  var distLow = minor - lower;
  var distHigh = upper - minor;
  if (distLow < distHigh) return lower;
  if (distHigh < distLow) return upper;
  if (mode === "half-up") return (minor >= 0n) ? upper : lower;
  if (mode === "half-down") return (minor >= 0n) ? lower : upper;
  return ((lower / stepBig) % 2n === 0n) ? lower : upper;
}
