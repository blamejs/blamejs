...
var regexLinear  = require("./regex-linear");
...
function _re(source) {
  // Each detector is built from an ASCII source string, so the file embeds no
  // attack-character literals, and compiled on b.regexLinear rather than the
  // platform engine.
  //
  // The subject is SQL this guard was handed to screen — the one string in the
  // process most likely to have been chosen by someone hostile. A Thompson NFA
  // costs the length of that subject whatever the pattern says, so no detector
  // can be turned into a denial of service by the text it is inspecting. The
  // platform engine offers no such bound, and several of these patterns pair a
  // bounded lazy span with a following literal, which is where a backtracking
  // engine does its worst work.
  //
  // Case-insensitive, and global-free: the first match is the finding.
  return regexLinear.compile(source, "i");
}

// `\b` — the word characters a boundary is a boundary between.
function _isWordChar(ch) {
  return ch.length === 1 && (codepointClass.ASCII_ALNUM.indexOf(ch) !== -1 || ch === "_");
}

// True when the next WORD in `tail` is one of `words`, and the word ends there —
// what `STDIN\b` asks. `STDINX` is not STDIN.
//
// Leading whitespace is skipped here rather than left to the pattern's `\s+`, so
// the answer does not depend on how much of a run the engine consumed. The
// lookahead this replaces did depend on it, and that is what made it wrong:
// `\s+` is greedy but backtracks, so with two spaces it could give one back,
// leave " STDIN" ahead of the lookahead, and report a client-side
// `COPY t TO  STDIN` as a server-side file access. One space was quiet, two
// fired.
function _startsWithExcludedWord(tail, words) {
  var start = 0;
  while (start < tail.length &&
         codepointClass.inRanges(tail.charCodeAt(start), codepointClass.WHITESPACE_RANGES)) {
    start += 1;
  }
  var rest = tail.slice(start);
  for (var i = 0; i < words.length; i += 1) {
    var w = words[i];
    if (rest.length < w.length) continue;
    if (rest.slice(0, w.length).toUpperCase() !== w) continue;
    if (!_isWordChar(rest.charAt(w.length))) return true;
  }
  return false;
}

// A detector carrying `exclude` fires when its pattern matches and the word
// FOLLOWING the match is not in the excluded set. It replaces a negative
// lookahead, which a linear-time engine cannot run — and the search has to
// continue past an excluded hit rather than stop, because the pattern it
// replaces would backtrack and find a later one. `COPY a TO STDOUT` is safe;
// `COPY a TO STDOUT; COPY b TO '/etc/x'` is not, and only the second `TO`
// says so.
function _matchesOutsideExclusions(compiled, subject, words) {
  var from = 0;
  for (;;) {
    var m = compiled.exec(subject, from);
    if (m === null) return false;
    if (!_startsWithExcludedWord(subject.slice(m.index + m[0].length), words)) return true;
    from = m.index + 1;
  }
}
...
var DETECTORS = [
  ...
  { code: "sql.file-access", severity: "critical", kind: "copy-file",
    family: "floor", dialect: "postgres",
    re: _re("\\bCOPY\\b[\\s\\S]{0,4000}?\\b(?:TO|FROM)\\b\\s+"),
    // Replaces the `(?!STDIN\b|STDOUT\b)` this pattern used to carry: those two
    // stream to the client rather than touching a server-side file, so they are
    // the safe forms and only they are excluded.
...
function _runDetectors(normalized, profile) {
  ...
  for (var i = 0; i < DETECTORS.length; i += 1) {
    var det = DETECTORS[i];
    var hit = det.exclude
      ? _matchesOutsideExclusions(det.re, normalized, det.exclude)
      : det.re.test(normalized);
    if (hit) {
