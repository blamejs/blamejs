...
function _timeoutFloorBreach(row, nowMs, opts) {
  ...
  if (idleMs > 0) {
    var lastActivity = numericBounds.finiteTimestamp(row.lastActivity);
    if ((nowMs - lastActivity) > idleMs) {
      return { action: "auth.session.expired_idle", metadata: { idleMs: nowMs - lastActivity, threshold: idleMs } };
    }
  }
  if (absMs > 0) {
    var createdAt = numericBounds.finiteTimestamp(row.createdAt);
...
async function verify(token, verifyOpts) {
  ...
  if (!row) return null;
  var nowMs = Date.now();
  if (numericBounds.finiteTimestamp(row.expiresAt) < nowMs) {
    if (cluster.isLeader()) {
      try { await _deleteBySidHash(sidHash); } catch (_e) { /* best-effort */ }
    }
    return null;
