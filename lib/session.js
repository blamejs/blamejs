...
async function create(opts) {
  ...
  // Reserved keys are never taken from caller-supplied payload data, on any of
  // the three writers that build this column (create / updateData / rotate).
  // A session created already carrying a device binding — or a fingerprint,
  // when no req was passed to overwrite it — would be accepted by strict
  // verification for a device that never bound.
  if (dataObj) {
...
async function rotate(oldToken, opts) {
  ...
  // The device binding b.sessionDeviceBinding keeps here under storeInSession.
  // Unlike __bj_fingerprint it is NOT sid-keyed, so it carries across a
  // rotation verbatim — and it has to, because rotate with replacement data
  // otherwise drops it and the next verification answers missing-bind. Login
  // is a rotation, so that is the moment a bound session would lose its
  // binding.
  ...
  if (opts.data !== undefined || wasBound || carriedDeviceBinding !== null) {
    ...
    // Reserved on the way IN as well, exactly as updateData treats it. rotate
    // is public and its `data` REPLACES the payload, so honouring a
    // caller-supplied binding here would hand back the same bypass the payload
    // path no longer allows — clear your binding, or install one computed from
    // the public fingerprint helper, by rotating. The stored value always wins;
    // b.sessionDeviceBinding writes it through the framework's own writer.
...
// The device binding is framework-managed state that decides whether a session
// is accepted, so the payload path must not be able to write it — the same
// reason __bj_fingerprint has always been stripped there. Left writable, an
// application could null its own binding, or install one it computed from the
// public fingerprint helper, and strict verification would agree. The accident
// is likelier than the abuse: one updateData carrying a key of that name from
// an ordinary payload destroys the binding silently.
//
// So b.sessionDeviceBinding writes it HERE instead. Underscore-prefixed and
// undocumented: it is the framework's own channel, not operator surface.
...
async function _updateData(token, data, opts, allowReservedBinding) {
  ...
  var existing = null;
  var storedFingerprint = null;
  // The device binding written by b.sessionDeviceBinding under storeInSession.
  // Carried across a REPLACE the same way the sid-keyed fingerprint is: it is
  // framework state living in the operator's payload, and an ordinary
  // updateData — a cart write, a preference flip — would otherwise erase it,
  // after which every strict verification answers missing-bind and the
  // operator's sessions stop working for a reason nothing reports.
  ...
  if (opts.merge === true && existing && typeof existing === "object") {
    ...
    if (data && typeof data === "object") {
      Object.keys(data).forEach(function (k) {
        ...
        // Reserved the same way, and only reachable through _setDeviceBinding.
  ...
  // Clearing the payload with updateData(token, null) is a supported operation
  // and clears the OPERATOR's data — not the framework state living beside it.
  // Both restores were guarded on `next` being an object, so a null replace
  // dropped the device binding and the sid-keyed fingerprint alike, and every
  // later strict check answered missing-bind for a session the caller had only
  // meant to empty. An object is created to hold them when there is something
  // to keep, and `next` stays null when there is not.
  // The stored binding carries forward unless THIS call is the framework's own
  // binding write. A payload write can no longer name the key at all, so the
  // only way it changes is through _setDeviceBinding.
  ...
  // Restored only when this call did not set it, so b.sessionDeviceBinding can
  // still bind and unbind through the same primitive every other writer uses.
...
function _releaseStore(previous) {
  ...
  try {
    ...
    // completed.
    // The store is consumer-supplied, so its close() may return a hand-written
    // thenable rather than a promise. containRejection reads the then-getter
    // inside a guard and hands it a CALLABLE fulfillment handler; attaching
    // here with a null one does neither, and a hand-written thenable calls
    // what it is handed. The close is reported as started rather than
...
module.exports = {
  ...
  // Framework-internal: the only writer for the reserved device-binding key.
  // b.sessionDeviceBinding uses it; nothing else should.
