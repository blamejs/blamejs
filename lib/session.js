...
// where not set). The fieldCrypto.sealRow call seals userId/data and
// produces userIdHash from userId.
function _sealForInsert(row) {
  var sealed = fieldCrypto.sealRow("_blamejs_sessions", row);
...
async function verify(token) {
  ...
  // Unseal sealed columns (userId, data) using the fieldCrypto pipeline
  // so we return cleartext to the caller — same shape as the previous
  // db().from(...).first() path delivered.
  var unsealed = fieldCrypto.unsealRow("_blamejs_sessions", row);
  var data = null;
  if (unsealed.data) {
    try { data = jsonSafe.parse(unsealed.data); } catch (_e) { data = null; }
  }
  return {
...
async function destroyAllForUser(userId) {
  ...
  var lookup = fieldCrypto.lookupHash("_blamejs_sessions", "userId", userId);
  if (!lookup) {
...
function _resetForTest() { /* no module state to reset; clusterStorage and fieldCrypto own theirs */ }
