...
async function logout(res, token, opts) {
  ...
  // RFC 9527 Clear-Site-Data — wipe the browser's cookies / storage / cache so
  // no client-side trace survives logout (throws on an unknown directive).
  res.setHeader("Clear-Site-Data", csd.headerValue(types, "b.session.logout"));
  // Expire the session cookie — belt-and-suspenders with the "cookies"
  // directive, and effective even if the client ignores Clear-Site-Data.
  res.setHeader("Set-Cookie",
    cookieName + "=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0");
  return await destroy(token);
}
