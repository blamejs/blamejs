...
var DANGEROUS_TAG_SUFFIXES = Object.freeze(DANGEROUS_TAG_PREFIXES.map(function (p) {
  return p.slice(2);
}));

var CORE_TAG_URI = "tag:yaml.org,2002:";
...
var SAFE_CORE_SUFFIXES = Object.freeze(SAFE_CORE_TAGS.map(function (t) {
  return t.slice(2);
}));
...
function _hexVal(ch) {
  ...
  if (ch >= "a" && ch <= "f") return ch.charCodeAt(0) - 0x57;
  if (ch >= "A" && ch <= "F") return ch.charCodeAt(0) - 0x37;
  return -1;
}

function _percentDecode(str) {
  if (str.indexOf("%") === -1) return str;
  var out = "";
  var i = 0;
  while (i < str.length) {
    if (str.charAt(i) === "%") {
      var hi = _hexVal(str.charAt(i + 1));
      var lo = _hexVal(str.charAt(i + 2));
      if (hi !== -1 && lo !== -1) {
...
function _resolvedTagSuffix(tag) {
  if (tag.charAt(0) === "!" && tag.charAt(1) === "<" && tag.charAt(tag.length - 1) === ">") {
    var uri = _percentDecode(tag.slice(2, -1));
    return uri.indexOf(CORE_TAG_URI) === 0 ? uri.slice(CORE_TAG_URI.length) : null;
  }
  if (tag.charAt(0) === "!" && tag.charAt(1) === "!") {
    return _percentDecode(tag.slice(2));
  }
  return null;
}

function _isDangerousTag(tag) {
  var suffix = _resolvedTagSuffix(tag);
  if (suffix === null) return false;
  for (var i = 0; i < DANGEROUS_TAG_SUFFIXES.length; i += 1) {
    if (suffix.indexOf(DANGEROUS_TAG_SUFFIXES[i]) === 0) return true;
  }
  return false;
}
...
function _isSafeCoreTag(tag) {
  var suffix = _resolvedTagSuffix(tag);
  return suffix !== null && SAFE_CORE_SUFFIXES.indexOf(suffix) !== -1;
}

var TAG_TAIL = codepointClass.ASCII_ALNUM + "_./:-%";

function _isTagSuffixStart(cc) {
  return codepointClass.isAsciiLetter(cc) || codepointClass.isAsciiDigit(cc) ||
         cc === 0x25;
}
...
function _scanTags(text) {
  ...
  for (var i = 0; i < text.length; i += 1) {
    ...
    var end;
    if (text.charAt(nameAt) === "<") {
      if (noCloser) continue;
      end = text.indexOf(">", nameAt + 1);
      if (end === -1) { noCloser = true; continue; }
      end += 1;
    } else {
      if (!_isTagSuffixStart(text.charCodeAt(nameAt))) continue;
      end = nameAt + 1;
...
function _detectIssues(input, opts) {
  ...
  codepointClass.detectCharThreats(input, opts, "yaml").forEach(report);
