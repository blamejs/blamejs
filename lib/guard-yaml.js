...
function _detectIssues(input, opts) {
  ...
  var anchors = _collectSigilNames(masked, "&", ANCHOR_LEAD_CHARS);
  var aliases = _collectSigilNames(masked, "*", ALIAS_LEAD_CHARS);
...
function _detectDuplicateKeysYaml(text) {
  ...
  for (var i = 0; i < lines.length; i += 1) {
    ...
    if (item && scopeAt > item.dash) {
      if (item.keyIndent === -1) item.keyIndent = scopeAt;
