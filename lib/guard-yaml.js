...
function _isAsciiLetter(cc) {
  return (cc >= 0x41 && cc <= 0x5A) || (cc >= 0x61 && cc <= 0x7A);
}
function _isAsciiDigit(cc) { return cc >= 0x30 && cc <= 0x39; }
function _isNameStart(cc) { return _isAsciiLetter(cc) || cc === 0x5F; }   // "_"
function _isNameChar(cc) {
  return _isAsciiLetter(cc) || _isAsciiDigit(cc) || cc === 0x5F || cc === 0x2D;
}
function _isSpace(cc) {
  return codepointClass.inRanges(cc, codepointClass.WHITESPACE_RANGES);
}
// The word boundary a `\b` supplies: the token cannot run into a further
// identifier character, or `nope` reads as `no`.
function _isWordChar(cc) {
  return _isAsciiLetter(cc) || _isAsciiDigit(cc) || cc === 0x5F;
}
...
function _countDocumentSeparators(text) {
  var n = 0;
  for (var i = 0; i + 3 < text.length; i += 1) {
    if (i > 0 && text.charCodeAt(i) !== 0x0A) continue;
    var at = i === 0 ? 0 : i + 1;
