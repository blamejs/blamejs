...
function _resolveBackend(opts) {
  ...
  if (requested && typeof requested === "object") {
    validateOpts.requireMethods(requested, ["publishRemote", "start", "stop"],
      "pubsub: custom backend", PubsubError, "pubsub/bad-backend", true);
  ...
  throw _err("pubsub/unknown-backend",
...
function create(opts) {
  ...
  function subscribe(channel, handler) {
    if (closed) throw _err("pubsub/closed", "pubsub.subscribe: instance closed");
    if (typeof channel !== "string" || channel.length === 0) {
      throw _err("pubsub/bad-opt", "pubsub.subscribe: channel must be a non-empty string");
    }
    if (typeof handler !== "function") {
      throw _err("pubsub/bad-opt", "pubsub.subscribe: handler must be a function");
    }
  ...
  function subscribePattern(pattern, handler) {
    if (closed) throw _err("pubsub/closed", "pubsub.subscribePattern: instance closed");
    if (typeof pattern !== "string" || pattern.length === 0) {
      throw _err("pubsub/bad-opt", "pubsub.subscribePattern: pattern must be a non-empty string");
    }
    if (typeof handler !== "function") {
      throw _err("pubsub/bad-opt", "pubsub.subscribePattern: handler must be a function");
    }
  ...
  async function publish(channel, payload) {
    if (closed) throw _err("pubsub/closed", "pubsub.publish: instance closed");
    if (typeof channel !== "string" || channel.length === 0) {
      throw _err("pubsub/bad-opt", "pubsub.publish: channel must be a non-empty string");
    }
