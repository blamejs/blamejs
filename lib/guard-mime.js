...
// Parameter token (RFC 9110 §8.3.1): tchar set per RFC 9110 §5.6.2.
var _isParamToken = safeBuffer.isHttpToken;

// Quoted-string body (between double quotes) per RFC 9110 §5.6.2 — HTAB and
        ...
        if (!_isQuotedStringBody(inner)) {
          issues.push({
            ...
            snippet: "parameter `" + pname + "` quoted-string contains " +
                     "non-printable bytes (RFC 9110 §5.6.2)",
          });
        }
