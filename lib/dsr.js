...
function dbTicketStore(opts) {
  ...
  return {
    ...
    get: async function (id) {
      ...
      return safeJson.parse(_unsealPayload(rows[0].payload, rows[0].id));
    },
    list: async function (filter) {
      filter = filter || {};
      ...
      return rows.map(function (r) { return safeJson.parse(_unsealPayload(r.payload, r.id)); });
    },
    update: async function (id, ticket) {
    ...
    purgeExpired: async function (asOfMs) {
      ...
      for (var i = 0; i < rows.length; i++) {
        try {
          var t = safeJson.parse(_unsealPayload(rows[i].payload, rows[i].id));
