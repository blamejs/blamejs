  ...
  function ensureSchema() {
    ...
    if (vault().isInitialized()) {
      ...
      for (var bi = 0; bi < (legacyRows || []).length; bi++) {
        ...
        // A legacy vault-less row may hold a plaintext payload up to the full
        // read ceiling. Sealing it expands it past b.sql's per-value ceiling,
        // so it cannot be re-sealed into the vaulted store. Leave it as-is
        // (it was already plaintext before the vault; the other rows still
        // migrate) and surface it rather than crashing provisioning with a
        // SqlBuilderError. The operator reduces / chunk-migrates it, then a
        // later boot re-seals it.
        if (Buffer.byteLength(String(lrow.payload == null ? "" : lrow.payload), "utf8") > VAULTED_SEAL_SAFE_MAX_BYTES) {
          try {
            observability().safeEvent("dsr.backfill.skipped_oversized", 1, { table: tableRaw });
