...
var POSTURE_DEADLINE_MS = Object.freeze({
  "gdpr":      C.TIME.days(30),
  "uk-gdpr":   C.TIME.days(30),
  "ccpa":      C.TIME.days(45),
  "lgpd-br":   C.TIME.days(15),
  "pipeda-ca": C.TIME.days(30),
  "appi-jp":   C.TIME.days(30),
  "pdpa-sg":   C.TIME.days(30),
  "pipl-cn":   C.TIME.days(15),
...
function dbTicketStore(opts) {
  ...
  try {
    qTable     = safeSql.quoteIdentifier(tableRaw, "sqlite", { allowReserved: true });
  ...
  function ensureSchema() {
    ...
    if (vault().isInitialized()) {
      _ensureDsrSealTable();
      // allow:hand-rolled-sql — a static, param-free legacy-detection filter that b.sql's structured where() cannot express as one OR-of-ANDs group
