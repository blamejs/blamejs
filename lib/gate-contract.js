 ...
 */
// Two namespaces meet on this function's error code, and the separator tells
// them apart. A CONTENT refusal carries the ruleId of the issue that caused it
// (`filename.traversal`, `archive.absolute-path`) — rule ids are dotted, and
// the `<prefix>.refused` fallback below matches them because it stands in for
// one. Everything the framework itself raises about configuration or a
// post-condition — bad-profile, bad-posture, bad-opt, bad-input, bad-spec,
// sanitize-amplified — is `<namespace>/<code>`, the convention the other ~5,300
// codes in the framework use.
