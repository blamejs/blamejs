 ...
 */
// The opts where zero is a documented VALUE rather than a malformed one.
// maxRuntimeMs is read as `if (maxRuntimeMs > 0)`, so zero selects "no runtime
// budget" and is the supported way to turn the cap off; requiring a positive
// integer across the board would take that setting away from operators who
// already use it. These are checked as non-negative instead - a string, an
// Infinity, a fraction and a negative are still refused, because those make
// the comparison meaningless rather than expressing a choice. Everything else
// stays strictly positive: for a `measured > cap` byte or depth limit, zero
// refuses all input, which is a strict setting rather than an uncapped one,
// and relaxing those would loosen guards that already refuse it today.
var ZERO_MEANS_UNCAPPED = ["maxRuntimeMs"];
...
function resolveProfileAndPosture(opts, cfg) {
  ...
  var resolved = Object.assign({}, cfg.defaults || {}, overlay, opts);
  ...
  //
  // The list of caps to check is DERIVED from the defaults, not hand-declared.
  // The hand-maintained `intOpts` array drifted from the DEFAULTS it was meant
  // to mirror: maxRuntimeMs ships in every guard's defaults and was named in
  // none of them, so `{ maxRuntimeMs: "5s" }` disabled the runtime budget on
  // all 27 - and guard-csv's four caps, guard-sql's maxBytes and guard-svg's
  // two were unlisted the same way. A key whose DEFAULT is a positive finite
  // integer must stay one when an operator overrides it; deriving that says so
  // once instead of asking 27 files to remember. `intOpts` still contributes,
  // for an opt that has no default to derive from.
  var toCheck = _positiveIntKeys(cfg.defaults);
  if (Array.isArray(cfg.intOpts)) {
    cfg.intOpts.forEach(function (k) {
      if (toCheck.indexOf(k) === -1) toCheck.push(k);
    });
  }
  toCheck = toCheck.filter(function (k) { return ZERO_MEANS_UNCAPPED.indexOf(k) === -1; });
  if (toCheck.length > 0) {
    numericBounds.requireAllPositiveFiniteIntIfPresent(resolved, toCheck,
      prefix + ".resolveOpts", ErrorClass, prefix + ".bad-opt");
  }
  ZERO_MEANS_UNCAPPED.forEach(function (k) {
...
// The keys of `defaults` whose default value is a positive finite integer -
// the caps, budgets and depth bounds an operator may raise or lower but must
// not replace with a shape that silently compares false against everything.
function _positiveIntKeys(defaults) {
  var out = [];
...
function defineGuard(spec) {
  ...
  var _resolveGuardOpts = function (o) {
    return resolveProfileAndPosture(o || {}, {
      ...
      // Declared here so the caps bind on EVERY path that resolves opts, not
      // just the generated validate() below.
