...
function _deepFreezePolicy(value, depth) {
  if (value === null || typeof value !== "object") return value;
  if (depth > MAX_POLICY_FREEZE_DEPTH) return value;
  if (!Object.isFrozen(value)) Object.freeze(value);
  var keys = Object.keys(value);
  for (var i = 0; i < keys.length; i += 1) _deepFreezePolicy(value[keys[i]], depth + 1);
  return value;
}

function _postureProfileOrThrow(posture, postures, ErrorClass, codePrefix, label) {
  var known = typeof posture === "string" && postures &&
    Object.prototype.hasOwnProperty.call(postures, posture) && postures[posture];
  if (!known) {
    throw ErrorClass.factory(codePrefix + "/bad-posture",
      label + ": unknown compliance posture " + JSON.stringify(posture) +
      (postures ? " (use " + Object.keys(postures).join(" / ") + ")" : ""));
  }
...
function makeProfileResolver(cfg) {
  ...
  return function resolveProfile(opts) {
    opts = opts || {};
    if (opts.compliancePosture !== undefined && opts.compliancePosture !== null) {
      throw ErrorClass.factory(codePrefix + "/bad-opt",
        label + ": option \"compliancePosture\" is not read here; this guard " +
        "takes the compliance regime as \"posture\"");
    }
    if (opts.posture !== undefined && opts.posture !== null && opts.posture !== "") {
      var pn = _postureProfileOrThrow(opts.posture, postures, ErrorClass, codePrefix, label);
...
function resolveProfileName(opts, postures, defaultProfile) {
  opts = opts || {};
  var postureProfile = null;
  if (opts.posture !== undefined && opts.posture !== null && opts.posture !== "") {
    postureProfile = _postureProfileOrThrow(opts.posture, postures,
      GateContractError, "gate-contract", "gateContract.resolveProfileName");
  }
 ...
 */
/**
 * @primitive  b.gateContract.freezePolicy
 * @signature  b.gateContract.freezePolicy(policy)
 * @since      0.20.32
 * @status     stable
 * @related    b.gateContract.defineGuard, b.gateContract.defineParser
 *
 * Freeze a profile table, defaults record or posture overlay through every
 * level and return it. `Object.freeze` is shallow, so a frozen `PROFILES` map
 * still holds writable profile objects, and those objects are what a guard
 * reads when it validates: a consumer could raise a cap for the whole process
 * by assigning to one. `defineGuard` and `defineParser` apply this to what
 * they are given; a namespace that exports its own `PROFILES` calls it.
 *
 * @example
 *   module.exports = { PROFILES: b.gateContract.freezePolicy(PROFILES) };
 */
function freezePolicy(policy) {
  return _deepFreezePolicy(policy, 0);
}
 ...
 * behavior does the rest: it throws on / leaves a reject-class finding so the
...
function _withCharPolicyEnums(defaults, spec) {
  ...
  Object.keys(derived).forEach(function (key) {
    if (declared && declared[key]) return;
    out[key] = derived[key];
  });
  return out;
}
 ...
 */
/**
 * @primitive  b.gateContract.makeIssueReporter
 * @signature  b.gateContract.makeIssueReporter(issues, maxPerKind?)
 * @since      0.20.1
 * @status     stable
 * @related    b.gateContract.aggregateIssues, b.gateContract.runIssueValidator
 *
 * Returns a function that appends an issue to `issues` until `maxPerKind`
 * issues of that `kind` at that `severity` have been appended, and drops
 * further ones of that kind and severity. A guard that reports one issue per
 * offending attribute or element otherwise allocates one object per offense,
 * so a body inside the size cap can carry hundreds of thousands of them. The
 * verdict is decided by the first issue of each kind and severity, and the
 * rest are memory; a kind reported as a warning per instance and as critical
 * for one instance keeps that critical one. `maxPerKind` defaults to
 * `MAX_ISSUES_PER_KIND`.
 *
 * @example
 *   var issues = [];
 *   var report = b.gateContract.makeIssueReporter(issues);
 *   for (var i = 0; i < 100000; i += 1) {
 *     report({ kind: "event-handler", severity: "critical", ruleId: "x.on" });
 *   }
 *   // issues.length === b.gateContract.MAX_ISSUES_PER_KIND
 */
var MAX_ISSUES_PER_KIND = 32;

function makeIssueReporter(issues, maxPerKind) {
  var cap = maxPerKind === undefined ? MAX_ISSUES_PER_KIND : maxPerKind;
  if (typeof cap !== "number" || !isFinite(cap) || cap < 1) {
    throw new TypeError("makeIssueReporter: maxPerKind must be a positive number");
  }
  var reported = Object.create(null);
  return function report(issue) {
    var kind = reported[issue.kind];
    if (kind === undefined) kind = reported[issue.kind] = Object.create(null);
    var severity = String(issue.severity);
    var n = kind[severity] || 0;
    if (n >= cap) return;
    kind[severity] = n + 1;
    issues.push(issue);
  };
 ...
 * postures. A resolved value named in `intOpts` must be a positive
 * finite integer, one named in `nonNegativeOpts` a non-negative finite
 * integer, and one named in `positiveNumberOpts` a positive finite
 * number, with fractions allowed; a name in `positiveNumberOpts` is
 * exempt from the integer checks. Each violation throws
 * `... + "/bad-opt"`.
 *
 * @opts
 ...
 *   intOpts:            string[],      // positive finite integers
 *   nonNegativeOpts:    string[],      // non-negative finite integers
 *   positiveNumberOpts: string[],      // positive finite numbers, fractions allowed
 *
 * @example
 *   var PROFILES = {
 *     strict:   { formulaInjectionPolicy: "reject",     bidiCharPolicy: "reject" },
...
function resolveProfileAndPosture(opts, cfg) {
  ...
  if (cfg.retiredOpts && typeof opts === "object") {
    var carriers = [opts];
    if (opts.profile && typeof opts.profile === "object") carriers.push(opts.profile);
    for (var ci = 0; ci < carriers.length; ci += 1) {
      var carrier = carriers[ci];
      var retired = Object.keys(cfg.retiredOpts).filter(function (k) {
        return Object.prototype.hasOwnProperty.call(carrier, k);
      });
      if (retired.length > 0) {
        throw ErrorClass.factory(prefix + "/bad-opt",
          "option " + JSON.stringify(retired[0]) + " is no longer read: " +
          cfg.retiredOpts[retired[0]]);
      }
    }
  }
  var overlay = {};
  ...
  if (opts.posture !== undefined && opts.posture !== null) {
    throw ErrorClass.factory(prefix + "/bad-opt",
      "option \"posture\" is not read here; this guard takes the compliance " +
      "regime as \"compliancePosture\"");
  }
  ...
  var numberOpts = Array.isArray(cfg.positiveNumberOpts) ? cfg.positiveNumberOpts : [];
  numberOpts.forEach(function (k) {
    numericBounds.requirePositiveFiniteNumberIfPresent(resolved[k],
      prefix + ".resolveOpts " + k, ErrorClass, prefix + "/bad-opt");
  });
  if (Array.isArray(cfg.intOpts) && cfg.intOpts.length > 0) {
    numericBounds.requireAllPositiveFiniteIntIfPresent(resolved, cfg.intOpts.filter(function (k) {
      return numberOpts.indexOf(k) === -1;
  ...
  if (Array.isArray(cfg.nonNegativeOpts)) {
    cfg.nonNegativeOpts.forEach(function (k) {
      ...
      if (numberOpts.indexOf(k) !== -1) return;
...
function _ctxValueForKind(kind, ctx, override) {
  ...
  if (!fields) return extractBytesAsText(ctx);
  var emptyPresent;
  var sawEmpty = false;
 ...
 *   sanitizeTransform:    function,   // (subject, resolvedOpts) -> cleaned; the factory wraps it with the refusal and cap checks
 *   sanitizeAmplificationCap: string, // name of the ratio option the generated sanitize measures its output against; a positive finite number, fractions allowed
 *   intOpts:              string[],   // options that must be positive finite integers
 *   positiveNumberOpts:   string[],   // options that must be positive finite numbers, fractions allowed (the amplification cap is one)
...
function defineGuard(spec) {
  ...
  var profiles = _deepFreezePolicy(spec.profiles, 0);
  ...
  var positiveNumberOpts = Array.isArray(spec.positiveNumberOpts) ? spec.positiveNumberOpts.slice() : [];
  if (typeof spec.sanitizeAmplificationCap === "string" &&
      positiveNumberOpts.indexOf(spec.sanitizeAmplificationCap) === -1) {
    positiveNumberOpts.push(spec.sanitizeAmplificationCap);
  }
  var _resolveGuardOpts = function (o) {
    return resolveProfileAndPosture(o || {}, {
      ...
      positiveNumberOpts: positiveNumberOpts,
      enumOpts:           _withCharPolicyEnums(defaults, spec),
      retiredOpts:        spec.retiredOpts || null,
    });
  };
  ...
  var out = {
    ...
    POSITIVE_NUMBER_OPTS: Object.freeze(positiveNumberOpts.slice()),
    ...
    PROFILES:            _deepFreezePolicy(profiles, 0),
    DEFAULTS:            _deepFreezePolicy(defaults, 0),
...
function defineParser(spec) {
  ...
  var out = {
    compliancePosture:   compliancePostureFn,
    PROFILES:            _deepFreezePolicy(spec.profiles, 0),
    COMPLIANCE_POSTURES: postures,
  };
...
module.exports = {
  ...
  makeIssueReporter:  makeIssueReporter,
  MAX_ISSUES_PER_KIND: MAX_ISSUES_PER_KIND,
