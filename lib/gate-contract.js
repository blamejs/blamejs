...
function _withCharPolicyEnums(defaults, spec) {
  ...
  var out = null;
  var canRepair = typeof spec.sanitize === "function" && spec.kind !== "entries";
  for (var i = 0; i < CHAR_POLICY_KEYS.length; i += 1) {
    var key = CHAR_POLICY_KEYS[i];
    if (typeof defaults[key] !== "string") continue;      // this guard has no such policy
    if (declared && declared[key]) continue;              // an explicit declaration wins
    if (!out) out = declared ? Object.assign({}, declared) : {};
    out[key] = canRepair ? CHAR_POLICY_BASE.concat(["strip"]) : CHAR_POLICY_BASE.slice();
  }
  return out || declared || null;
}
