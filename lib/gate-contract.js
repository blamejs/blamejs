 ...
 * issues of that `kind` have been appended, and drops further ones of that
 * kind. A guard that reports one issue per offending attribute or element
 * otherwise allocates one object per offense, so a body inside the size cap
 * can carry hundreds of thousands of them. The verdict is decided by the
 * first, and the rest are memory. `maxPerKind` defaults to
 * `MAX_ISSUES_PER_KIND`.
 *
 * @example
 *   var issues = [];
...
function makeIssueReporter(issues, maxPerKind) {
  ...
  return function report(issue) {
    var n = reported[issue.kind] || 0;
    if (n >= cap) return;
    reported[issue.kind] = n + 1;
    issues.push(issue);
  };
}
