...
function resolveProfileAndPosture(opts, cfg) {
  ...
  //
  // Only the caps the CALLER declares. This is a general overlay helper, so it
  // must not read meaning into a value's shape: a consumer whose defaults hold
  // `{ retries: 3, weight: 1 }` is entitled to override them with 0 and 0.5,
  // and inferring "positive integer default therefore positive integer cap"
  // took that away. defineGuard knows its own defaults ARE caps and derives the
  // list there, which is where that knowledge lives.
  var toCheck = Array.isArray(cfg.intOpts) ? cfg.intOpts.slice() : [];
  ...
  if (Array.isArray(cfg.intOpts)) {
    ZERO_MEANS_UNCAPPED.forEach(function (k) {
      if (cfg.intOpts.indexOf(k) === -1 || resolved[k] === undefined) return;
...
function defineGuard(spec) {
  ...
  var _resolveGuardOpts = function (o) {
    return resolveProfileAndPosture(o || {}, {
      ...
      // Derived from the guard's OWN defaults, unioned with anything the spec
      // names explicitly. A guard's positive-integer defaults are caps by
      // construction, which is knowledge this layer has and the general
      // resolver above does not. Deriving rather than hand-listing is what
      // stops the drift that left maxRuntimeMs unchecked in all 27 guards,
      // guard-csv with no list at all, and guard-sql's and guard-svg's caps
      // unnamed. Declared here so the caps bind on EVERY path that resolves
      // opts, not just the generated validate() below.
      intOpts:            _capKeys(defaults, spec.intOpts),
