...
function seal(plaintext) {
  ...
  _requireInit();
  var sealed = VAULT_PREFIX + encrypt(String(plaintext), keys);
  // metrics tap — no-op when no registry is active. Routes into
  // framework_vault_seal_total when an operator has wired b.metrics.create().
  try { require("../metrics").tap("vault.seal", 1); } catch (_e) { /* boot-order tolerance */ }
  return sealed;
}
...
function unseal(value) {
  ...
  var plain = decrypt(payload, keys);
  try { require("../metrics").tap("vault.unseal", 1); } catch (_e) { /* boot-order tolerance */ }
  return plain;
}
