  ...
  function _resealNow(actor) {
    ...
    try {
      ...
      try {
        ...
        // codeql[js/file-system-race] lstat-then-open is intentional and
        // hardened: the lstat (line 277) only enforces the symlink-refuse
        // policy; the inode equality + size re-check below (line 290) is
        // the TOCTOU defense — if the source is swapped between lstat and
        // open, ino or size disagree and we throw seal-pem-file/toctou-detected.
