...
// the original write fd around. O_NOFOLLOW refuses to fsync through a
// symlink swapped in for the staged file (CWE-59); the constant is
// absent on Windows (where a symlink swap inside an owner-only dir is
// not the threat model), so it degrades to 0 there.
function _fsyncFileByPath(p) {
  try {
    var fd = nodeFs.openSync(p, nodeFs.constants.O_RDWR | (nodeFs.constants.O_NOFOLLOW || 0));
