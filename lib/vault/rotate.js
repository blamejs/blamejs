...
function _fsyncFileByPath(p) {
  try {
    // codeql[js/insecure-temporary-file] p is an operator-supplied vault
    // data path (dataDir/stagingDir descendant), never an os.tmpdir-rooted
    // attacker-reachable path. The open is read/write only to satisfy
    // fsync semantics on platforms that demand write access.
...
async function rotate(opts) {
  ...
  if (nodeFs.existsSync(encDbPath) && dbKey) {
    ...
    // Per-rotation unique subdir inside the operator-supplied stagingDir.
    // mkdtempSync guarantees the suffix is non-predictable so an attacker
    // with same-machine access cannot pre-create the scratch DB or its
    // verify-pair under a known name.
    var rotateScratchDir = nodeFs.mkdtempSync(nodePath.join(stagingDir, "_blamejs_rotate-"));
    var tmpDbPath = nodePath.join(rotateScratchDir, "rotate.tmp.db");
    // Exclusive-create — fails EEXIST if anything pre-created the path
    // (mkdtempSync makes that essentially impossible, but the extra gate
    // is cheap insurance against operator-supplied stagingDir collisions).
    var tmpDbFd = nodeFs.openSync(tmpDbPath, "wx", 0o600);
    try {
      nodeFs.writeSync(tmpDbFd, plainBytes);
      nodeFs.fsyncSync(tmpDbFd);
    } finally {
      try { nodeFs.closeSync(tmpDbFd); } catch (_c) { /* best-effort */ }
    }
    ...
    // Round-trip verify on the staged DB. Reuse the per-rotation scratch
    // dir (mkdtempSync above) so the verify scratch DB lives under an
    // unpredictable path and can't be pre-created by an attacker.
    _emit(progress, { phase: "verify" });
    var verifyTmp = nodePath.join(rotateScratchDir, "verify.tmp.db");
    var verifyFd = nodeFs.openSync(verifyTmp, "wx", 0o600);
    try {
      var verifyPlain = bCrypto.decryptPacked(
        nodeFs.readFileSync(nodePath.join(stagingDir, paths.encryptedDb)), dbKey);
      nodeFs.writeSync(verifyFd, verifyPlain);
      nodeFs.fsyncSync(verifyFd);
    } finally {
      try { nodeFs.closeSync(verifyFd); } catch (_c) { /* best-effort */ }
    }
    ...
    try {
      ...
      // Best-effort removal of the per-rotation scratch dir; if any debris
      // remains (e.g. a sidecar this code path didn't anticipate), rmSync
      // recursive sweeps it so we don't leave operator-visible cruft in
      // stagingDir.
      try { nodeFs.rmSync(rotateScratchDir, { recursive: true, force: true }); }
      catch (e) { rotateLog.debug("cleanup-failed", { op: "fs.rmSync", path: rotateScratchDir, error: e.message }); }
    }
    if (!verifyResult.ok) {
