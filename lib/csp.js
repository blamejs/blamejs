...
// Directives that carry meaning with no value at all. `sandbox` with an
// empty value list applies every sandbox restriction, which is the strictest
// form of the directive, not an omitted one.
var VALUELESS_DIRECTIVES = [
 ...
 *
 * A directive set that would emit nothing is refused with `csp/empty`
 * rather than returned as an empty string: a header with no directives
 * constrains nothing while reading, at the call site, as a policy that
 * was set. That covers an empty object, an array (which passes a bare
 * `typeof x === "object"` test), and a set whose every directive
...
function build(directives, opts) {
  ...
  return _refuseEmptyPolicy(_emitCanonical(directives));
}
...
function _emitCanonical(directives) {
  ...
  for (var di = 0; di < ALL_DIRECTIVES.length; di += 1) {
    ...
    if (Array.isArray(vals) && vals.length === 0) {
      // Directives like upgrade-insecure-requests accept no value. `sandbox`
      // belongs here too: with no value it applies EVERY sandbox restriction,
      // so an empty list is the strictest form of the directive rather than
      // an absent one.
      if (VALUELESS_DIRECTIVES.indexOf(d) !== -1) {
        out.push(d);
        continue;
      }
      continue;
...
// A header with nothing in it constrains nothing, and the caller who set it
// believes the page is protected. Applied to the policy a caller RECEIVES,
// never inside _emitCanonical: mergeDirectives runs the additions through the
// emitter on their own to validate them, and adding nothing to a base policy
// is a valid no-op whose result is the base. Checking the intermediate pass
// refused a merge whose output was never empty.
//
// Emptiness arrives three ways — an empty directive object, an array (which
// passes a `typeof x === "object"` check while `Object.keys([])` is empty),
// and a set whose every directive carries an empty value list.
//
// b.cdnCacheControl.build already takes this position for a far less
// consequential header; the security-critical builder was the lenient one.
function _refuseEmptyPolicy(policy) {
  if (policy.length === 0) {
    throw new CspError("csp/empty",
      "csp: no directives to emit — refusing to build an empty Content-Security-Policy, " +
      "which would constrain nothing while appearing to be set");
  }
  return policy;
}
...
function mergeDirectives(base, additions, opts) {
  ...
  // Only when there is something to validate. Adding nothing is a valid no-op
  // whose result is the base policy, and an additions-only map with no keys
  // emits nothing — which build() now refuses, correctly, for a policy a
  // caller would receive. This pass is not that.
  if (addKeys.length > 0) {
    var validationMap = {};
