...
function verify(opts) {
  ...
  // verifyAll() below, which walks every SignerInfo against an
  // operator-supplied key map.
  ...
  _audit(opts.audit, "mail.crypto.smime.verify", "success", {
    ...
    chainVerified: !!(opts.trustAnchorCertsPem && opts.trustAnchorCertsPem.length > 0),
  });
...
function verifyAll(opts) {
  ...
  for (var i = 0; i < sd.signerInfos.length; i += 1) {
    ...
    // Compose verify() per signer — chain validation propagated when
    // operator supplies the trust-anchor set.
    verify({
      message:             opts.message,
      signature:           opts.signature,
      signerPublicKey:     pub,
      trustAnchorCertsPem: opts.trustAnchorCertsPem,
      audit:               opts.audit,
    });
    results.push({
      serialHex: serialHex,
  ...
  return { valid: true, signers: results };
}
