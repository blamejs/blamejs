 ...
 *     - Refuses RSA keys < 2048 bits (`b.mail.dkim.RSA_MIN_BITS`, the
 *       framework RSA floor, following the RFC 8301 §3.2 recommendation
 *       across the mail surface).
 ...
 *   - RFC 5280 (X.509 PKI)
 *   - RFC 8301 §3.2 (DKIM RSA key sizes; the framework RSA floor follows its 2048-bit recommendation)
 *
 * CVE citations:
...
var RSA_MIN_BITS = dkim.RSA_MIN_BITS;
...
function _verifyTrustChain(sd, trustAnchorCertsPem, signerPublicKey, auditHandle) {
  ...
  var res = x509Chain.resolveChain(leaf, chain, roots, {
    issued: function (issuer, subject) {
      try {
        return subject.issuer === issuer.subject && x509Chain.isCaCert(issuer) &&
  ...
  if (res.reason === "pathlen") {
    throw new MailCryptoError("mail-crypto/smime/chain-pathlen-exceeded",
      "chain exceeds a CA basicConstraints pathLenConstraint (RFC 5280 §4.2.1.9)");
  }
  if (res.reason === "nameconstraint") {
    throw new MailCryptoError("mail-crypto/smime/name-constraint-violation",
      "chain violates a CA nameConstraints (RFC 5280 §4.2.1.10)");
  }
  throw new MailCryptoError("mail-crypto/smime/untrusted-chain",
    "no trust anchor or intermediate found for issuer '" + leaf.issuer + "'");
}
  ...
  if (pub && pub.asymmetricKeyType === "rsa") {
    ...
    if (bits < RSA_MIN_BITS) {
      throw new MailCryptoError("mail-crypto/smime/rsa-too-small",
        ...
        " (the framework RSA floor, following the RFC 8301 §3.2 recommendation)");
    }
  }
