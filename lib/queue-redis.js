...
var FAIL_LUA = [
  ...
  // and another worker completed (or re-leased) it — must not re-queue a job that
  // is no longer inflight (it would resurrect a completed job for re-execution).
  // Mirrors queue-local fail()'s `WHERE status='inflight'` guard. Returns -1 so
  // the caller can report "did not act" rather than a retry/dlq outcome.
  'local removed = redis.call("ZREM", inflightKey, jobId)',
