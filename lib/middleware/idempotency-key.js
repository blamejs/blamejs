 ...
 *   bodyFingerprint:       function, // (req) => Buffer|string|object|null — operator-supplied body extractor; called instead of the inline req._rawBody||req.body lookup so body-parser ordering works regardless of mount order
 ...
 *     // Optional: provide a body-fingerprint extractor so the
 *     // middleware mounts BEFORE body-parser (operator's preferred
 *     // wiring) and still fingerprints the parsed body on each
 *     // request. The hook fires from inside the middleware so it
 *     // runs after the parser whatever order the operator chose.
