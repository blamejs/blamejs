...
function dbStore(opts) {
  ...
  var tableName = opts.tableName !== undefined ? opts.tableName : "blamejs_idempotency_keys";
  try { safeSql.validateIdentifier(tableName); }
  catch (sqlErr) {
  ...
  if (doInit) {
    // Two statements — CREATE TABLE + CREATE INDEX. Identifier names
    // are validated above so direct interpolation is safe.
    db.prepare("CREATE TABLE IF NOT EXISTS " + tableName + " (" +
      "k TEXT PRIMARY KEY, " +
      "v TEXT NOT NULL, " +
      "expires_at INTEGER NOT NULL)").run();
    db.prepare("CREATE INDEX IF NOT EXISTS " + tableName + "_expires_idx ON " +
      tableName + "(expires_at)").run();
  }
  ...
  var stmtGet     = db.prepare("SELECT v, expires_at FROM " + tableName + " WHERE k = ?");
  var stmtUpsert  = db.prepare("INSERT INTO " + tableName + "(k, v, expires_at) VALUES (?, ?, ?) " +
    "ON CONFLICT(k) DO UPDATE SET v = excluded.v, expires_at = excluded.expires_at");
  var stmtDelete  = db.prepare("DELETE FROM " + tableName + " WHERE k = ?");
  ...
  return {
    get: function (key) {
      ...
      try { return JSON.parse(row.v); }                                                              // allow:bare-json-parse — row.v written by our own .set() below
      catch (_e) {
        // Corrupt row — delete + treat as miss. The middleware emits
        // store_read_failed audit when it sees a thrown error; we
        // suppress here so the middleware sees a clean miss instead.
        stmtDelete.run(key);
        return null;
      }
    },
