...
function composePipeline(entries, opts) {
  ...
  return function composedPipeline(req, res, finalNext) {
    ...
    function dispatch(err) {
      ...
      if (idx >= resolved.length) return finalNext();
      var entry = resolved[idx];
      idx += 1;
      try {
        entry.mw(req, res, dispatch);
      } catch (syncErr) {
        // Synchronous throw from a middleware bubbles to finalNext
        // as an error — same shape Express uses.
        finalNext(syncErr);
      }
    }
    dispatch();
