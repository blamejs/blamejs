      ...
      async function dispatch(err) {
        ...
        try {
          if (err) {
            ...
            // Settle only if the handler ENDED the response (handled it). A
            // bare return without next may be a callback-style handler that
            // calls next() later; the finish/close listener covers a deferred
            // end. Never settle merely because the function returned.
            if (!advanced && _responseEnded(res)) _resolveOnce();
          } else {
            // Regular middleware: (req, res, next)
            await entry.mw(req, res, _next);
            // A middleware that ended the response and did not call next has
            // halted the chain — settle so the awaiter isn't pinned, without
            // advancing the router. A bare return (no response, no next) is a
            // callback-style middleware that will call next() later; leave the
            // promise pending until it does (or the response finishes).
            if (!advanced && _responseEnded(res)) _resolveOnce();
          }
        } catch (syncErr) {
