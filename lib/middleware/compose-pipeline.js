...
function composePipeline(entries, opts) {
  ...
  return function composedPipeline(req, res, finalNext) {
    return new Promise(function (resolve, reject) {
      ...
      // A middleware (or error handler) that returns WITHOUT calling next
      // has halted the chain: it handled the request itself. Settle the
      // outer promise so the awaiting router is released — a never-settled
      // promise pins its req/res closure forever — but do NOT call
      // finalNext: the router's next-flag stays false, so it won't run the
      // route handler on top of an already-sent response.
      ...
      async function dispatch(err) {
        ...
        try {
          if (err) {
            ...
            // 3-arg middleware that doesn't call next — chain halts. The
            // middleware wrote the response itself; settle the promise so
            // the awaiter isn't pinned, without advancing the router.
            if (!advanced) _resolveOnce();
          }
        } catch (syncErr) {
