...
function create(opts) {
  ...
  var clientIpResolver = opts.clientIpResolver;
  if (clientIpResolver != null && typeof clientIpResolver !== "function") {
    throw _err("BAD_OPT", "clientIpResolver must be a function(req) => ip|null");
  }
  var trustedProxies = Array.isArray(opts.trustedProxies) ? opts.trustedProxies.slice()
    : (typeof opts.trustedProxies === "string" ? [opts.trustedProxies] : []);
  for (var ti = 0; ti < trustedProxies.length; ti++) {
    if (!_validateCidr(trustedProxies[ti])) {
      throw _err("BAD_OPT",
        "trustedProxies[" + ti + "] is not a valid CIDR, got " + JSON.stringify(trustedProxies[ti]));
    }
  }
  var trustProxyOpt = opts.trustProxy === true || typeof opts.trustProxy === "number";
  if (trustProxyOpt && trustedProxies.length === 0 && !clientIpResolver) {
    throw _err("BAD_OPT",
  ...
  function _isTrustedProxy(addr) {
    for (var i = 0; i < trustedProxies.length; i++) {
      if (ssrfGuard.cidrContains(trustedProxies[i], addr)) return true;
    }
    return false;
  }
  // Resolve the client IP for the gate decision. clientIpResolver (operator
  // owns it) wins; else peer-gated XFF when trustedProxies is declared; else
  // the socket address only (forwarded headers ignored — fail safe).
  function _resolveClientIp(req) {
    if (clientIpResolver) return clientIpResolver(req);
    if (trustedProxies.length) return requestHelpers.clientIp(req, { trustProxy: _isTrustedProxy });
