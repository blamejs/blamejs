...
function create(opts) {
  ...
  return function requireAuth(req, res, next) {
    ...
    if (onDeny) {
      try {
        // An async hook that rejects is the same refusal failing, a turn
        // later; unobserved it ends the process. The value is still returned
        // as-is, so a caller awaiting it sees the rejection too.
