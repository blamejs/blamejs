 ...
 *     trustedProxies: string|string[],  // CIDRs of your reverse proxies — peer-gates X-Forwarded-For for audit attribution
 *     clientIpResolver: function(req): string|null,  // own the audit-actor IP
 *     trustProxy:    boolean|number,    // legacy; forgeable — attribution only, never a blocking input
 *   }
 *
 * @example
...
function create(opts) {
  ...
  validateOpts(opts, [
    ...
    "trustProxy", "trustedProxies", "clientIpResolver",
  ], "middleware.botGuard");
  var trustProxy = opts.trustProxy === true || typeof opts.trustProxy === "number"
    ? opts.trustProxy : false;
  // actor.ip for the audit event. When trustedProxies / clientIpResolver is
  // declared the source is resolved peer-gated (X-Forwarded-For honored only
  // from a trusted proxy peer) so a direct caller can't forge audit
  // attribution; otherwise the legacy trustProxy form is used (forgeable —
  // attribution only, never a blocking input).
  var _ipResolver;
  try {
  ...
  var _xffIp = _ipResolver.peerGated
    ? _ipResolver.resolve
    : function (req) { return requestHelpers.clientIp(req, { trustProxy: trustProxy }); };
