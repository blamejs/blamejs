...
function _serverAddress(req) {
  // allow:raw-xfp-telemetry-only — display-only: the server.address span attribute is telemetry, not an authority trust decision
  var hostHeader = req.headers && (req.headers["x-forwarded-host"] || req.headers[":authority"] || req.headers.host);   // allow:raw-xfp-telemetry-only — display-only span attribute, not an authority trust decision
...
function create(opts) {
  ...
  var ignorePaths       = Array.isArray(opts.ignorePaths) ? opts.ignorePaths.slice() : null;
  if (ignorePaths) {
    for (var ip = 0; ip < ignorePaths.length; ip++) {
      if (ignorePaths[ip] instanceof RegExp) {
        ...
        guardRegex().assertStateless(ignorePaths[ip],
          "middleware.spanHttpServer: ignorePaths[" + ip + "]",
          SpanHttpError, "span-http/stateful-pattern");
      }
    }
  }
  ...
  return function spanHttpServerMiddleware(req, res, next) {
    ...
    function _finish(err) {
      ...
      if (typeof onEnd === "function") safeAsync.safeInvoke(onEnd, span.toJSON());
      if (auditOn) {
        try {
