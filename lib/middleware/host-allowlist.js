...
function create(opts) {
  ...
  return function hostAllowlistMiddleware(req, res, next) {
    ...
    // `Host`. Node maps neither into the other -- the comment here used to say
    // it did -- so reading `headers.host` alone denied every HTTP/2 request
    // under `missing-host`, which is the shape this branch exists to catch.
    var raw = requestHelpers.requestHost(req);
    if (typeof raw !== "string" || raw.length === 0) {
      // RFC 9110 §7.2 — a request carrying neither a Host header nor an
      // HTTP/2 `:authority` names no target and is malformed. Reject it.
