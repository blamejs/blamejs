...
// Document-Policy is not sent by default.
//
// The default named `document-write`, `unsized-media` and `oversized-images`.
// The last two came out of the Document Policy origin trial and are not in
// shipping Chrome; `document-write` is not implemented as a document policy
// either; and Firefox and Safari do not implement `Document-Policy` at all.
// Measured on Chrome 152, the header carried no mitigation and logged one
// `Unrecognized document policy feature name` warning per feature on every
// navigation -- console noise an operator learns to scroll past, which is how
// a real warning gets missed later.
//
// What actually stops `document.write` is the `require-trusted-types-for
// 'script'` in DEFAULT_CSP above: under it the sink is guarded and throws.
//
// `documentPolicy` remains an option for an operator targeting a browser that
// gains support, and `documentPolicyReportOnly` is unchanged.
