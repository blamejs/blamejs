...
// Note: as of v0.1.101, csrf-protect no longer buffers + parses the
// request body itself. Operators who use form-urlencoded POSTs MUST
// register `b.middleware.bodyParser()` before csrf-protect so that
...
function create(opts) {
  ...
  return function csrfProtect(req, res, next) {
    ...
    if (typeof submitted !== "string" || submitted.length === 0) {
      ...
      // bodyParser before csrf-protect; the inline buffer/parse path
      // that lived here pre-v0.1.101 has been lifted into bodyParser.
