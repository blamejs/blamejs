...
function create(opts) {
  ...
  if (hasCookie) {
    ...
    // this boot check is the only enforcement point. Browsers apply the prefix
    // test ASCII case-INSENSITIVELY, so `__host-`/`__SECURE-` get the same
    // browser enforcement as `__Host-`/`__Secure-`; compare a lowercased copy so
    // a case-variant name can't dodge the invariant here and then be silently
    // rejected by the browser. Catch operator-side typos at boot.
