...
function _checkOriginAllowed(req, allowedOrigins, isHttpsFn, requireOrigin) {
  ...
  if (typeof origin !== "string" && typeof referer !== "string") {
    ...
    // A REASON string, like every other exit from this function. It returned an
    // object on this one path, and the caller builds its denial line by
    // concatenation, so the audit trail recorded
    // `origin/referer: [object Object]` -- the one configuration an operator
    // turns on deliberately was the one whose refusal said least, and in the
    // trail it was indistinguishable from any other object arriving here later.
    // The refusal itself was never affected: the object is truthy and the
    // caller compares against null.
  ...
  // Through requestHelpers, which reads the HTTP/2 `:authority` as well as
  // `Host`. Reading `headers.host` alone made this a bare scheme over h2 --
  // what every browser negotiates against this framework's TLS listeners -- so
  // the baseline was null, every browser Origin mismatched, and the gate
  // refused every form submission.
