...
function _parseCookieHeader(header) {
  ...
  // computed-write key (`out[name] = value`) — that is the CWE-915
  // unsafe-reflection / CWE-1321 prototype-pollution sink. POISONED names
  // (`__proto__` / `constructor` / `prototype`) are dropped, first-
  // occurrence-wins is preserved, and the null-prototype accumulator means
  // even a slipped name cannot reach Object.prototype.
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (Object.prototype.hasOwnProperty.call(seen, k)) continue;  // first-occurrence wins
    seen[k] = true;
