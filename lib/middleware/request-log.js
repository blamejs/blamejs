...
function create(opts) {
  ...
  return function requestLog(req, res, next) {
    ...
    function _emit() {
      ...
      // The Date.now branch that used to back this up is gone with the local
      // conversion: it was unreachable, and it was the one path that could
      // report a request duration measured on a clock an operator can correct
      // — including a negative one, on a host stepped backwards mid-request.
      var durationMs = sw.ms();
      var entry = {};
