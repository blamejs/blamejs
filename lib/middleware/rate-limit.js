 ...
 *
 * In-memory only — fine for single-process deployments. For multi-process
 * setups, plug a backend with a setKey/getKey API (Redis, framework's own
 * SQLite via _blamejs_counters, etc.). Cross-process backend lands in a
 * later release; this is the in-process fast path.
 *
 * Options:
 *   {
...
function create(opts) {
  ...
  var gcInterval = setInterval(function () {
    var cutoff = Date.now() - 60 * 60 * 1000;  // older than 1h
