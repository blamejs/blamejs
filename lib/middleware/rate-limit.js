...
function _memoryTokenBucketBackend(opts) {
  // Every window in this file is measured against this, so an application can
  // put its rate limiter on the same reading as the budgets it pairs with —
  // auth.lockout and cache both already take one. A wall-clock step forward
  // otherwise clears a window mid-flight: a source that had spent its
  // allowance simply gets a fresh one.
...
function _clusterBackend(opts) {
  // Deliberately the wall clock, and _resolveBackend refuses opts.clock for
  // this backend — see the reason there. Every node has to agree about which
  // window a request falls in, and only a shared epoch reading does that.
  var clock = Date.now;
...
function _resolveBackend(opts) {
  ...
  if (requested === "cluster") {
    // A cluster window is SHARED, and this backend derives its boundary as
    // floor(now / windowMs) * windowMs and writes that into a row every node
    // upserts against, keeping whichever windowStart is newer. Two nodes
    // reading different clocks therefore disagree about which window a request
    // belongs to: the one running ahead keeps proposing a newer boundary and
    // resets the shared counter, so the advertised cluster-wide limit is not
    // the limit anyone gets, and pruning by one node's reading can drop
    // another's counters.
    //
    // A process-local reading — the monotonic kind a caller uses precisely
    // because it does not follow the wall clock — is the shape that breaks it,
    // and nothing here can tell that apart from an epoch-accurate one. Refused
    // at configuration rather than silently ignored: a caller who passed a
    // clock and had it dropped would believe a budget was being measured the
    // way they asked.
    if (opts.clock) {
...
function create(opts) {
  ...
  validateOpts(opts, [
    ...
    // The reading every window is measured against. Enumerated here because
    // this validator REFUSES an unlisted name, so a caller passing the same
    // clock they gave auth.lockout and cache was rejected rather than ignored.
    "clock",
  ...
  // Type-checked HERE, not where it is called. `opts.clock || Date.now` takes a
  // number or a string as readily as a function, and the failure then surfaces
  // inside backend.take() — which this middleware catches and FAILS OPEN. A
  // mistyped clock would therefore not raise at boot; it would silently stop
  // limiting every request. Boot-time configuration is the tier that throws.
  ...
  // The other two options that are only ever functions. These already failed on
  // a mistyped value, but at the first REQUEST rather than at configuration —
  // loudly, unlike clock, yet still after the process reported itself healthy
  // and started serving. `statusOnLimit` and `bodyOnLimit` are deliberately not
  // here: a status number and a static body are valid values for them.
