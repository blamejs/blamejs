 ...
 * behavior: GET/HEAD only, SHA3-512 ETag with conditional 304,
 ...
 *     cacheControl:  string,    // default "public, max-age=300"; an earlier no-store on the response is kept
...
function create(opts) {
  ...
  function _writeBody(req, res, body, etag, contentType) {
    ...
    if (requestEtag && requestEtag === etag) {
      res.writeHead(C.HTTP.STATUS.NOT_MODIFIED, {
        "ETag":          etag,
    ...
    res.writeHead(C.HTTP.STATUS.OK, requestHelpers.finalizeHeaders(res, headers));
