...
function create(handler, opts) {
  ...
  return async function sseMiddleware(req, res) {
    ...
    var baseHeaders = {
      ...
      "Cache-Control": cdnCacheControl.keepNoStore(res, "no-cache, no-transform"),
    ...
    res.writeHead(requestHelpers.HTTP_STATUS.OK, requestHelpers.finalizeHeaders(res, headers));
    res.write(":\n\n");
