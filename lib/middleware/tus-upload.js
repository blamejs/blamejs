...
var TUS_ID_BYTES = C.BYTES.bytes(18);
...
function create(opts) {
  ...
  async function _handleOptions(req, res) {
    ...
    res.writeHead(STATUS_NO_CONTENT, requestHelpers.finalizeHeaders(res, headers));
    res.end();
  }
  ...
  async function _handleCreate(req, res) {
    ...
    res.writeHead(STATUS_CREATED, requestHelpers.finalizeHeaders(res, headers));
    res.end();
  ...
  async function _handleHead(req, res, id) {
    ...
    var expHdr = _expirationHeader(rec);
    if (expHdr) headers["Upload-Expires"] = expHdr;
    res.writeHead(STATUS_OK, requestHelpers.finalizeHeaders(res, headers));
    res.end();
  }
  ...
  return async function tusUploadMiddleware(req, res, next) {
    ...
    if (method !== "OPTIONS") {
      ...
      if (SUPPORTED_VERSIONS.indexOf(version) === -1) {
        ...
        res.writeHead(STATUS_PRECONDITION_FAILED, requestHelpers.finalizeHeaders(res, hdrs));
