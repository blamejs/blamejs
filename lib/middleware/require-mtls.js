...
function create(opts) {
  ...
  return function requireMtlsMiddleware(req, res, next) {
    ...
    if (revocationSource) {
      var revoked;
      try {
        revoked = revocationSource.isRevoked(fp.hex) === true;
        if (!revoked) {
          // A revoke(serial) / serial-only entry (importIssuance / revokeGeneration
          // of an unfingerprinted leaf) carries fingerprint:null and can't match by
          // fingerprint — also check the peer certificate's serial number.
          var serial = null;
          try { serial = new nodeCrypto.X509Certificate(peerCert.raw).serialNumber; }
          catch (_se) { serial = null; }
          if (serial) revoked = revocationSource.isRevoked(serial) === true;
        }
