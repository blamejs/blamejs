...
function create(opts) {
  ...
  validateOpts(opts, [
    ...
    // The fields the binding is computed FROM. Without this the only reachable
    // policy through this middleware was the default set — which includes the
    // full client address — so an operator who bound sessions on a narrower
    // basis at session.create could not have that basis honoured here, and a
    // roaming client was refused by a comparison they had already opted out of.
    // The proxy options travel with it: resolving the client address is what
    // the default fields depend on, and getting it wrong is what makes the
    // binding wrong.
    "fingerprintFields", "trustedProxies", "clientIpResolver",
  ], "middleware.attachUser");
  // Type-checked here rather than normalized at the call site. A mistyped
  // fingerprintFields silently became null and fell back to the DEFAULT basis
  // — which is the policy this option exists to override, so a typo quietly
  // reinstated the behaviour the operator was configuring their way out of.
  // A bad clientIpResolver is worse: session.verify throws on every request,
  // the catch below treats each one as unauthenticated, and the process reports
  // itself healthy while nobody can sign in.
  ...
  return async function attachUser(req, res, next) {
    ...
    try {
      ...
      // Only when supplied: session.verify has its own defaults, and passing
      // null would be a policy rather than an absence.
