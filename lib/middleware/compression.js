...
function _appendVary(existing, token) {
  var merged = requestHelpers.appendVaryValue(existing, token);
  if (merged !== null) return merged;
  return existing == null ? token : String(existing);
}
  ...
  return function compression(req, res, next) {
    ...
    function _applyCompressedHeaders(headersObj) {
      if (headersObj) {
        ...
        var ownVary = [];
        var vk = Object.keys(headersObj);
        for (var v = 0; v < vk.length; v++) {
          if (vk[v].toLowerCase() === "vary") { ownVary.push(headersObj[vk[v]]); delete headersObj[vk[v]]; }
        }
        ownVary.push("Accept-Encoding");
        headersObj["Vary"] = requestHelpers.mergeVary(
          { getHeader: function (n) { return originalGetHeader ? originalGetHeader(n) : undefined; } },
          { Vary: ownVary.join(", ") }).Vary;
      } else {
