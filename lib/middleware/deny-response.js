 ...
 *   onThrow:       function (err) ? — audit sink when onDeny fails, whether
 *                              it threw or returned a promise that rejected
 */
function denyResponse(req, res, ctx) {
  ...
  if (_isFn(ctx.onDeny)) {
    try {
      // An async hook that rejects is the same refusal failing, a turn later —
      // and by then this function has returned, so nothing here can catch it.
      // The value is returned as it came in, so a caller awaiting it still
      // sees the rejection; what it cannot do is reach the process unobserved.
