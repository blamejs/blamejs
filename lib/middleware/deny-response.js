...
function denyResponse(req, res, ctx) {
  ...
  if (ctx.problem) {
    ...
    if (extra) {
      var mergedExtra = requestHelpers.finalizeHeaders(res, extra);
      var hk = Object.keys(mergedExtra);
      for (var h = 0; h < hk.length; h += 1) {
        res.setHeader(hk[h], mergedExtra[hk[h]]);
      }
    }
  ...
  res.writeHead(ctx.status, requestHelpers.finalizeHeaders(res, head));
  res.end(denyOut);
  return undefined;
}
