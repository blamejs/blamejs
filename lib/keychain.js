 ...
 *
 * Stores / retrieves / removes a (service, account) -> password binding
 * via the host operating system's native credential store, with an
 * encrypted-file fallback when no native store is reachable. Operators
 * call this from CLI bootstraps that need to materialize a database
 * password, an outbound webhook secret, an SMTP relay credential, etc.,
 * without baking the value into a config file or env var.
 *
 ...
 *     passphrase:   process.env.BLAMEJS_KEYCHAIN_PASSPHRASE,
 *   });
 *   // → { password: "s3cr3t", backend: "macos-security" | "linux-secret-tool" |
 *   //                          "windows-credential" | "file" }
 *
 *   await b.keychain.remove({ service, account, fallbackFile });
 *
 * Backend dispatch:
 *
 *   - macOS  — `security` CLI from /usr/bin/security. add-generic-password,
 *              find-generic-password, delete-generic-password.
 *   - Linux  — `secret-tool` from libsecret-tools. store/lookup/clear.
 *              Password written on stdin (NEVER argv) so it does not
 *              appear in /proc/<pid>/cmdline.
 *   - Windows — PowerShell + the CredentialManager module (Get-/New-/
 *              Remove-StoredCredential). Password written on stdin via
 *              the calling host so it does not appear in the process
 *              command line. When CredentialManager is unavailable the
 *              call falls through to the file fallback.
 *   - File   — XChaCha20-Poly1305-sealed JSON object whose key is
 *              derived via Argon2id from `opts.passphrase`. The wrap
 *              format is shared with `b.vault.wrap` (magic 0xE2). File
 *              mode is 0o600. Atomic via `b.atomicFile.write`.
 *
 * Process-list-safety: every native-tool invocation passes the password
 * on stdin where the tool supports it. macOS `security` accepts the
 * password via stdin only on `add-generic-password -w -` (a single
 * dash); we use that form so the plaintext never hits argv. Operators
 * on macOS hardened against ps-snooping should still scope the boot
 * window to a small enclave — `security` writes the value into a
 * keychain DB the operator owns; an attacker with FS access already
 * has the keychain.
 *
 ...
 * `keychain.removed` (or the `.failed` sibling). Audit metadata records
 * service / account / backend / outcome. The password value is never
 * audited.
 */
...
// Pull KeychainError off framework-error if registered there; otherwise
// build it locally (codebase-patterns enforces a single error class per
// primitive — registration in lib/framework-error.js is the canonical
// home).
var KeychainError;
try {
  KeychainError = require("./framework-error").KeychainError;
} catch (_e) { /* fall through */ }
if (typeof KeychainError !== "function") {
  KeychainError = defineClass("KeychainError", { alwaysPermanent: true });
}
  ...
  return new Promise(function (resolve, reject) {
    ...
    stream.on("data", function (chunk) {
      ...
      if (total > capBytes) {
        ...
        reject(new KeychainError("keychain/native-output-too-large",
          "native tool output exceeded " + capBytes + " bytes"));
        return;
      }
      chunks.push(chunk);
    });
