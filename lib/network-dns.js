...
// CLOSED: only the network / resolver-availability failures that a retry can
// plausibly fix are transient (a timeout, a resolver or transport being
// unreachable, a DoH/DoT round-trip that failed) — everything else (bad config /
// options, a malformed reply, an unsupported query, an empty/NXDOMAIN-style
// no-result) is permanent, so an unknown or input error is not retried forever.
var DNS_TRANSIENT_CODES = {
  "dns/lookup-timeout":        true,
  "dns/system-failed":         true,
  "dns/transport-unavailable": true,
  ...
  "dns/dot-failed":            true,
  "dns/no-system-resolvers":   true,
  "dns/dnr-no-resolvers":      true,
  "dns/ddr-not-discovered":    true,
  "dns/setservers-failed":     true,
};
