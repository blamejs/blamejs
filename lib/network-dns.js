...
function _endsWithRootMarker(host) {
  ...
  var last = host.charAt(host.length - 1);
...
function _validateHostShape(host, primitive) {
  ...
  if (Buffer.byteLength(host, "utf8") === host.length) return host;
  ...
  // entirely.
  //
  // Recognising only the ASCII marker here would hand the system resolver a
  // RELATIVE name and let the search list expand it into a different, cacheable
  // one — so the shared predicate decides, not a second local rule.
  var absolute = _endsWithRootMarker(host);
  var ascii = _dnsToALabel(absolute ? host.slice(0, host.length - 1) : host, primitive);
...
function _dnsQueryLabels(host, primitive) {
  var h = String(host);
  if (_endsWithRootMarker(h)) h = h.slice(0, h.length - 1);
  if (h.length === 0 || h === ".") return [];
  // A name is pure ASCII exactly when its UTF-8 encoding is one byte per
  // character, so this needs no character table and no pattern.
  //
  // The conversion also strips the root marker in whichever spelling it
  // arrived: an absolute internationalized name may mark its root with U+3002
  // rather than an ASCII dot, which UTS #46 only turns into a dot during the
  // conversion — after the strip above has run.
  if (Buffer.byteLength(h, "utf8") !== h.length) h = _dnsToALabel(h, primitive);
