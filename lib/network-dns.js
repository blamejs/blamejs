...
// RFC 1034 §3.1 — `foo.` is the ABSOLUTE form of `foo`: the same name, with
// the root zone written out. Every host entering this module is normalized to
// the relative form once, at the public entry point, because the transports
// disagree about the absolute one: glibc's getaddrinfo accepts it, musl's does
// not (so the same FQDN that resolves on Debian answers ENOTFOUND on Alpine),
// and the encrypted transports refuse it outright — splitting on "." leaves a
// trailing empty label, which RFC 1035 LDH validation rejects as length 0.
// Normalizing at the boundary rather than in each transport also keeps one
// cache entry per name instead of one per spelling.
//
// EXACTLY ONE trailing dot is removed, because a name carries exactly one root
// label. `example.com..` is not an absolute name with a stray dot — it is a
// name with an empty label, as malformed as `example..com`, and stripping the
// run would quietly rewrite it into a DIFFERENT valid name and resolve that
// instead. Removing one dot leaves the second for LDH validation to refuse.
// A bare "." (the root itself) is left intact for the same reason.
...
// The same normalization, plus the one check that must happen at EVERY entry
// point rather than only where full LDH validation runs.
//
// After the root dot is removed, a remaining empty label means the caller
// handed us a malformed name — `example.com..`, `example..com`, `.example.com`.
// The encrypted transports catch that in their RFC 1035 LDH pass, but the
// system-resolver branch of lookup() has none, and getaddrinfo happily treats
// `example.com.` as absolute: `example.com..` normalized to `example.com.` and
// then RESOLVED, returning (and caching) the address of a DIFFERENT name than
// the one asked for. Reject it here, once, so no entry point can skip it.
//
// An IP literal is passed through untouched — `::1` is not a domain name and
// its colons are not labels.
function _normalizeAndValidateHost(host, primitive) {
  var normalized = _normalizeHost(host);
...
async function resolveSecure(host, type) {
  host = _normalizeAndValidateHost(host, "dns.resolveSecure");
  type = type || "A";
  if (!STATE.doh) {
...
async function _querySvcbLike(host, qtype, opts) {
  host = _normalizeAndValidateHost(host, "dns.querySvcb");
  opts = opts || {};
...
async function lookup(host, opts) {
  host = _normalizeAndValidateHost(host, "dns.lookup");
  opts = opts || {};
...
async function _resolveProtocol(host, family, opts) {
  host = _normalizeAndValidateHost(host, "dns.resolve");
  opts = opts || {};
