...
//   - A non-ASCII code unit. `Buffer.write(s, "ascii")` keeps only its low
//     byte, so a U-label would be silently mangled into different octets
//     rather than being required as the A-label the wire carries.
//
...
function _dnsQueryLabels(host, primitive) {
  ...
  for (var i = 0; i < labels.length; i += 1) {
    ...
    if (len !== labels[i].length) {
      throw new DnsError("dns/bad-host",
        primitive + ": host label is not ASCII — an internationalized name must " +
        "be converted to its A-label (xn--) form before it reaches the wire");
    }
