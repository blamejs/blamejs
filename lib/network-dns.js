...
function _validateHostShape(host, primitive) {
  ...
  // another.
  //
  // The trailing root dot is carried across rather than dropped: a resolver
  // reads it as "already fully qualified, do not apply the search list", and
  // under an elevated ndots losing it can resolve a different name entirely.
  var absolute = host.length > 1 && host.charAt(host.length - 1) === ".";
...
function _dnsQueryLabels(host, primitive) {
  var h = String(host);
  if (h.length > 1 && h.charAt(h.length - 1) === ".") h = h.slice(0, h.length - 1);
