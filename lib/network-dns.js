...
function _dnsQueryLabels(host, primitive) {
  var h = String(host);
  // Only the ASCII branch strips its own root marker. A name that goes through
  // the conversion must NOT be pre-stripped: canonicalDomain removes exactly
  // one marker and refuses a doubled one, and taking the first here would let
  // the second come off inside it — turning a name with an empty final label
  // into a real, different name instead of refusing it.
  if (Buffer.byteLength(h, "utf8") !== h.length) {
    h = _dnsToALabel(h, primitive);
  } else if (h.length > 1 && h.charAt(h.length - 1) === ".") {
