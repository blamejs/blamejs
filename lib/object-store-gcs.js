...
function create(config) {
  ...
  if (!serviceAccount && config.serviceAccountFile) {
    try {
      serviceAccount = jsonSafe.parse(fs.readFileSync(config.serviceAccountFile), { schema: SERVICE_ACCOUNT_SCHEMA });
    } catch (e) {
  ...
  var allowedProtocols = config.allowedProtocols || urlSafe.ALLOW_HTTP_TLS;
  urlSafe.parse(endpoint,      { allowedProtocols: allowedProtocols, errorClass: ObjectStoreError });
  urlSafe.parse(tokenEndpoint, { allowedProtocols: allowedProtocols, errorClass: ObjectStoreError });
  ...
  async function _ensureToken() {
    ...
    var tokenResp = jsonSafe.parse(res.body);
  ...
  async function put(key, body, opts) {
    ...
    var meta = jsonSafe.parse(res.body);
  ...
  async function get(key) {
    ...
    var res = await _httpRequest("GET", url, { "Authorization": "Bearer " + token }, null, reqOpts);
    return res.body;
  }
  ...
  async function deleteKey(key) {
    ...
    var url = _objectUrl(key);
    try {
      await _httpRequest("DELETE", url, { "Authorization": "Bearer " + token }, null, reqOpts);
      return true;
    } catch (e) {
  ...
  async function list(prefix, opts) {
    ...
    var res = await _httpRequest("GET", url, { "Authorization": "Bearer " + token }, null, reqOpts);
    var listResp = jsonSafe.parse(res.body);
