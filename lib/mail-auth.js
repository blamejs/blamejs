...
// RFC 9989 base policy keys (DMARCbis, published as RFC 9989)
// extensions:
...
async function dmarcEvaluate(opts) {
  ...
  try {
    ...
    if (rec) {
      ...
      // RFC 9989 §4.10 (DNS tree walk) — fall through to organizational
...
//
// From-header discipline (RFC 9989 §5.3.1, RFC 9989 §5.3.1 before it):
...
function dmarcBuildAggregateReport(report, opts) {
  ...
  // RFC 9990 §3.5.2 — report-format version is "1.0" (the `version`
