...
function _dmarcAuthorDomainLabels(domain) {
  ...
  if (d.length > 1 && d.charAt(d.length - 1) === ".") d = d.slice(0, d.length - 1);
  if (d.length === 0) return null;
  var labels = d.split(".");
  for (var i = 0; i < labels.length; i += 1) {
    if (labels[i].length === 0) return null;
  }
  return labels;
}
...
async function _dmarcTreeWalk(domain, dnsLookup) {
  ...
  // True once the Author Domain's own record is in hand. From that point an
  // ancestor can only refine the walk, never decide it: the record at the most
  // specific name supplies `p=` directly. A broken record published by a parent
  // is then a fact about the parent, and a domain owner controls what they
  // publish, not what their parent does — abandoning their `p=reject` over it
  // would let a parent zone turn a policy that resolved cleanly into a
  // permerror, which a receiver reads as no policy at all. That is the same
  // exemption the unresolvable-ancestor case already makes, and the two must
  // not disagree about which failure discards a policy the walk already has.
  //
  // Before that point there is no such record, the malformed name may be the
  // one whose policy would have applied, and the walk cannot know what it said:
  // the error stands rather than skipping to a weaker record above it.
  var haveStartRecord = false;
  ...
  for (var i = 0; i < targets.length; i += 1) {
    ...
    try {
      ...
      if (_isPermanentDmarcError(e)) {
        if (haveStartRecord) { skipped = true; continue; }
        throw e;
      }
      transient = true;
      skipped = true;
...
function _dmarcOrganizationalDomain(found, startDomain) {
  ...
  for (i = 0; i < found.length; i += 1) {
    // A `psd=y` record AT the starting domain does not name the domain below
    // it — the walk started there, so there is nothing below to name.
    if (found[i].policy.psd !== "y" || found[i].domain === startDomain) continue;
...
async function dmarcEvaluate(opts) {
  ...
  try {
    ...
    // Only `psd=n` DECLARES a boundary. The fewest-labels fallback is an
    // inference from what happened to be published, and narrowing alignment on
    // an inference would refuse mail that aligns perfectly well today.
    if (selected && selected.via === "psd-n") alignmentBoundary = selected.domain;
    ...
    if (atStart) {
      ...
      var chosen = _dmarcRecordAt(found, selected.domain);
      // `psd=y` names the domain BELOW it as organizational, and that domain
      // published nothing — the policy still comes from the record that was
      // actually found, which sits at the public-suffix operator's name.
      if (!chosen && found.length > 0) chosen = found[found.length - 1];
      if (chosen) {
        // The record governs a subdomain here, so `sp=` is the operative
        // policy when it is set.
        chosen.policy.p = chosen.policy.sp || chosen.policy.p;
        policy = chosen.policy;
        policyOriginDomain = chosen.domain;
        if (selected.via === "psd-y" || chosen.policy.psd === "y") psdPolicyApplied = true;
  ...
  var alignSpf = walkSkipped ? "s" : policy.aspf;
  var alignDkim = walkSkipped ? "s" : policy.adkim;
