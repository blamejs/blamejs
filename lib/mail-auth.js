...
function _parseArcTagList(value) {
  ...
  // §3.8 imports `tag-list` from RFC 6376 §3.2, which interprets tag names
  // case-sensitively, so the names are not folded.
...
// Three times the largest ratio a generated report reached (109:1 at 10,000
// records) and about a third of what the gzip format itself can produce.
var DMARC_RUA_MAX_EXPANSION_RATIO = 300;
// What one <record> and one report header expand to as XML elements.
...
function dmarcParseAggregateReport(input, opts) {
  ...
  // Auto-detect gzip via magic 0x1f 0x8b (RFC 1952) and ZIP via the local
  // file header 0x50 0x4b 0x03 0x04 (APPNOTE §4.3.7). RFC 7489 §7.2.1.1
  // names both as the encodings a report is sent under, and the input
  // arrives from anyone who chooses to send one to a published `rua=`
  // address.
  ...
  if (looksZip || (contentType.indexOf("zip") !== -1 && !looksGzip &&
    ...
    // A ZIP holds entries, so unpacking one is a decision about which entry to
    // read and this function has a Buffer of XML as its whole contract.
    // `b.archive.read.zip` is the reader, and it is asynchronous, so composing
    // it here would make this function asynchronous for every caller including
    // the gzip ones. The composition is two lines and the operator picks the
    // entry:
    //
    //   var reader = b.archive.read.zip(b.archive.adapters.buffer(bytes));
    //   var xml    = await reader.extractEntry(<the .xml entry>);
    //   b.mail.dmarc.parseAggregateReport(xml);
  ...
  if (contentType.indexOf("gzip") !== -1 || looksGzip) {
    try {
      // Through the framework's bounded-decompression primitive rather than a
      // second implementation of the same caps. It refuses over the absolute
      // output size before allocating, refuses over the expansion ratio after,
      // and emits the refusal to the audit trail.
      //
      // The absolute cap is what does the work here, and the ratio is a
      // backstop. A DMARC aggregate report is repetitive XML and compresses
      // accordingly: measured on generated reports, 100 records reach 38:1,
      // 1,000 reach 63:1 and 10,000 reach 109:1, and the gzip format's own
      // ceiling is about 1032:1. So a ratio low enough to separate a report
      // from a bomb does not exist for this input, and the sibling parser's
      // 50:1 default would refuse a report of a few hundred records. The cap
      // set here sits about three times above the largest measured legitimate
      // ratio, which bounds the smallest input that can reach the output cap
      // without refusing mail anyone actually sends.
      ...
      // Distinguish "decompressed bytes exceed a cap" (amplification —
      // the operator should rate-limit the source) from "stream is
      // malformed" (an operator-level diagnostic) so audit and alert
      // wiring can react differently. Defends the decompression-
      // amplification class (CWE-409 / CVE-2025-0725).
      //
      // Read on the CODE, not the wording. zlib's own refusal arrives as
      // `.cause` once safeDecompress has wrapped it, and what it says there has
      // changed across Node releases -- "Output length exceeded the limit" and
      // "Cannot create a Buffer larger than N bytes" are the same refusal.
  ...
  var parsed;
  // The element cap is derived from the record cap rather than left at the
  // parser's default, because the two are statements about one thing and the
  // default is the smaller of them by two orders of magnitude: `<record>`
  // expands to seventeen elements in the shape RFC 7489 Appendix C defines, so
  // the parser's 10,000-element default refuses at about 580 records while the
  // cap below says 10,000. A report from a domain with real mail volume has
  // thousands, so the ordinary case was refused -- and refused as malformed
  // XML, which says nothing about the limit it actually met.
  //
  // Thirty-two per record is twice the base shape, which covers the optional
  // children a reporter may include: several auth results, `<reason>`,
  // `<envelope_to>`, `<envelope_from>`, `<selector>`, `<human_result>`. The
  // byte cap above remains the bound that matters; this one stops a report
