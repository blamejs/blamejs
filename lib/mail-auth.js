...
function _dmarcAuthorDomainLabels(domain) {
  ...
  // Only the ASCII branch strips its own root marker. canonicalDomain strips
  // exactly one and refuses a doubled one, so taking one off here first would
  // let the second come off inside it: `münchen.example。.` carries an empty
  // final label and would be repaired into a real, different domain, a policy
  // discovered under THAT name, and the message then compared against a name it
  // never claimed.
  //
  // Letting canonicalDomain own the strip also keeps the Author Domain in the
  // same canonical form as the authenticated SPF and DKIM domains, which reach
  // `_alignmentCheck` through canonicalDomain directly — compensating here
  // instead left a message unable to align with itself.
  if (Buffer.byteLength(d, "utf8") !== d.length) {
    d = publicSuffix.canonicalDomain(d);
    if (!d) return null;
  } else if (d.length > 1 && d.charAt(d.length - 1) === ".") {
