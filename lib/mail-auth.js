...
var arcSign = require("./mail-arc-sign");
...
var SPF_VERSION_GRAMMAR = Object.freeze({
  end:              "space",
  nameIgnoresCase:  true,
  valueIgnoresCase: true,
});

var DMARC_VERSION_GRAMMAR = Object.freeze({ wspAroundEquals: true, nameIgnoresCase: true });

var DOMAIN_VALUED_PROPS = Object.freeze({ from: 1, domain: 1 });
...
function _parseSpfRecord(text) {
  ...
  if (!structuredFields.recordVersionMatches(trimmed, "v", "spf1", SPF_VERSION_GRAMMAR)) {
...
async function _fetchSpfRecord(domain, dnsLookup) {
  ...
  for (var i = 0; i < records.length; i += 1) {
    ...
    if (structuredFields.recordVersionMatches(rec, "v", "spf1", SPF_VERSION_GRAMMAR)) matches.push(rec);
  }
...
async function _fetchDmarcRecord(domain, dnsLookup) {
  ...
  for (var i = 0; i < records.length; i += 1) {
    var rec = Array.isArray(records[i]) ? records[i].join("") : records[i];
    if (structuredFields.recordVersionMatches(rec, "v", "DMARC1", DMARC_VERSION_GRAMMAR)) matches.push(rec);
  }
...
function _parseDmarcRecord(text) {
  ...
  if (policy.v !== "DMARC1" ||
      !structuredFields.recordVersionMatches(String(text), "v", "DMARC1", DMARC_VERSION_GRAMMAR)) {
...
function _arcSkippedHops(hops, duplicatedSlots) {
  return hops.filter(Boolean).map(function (h) {
    var aarDuplicated = !!(duplicatedSlots && duplicatedSlots[h.instance + ":arc-authentication-results"]);
    return { instance: h.instance,
             sealDomain: null,
             authenticationResults: aarDuplicated ? null : (h["arc-authentication-results"] || null),
             hasSeal: !!h["arc-seal"],
...
async function arcVerify(rfc822, opts) {
  ...
  for (var i = 0; i < headers.length; i += 1) {
    ...
    var arcInstance = arcSign._arcHeaderInstance(name, value);
    if (arcInstance.invalid) { invalidTagList = true; continue; }
    var inst = arcInstance.instance;
    if (inst === null) continue;
    if (inst > maxInstanceSeen) maxInstanceSeen = inst;
    var slotKey = inst + ":" + name;
    if (seenSlot[slotKey]) { duplicate = true; duplicatedSlots[slotKey] = true; continue; }
  ...
  if (invalidTagList) {
    return {
      chainStatus: "fail",
      reason:      "invalid-tag-list",
      hopCount:    hops.filter(Boolean).length,
      hops:        _arcSkippedHops(hops),
    };
  }
  ...
  if (duplicate) {
    return {
      ...
      hopCount:    hops.filter(Boolean).length,
      hops:        _arcSkippedHops(hops, duplicatedSlots),
    };
  }
  ...
  for (var hopIdx = 0; hopIdx < hops.length; hopIdx += 1) {
    ...
    var amsTags = dkim._parseTagList(hop["arc-message-signature"]);
    var asTags  = dkim._parseTagList(hop["arc-seal"]);
    ...
    perHop.push({
      ...
      sealDomain:               typeof asTags.d === "string" ? asTags.d.toLowerCase() : null,
      authenticationResults:    hop["arc-authentication-results"],
      hasSeal:                  true,
  ...
  for (var hi = 0; hi < hops.length; hi += 1) {
    var sealCv = dkim._parseTagList(hops[hi]["arc-seal"]).cv;
    var hopCv = (sealCv === "none" || sealCv === "pass" || sealCv === "fail") ? sealCv : null;
    perHopCv.push(hopCv);
    if (sealCv === undefined) {
    ...
    if (hopCv === null) {
      hopRuleViolation = "invalid-cv-at-i=" + (hi + 1);
      break;
    }
    if (hi === 0 && hopCv !== "none") {
...
async function _verifyArc(rfc822, hop, allHops, kind, dnsLookup, dkim) {
  ...
  var tags = dkim._parseTagList(sigValue);
  ...
  try {
    ...
    keyTags = dkim._selectKeyRecordTags(records);
  } catch (e) {
    var verdict = (e && (e.code === "ENOTFOUND" || e.code === "ENODATA" || e.code === "dkim/key-not-found"))
                  ? "permerror" : "temperror";
...
async function _verifyAmsViaDkim(rfc822, hop, sigValue, tags, dkim, dnsLookup) {
  ...
  for (var i = 0; i < headerLines.length; i += 1) {
    ...
    if (name === "arc-authentication-results") {
      var aarInst = arcSign._arcHeaderInstance(name, khv.value.trim()).instance;
...
function _runVerify(signedString, sigB64, algorithm, keyB64, label) {
  ...
  if (!safeBuffer.isCanonicalBase64(sigB64)) {
    return { result: "permerror", errors: [label + ": b= is not base64 (RFC 6376 §3.5)"] };
  }
...
async function arcEvaluate(rfc822, opts) {
  ...
  if (Array.isArray(verdict.hops)) {
    for (var fi = 0; fi < verdict.hops.length; fi += 1) {
      var fhop = verdict.hops[fi];
      if (fhop && fhop.instance === verdict.hopCount) {
        out.finalAr = fhop.authenticationResults || null;
      }
    }
  }
...
function authResultsEmit(opts) {
  ...
  for (var i = 0; i < opts.results.length; i += 1) {
    ...
    for (var pk = 0; pk < propKeys.length; pk += 1) {
      ...
      if (DOMAIN_VALUED_PROPS[k] === 1 && rv.indexOf("@") !== -1) {
        throw new MailAuthError("mail-auth/ar-address-in-domain-property",
          "authResults.emit: " + props[k] + " is the domain (IANA Email Authentication " +
          "Methods registry; RFC 7489 §3.1 aligns on the From domain), got the address " +
          "'" + rv + "'; the local part is not the receiver's to publish in a header " +
          "every intermediary reads");
      }
      clause += " " + props[k] + "=" + rv;
    }
    clauses.push(clause);
...
async function inboundVerify(opts) {
  ...
  if (opts.authservId) {
    ...
    if (from.domain) dmarcEntry.from = from.domain;
...
function dmarcParseAggregateReport(input, opts) {
  ...
  if (Buffer.isBuffer(input)) bytes = input;
    ...
    var sized;
    try {
      sized = safeJson.measureBytes(input, {
        limit: DMARC_RUA_MAX_REPORT_BYTES, followToJson: false,
      });
    } catch (e) {
      throw new MailAuthError("mail-auth/dmarc-rua-bad-input",
        "dmarc.parseAggregateReport: report has no JSON form: " +
