...
async function _verifyAmsViaDkim(rfc822, hop, sigValue, tags, dkim, dnsLookup) {
  ...
  // signal the verifier (via its internal Symbol channel) to skip the §3.5
  // AUID/d= binding check that would otherwise permerror every ARC chain.
  var verifyOpts = { dnsLookup: dnsLookup };
  verifyOpts[dkim.ARC_AMS_REUSE] = true;
