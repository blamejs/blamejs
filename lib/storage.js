 ...
 *
 *   1. Single-backend (v0.0.5 syntax — preserved):
 *        storage.init({ backend: 'local', uploadDir: './data/uploads' })
 *
 *   2. Multi-backend (v0.0.9):
 *        storage.init({
 *          backends: {
  ...
  if (opts.backend) {
    if (opts.backend === "s3") {
      throw new Error(
        ...
        "Deferred to v0.0.10 — use 'local' or 'http-put' for now."
      );
    }
...
async function getFileStream(key, sealedKey, opts) {
  // For v0.0.9 we still buffer-then-stream because per-file XChaCha20 encryption
  // needs the whole ciphertext to verify the AEAD tag. Chunked-encryption
  // with per-chunk AEAD lands later; documented in roadmap risk register.
