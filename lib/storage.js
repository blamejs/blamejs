 ...
 * partial assemblies.
 *
 * Each chunk is sealed via the framework vault (same envelope as
 * `saveFile`). Assembly metadata (`createdAt` / `totalChunks` /
 * per-chunk hash) is sealed separately. assemblyId shape is validated
 * to refuse path-traversal, control chars, and oversize.
 *
 * @opts
...
function chunkScratch(opts) {
  ...
  function _metaKey(assemblyId) {
    return rootKeyPrefix + "/" + assemblyId + "/.meta";
  }
  function _pickOpts() {
    return backendOverride ? { backend: backendOverride } : {};
  }

  async function _readMeta(assemblyId, sealedKey) {
    try {
      return await getFileBuffer(_metaKey(assemblyId), sealedKey, _pickOpts());
    } catch (_e) {
      return null;
    }
  }
  async function _writeMeta(assemblyId, meta, prevSealedKey) {
    // Overwrites the previous meta atomically via saveFile's put
    // (backends do an atomic rename for local; cloud put is atomic
    // by spec). Returns the new sealed key.
    var bytes = Buffer.from(JSON.stringify(meta), "utf8");
    var saved = await saveFile(bytes, _metaKey(assemblyId), _pickOpts());
    // Old sealed key is unreachable post-overwrite (vault envelope is
    // tied to ciphertext); operator can drop the prevSealedKey.
  ...
  async function removeAssembly(assemblyId) {
    ...
    try { await deleteFile(_metaKey(assemblyId), _pickOpts()); }
    catch (_e) { /* meta may not exist */ }
