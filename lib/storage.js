...
function _err(code, message, permanent) {
  return new StorageError(code, message, permanent);
}
...
function presignedUploadUrl(key, opts) {
  _requireInit();
  ...
  opts = opts || {};
  var picked = _pickBackend(opts);
  if (typeof picked.backend.presignedUploadUrl !== "function") {
    throw _err("PRESIGN_NOT_SUPPORTED",
      "backend '" + picked.backend.name + "' (protocol '" + picked.backend.protocol +
      "') does not support presigned upload URLs", true);
  }
  var result = picked.backend.presignedUploadUrl(Object.assign({}, opts, { key: key }));
