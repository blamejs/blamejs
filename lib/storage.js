...
function init(opts) {
  ...
  if (!opts) throw _err("storage/invalid-config", "storage.init() requires options", true);
...
function _normalizeConfig(opts) {
  ...
  if (opts.backend) {
    if (opts.backend === "s3") {
      throw _err("storage/invalid-config",
...
function _validateResidency() {
  ...
  if (defaultClassification === "personal") {
    ...
    if (!found) {
      throw _err("storage/no-personal-backend",
...
function _pickBackend(opts) {
  ...
  if (opts.backend) {
    ...
    if (!b) throw _err("storage/unknown-backend", "no backend named '" + opts.backend + "'", true);
    if (opts.classification && !b.servesClassification(opts.classification)) {
      throw _err("storage/classification-mismatch",
  ...
  if (refuseUnclassified && !opts.classification) {
    throw _err("storage/unclassified",
  ...
  if (!classification) {
    ...
    throw _err("storage/no-backends", "no backends configured", true);
  }
  ...
  throw _err("storage/no-backend-for-classification",
...
function _decryptBuffer(packed, sealedKey) {
  if (!sealedKey) {
    throw _err("storage/key-required", "encryptionKey is required (no legacy plaintext support)", true);
  }
...
async function saveFile(buffer, key, opts) {
  _requireInit();
  if (!Buffer.isBuffer(buffer)) throw _err("storage/invalid-body", "saveFile body must be a Buffer", true);
  opts = opts || {};
...
async function saveRaw(buffer, key, opts) {
  _requireInit();
  if (!Buffer.isBuffer(buffer)) throw _err("storage/invalid-body", "saveRaw body must be a Buffer", true);
  opts = opts || {};
  var picked = _pickBackend(opts);
 ...
 * backend reports the key present, `false` on `objectstore/not-found`. Other
 ...
 *
 * Every backend reports a missing key as `objectstore/not-found`, so this
 * answers `false` on local, S3/sigv4, Azure Blob, GCS and http-put alike.
 *
 * @opts
 *   classification:  string,    // route to a backend serving this classification
 *   backend:         string,    // explicit backend by name
...
async function exists(key, opts) {
  ...
  try {
    ...
    if (e && e.code === "objectstore/not-found") return false;
    throw e;
  }
}
 ...
 * `storage/versions-unsupported` rather than silently returning the current view,
...
async function listVersions(prefix, opts) {
  ...
  if (typeof picked.backend.listVersions !== "function") {
    throw _err("storage/versions-unsupported",
...
function _presign(direction, key, opts) {
  ...
  if (typeof key !== "string" || key.length === 0) {
    throw _err("storage/invalid-key", "presigned" + direction + "Url: key is required", true);
  }
  opts = opts || {};
  var picked = _pickBackend(opts);
  var fnName = "presigned" + direction + "Url";
  if (typeof picked.backend[fnName] !== "function") {
    throw _err("storage/presign-not-supported",
 ...
 * `storage/presign-not-supported`. Emits `system.storage.presign` with
 * `direction: "upload"`.
 *
 * @opts
 *   classification:  string,    // route to a backend serving this classification
 ...
 * backends throw `storage/presign-not-supported`.
 *
 * @opts
 *   classification:  string,    // route to a backend serving this classification
 *   backend:         string,    // explicit backend by name
...
function presignedUploadPolicy(key, opts) {
  _requireInit();
  if (typeof key !== "string" || key.length === 0) {
    throw _err("storage/invalid-key", "presignedUploadPolicy: key is required", true);
  }
  opts = opts || {};
  var picked = _pickBackend(opts);
...
function _requireInit() {
  if (!initialized) throw _err("storage/not-initialized", "storage.init() must be called before any file operation", true);
}
...
function _validateAssemblyId(id) {
  if (typeof id !== "string" || id.length === 0) {
    throw _err("storage/invalid-argument", "chunkScratch: assemblyId must be a non-empty string", true);
  }
  if (id.length > ASSEMBLY_ID_MAX_LEN) {
    throw _err("storage/invalid-argument",
 ...
 *   - AWS S3 Multipart Upload — the cloud-vendor analog;
...
function chunkScratch(opts) {
  ...
  numericBounds.requirePositiveFiniteIntIfPresent(
    opts.maxChunkBytes, "chunkScratch.maxChunkBytes", StorageError, "storage/invalid-argument");
  numericBounds.requirePositiveFiniteIntIfPresent(
    opts.staleAfterMs, "chunkScratch.staleAfterMs", StorageError, "storage/invalid-argument");
  ...
  async function saveChunk(args) {
    if (!args || typeof args !== "object") {
      throw _err("storage/invalid-argument", "chunkScratch.saveChunk: args must be an object", true);
    }
    _validateAssemblyId(args.assemblyId);
    _validateChunkIndex(args.chunkIndex);
    if (!Buffer.isBuffer(args.data)) {
      throw _err("storage/invalid-argument", "chunkScratch.saveChunk: data must be a Buffer", true);
    }
  ...
  async function getChunk(args) {
    if (!args || typeof args !== "object") {
      throw _err("storage/invalid-argument", "chunkScratch.getChunk: args must be an object", true);
    }
    _validateAssemblyId(args.assemblyId);
    _validateChunkIndex(args.chunkIndex);
    if (typeof args.encryptionKey !== "string" || args.encryptionKey.length === 0) {
      throw _err("storage/invalid-argument", "chunkScratch.getChunk: encryptionKey required", true);
    }
  ...
  async function listChunks(assemblyId) {
    ...
    if (typeof picked.backend.list !== "function") {
      throw _err("storage/unsupported",
  ...
  async function assemble(args) {
    if (!args || typeof args !== "object") {
      throw _err("storage/invalid-argument", "chunkScratch.assemble: args must be an object", true);
    }
    _validateAssemblyId(args.assemblyId);
    ...
    if (typeof args.expectedTotal === "number" && indices.length !== args.expectedTotal) {
      throw _err("storage/incomplete-assembly",
