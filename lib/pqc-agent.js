...
// reports { type:"TLSGroup", name:"X25519MLKEM768" }. The name is therefore
// authoritative, and a name that does not carry "MLKEM" means the peer offered
// no hybrid and the handshake fell back to classical X25519 (the framework's
// last-resort group) — emit the downgrade so operators can see which
// dependencies are not yet post-quantum ready.
//
// An ABSENT name is NOT evidence of a hybrid. It means the handshake reported
// no ephemeral group at all — a resumed session, or a key exchange that is not
// ECDHE — so there is nothing to judge and nothing is emitted. (Older Node did
// not model a hybrid as ECDH and returned {} for one, which made an absent
// name ambiguous; the framework's floor now reports the group by name, so the
