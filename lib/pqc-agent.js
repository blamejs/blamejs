...
function create(opts) {
  ...
  agent.reloadCerts = function (newMaterial) {
    return _reloadCertsOnAgent(agent, opts, built, newMaterial);
  };
  return agent;
}

function _reloadCertsOnAgent(agent, originalOpts, builtOpts, newMaterial) {
  ...
  // Build the next secure context from the same posture / TLS opts the
  // agent was created with, swapping only the cert / key / ca fields.
  var nextOpts = Object.assign({}, builtOpts, {
  ...
  try {
    ...
    if (e && /ca\b/i.test(String(e.message || e))) {                                                 // allow:regex-no-length-cap — error-message shape match
      throw new PqcAgentError("pqcagent/reload-bad-ca",
        "agent.reloadCerts: ca bundle failed to parse: " + (e && e.message));
    }
    throw new PqcAgentError("pqcagent/reload-mismatch",
      "agent.reloadCerts: cert/key mismatch or malformed PEM (" + (e && e.message) + ")");
  }
