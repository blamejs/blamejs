...
function _b64urlDecodeJson(seg) {
  if (!seg) return null;
  try {
    return safeJson.parse(_b64urlToString(seg), { rejectProto: true });
  } catch (_e) {
    return null;
  }
}

function _duplicateKeysOf(seg) {
  if (!seg) return [];
  var json;
  try { json = _b64urlToString(seg); } catch (_e) { return []; }
  try { return guardJson().detectDuplicateKeys(json); } catch (_e) { return []; }
}
...
function _detectIssues(input, opts) {
  ...
  var headerDups = _duplicateKeysOf(headerSeg);
  if (headerDups.length > 0) {
    issues.push({
      kind: "duplicate-key", severity: "critical",
      ruleId: "jwt.duplicate-key",
      snippet: "JWT header repeats parameter " + JSON.stringify(headerDups[0]) +
               ": RFC 7515 §4 requires unique members; a first-wins verifier " +
               "reads a different value than a last-wins one (algorithm-confusion smuggle)",
    });
  }
  ...
  var payloadDups = _duplicateKeysOf(payloadSeg);
  if (payloadDups.length > 0) {
    issues.push({
      kind: "duplicate-key", severity: "critical",
      ruleId: "jwt.duplicate-key",
      snippet: "JWT payload repeats claim " + JSON.stringify(payloadDups[0]) +
               ": RFC 7519 §4 requires unique member names; a first-wins reader " +
               "sees a different claim value than a last-wins one",
    });
  }
  {
...
function kidSafe(kid) {
  ...
  var _kidCtl = codepointClass.firstControlCharOffset(kid, { forbidTab: true });
  if (_kidCtl !== -1) {
