...
function _importIssuerKey(k) {
  ...
  try {
    ...
    var key = nodeCrypto.createPublicKey(k);                  // PEM string / key spec
...
function buildChallenge(opts) {
  ...
  var parts = ['PrivateToken challenge="' + challenge.toString("base64url") + '"'];
  if (opts.tokenKey !== undefined && opts.tokenKey !== null) {
    ...
    parts.push('token-key="' + Buffer.from(spki).toString("base64url") + '"');
  }
