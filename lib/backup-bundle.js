...
class BackupBundleError extends FrameworkError {
  constructor(code, message) {
    super(message, code);
    this.name = "BackupBundleError";
    this.permanent = true;
    this.isBackupBundleError = true;
  }
}
...
async function create(opts) {
  ...
  _ensureDir(outDir);
  _ensureDir(path.join(outDir, "files"));
  ...
  for (var i = 0; i < opts.files.length; i++) {
    ...
    _ensureDir(path.dirname(destFull));
