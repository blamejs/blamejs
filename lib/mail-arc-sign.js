...
var ARC_MAX_COMMENT_DEPTH = 32;

function _skipCfws(s, pos) {
  var n = s.length;
  for (;;) {
    while (pos < n && (s.charCodeAt(pos) === 0x20 || s.charCodeAt(pos) === 0x09)) pos += 1;
    if (pos >= n || s.charCodeAt(pos) !== 0x28) return pos;
    var depth = 0;
    var closed = false;
    while (pos < n) {
      ...
      if (c === 0x28) {
        depth += 1;
        if (depth > ARC_MAX_COMMENT_DEPTH) return -1;
      } else if (c === 0x29) {
        depth -= 1;
        if (depth === 0) { pos += 1; closed = true; break; }
      }
      pos += 1;
    }
    if (!closed) return -1;
...
function _arcHeaderInstance(lcName, value) {
  var s = structuredFields.unfoldHeaderContinuations(value);
  var instance = null;
  var pos = _skipCfws(s, 0);
  if (pos !== -1 && s.charAt(pos) === "i") pos = _skipCfws(s, pos + 1);
  else pos = -1;
  if (pos !== -1 && s.charAt(pos) === "=") pos = _skipCfws(s, pos + 1);
  else pos = -1;
  if (pos !== -1) {
    var start = pos;
    while (pos < s.length && pos - start < 4 &&
           s.charCodeAt(pos) >= 0x30 && s.charCodeAt(pos) <= 0x39) pos += 1;
    var digits = s.slice(start, pos);
    var after = _skipCfws(s, pos);
    if (digits.length >= 1 && digits.length <= 3 && after !== -1 && s.charAt(after) === ";") {
      var n = parseInt(digits, 10);
      if (n >= 1) instance = n;
    }
  }
  if (lcName !== "arc-authentication-results" && dkim._parseTagList(s) === null) {
    return { instance: null, invalid: true };
  }
  return { instance: instance, invalid: false };
}
...
function _arcExtractPriorHops(parsedHeaders) {
  ...
  for (var i = 0; i < parsedHeaders.length; i += 1) {
    ...
    var instance = _arcHeaderInstance(lcName, h.value).instance;
    if (instance === null || instance > ARC_MAX_HOPS_FOR_EXTRACT) continue;
...
function sign(opts) {
  ...
  var canonHeaders = dkim.canonicalizeSignedHeaders(
    amsParsedHeaders, headersToSign, "relaxed").canonicalized;
...
module.exports = {
  ...
  _arcHeaderInstance: _arcHeaderInstance,
};
