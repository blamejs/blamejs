...
function sign(opts) {
  ...
  // authservId / domain / selector are required non-empty strings AND are
  // interpolated verbatim into the ARC header block (ARC-Authentication-
  // Results / ARC-Seal / ARC-Message-Signature), so they can never carry a
  // CR / LF / NUL — that would smuggle a header. (authResults is guarded
  // just below.)
  var arcHeaderFields = [
