...
var PROFILES = Object.freeze({
  ...
  "balanced": {
    ...
    requireMaskPolicy:         "audit-bare-ip",
    family:                    "either",
    maxBytes:                  C.BYTES.bytes(64),
    maxRuntimeMs:              C.TIME.seconds(2),
  },
...
function _ipv6HexPrefixMatch(hex, reservedHex, prefixBits) {
  var fullNibbles = Math.floor(prefixBits / 4);
  if (hex.slice(0, fullNibbles) !== reservedHex.slice(0, fullNibbles)) return false;
  var remBits = prefixBits - fullNibbles * 4;
  if (remBits === 0) return true;
  var mask = (0xF << (4 - remBits)) & 0xF;
  var aNib = parseInt(hex.charAt(fullNibbles), 16);
  var rNib = parseInt(reservedHex.charAt(fullNibbles), 16);
