...
function maskNonStructural(text) {
  ...
  for (var li = 0; li < lines.length; li += 1) {
    ...
    var dashIndent = -1;
    // Was the token just read a quoted scalar? Only a quoted key may take its
    // colon with no space after it, which is YAML's JSON compatibility.
    ...
    while (i < line.length) {
      ...
      var quotedBefore = prevWasQuoted;
      prevWasQuoted = false;
      ...
      if (_isPlainSpace(ch)) {
        masked += ch; i += 1; prevWasQuoted = quotedBefore; continue;
      }
      ...
      if (ch === '"' || ch === "'") {
        ...
        atNodeStart = false;
        // Remembered for the colon rule above: only a QUOTED key may be
        // followed by its colon with no space between.
        prevWasQuoted = true;
        continue;
      }
