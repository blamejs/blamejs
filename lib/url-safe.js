...
function parse(url, opts) {
  ...
  if (url == null || url === "") {
    throw _makeError(errClass, "url-safe/missing", "url is required");
  }
  ...
  if (url instanceof URL) {
    ...
    try {
      ...
      throw _makeError(errClass, "url-safe/malformed", "malformed URL: " + e.message);
    }
  }

  if (allowed.indexOf(parsed.protocol) === -1) {
    throw _makeError(errClass, "url-safe/protocol-disallowed",
