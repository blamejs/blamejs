...
function _checkSchema(schema, root, isRoot, checked) {
  ...
  if (Object.prototype.hasOwnProperty.call(schema, "definitions")) {
    ...
    Object.keys(schema.definitions).forEach(function (k) { _checkSchema(schema.definitions[k], root, false, checked); });
  }
  ...
  if ("elements" in schema) _checkSchema(schema.elements, root, false, checked);
  if ("values" in schema) _checkSchema(schema.values, root, false, checked);
  if ("properties" in schema || "optionalProperties" in schema) {
    ...
    Object.keys(props).forEach(function (k) { if (Object.prototype.hasOwnProperty.call(opt, k)) throw new JtdError("jtd/bad-schema", "jtd: '" + k + "' in both properties and optionalProperties"); _checkSchema(props[k], root, false, checked); });
    Object.keys(opt).forEach(function (k) { _checkSchema(opt[k], root, false, checked); });
  ...
  if ("discriminator" in schema) {
    ...
    Object.keys(schema.mapping).forEach(function (k) {
      ...
      _checkSchema(sub, root, false, checked);
