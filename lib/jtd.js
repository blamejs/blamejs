...
function _checkSchema(schema, root, isRoot, checked, active) {
  ...
  if (active.has(schema)) throw new JtdError("jtd/bad-schema", "jtd: the schema object graph contains a cycle");
  if (checked.has(schema)) return;
  active.add(schema);
  _checkSchemaNode(schema, root, checked, active);
  active.delete(schema);
  checked.add(schema);
}

function _checkSchemaNode(schema, root, checked, active) {
  ...
  if (Object.prototype.hasOwnProperty.call(schema, "definitions")) {
    ...
    Object.keys(schema.definitions).forEach(function (k) { _checkSchema(schema.definitions[k], root, false, checked, active); });
  }
  ...
  if ("elements" in schema) _checkSchema(schema.elements, root, false, checked, active);
  if ("values" in schema) _checkSchema(schema.values, root, false, checked, active);
  if ("properties" in schema || "optionalProperties" in schema) {
    ...
    Object.keys(props).forEach(function (k) { if (Object.prototype.hasOwnProperty.call(opt, k)) throw new JtdError("jtd/bad-schema", "jtd: '" + k + "' in both properties and optionalProperties"); _checkSchema(props[k], root, false, checked, active); });
    Object.keys(opt).forEach(function (k) { _checkSchema(opt[k], root, false, checked, active); });
  ...
  if ("discriminator" in schema) {
    ...
    Object.keys(schema.mapping).forEach(function (k) {
      ...
      _checkSchema(sub, root, false, checked, active);
