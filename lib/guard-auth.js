 ...
 *   oauthMaxBytes:     number,       // guardOauth's own flow cap (see below)
 *
 * Every `b.guardOauth` option is forwarded to it when an `oauthFlow` is
 * present — `seenCodeStore` and `codeReusePolicy` included, which the
 * code-replay check needs to be satisfiable through this wrapper:
 *
 *   pkcePolicy, statePolicy, redirectUriPolicy, responseTypePolicy,
 *   scopeTamperingPolicy, issuerOnCallbackPolicy, codeReusePolicy,
 *   allowedRedirectUris, allowedResponseTypes, seenCodeStore, maxParamBytes,
 *   compliancePosture
 *
 * The exception is `maxBytes`, which both guards define and mean differently:
 * here it caps the whole auth bundle, in `b.guardOauth` it caps the flow. Pass
 * `oauthMaxBytes` to set the child's; the two are independent and both apply.
 *
 * @example
