...
// compliancePosture is deliberately ABSENT. resolveOpts has already collapsed
// it into childProfile — and an explicit childProfile outranks it there — so
// forwarding the posture too would have the child resolve it a second time and
// overwrite the profile the operator asked for. The posture still reaches the
// child, as the childProfile it resolved to.
 ...
 *
 * Two names mean different things on each side and so are handled explicitly.
 * `maxBytes` caps the whole auth bundle here and the flow in `b.guardOauth`;
 * pass `oauthMaxBytes` for the child's, and both apply independently.
 * `compliancePosture` is resolved HERE into `childProfile`, where an explicit
 * `childProfile` outranks it, so it is not forwarded a second time — the
 * posture reaches the child as the profile it resolved to.
 *
 * @opts
