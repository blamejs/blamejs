 ...
 *
 * No-MVP scope (locked v1):
 *
 *   - Single-flight wrap (concurrent calls collapse)
 *   - Stale-while-revalidate opt-in
 *   - LRU eviction on memory backend
 *   - Cluster backend with shared table
 *   - Custom backend escape hatch
 *
 * Out of scope (intentional, not deferred-and-forgotten):
 *
 *   - Distributed pubsub invalidation across nodes (cluster backend
 *     gives always-fresh; memory caches go stale-on-other-nodes by design)
 *   - Tag-based invalidation
 *   - Compression / custom serializer for cluster backend
 *   - Read-through built into the backend (wrap is the operator's
 *     read-through)
 */
