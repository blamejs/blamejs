...
function verify(msg, opts) {
  ...
  if (opts.requiredComponents !== undefined && opts.requiredComponents !== null) {
    requiredComponents = opts.requiredComponents.map(function (c) { return c.split(";")[0].toLowerCase(); });  // allow:bare-split-on-quoted-header-token-grammar — operator-supplied component-ids, same token grammar as covered
  } else {
  ...
  for (var rci = 0; rci < requiredComponents.length; rci += 1) {
    if (coveredLower.indexOf(requiredComponents[rci]) === -1) missingComponents.push(requiredComponents[rci]);
  }
