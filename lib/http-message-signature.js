...
// _sfString / _sfList / _sfDict — minimal Structured Fields (RFC 9651)
// formatters scoped to what RFC 9421 needs. Full RFC 9651 is overkill
...
function _sfQuotedString(s) {
  // RFC 9651 §3.3.3 — escape DQUOTE and backslash. Invalid bytes (any
  // byte outside 0x20..0x7E) refuse to encode rather than silently lose
  // information.
  for (var i = 0; i < s.length; i++) {
    var c = s.charCodeAt(i);
    if (c < 0x20 || c > 0x7E) {                                                  // RFC 9651 §3.3.3 printable-ASCII range
...
// _parseSignatureInput — minimal RFC 9651 dictionary parser scoped to
// what RFC 9421 emits. A full RFC 9651 parser is overkill here.
function _parseSignatureInput(headerValue) {
  ...
  while (i2 < coveredRaw.length) {
    ...
    // for backslash-escapes per RFC 9651.
    var qStart = i2 + 1;
    var qEnd = qStart;
    ...
    // Single-pass RFC 9651 §3.3.3 unescape — NOT two chained .replace() passes,
  ...
  if (paramsRaw.length > 0) {
    // RFC 9421 §2.3 + RFC 9651 §3.1.2 — parameter values may be
    // sf-string. A bare `paramsRaw.split(";")` would slice through a
    // legitimate `;tag="x;y"` parameter. Quote-aware splitter
    // mirrors cdn-cache-control._splitTopLevelCommas (RFC 9651
...
function _parseSignature(headerValue, label) {
  ...
  if (headerValue.indexOf(prefix) !== 0) {
    ...
    // matching label.
    var parts = headerValue.split(",");                                                        // allow:bare-split-on-quoted-header-token-grammar — RFC 9421 §2.4 Signature header values are `label=:b64:` form; base64 alphabet excludes `,` and the label tokens are RFC 9651 §3.3.4 sf-token (no DQUOTE in practice)
...
function verify(msg, opts) {
  ...
  // RFC 9421 §2.4 carries the signature as an RFC 9651 §3.3.5 sf-binary — the
