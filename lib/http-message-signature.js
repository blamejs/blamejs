...
function _parseSignatureInput(headerValue) {
  ...
  while (i2 < coveredRaw.length) {
    ...
    i2 = qEnd + 1;
    // Optional ;param=value;param=... suffix immediately following. The scan
    // must be quote-aware: an sf-string parameter value (e.g. ;name="my key")
    // may contain interior whitespace, which is NOT a token separator. Only a
    // top-level (unquoted) whitespace ends this covered component — a bare
    // non-whitespace run would truncate ;name="my key" at the interior space
    // and the signature base could no longer be rebuilt. Backslash-escapes
    // inside the quoted value are skipped, mirroring the bare-name scan above.
    var suffixStart = i2;
    var inSuffixQuote = false;
    while (i2 < coveredRaw.length) {
      var sCh = coveredRaw.charAt(i2);
      if (inSuffixQuote) {
        if (sCh === "\\" && i2 + 1 < coveredRaw.length) { i2 += 2; continue; }
        if (sCh === "\"") inSuffixQuote = false;
        i2++;
      } else {
        if (/\s/.test(sCh)) break;
        if (sCh === "\"") inSuffixQuote = true;
        i2++;
      }
    }
