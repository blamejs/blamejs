...
function _parseSignatureInput(headerValue) {
  ...
  if (paramsRaw.length > 0) {
    ...
    var paramParts = _splitTopLevelSemis(paramsRaw);
    for (var j = 0; j < paramParts.length; j++) {
      ...
      if (vv.charAt(0) === "\"" && vv.charAt(vv.length - 1) === "\"") {
        params[k] = vv.slice(1, -1).replace(/\\"/g, "\"").replace(/\\\\/g, "\\");
      } else {
        var num = Number(vv);
...
// Quote-aware top-level `;` splitter — mirrors cdn-cache-control's
// _splitTopLevelCommas. RFC 8941 §3.3.3 quoted-string state with
// backslash-escape so an sf-string parameter value containing `;`
// doesn't split mid-value.
