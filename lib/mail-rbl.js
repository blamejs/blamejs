...
function _expandIpv6(ip) {
  // RFC 4291 §2.2 — split on '::' for the compressed-zero run.
  var parts = ip.split("::");
  if (parts.length > 2) return null;
  var left  = parts[0].length ? parts[0].split(":") : [];
  var right = parts.length === 2 ? (parts[1].length ? parts[1].split(":") : []) : [];
  if (parts.length === 1 && left.length !== 8) return null;                                              // allow:raw-byte-literal — IPv6 group count
  var missing = 8 - left.length - right.length;                                                          // allow:raw-byte-literal — group count
  if (missing < 0) return null;
  var middle = [];
  for (var i = 0; i < missing; i += 1) middle.push("0");
  var groups = left.concat(middle).concat(right);
  if (groups.length !== 8) return null;                                                                  // allow:raw-byte-literal — group count
  var hex = "";
  for (var g = 0; g < groups.length; g += 1) {
    var gv = groups[g].toLowerCase();
    if (gv.length === 0 || gv.length > 4) return null;                                                   // allow:raw-byte-literal — IPv6 group max 4 hex chars
    for (var hc = 0; hc < gv.length; hc += 1) {
      var cp = gv.charCodeAt(hc);
      var isDigit = cp >= 0x30 && cp <= 0x39;                                                            // allow:raw-byte-literal — ASCII '0'..'9'
      var isLowerHex = cp >= 0x61 && cp <= 0x66;                                                         // allow:raw-byte-literal — ASCII 'a'..'f' (already lowercased above)
      if (!isDigit && !isLowerHex) return null;
    }
    hex += gv.padStart(4, "0");                                                                          // allow:raw-byte-literal — 4 hex chars per IPv6 group
  }
  return hex;
}
