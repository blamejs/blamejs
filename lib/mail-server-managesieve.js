  ...
  function _handleGetScript(state, socket, parsed) {
    ...
    return Promise.resolve()
      ...
      .then(function (script) {
        ...
        // literal followed by the body and CRLF, then OK. The count is
        // announced on the line above, so the client reads exactly that many
        // octets and then expects the CRLF; making it conditional on what the
        // body ends with serves a CRLF-terminated script unclosed, and the
        // client reads the OK line where the terminator belongs.
        socket.write("{" + bytes + "}\r\n");
        socket.write(body);
        socket.write("\r\n");
