...
function create(opts) {
  ...
  function _handleStartTls(state, socket) {
    ...
    // RFC 5804 §2.2 + CVE-2021-33515 / CVE-2021-38371 class —
    // discard any bytes the client queued before the upgrade so
    // pre-handshake injection can't survive into the post-TLS
    // session.
    state.lineBuffer = Buffer.alloc(0);
    state.pendingLiteral = null;
