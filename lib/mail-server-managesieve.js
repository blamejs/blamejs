...
function create(opts) {
  ...
  function _drainBuffer(state, socket) {
    ...
    // not-running branch. Commands queue while a handler runs, a store call or
    // a credential check, and a client that sends faster than they complete
    // would grow this buffer for as long as it liked. A per-line cap says
    // nothing about how many lines are held at once.
    //
    // Only the literal's OUTSTANDING octets are exempt, not the whole buffer.
    // Exempting a transfer entirely let a client put a literal opener and then
    // any amount of pipelined commands in one write: the bound was skipped for
    // all of it, and once the literal was consumed the pump continued through
    // the queue from inside `step()` without passing here again.
    var outstanding = 0;
    if (state.pendingLiteral) {
      outstanding = Math.max(0, state.pendingLiteral.size - state.pendingLiteral.body.length);
    } else if (state.pendingAuth && state.pendingAuth.irBytes !== null &&
               state.pendingAuth.irBytes !== undefined) {
