...
function create(opts) {
  ...
  function _drainBuffer(state, socket) {
    ...
    function step() {
      ...
      if (state.pendingLiteral) {
        ...
        var discarding = pl.discard === true;
        var have = discarding ? pl.discarded : pl.body.length;
        var need = pl.size - have;
        if (state.lineBuffer.length < need) {
          if (discarding) pl.discarded = have + state.lineBuffer.length;
          else pl.body = Buffer.concat([pl.body, state.lineBuffer]);
          state.lineBuffer = Buffer.alloc(0);
          return done();
        }
        if (discarding) pl.discarded = have + need;
        else pl.body = Buffer.concat([pl.body, state.lineBuffer.subarray(0, need)]);
        state.lineBuffer = state.lineBuffer.subarray(need);
        _consumeLiteralTerminator(state);
        state.pendingLiteral = null;
        _clearLiteralDeadline(state);
        if (discarding) return step();
  ...
  function _handleLine(state, socket, line) {
    ...
    try {
      ...
      if (result && typeof result.then === "function") {
        return result.then(
          function () { _discardRefusedLiteral(state, socket, parsed); },
          function (e) {
            try {
            ...
            _discardRefusedLiteral(state, socket, parsed);
          }
        );
      }
      _discardRefusedLiteral(state, socket, parsed);
  ...
  function _discardRefusedLiteral(state, socket, parsed) {
    ...
    if (!parsed || parsed.literalBytes === null || parsed.literalBytes === undefined) return;
    if (!parsed.literalPlus) return;
    if (state.pendingLiteral || state.pendingAuth) return;
    if (parsed.literalBytes === 0) return;
    _emit("mail.server.managesieve.refused_literal_discarded",
      { connectionId: state.id, verb: parsed.verb, size: parsed.literalBytes }, "denied");
    state.pendingLiteral = {
      verb:      parsed.verb,
      name:      null,
      size:      parsed.literalBytes,
      plus:      true,
      body:      Buffer.alloc(0),
      discard:   true,
      discarded: 0,
