  ...
  return new Promise(function (resolve, reject) {
    ...
    session.once("error", function (err) {
      ...
      networkTls().annotateOutboundFailure(err, {
        ...
        minVersion: connectOpts.minVersion,
        ecdhCurve:  connectOpts.ecdhCurve,
      });
      _fail(err);
    });
  });
 ...
 * <code>b.auth.saml.fetchMdq</code>) route it through here. An empty or absent
 * pin returns the client unchanged.
 *
 * @example
...
function pinnedClient(client, allowedHosts) {
  if (!Array.isArray(allowedHosts) || allowedHosts.length === 0) return client;
  return {
    request: function (opts) {
      var target = opts && opts.url;
      var host;
      // Through safeUrl so the destination is read the way the rest of the
      // framework reads one — obfuscated host spellings collapse and a
      // non-http scheme is refused outright. Both schemes are permitted here
      // because this is the host pin, not the scheme policy: the client this
      // wraps applies its own, and a caller running against a loopback issuer
      // legitimately dials http.
      try { host = safeUrl.parse(String(target), { allowedProtocols: safeUrl.ALLOW_HTTP_ALL }).hostname; }
      catch (_e) { host = null; }
      // An unparseable destination cannot be shown to satisfy the pin, so it
      // is refused rather than passed through unchecked.
      if (host === null || !hostAllowed(host, allowedHosts, opts && opts.method)) {
        return Promise.reject(_makeError(opts && opts.errorClass, "HOST_DISALLOWED",
          "host '" + (host === null ? String(target) : host) +
          "' not in allowedHosts (method=" +
          String((opts && opts.method) || "GET").toUpperCase() + ")", true));
      }
      // Redirect following is turned off on the way through: a hop this
      // wrapper never sees is a hop the pin never checked. Set on a copy so a
      // caller's own request object is not mutated.
  ...
  return new Promise(function (resolve, reject) {
    ...
    req.on("error", function (e) {
      ...
      networkTls().annotateOutboundFailure(e, {
        ...
        minVersion: agentTls.minVersion,
        ecdhCurve:  agentTls.ecdhCurve,
      });
