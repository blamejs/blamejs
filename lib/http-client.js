...
function _validateDownloadOpts(opts) {
  ...
  if (typeof opts.url !== "string" || opts.url.length === 0) {
    throw _hcErr("httpclient/bad-opts", "downloadStream: url must be a non-empty string");
  }
  if (typeof opts.dest !== "string" || opts.dest.length === 0) {
    throw _hcErr("httpclient/bad-opts", "downloadStream: dest must be a non-empty string");
  }
  if (opts.hash !== undefined &&
      (typeof opts.hash !== "string" || ALLOWED_DOWNLOAD_HASH_ALGS.indexOf(opts.hash) === -1)) {
  ...
  if (opts.expected !== undefined &&
      (typeof opts.expected !== "string" || opts.expected.length === 0 ||
       !/^[0-9a-fA-F]+$/.test(opts.expected))) {
    throw _hcErr("httpclient/bad-opts",
      "downloadStream: expected must be a non-empty hex digest");
  }
  if (opts.timeoutMs !== undefined &&
      (typeof opts.timeoutMs !== "number" || !isFinite(opts.timeoutMs) || opts.timeoutMs <= 0)) {
    throw _hcErr("httpclient/bad-opts",
      "downloadStream: timeoutMs must be a positive finite number");
  }
...
function _validateUploadOpts(opts) {
  ...
  if (typeof opts.url !== "string" || opts.url.length === 0) {
    throw _hcErr("httpclient/bad-opts", "uploadMultipartStream: url must be a non-empty string");
  }
  if (!opts.file || typeof opts.file !== "object") {
    throw _hcErr("httpclient/bad-opts", "uploadMultipartStream: file must be an object");
  }
  if (typeof opts.file.path !== "string" || opts.file.path.length === 0) {
    throw _hcErr("httpclient/bad-opts", "uploadMultipartStream: file.path must be a non-empty string");
  }
  if (typeof opts.file.fieldName !== "string" || opts.file.fieldName.length === 0) {
    throw _hcErr("httpclient/bad-opts", "uploadMultipartStream: file.fieldName must be a non-empty string");
  }
  ...
  if (opts.timeoutMs !== undefined &&
      (typeof opts.timeoutMs !== "number" || !isFinite(opts.timeoutMs) || opts.timeoutMs <= 0)) {
    throw _hcErr("httpclient/bad-opts",
      "uploadMultipartStream: timeoutMs must be a positive finite number");
  }
  if (opts.maxBytes !== undefined &&
      (typeof opts.maxBytes !== "number" || !isFinite(opts.maxBytes) || opts.maxBytes <= 0 ||
       Math.floor(opts.maxBytes) !== opts.maxBytes)) {
