...
//
// This is deliberately not _dropAllTransports(): that force-destroys, which
// is right when the caller has decided the transports are unwanted, and wrong
...
// and let existing work finish. Idle keep-alive sockets are closed (they would
// otherwise linger holding the old posture), and an h2 session is closed
// gracefully — close() refuses new streams while letting open ones complete.
function _retireTransportsForPostureChange() {
  _transports.forEach(function (t) {
    ...
    if (t && t.kind === "h2" && t.session) _drainH2Session(t.session);
  });
  _transports.clear();
}
...
function _getTransport(u, opts, ips) {
  ...
  promise.then(
    function (t) {
      ...
      if (_transports.get(key) === promise) _transports.set(key, t);
    },
    function (_err) {
