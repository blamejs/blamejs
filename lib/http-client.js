...
async function downloadStream(opts) {
  ...
  try {
    // codeql[js/insecure-temporary-file] tmpPath is `dest + ".tmp-" +
    // bCrypto.generateToken(8 bytes)` (line 1802) — the 16-hex-char
    // random suffix makes the path non-predictable; tmpPath lives in
    // dest's operator-chosen parent dir, not os.tmpdir. The fd is
    // opened for fsync-then-close only.
