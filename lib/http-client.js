...
function _rejectStreamHttpError(stream, errorClass, statusCode, statusMessage, reject) {
  ...
  // Collect at most `cap` bytes of the error body — slice each chunk to the
  // remaining room so the bounded collector never overflows; the rest drains.
  stream.on("data", function (c) {
    var got = collector.bytesCollected();
    if (done || got >= cap) return;
    var room = cap - got;
    try { collector.push(c.length > room ? c.subarray(0, room) : c); }
    catch (_e) { /* cap reached — ignore the remainder */ }
  });
