...
function create(opts) {
  ...
  // The failure state must outlive the longest lockout it can trigger (plus the
  // decay window) so a follow-up failure still finds the lockNumber and a check
  // during the lockout still sees the lock. Passed as a DURATION the cache
  // resolves with its OWN clock — an absolute expiry computed from the
  // injectable lockout clock would not survive a cache on a different clock. A
  // function ladder is unbounded, so cap it generously enough to cover any
  // realistic lockout; the decay logic handles logical staleness either way.
  var _maxStateTtlMs = (typeof lockoutDurations === "function"
    ? C.TIME.days(30)
    : lockoutDurations[lockoutDurations.length - 1]) + windowMs;
  ...
  // across nodes (each with its own in-process ordering) lose increments and a
  // brute-force attacker spread across nodes stays under the lockout threshold.
  // cache.update runs the whole decision under a compare-and-set (with retry on
  // the cluster backend), so every failure is counted. The mutator is pure (it
  // may re-run on a CAS retry) — it only computes the next state and records the
  // outcome for the post-commit audit/observability emits below.
