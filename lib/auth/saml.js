...
function create(opts) {
  ...
  function verifyResponse(samlResponseB64, vopts) {
    ...
    for (var i = 0; i < confirmations.length; i++) {
      ...
      if (method === "urn:oasis:names:tc:SAML:2.0:cm:holder-of-key") {
        ...
        if (!noaHok) continue;
        var noaHokSec = Date.parse(noaHok) / 1000;
        if (!isFinite(noaHokSec) || noaHokSec < nowSec - clockSkewSec) continue;
        if (nbHok) {
          var nbHokSec = Date.parse(nbHok) / 1000;
          if (!isFinite(nbHokSec) || nbHokSec > nowSec + clockSkewSec) continue;
        }
      ...
      if (!isFinite(t) || t < nowSec - clockSkewSec) continue;
   ...
   *   allowUnsigned:      boolean,            // default false — the parse refuses when no idpVerifyKey is configured; set true to accept an unsigned message
   *
   * @example
  ...
  function parseLogoutRequest(samlRequestB64, vopts) {
    ...
    if (_verifyConfig(vopts.idpVerifyKey, vopts.idpVerifyAlg, "parseLogoutRequest", vopts.allowUnsigned)) {
  ...
  function parseLogoutResponse(samlResponseB64, vopts) {
    ...
    if (_verifyConfig(vopts.idpVerifyKey, vopts.idpVerifyAlg, "parseLogoutResponse", vopts.allowUnsigned)) {
      if (typeof vopts.queryString !== "string") {
        throw new AuthError("auth-saml/no-query-string",
   ...
   *
   * @opts
   *   idpVerifyKey: Uint8Array,                      // verify the embedded XMLDSig signature against this IdP key
  ...
  function parseLogoutRequestPost(samlRequestB64, vopts) {
    ...
    if (_verifyConfig(vopts.idpVerifyKey, vopts.idpVerifyAlg, "parseLogoutRequestPost", vopts.allowUnsigned)) {
   ...
   *     idpVerifyKey: idpPublicKey, idpVerifyAlg: "ml-dsa-65",
   *   });
   */
  ...
  function parseLogoutResponseSoap(soapXml, vopts) {
    ...
    if (_verifyConfig(vopts.idpVerifyKey, vopts.idpVerifyAlg, "parseLogoutResponseSoap", vopts.allowUnsigned)) {
...
function _verifyConfig(idpVerifyKey, idpVerifyAlg, ctx, allowUnsigned) {
  if (!idpVerifyKey && !idpVerifyAlg) {
    if (allowUnsigned === true) return false;
    throw new AuthError("auth-saml/slo-verification-required",
      ctx + ": no idpVerifyKey configured — an inbound Single-Logout message " +
      "must be signature-verified by default (a forged unsigned LogoutRequest " +
      "otherwise force-logs-out any user). Configure idpVerifyKey + idpVerifyAlg, " +
      "or pass allowUnsigned:true to accept unsigned messages (not recommended).");
  }
  if (!idpVerifyKey) {
