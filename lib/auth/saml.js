...
function create(opts) {
  ...
  function buildLogoutRequest(bopts) {
    ...
    var deflated = zlib2.deflateRawSync(Buffer.from(xml, "utf8"));
  ...
  function parseLogoutRequest(samlRequestB64, vopts) {
    ...
    try {
      ...
      xml = zlib2.inflateRawSync(deflated, { maxOutputLength: 1024 * 1024 }).toString("utf8");        // allow:raw-byte-literal — 1 MiB max SAMLRequest decompressed
    } catch (e) {
...
function _sigAlgUrn(alg) {
  ...
  var nodeCrypto2 = require("node:crypto");
  if (alg === "ml-dsa-65") {
    return {
