...
function create(opts) {
  ...
  function buildLogoutRequest(bopts) {
    ...
    if (bopts.signingKey || bopts.signingAlg) {
      if (!(bopts.signingKey instanceof Uint8Array)) {
        throw new AuthError("auth-saml/bad-signing-key",
          "buildLogoutRequest: signingKey must be a Uint8Array");
      }
      var sigAlgUrn = _sigAlgUrn(bopts.signingAlg);
      if (!sigAlgUrn) {
        throw new AuthError("auth-saml/bad-signing-alg",
          "buildLogoutRequest: signingAlg must be 'ml-dsa-65' / 'ml-dsa-87' / 'ed25519'");
      }
  ...
  function buildLogoutResponse(bopts) {
    ...
    if (bopts.signingKey || bopts.signingAlg) {
      if (!(bopts.signingKey instanceof Uint8Array)) {
        throw new AuthError("auth-saml/bad-signing-key",
          "buildLogoutResponse: signingKey must be a Uint8Array");
      }
      var sigAlgUrn = _sigAlgUrn(bopts.signingAlg);
      if (!sigAlgUrn) {
        throw new AuthError("auth-saml/bad-signing-alg",
          "buildLogoutResponse: signingAlg must be 'ml-dsa-65' / 'ml-dsa-87' / 'ed25519'");
      }
      query += "&SigAlg=" + encodeURIComponent(sigAlgUrn.urn);
      var sigBytes = sigAlgUrn.sign(Buffer.from(query, "utf8"), bopts.signingKey);
      query += "&Signature=" + encodeURIComponent(Buffer.from(sigBytes).toString("base64"));
  ...
  function parseLogoutResponse(samlResponseB64, vopts) {
    ...
    try {
      ...
      xml = zlib2.inflateRawSync(deflated, { maxOutputLength: 1024 * 1024 }).toString("utf8");        // allow:raw-byte-literal — 1 MiB max SAMLResponse decompressed
    } catch (e) {
   ...
   *
   * @example
   *   var lr = sp.buildLogoutRequestSoap({...});
   *   var resp = await b.httpClient.request(lr.action, {
   *     method: "POST", body: lr.body,
...
// XMLEnc Algorithm URIs we support. Symmetric content encryption:
//   - http://www.w3.org/2009/xmlenc11#aes128-gcm   (XMLEnc 1.1 §5.2.4)
//   - http://www.w3.org/2009/xmlenc11#aes256-gcm
//   - urn:ietf:params:xml:ns:xmlenc:xchacha20-poly1305  (framework default)
// Asymmetric key transport:
//   - http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p   (RFC 4055)
//   - urn:ietf:params:xml:ns:xmlenc:ml-kem-1024         (framework PQC)
function _decryptEncryptedAssertion(encAssertion, spPrivateKeyPem) {
  ...
  if (keyAlg === "http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p" ||
  ...
  } else if (keyAlg === "urn:ietf:params:xml:ns:xmlenc:ml-kem-1024") {
    ...
    var bCryptoLocal = require("../crypto");
    try {
      cek = bCryptoLocal.decryptEnvelope({
        envelope:   wrappedKey,
    ...
    throw new AuthError("auth-saml/encrypted-unsupported-key-alg",
      ...
      "(supported: rsa-oaep-mgf1p, xmlenc11#rsa-oaep, framework ml-kem-1024)");
  }
  ...
  if (contentAlg === "http://www.w3.org/2009/xmlenc11#aes128-gcm" ||
  ...
  } else if (contentAlg === "urn:ietf:params:xml:ns:xmlenc:xchacha20-poly1305") {
    ...
    var bCryptoLocal2 = require("../crypto");
    try {
      clearBytes = bCryptoLocal2.aeadDecrypt({
    ...
    throw new AuthError("auth-saml/encrypted-unsupported-content-alg",
      ...
      "(supported: xmlenc11#aes128-gcm, xmlenc11#aes256-gcm, framework xchacha20-poly1305)");
  }
...
// Standard XMLDSig vocabulary doesn't enumerate ML-DSA / SLH-DSA yet;
// the framework uses framework-namespaced URNs that operators verify
// against the documented spec for each alg.
var PQC_XMLDSIG_SIG = {
  "urn:ietf:params:xml:ns:saml:bindings:xmldsig:sig-alg:ml-dsa-65": "ml-dsa-65",
  "urn:ietf:params:xml:ns:saml:bindings:xmldsig:sig-alg:ml-dsa-87": "ml-dsa-87",
  "http://www.w3.org/2021/04/xmldsig-more#ed25519":                 "ed25519",
};
function _pqcXmldsigUriFor(alg) {
  if (alg === "ml-dsa-65") return "urn:ietf:params:xml:ns:saml:bindings:xmldsig:sig-alg:ml-dsa-65";
  if (alg === "ml-dsa-87") return "urn:ietf:params:xml:ns:saml:bindings:xmldsig:sig-alg:ml-dsa-87";
  if (alg === "ed25519")   return "http://www.w3.org/2021/04/xmldsig-more#ed25519";
  return null;
}
...
function _embedXmlDsig(bodyXml, refId, signingKey, signingAlg) {
  ...
  // Pipeline:
  //   1. exclusive-c14n + SHA3-512 digest the LogoutRequest element
  //      (no Signature child yet — enveloped-signature transform).
  //   2. Build the SignedInfo with that digest in the Reference.
  //   3. exclusive-c14n the SignedInfo and PQC-sign that.
  //   4. Emit the ds:Signature element inside the LogoutRequest root.
  if (!(signingKey instanceof Uint8Array)) {
    throw new AuthError("auth-saml/bad-signing-key",
      "_embedXmlDsig: signingKey must be a Uint8Array");
  }
  ...
  var sigMethodUri = _pqcXmldsigUriFor(signingAlg);
  var digestMethodUri = "http://www.w3.org/2007/05/xmldsig-more#sha3-512";
  var c14n = xmlC14n();
  var refDigest = nodeCrypto.createHash("sha3-512").update(c14n.canonicalize(c14n.parse(bodyXml))).digest();
  var signedInfo =
...
function _verifyEmbeddedXmlDsig(xml, idpVerifyKey, idpVerifyAlg, expectedRootLocal) {
  ...
  var expectedSigUri = _pqcXmldsigUriFor(idpVerifyAlg);
  var c14n = xmlC14n();
  var root = c14n.parse(xml);
  var rootLocal = root.name.split(":").pop();
  ...
  if (!PQC_XMLDSIG_SIG[sigUri]) {
    throw new AuthError("auth-saml/unsupported-sig-alg",
      "_verifyEmbeddedXmlDsig: SignatureMethod " + sigUri + " not in PQC allowlist");
  }
  var refNode = _findChild(signedInfo, "Reference");
  if (!refNode) {
  ...
  var rootForDigest = JSON.parse(JSON.stringify(root));
...
function _sigAlgUrn(alg) {
  if (alg === "ml-dsa-65") {
    return {
      urn:    "urn:ietf:params:xml:ns:saml:bindings:redirect:sig-alg:ml-dsa-65",
  ...
  if (alg === "ml-dsa-87") {
    return {
      urn:    "urn:ietf:params:xml:ns:saml:bindings:redirect:sig-alg:ml-dsa-87",
  ...
  if (alg === "ed25519") {
    return {
      ...
      sign:   function (bytes, sk) {
        // sk is operator-supplied — accept either a raw 32-byte
        // Uint8Array or a node:crypto KeyObject for PEM keys. Wrap
        // raw into KeyObject for nodeCrypto.sign.
