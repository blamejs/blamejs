 ...
 * @since     0.8.62
 * @status    stable
 * @related   b.auth.sdJwtVc.issuer, b.auth.oauth.create
 *
 ...
 *
 * @example
 *   var sdJwtIssuer = b.auth.sdJwtVc.issuer.create({...});
...
function create(opts) {
   ...
   * @signature issuer.createCredentialOffer({ subject, credentialIds, txCode? })
   * @since     0.8.62
   *
   ...
   * @signature issuer.exchangePreAuthorizedCode({ preAuthCode, txCode? })
   * @since     0.8.62
   *
   ...
   * @signature issuer.issueCredential({ accessToken, credentialIdentifier, format, vct, proof, claims, selectivelyDisclosed?, ttlMs? })
   * @since     0.8.62
   *
