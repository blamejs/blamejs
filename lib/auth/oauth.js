...
function create(opts) {
  ...
  async function _postForm(endpoint, body, extraHeaders) {
    var hc = httpClient;
    var req = {
      url:     endpoint,
      method:  "POST",
   ...
   *
   * @opts
   *   scope:                   string[] | string,   // scope for this token (default: none — M2M sends no scope)
   *   tokenEndpointAuthMethod: string,              // "client_secret_post" (default) | "client_secret_basic"
   *
   * @example
  ...
  async function clientCredentials(ccopts) {
    ...
    // for client_credentials, so only an explicitly-supplied scope is sent.
    var reqScope = _scopeParam(ccopts.scope === undefined ? null : ccopts.scope);
    if (reqScope) body.set("scope", reqScope);
    // RFC 6749 §2.3.1 client authentication: client_secret_post carries the
    // credentials in the body; client_secret_basic in an HTTP Basic header.
    var authMethod = ccopts.tokenEndpointAuthMethod || tokenEndpointAuthMethod;
    var extraHeaders = null;
    if (authMethod === "client_secret_basic") {
      extraHeaders = { Authorization: "Basic " + Buffer.from(clientId + ":" + clientSecret, "utf8").toString("base64") };
    } else {
      body.set("client_id",     clientId);
      body.set("client_secret", clientSecret);
    }
    var raw = await _postForm(endpoint, body, extraHeaders);
