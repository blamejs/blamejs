...
// (`locations` / `actions` / `datatypes`) exceed the requested set, is
// refused. This is the client-side mirror of the AS-side subset rule
// (RFC 9396 §6.3) and defends against an upstream privilege-escalation.
...
// (RFC 9396 §2.1 — locations / actions / datatypes are the registered
// array-valued common data fields).
var RAR_SUBSET_FIELDS = Object.freeze(["locations", "actions", "datatypes"]);
 ...
 *     algorithm?:          string,         // JWS alg (default: ES256)
...
function buildClientAttestation(aopts) {
  ...
  var alg = aopts.algorithm || "ES256";
  if (ATTESTATION_ALGS.indexOf(alg) === -1) {
    throw new OAuthError("auth-oauth/attestation-alg-not-accepted",
      "buildClientAttestation: alg '" + alg + "' is not an accepted attestation algorithm");
  }
...
function buildClientAttestationPop(popts) {
  ...
  var alg = popts.algorithm || "ES256";
  if (ATTESTATION_ALGS.indexOf(alg) === -1) {
    throw new OAuthError("auth-oauth/attestation-alg-not-accepted",
      "buildClientAttestationPop: alg '" + alg + "' is not an accepted attestation algorithm");
  }
 ...
 *
 * Returns `{ clientId, cnfJwk, attestation, pop }` on success; throws a
 * typed `OAuthError` on any failure. Opt-in / additive — an AS that
 * doesn't accept attestation-based auth never calls it.
 *
 * @opts
 *   {
 ...
 *     seenJti?:           function,  // (jti, iat) → truthy when UNSEEN (atomic)
 *   }
 *
 * @example
 *   var v = b.auth.oauth.verifyClientAttestation(
 ...
 */
function verifyClientAttestation(attestationJwt, popJwt, vopts) {
  vopts = vopts || {};
  validateOpts(vopts, [
  ...
  // returns truthy when the jti was UNSEEN (first sighting). Hot dep —
  // a thrown callback is surfaced as a typed error, not swallowed.
  if (typeof vopts.seenJti === "function") {
    var unseen;
    try { unseen = vopts.seenJti(pp.jti, pp.iat); }
    catch (e) {
