...
var log = createLogger("auth.oauth");
...
var PRESETS = Object.freeze({
  ...
  // generic — operator passes everything manually.
});
...
var DEFAULT_DISCOVERY_CACHE_MS = 60 * 60 * 1000;
var DEFAULT_CLOCK_SKEW_MS      = 60 * 1000;
...
function create(opts) {
  ...
  var discoveryCache = null;       // { config, expiresAt }
  var jwksCache = null;            // { keys, expiresAt }
  ...
  async function _discover() {
    ...
    discoveryCache = { config: config, expiresAt: now + discoveryCacheMs };
    return config;
  }
  ...
  async function _getJwks() {
    ...
    jwksCache = { keys: jwks.keys, expiresAt: now + jwksCacheMs };
    return jwks.keys;
  }
