...
function create(opts) {
  ...
  function clientCredentialsManager(mopts) {
    ...
    var PERMANENT_REFRESH_CODES = { "auth-oauth/bad-token-json": 1, "auth-oauth/no-access-token": 1 };
    function _isTransientRefreshError(e) {
      // A refresh failure is TRANSIENT (safe to ride out on the still-valid cache)
      // when it is a 429 or 5xx from the token endpoint, or a transport / network
      // error. A 4xx (invalid_client / invalid_scope) or a malformed successful
      // response (no-access-token / bad-token-json) is PERMANENT and must
      // propagate — masking it would hide a credential/config error and repeat it.
      /* c8 ignore next -- e is always a coded OAuthError from clientCredentials; the codeless guard is defensive */
      if (!e || !e.code) return false;
      var m = /^auth-oauth\/token-error-(\d+)$/.exec(e.code);
      if (m) { var s = parseInt(m[1], 10); return s === 429 || s >= 500; }
      // malformed-response codes permanent; transport/network transient (hasOwnProperty — proto-shadow-safe).
      return !Object.prototype.hasOwnProperty.call(PERMANENT_REFRESH_CODES, e.code);
    }
