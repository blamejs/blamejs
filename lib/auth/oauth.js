...
function create(opts) {
  ...
  function _scopeParam(override) {
    var s = override !== undefined ? override : scope;
   ...
   * <code>grant_type=client_credentials</code> (plus the configured or
   * overridden scope) to the token endpoint with the client's
   * id/secret, and returns the access token. Per the RFC a refresh_token is
   * NEVER issued for this grant — present the credentials again to renew (see
   * <code>clientCredentialsManager</code> for a cached, auto-renewing wrapper).
   *
   ...
   *
   * @opts
   *   scope: string[] | string,   // override the client's configured scope for this token
   *
   * @example
  ...
  async function clientCredentials(ccopts) {
    ...
    var reqScope = _scopeParam(ccopts.scope);
  ...
  function clientCredentialsManager(mopts) {
    ...
    var cached = null;        // { accessToken, expiresAt }
    var inflight = null;      // shared Promise while a fetch is running
    var backoffUntil = 0;     // epoch-ms; a 429 backoff is active while now < this
    ...
    function _usable(now) {
      ...
      return cached && cached.expiresAt !== null && cached.expiresAt - now > skewMs;
    }
    ...
    async function getToken() {
      ...
      // Inside a 429 backoff window: serve the still-valid cached token rather
      // than re-hammer the AS (but never an expired one).
      if (now < backoffUntil && _servableDuringBackoff(now)) return cached.accessToken;
      ...
      inflight = clientCredentials({ scope: mopts.scope }).then(function (t) {
        ...
        inflight = null;
        if (e && e.code === "auth-oauth/token-error-429") {
          backoffUntil = Date.now() + backoffMs;
          if (_servableDuringBackoff(Date.now())) return cached.accessToken;   // ride out on the still-valid cached token
        }
        throw e;
      });
      return inflight;
