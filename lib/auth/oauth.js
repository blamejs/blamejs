...
async function verifyClientAttestation(attestationJwt, popJwt, vopts) {
  ...
  if (!numericBounds.isFiniteNumericDate(ap.exp) || ap.exp + skewSec < nowSec) {
  ...
  if (pp.exp !== undefined && (!numericBounds.isFiniteNumericDate(pp.exp) || pp.exp + skewSec < nowSec)) {
...
function create(opts) {
  ...
  async function _fetchJson(url, fetchOpts) {
    ...
    return safeJson.parseTyped(res.body.toString("utf8"), {
      maxBytes:   OAUTH_MAX_RESPONSE_BYTES,
      errorClass: OAuthError,
      code:       "auth-oauth/bad-json",
      label:      url + " response not JSON",
    });
  }
  ...
  async function verifyIdToken(idToken, vopts) {
    ...
    if (vopts.skipExpCheck) {
      ...
      if (!numericBounds.isFiniteNumericDate(payload.exp) || payload.exp + skewSec < now) {
