...
function create(opts) {
  ...
  function clientCredentialsManager(mopts) {
    ...
    async function getToken() {
      ...
      // Inside a 429 backoff window: serve the cached token rather than re-hammer.
      if (now < backoffUntil && cached) return cached.accessToken;
      ...
      inflight = clientCredentials({ scope: mopts.scope }).then(function (t) {
        ...
        if (e && e.code === "auth-oauth/token-error-429") {
          ...
          if (cached) return cached.accessToken;   // ride out the backoff on the old token
        }
        throw e;
      });
      return inflight;
