...
function create(opts) {
  ...
  async function revokeToken(token, ropts) {
    ...
    body.set("client_id", clientId);
    if (clientSecret) body.set("client_secret", clientSecret);
    // RFC 7009 authenticates the client at the revocation endpoint the same way
    // as at the token endpoint — apply client_secret_basic when configured.
  ...
  function clientCredentialsManager(mopts) {
    ...
    function _usable(now) {
      ...
      // Normally refresh skewMs before expiry. But a token whose whole lifetime
      // is <= skewMs can never satisfy that (it is born inside the skew window),
      // so serve it while still valid rather than re-fetching on every call.
      return cached.expiresAt - now > skewMs || (cached.lifetimeMs !== null && cached.lifetimeMs <= skewMs);
    }
    ...
    async function getToken() {
      ...
      inflight = clientCredentials({ scope: mopts.scope }).then(function (t) {
        ...
        if (e && e.code === "auth-oauth/token-error-429") {
          // Honor the AS's Retry-After (RFC 6585 §4) when it stated one, else the
          // configured fixed backoff.
          var waitMs = (typeof e.retryAfterMs === "number" && e.retryAfterMs > 0) ? e.retryAfterMs : backoffMs;
