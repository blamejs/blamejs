    ...
    async function getToken() {
      ...
      inflight = clientCredentials({ scope: mopts.scope }).then(function (t) {
        ...
        if (e && e.code === "auth-oauth/token-error-429") {
          ...
          // configured fixed backoff. Only a 429 opens the backoff window.
          var waitMs = (typeof e.retryAfterMs === "number" && e.retryAfterMs > 0) ? e.retryAfterMs : backoffMs;
          backoffUntil = Date.now() + waitMs;
        }
        // Any refresh failure (a 429, or a transient 5xx / network error) rides
        // out on the STILL-VALID cached token rather than failing the caller;
        // only re-throw when no valid cached token remains.
