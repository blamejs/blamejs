...
function _plainFromCbor(value) {
  ...
  if (value instanceof Map) {
    ...
    value.forEach(function (v, k) {
      // Non-string keys are legal CBOR but have no place on a plain object;
      // stringifying them would collide 1 with "1".
      if (typeof k === "string") out[k] = _plainFromCbor(v);
    });
    return out;
  }
  return value;
...
async function verifyRegistration(opts) {
  ...
  try {
    ...
    // attestation into an unhandled exception and a 500 where a refusal
    // belongs. Everything else -- a binding that disagrees, an unanchored
    // chain, a policy the ceremony failed -- stays exceptional, because which
    // check refused is the actionable part.
