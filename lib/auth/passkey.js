...
function _storedKeyBytes(stored) {
  if (typeof stored === "string") {
    ...
    if (_canonicalCredentialId(stored) === null) {
...
function _plainFromCbor(value) {
  ...
  if (value instanceof Map) {
    // Null prototype, so a signed extension named `__proto__` becomes an own
    // property like any other. On an ordinary object that assignment hits the
    // legacy prototype setter instead: the key vanishes from Object.keys and
    // from JSON, while its contents reappear as INHERITED properties on the
    // result -- verified data silently lost, and unverified-looking data
    // silently gained. Also removes the prototype chain from a value built
    // out of authenticator-controlled bytes.
...
function _anchorPlan(attestationObject) {
  ...
  if (fmt === "android-safetynet") {
    return { roots: null, safetyNet: true, fmt: named };
  }
  var roots = attestationRoots.ROOTS_BY_FORMAT[fmt];
  if (!roots) return { roots: null, safetyNet: false, fmt: named };
  ...
  if (!(attStmt instanceof Map) || !attStmt.has("x5c")) {
    return { roots: null, safetyNet: false, fmt: named };
  }
  return { roots: roots, safetyNet: false, fmt: named };
}
...
async function verifyRegistration(opts) {
  ...
  try {
    ...
    //
    // The verifier spends ONE code, webauthn/verify-failed, on both: a bad
    // signature and a failed binding (the apple attestation nonce, TPM's
    // certInfo extraData and attested Name). Those two formats are therefore
    // left exceptional, so an attestation-binding violation -- an attack
    // indicator -- is never quietly reported as an ordinary decline. For
    // every other format the code means a signature and nothing else.
    // Keyed on the format rather than the vendor's message text, which is
    // prose and not a contract.
    if (e && e.code === "webauthn/verify-failed" &&
        regAnchorPlan.fmt !== "apple" && regAnchorPlan.fmt !== "tpm") {
      return {
        verified: false,
  ...
  var rv = {
    ...
    registrationInfo: {
      ...
      credentialType:       opts.response.type,
      ...
      clientExtensionResults: opts.response.clientExtensionResults,
    },
  };
...
function signalUnknownCredential(opts) {
  ...
  var credentialId = _canonicalCredentialId(opts.credentialId);
...
function signalAllAcceptedCredentials(opts) {
  ...
  var userId = _canonicalCredentialId(opts.userId);
  if (userId === null) {
  ...
  for (var i = 0; i < opts.allAcceptedCredentialIds.length; i++) {
    var one = typeof opts.allAcceptedCredentialIds[i] === "string"
      ? _canonicalCredentialId(opts.allAcceptedCredentialIds[i]) : null;
    if (one === null) {
      throw new AuthError("auth-passkey/bad-accepted-list",
...
function signalCurrentUserDetails(opts) {
  ...
  _requireString(opts.rpId, "rpId");
  _requireString(opts.userId, "userId");
  var currentUserId = _canonicalCredentialId(opts.userId);
