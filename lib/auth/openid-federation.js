...
function verifyEntityStatement(jwt, jwks, vopts) {
  ...
  var nowMs = (typeof vopts.now === "number" && isFinite(vopts.now)) ? vopts.now : Date.now();
  ...
  if (!numericBounds.isFiniteNumericDate(parsed.claims.iat) || parsed.claims.iat > nowSec + skew) {
    throw new AuthError("auth-openid-federation/iat-future",
      "verifyEntityStatement: iat is in the future or missing");
  }
  if (!numericBounds.isFiniteNumericDate(parsed.claims.exp) || parsed.claims.exp < nowSec - skew) {
    throw new AuthError("auth-openid-federation/expired",
      "verifyEntityStatement: statement expired or exp is not a finite NumericDate");
  }
