...
// Validate that ONE claim's MERGED operator set is a permitted combination per
// OIDF 1.0 section 6.1.3.1. `value` pins an exact value; per section 6.1.3.1.1 it
// MAY co-exist with a constraint operator (one_of / subset_of / superset_of) as
// long as the pinned value SATISFIES it -- so a subordinate pinning a value INSIDE
// a superior's set is a valid narrowing, while a value OUTSIDE it (anchor
// one_of:[private_key_jwt] + subordinate value:none) is the trust downgrade this
// refuses. `add` values must stay within `subset_of` and cover `superset_of`;
// `default` must satisfy a co-present one_of / subset_of / superset_of; `one_of`
// (single-value domain) cannot combine with the array operators; `superset_of`
// must be within `subset_of`. Any violation refuses the chain -- same-operator
// value/default equality is checked during the merge.
function _assertPolicyCombination(claim, acc) {
  ...
  function conflict(why) {
    throw new AuthError("auth-openid-federation/policy-merge-conflict",
      "metadata_policy['" + claim + "']: incompatible operator combination across the chain -- " +
      why + " (a subordinate cannot override a superior's constraint with a different operator)");
  }
  if (has("value")) {
    // `value` pins an exact value: it MAY co-exist with a CONSTRAINT
    // (one_of/subset_of/superset_of) when the pinned value satisfies it, but NOT
    // with another MODIFIER -- a subordinate `add` would WIDEN the anchor's
    // pinned value (a trust downgrade), and `default` is meaningless against it.
    if (has("add")) conflict("`value` cannot be combined with `add`");
    if (has("default")) conflict("`value` cannot be combined with `default`");
    if (has("one_of") && acc.one_of.indexOf(acc.value) === -1) {
      conflict("`value` must be one of `one_of`");
    }
    if (has("subset_of") && !_arraySubset(asArray(acc.value), acc.subset_of)) {
      conflict("`value` must be a subset of `subset_of`");
    }
    if (has("superset_of") && !_arraySubset(acc.superset_of, asArray(acc.value))) {
      conflict("`value` must include every `superset_of` value");
    }
  }
  if (has("one_of") && (has("add") || has("subset_of") || has("superset_of"))) {
    conflict("`one_of` cannot be combined with an array operator");
  }
  if (has("add") && has("subset_of") && !_arraySubset(acc.add, acc.subset_of)) {
    conflict("`add` values must be a subset of `subset_of`");
  }
  if (has("add") && has("superset_of") && !_arraySubset(acc.superset_of, acc.add)) {
    conflict("`add` must include every `superset_of` value");
  }
  if (has("default")) {
    if (has("one_of") && acc.one_of.indexOf(acc.default) === -1) {
      conflict("`default` must be one of `one_of`");
    }
    if (has("subset_of") && !_arraySubset(asArray(acc.default), acc.subset_of)) {
      conflict("`default` must be a subset of `subset_of`");
    }
    if (has("superset_of") && !_arraySubset(acc.superset_of, asArray(acc.default))) {
      conflict("`default` must include every `superset_of` value");
    }
  }
  if (has("subset_of") && has("superset_of") && !_arraySubset(acc.superset_of, acc.subset_of)) {
    conflict("`superset_of` must be a subset of `subset_of`");
  }
}
