...
// OIDF 1.0 section 6.1.3. `value` sets an exact value and the spec permits it
// ONLY alongside `essential`, so a merged `value` co-present with any constraint
// (one_of / subset_of / superset_of) or other modifier (add / default) is an
// invalid policy -- and the primary cross-level TRUST DOWNGRADE a subordinate
// reaches by pinning `value` (or `add`) on a claim a superior constrained with a
// DIFFERENT operator (anchor one_of:[private_key_jwt] + subordinate value:none
// resolved to none before this check). `add` values must stay within `subset_of`
// and cover `superset_of`; `default` must satisfy a co-present one_of / subset_of
// / superset_of; `one_of` (single-value domain) cannot combine with the array
// operators; `superset_of` must be within `subset_of`. Any violation refuses the
// chain -- the same-operator value/default equality is checked during the merge.
function _assertPolicyCombination(claim, acc) {
  ...
  if (has("value")) {
    ["add", "default", "one_of", "subset_of", "superset_of"].forEach(function (op) {
      if (has(op)) conflict("`value` cannot be combined with `" + op + "`");
    });
  }
    ...
    _OP_ORDER.forEach(function (op) {
      ...
      switch (op) {
        ...
        case "add":
          ...
          out[claimName] = _arrayUnion(out[claimName], v);
          break;
        case "one_of":
          if (!Array.isArray(v)) {
          ...
          if (out[claimName] !== undefined && out[claimName] !== null) {
            if (!Array.isArray(out[claimName])) {
              throw new AuthError("auth-openid-federation/policy-subset-of-failed",
                ...
                " is not an array; subset_of constrains an array-valued claim");
            }
