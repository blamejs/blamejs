...
async function _fetchJwks(uri, cacheMs) {
  ...
  return await jc.wrap(key, async function () {
    ...
    return jwks.keys;
  }, { ttlMs: cacheMs === undefined ? DEFAULT_JWKS_CACHE_MS : cacheMs });
}
...
async function verifyExternal(token, opts) {
  ...
  numericBounds.requireNonNegativeFiniteIntIfPresent(opts.jwksCacheMs,
    "verifyExternal: opts.jwksCacheMs", AuthError, "auth-jwt-external/bad-jwks-cache-ms");
  ...
  if (!numericBounds.isFiniteNumericDate(payload.exp)) {
    throw new AuthError("auth-jwt-external/missing-exp",
      "claim 'exp' missing or not a finite NumericDate (a non-finite exp would evade the expiry check)");
  }
