...
function _validateRule(rule) {
  if (!rule || typeof rule !== "object") {
    throw _err("retention/bad-rule", "rule must be an object");
  }
  if (typeof rule.name !== "string" || rule.name.length === 0) {
    throw _err("retention/bad-rule", "rule.name (string) is required");
  }
  if (typeof rule.table !== "string" || rule.table.length === 0) {
    throw _err("retention/bad-rule", "rule.table (string) is required");
  }
  _validateRuleIdentifier(rule.table, "rule.table");
  if (typeof rule.ageField !== "string" || rule.ageField.length === 0) {
    throw _err("retention/bad-rule", "rule.ageField (string) is required");
  }
  _validateRuleIdentifier(rule.ageField, "rule.ageField");
  if (typeof rule.ttlMs !== "number" || !isFinite(rule.ttlMs) || rule.ttlMs <= 0) {
    throw _err("retention/bad-rule", "rule.ttlMs must be a positive finite number");
  }
  var action = rule.action;
  if (typeof action !== "string" && typeof action !== "function") {
    throw _err("retention/bad-rule", "rule.action must be 'erase' / 'delete' / 'soft-delete' or a function(row)");
  }
  ...
  numericBounds.requirePositiveFiniteIntIfPresent(rule.batchSize,
    "rule.batchSize", RetentionError, "retention/bad-rule");
  if (rule.softDeleteField !== undefined &&
      (typeof rule.softDeleteField !== "string" || rule.softDeleteField.length === 0)) {
    throw _err("retention/bad-rule", "rule.softDeleteField must be a non-empty string");
  }
  if (rule.softDeleteField !== undefined) {
  ...
  if (rule.legalHoldField !== undefined &&
    ...
    throw _err("retention/bad-rule", "rule.legalHoldField must be a non-empty string");
  }
  if (rule.legalHoldField !== undefined) {
  ...
  if (rule.subjectField !== undefined &&
    ...
    throw _err("retention/bad-rule", "rule.subjectField must be a non-empty string");
  }
  if (rule.subjectField !== undefined) {
  ...
  if (rule.cascade !== undefined) {
    if (!Array.isArray(rule.cascade) || rule.cascade.length === 0) {
      throw _err("retention/bad-rule", "rule.cascade must be a non-empty array of { table, foreignKey } entries");
    }
    for (var ci = 0; ci < rule.cascade.length; ci++) {
      ...
      if (!c || typeof c.table !== "string" || c.table.length === 0 ||
        ...
        throw _err("retention/bad-rule", "rule.cascade[" + ci + "] must be { table: string, foreignKey: string }");
      }
  ...
  if (rule.stages !== undefined) {
    if (!Array.isArray(rule.stages) || rule.stages.length === 0) {
      throw _err("retention/bad-rule", "rule.stages must be a non-empty array of { atMs, action } entries");
    }
    for (var si = 0; si < rule.stages.length; si++) {
      ...
      if (!stage || typeof stage.atMs !== "number" || !isFinite(stage.atMs) || stage.atMs <= 0) {
        throw _err("retention/bad-rule", "rule.stages[" + si + "].atMs must be a positive finite number");
      }
...
function create(opts) {
  ...
  if (!opts.db || typeof opts.db.prepare !== "function") {
    throw _err("retention/bad-opt", "create: opts.db is required (a b.db handle with .prepare(sql))");
  }
  var db = opts.db;
  ...
  function declare(rule) {
    _validateRule(rule);
    if (rules[rule.name]) {
      throw _err("retention/duplicate-rule", "rule '" + rule.name + "' is already declared");
    }
  ...
  async function run(name, runOpts) {
    var rule = rules[name];
    if (!rule) throw _err("retention/no-such-rule", "rule '" + name + "' not declared");
    runOpts = runOpts || {};
    ...
    try {
      ...
      throw _err("retention/sweep-failed",
