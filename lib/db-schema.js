...
function reconcileIndex(database, tableName, idx) {
  ...
  if (dialect === "mysql") {
    try {
      ...
      // The driver's own errno, not its wording: a message test for `exist`
      // also matches "Table 'db.X' doesn't exist", so a CREATE INDEX that
      // failed because its table was missing was swallowed and the reconciler
      // returned as though the index had been created.
