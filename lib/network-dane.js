...
function _normaliseTlsa(rec, i) {
  ...
  var u = rec.usage, s = rec.selector, m = rec.matchingType;
  if (USAGES[u] === undefined) throw new DaneError("dane/unsupported-usage", "dane: tlsa[" + i + "] unsupported certificate usage " + u + " (0-3)");
  if (SELECTORS[s] === undefined) throw new DaneError("dane/unsupported-selector", "dane: tlsa[" + i + "] unsupported selector " + s);
  if (!(m in MATCHING)) throw new DaneError("dane/unsupported-matching", "dane: tlsa[" + i + "] unsupported matching type " + m);
  return { usage: u, selector: s, matchingType: m, data: _bytes(rec.data, "tlsa[" + i + "].data") };
}
