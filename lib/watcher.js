...
function _detectAutoMode(rootPath) {
  ...
  //   2. /.dockerenv present AND the root's mount has option "bind" —
  //      Docker bind-mount from a non-Linux host (Docker Desktop)
  //      where libuv won't see events; poll.
  //   3. Otherwise — fs.
  //
  ...
  for (var i = 0; i < lines.length; i += 1) {
    var ln = lines[i];
    if (!ln) continue;
    // Format: <id> <parent> <major:minor> <root> <mountpoint> <options> - <fstype> <source> <super-options>
    ...
    if (rootPath === mountPoint ||
      ...
      if (mountPoint.length > bestLen) {
        ...
        bestMatch = { mountPoint: mountPoint, options: options, fstype: fstype };
      }
    }
  }
  ...
  // Many Docker Desktop releases mount the host bind-mount with an
  // overlayfs / ext4 fstype on the Linux VM side but events still don't
  // propagate. Heuristic — if we're inside a container AND the mount
  // options include "rw" with the kernel's bind marker, fall back to
  // poll. This catches Docker Desktop bind-mounts that report ext4 but
  // are actually bridged from the host.
  // (Operator can still force fs via mode: "fs"; force poll via mode: "poll".)
  if (inContainer && bestMatch.options &&
      bestMatch.options.indexOf("bind") !== -1) {
    return {
      mode:        "poll",
