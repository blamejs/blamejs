...
// that can mark the root of an absolute name. Verified against Node's own
// mapping to be exactly this set across the BMP and the SMP.
...
function _normalizeInput(domain) {
  ...
  if (s.length > 253) {
    ...
    // length-byte + null). Measured without the root marker, which the wire
    // form does not carry as a character.
    throw _err("public-suffix/invalid-domain",
