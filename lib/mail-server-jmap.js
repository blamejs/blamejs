...
var MEDIA_TYPE_RE =
  /^[A-Za-z0-9!#$%&'*+.^_`|~-]+\/[A-Za-z0-9!#$%&'*+.^_`|~-]+(?:\s*;\s*[A-Za-z0-9!#$%&'*+.^_`|~-]+=(?:[A-Za-z0-9!#$%&'*+.^_`|~-]+|"(?:[\t\x20-\x21\x23-\x5B\x5D-\x7E]|\\[\t\x20-\x7E])*"))*$/;   // allow:regex-no-length-cap — anchored, length bounded by the caller before this runs
...
function create(opts) {
  ...
  function downloadHandler(req, res) {
    ...
    // `?accept=` is deliberately not read. RFC 8620 section 6.2 makes it a
    // request, and the response type is decided from what the blob is; parsing
    // it would only invite it back into the header it used to reach.
    ...
    Promise.resolve()
      ...
      .then(function (result) {
        ...
        // The requester does not choose the response type. RFC 8620 section 6.2
        // makes `accept` a REQUEST, and honoring it unconstrained returns
        // attacker-influenced bytes -- a mail attachment is the ordinary case --
        // under a type the URL picked, so `?accept=text/html` is script
        // execution in the application's origin against a session the endpoint
        // has already authenticated. The answer is what the blob IS.
        //
        // A backend type that is not a plausible media type is not written into
        // a header either: it is caller data reaching a response header, and
        // the defensive answer for a malformed one is the generic type.
        var rawType = String(result.type || "");
        var bType = rawType.length <= MAX_MEDIA_TYPE_BYTES && MEDIA_TYPE_RE.test(rawType)
          ? rawType
          : "application/octet-stream";
