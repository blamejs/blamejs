  ...
  async function dispatch(actor, body) {
    ...
    for (var i = 0; i < parsed.methodCalls.length; i += 1) {
      ...
      try {
        ...
        var resultErrorName = _resultIsMethodError(result) ? methodErrorName(result.type) : null;
...
function _fromHeaderAddress(rfc822) {
  ...
  return _firstMailboxAddress(value);
}
  ...
  while (i < value.length) {
    ...
    if (ch === '"') {
      ...
      while (i < value.length) {
        ...
        if (q === "\\") { quoted += value.slice(i, i + 2); i += 2; continue; }
        quoted += q;
        i += 1;
        if (q === '"') break;
      }
    ...
    if (ch === "<") {
      ...
      var inner = value.slice(i + 1, close).trim();
...
function _bodyPartFrom(part, path, prefix) {
  ...
  return {
    ...
    type:        contentType,
    charset:     _headerParam(headers.get("content-type"), "charset"),
    ...
    subParts:    subParts === null ? [] : subParts,
  };
}
...
function emailBodyProperties(tree, opts) {
  ...
  (function collect(node) {
    if (node.subParts && node.subParts.length > 0) {
      for (var i = 0; i < node.subParts.length; i += 1) collect(node.subParts[i]);
      return;
    }
    if (node.disposition === "attachment" || (node.disposition === "inline" && node.name !== null)) {
      attachments.push(node);
      return;
    }
    if (node.type === "text/plain") { textBody.push(node); return; }
    if (node.type === "text/html")  { htmlBody.push(node); return; }
    attachments.push(node);
  })(bodyStructure);
