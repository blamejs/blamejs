 ...
 *   });
 *
 *   app.post("/jmap/api", b.middleware.bearerAuth({ verify }), jmap.apiHandler);
 */
function create(opts) {
  ...
  function apiHandler(req, res) {
    ...
    dispatch(actor, rawBody).then(function (response) {
      res.statusCode = response && response.type ? 400 : 200;                                       // allow:raw-byte-literal — HTTP status codes
      res.setHeader("Content-Type", "application/json; charset=utf-8");
