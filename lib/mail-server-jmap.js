...
function create(opts) {
  ...
  function uploadHandler(req, res) {
    ...
    // `/jmap/upload/{accountId}`; the last path segment IS the
    // accountId. Strip any trailing slash + ignore query strings.
    var rawUrl = String(req.url || "");
    var qIdx = rawUrl.indexOf("?");
    var pathOnly = qIdx === -1 ? rawUrl : rawUrl.slice(0, qIdx);
    var segments = pathOnly.replace(/\/+$/, "").split("/");
    ...
    if (!accountId || !/^[A-Za-z0-9_-]{1,64}$/.test(accountId)) {
      ...
      res.end(JSON.stringify({
        type:        "urn:ietf:params:jmap:error:invalidArguments",
        description: "Upload URL missing or malformed accountId path segment",
      }));
      return;
    }
  ...
  function downloadHandler(req, res) {
    ...
    var qIdx = rawUrl.indexOf("?");
    var pathOnly = qIdx === -1 ? rawUrl : rawUrl.slice(0, qIdx);
    var query = qIdx === -1 ? "" : rawUrl.slice(qIdx + 1);
    var acceptType = "";
    query.split("&").forEach(function (pair) {
      if (!pair) return;
      var eq = pair.indexOf("=");
    ...
    var segments = pathOnly.replace(/\/+$/, "").split("/").filter(Boolean);
    // Expect tail .../jmap/download/{accountId}/{blobId}/{name}
    // The router may strip the prefix, so take the last 3 segments.
    if (segments.length < 3) {
      res.statusCode = 400;
      res.setHeader("Content-Type", "application/json; charset=utf-8");
      res.end(JSON.stringify({
        type:        "urn:ietf:params:jmap:error:invalidArguments",
        description: "Download URL must carry /{accountId}/{blobId}/{name}",
      }));
      return;
    }
    var accountId = (req.params && req.params.accountId) || segments[segments.length - 3];
    var blobId    = (req.params && req.params.blobId)    || segments[segments.length - 2];
    var fileName  = (req.params && req.params.name)      || segments[segments.length - 1];
    if (!/^[A-Za-z0-9_-]{1,64}$/.test(accountId)) {
      res.statusCode = 400;
      res.setHeader("Content-Type", "application/json; charset=utf-8");
      res.end(JSON.stringify({
        type:        "urn:ietf:params:jmap:error:invalidArguments",
        description: "Download URL has malformed accountId segment",
      }));
      return;
    }
    Promise.resolve()
      ...
      .then(function (result) {
        ...
        var bType  = (result && result.type) || acceptType || "application/octet-stream";
