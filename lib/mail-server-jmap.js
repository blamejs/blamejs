...
// RFC 9110 section 8.3.1 media type, restricted to what may be written into a
// response header unquoted: type/subtype in RFC 9110 token characters, with
// optional `; parameter=value` runs. A backend-supplied type that does not
// match is answered as the generic type rather than written through.
// The length is bounded BEFORE the pattern runs, so the scan is never handed
// an unbounded string. RFC 6838 caps type and subtree names at 127 octets
// each; this leaves room for those plus a short parameter run.
var MAX_MEDIA_TYPE_BYTES = 320;

// A parameter value is a token OR a quoted-string (RFC 9110 section 5.6.6), so
// `text/plain; charset="utf-8"` is valid and must survive.
//
// The quoted form is spelled as the POSITIVE `qdtext` class -- HTAB, SP, `!`,
// `#`-`[`, `]`-`~` -- rather than as "anything except a few bytes". Written the
// other way it would still admit every control character, and the bytes that
// end a header are exactly the ones that must not get through. This class
// excludes all of them by construction, along with the `"` and `\` that would
// end or escape the string itself.
//
// `quoted-pair` is admitted alongside it, because RFC 9110 section 5.6.4 makes
// `\` an escape inside a quoted-string and `boundary="a\"b"` is a valid media
// type. What may follow the backslash is HTAB or a printable, so an escaped CR
// or LF is still refused. The two alternatives are disjoint on their first
// character, so there is nothing for the scan to backtrack over.
// The whitespace around a parameter separator is OWS -- SP and HTAB, RFC 9110
// section 5.6.3 -- and not `\s`, which in JavaScript also matches CR, LF, form
// feed, vertical tab and the Unicode spaces. Written as `\s*` this validator
// accepted `text/plain;\r\ncharset=utf-8`, which is the folded shape it exists
// to refuse: node then rejects the value at `setHeader` with ERR_INVALID_CHAR,
// so a blob whose stored type carried a line break answered 500 instead of
...
function create(opts) {
  ...
  function downloadHandler(req, res) {
    ...
    // `?accept=` -- the `{type}` variable of the download URL this server
    // advertises. RFC 8620 section 6.2 defines it as the type for the server to
    // set in the response, since a blobId carries no innate content type. It is
    // read here and used only when the backend reports none, and only when it
    // is a well-formed media type; a percent-encoding this decoder cannot read
    // yields nothing rather than throwing, and the generic type applies.
    ...
    Promise.resolve()
      ...
      .then(function (result) {
        ...
        // What the blob IS, when the backend says; otherwise the type the
        // download URL carried. RFC 8620 section 6.2 defines that variable as
        // "the type for the server to set in the Content-Type header of the
        // response", precisely because "the blobId only represents the binary
        // data and does not have a content-type innately associated with it" --
        // so for a backend that stores bytes alone, the URL is the only place
        // the type can come from, and ignoring it leaves every download a
        // generic octet-stream the client cannot dispatch.
        //
        // What made this dangerous was never the type on its own. It was a
        // response with an attacker-influenced type AND no disposition, which
        // renders inline: `?accept=text/html` then executes in the
        // application's origin against a session this endpoint has already
        // authenticated. The disposition below is unconditional now, so the
        // response is downloaded rather than rendered whatever the type says.
        //
        // Either source is caller data reaching a response header, so neither
        // is written unless it is a well-formed media type; a malformed one
        // takes the generic type rather than the header.
        ...
        // ALWAYS an attachment, not only when a filename happens to match.
        // Without a disposition the response renders inline, so a blob with no
        // filename -- or one carrying a space -- was served inline under
        // whatever type applied. The filename rides along only when it is safe
        // to quote; an unsafe one costs the name, not the disposition.
