...
async function emit(opts) {
  ...
  sections.push(_section("Restore Procedure",
    ...
    "`b.backup.verifyManifestSignature(bundleDir, { expectedFingerprint })` " +
    "(the fingerprint of the audit-sign key the backing-up host signs with; " +
    "without it a signed manifest is trusted only when this host holds that key).\n" +
    "3. Restore staging dir: `await backup.read(bundleId, '/restore/staging')`.\n" +
    "4. Decrypt + verify: `await b.restoreBundle.extract({ bundleDir, " +
    "stagingDir, passphrase, requireSignature: true, expectedFingerprint })`.\n" +
    "5. Verify audit chain on the restored DB: " +
    "`await b.auditChain.verifyChain(function (sql, params) { " +
    "return restoredDb.all(sql, params); }, 'audit_log')`. " +
    "The first argument runs a query and resolves to the rows; the second " +
    "names the chain table in the restored database.\n" +
