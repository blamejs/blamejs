"use strict";
/**
 * dr-runbook — disaster-recovery runbook generator.
 *
 * Composes the operator's b.budr breach-decision registry, b.cluster
 * topology, b.backup configuration, role-recovery contacts, and
 * incident-contact placeholders into a posture-appropriate Markdown
 * runbook a regulator can read alongside the framework's audit chain.
 *
 ...
 *       hipaaPrivacyOfficer: "bob@example.com",
 *     },
 *     budr:     b.budr,        // optional — pulls disclosure deadlines per posture
 *     cluster:  b.cluster,     // optional — emits topology section
 *     backup:   bk,            // optional — emits backup-config summary
 *   });
 *   // → { paths: ["./docs/dr/runbook.md"], posture, sectionCount }
 *
 * Posture-driven content:
 *   hipaa     — §164.308(a)(7) contingency plan + §164.310(a)(2)(i)
 *               facility-recovery checklist
 *   pci-dss   — Req. 12.10 incident-response touch-points + DSS 4.0
 *               12.10.5 (containment / eradication / recovery)
 *   gdpr      — Art. 33 72h notification timeline + Art. 32 measures
 *   soc2      — CC7.4 / CC9.1 — recovery objectives + change control
 *   dora      — Art. 11/12/24 — operational resilience continuity plan
 *
 * The runbook is plain Markdown — operators commit it under
 * `docs/dr/` and version it with their service. Re-emitting overwrites
 * the file, so an operator-supplied template change ships through git
 * review before the runbook lands.
 */
