 ...
 *
 *   <strong>Decoder defenses</strong> (all on by default):
...
function _encodeFloat(value) {
  ...
  if (f4.readFloatBE(1) === value) return f4;
...
function _doubleToHalfBits(value) {
  ...
  if (fbuf.readFloatBE(0) !== value) return -1;
  var f = fbuf.readUInt32BE(0);
  var sign = (f >>> 16) & 0x8000;
  var exp = (f >>> 23) & 0xff;
  var mant = f & 0x7fffff;
  var unbiased = exp - 127 + 15;
  if (unbiased >= 0x1f) return -1;
  if (unbiased <= 0) {
    if (unbiased < -10) return -1;
    var fullMant = mant | 0x800000;
    var shift = 14 - unbiased;
    if (fullMant & ((1 << shift) - 1)) return -1;
    return sign | (fullMant >>> shift);
  }
  if (mant & 0x1fff) return -1;
...
function _encodeValue(value, opts, depth) {
  if (depth > ABSOLUTE_MAX_DEPTH) {
    throw new CborError("cbor/too-deep",
      "cbor.encode: value nests deeper than " + ABSOLUTE_MAX_DEPTH + " levels");
  }
  ...
  if (Array.isArray(value)) {
    ...
    for (var i = 0; i < value.length; i++) parts.push(_encodeValue(value[i], opts, depth + 1));
    return Buffer.concat(parts);
  }
  if (value instanceof Tag) {
    return Buffer.concat([_head(6, value.tag), _encodeValue(value.value, opts, depth + 1)]);
  }
  if (value instanceof Map || (typeof value === "object")) {
    return _encodeMap(value, opts, depth);
  }
  throw new CborError("cbor/unencodable",
    "cbor.encode: value of type " + (typeof value) + " is not CBOR-encodable");
}

function _encodeMap(value, opts, depth) {
  var entries = [];
  if (value instanceof Map) {
    value.forEach(function (v, k) {
      entries.push([_encodeValue(k, opts, depth + 1), _encodeValue(v, opts, depth + 1)]);
    });
  } else {
    var keys = Object.keys(value);
    for (var i = 0; i < keys.length; i++) {
      entries.push([_encodeValue(keys[i], opts, depth + 1), _encodeValue(value[keys[i]], opts, depth + 1)]);
    }
  }
 ...
 * <code>undefined</code>.
 *
 * A value nested more than 256 levels is refused as
 * <code>cbor/too-deep</code>, the depth <code>decode</code> also refuses.
 *
 * @opts
 *   {
...
function encode(value, opts) {
  opts = opts || {};
  return _encodeValue(value, opts, 0);
}
 ...
 * reserved additional-info (28 to 30), tags not in
...
function decode(buffer, opts) {
  ...
  if (opts.requireDeterministic === true) {
    var reencoded = _encodeValue(value, {}, 0);
...
function _readArgument(state, ai) {
  ...
  if (ai === 27) {
    ...
    return big <= 9007199254740991n ? Number(big) : big;
  }
  if (ai === 31) {
...
function _decodeItem(state, depth) {
  ...
  switch (major) {
    case 0: return _readArgument(state, ai);
    case 1: {
    ...
    default: return _decodeSimpleOrFloat(state, ai);
  }
}
