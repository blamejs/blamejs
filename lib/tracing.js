...
function create(opts) {
  ...
  function requestMiddleware() {
    return function tracingMiddleware(req, res, next) {
      var route = (req.routePattern && typeof req.routePattern === "string")
                    ? req.routePattern
                    : ((req.url || "").split("?")[0] || "/");
      var spanName = "HTTP " + (req.method || "GET") + " " + route;
      var attrs = {
        "http.method": req.method || "GET",
        "http.route":  route,
        "http.url":    req.url || "",
      };
      // Honor incoming traceparent — pass it via attributes so even
      // pass-through mode preserves the parent ID for logs.
      var parent = extractContext(req.headers);
      if (parent) {
        attrs["traceparent.parent"] = parent.traceId + "-" + parent.spanId;
      }
      span(spanName, function (s) {
        // Stash the active span on req for handlers / metrics middleware
        // to read via b.tracing.currentSpan() OR req.span.
        req.span = s;
        // Wrap res.end so we record the status.
        var origEnd = res.end;
        ...
        res.end = function () {
          try { s.setAttribute("http.status_code", status); } catch (_e) {}
