 ...
 *   - Per CLAUDE.md rule #1 (zero npm runtime deps). Operators who
 *     want tracing carry the cost; operators who don't pay nothing.
 *   - OTel API is unstable enough that pinning a vendored version
...
function create(opts) {
  ...
  function requestMiddleware() {
    return function tracingMiddleware(req, res, next) {
      // The router populates req.routePattern AFTER this middleware
      // runs; at start we only have the URL. Use it as the initial
      // span name + http.route, then update both at res.end if the
      // route matcher resolved a template.
      var initialRoute = ((req.url || "").split("?")[0] || "/");
      ...
      span(spanName, function (s) {
        req.span = s;
        // Status capture — handlers may set res.statusCode directly
        // (modern Node pattern) instead of calling writeHead.
        var origEnd = res.end;
        var origWriteHead = res.writeHead;
        var statusFromWriteHead = null;
        res.writeHead = function (st) {
          statusFromWriteHead = st;
          return origWriteHead.apply(res, arguments);
        };
        res.end = function () {
          var status = statusFromWriteHead != null ? statusFromWriteHead
                     : (typeof res.statusCode === "number" ? res.statusCode : 200);
          try {
            s.setAttribute("http.status_code", status);
            // Promote to route template if the matcher set it.
            if (req.routePattern && typeof req.routePattern === "string"
                && req.routePattern !== initialRoute) {
              s.setAttribute("http.route", req.routePattern);
              if (typeof s.updateName === "function") {
                s.updateName("HTTP " + (req.method || "GET") + " " + req.routePattern);
              }
            }
          } catch (_e) {}
