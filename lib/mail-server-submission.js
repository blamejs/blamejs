...
function create(opts) {
  ...
  function _handleConnection(rawSocket) {
    ...
    socket.on("data", function (chunk) {
      // Counted HERE, at the wire, and nowhere else. _ingestBytes re-feeds the
      // tail of a chunk through itself when a BDAT payload and the next command
      // arrive in one packet, so a counter kept inside it credits those bytes
      // twice — a peer pipelining a one-byte payload with its next command
      // would draw roughly double the rate it was really sending. Every wire
      // byte passes this line exactly once.
