...
function create(opts) {
  ...
  function _handleConnection(rawSocket) {
    ...
    // When the DATA body phase began, for the slow-loris byte-rate floor.
    var dataStartedAt = 0;
    ...
    function _ingestBytes(state, socket, chunk) {
      ...
      if (inDataBody) {
        ...
        if (rateLimit.bodyRateStarved(bodyScanner.bytesSeen() + chunk.length,
                                      Date.now() - dataStartedAt)) {
          _emit("mail.server.submission.data_refused",
