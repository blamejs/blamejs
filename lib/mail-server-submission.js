...
function create(opts) {
  ...
  function _handleConnection(rawSocket) {
    ...
    function _ingestBytes(state, socket, chunk) {
      ...
      if (inBdatChunk) {
        ...
        if (bdatRemaining === 0) {
          ...
          if (wasLast) {
            ...
            // BDAT payloads as opaque byte streams; dot-stuffing is a
            // DATA-only artefact. Pass collected bytes straight to the
      ...
      while ((crlf = lineBuffer.indexOf("\r\n")) !== -1) {
        ...
        if (inBdatChunk) {
          ...
          // command they're now sitting in the (UTF-8) lineBuffer; drain
          // them into the BDAT collector before returning so the next
          // socket chunk doesn't double-consume them.
          if (lineBuffer.length > 0) {
            var pendingBytes = Buffer.from(lineBuffer, "utf8");
            lineBuffer = "";
    ...
    function _handleBdat(state, socket, line) {
      ...
      bdatRemaining = sizeN;
      bdatIsLast    = isLast;
      // size=0 + LAST is a valid sequence — finalises an empty message
      // body (matches DATA + immediate `.`).
      if (sizeN === 0) {
