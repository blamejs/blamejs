...
function create(opts) {
  ...
  // The RFC 5233 separator between a local part and its detail — "+" on most
  // systems, "-" on qmail-derived ones. OFF unless the operator names it,
  // because subaddressing is a DELIVERY convention and this framework does not
  // implement delivery: it cannot know that `alice+tag@example.com` reaches
  // `alice@example.com` here. On a deployment that allocates plus-addresses as
  // distinct mailboxes, folding by default would hand one account send-as
  // authority over another's — widening a security boundary on an assumption.
  // Set it to what the delivery side splits on, and a subaddress of the
  // account's own mailbox becomes usable as a sender.
  ...
  function _handleConnection(rawSocket) {
    ...
    var state = {
      ...
      tls:           implicitTls,
      stage:         "connect",
      closed:        false,          // set once by _closeSession; never cleared
      helo:          null,
      authenticated: false,
      actor:         null,
      mailFrom:      null,
    ...
    // A session also ends without this listener deciding it: the peer hangs up,
    // or `close()` destroys a lingering socket at shutdown. Neither runs a
    // refusal, so nothing marked the session, and a command still waiting on a
    // verdict resumed the drain when it answered and ran the peer's pipelined
    // remainder on a connection that no longer exists. Registered on the raw
    // socket, which closes with the upgraded one on top of it.
    ...
    function _ingestBytes(state, socket, chunk) {
      // Bytes already in flight when the session was refused arrive after the
      // socket was destroyed, and buffering them re-fills what the close
      // emptied.
      ...
      // Two different limits, because the buffer holds two different things.
      //
      // The per-line bound is what "Line too long" means, and it applies to
      // each command whether it is drained now or held. The AGGREGATE bound
      // only matters while a verdict is outstanding: the buffer then holds the
      // client's pipelined remainder, and measuring that against the per-line
      // limit would disconnect a legal PIPELINING client (RFC 2920) for the
      // SUM of its commands — a MAIL plus a page of near-limit RCPTs is
      // nothing unusual. A pipelined transaction is MAIL, up to maxRcptsPerMsg
      // recipients, and DATA, so that is what bounds the held total.
      ...
      // A command still waiting on its verdict owns the connection: bytes that
      // arrive in a LATER segment wait in the buffer exactly as pipelined ones
      // do, or the same out-of-order answer arrives by a different route.
      if (state.commandPending) return;
      _drainLines(state, socket);
    }

    // The aggregate bound, asked wherever a command STARTS waiting as well as
    // wherever bytes arrive.
    //
    // Asked only on arrival, it was answered while `commandPending` was still
    // false: an `AUTH` line and an over-cap remainder in one segment are
    // checked before the AUTH is taken, so the remainder was never measured
    // and the completion drained it. What bounds the held total is a command
    // being in flight, and that becomes true inside the drain, not at the
    // socket.
    ...
    // The longest single command in the buffer — every complete line, plus the
    // unterminated tail, which is a line still arriving. What "Line too long"
    // is about, and unaffected by how many commands are queued behind it.
    ...
    // Close the SESSION, not just the socket. A command already waiting on a
    // verdict resumes the drain when it answers, and the buffer it resumes
    // into still holds whatever the client pipelined. Without the stage the
    // resume has nothing to test, so a refusal that destroys the socket is
    // followed by the queued `MAIL` / `RCPT` / `BDAT ... LAST` reaching message
    // finalization on a connection the server already refused.
    // Its own field, not a `stage` value: `stage` is the protocol position and
    // every handler assigns it -- "rcpt" when a sender is accepted, "data-body"
    // at DATA, "ehlo" after STARTTLS -- so a close landing while one of those
    // was in flight would be overwritten by the continuation.
    //
    // Measured: emptying the buffer is what closes the path today, so no
    // sequence reaches the drain with anything left to run even when the
    // marker is overwritten, and no test distinguishes the two spellings. The
    // marker is what keeps it closed if a future edit stops emptying, which is
    // why it is a field nothing else writes rather than a `stage` value.
    function _closeSession(state, socket) {
      state.closed = true;
      lineBuffer = Buffer.alloc(0);
      // The marker above is set synchronously so the reader stops at once;
      // only the socket teardown waits for the reply to flush.
      mailServerNet.destroySocketAfterFlush(socket);
    }

    // Drain whole commands out of the line buffer. Split out of _feedChunk so
    // a handler that had to wait for a verdict can resume the client's
    // pipelined remainder once it has answered.
    function _drainLines(state, socket) {
      ...
      while ((crlf = lineBuffer.indexOf(crlfNeedle)) !== -1) {
        ...
        // A command still waiting on a verdict owns the connection until it
        // answers. Under PIPELINING (RFC 2920) the next lines are already
        // here, and draining them would answer a RCPT before the MAIL it
        // depends on — or let an RSET be undone by the MAIL continuation that
        // lands after it. The bytes stay in the buffer; the handler resumes
        // this loop when it replies.
        // The bound is asked HERE too, because this is where a command starts
        // waiting: the buffer already holds whatever the client pipelined
        // behind it, and on arrival that was measured with nothing pending.
    ...
    function _runAuthStep(state, socket, clientResponse) {
      // Held for the same reason the sender-policy hook holds it: the verify
      // is asynchronous and the AUTH guards read state it writes, so a client
      // that pipelines two credential exchanges in one segment would have both
      // dispatched while the session was still unauthenticated. Set BEFORE the
      // first yield, so the line loop sees it as soon as this returns.
      ...
      return Promise.resolve()
        ...
        .then(function (result) {
          ...
          if (result && result.pending && typeof result.challenge === "string") {
            // A CR / LF / NUL here ends the 334 line early and the remainder
            // reaches the client as a second SMTP reply. Reachable whenever the
            // mechanism composes its challenge from client input, as SCRAM and
            // CRAM do with the client's nonce.
        ...
        // Released once, on every path, and the loop resumes with whatever the
        // client pipelined behind this exchange. Every arm above has already
        // answered by now, including the one that sends a 334 challenge and
    ...
    // Ask the operator's policy whether this actor may speak for this address.
    //
    // Returns true to accept, false to fall through to the refusal, or null
    // when it has ALREADY answered the client — which is what a policy that
    // throws or rejects gets. It fails closed: this is the control that stops
    // an authenticated account sending as someone else, and a policy that
    // could not decide has not decided in the consumer's favour. The refusal
    // is deliberately the same one an unowned address gets, so a broken policy
    // cannot be told apart from a genuine refusal by probing.
    //
    // It takes the socket, like the nineteen other functions here that write to
    // one. Reading the connection closure's binding instead answers on the
    // PLAINTEXT socket: STARTTLS replaces it, and this listener requires the
    // upgrade before it accepts an envelope, so by the time a MAIL FROM reaches
    // a sender policy the captured socket is always the wrong one and the
    // refusal reaches nobody. The client then waits out the idle timeout, which
    // is indistinguishable from a dead server and is the worst shape a refusal
    // can take -- a client that receives no code cannot classify the failure,
    // so it retries the same message.
    ...
    // Async because the identity binding may consult an operator policy that
    // resolves per request. Every caller dispatches it without awaiting, the
    // same way the RCPT handler's policy path already works.
    async function _handleMailFrom(state, socket, line) {
      ...
      // Two different faults, and they were answered with one sentence.
      //
      // A second MAIL with a transaction already open is RFC 5321 section
      // 4.1.4's "sender already specified", and the client is told to issue
      // EHLO -- which it already did, successfully. A reader who trusts the
      // reply goes looking for a session that lost its EHLO, and there is no
      // such session. The verdict was right; only the sentence was wrong.
      //
      // `state.mailFrom` is the condition the RFC names, and the guard tested
      // `state.stage !== "mail"` -- a value nothing in this file ever assigns,
      // so that half never excluded anything.
      // `!== null`, not truthiness: `MAIL FROM:<>` is the null reverse path
      // (RFC 5321 section 4.5.5, the bounce sender) and stores as the EMPTY
      // STRING, which is falsy. A truthiness test would miss exactly the
      // standard case and go on answering it with the wrong sentence. `null`
      // is what _resetTransaction writes, so it is the only value that means
      ...
      if (state.authenticated && identityBinding === "strict") {
        ...
        // Compared at the mailbox the address is DELIVERED to, not at its
        // spelling. RFC 5233 says `alice+tag@example.com` is delivered to
        // `alice@example.com`, so refusing it as a sender makes one identity
        // give two answers — and because the detail part is chosen by whoever
        // writes the address, it can never be enumerated into the set. The
        // fold is applied to both sides, so an entry that itself carries a
        // tag covers the mailbox it lands in too.
        var deliveredTo = mailServerNet.foldSubaddress(mailFrom, subaddressDelimiter);
        // An EMPTY set is "this account has no send-as identity assigned",
        // never "no restriction" — so nothing matches and the refusal below
        // names that case for what it is.
        var matched = false;
        if (allowed.length !== 0) {
        ...
        // The set is fixed at construction, so it cannot answer a question the
        // consumer resolves per request — an alias that lands in this
        // account's mailbox is an address it may legitimately speak for, and
        // nothing in a frozen array can say so. Asked only after the set has
        // declined, so a consumer that supplies none is unaffected and one
        // that does is not consulted on every ordinary send.
        if (!matched && typeof senderPolicy === "function") {
          // Set BEFORE the first yield, so the line loop sees it the moment
          // this function returns its promise and holds the client's
          // pipelined remainder rather than answering past this command.
          state.commandPending = true;
          try {
          ...
          // Every path from here answers the client, and each resumes the
          // loop after doing so — including this one, where the policy has
          // already written its refusal.
      ...
      if (typeof opts.recipientPolicy === "function") {
        ...
        Promise.resolve()
          ...
          .then(function (verdict) {
            ...
            // Same reasoning as the MX listener's recipient refusal: the reason
            // reaches a line-oriented reply, and an operator policy that quotes
            // the destination is quoting something the client supplied. A CR or
            // LF would let the remainder be read as a second server reply.
