  ...
  function _handleConnection(rawSocket) {
    ...
    function _ingestBytes(state, socket, chunk) {
      ...
      if (inBdatChunk) {
        ...
        // The same floor as DATA, on the same window. A body arrives by two
        // paths on this listener and a defence applied to one of them is a
        // defence a client opts out of by advertising CHUNKING — which is the
        // shape that put the bare-LF screen on this path earlier in the same
        // release. Counted across the whole BDAT sequence rather than per
        // chunk, so a trickle cannot restart the measurement with each new
        // BDAT command.
        if (bodyRateWindow.starved(bdatTotalBytes + consumeN, Date.now())) {
      ...
      if (inDataBody) {
        ...
        //
        // Slow-loris floor — the same policy the mx listener applies, from the
        // same place, because a floor enforced on one listener and not its
        // sibling is the shape this release keeps finding.
        if (bodyRateWindow.starved(bodyScanner.bytesSeen() + chunk.length, Date.now())) {
          _emit("mail.server.submission.data_refused",
            { connectionId: state.id, reason: "body-rate-below-floor",
              minBytesPerSecond: rateLimit.minBytesPerSecond() }, "denied");
          _writeReply(socket, REPLY_421_SERVICE_NOT_AVAIL,
    ...
    function _handleBdat(state, socket, line) {
      ...
      // prompt opens it on that path. Re-opening it per BDAT command would let a
      // trickling client restart the measurement with every chunk header and
      // never be judged.
      if (bdatTotalBytes === 0) bodyRateWindow.start(Date.now());
      bdatRemaining = sizeN;
      bdatIsLast    = isLast;
