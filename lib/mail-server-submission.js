...
// Server-side options for an IMPLICIT-TLS listener (submissions / imaps /
// pop3s), where the socket is wrapped rather than handed to tls.createServer.
// RFC 8879 certificate compression rides along for the same reason it does on
// the HTTP listener: the certificate is the same ML-DSA-87 chain and the
// largest thing in the handshake. Omitted rather than passed empty on a
// runtime without the extension.
function _implicitTlsOpts(secureContext) {
  var out = { isServer: true, secureContext: secureContext };
  var certCompression = C.TLS_CERT_COMPRESSION();
  if (certCompression.length > 0) out.certificateCompression = certCompression;
  return out;
}
...
function create(opts) {
  ...
  function _handleConnection(rawSocket) {
    ...
    var socket = implicitTls
      ? new nodeTls.TLSSocket(rawSocket, _implicitTlsOpts(opts.tlsContext))
      : rawSocket;
