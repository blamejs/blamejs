...
// through this helper so the value handed to fs is freshly re-derived
// from `nodePath.resolve(root, ...)` and proven to stay inside the
// served root. The barrier is intentionally re-applied at each sink
// (not just once at request entry) so the relationship between the
// confinement check and the fs call is local + explicit: a candidate
// that escapes the root — via `..`, an absolute path, a Windows
// drive-letter / UNC prefix, or a swapped variable — yields `null`
// and the caller refuses before touching disk.
//
// Returns the resolved, confined absolute path on success, or `null`
// when the candidate is not a string, carries a NUL byte, or resolves
// outside `root`. Callers MUST treat `null` as a refusal.
function _assertInsideRoot(root, candidate) {
  ...
  var resolved = nodePath.resolve(candidate);
  ...
  async function middleware(req, res, next) {
    ...
    if (contentSafety) {
      ...
      if (safetyGate && typeof safetyGate.check === "function") {
        ...
        try {
          gateHandle = await fsp.open(gateConfined, gateOpenFlags);
