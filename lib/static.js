...
async function _readMeta(absPath) {
  ...
  await new Promise(function (resolve, reject) {
    // codeql[js/path-injection] absPath is realpath-validated against `root` by _resolveSafe (lines 181-207, lexical resolve + startsWith + realpath symlink-escape check) before reaching every _readMeta call site; the integrity() public API at line 585 calls nodePath.resolve on operator-supplied absPath. A request URL cannot escape the configured root.
  ...
  async function middleware(req, res, next) {
    ...
    if (contentSafety) {
      ...
      if (safetyGate && typeof safetyGate.check === "function") {
        // codeql[js/file-system-race] absPath is realpath-validated against
        // `root` by _resolveSafe (lines 181-207) before this read. fsp.open
        // + fh.readFile narrows the open-to-read race window to a single
        // fd; an attacker swapping the file mid-serve hits the open() call,
        // not the read.
        var gateBuf;
        var gateFh = null;
        try {
          gateFh = await fsp.open(absPath, "r");
          gateBuf = await gateFh.readFile();
        } catch (_e) {
          stats.failures += 1;
          if (gateFh) { try { await gateFh.close(); } catch (_c) { /* best-effort */ } }
          return _writeError(res, HTTP.INTERNAL_SERVER_ERROR,
            "read_failed", "Internal Server Error");
        }
        try { await gateFh.close(); } catch (_c) { /* best-effort */ }
        var gateDecision;
        try {
    ...
    // codeql[js/path-injection] absPath is realpath-validated against `root` by _resolveSafe (lines 181-207, lexical resolve + startsWith + realpath symlink-escape check) at line 749 before any serve work; a request URL cannot escape the configured root.
