...
function create(opts) {
  ...
  var stmtListFolders      = db.prepare(sql.select(foldersTable, SQL)
    .columns(["id", "name", "role", "parent_id", "modseq_max", "owner"]).toSql().sql);
  ...
  return {
    ...
    createFolder:     function (name, folderOpts) {
      ...
      stmtInsertFolder.run(name, role, parentId, 0, uidvalidity, owner);
...
function _fetchByObjectId(args) {
  ...
  return {
    ...
    hasAttachment:  unsealed.has_attachment === 1,
    attachmentCount: unsealed.attachment_count || 0,
...
function _ensureSchema(db, tables) {
  ...
  _ddl(sql.createTable(messagesTable, [
    ...
    { name: "has_attachment",  type: "int",  notNull: true, default: 0 },
    { name: "attachment_count", type: "int", notNull: true, default: 0 },
  ...
  [["has_attachment", 0], ["attachment_count", 0]].forEach(function (col) {
    if (presentColumns.indexOf(col[0]) !== -1) return;
    try {
      _ddl(sql.alterTable(messagesTable,
        { addColumn: { name: col[0], type: "int", notNull: true, default: col[1] } }, DDL));
    } catch (e) {
      if (_messageColumns().indexOf(col[0]) === -1) throw e;
    }
  });
