...
function _moveMessages(args) {
  ...
  // Both folders share the destination's modseq sequence on move
  // (IMAP4rev2 §6.6.2 — destination MUST bump its modseq for the
  // arrival; source MUST bump for the removal). Per RFC 7162 each
  // folder owns its own modseq counter — we bump both.
  var srcModseq = (fromFolder.modseq_max || 0) + 1;
  var dstModseq = (toFolder.modseq_max  || 0) + 1;
  var movedModseq = Math.max(srcModseq, dstModseq);
  var changed = 0;
  for (var i = 0; i < args.objectids.length; i += 1) {
    var r = args.stmtMoveByObjectId.run(toFolder.id, movedModseq, args.objectids[i], fromFolder.id);
