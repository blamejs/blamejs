...
function _loadAndVerify(name) {
  ...
  // CRYPTO-11 — cross-check meta.publicKeyFingerprint against the
  // inlined pubkey before signature verify. An attacker who has
  // swapped BOTH the payload AND the pubkey module (matched signature
  // under the swap key) would otherwise pass every previous integrity
  // layer; the fingerprint becomes decorative. Compute the
  // sha3-512(PUBKEY_PEM) once at module load (memoized below) and
  // compare each entry's declared fingerprint against the actual
  // pubkey we'll verify under.
...
//
// CRYPTO-10 — when the opt-out env-var is set, emit an audit chain
// row at boot so a misconfigured CI / staging deploy that flips the
// flag in production surfaces in operator review. Defer-boot-verify
// is intended for test rigs only; lacking the audit trail it was a
// silent footgun.
if (safeEnv.readVar("BLAMEJS_VENDOR_DATA_DEFER_BOOT_VERIFY", { default: "" }) !== "1") {
  ...
  // yet (boot sequencing). Try once; drop silent on failure — the
  // env-var itself is observable from the operator's deploy config
  // even when the audit chain doesn't capture it.
  try {
    audit().safeEmit({
      action:   "vendor-data.boot_verify_deferred",
      outcome:  "denied",
      metadata: {
        reason: "BLAMEJS_VENDOR_DATA_DEFER_BOOT_VERIFY=1 in environment — " +
                "verifyAll() skipped at module load. This is a test-rig opt-out; " +
