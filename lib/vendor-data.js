...
function _timingSafeHexEqual(a, b) {
  ...
  // Hex digests are ASCII; UTF-8 encode is identity. Buffer.from
  // accepts the hex strings as utf8 (not "hex"); we're comparing
  // the string representations, not their decoded bytes — either
  // is fine for constant-time-over-length-N.
  var ba = Buffer.from(a, "utf8");
  var bb = Buffer.from(b, "utf8");
  return nodeCrypto.timingSafeEqual(ba, bb);                                    // allow:raw-timing-safe-equal — length-precheck above guarantees same-length inputs; b.crypto wrapper would be circular at boot
}
