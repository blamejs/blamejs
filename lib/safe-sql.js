 ...
 * identifier (`'...'` / `"..."` / a backtick-quoted name) directly follows a
 * word character. The same quote- and comment-aware single pass as
 * `countPlaceholders` / `toPositional` (doubled-quote escapes respected), so a
 * comment marker inside a string literal is copied verbatim, never collapsed.
 * The executed SQL is unchanged — this copy only feeds a scanner.
 *
 * @example
 *   var b = require("blamejs");
...
function normalizeForScan(sql) {
  ...
  while (i < len) {
    ...
    if (c === "'" || c === '"' || c === "`") {
      ...
      // downstream whitespace-anchored tokenizer sees the boundary; then copy
      // the whole quoted run verbatim (doubled-quote escapes preserved) so a
      // comment marker inside the literal is never collapsed.
