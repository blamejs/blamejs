 ...
 *
 * Public API:
 *   sqlSafe.validateIdentifier(name, opts?)        throws on bad shape
 *   sqlSafe.quoteIdentifier(name, dialect?)        returns "name" / `name`
 *   sqlSafe.assertOneOf(name, allowlist)           throws unless in list
 *   sqlSafe.SqlSafeError                           error class
 *
 ...
 *   sqlSafe.assertOneOf(operatorTableName, ALLOWED_TABLES);
 *   var sql = "INSERT INTO " + sqlSafe.quoteIdentifier(operatorTableName) + " …";
 *
...
class SqlSafeError extends FrameworkError {
  constructor(message, code) {
    super(message);
    this.name = "SqlSafeError";
    this.code = code || "sql/invalid";
    this.isSqlSafeError = true;
  }
}
...
function validateIdentifier(name, opts) {
  ...
  if (typeof name !== "string") {
    throw new SqlSafeError("identifier must be a string (got " + typeof name + ")",
      "sql/bad-type");
  }
  if (name.length === 0) {
    throw new SqlSafeError("identifier must not be empty", "sql/empty");
  }
  if (name.length > MAX_IDENTIFIER_LENGTH) {
    throw new SqlSafeError(
      "identifier '" + name + "' exceeds maxLength " + MAX_IDENTIFIER_LENGTH,
      "sql/too-long"
    );
  }
  if (name.indexOf("\0") >= 0) {
    throw new SqlSafeError("identifier contains null byte", "sql/null-byte");
  }
...
function assertOneOf(name, allowlist) {
  if (typeof name !== "string") {
    throw new SqlSafeError("name must be a string", "sql/bad-type");
  }
  ...
  if (allowlist && typeof allowlist.has === "function") {
    ...
    throw new SqlSafeError("allowlist must be a Set or array", "sql/bad-allowlist");
  }
  if (!ok) {
    throw new SqlSafeError(
