 ...
 *   }
 */
var SafeSqlError = frameworkError.defineMessageFirstClass(
  "SafeSqlError", "sql/invalid");
...
function validateIdentifier(name, opts) {
  ...
  if (opts.pattern) {
    if (!(opts.pattern instanceof RegExp)) {
      throw new SafeSqlError("opts.pattern must be a RegExp (got " + typeof opts.pattern + ")",
        "sql/bad-pattern");
    }
    try {
    ...
    try {
      guardRegex().assertStateless(opts.pattern, "safeSql.validateIdentifier pattern");
    } catch (stateful) {
      throw new SafeSqlError(stateful.message, "sql/stateful-pattern");
    }
...
function quoteIdentifier(name, dialect, opts) {
  ...
  if (dialect === "mysql") {
    ...
    return bt + name.split(bt).join(bt + bt) + bt;
  }
...
function isDefaultIdentifier(name) {
  ...
  if (!codepointClass.isAsciiLetter(first) && first !== 0x5F) return false;
...
function assertSingleStatement(sql, opts) {
  ...
  while (i < len) {
    ...
    if (ch === "'" || ch === '"' || ch === BACKTICK) {
      ...
      while (i < len) {
        if (sql.charAt(i) === qch) {
          if (sql.charAt(i + 1) === qch) { i += 2; continue; }
