...
var DEFAULTS = Object.freeze({
  ...
  // Audit defaults follow b.apiKey: failures are actionable security
  // signals (bad signature / replay / expired = potential attacks);
  // successes are noise per request. Operators pass `audit: b.audit`
  // to receive emissions; the audit chain captures failures by
  // default and successes only when explicitly opted in.
  auditFailures:   true,
  auditSuccess:    false,
});
...
function verifier(opts) {
  ...
  function _auditEmit(outcome, reason, info) {
    if (!audit) return;
  ...
  function _failure(code, message, reason) {
    _emitEvent("webhook.verify", 1, { outcome: "failure", reason: reason || code });
    _auditEmit("failure", reason || code, null);
  ...
  async function verify(input) {
    ...
    if (parsed.t === null && parsed.id === null && Object.keys(parsed.sigs).length === 0) {
      throw _failure("BAD_HEADER_FORMAT", "webhook: signature header could not be parsed", "bad-header-format");
    }
    if (parsed.t === null) {
      throw _failure("MISSING_TIMESTAMP", "webhook: t= field missing from signature header", "missing-timestamp");
    }
    var ts = Number(parsed.t);
    if (!isFinite(ts) || ts < 0 || Math.floor(ts) !== ts) {
      throw _failure("BAD_TIMESTAMP", "webhook: t= field is not a non-negative integer, got " + JSON.stringify(parsed.t), "bad-timestamp");
    }
    if (parsed.id === null || parsed.id.length === 0) {
      throw _failure("MISSING_ID", "webhook: id= field missing from signature header", "missing-id");
    }
    var sigKids = Object.keys(parsed.sigs);
    if (sigKids.length === 0) {
      throw _failure("MISSING_SIGNATURE", "webhook: no v<kid>= segment found in signature header", "missing-signature");
    }
    ...
    if (ageMs > toleranceMs) {
      throw _failure("EXPIRED", "webhook: timestamp older than toleranceMs (age=" + ageMs + "ms)", "expired");
    }
    if (-ageMs > clockSkewMs) {
      throw _failure("FUTURE", "webhook: timestamp in the future beyond clockSkewMs (skew=" + (-ageMs) + "ms)", "future");
    }
    ...
    if (!ok) {
      throw _failure("BAD_SIGNATURE", "webhook: cryptographic verification failed", "bad-signature");
    }
    ...
    if (nonceStore) {
      ...
      if (!fresh) {
        throw _failure("REPLAY", "webhook: id '" + parsed.id + "' has been seen before", "replay");
      }
    }

    _emitEvent("webhook.verify", 1, { outcome: "success", kid: matchedKid });
    _auditEmit("success", null, { kid: matchedKid });
