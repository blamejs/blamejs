 ...
 *
 *   PQC signatures are emitted as base64url (66,475 characters for an
 *   SLH-DSA-SHAKE-256f signature, against 99,712 as hex) to fit common
 *   front-end header caps; the verifier accepts EITHER encoding for
 *   transition windows.
 *
...
function _validateAlgo(algo) {
  if (algo !== ALGOS.HMAC_SHA3_512 && algo !== ALGOS.PQC_PEM) {
    throw _err("webhook/bad-opt", "webhook: algo must be one of " +
...
function _validateKeysShape(name, algo, keys, side) {
  if (!keys || typeof keys !== "object" || Array.isArray(keys)) {
    throw _err("webhook/bad-opt", name + ": keys must be a non-empty object map of kid → key, got " +
      typeof keys);
  }
  var kids = _objectKeys(keys);
  if (kids.length === 0) {
    throw _err("webhook/bad-opt", name + ": keys must have at least one kid entry");
  }
  for (var i = 0; i < kids.length; i++) {
    var kid = kids[i];
    if (kid.length === 0 || /[,=\s]/.test(kid)) {
      throw _err("webhook/bad-opt", name + ": kid must be non-empty and contain no comma/equals/whitespace, got " +
        JSON.stringify(kid));
    }
    if (kid === "t" || kid === "id") {
      throw _err("webhook/bad-opt", name + ": kid '" + kid + "' is reserved (collides with header field)");
    }
    var k = keys[kid];
    if (algo === ALGOS.HMAC_SHA3_512) {
      if (!Buffer.isBuffer(k) && typeof k !== "string") {
        throw _err("webhook/bad-opt", name + ": HMAC key for kid '" + kid +
          "' must be a Buffer or string, got " + typeof k);
      }
      if (k.length === 0) {
        throw _err("webhook/bad-opt", name + ": HMAC key for kid '" + kid + "' must be non-empty");
      }
      ...
      if (side === "signer") {
        if (!k || typeof k !== "object" || Array.isArray(k) ||
          ...
          throw _err("webhook/bad-opt", name + ": PQC signer key for kid '" + kid +
            "' must be { privateKey, publicKey } as PEM strings/Buffers");
        }
      } else {
        if (typeof k !== "string" && !Buffer.isBuffer(k)) {
          throw _err("webhook/bad-opt", name + ": PQC verifier key for kid '" + kid +
...
function _assertPqcAlgorithmMatches(name, pqcAlgorithm, keys, side) {
  ...
  if (PQC_ALGORITHMS.indexOf(pqcAlgorithm) === -1) {
    throw _err("webhook/bad-opt", name + ": pqcAlgorithm must be one of " +
  ...
  for (var i = 0; i < kids.length; i++) {
    ...
    if (detected && detected !== pqcAlgorithm) {
      throw _err("webhook/bad-opt", name + ": pqcAlgorithm '" + pqcAlgorithm +
...
function _validateBody(body) {
  if (typeof body !== "string" && !Buffer.isBuffer(body)) {
    throw _err("webhook/bad-body", "webhook: body must be a string or Buffer, got " + typeof body);
  }
}
...
function _parseSignatureHeader(headerValue) {
  ...
  for (var i = 0; i < segs.length; i++) {
    ...
    else if (name.length > 0) sigs[name] = value;
  }
...
function _validateSignerOpts(opts) {
  ...
  if (opts.defaultKid !== undefined) {
    if (!_hasOwn(opts.keys, opts.defaultKid)) {
      throw _err("webhook/bad-opt", "webhook.signer: defaultKid '" + opts.defaultKid +
        "' not present in keys (have: " + JSON.stringify(kids) + ")");
    }
  } else if (kids.length > 1) {
    throw _err("webhook/bad-opt", "webhook.signer: defaultKid required when keys has " +
      kids.length + " entries");
  }
  ...
  if (opts.pqcAlgorithm !== undefined) {
    if (opts.algo !== ALGOS.PQC_PEM) {
      throw _err("webhook/bad-opt", "webhook.signer: pqcAlgorithm only meaningful with algo='pqc-pem'");
    }
...
function signer(opts) {
  ...
  function _signOne(body, kid) {
    ...
    if (keyForKid == null) {
      throw _err("webhook/bad-opt", "webhook.signer: unknown kid '" + kid + "'");
    }
    var timestamp = Math.floor(nowFn() / C.TIME.seconds(1));
    var id = idGen();
    if (typeof id !== "string" || id.length === 0 || /[,=\s]/.test(id)) {
      throw _err("webhook/bad-opt", "webhook.signer: idGenerator must return a non-empty string with no comma/equals/whitespace, got " + JSON.stringify(id));
    }
  ...
  async function send(input) {
    if (!input || typeof input !== "object") {
      throw _err("webhook/bad-opt", "webhook.signer.send: input must be { url, body, headers? }");
    }
    var url = input.url;
    var body = input.body;
    safeUrl.parse(url, {
    ...
    try {
      ...
      if (e && e.isSsrfError) throw _err("webhook/ssrf-refused", "webhook.signer.send: " + e.message);
      throw e;
    }
    _validateBody(body);
...
function _validateVerifierOpts(opts) {
  ...
  validateOpts.optionalObjectWithMethod(opts.nonceStore, "checkAndInsert",
    "webhook.verifier: nonceStore", WebhookError, "webhook/bad-opt",
  ...
  if (opts.pqcAlgorithm !== undefined) {
    if (opts.algo !== ALGOS.PQC_PEM) {
      throw _err("webhook/bad-opt", "webhook.verifier: pqcAlgorithm only meaningful with algo='pqc-pem'");
    }
 ...
 * with a stable `code` (`webhook/missing-header` / `webhook/bad-header-format` /
 * `EXPIRED` / `FUTURE` / `webhook/unknown-kid` / `webhook/bad-signature` / `REPLAY` /
...
function verifier(opts) {
  ...
  async function verify(input) {
    if (!input || typeof input !== "object") {
      throw _err("webhook/bad-opt", "webhook.verifier.verify: input must be { body, headers }");
    }
    var headers = input.headers;
    if (!headers || typeof headers !== "object") {
      throw _err("webhook/bad-opt", "webhook.verifier.verify: headers must be an object");
    }
    var body = input.body;
    _validateBody(body);
    ...
    if (typeof headerValue !== "string" || headerValue.length === 0) {
      throw _failure("webhook/missing-header",
    ...
    if (parsed.t === null && parsed.id === null && Object.keys(parsed.sigs).length === 0) {
      throw _failure("webhook/bad-header-format", "webhook: signature header could not be parsed", "bad-header-format", ctxReq);
    }
    if (parsed.t === null) {
      throw _failure("webhook/missing-timestamp", "webhook: t= field missing from signature header", "missing-timestamp", ctxReq);
    }
    var ts = Number(parsed.t);
    if (!numericBounds.isNonNegativeFiniteInt(ts) || String(ts) !== parsed.t) {
      throw _failure("webhook/bad-timestamp", "webhook: t= field is not a canonical non-negative integer, got " + JSON.stringify(parsed.t), "bad-timestamp", ctxReq);
    }
    if (parsed.id === null || parsed.id.length === 0) {
      throw _failure("webhook/missing-id", "webhook: id= field missing from signature header", "missing-id", ctxReq);
    }
    var sigKids = Object.keys(parsed.sigs);
    if (sigKids.length === 0) {
      throw _failure("webhook/missing-signature", "webhook: no v<kid>= segment found in signature header", "missing-signature", ctxReq);
    }

    var nowMs = nowFn();
    var ageMs = nowMs - C.TIME.seconds(ts);
    if (ageMs > toleranceMs) {
      throw _failure("webhook/expired", "webhook: timestamp older than toleranceMs (age=" + ageMs + "ms)", "expired", ctxReq);
    }
    if (-ageMs > clockSkewMs) {
      throw _failure("webhook/future", "webhook: timestamp in the future beyond clockSkewMs (skew=" + (-ageMs) + "ms)", "future", ctxReq);
    }
    ...
    if (matchedKid === null) {
      throw _failure("webhook/unknown-kid",
    ...
    if (!ok) {
      throw _failure("webhook/bad-signature", "webhook: cryptographic verification failed", "bad-signature", ctxReq);
    }
    ...
    if (nonceStore) {
      ...
      if (!fresh) {
        throw _failure("webhook/replay", "webhook: id '" + parsed.id + "' has been seen before", "replay", ctxReq);
      }
    }
...
var STRIPE_DEFAULT_TOLERANCE_MS = C.TIME.minutes(5);
var STRIPE_MIN_TOLERANCE_MS = C.TIME.seconds(30);
