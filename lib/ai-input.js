...
var PATTERNS = [
  ...
  { id: "base64-marker-around-instructions", severity: 2, re:
    /(?<![A-Za-z0-9+/])[A-Za-z0-9+/]{40,}={0,2}\s+(?:means|decodes?\s+to|=)/i },
 ...
 *                  //     sample: "Ignore all prior instructions" }
 */
function classify(input, opts) {
  opts = opts || {};
  var errorClass = opts.errorClass || AiInputError;
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxBytes, "aiInput.classify: opts.maxBytes", errorClass, "ai-input/bad-max-bytes");
...
function classifyWithSources(input, sources, opts) {
  ...
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxSources, "aiInput.classifyWithSources: opts.maxSources", errorClass, "ai-input/bad-max-sources");
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxSourceBytes, "aiInput.classifyWithSources: opts.maxSourceBytes", errorClass, "ai-input/bad-max-source-bytes");
  var maxSources = opts.maxSources || DEFAULT_MAX_SOURCES;
