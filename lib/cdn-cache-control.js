...
function parse(headerValue) {
  ...
  for (var p = 0; p < pieces.length; p += 1) {
    ...
    var eq = raw.indexOf("=");
    var key, val;
    if (eq === -1) { key = raw.toLowerCase(); val = true; }
    else {
      ...
      // numeric, and the spec says quoted-string is also valid).
    ...
    if (NUMERIC_DIRECTIVES.indexOf(key) !== -1) {
      var n = Number(val);
      if (isFinite(n) && n >= 0) {
        var camel = _camel(key);
        out[camel] = Math.floor(n);
      }
      continue;
    }
    // Boolean directive → set the camelCase flag.
    if (BOOLEAN_DIRECTIVES.indexOf(key) !== -1) {
      out[_camel(key)] = (val === true || val === "" || val === "true");
      continue;
    }
    // Unknown / extension directive → land in `directives` map.
    out.directives[key] = val;
  }
  return out;
}
