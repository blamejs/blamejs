...
// RFC 9111 §5.2 token: `token = 1*tchar` where tchar excludes delimiter
// chars. Directive keys are tchar-only and conventionally ASCII-lower.
var DIRECTIVE_KEY_RE = /^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/;                                           // allow:duplicate-regex — RFC 9111 §5.2 tchar (=RFC 8941 token shape)
...
function build(opts) {
  ...
  // Operator-supplied extension directives (must be RFC 9111 §5.2 tchar
  ...
  if (opts.extensions !== undefined && opts.extensions !== null) {
    ...
    // regex. RFC 9111 §5.2 token directives are tiny in practice
    ...
    for (var e = 0; e < ekeys.length; e += 1) {
      ...
      if (ek.length === 0 || ek.length > DIRECTIVE_MAX || !DIRECTIVE_KEY_RE.test(ek)) {
        throw new CdnCacheControlError("cdn-cache-control/bad-extension-key",
          "build: extensions['" + ek + "'] — key must match RFC 9111 §5.2 token grammar " +
          "(<= " + DIRECTIVE_MAX + " chars)");
      }
      if (seen[ek]) continue;
      ...
      if (ev === true) {
        ...
        // Value must be either RFC 9111 §5.2 token OR sf-string quoted.
        ...
        if (ev.length === 0 || ev.length > DIRECTIVE_MAX || !DIRECTIVE_KEY_RE.test(ev)) {
          throw new CdnCacheControlError("cdn-cache-control/bad-extension-value",
            "build: extensions['" + ek + "'] string value must match RFC 9111 §5.2 token grammar " +
            "(<= " + DIRECTIVE_MAX + " chars); " +
