...
var DIRECTIVE_KEY_RE = /^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/;                                           // allow:duplicate-regex — RFC 7234 §5.2 tchar (=RFC 9651 token shape)
...
function parse(headerValue) {
  ...
  for (var p = 0; p < kvps.length; p += 1) {
    ...
    if (kvp.value === null) {
      ...
      val = kvp.value.trim();
      bare = false;
      // Unquote sf-string per RFC 9651 §3.3.3 (defensive — operators
