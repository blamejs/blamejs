...
function parse(headerValue) {
  ...
  // Control-byte refusal on RAW value (same discipline as
  // b.mail.requireTls.parseTlsRequiredHeader — leading/trailing CR/LF
  // must NOT be trim()ed away before the gate fires).
  for (var c = 0; c < headerValue.length; c += 1) {
    var cc = headerValue.charCodeAt(c);
    if (cc === 9) continue;                                                                        // allow:raw-byte-literal — ASCII HT (folding whitespace)
    if (cc < 32 || cc === 127) {                                                                  // allow:raw-byte-literal — C0 + DEL codepoint range
      throw new CdnCacheControlError("cdn-cache-control/bad-header-value",
        "parse: value contains control characters");
    }
  }
  ...
  var out = { directives: {} };
  var pieces = _splitTopLevelCommas(trimmed);
  for (var p = 0; p < pieces.length; p += 1) {
    ...
    if (eq === -1) {
      ...
      // arrives here too — `_splitTopLevelCommas` already preserved
      // the inner comma; this just strips the surrounding quotes so
      // operators read the field-name list directly.
...
// Split a directive list on top-level commas, respecting RFC 8941
// quoted-string boundaries so `private="Authorization, Cookie"` stays
// as one piece. Tolerates backslash-escapes inside the quoted-string
// per RFC 8941 §3.3.3. Operators upstream of this primitive
// (proxies, framework middleware) already split CRLF; this defends
// the comma-split path against quoted commas in qualified-form
// directive values (RFC 9111 §5.2.2.4 / §5.2.2.6).
function _splitTopLevelCommas(s) {
  ...
  for (var i = 0; i <= s.length; i += 1) {
    ...
    if (ch === "," && i < s.length) {
      ...
      // Sentinel pseudo-comma at end-of-string flushes the trailing
      // piece. We can only reach this branch when inQuote is false:
      // the top-of-loop `if (inQuote) continue` would have absorbed
      // the sentinel under a still-open quoted-string, so an
      // unterminated `"..."` at end-of-input drops its trailing
      // piece automatically without an explicit guard.
