...
/**
 * @primitive b.cdnCacheControl.keepNoStore
 * @signature b.cdnCacheControl.keepNoStore(res, fallback)
 * @since     0.20.30
 * @status    stable
 * @related   b.cdnCacheControl.parse, b.middleware.noCache
 *
 * Choose the `Cache-Control` value a framework writer sends in place of its
 * own default. When `res` already carries a `Cache-Control` with the
 * `no-store` directive, such as the one `b.middleware.noCache` sets, it
 * returns that value, adding `no-transform` when `fallback` has it and the
 * existing value does not. Otherwise it returns `fallback`. A `Cache-Control`
 * that `b.cdnCacheControl.parse` refuses throws
 * `cdn-cache-control/bad-header-value`.
 *
 * `b.render`, `b.middleware.sse`, `b.middleware.openapiServe`,
 * `b.middleware.asyncapiServe`, `b.middleware.securityTxt`, `b.middleware.assetlinks`,
 * `b.middleware.webAppManifest`, `b.middleware.protectedResourceMetadata`,
 * `b.a2a.middleware.agentCard` and the JMAP EventSource handler choose their
 * `Cache-Control` this way.
 *
 * @example
 *   res.setHeader("Cache-Control", "no-store");
 *   b.cdnCacheControl.keepNoStore(res, "private, no-cache, must-revalidate");
 *   // → "no-store"
 *   b.cdnCacheControl.keepNoStore(res, "no-cache, no-transform");
 *   // → "no-store, no-transform"
 */
function keepNoStore(res, fallback) {
  if (!res || typeof res.getHeader !== "function") return fallback;
  var stated = res.getHeader("Cache-Control");
  if (Array.isArray(stated)) stated = stated.join(", ");
  if (typeof stated !== "string") return fallback;
  var parsed = parse(stated);
  if (!parsed || parsed.noStore !== true) return fallback;
  var fallbackParsed = parse(fallback);
  if (fallbackParsed && fallbackParsed.noTransform === true && parsed.noTransform !== true) {
    return stated + ", no-transform";
  }
  return stated;
}
