 ...
 *
 * Public API:
 *   yaml.parse(input, opts?)        value | throws YamlSafeError
 *   yaml.YamlSafeError              error class
 *
 * Defaults:
 *   maxBytes:  1 MiB
 *   maxDepth:  100
...
function parse(input, opts) {
  ...
  else if (input instanceof Uint8Array)   input = Buffer.from(input).toString("utf8");
  else if (typeof input !== "string") {
    throw new YamlSafeError("input must be string, Buffer, or Uint8Array", "yaml/wrong-input-type");
  }
  ...
  if (Buffer.byteLength(input, "utf8") > maxBytes) {
    throw new YamlSafeError("input exceeds maxBytes", "yaml/too-large");
  }
  ...
  for (var i = 0; i < rawLines.length; i++) {
    ...
    if (indent < raw.length && raw.charAt(indent) === "\t") {
      throw new YamlSafeError("tab in indentation (YAML 1.2 forbids)", "yaml/tab-indent", i + 1, indent + 1);
    }
  ...
  function _bumpKeys(line) {
    keyCount += 1;
    if (keyCount > maxKeys) {
      throw new YamlSafeError("input exceeds maxKeys", "yaml/too-many-keys", line, 1);
    }
  }
  ...
  function parseValueAtLine(startIdx, parentIndent, depth) {
    if (depth > maxDepth) {
      throw new YamlSafeError("input exceeds maxDepth", "yaml/too-deep",
  ...
  function _scanKeyRange(content, lineNumber, indent) {
    ...
    if (content.charAt(0) === "?") {
      throw new YamlSafeError("complex keys (`? key`) are not supported",
  ...
  function _decodeKeyLiteral(raw, lineNumber, col) {
    ...
    if (POISONED_KEYS.has(trimmed)) {
      throw new YamlSafeError("forbidden key '" + trimmed + "'",
      ...
      if (afterColon.length > 0) {
        ...
        if (afterColon.charAt(0) === "|" || afterColon.charAt(0) === ">") {
          ...
          if (!/^[|>][-+]?[0-9]?\s*$/.test(afterColon)) {
            throw new YamlSafeError("malformed block scalar header",
  ...
  function _parseBlockSequence(startIdx, indent, depth) {
    ...
    while (k < lines.length) {
      ...
      if (ln.indent < indent) break;
      if (ln.indent !== indent) {
        throw new YamlSafeError("unexpected indent in sequence",
  ...
  function _parseInlineValue(text, lineNumber, col) {
    ...
    if (t.charAt(0) === '"') {
      ...
      if (afterDq.length > 0 && afterDq.replace(/^\s+/, "") !== "") {
        throw new YamlSafeError("unexpected content after quoted string",
  ...
  function _parseFlowSequence(text, lineNumber, col, depthIncoming) {
    ...
    while (p < text.length) {
      ...
      if (text.charAt(p) === "]") { return arr; }
      throw new YamlSafeError("expected ',' or ']' in flow sequence",
        "yaml/bad-flow", lineNumber, col + p);
    }
    throw new YamlSafeError("unterminated flow sequence",
  ...
  function _parseFlowMapping(text, lineNumber, col, depthIncoming) {
    if (depthIncoming > maxDepth) {
      throw new YamlSafeError("input exceeds maxDepth", "yaml/too-deep", lineNumber, col);
    }
    var p = 1;
    var result = Object.create(null);
    while (p < text.length) {
      ...
      var key = keyVal.key;
      if (typeof key !== "string") {
        throw new YamlSafeError("non-string flow-mapping key",
      ...
      p = _flowSkipWsIndex(text, p);
      if (text.charAt(p) !== ":") {
        throw new YamlSafeError("expected ':' in flow mapping",
          "yaml/bad-flow", lineNumber, col + p);
      }
      p += 1;
      p = _flowSkipWsIndex(text, p);
      ...
      if (text.charAt(p) === ",") { p += 1; continue; }
      if (text.charAt(p) === "}") { return result; }
      throw new YamlSafeError("expected ',' or '}' in flow mapping",
        "yaml/bad-flow", lineNumber, col + p);
    }
    throw new YamlSafeError("unterminated flow mapping",
      "yaml/unterminated-flow", lineNumber, col);
  }

  function _parseFlowValue(text, p, lineNumber, col, depthIncoming) {
    if (depthIncoming > maxDepth) {
      throw new YamlSafeError("input exceeds maxDepth", "yaml/too-deep", lineNumber, col + p);
    }
  ...
  function _findMatchingBracket(text, start, open, close, lineNumber, col) {
    ...
    throw new YamlSafeError("unterminated flow brackets",
  ...
  function _findClosingQuote(text, start, quote, lineNumber, col) {
    ...
    throw new YamlSafeError("unterminated quoted string",
  ...
  function _decodeDoubleQuoted(raw, lineNumber, col) {
    if (raw.charAt(0) !== '"') {
      throw new YamlSafeError("expected '\"'", "yaml/bad-string", lineNumber, col);
    }
    var i = 1;
    var out = "";
    while (i < raw.length) {
        ...
        switch (esc) {
          ...
          case "u": {
            ...
            if (!/^[0-9a-fA-F]{4}$/.test(hex)) {
              throw new YamlSafeError("bad \\u escape", "yaml/bad-escape", lineNumber, col + i);
            }
          ...
          case "U": {
            ...
            if (code > 0x10FFFF) {
              throw new YamlSafeError("\\U code point > U+10FFFF",
          ...
          default:
            throw new YamlSafeError("unknown escape '\\" + esc + "'",
              "yaml/bad-escape", lineNumber, col + i);
        }
        continue;
      }
    ...
    throw new YamlSafeError("unterminated double-quoted string",
  ...
  function _decodeSingleQuoted(raw, lineNumber, col) {
    ...
    throw new YamlSafeError("unterminated single-quoted string",
      "yaml/unterminated-string", lineNumber, col);
  }
  ...
  function _parseBlockScalar(startIdx, parentIndent, headerContent) {
    ...
    for (var i = 0; i < rest.length; i++) {
      ...
      if (ch === "-" || ch === "+") {
        if (chomp) throw new YamlSafeError("multiple chomping indicators",
          "yaml/bad-block-scalar", headerLine.lineNumber, parentIndent + 1);
        chomp = ch;
      } else if (ch >= "1" && ch <= "9") {
        if (explicitIndent != null) throw new YamlSafeError("multiple indentation indicators",
          "yaml/bad-block-scalar", headerLine.lineNumber, parentIndent + 1);
...
function _preValidate(input) {
  ...
  if (mt) {
    ...
    throw new YamlSafeError("tags are not supported",
  ...
  if (md) {
    ...
    throw new YamlSafeError("directives are not supported",
