...
class CsvSafeError extends Error {
  constructor(message, code, position) {
    super(message);
    this.name = "CsvSafeError";
    this.code = code || "csv/invalid";
    this.position = position || null;
    this.isCsvSafeError = true;
  }
}
...
function parse(input, opts) {
  ...
  if (Buffer.isBuffer(input))            input = input.toString("utf8");
    ...
    throw new CsvSafeError("input must be string, Buffer, or Uint8Array", "csv/wrong-input-type");
  }
  if (input.charCodeAt(0) === 0xFEFF) input = input.slice(1);
  if (Buffer.byteLength(input, "utf8") > opts.maxBytes) {
    throw new CsvSafeError("input exceeds maxBytes", "csv/too-large");
  }
  ...
  function pushField() {
    if (Buffer.byteLength(field, "utf8") > opts.maxFieldBytes) {
      throw new CsvSafeError("field exceeds maxFieldBytes at row " + (rows.length + 1), "csv/field-too-large");
    }
  ...
  function pushRow() {
    ...
    if (rows.length > opts.maxRows) {
      throw new CsvSafeError("row count exceeds maxRows", "csv/too-many-rows");
    }
    row = [];
  }
  ...
  if (inQuote) throw new CsvSafeError("unterminated quoted field", "csv/unterminated-quote");
...
function stringify(rows, opts) {
  ...
  if (!Array.isArray(rows)) {
    throw new CsvSafeError("stringify expects an array of rows", "csv/wrong-input-type");
  }
  if (rows.length === 0) return "";
  ...
  if (Array.isArray(rows[0])) {
    ...
    throw new CsvSafeError("rows must be arrays or objects", "csv/wrong-input-type");
  }
