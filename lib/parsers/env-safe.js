 ...
 *
 * Public API:
 *   env.parse(input, opts?)            object | throws EnvSafeError
 *   env.load(filepath, opts?)          { values, diff } | throws
 *   env.EnvSafeError                   error class
 *
...
class EnvSafeError extends Error {
...
function parse(input, opts) {
  ...
  if (Buffer.isBuffer(input))             input = input.toString("utf8");
    ...
    throw new EnvSafeError("input must be string, Buffer, or Uint8Array", "env/wrong-input-type");
  }
  ...
  if (Buffer.byteLength(input, "utf8") > maxBytes) {
    throw new EnvSafeError("input exceeds maxBytes", "env/too-large");
  }
  ...
  for (var i = 0; i < rawLines.length; i++) {
    ...
    if (eqIdx < 0) {
      throw new EnvSafeError("missing '=' separator", "env/bad-line", lineNumber);
    }
    var key = trimmed.substring(0, eqIdx).replace(/[ \t]+$/, "");
    var rest = trimmed.substring(eqIdx + 1);

    if (key.length === 0) {
      throw new EnvSafeError("empty key", "env/empty-key", lineNumber);
    }
    if (POISONED_KEYS.has(key)) {
      throw new EnvSafeError("forbidden key '" + key + "'", "env/poisoned-key", lineNumber);
    }
    ...
    if (seen.has(key)) {
      throw new EnvSafeError("duplicate key '" + key + "'", "env/duplicate-key", lineNumber);
    }
    ...
    if (seen.size > maxKeys) {
      throw new EnvSafeError("input exceeds maxKeys", "env/too-many-keys", lineNumber);
    }
  }
...
function _decodeDoubleQuoted(rest, lineNumber) {
      ...
      switch (esc) {
        ...
        case "u": {
          ...
          if (!/^[0-9a-fA-F]{4}$/.test(hex)) {
            throw new EnvSafeError("bad \\u escape", "env/bad-escape", lineNumber);
          }
        ...
        default:
          throw new EnvSafeError("unknown escape '\\" + esc + "'", "env/bad-escape", lineNumber);
      }
    }
    if (ch === '"') {
  ...
  throw new EnvSafeError("unterminated double-quoted value", "env/unterminated-string", lineNumber);
}
...
function _decodeSingleQuoted(rest, lineNumber) {
  ...
  if (end < 0) {
    throw new EnvSafeError("unterminated single-quoted value", "env/unterminated-string", lineNumber);
  }
...
function _coerceType(rawValue, type, key) {
  ...
  if (type === "number") {
    ...
    if (Number.isNaN(n)) {
      throw new EnvSafeError("'" + rawValue + "' is not a number for key '" + key + "'",
        "env/bad-type");
    }
    return n;
  }
  ...
  if (type === "json") {
    ...
    try { return jsonSafe.parse(rawValue); }
    catch (e) {
      throw new EnvSafeError("invalid JSON for key '" + key + "': " + e.message,
        "env/bad-type");
    }
  }
  throw new EnvSafeError("unknown type '" + type + "' for key '" + key + "'",
    "env/bad-schema");
}
...
function load(filepath, opts) {
  if (typeof filepath !== "string") {
    throw new EnvSafeError("load requires a file path", "env/bad-arg");
  }
  opts = opts || {};
  ...
  if (expected) {
    for (var k in expected) {
      if (Object.prototype.hasOwnProperty.call(rawValues, k)) {
        ...
        throw new EnvSafeError("required key '" + k + "' missing from " + filepath,
  ...
  if (snapshotPath && atomicFile.exists(snapshotPath)) {
    try {
      ...
      var jsonSafe = require("./../json-safe");
      prevValues = jsonSafe.parse(snapBuf) || {};
  ...
  function _safeRecord(action, metadata) {
    try {
      ...
      if (typeof console !== "undefined") {
        console.error("[blamejs:env] audit.record failed: " + e.message);
      }
    }
  }
...
function readVar(name, schema) {
  if (typeof name !== "string" || name.length === 0) {
    throw new EnvSafeError("readVar requires a non-empty name", "env/bad-arg");
  }
  schema = schema || {};
  ...
  if (!present) {
    ...
    if (required) {
      throw new EnvSafeError(name + " env var is not set or is empty", "env/missing-required");
    }
    return undefined;
  }
