 ...
 *
 * Public API:
 *   toml.parse(input, opts?)            object | throws TomlSafeError
 *   toml.TomlSafeError                  error class
 *
 * Defaults:
 *   maxBytes:  1 MiB
 *   maxDepth:  100
...
var TIME_ONLY_RE = /^\d{2}:\d{2}:\d{2}(?:\.\d+)?$/;
...
function parse(input, opts) {
  ...
  if (Buffer.isBuffer(input))             input = input.toString("utf8");
    ...
    throw new TomlSafeError("input must be string, Buffer, or Uint8Array", "toml/wrong-input-type");
  }
  ...
  if (Buffer.byteLength(input, "utf8") > maxBytes) {
    throw new TomlSafeError("input exceeds maxBytes", "toml/too-large");
  }
  ...
  function _err(msg, code) {
    return new TomlSafeError(msg, code || "toml/invalid", line, col);
  }
  ...
  function _parseNumber(firstChar) {
    ...
    if (_peek() === "0") {
      ...
      if (prefix === "x" || prefix === "o" || prefix === "b") {
        ...
        if (!Number.isSafeInteger(n)) {
          throw new TomlSafeError("integer overflow (use a quoted string for 64-bit values)",
    ...
    if (raw === "" || raw === "-" || raw === "+") {
      throw new TomlSafeError("invalid number", "toml/bad-number", startLine, startCol);
    }
    if (hasDot || hasExp) {
      var f = parseFloat(raw);
      if (isNaN(f)) {
        throw new TomlSafeError("invalid float", "toml/bad-number", startLine, startCol);
      }
      return f;
    }
