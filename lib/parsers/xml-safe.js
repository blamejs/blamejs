 ...
 *
 * Public API:
 *   xml.parse(input, opts?)            → JS object | throws XmlSafeError
 *   xml.XmlSafeError                   → error class
 *
 * Defaults:
 *   maxBytes:        1 MiB
 *   maxDepth:        100
...
class XmlSafeError extends Error {
  constructor(message, code, position) {
    super(message);
    this.name = "XmlSafeError";
    this.code = code || "xml/invalid";
    this.position = position || null;
    this.isXmlSafeError = true;
  }
}
...
function parse(input, opts) {
  ...
  if (Buffer.isBuffer(input))            input = input.toString("utf8");
    ...
    throw new XmlSafeError("input must be string, Buffer, or Uint8Array", "xml/wrong-input-type");
  }
  ...
  if (Buffer.byteLength(input, "utf8") > maxBytes) {
    throw new XmlSafeError("input exceeds maxBytes", "xml/too-large");
  }
  ...
  function _err(msg, code) {
    return new XmlSafeError(msg + " at position " + pos, code || "xml/invalid", pos);
  }
