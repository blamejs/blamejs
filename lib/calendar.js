...
function expandRecurrence(event, opts) {
  ...
  function _bySet(arr, lo, hi) {
    ...
    for (var i = 0; i < arr.length; i += 1) {
      ...
      if (isFinite(n) && n >= lo && n <= hi) s[n] = true;
    }
    return s;
  }
  ...
  function _isoWeekOf(d) {
    // ISO 8601 week-of-year. Algorithm per the ECMAScript reference
    // (Date.prototype.toLocaleDateString's `iso8601` calendar uses the
    // same numbering). Returns 1..53.
    ...
    return Math.ceil((((tmp - yearStart) / 86400000) + 1) / 7);                                        // allow:raw-time-literal — 86400000 ms/day, 7 days/week // allow:raw-byte-literal — same constants in ms/day form
  }
  ...
  function _matchesBy(t) {
    ...
    if (byWeekNoSet) {
      var wkno = _isoWeekOf(d);
      // RFC 5545 negative-week semantics: -1 is the last ISO week of
      // the year (52 or 53). For ISO weeks, the last week is whatever
      // _isoWeekOf returns for Dec 28 of that year.
      var lastWeek = _isoWeekOf(new Date(Date.UTC(d.getUTCFullYear(), 11, 28)));                       // allow:raw-byte-literal — Dec 28 always in last ISO week
      if (!byWeekNoSet[wkno] && !byWeekNoSet[-(lastWeek - wkno + 1)]) return false;
    }
    if (byYearDaySet) {
      var yd = _yearDayOf(d);
