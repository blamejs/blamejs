...
function toIcal(jsCal, opts) {
  ...
  if (jsCal.start) lines.push("DTSTART" + (jsCal.timeZone ? ";TZID=" + jsCal.timeZone : "") + ":" +
                              _localDateTimeToIcal(jsCal.start));
...
function expandRecurrence(event, opts) {
  ...
  var t = startMs;
  while (out.length < count && out.length < maxCount) {
    if (t > untilMs) break;
...
function _utcDateTimeToIcal(s) {
  // JSCalendar UTCDateTime "2026-05-22T10:00:00Z" → "20260522T100000Z"
  return String(s).replace(/[-:]/g, "");
}

function _localDateTimeToIcal(s) {
  // JSCalendar LocalDateTime "2026-05-22T09:00:00" → "20260522T090000"
  return String(s).replace(/[-:]/g, "");
}
