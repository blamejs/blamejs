 ...
 *   // The suffix is the error's CODE. An error carrying none reports as
 *   // internal-error, whatever its message says.
 *   var err = new Error("record 12: unterminated quote");
 *   err.code = "csv/invalid-record";
 *   b.problemDetails.fromError(err).type;
...
function respond(res, problem, req) {
  ...
  var status = (typeof problem.status === "number" && Number.isInteger(problem.status) &&
                problem.status >= 100 && problem.status <= 599) ? problem.status : C.HTTP.STATUS.INTERNAL_SERVER_ERROR;
