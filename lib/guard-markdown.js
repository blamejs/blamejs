...
function _renderInline(s, opts, depth, rootMatch, off) {
  ...
  while (i < s.length) {
    ...
    if (ch === "!" && s.charAt(i + 1) === "[") {
      var img = _parseLink(s, i + 1, match, base);
      if (img !== null) {
        flush(i);
    ...
    // Link.
    if (ch === "[") {
      var link = _parseLink(s, i, match, base);
      if (link !== null) {
        flush(i);
...
function _matchIn(match, base, len, z) {
  var m = match[z + base];
  if (m === undefined || m === -1) return -1;
  m -= base;
...
function _parseLink(s, at, match, base) {
  if (s.charAt(at) !== "[") return null;
  var off = base === undefined ? 0 : base;
  var textEnd = _matchIn(match, off, s.length, at);
  if (textEnd === -1 || s.charAt(textEnd + 1) !== "(") return null;
  var urlEnd = _matchIn(match, off, s.length, textEnd + 1);
