...
function _renderInline(s, opts, depth) {
  ...
  while (i < s.length) {
    ...
    if (ch === "!" && s.charAt(i + 1) === "[") {
      var img = _parseLink(s, i + 1, match);
      if (img !== null) {
        flush(i);
    ...
    // Link.
    if (ch === "[") {
      var link = _parseLink(s, i, match);
      if (link !== null) {
        flush(i);
        var href = _safeHref(link.url, opts);
        var label = _renderInline(link.text, opts, d + 1);
...
function _bracketMap(s) {
  var match = new Array(s.length);
  for (var z = 0; z < s.length; z += 1) match[z] = -1;
  var square = [];
  var round = [];
  for (var i = 0; i < s.length; i += 1) {
    var c = s.charAt(i);
    if (c === "\\") { i += 1; continue; }
    if (c === "[") square.push(i);
    else if (c === "]") { if (square.length > 0) match[square.pop()] = i; }
    else if (c === "(") round.push(i);
    else if (c === ")") { if (round.length > 0) match[round.pop()] = i; }
  }
  return match;
}
...
function _parseLink(s, at, match) {
  if (s.charAt(at) !== "[") return null;
  var textEnd = match[at];
  if (textEnd === -1 || s.charAt(textEnd + 1) !== "(") return null;
  var urlEnd = match[textEnd + 1];
...
function _renderBlocks(lines, opts, depth) {
  var d = depth === undefined ? 0 : depth;
  var out = "";
  var i = 0;
  while (i < lines.length) {
    ...
    if (_isThematicBreak(line)) { out += "<hr>\n"; i += 1; continue; }
    ...
    if (_isIndentedCode(line)) {
      var codeLines = [];
      while (i < lines.length && (_isIndentedCode(lines[i]) || _isBlank(lines[i]))) {
        // A blank line inside indented code belongs to the block only if more
        // indented code follows; otherwise it ends it.
        if (_isBlank(lines[i])) {
          var j = i + 1;
          while (j < lines.length && _isBlank(lines[j])) j += 1;
          if (j >= lines.length || !_isIndentedCode(lines[j])) break;
          codeLines.push("");
          i += 1;
          continue;
        }
        codeLines.push(lines[i].charAt(0) === "\t"
          ? lines[i].slice(1)
          : lines[i].slice(INDENTED_CODE_COLUMNS));
        i += 1;
      }
    ...
    if (line.charAt(_leadingSpaces(line)) === ">") {
      if (d >= opts.maxBlockquoteDepth) {
        throw _err("markdown/blockquote-depth",
          "b.guardMarkdown.render: blockquote nesting exceeds maxBlockquoteDepth (" +
          opts.maxBlockquoteDepth + ")");
      }
      var quoted = [];
      while (i < lines.length && !_isBlank(lines[i]) &&
             lines[i].charAt(_leadingSpaces(lines[i])) === ">") {
        var q = lines[i].slice(_leadingSpaces(lines[i]) + 1);
        if (q.charAt(0) === " ") q = q.slice(1);
        quoted.push(q);
        i += 1;
      }
      out += "<blockquote>\n" + _renderBlocks(quoted, opts, d + 1) + "</blockquote>\n";
      continue;
    }
    ...
    if (bullet !== -1 || ordered !== -1) {
      ...
      out += "<" + tag + ">\n";
      while (i < lines.length) {
        var at = isOrdered ? _orderedAt(lines[i]) : _bulletAt(lines[i]);
        if (at === -1) break;
        out += "<li>" + _renderInline(lines[i].slice(at).trim(), opts) + "</li>\n";
        i += 1;
      }
      out += "</" + tag + ">\n";
      continue;
