...
function _inlineLinks(input, limit, mates) {
  ...
  var failedStop = -1;
  var closes = new Map();
  var nextIndexTables = new Map();
  function nextOf(ch, from) {
    var table = nextIndexTables.get(ch);
    if (table === undefined) {
      table = _makeNextIndexOf(input, ch);
      nextIndexTables.set(ch, table);
    }
    return from >= table.length ? -1 : table[from];
  }
  var spaceEnds = new Map();
  function skipSpace(from) {
    var known = spaceEnds.get(from);
    if (known !== undefined) return known;
    var v = markupTokenizer.skipMarkupSpace(input, from);
    spaceEnds.set(from, v);
    return v;
  }
...
function _makeNextIndexOf(s, ch) {
  var n = s.length;
  var next = new Int32Array(n + 1);
  next[n] = -1;
  for (var i = n - 1; i >= 0; i -= 1) {
    next[i] = s.charAt(i) === ch ? i : next[i + 1];
  }
  return next;
}

function _linkCloses(s, at, nextOf, skipSpace) {
  var skip = skipSpace === undefined
    ? function (from) { return markupTokenizer.skipMarkupSpace(s, from); }
    : skipSpace;
  var p = skip(at);
  var closer = _titleCloserFor(s.charAt(p));
  if (closer !== "") {
    var q = nextOf === undefined
      ? s.indexOf(closer, p + 1)
      : nextOf(closer, p + 1);
    if (q === -1) return false;
    p = skip(q + 1);
  }
...
function _refDefs(input, mates) {
  ...
  for (var at = 0; at < input.length; at += 1) {
    ...
    while (i < input.length &&
           (input.charAt(i) === ">" || _isSpaceOrTab(input.charAt(i)))) i += 1;
    if (input.charAt(i) !== "[") continue;
    var close = _mateOf(mates, i);
...
function _fenceInfoAt(line) {
  var i = 0;
  var column = 0;
  while (i < line.length &&
         (line.charAt(i) === ">" || _isSpaceOrTab(line.charAt(i)))) {
    if (line.charAt(i) === ">") {
      column = _columnAfter(column, ">");
      i += 1;
      continue;
    }
    var j = i;
    ...
    if (line.charAt(j) === ">") { i = j; column = runColumn; continue; }
    break;
  }
  var indent = 0;
  while (i < line.length && _isSpaceOrTab(line.charAt(i))) {
    var next = _columnAfter(column, line.charAt(i));
    indent += next - column;
    column = next;
    i += 1;
  }
  if (indent > FENCE_MAX_INDENT) return null;
  var fence = line.slice(i, i + 3);
...
function _codeFenceLangs(input) {
  ...
  for (var li = 0; li < lines.length; li += 1) {
    var info = _fenceInfoAt(lines[li]);
...
function _detectIssues(input, opts) {
  ...
  codepointClass.detectCharThreats(input, opts, "markdown")
