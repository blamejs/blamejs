...
function _inlineLinks(input) {
  ...
  for (var i = 0; i < input.length; i += 1) {
    ...
    while (u < input.length && input.charAt(u) !== ")" &&
           !markupTokenizer.isMarkupSpace(input.charCodeAt(u))) u += 1;
    // Resume past whatever was scanned on every failure path below, so a
    // document of `[a](` prefixes does not rescan the same span from each one.
    if (u === urlStart) { i = u - 1; continue; }                     // no URL
    var urlEnd = _linkCloses(input, u) ? u : _backOffToTitle(input, urlStart, u);
    if (urlEnd === -1) { i = u - 1; continue; }
