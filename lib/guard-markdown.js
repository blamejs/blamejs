...
function _sameConstructAs(match) {
  var copy = Object.create(Object.getPrototypeOf(match));
  Object.keys(match).forEach(function (k) { copy[k] = match[k]; });
  copy.counted = false;
  return copy;
}
...
function _inlineMatchClass(input) {
  function InlineMatch(bang, urlStart, urlEnd, index) {
    this.bang = bang;
    this.urlStart = urlStart;
    this.urlEnd = urlEnd;
    this.index = index;
  }
  Object.defineProperty(InlineMatch.prototype, "url", {
    enumerable: true,
    get: function () { return input.slice(this.urlStart, this.urlEnd); },
  });
  return InlineMatch;
}
...
function _inlineLinks(input, limit, mates) {
  var out = [];
  var InlineMatch = _inlineMatchClass(input);
  ...
  for (var i = 0; i < input.length; i += 1) {
    ...
    if (input.charAt(urlStart) === "<") {
      ...
      if (gt !== -1 && closesAt(gt + 1)) {
        out.push(new InlineMatch(bang, urlStart + 1, gt, i));
    ...
    out.push(new InlineMatch(bang, urlStart, urlEnd, i));
    if (limit !== undefined && out.length >= limit) break;
  }
  return out;
}
 ...
 *                                      { profile: "strict" });
 *   bad.ok;                                            // → false
 *   bad.issues[0].ruleId;                              // → "markdown.link-scheme"
 */
