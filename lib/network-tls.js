...
function applyToContext(opts) {
  ...
  var override = base.ecdhCurve !== undefined ? base.ecdhCurve : base.groups;
  var overrideStr = Array.isArray(override) ? override.join(":") : override;
  if (typeof overrideStr === "string" && overrideStr.length > 0) {
    base.ecdhCurve = overrideStr;
...
function explainOutboundFailure(err, ctx) {
  ...
  var minVersion = _hasOwn(ctx, "minVersion") ? ctx.minVersion : posture.minVersion;
  var offered = _hasOwn(ctx, "ecdhCurve") ? ctx.ecdhCurve : posture.ecdhCurve;

  var peer = "";
  if (typeof ctx.host === "string" && ctx.host.length > 0) {
    peer = " by " + ctx.host +
      (typeof ctx.port === "number" && isFinite(ctx.port) ? ":" + ctx.port : "");
  }
  ...
  // Alert 40 (handshake_failure) and the curve-selection failures a narrowed
  // list produces. Carrying a classical group is NOT proof the peer can pick
  // one -- a TLS 1.3 peer restricted to secp256r1 shares nothing with a list
  // of the hybrids plus X25519 -- so the presence of a fallback changes the
  // wording rather than suppressing the explanation.
  var isGroup = code === "ERR_SSL_SSL/TLS_ALERT_HANDSHAKE_FAILURE" ||
                code === "ERR_SSL_NO_SHARED_GROUP" ||
                code === "ERR_SSL_WRONG_CURVE" ||
                /alert handshake failure|no shared group|wrong curve/i.test(text);
  var list = typeof offered === "string" && offered.length > 0 ? offered.split(":") : [];
  if (!isGroup || list.length === 0) return null;

  var hasClassicalFallback = false;
  for (var i = 0; i < list.length; i += 1) {
    if (list[i].length > 0 && list[i].indexOf("MLKEM") === -1) hasClassicalFallback = true;
  }
  var why = EXPLAIN_PREFIX + peer + " - the peer most likely supports none of " +
    "the key-exchange groups this client offers (" + offered + ").";
  why += hasClassicalFallback
    // The list already ends in a classical group, so the peer is restricted to
    // one outside the list rather than merely lacking a hybrid.
    ? " The list includes a classical group, so this points at a peer " +
      "restricted to a group the list does not name rather than at one " +
      "without post-quantum support."
    : " The list names only post-quantum hybrids, so a peer without one has " +
      "nothing left to negotiate.";
  return why + " Underlying error: " + text;
}
 ...
 *   port:       number,   // appended to host when both are present
 *   minVersion: string,   // default: the live posture
 *   ecdhCurve:  string,   // default: the live posture
 *
 * @example
...
function annotateOutboundFailure(err, ctx) {
  try {
    ...
    // error that is retried on h1, say). Rewriting twice would nest one
    // explanation inside the "Underlying error:" of the next.
    if (err && typeof err.message === "string" &&
        err.message.indexOf(EXPLAIN_PREFIX) === 0) {
      return err;
    }
