...
function explainOutboundFailure(err, ctx) {
  ...
  var minVersion = ctx.minVersion === undefined ? posture.minVersion : ctx.minVersion;
  var offered = ctx.ecdhCurve === undefined ? posture.ecdhCurve : ctx.ecdhCurve;
  ...
  // list produces. Only worth saying when the offered list has no classical
  // fallback, since with one the peer always has something to pick.
  ...
  var list = typeof offered === "string" ? offered.split(":") : [];
  var hasClassicalFallback = false;
  for (var i = 0; i < list.length; i += 1) {
    if (list[i].indexOf("MLKEM") === -1 && list[i].length > 0) hasClassicalFallback = true;
  }
  if (isGroup && list.length > 0 && !hasClassicalFallback) {
    return EXPLAIN_PREFIX + peer + " - the peer most likely supports " +
      "none of the key-exchange groups this client offers (" + offered + "), " +
      "which name only post-quantum hybrids. A peer without a hybrid has " +
      "nothing left to negotiate once the classical group is dropped. " +
