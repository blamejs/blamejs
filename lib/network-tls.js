...
function evaluateOcspResponse(ocspDer, opts) {
  ...
  if (!match.issuerKeyHash) {
    return { ok: false, status: parsed.status, signatureValid: true,
             errors: ["OCSP CertID is missing issuerKeyHash — cannot bind the response to the issuer (RFC 6960 §4.1.1)"] };
  }
  var expected;
  try {
    expected = opts.issuerCertDer
      ? _expectedOcspCertIdHashes(opts.issuerCertDer, match.certIdHashAlgOid)
      : { nameHash: null, keyHash: _expectedOcspKeyHashFromPem(issuerPem, match.certIdHashAlgOid) };
  } catch (e) {
    return { ok: false, status: parsed.status, signatureValid: true,
             errors: [(e && e.message) || String(e)] };
  }
  if (expected.nameHash) {
    if (!match.issuerNameHash ||
        expected.nameHash.length !== match.issuerNameHash.length ||
...
function _expectedOcspKeyHashFromPem(issuerPem, certIdHashAlgOid) {
  var nodeHash = OCSP_CERTID_HASH_OID_TO_NODE[certIdHashAlgOid];
  if (!nodeHash) {
    throw new TlsTrustError("tls/ocsp-bad-certid-hash-alg",
      "OCSP CertID hashAlgorithm OID '" + certIdHashAlgOid +
  ...
  var spkiDer;
  try { spkiDer = nodeCrypto.createPublicKey(issuerPem).export({ type: "spki", format: "der" }); }
  catch (e) {
    throw new TlsTrustError("tls/ocsp-bad-issuer-key",
      "could not derive the issuer public key from opts.issuerPem for the RFC 6960 §4.1.1 CertID binding (supply opts.issuerCertDer for a delegated responder): " +
      ((e && e.message) || String(e)));
  }
  var spkiKids = asn1.readSequence(asn1.readNode(spkiDer).value);
  var keyBytes = asn1.readBitString(spkiKids[1]);
  return nodeCrypto.createHash(nodeHash).update(keyBytes).digest();
}
