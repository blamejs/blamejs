...
var ocsp = Object.freeze({
  ...
  requireGood: async function (opts) {
    ...
    // opts.issuerPem is the OCSP-signing cert — usually the issuer itself, but
    // a delegated responder's key in some deployments. Forward its DER for the
    // RFC 6960 §4.1.1 CertID issuer binding when it parses as a cert; a bare key
    // (delegated-responder-only) leaves the bind on serial + signature.
    var rgIssuerCertDer = null;
    try { rgIssuerCertDer = new nodeCrypto.X509Certificate(opts.issuerPem).raw; }
    catch (_e) { rgIssuerCertDer = null; }
