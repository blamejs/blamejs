...
var OCSP_CERTID_HASH_OID_TO_NODE = Object.freeze(Object.assign(Object.create(null), {
  "1.3.14.3.2.26":          "sha1",
  "2.16.840.1.101.3.4.2.1": "sha256",
  "2.16.840.1.101.3.4.2.2": "sha384",
  "2.16.840.1.101.3.4.2.3": "sha512",
}));
var OCSP_CERTID_NODE_TO_HASH_OID = Object.freeze(Object.assign(Object.create(null), {
  sha256: OID_SHA256,
  sha384: "2.16.840.1.101.3.4.2.2",
  sha512: "2.16.840.1.101.3.4.2.3",
  sha1:   OID_SHA1,
}));
var OCSP_CERTID_HASH_ALGS = Object.freeze(Object.keys(OCSP_CERTID_NODE_TO_HASH_OID));
...
function parseOcspResponse(der) {
  ...
  var top;
  try { top = asn1.readNodeStrict(der); }
  catch (e) {
    if (e && e.code === "asn1/trailing-data") {
      throw new TlsTrustError("tls/ocsp-trailing-data",
        "parseOcspResponse: trailing bytes after the top-level OCSPResponse DER structure");
    }
    throw e;
  }
...
function evaluateOcspResponse(ocspDer, opts) {
  ...
  if (opts.issuerCertDer && !Buffer.isBuffer(opts.issuerCertDer)) {
    return { ok: false, status: parsed.status, signatureValid: true,
             errors: ["evaluateOcspResponse: opts.issuerCertDer must be a Buffer (issuer cert DER)"] };
  }
  var issuerDerForBinding = opts.issuerCertDer || null;
  if (!issuerDerForBinding) {
    try { issuerDerForBinding = new nodeCrypto.X509Certificate(issuerPem).raw; }
    catch (_e) { issuerDerForBinding = null; }
  }
  if (!issuerDerForBinding) {
    return { ok: false, status: parsed.status, signatureValid: true,
             errors: ["OCSP evaluation cannot bind the response to its issuer (RFC 6960 §4.1.1): pass opts.issuerCertDer, or an opts.issuerPem that is the issuer certificate. A bare public key cannot supply the issuer name, which is required to distinguish two issuers that reuse one key"] };
  }
  var wantSerial = _normOcspSerial(opts.serialHex);
  var match = null, missingHashSeen = false;
  var issuerMismatchSeen = false, hashAlgErr = null;
  for (var i = 0; i < parsed.basic.responses.length; i += 1) {
    var r = parsed.basic.responses[i];
    if (_normOcspSerial(r.certIdSerialHex) !== wantSerial) continue;
    if (!r.issuerNameHash || !r.issuerKeyHash) { missingHashSeen = true; continue; }
    var exp;
    try { exp = _expectedOcspCertIdHashes(issuerDerForBinding, r.certIdHashAlgOid); }
    catch (e) { hashAlgErr = (e && e.message) || String(e); continue; }
    var nameOk = exp.nameHash.length === r.issuerNameHash.length &&
                 bCrypto.timingSafeEqual(exp.nameHash, r.issuerNameHash);
    var keyOk  = exp.keyHash.length === r.issuerKeyHash.length &&
                 bCrypto.timingSafeEqual(exp.keyHash, r.issuerKeyHash);
    if (nameOk && keyOk) { match = r; break; }
    issuerMismatchSeen = true;
  }
  if (!match) {
    if (hashAlgErr) {
      return { ok: false, status: parsed.status, signatureValid: true, errors: [hashAlgErr] };
    }
    if (issuerMismatchSeen) {
      return { ok: false, status: parsed.status, signatureValid: true,
               errors: ["OCSP CertID issuerNameHash/issuerKeyHash does not match the issuer of the cert under validation (RFC 6960 §4.1.1 — wrong-issuer response)"] };
    }
    if (missingHashSeen) {
      return { ok: false, status: parsed.status, signatureValid: true,
               errors: ["OCSP CertID is missing issuerNameHash/issuerKeyHash — cannot bind the response to the issuer (RFC 6960 §4.1.1)"] };
    }
    return { ok: false, status: parsed.status, signatureValid: true,
  ...
  if (!isFinite(nextUpdateMs)) {
    var maxAgeMs = numericBounds.isNonNegativeFiniteInt(opts.maxAgeMs)
      ? opts.maxAgeMs : C.TIME.hours(24);
    if (now - thisUpdateMs > maxAgeMs + clockSkewMs) {
      return { ok: false, status: parsed.status, signatureValid: true,
               certStatus: match.certStatus,
               thisUpdate: match.thisUpdate, nextUpdate: match.nextUpdate,
               nonce: nonceCheck,
               errors: ["OCSP response omits nextUpdate and thisUpdate is older than the maximum accepted age (RFC 6960 §4.2.2.1 — without a stated nextUpdate a signed 'good' has no expiry and cannot defeat revocation; widen with opts.maxAgeMs)"] };
    }
  }
  return {
...
function buildOcspRequest(opts) {
  ...
  var certIdHashAlg = (opts.certIdHashAlg === undefined || opts.certIdHashAlg === null)
    ? "sha256" : opts.certIdHashAlg;
  var certIdHashOid = (typeof certIdHashAlg === "string") ? OCSP_CERTID_NODE_TO_HASH_OID[certIdHashAlg] : undefined;
  if (typeof certIdHashOid !== "string") {
    throw new TlsTrustError("tls/ocsp-bad-certid-hash-alg",
      "buildRequest: opts.certIdHashAlg " + JSON.stringify(certIdHashAlg) + " is not a recognized CertID hash " +
      "(" + OCSP_CERTID_HASH_ALGS.join(" | ") + "). SHA-256 is the default; sha1 is offered only for a " +
      "responder that indexes its database by SHA-1 CertID (RFC 6960 §4.1.1 — a lookup key, not " +
      "an integrity check: the response is bound by the responder's signature, not this hash)");
  }
  var iss = _extractIssuerNameDerAndKeyBitString(opts.issuerCertDer);
  var serial = _extractLeafSerial(opts.leafCertDer);
  var nameHash = nodeCrypto.createHash(certIdHashAlg).update(iss.issuerNameDer).digest();
  var keyHash  = nodeCrypto.createHash(certIdHashAlg).update(iss.issuerKey).digest();
  setImmediate(function () {
    try {
      ...
      auditMod.safeEmit({
        ...
        outcome:  "success",
        metadata: { hashAlgorithm: certIdHashAlg, note: "RFC 6960 §4.1.1 — CertID lookup hash (SHA-256 default)" },
      });
    } catch (_e) { /* drop-silent */ }
  });
  var algId = asn1.writeSequence([asn1.writeOid(certIdHashOid), asn1.writeNull()]);
  ...
  return { requestDer: requestDer, nonce: nonceBytes, certIdHashAlg: certIdHashAlg };
}

function _ocspCertIdHashHint(certIdHashAlg) {
  if (certIdHashAlg === "sha1") return "";
  return " (the request identified the certificate by a " + certIdHashAlg + " CertID; a responder " +
    "that indexes revocation only by SHA-1 CertIDs cannot answer it, and certIdHashAlg: \"sha1\" " +
    "sends a SHA-1 CertID to such a responder)";
}
...
async function fetchOcspResponse(opts) {
  ...
  var built = buildOcspRequest({
    ...
    certIdHashAlg: opts.certIdHashAlg,
  });
  var res;
  try {
  ...
  if (res.status !== C.HTTP.STATUS.OK || !Buffer.isBuffer(res.body) || res.body.length === 0) {
    throw new TlsTrustError("tls/ocsp-fetch-bad-status",
      ...
      _ocspCertIdHashHint(built.certIdHashAlg));
  }
...
var ocsp = Object.freeze({
  ...
  CERTID_HASH_ALGS:     OCSP_CERTID_HASH_ALGS,
