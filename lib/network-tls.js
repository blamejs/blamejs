...
function explainOutboundFailure(err, ctx) {
  ...
  if (code === "ERR_SSL_SSL/TLS_ALERT_HANDSHAKE_FAILURE" ||
    ...
    // capped the connection lower — through a WebSocket tlsOpts override or
    // their own agent — never attempted 1.3, so pointing at its cipher list
    // sends them after a setting that had no part in the failure.
    var reached13 = !_hasOwn(ctx, "maxVersion") || ctx.maxVersion === undefined ||
                    ctx.maxVersion === null || ctx.maxVersion === "TLSv1.3";
