...
function _readPathFile(filepath) {
  // Open-then-read off the fd so a swap between readdir and open
  // can't redirect us to a different inode.
  ...
  try {
    ...
    try { nodeFs.closeSync(fd); } catch (_c) { /* best-effort */ }
  }
}
...
function _readPath(p) {
  ...
  if (stat.isDirectory()) {
    ...
    for (var i = 0; i < files.length; i++) {
      try {
        parts.push(_readPathFile(nodePath.join(p, files[i])));
      } catch (e) {
        // Tolerate ENOENT — attacker (or normal churn) removed the file
        // between readdir and open; the remaining bundle is still valid.
