...
//
// b.network.tls.ocsp wraps these to give operators a one-call shape
// for "refuse if no OCSP good response" outbound and "must-staple"
// inbound. Full RFC 6960 OCSP request building (ASN.1 OCSPRequest +
// HTTP transport + DER OCSPResponse parsing) is OUT of scope for this
// patch — node:tls already does the protocol. The framework adds the
// outcome-policy layer.

function _connectAndCheckOcsp(opts, requireGood) {
  return new Promise(function (resolve, reject) {
      ...
      if (!response || response.length === 0) {
        if (requireGood) {
          ...
          reject(new TlsTrustError("tls/ocsp-empty",
            "OCSP response was empty and requireGood is set"));
          return;
        }
      }
    ...
    sock.on("secureConnect", function () {
      ...
      // OCSPResponse event AND requireGood is set, refuse.
      if (!ocspResponseSeen) {
        if (requireGood) {
          sock.destroy();
          reject(new TlsTrustError("tls/ocsp-not-stapled",
            "TLS peer did not staple an OCSP response and requireGood is set"));
          return;
        }
        var rv = {
          authorized: sock.authorized,
...
var ocsp = Object.freeze({
  ...
  // peerCert }. requireGood: true makes empty / not-stapled responses
  // refuse instead of resolve.
  connect: function (opts) {
    return _connectAndCheckOcsp(opts || {}, false);
  },
  requireGood: function (opts) {
