...
function _detectMagicMimes(buf) {
  ...
  // A leading UTF-8 BOM (EF BB BF) precedes only the text-family images
  // (SVG / XML); skip it so a BOM-prefixed SVG still matches its `<?xml` /
  // `<svg` signature and routes to guardSvg at every profile. A binary raster
  // never carries a BOM, so shifting every signature by the BOM width cannot
  ...
  for (var i = 0; i < MAGIC_BYTES.length; i += 1) {
    ...
    var offset = (entry.offset || 0) + bom;
    if (!_bytesAt(buf, offset, entry.bytes)) continue;
    if (entry.tail && !_bytesAt(buf, entry.tailOffset + bom, entry.tail)) continue;
