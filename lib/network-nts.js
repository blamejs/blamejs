...
function querySingle(opts) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    // The family follows the address. A udp4 socket cannot reach an IPv6
    // destination at all, so an IPv6 NTS server was unusable — and worse than
    // unusable under requireNts, where a reachable server that could not be
    // addressed refused the boot. A hostname stays on udp4: resolving it here
    // to pick a family would bypass the framework's own resolver policy, and
    // that is a larger change than the literal case needs.
    var sock = dgram.createSocket(net.isIP(opts.host) === 6 ? "udp6" : "udp4");
    var settled = false;
    function done(err, result) {
      if (settled) return;
      settled = true;
