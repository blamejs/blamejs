...
function canonicalizeHost(host) {
  ...
  if (family === 6) {
    ...
    // canonicalize equal. Both /96 prefixes whose low 32 bits ARE a 1:1 IPv4
    // target fold: ::ffff:0:0/96 (IPv4-mapped) and 64:ff9b::/96 (NAT64
    // well-known prefix — the embedded v4 is the host being reached). classify()
    // re-classifies both by the embedded v4, so the canonical form agrees. 6to4
    // (2002::/16) is NOT folded — it is a /48 PREFIX, not a 1:1 alias, so
    // collapsing it onto one IPv4 would be wrong; an embedded-v4 suffix in any
    // other prefix is likewise a distinct address and stays IPv6.
    if (_ipv6PrefixMatch(IPV6_V4_MAPPED_PREFIX, C.BYTES.bytes(96), v6bytes) ||
        _ipv6PrefixMatch(IPV6_NAT64_PREFIX, C.BYTES.bytes(96), v6bytes)) {
