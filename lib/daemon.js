 ...
 *   bootDeathWindowMs: number,  // detached: an abnormal child exit within this window of spawn is audited as a boot failure; default 5000
 *
 * @example
...
function start(opts) {
  ...
  if (typeof opts.command === "string" && opts.command.length > 0) {
    ...
    child.on("exit", function (code, signal) {
      ...
      if (wasOurs && abnormal && withinBoot) {
        _safeAuditEmit("daemon.spawn_failed", "failure", {
          pidFile:  pidFile,
          command:  opts.command,
          exitCode: code,
