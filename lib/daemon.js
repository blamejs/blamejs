...
// Instead ATOMICALLY claim the sidecar (rename it to a per-pid path) FIRST, so
// the ownership check and the removal act on the same exclusive file. If the
// claimed file is ours, delete it; if a restart had already rewritten it (a
// different pid), restore it so the new daemon keeps its pidfile. `readPid`
// defaults to the hardened reader and is injectable for tests. Returns whether
// the reaped sidecar was this child's.
function _reapOwnStalePidfile(pidFile, childPid, readPid) {
  ...
  try {
    ...
    return false;   // pidFile already gone (stop() reaped it, or a restart's rename) — nothing of ours to reap
  }
  ...
  if (mine) {
    ...
    // so the restore can never clobber the newest pidfile with our older claimed
    // one (a plain rename would). Either way, drop our claim afterward.
    try { nodeFs.linkSync(claim, pidFile); } catch (_e4) { /* pidFile already taken by a newer daemon (EEXIST) or link unsupported — leave the newer file */ }
    try { nodeFs.unlinkSync(claim); } catch (_e5) { /* best-effort — claim already gone */ }
  }
  return mine;
}
