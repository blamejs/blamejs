...
// pidFiles with an in-flight SAME-PROCESS stop(). The detached boot-death exit
// handler consults this so a stop()-induced exit within the boot window is not
// misread as a spawn failure. A cross-process stop instead relinquishes the
// pidfile, which the handler's wasOurs check already honors.
var _STOPPING = Object.create(null);
    ...
    bootDeathWindowMs: function (value) {
      if (value !== undefined && (typeof value !== "number" || !isFinite(value) || value < 0)) {
        throw new DaemonError("daemon/bad-boot-window",
          "daemon.start: opts.bootDeathWindowMs must be a non-negative finite " +
          "number of milliseconds when present");
      }
    },
...
function start(opts) {
  ...
  if (typeof opts.command === "string" && opts.command.length > 0) {
    ...
    child.on("exit", function (code, signal) {
      ...
      // Was the sidecar still OURS at exit? Decided by the pidFile READ (not the
      // unlink result), so the boot-death signal survives a swallowed unlink
      // failure. A stop() or a fast operator restart that rewrote/cleared it
      // means someone else owns it now — not a boot death.
      var wasOurs = false;
      try {
        if (String(_readPidFile(pidFile)) === String(child.pid)) {
          wasOurs = true;
          nodeFs.unlinkSync(pidFile);   // reap our own stale sidecar
        }
      } catch (_e) { /* best-effort — file may already be reaped (e.g. by stop), or the unlink raced */ }
      ...
      // A same-process stop() sends SIGTERM but unlinks the pidfile only after
      // observing the exit, so wasOurs is still true here; _STOPPING marks that
      // intent so an operator stop within the boot window is not misread as a
      // boot death (which would emit spawn_failed right before daemon.stopped).
      if (wasOurs && abnormal && withinBoot && !_STOPPING[pidFile]) {
        _safeAuditEmit("daemon.spawn_failed", "failure", {
          pidFile:  pidFile,
          command:  opts.command,
          exitCode: code,
...
async function stop(opts) {
  ...
  // Mark this pidFile as an in-flight stop for the boot-death handler, then drive
  // the signal-and-wait. The finally clears the mark on every exit (return OR a
  // kill-failed throw) so a later start() at the same path is never suppressed.
  _STOPPING[pidFile] = true;
  try {
    return await _stopLivePid(pidFile, pid, signal, timeoutMs, pollMs, opts);
  } finally {
    delete _STOPPING[pidFile];
  }
}

// Signal a confirmed-live pid and wait for exit, escalating SIGTERM -> SIGKILL.
// Extracted from stop() so the _STOPPING mark wraps every exit via try/finally.
