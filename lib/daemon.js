...
// Install the Windows cooperative-stop watcher for a foreground daemon. Watches
// the pidFile's DIRECTORY (fs.watch on a not-yet-existent file would throw) and
// fires the orchestrator's graceful shutdown the first time the sibling
// <pidFile>.stop sentinel appears — the same orchestrator.shutdown() the POSIX
// signal path drives, so the exit code is set from the phase result and the
// event loop drains once the phases release the daemon's resources. The watcher
// is unref'd so it never itself keeps the process alive; a clean daemon exits
// on its own once shutdown completes. If the sentinel is never written the
// watcher is a no-op that the release phase closes at shutdown. Returns a handle
// exposing close(); returns null on a platform where fs.watch is unavailable
// (the operator can still hard-stop via daemon.stop()'s TerminateProcess).
function _installStopSentinelWatcher(pidFile, orchestrator) {
  ...
  function _maybeFire() {
    if (fired) return;
    // fs.watch fires on ANY change in the directory; only act once the sentinel
    // itself exists, so unrelated writes in the dir don't trigger shutdown.
  ...
  var watcher = null;
  try {
    watcher = nodeFs.watch(dir, function (_event, filename) {
      if (!filename || String(filename) === sentinelName) _maybeFire();
    });
    if (watcher && typeof watcher.unref === "function") watcher.unref();
  } catch (_e) {
    // fs.watch unsupported on this platform/filesystem — no cooperative channel;
    // daemon.stop() still hard-stops via TerminateProcess after its timeout.
    watcher = null;
  }
  // The sentinel may already exist (stop() raced ahead of this install).
  _maybeFire();
  return {
    close: function () { if (watcher) { try { watcher.close(); } catch (_e) { /* best-effort */ } } },
