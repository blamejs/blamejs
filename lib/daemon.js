...
function start(opts) {
  ...
  if (typeof opts.command === "string" && opts.command.length > 0) {
    ...
    child.on("exit", function (code, signal) {
      ...
      try { beingStopped = String(nodeFs.readFileSync(_stoppingMarkerPath(pidFile), "utf8")).trim() === String(child.pid); }
      catch (_m) { /* no marker (the common case) or unreadable — not being stopped */ }
