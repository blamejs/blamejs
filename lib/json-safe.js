 ...
 * Public API:
 *
 *   json.parse(input, opts?)               → value | throws JsonSafeError
 *                                            (with opts.collectErrors: { ok, value, errors[] })
 *   json.parseOrDefault(input, fallback, opts?) → value (no throw)
 *   json.stringify(value, opts?)           → string | throws JsonSafeError
 ...
 *   json.JsonSafeError                     → error class
 *
...
class JsonSafeError extends Error {
  constructor(message, code, path) {
    ...
    this.path = path || null;
    this.isJsonSafeError = true;
  }
}

var ABSOLUTE_MAX_BYTES = 64 * 1024 * 1024;
...
function parse(input, opts) {
  ...
  if (Buffer.isBuffer(input)) {
    ...
    throw new JsonSafeError("input must be a string, Buffer, or Uint8Array", "json/wrong-input-type");
  }

  if (input.charCodeAt(0) === 0xFEFF) input = input.slice(1);

  var maxBytes = _capInt(opts.maxBytes, DEFAULT_MAX_BYTES, ABSOLUTE_MAX_BYTES);
  var byteLength = Buffer.byteLength(input, "utf8");
  if (byteLength > maxBytes) {
    throw new JsonSafeError("input exceeds maxBytes (" + byteLength + " > " + maxBytes + ")", "json/too-large");
  }
  ...
  try {
    ...
    throw new JsonSafeError("invalid JSON: " + e.message, "json/syntax");
  }
  ...
  if (opts.expectType) {
    ...
    if (actual !== opts.expectType) {
      throw new JsonSafeError("expected " + opts.expectType + " at root, got " + actual, "json/type-mismatch");
    }
  }
  if (Array.isArray(opts.requiredKeys) && parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
    for (var i = 0; i < opts.requiredKeys.length; i++) {
      if (!Object.prototype.hasOwnProperty.call(parsed, opts.requiredKeys[i])) {
        throw new JsonSafeError("missing required key '" + opts.requiredKeys[i] + "'", "json/missing-key");
      }
    }
  }
...
function _walkAndCheck(value, depth, maxDepth, allowProto) {
  if (depth > maxDepth) {
    throw new JsonSafeError("nesting exceeds maxDepth (" + maxDepth + ")", "json/too-deep");
  }
...
function stringify(value, opts) {
  ...
  try {
    ...
    if (e && e.isJsonSafeError) throw e;
    ...
    if (e instanceof TypeError && /circular/i.test(e.message)) {
      throw new JsonSafeError("circular reference: " + e.message, "json/circular");
    }
    throw new JsonSafeError("stringify failed: " + e.message, "json/stringify");
  }
}
  ...
  function ser(v) {
    ...
    if (typeof v === "number") {
      if (!Number.isFinite(v)) {
        throw new JsonSafeError("non-finite number cannot be canonicalized", "json/non-finite");
      }
    ...
    throw new JsonSafeError("cannot canonicalize value of type " + typeof v, "json/uncanonical");
  }
...
function registerFormat(name, validator) {
  if (typeof name !== "string" || !/^[a-z][a-z0-9-]*$/.test(name)) {
    throw new JsonSafeError("format name must match [a-z][a-z0-9-]*: " + name, "json/bad-format-name");
  }
  if (typeof validator !== "function") {
    throw new JsonSafeError("format validator must be a function", "json/bad-format-validator");
  }
...
function validate(value, schema, opts) {
  ...
  if (!schema || typeof schema !== "object") {
    throw new JsonSafeError("validate: schema must be an object", "json/bad-schema");
  }
...
function _validateNode(value, schema, path, report) {
  ...
  if (schema.type) {
    if (schema.type === "integer") {
      if (typeof value !== "number" || !Number.isInteger(value)) {
        report(new JsonSafeError(path + ": expected integer, got " + _typeName(value), "json/validation", path));
        return;
      }
    } else if (_typeName(value) !== schema.type) {
      report(new JsonSafeError(path + ": expected " + schema.type + ", got " + _typeName(value), "json/validation", path));
      return;
    }
  }
  ...
  if (typeof value === "string") {
    if (schema.minLength != null && value.length < schema.minLength) {
      report(new JsonSafeError(path + ": string length " + value.length + " < minLength " + schema.minLength, "json/validation", path));
    }
    if (schema.maxLength != null && value.length > schema.maxLength) {
      report(new JsonSafeError(path + ": string length " + value.length + " > maxLength " + schema.maxLength, "json/validation", path));
    }
    if (schema.pattern) {
      var re = schema.pattern instanceof RegExp ? schema.pattern : new RegExp(schema.pattern);
      if (!re.test(value)) {
        report(new JsonSafeError(path + ": does not match pattern", "json/validation", path));
      }
    }
    if (schema.format) {
      var f = formats[schema.format];
      if (!f) {
        report(new JsonSafeError(path + ": unknown format '" + schema.format + "'", "json/unknown-format", path));
      } else if (!f(value)) {
        report(new JsonSafeError(path + ": does not match format '" + schema.format + "'", "json/validation", path));
      }
    }
  }
  ...
  if (typeof value === "number") {
    if (schema.minimum != null && value < schema.minimum) {
      report(new JsonSafeError(path + ": " + value + " < minimum " + schema.minimum, "json/validation", path));
    }
    if (schema.exclusiveMinimum != null && value <= schema.exclusiveMinimum) {
      report(new JsonSafeError(path + ": " + value + " <= exclusiveMinimum " + schema.exclusiveMinimum, "json/validation", path));
    }
    if (schema.maximum != null && value > schema.maximum) {
      report(new JsonSafeError(path + ": " + value + " > maximum " + schema.maximum, "json/validation", path));
    }
    if (schema.exclusiveMaximum != null && value >= schema.exclusiveMaximum) {
      report(new JsonSafeError(path + ": " + value + " >= exclusiveMaximum " + schema.exclusiveMaximum, "json/validation", path));
    }
  }
  ...
  if (Array.isArray(value)) {
    if (schema.minItems != null && value.length < schema.minItems) {
      report(new JsonSafeError(path + ": array length " + value.length + " < minItems " + schema.minItems, "json/validation", path));
    }
    if (schema.maxItems != null && value.length > schema.maxItems) {
      report(new JsonSafeError(path + ": array length " + value.length + " > maxItems " + schema.maxItems, "json/validation", path));
    }
    if (schema.items) {
    ...
    if (Array.isArray(schema.required)) {
      for (var rk = 0; rk < schema.required.length; rk++) {
        if (!Object.prototype.hasOwnProperty.call(value, schema.required[rk])) {
          report(new JsonSafeError(path + ": missing required key '" + schema.required[rk] + "'", "json/validation", path));
        }
      }
    }
    ...
    if (schema.properties) {
      for (var k in value) {
        ...
        if (Object.prototype.hasOwnProperty.call(schema.properties, k)) {
          ...
          report(new JsonSafeError(path + ": unknown key '" + k + "'", "json/validation", path + "." + k));
        }
      }
    }
  }
