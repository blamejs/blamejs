...
class RestoreRollbackError extends FrameworkError {
  constructor(code, message) {
    super(message, code);
    this.name = "RestoreRollbackError";
    this.permanent = true;
    this.isRestoreRollbackError = true;
  }
}

function _filesystemSafeNow() {
...
function _ensureDir(p) { fs.mkdirSync(p, { recursive: true, mode: 0o700 }); }
...
function swap(opts) {
  ...
  _ensureDir(rollbackRoot);

  var swappedAt = _filesystemSafeNow();
...
async function rollback(opts) {
  ...
  if (fs.existsSync(opts.dataDir)) {
    var rollbackRoot = _resolveRollbackRoot(opts);
    _ensureDir(rollbackRoot);
    discardedAt = _filesystemSafeNow();
