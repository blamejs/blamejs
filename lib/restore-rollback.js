...
function swap(opts) {
  ...
  try {
    // Exclusive-create — if an attacker pre-created markerPath the open
    // fails with EEXIST and we skip the marker write. The marker is
    // best-effort (rollback dir mtime is the authoritative timestamp);
    // refusing to overwrite an unexpected pre-existing file is safer.
    var markerFd = nodeFs.openSync(markerPath, "wx", 0o600);
    try {
      nodeFs.writeSync(markerFd, JSON.stringify(marker, null, 2) + "\n");
    } finally {
      try { nodeFs.closeSync(markerFd); } catch (_c) { /* best-effort */ }
    }
  } catch (_e) { /* marker write is best-effort (incl. EEXIST) */ }
