...
async function main(argv, opts) {
  ...
  // `-v` / `--version` is a boolean version flag, decided from the PARSED
  // args (not by rewriting raw tokens). The schemaless parser treats
  // `--version <token>` as a value flag and swallows the next token as its
  // value; when it does, that token is really the subcommand, so restore it
  // as a positional and normalize the flag to boolean. A `-v` / `--version`
  // used as ANOTHER option's value (e.g. `dev --arg -v` → run `node -v`)
  // stays that option's value and never becomes `flags.version`, so it is
  // left intact — as is a version token after the `--` terminator (a literal
  // positional, e.g. a file named `--version`).
  if (typeof args.flags.version === "string") {
    args.pos.unshift(args.flags.version);
    args.flags.version = true;
  }

  // Version is honored only as the WHOLE invocation — a bare `-v` /
  // `--version` with no subcommand. A stray version flag alongside a command
  // (in either order) leaves the positional list non-empty, so the command
  // dispatches instead of a silent version no-op that returns a false 0.
