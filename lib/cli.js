  ...
  if (sub === "verify-chain") {
    ...
    try {
      ...
      if (typeof pubKeyPathV === "string" && pubKeyPathV.length > 0) {
        ...
        var fpV;
        try {
          // Fingerprint the key material, not the file's bytes. A fingerprint
          // is a hash of the PEM text, so the same key saved with CRLF line
          // endings — or any other equivalent serialization — hashes
          // differently, matches no anchor, and gets a healthy chain reported
          // as signed under an unknown key. Re-exporting through the parsed
          // key gives one spelling for one key, which is what the anchor's
          // fingerprint was computed from.
          pemV = nodeCrypto.createPublicKey(pemV)
            .export({ type: "spki", format: "pem" }).toString();
          fpV = auditSign.fingerprintOf(pemV);
        } catch (e) {
