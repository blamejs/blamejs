...
async function _runAudit(args, ctx) {
  ...
  if (sub === "verify-chain") {
    ...
    try {
      ...
      var archiveDirV = args.flags["archive-dir"];
      ...
      var pubKeyPathV = args.flags["public-key"];
      if (typeof pubKeyPathV === "string" && pubKeyPathV.length > 0) {
        ...
        try {
          // A fingerprint is a hash of the PEM TEXT, so one key can answer to
          // two of them and this has to hold both.
          //
          // The file's own bytes are one: a key an operator supplied to
          // `rotateSigningKey` before that call canonicalized what it
          // ingested was fingerprinted exactly as written, CRLF and all, and
          // the anchors signed under it carry that hash. Hashing only a
          // re-export would report those healthy volumes as signed by an
          // unknown key.
          //
          // The re-export is the other: the same key saved with different
          // line endings is the same key, and hashing only the file's bytes
          // would refuse it for how it was saved.
          //
          // Both are derived from a PEM that parsed into the key we hold, so
          // carrying the pair resolves either spelling without widening what
          // counts as a valid key. The matching spelling is what gets handed
          // back, because that is the text the anchor's fingerprint came from.
          var canonicalPem = nodeCrypto.createPublicKey(pemV)
            .export({ type: "spki", format: "pem" }).toString();
          byFingerprint[auditSign.fingerprintOf(pemV)] = pemV;
          byFingerprint[auditSign.fingerprintOf(canonicalPem)] = canonicalPem;
        } catch (e) {
          _writeLine(ctx.stderr, "blamejs audit verify-chain: --public-key at " +
            pubKeyPathV + " is not a PEM public key: " + ((e && e.message) || String(e)));
          return 2;
        }
        vcOpts.resolvePublicKey = function (wantFp) {
          return Object.prototype.hasOwnProperty.call(byFingerprint, String(wantFp))
            ? byFingerprint[String(wantFp)] : null;
        };
      }
