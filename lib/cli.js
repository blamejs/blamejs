  ...
  if (sub === "verify-chain") {
    ...
    try {
      ...
      if (typeof archiveDirV === "string" && archiveDirV.length > 0) {
        vcOpts.resolveArchive = function (bundleId) {
          var entries;
          ...
          for (var ei = 0; ei < entries.length; ei += 1) {
            ...
            var parsed;
            // safeJson.parse, not JSON.parse: these files come from wherever
            // the operator points the flag, and the prototype / depth / size
            // defenses are the reason the primitive exists.
            try { parsed = safeJson.parse(nodeFs.readFileSync(manifestPath, "utf8")); }
        ...
        if (vc.purgeAnchor) {
          ...
          anchorNote = ", purgeAnchor=" +
            ...
            (vc.purgeAnchor.archiveResolved
              ? ", archive " + vc.purgeAnchor.archiveBundleId + " found"
