...
async function main(argv, opts) {
  ...
  // Top-level version — honored only when it is the WHOLE invocation. A
  // stray `-v` / `--version` alongside a subcommand must not short-circuit
  // it: otherwise `blamejs migrate up --db x -v` would print the version and
  // return 0 without running the migration, handing automation a false pass.
