...
function _resolvePath(p, cwd) {
  // Anything that is not a non-empty string is handed back untouched, so a
  // bare `--flag` (which parses as `true`) reaches the caller's own type check
  // rather than throwing inside nodePath.
...
var AUDIT_USAGE = [
  ...
  "  --public-key <path>    Audit-signing public key (PEM), to check the purge",
  "                         anchor that licenses any deleted rows. Without it a",
  "                         purged chain reports exit 3 rather than verified.",
  "  --archive-dir <path>   Directory holding archive bundles. The anchor names",
  "                         the archive its deleted rows went into; with this,",
  "                         one that is missing fails the verify instead of",
  ...
  "  2  bad invocation",
  "  3  verify-chain could not check the purge anchor (no signing key available)",
].join("\n");
...
async function _runAudit(args, ctx) {
  ...
  if (sub === "verify-chain") {
    ...
    try {
      ...
      var vcOpts = maxRowsN ? { maxRows: maxRowsN } : {};
      // The anchor names an archive; --archive-dir says where to look for it.
      // Without it the walk still runs and the result says the archive was not
      // resolved, because reporting one as found when nothing looked for it is
      // the false guarantee this whole surface exists to avoid.
      //
      // A bundle's directory name is chosen by whoever ran `audit archive` and
      // has nothing to do with the identifier the anchor records — that is the
      // checkpoint id inside the bundle's own manifest. So this reads the
      // manifests rather than guessing a filename, which would report every
      // normally-produced archive as missing.
      // Against ctx.cwd, not the process's — the CLI context carries its own
      // working directory and --db already honours it. A relative path that
      // resolved differently from one flag to the next would report a present
      // archive as missing, which is the one answer this command must not
      // give wrongly.
      var archiveDirV = _resolvePath(args.flags["archive-dir"], ctx.cwd);
      if (typeof archiveDirV === "string" && archiveDirV.length > 0) {
        vcOpts.resolveArchive = function (bundleId, range) {
          var entries;
          try { entries = nodeFs.readdirSync(archiveDirV); }
          catch (_e) { return false; }
          for (var ei = 0; ei < entries.length; ei += 1) {
            var manifestPath = nodePath.join(archiveDirV, entries[ei], "manifest.json");
            var parsed;
            // Capped at the read, not at the parse. These files come from
            // wherever the operator points the flag, and readFileSync would
            // pull an oversized manifest entirely into memory before
            // safeJson's limits could apply — the same reason the bundle
            // reader in b.auditTools uses this primitive. safeJson then adds
            // the prototype and depth defenses on top.
            try {
              parsed = safeJson.parse(atomicFile.fdSafeReadSync(manifestPath, {
                maxBytes: C.BYTES.mib(4), encoding: "utf8",
              }));
            } catch (_e2) { continue; }   // not a bundle directory
            if (!parsed) continue;
            // Identify the bundle before opening anything. The cheap
            // discriminators run first so the expensive proof below runs at
            // most once, over the single bundle that claims to be the one the
            // anchor names.
            //
            // The identifier alone does not name one bundle: successive
            // archives taken before the next checkpoint all resolve the same
            // covering checkpoint, so they all carry its id, and accepting a
            // sibling would report the anchored slice as present because a
            // DIFFERENT one still is. The range the anchor signed is what
            // distinguishes them, so the manifest has to agree with it.
            if (!parsed.range) continue;
            if (Number(parsed.range.lastCounter) !== Number(range && range.lastCounter) ||
                String(parsed.range.lastRowHash) !== String(range && range.lastRowHash)) {
              continue;
            }
            // A start of 0 is what an anchor written before that field was
            // covered holds; counters begin at 1, so it records no start
            // rather than claiming one, and there is nothing to compare.
            var wantFirst = Number(range && range.firstCounter) || 0;
            if (wantFirst !== 0 && Number(parsed.range.firstCounter) !== wantFirst) continue;
            // A bundle covered by a checkpoint is identified by that
            // checkpoint's id. One without a covering checkpoint has no such
            // id, and the purge records `manifest:<lastCounter>` instead —
            // matching only the first shape reports every checkpoint-less
            // archive as missing.
            var idMatches = (parsed.checkpoint &&
                String(parsed.checkpoint.checkpointId) === String(bundleId)) ||
              "manifest:" + String(parsed.range.lastCounter) === String(bundleId);
            if (!idMatches) continue;

            // A manifest naming the right archive is not the archive. The
            // payload beside it has to be there AND be what the manifest says
            // it is: a truncated or altered blob decrypts to nothing, so the
            // rows it accounts for cannot be produced, and reporting the
            // archive as found would be the false guarantee this flag exists
            // to remove. The bytes are not decrypted — that needs the bundle
            // passphrase, which this command does not take — so "found" means
            // present and matching the manifest's own checksums, which is the
            // most the check can honestly claim.
            var bundleDir = nodePath.join(archiveDirV, entries[ei]);
            // The set is whatever the bundle reader will demand of THIS
            // manifest, not a fixed list: an archive is refused without its
            // covering checkpoint, so a bundle missing checkpoint.enc cannot
            // be verified or restored and must not be reported as found.
            var needed = [[frameworkFiles.fileName("rowsEnc"), "rowsSha3_512", C.BYTES.mib(512)]];
            if (parsed.kind === auditTools.KIND_ARCHIVE || parsed.checkpoint) {
              needed.push([frameworkFiles.fileName("checkpointEnc"), "checkpointSha3_512", C.BYTES.mib(4)]);
            }
            var payloadOk = needed.every(function (f) {
              var filePath = nodePath.join(bundleDir, f[0]);
              try {
                // isFile, not just a size: a directory reports a non-zero size
                // on most filesystems, so size alone accepts one standing where
                // the payload should be — which the bundle reader cannot open.
                var st = nodeFs.statSync(filePath);
                if (!st.isFile() || st.size === 0) return false;
                // A manifest with no checksum for a member it requires was not
                // written by this framework, and removing the field is how an
                // altered bundle would evade the comparison — so an absent
                // expectation is a failure, never a pass. Every bundle the
                // archiver produces records one for each member it includes.
                var want = parsed.checksum && parsed.checksum[f[1]];
                if (!want) return false;
                // For the rows payload, the anchor's own digest wins when it
                // has one. The manifest travels WITH the bytes it describes,
                // so whoever rewrites rows.enc rewrites its checksum too and a
                // comparison between them only shows the bundle agrees with
                // itself. The anchor's copy is covered by the signature, so it
                // says which bytes the archive has to be in a claim only the
                // signing key could make — and a manifest that disagrees with
                // it is not the archive the anchor named.
                var signedWant = null;
                if (f[1] === "rowsSha3_512") signedWant = range && range.rowsDigest;
                // The checkpoint is bound the same way. Without it, replacing
                // checkpoint.enc and updating the manifest's checksum for it
                // passes every test here, and the bundle then fails to verify
                // for the one member an archive is refused without — reported
                // as producible, unusable in fact.
                if (f[1] === "checkpointSha3_512") signedWant = range && range.checkpointDigest;
                if (signedWant) {
                  if (String(want) !== String(signedWant)) return false;
                  want = signedWant;
                }
                // Same caps the bundle reader applies to these members, so a
                // blob it would refuse as oversized is not accepted here.
                return backupCrypto.checksum(
                  atomicFile.fdSafeReadSync(filePath, { maxBytes: f[2] })) === String(want);
              } catch (_e3) { return false; }
            });
            if (!payloadOk) continue;
            // The payloads match. What says how to OPEN them is the manifest,
            // and nothing above covers it: an attacker who can edit this
            // directory changes salts.rows alone, leaves both encrypted
            // payloads and both signed digests untouched, and every check so
            // far still passes — while the key derived from that salt opens
            // nothing. The archive would be reported producible and be
            // unreadable, which is the one claim this flag exists to make
            // honestly.
            //
            // Compared against the digest the SIGNATURE covers. Empty on an
            // anchor written before the field existed, and skipped there
            // rather than refused: a volume purged by an earlier build has no
            // manifest digest to check, and turning that into a failure would
            // strand it with no way back.
            if (range && range.manifestDigest) {
              var manifestOk = false;
              try {
                manifestOk = String(backupCrypto.checksum(
                  Buffer.from(canonicalJson.stringify(parsed), "utf8"))) ===
                  String(range.manifestDigest);
              } catch (_e4) { manifestOk = false; }
      ...
      // This command opens a database FILE; it never ran db.init, so it holds
      // no signing key of its own. The purge anchor still has to be checked —
      // it is the one statement that licenses missing rows — so the key comes
      // from the operator. Read once, here, rather than per call.
      // A flag written without its value parses as `true`. Ignoring that
      // silently is the worst reading of it: the operator asked for the
      // anchor's signature to be checked, the command does not check it, and
      // reports success. Both of these flags exist to turn an unverified
      // result into a verified one, so a malformed invocation of either is a
      // bad invocation rather than a quieter run.
      var flagChecks = [["public-key", args.flags["public-key"]],
        ["archive-dir", args.flags["archive-dir"]]];
      for (var fi = 0; fi < flagChecks.length; fi += 1) {
        var fv = flagChecks[fi][1];
        if (fv !== undefined && (typeof fv !== "string" || fv.length === 0)) {
          _writeLine(ctx.stderr, "blamejs audit verify-chain: --" + flagChecks[fi][0] +
            " requires a path");
          return 2;
        }
      }

      var pubKeyPathV = _resolvePath(args.flags["public-key"], ctx.cwd);
      if (typeof pubKeyPathV === "string" && pubKeyPathV.length > 0) {
        var pemV;
        try { pemV = nodeFs.readFileSync(pubKeyPathV, "utf8"); }
        catch (e) {
          _writeLine(ctx.stderr, "blamejs audit verify-chain: cannot read --public-key at " +
            pubKeyPathV + ": " + ((e && e.message) || String(e)));
          return 2;
        }
        // Bound to the fingerprint the anchor names. Returning this key for
        // whatever fingerprint is asked for would make that column
        // unprotected: edit it alone and the signature still verifies against
        // the supplied key, so the anchor would be reported as
        // signature-verified while naming a key that did not sign it.
        //
        // Parse before fingerprinting. A fingerprint is a hash of the bytes,
        // so ANY text produces one — including a JSON file or a private key —
        // and that fingerprint simply matches no anchor. The chain would then
        // be reported as signed under an unknown key: a tampering alarm on a
        // healthy volume, caused by a wrong path on the command line. Parsing
        // separates "you passed the wrong file" from "this anchor is forged".
        // One key can answer to two fingerprints, because a fingerprint hashes
        // the PEM text — see b.auditSign.pinnedKeyResolver, which owns that
        // rule for every place an operator names a key: here, and
        // `db.init({ purgeAnchorPublicKey })`.
        try {
          vcOpts.resolvePublicKey = auditSign.pinnedKeyResolver(pemV);
        } catch (e) {
          _writeLine(ctx.stderr, "blamejs audit verify-chain: --public-key at " +
            pubKeyPathV + " is not a PEM public key: " + ((e && e.message) || String(e)));
          return 2;
        }
      }
      var vc = await auditChain.verifyChain(queryAllAsync, tableV, vcOpts);
      if (vc.ok) {
        var anchorNote = "";
        if (vc.purgeAnchor) {
          // Say what was and was not established. A chain that verifies over a
          // purge is only as good as the anchor licensing it, and whether the
          // archive those rows went into was actually found is a separate
          // question from whether the boundary was signed.
          anchorNote = ", purgeAnchor=" +
            (vc.purgeAnchor.signatureVerified ? "signature-verified" : "trusted-unsigned") +
            " below counter " + vc.purgeAnchor.belowCounter +
            (vc.purgeAnchor.archiveResolved
              ? ", archive " + vc.purgeAnchor.archiveBundleId + " found (covers " +
                vc.purgeAnchor.archiveCoversFrom + ".." + vc.purgeAnchor.archiveCoversTo +
                (vc.purgeAnchor.archiveCoversFrom > 1
                  ? "; earlier purges were licensed by anchors this one replaced — " +
                    "their archives are not named here"
                  : "") + ")"
              : ", archive " + vc.purgeAnchor.archiveBundleId + " NOT resolved");
        }
        _writeLine(ctx.stdout, "OK — chain verified" +
          " (table=" + vc.table +
          ", rowsVerified=" + vc.rowsVerified + anchorNote + ")");
        return 0;
      }
      // "Could not check" is not "was tampered with". Reporting an
      // unverifiable anchor as a chain break would send an operator hunting a
      // breach on a healthy volume; its own exit code says what to fix.
      if (vc.purgeAnchorUnchecked) {
        _writeLine(ctx.stderr, "UNCHECKED — " + vc.reason);
        _writeLine(ctx.stderr, "  Pass --public-key <path> with the audit-signing public key, " +
          "in PEM. A key rotated out since the anchor was written is recorded " +
          "in audit-sign.pubkeys.json, which holds the PEM to extract.");
        return 3;
      }
