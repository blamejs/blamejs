...
async function main(argv, opts) {
  ...
  // Version is honored only as the WHOLE invocation — decided from the RAW
  // argv (every token a version flag), never from the parsed positional
  // count: _parseArgs treats `--version` as a value flag, so `--version
  // <cmd>` swallows the subcommand as its "value" (pos ends up empty) and a
  // positional-count check would mistake it for a bare version request. A
  // stray `-v` / `--version` alongside a command — in EITHER order — must not
  // short-circuit it (that would hand automation a false exit 0), so the
  // version token is stripped and the command dispatches normally.
  var _isVersionFlag = function (t) { return t === "-v" || t === "--version"; };
  // Only tokens BEFORE the `--` option terminator participate in version
  // handling; everything at/after `--` is literal positional data (e.g.
  // `file-type detect -- --version` must inspect a file NAMED --version).
  var _termIdx = argv.indexOf("--");
  var _preTerm = _termIdx === -1 ? argv : argv.slice(0, _termIdx);
  var _hasVersionFlag = _preTerm.some(_isVersionFlag);
  // Bare version request: no terminator, and every token is a version flag.
  if (_termIdx === -1 && _hasVersionFlag && _preTerm.every(_isVersionFlag)) {
    _writeLine(ctx.stdout, C.version);
    return 0;
  }
  // Otherwise a pre-terminator version flag is stray — strip it (before `--`
  // only) so it can't swallow the subcommand as its value; the command runs.
  if (_hasVersionFlag) {
    argv = argv.filter(function (t, i) {
      return (_termIdx !== -1 && i >= _termIdx) || !_isVersionFlag(t);
    });
  }
