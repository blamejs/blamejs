...
async function main(argv, opts) {
  ...
  var _versionTokens = argv.filter(_isVersionFlag);
  if (_versionTokens.length > 0 && _versionTokens.length === argv.length) {
    _writeLine(ctx.stdout, C.version);
    return 0;
  }
  if (_versionTokens.length > 0) {
    argv = argv.filter(function (t) { return !_isVersionFlag(t); });
  }
