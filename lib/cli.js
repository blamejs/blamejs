...
var AUDIT_USAGE = [
  ...
  "  --audit-key <path>     Audit-signing PUBLIC key (PEM). Needed to check a",
  "                         purge anchor's signature; without it a volume that",
  "                         has been purged reports the anchor as unchecked.",
  "",
  "purge flags:",
  ...
  "  2  bad invocation",
  "  3  verify-chain could not check a signed purge anchor (no --audit-key)",
].join("\n");
...
async function _runAudit(args, ctx) {
  ...
  if (sub === "verify-chain") {
    ...
    try {
      ...
      // This command opens the database file directly and never initializes
      // b.auditSign, so the chain module has no key material of its own to
      // reach for. Supply one from --audit-key when the operator names it; the
      // resolver ignores the fingerprint because a PEM on the command line is
      // the operator saying "check it against this". Without the flag the
      // anchor is reported as unchecked rather than as invalid — a purged
      // volume is not a tampered one.
      var vcOpts = maxRowsN ? { maxRows: maxRowsN } : {};
      var auditKeyPath = args.flags["audit-key"];
      if (auditKeyPath && auditKeyPath !== true) {
        var auditKeyPem;
        try { auditKeyPem = nodeFs.readFileSync(_resolvePath(String(auditKeyPath), ctx.cwd), "utf8"); }
        catch (e) {
          _writeLine(ctx.stderr, "blamejs audit verify-chain: cannot read --audit-key: " +
            ((e && e.message) || String(e)));
          return 2;
        }
        // Answer only for the fingerprint the anchor actually names. Returning
        // the PEM regardless would mean editing just the stored fingerprint
        // still verified OK here while failing in-process — the fingerprint is
        // part of what the anchor claims, so it has to be part of what is
        // checked.
        var auditKeyFp = null;
        try { auditKeyFp = auditSign.fingerprintOf(auditKeyPem); }
        catch (e) {
          _writeLine(ctx.stderr, "blamejs audit verify-chain: --audit-key is not a " +
            "readable public key: " + ((e && e.message) || String(e)));
          return 2;
        }
        vcOpts.resolvePublicKey = function (fp) {
          return fp === auditKeyFp ? auditKeyPem : null;
        };
      }
      ...
      if (vc.ok) {
        ...
        // A gap the chain was told to skip is reported as what it is. The
        // anchor's signature is checked, so the counter is trustworthy; the
        // ARCHIVE it names is not resolved, because nothing here knows where a
        // consumer keeps bundles. Saying "a verified archive removed these
        // rows" would assert a verification nobody performed, and an operator
        // reading it could not tell the two apart.
        if (vc.purgeAnchor && vc.purgeAnchor.honored) {
          _writeLine(ctx.stdout,
            "     rows at or below counter " + vc.purgeAnchor.belowCounter +
            " are absent, on a purge anchor whose signature verified.");
          _writeLine(ctx.stdout,
            "     The archive it names was NOT resolved — confirm that bundle exists" +
            " before treating those rows as archived.");
        }
        return 0;
      }
      // A verifier that could not reach any key has not found tampering; it has
      // found that it cannot answer. Reporting that as FAIL would make the
      // documented no-key run indistinguishable from a forged anchor, and an
      // operator would either treat a healthy purged volume as an incident or
      // learn to ignore the one message that matters.
      if (vc.purgeAnchorUnchecked) {
        _writeLine(ctx.stderr, "UNCHECKED — " + vc.reason);
        _writeLine(ctx.stderr,
          "  This volume has been purged and its anchor is signed, but no audit-signing" +
          " key was available to check that signature. Re-run with --audit-key <path>" +
          " to verify it. Nothing here says the chain is bad.");
        return 3;
      }
