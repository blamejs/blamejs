...
var RESTORE_USAGE = [
  ...
  "  --staging-root <path>  Directory for the pulled and decrypted bundle; must be on the",
  "                         filesystem of the data dir (default: the rollback dir's parent)",
  "  --expected-fingerprint <hex>  Trust a manifest signature only from the audit-sign key",
  "                         with this fingerprint. The CLI loads no audit-sign key, so a",
  "                         signed bundle needs this flag or --no-verify-signature",
  "  --require-signature    Refuse a bundle whose manifest is not signed",
  "  --no-verify-signature  Restore without checking the manifest signature",
  "",
  "rollback flags:",
...
async function _runRestore(args, ctx) {
  ...
  if (sub === "apply") {
    ...
    var stagingRootFlag = args.flags["staging-root"];
    if (stagingRootFlag !== undefined && (stagingRootFlag === true || String(stagingRootFlag).length === 0)) {
      return report.error("--staging-root requires a directory path", 2);
    }
    var stagingRootA = stagingRootFlag !== undefined ? _resolvePath(String(stagingRootFlag), ctx.cwd) : undefined;
    ...
    var sigOptsA = cliHelpers.resolveSignatureOptions(args, report);
    if (!sigOptsA) return 2;
    try {
      ...
      var rA = restore.create({
        ...
        requireSignature:    sigOptsA.requireSignature,
        expectedFingerprint: sigOptsA.expectedFingerprint,
        verifySignature:     sigOptsA.verifySignature,
      });
...
var BACKUP_USAGE = [
  ...
  "                         signed bundle needs this flag or --no-verify-signature",
  "  --require-signature    Refuse a bundle whose manifest is not signed",
  "  --no-verify-signature  Decrypt without checking the manifest signature",
].join("\n");
...
async function _runBackup(args, ctx) {
  ...
  var sigOpts = cliHelpers.resolveSignatureOptions(args, report);
  if (!sigOpts) return 2;
  ...
  if (sub === "verify") {
    ...
    try {
      var r = await restoreBundle.extract({
        ...
        passphrase: pp,
        requireSignature:    sigOpts.requireSignature,
        expectedFingerprint: sigOpts.expectedFingerprint,
        verifySignature:     sigOpts.verifySignature,
      });
