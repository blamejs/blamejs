...
// The framework's outbound TLS posture as a fresh plain options object,
// ready to merge into any tls.connect / https.request / https.Agent options:
//
//   var connectOpts = Object.assign({ host: h, port: p },
//                                   C.TLS_OUTBOUND_POSTURE());
//
// Every protocol client the framework ships (DoH, DoT, NTS-KE, Redis,
// syslog, WebSocket, proxy tunnels, the HTTP client) merges this instead of
// repeating the individual keys, so raising the posture is one edit that
// cannot reach some outbound paths and miss others. `certificateCompression`
// is omitted on a runtime that does not implement RFC 8879 rather than
// passed empty.
function TLS_OUTBOUND_POSTURE() {
  var posture = {
    minVersion: "TLSv1.3",
    ecdhCurve:  TLS_GROUP_CURVE_STR,
  };
  var certAlgs = TLS_CERT_COMPRESSION();
...
module.exports = {
  ...
  TLS_OUTBOUND_POSTURE:   TLS_OUTBOUND_POSTURE,
