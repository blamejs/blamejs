...
function _defaultClassify(event) {
  ...
  // `warning` is an outcome as well as a severity, and it reaches the same
  // level. The default review threshold is `warning`, so classifying the
  // outcome as `info` would leave every partial result out of the review --
  // and finding partial results is what the outcome was added for.
...
function create(opts) {
  ...
  validateOpts.auditShape(opts.audit, "auditDailyReview",
    AuditDailyReviewError, "audit-daily-review/bad-audit");
  if (!opts.audit) {
    throw _err("audit-daily-review/audit-required",
      "auditDailyReview.create: opts.audit is required (must expose query() / safeEmit())");
  }
  if (typeof opts.audit.query !== "function") {
    throw _err("audit-daily-review/audit-query-missing",
      "auditDailyReview.create: opts.audit.query must be a function");
  }
  validateOpts.shape(opts, {
    audit:        { methods: ["query", "safeEmit"] },
    notify:       { rule: "optional-function",     code: "audit-daily-review/bad-notify" },
    classify:     { rule: "optional-function",     code: "audit-daily-review/bad-classify" },
    now:          { rule: "optional-function",     code: "audit-daily-review/bad-now" },
    posture:      { rule: "optional-string",       code: "audit-daily-review/bad-posture" },
    cron:         { rule: "optional-string",       code: "audit-daily-review/bad-cron" },
    severityThreshold: { rule: "optional-string",  code: "audit-daily-review/bad-severity" },
    queryLimit:   { rule: "optional-positive-int", code: "audit-daily-review/bad-querylimit" },
    historyLimit: { rule: "optional-positive-int", code: "audit-daily-review/bad-historylimit" },
    ...
    lookbackHours: function (value, label) {
      ...
      if (typeof value !== "number" || !isFinite(value) || value <= 0) {
        throw _err("audit-daily-review/bad-lookback",
          "auditDailyReview.create: lookbackHours must be a positive finite number");
      }
    },
  }, "auditDailyReview", AuditDailyReviewError, "audit-daily-review/bad-opts",
  ...
  if (SEVERITY_ORDER.indexOf(severityThreshold) === -1) {
    throw _err("audit-daily-review/bad-severity",
  ...
  if (posture && POSTURES_REQUIRING_NOTIFY.indexOf(posture) !== -1 && !opts.notify) {
    throw _err("audit-daily-review/notify-required-under-posture",
  ...
  async function run() {
    ...
    try {
      ...
      throw _err("audit-daily-review/query-failed",
        "auditDailyReview.run: audit.query failed: " + ((e && e.message) || String(e)));
    }

    var bySeverity = { info: 0, notice: 0, warning: 0, alert: 0, critical: 0 };
    // Every outcome the vocabulary carries gets its own bucket. `warning` is
    // one of them, and counting it under `other` would hide exactly the rows a
    // daily review exists to surface: the partial results.
    var byOutcome  = { success: 0, warning: 0, failure: 0, denied: 0, other: 0 };
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var oc = r && r.outcome;
      if (Object.prototype.hasOwnProperty.call(byOutcome, oc) && oc !== "other") byOutcome[oc]++;
  ...
  async function start() {
    if (!schedulerMod) {
      throw _err("audit-daily-review/no-scheduler",
