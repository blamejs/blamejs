...
function decrypt(compact, recipientPrivateKeyPem, opts) {
  ...
  var header;
  // Header is base64url-decoded above; route through safeJson.parse
  // for proto-pollution + depth + size defenses (operator-supplied
  // compact bytes are adversarial).
  var headerBytes = bCrypto.fromBase64Url(parts[0]);
  ...
  var fwEnvelopeBuf = bCrypto.fromBase64Url(parts[3]);
  var fwEnvelopeB64 = fwEnvelopeBuf.toString("base64");
  var plaintext = bCrypto.decrypt(fwEnvelopeB64, { privateKey: recipientPrivateKeyPem });
  if (opts.audit !== false) {
    audit.safeEmit({
