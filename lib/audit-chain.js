...
function purgeAnchorPayload(anchor, opts) {
  ...
  // The digest of the archive's encrypted rows, so the signature covers WHICH
  // BYTES the named archive has to be. A manifest states its own checksums and
  // whoever can rewrite the payload can rewrite them with it, so a checksum
  // read from there proves the bundle is internally consistent and nothing
  // more. Signed here, it is a claim only the signing key can make.
  //
  // Empty for an anchor written before the field existed. Those are
  // branch-build volumes only — the format has never shipped — but the empty
  // case still has to be representable, because `signExistingPurgeAnchor`
  // re-signs an anchor from whatever the row already holds.
  var rowsDigest = anchor.archiveRowsDigest == null ? "" : String(anchor.archiveRowsDigest);
  // The covering checkpoint is signed for the same reason, and separately: an
  // archive is refused without it, so a bundle whose checkpoint payload has
  // been replaced cannot be verified or restored. Binding only the rows would
  // leave that member checked against the manifest alone — which the same hand
  // that rewrote the payload rewrites with it — so an archive could be
  // reported producible while being unusable.
  var ckptDigest = anchor.archiveCheckpointDigest == null
    ? "" : String(anchor.archiveCheckpointDigest);
  // The manifest, which is how those payloads are OPENED. The two digests above
  // cover the encrypted bytes and say nothing about the salts the decryption
  // key is derived from — so editing salts.rows alone left every payload and
  // every signed digest intact, the archive reported as producible, and the
  // bundle undecryptable. A digest over the canonical manifest closes that:
  // canonical rather than the file's raw bytes so a reformat is not a refusal,
  // while any change to a value it carries is.
  ...
  var head =
    ...
    String(purgedAt) + "\n" +
    String(fencingToken);
  // Each field the payload gained is one more line on the end, so the earlier
  // layouts are prefixes of the current one. NOTHING is ever signed as
  // anything but "current" — the public primitive always builds that — but
  // verification retries the older shapes for an anchor written by a build
  // that predates a field. See verifyPurgeAnchor for why each retry is safe
  // and why each is refused once the field it omits actually has a value.
...
// purgeAnchorSignatureVerifies(row, signature, publicKeyPem) — does this
// signature cover this anchor, under the current layout or a historical one?
//
// An anchor written by a build that predates a signed field has a signature
// over the shorter bytes, and the additive migration fills the new column with
// a default — so rebuilding the current layout produces different bytes and
// the anchor reads as forged. Refusing it would strand a volume nobody
// touched, and the repair that would fix it needs a booted database, which the
// refusal prevents.
//
// Each retry is GATED ON THE FIELD IT OMITS STILL BEING EMPTY, which is the
// only state an anchor predating that field can be in, and which means the
// same thing in both layouts. An anchor that actually records the field gets
// no second chance, so none of this becomes a way to strip a field from an
// anchor that has one. Producing any of these signatures still requires the
// signing key, so nothing is admitted that was not written by something
// holding it.
//
// Ordered newest-first. A future signed field adds one entry here and one line
// in purgeAnchorPayload, rather than another special case.
//
// Every caller that decides an anchor is genuine goes through this, not just
// the read path: the repair that re-signs a rotated-key anchor asks the same
// question, and a repair that recognised fewer layouts than the verify would
// refuse to fix the exact anchors the verify was written to rescue.
/**
...
function purgeAnchorSignatureVerifies(row, signature, publicKeyPem) {
  ...
  // Newest field first, and every older rung additionally requires the newer
  // fields to be EMPTY. That gating is what keeps a retry from being a way to
  // strip a field: an anchor that records a manifest digest gets no layout
  // that omits one, so a signature minted over the shorter bytes cannot be
  // replayed against a row that has since gained the value.
  //
  // The no-manifest-digest rung is what an anchor written before 0.18.58 needs
  // — the additive migration fills its new column with a default, so rebuilding
  // the current layout would produce different bytes and report a healthy
  // volume as forged. A refusal with no reachable recovery is a worse bug than
  // the one being fixed.
  var layouts = [
...
function verifyPurgeAnchor(row, opts) {
  ...
  // Built here only so an unbuildable one is refused as corrupt; the bytes a
  // signature is checked against are rebuilt per layout below.
  ...
  try {
    // NOT getPublicKeyByFingerprint: that consults the rotated-key history,
    // which is unsealed so a verifier with no passphrase can read it. Anyone
    // able to write the audit store can write that file too, and adding a
    // self-consistent entry for a keypair they generated needs no secret — so
    // a history-resolved key licensing deleted rows is the attacker vouching
    // for themselves. A checkpoint resolved that way only adds a claim; a
    // purge anchor erases. Only the live key, which is sealed behind a
    // passphrase this process was given, carries that.
    //
    // A caller-supplied resolver is a different matter: it is the operator
    // naming the key on the command line, which is a trust root they chose,
    // and the fingerprint binding below still holds it to the anchor's name.
  ...
  if (!publicKeyPem) {
    // Both outcomes below refuse. The difference is only what the operator is
    // told, and the history is read HERE purely to say which situation it is —
    // never to grant anything, so an attacker who plants an entry buys a
    // better error message and nothing else.
...
// A signed anchor always has these two; everything after them arrived later.
var PURGE_ANCHOR_SIGNATURE_COLUMNS = ["signature", "publicKeyFingerprint"];
// The columns that joined the signed anchor after it first became signed, in
// the order they arrived. Each one is covered by the signature, so each needs
// its own rung on the ladder below — a projection that omits a column the table
// HAS rebuilds the payload with that field's default and reports an untouched
// anchor as forged. fencingToken is the reason this list is not just the
// digests: it defaults to 0, so omitting it condemns every anchor written in
// cluster mode.
...
//
// Derived from the list above rather than written out, because writing it out
// is how a rung goes missing: the checkpoint-digest step and the rows-digest
// step were once one entry, so a table carrying the rows digest but not the
// checkpoint digest lost both. Its anchor was signed under a layout that COVERS
// the rows digest, so it rebuilt with an empty one and an untouched volume
// verified as forged — with the repair path behind the refusal that raises.
// One rung per versioned column now holds by construction, and the rungs line
// up with the payload layouts: `current`, `no-checkpoint-digest`, `no-digest`,
// `no-range`.
var PURGE_ANCHOR_PROJECTIONS = (function () {
  ...
  // The last rung drops the signature columns too: an anchor written before
  // signing existed at all.
...
//
// The driver's own structured code is asked FIRST, through the one predicate
// every caller shares — it is stable across locales and phrasings, and MySQL's
// contracted "Table 'db.X' doesn't exist" matches none of the wordings below,
// so a MySQL schema without the anchor table propagated its error instead of
// reading as an absence.
//
// The message test stays as the fallback for drivers that carry no structured
// code, and it stays NARROWER than the shared predicate's siblings elsewhere:
...
// purged legacy chain would verify from ZERO_HASH with its anchor unread. A
// message this does not recognize propagates, so an unfamiliar failure stops
// the verify rather than passing for an absence.
...
// Structured code first, for the same reason as the table check above; the
// wording fallback covers a driver that carries none.
      ...
      if (typeof opts.resolveArchive === "function") {
        ...
        try {
          ...
          producible = await opts.resolveArchive(String(anchorRow.archiveBundleId), {
            ...
            // The digest the SIGNATURE covers, so a resolver can check the
            // bundle against something the bundle does not get to state about
            // itself. Empty on an anchor written before the field existed.
            rowsDigest:   anchorRow.archiveRowsDigest == null
                          ? "" : String(anchorRow.archiveRowsDigest),
            // Same, for the member an archive is refused without.
            checkpointDigest: anchorRow.archiveCheckpointDigest == null
                          ? "" : String(anchorRow.archiveCheckpointDigest),
            // And the manifest, which is how those payloads are opened. A
            // resolver checking only the payload digests accepts a bundle
            // whose salts were altered — every byte it compares still matches,
            // and the key derived from those salts opens nothing.
