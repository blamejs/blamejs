...
async function verifyChain(queryAllAsync, tableName, opts) {
  ...
  if (tableName === "audit_log") {
    ...
    if (Array.isArray(anchor) && anchor.length > 0) {
      ...
      if (!anchorSigVerdict.ok && opts.allowUnsignedPurgeAnchor === true) {
        // Only the verdict and the trusted-not-verified flag are set here; the
        // anchor is then adopted by the common path below, which is the single
        // place that decides what a honoured anchor means.
      ...
      // Kept separately from skipBeforeCounter, which an incremental verify
      // (opts.from) raises to the predecessor of the requested range. Reporting
      // that as the anchor's boundary would say an anchor authorizing a purge
      // through counter 100 authorized one through 149 — and a caller reading
      // it would classify rows that are present, and merely outside the range
      // asked for, as purged.
      purgeAnchorCounter = aCounter;
    }
  }
  ...
  if (walked.ok && purgeAnchorHonored) {
    walked.purgeAnchor = {
      honored:         true,
      belowCounter:    purgeAnchorCounter,
