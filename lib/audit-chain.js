...
function purgeAnchorPayload(anchor) {
  ...
  if (!numericBounds.isNonNegativeSafeInt(counter)) {
    throw new TypeError("purgeAnchorPayload: lastPurgedCounter must be a whole " +
      "non-negative number below 2^53 — above that, two distinct stored values " +
      "render identically and one signature would cover both");
  }
 ...
 *   resolveArchive:   function, // (archiveBundleId) => truthy if the archive can still be produced; an anchor naming one that cannot is refused
...
function verifyPurgeAnchor(row, opts) {
  ...
  if (!numericBounds.isNonNegativeSafeInt(counter)) {
    return { status: "corrupt", reason: "corrupted purge anchor: lastPurgedCounter is not a whole non-negative number below 2^53" };
  }
...
var PURGE_ANCHOR_SIGNED_COLUMNS = PURGE_ANCHOR_LEGACY_COLUMNS
  .concat(["signature", "publicKeyFingerprint", "fencingToken"]);
...
async function _readPurgeAnchorRow(queryAllAsync) {
  try {
    return await _selectPurgeAnchor(queryAllAsync, PURGE_ANCHOR_SIGNED_COLUMNS);
  } catch (e) {
    // A missing TABLE is answered here rather than by trying again: the second
    // read would fail the same way, and the fallback exists for the narrower
    // case of a table that is present but older.
    if (_isMissingRelation(e)) return null;
    if (!_isMissingColumn(e)) throw e;
  }
  try {
    return await _selectPurgeAnchor(queryAllAsync, PURGE_ANCHOR_LEGACY_COLUMNS);
  } catch (e2) {
    if (_isMissingRelation(e2)) return null;
    throw e2;
  }
}
 ...
 * `resolveArchive(archiveBundleId)` to supply that check: an anchor naming an
 * archive that cannot be produced then STOPS the verify rather than licensing
 * the gap. Without it the walk proceeds and the result reports
