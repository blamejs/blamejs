 ...
 *
 * Checkpoint signing (ML-DSA-87 over (atRow || atRowHash) every N rows or
 * M minutes) is deferred to Phase 7. v0.0.4 ships chain math only — full
...
// given audit table. Returns { prevHash: ZERO_HASH, counter: 0 } if empty.
function getChainTip(database, tableName) {
...
// log warning, etc.).
function verifyChain(database, tableName, opts) {
  opts = opts || {};
  ...
  if (rows.length === 0) {
    return { ok: true, table: tableName, rowsVerified: 0, lastHash: ZERO_HASH };
  }
