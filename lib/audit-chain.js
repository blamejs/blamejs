 ...
 *
 * @opts
 *   layout: string,  // default: "current". Also "no-digest" and "no-range", which reproduce the payload as it stood before each field joined it — for VERIFYING an anchor written by an earlier build, never for signing a new one. See `PURGE_ANCHOR_LAYOUTS`
 *
 * @example
...
function verifyPurgeAnchor(row, opts) {
  ...
  // Retries, against the payload as it stood before each field joined it.
  //
  // An anchor written by a build that predates a field has a signature over
  // the shorter bytes, and the additive migration fills the column with a
  // default — so rebuilding the current layout produces different bytes and
  // the anchor reads as forged. Refusing it would strand a volume nobody
  // touched, and the repair that would fix it needs a booted database, which
  ...
  // holding it.
  //
  // Ordered newest-first. A future signed field adds one entry here and one
  // line in purgeAnchorPayload, rather than another special case.
  ...
  var retries = [
    { layout: "no-digest", when: storedDigest === "" },
    { layout: "no-range",  when: storedDigest === "" && storedRange === 0 },
  ];
  for (var ri = 0; ri < retries.length && !ok; ri += 1) {
    if (!retries[ri].when) continue;
    try {
      ok = auditSign.verify(
        purgeAnchorPayload(row, { layout: retries[ri].layout }), signature, publicKeyPem);
    } catch (_e2) { ok = false; }
  }
