...
var LocalHttpError = defineClass("LocalHttpError", { alwaysPermanent: true });
...
// A dotted-decimal octet is only in 127.0.0.0/8 if every octet is 0..255. A
// loose \d{1,3} would admit "127.300.0.1", which is NOT a valid IP and would be
// resolved as a hostname — an SSRF bypass of the loopback guard.
function _octetInRange(s) { var n = Number(s); return n >= 0 && n <= 255; }

// Loopback-only guard for the TCP mode: keeps the "SSRF-safe by construction"
// property — a caller cannot point the loopback client at an internal host.
function _isLoopbackHost(host) {
  if (typeof host !== "string") return false;
  var h = _canonHost(host);
  if (h === "localhost" || h === "127.0.0.1" || h === "::1") return true;
  var m = /^127\.(\d{1,3})\.(\d{1,3})\.(\d{1,3})$/.exec(h);   // 127.0.0.0/8, octet-validated
  return !!(m && _octetInRange(m[1]) && _octetInRange(m[2]) && _octetInRange(m[3]));
}
 ...
 *   host:             string,          // loopback host for the TCP+token mode (127.0.0.1 / ::1 / localhost)
...
function create(opts) {
  ...
  if (hasSocket) {
    ...
    if (!_isLoopbackHost(opts.host)) {
      throw new LocalHttpError("local-http/non-loopback-host",
        "create: opts.host must be a loopback address (127.0.0.0/8 / ::1 / localhost) — a non-loopback " +
  ...
  function postJson(path, obj, popts) {
