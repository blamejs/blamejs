 ...
 * which the same 1 MiB cap bounds.
 * Throws an `McpError` with a code matching the violation
 * (`mcp/bad-json` / `mcp/bad-envelope` / `mcp/bad-version` / `mcp/bad-method` /
 * `mcp/bad-id` / `mcp/bad-params`). Returns the parsed envelope on success.
 *
 * @opts
...
function parseRequest(body, opts) {
  ...
  if (body && typeof body === "object" &&
    ...
    var sized;
    try {
      sized = safeJson.measureBytes(body, {
        limit: C.BYTES.mib(1), followToJson: false,
      });
    } catch (_m) {
      throw errorClass.factory("mcp/bad-json",
        "mcp.parseRequest: body has no JSON form");
    }
    if (sized.exceeded) {
      throw errorClass.factory("mcp/bad-json",
        "mcp.parseRequest: body exceeds the " + C.BYTES.mib(1) + " byte cap");
    }
  }
  var parsed;
  try {
...
function refuse(res, code, message, id) {
  ...
  res.statusCode = code === JSONRPC_PARSE_ERROR || code === JSONRPC_INVALID_REQUEST ? 400 :
                   ...
                   code === JSONRPC_AUTH_REQUIRED ? 401 :
...
function serverGuard(opts) {
  ...
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxBodyBytes, "mcp.serverGuard: opts.maxBodyBytes", errorClass, "mcp/bad-max-bytes");
  ...
  return function mcpGuard(req, res, next) {
    Promise.resolve().then(function () {
      ...
      return claimsPromise.then(function (claims) {
        ...
        var normPath = codepointClass.trimTrailingChars(path, "/");
...
function _validateValueAgainstSchema(value, schema, path) {
  ...
  if (typeof value === "string") {
    ...
    if (typeof schema.pattern === "string") {
      if (value.length > 4096) return path + ": value exceeds 4096-char cap before regex test";
