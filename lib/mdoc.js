...
function _validityMs(v, name) {
  ...
  if (typeof raw === "number" && isFinite(raw)) return raw * C.TIME.seconds(1);
...
function _verifyChain(chainDer, anchorsPem, at) {
  ...
  var atMs = at.getTime();
  var res = x509Chain.resolveChain(pool[0], pool, anchors, {
    validAt: function (cert) {
      return atMs >= cert.validFromDate.getTime() && atMs <= cert.validToDate.getTime();
    },
  });
  if (res.ok) return;
  if (res.invalidCert) {
    throw new MdocError("mdoc/cert-expired", "mdoc.verifyIssuerSigned: certificate '" + res.invalidCert.subject + "' is not valid at the asserted time");
  }
  if (res.reason === "pathlen") {
    throw new MdocError("mdoc/chain-pathlen-exceeded", "mdoc.verifyIssuerSigned: certificate chain exceeds a CA basicConstraints pathLenConstraint (RFC 5280 §4.2.1.9)");
  }
  if (res.reason === "nameconstraint") {
    throw new MdocError("mdoc/name-constraint-violation", "mdoc.verifyIssuerSigned: certificate chain violates a CA nameConstraints (RFC 5280 §4.2.1.10)");
  }
