...
function serialize(name, value, attrs) {
  ...
  // operator) gets the same enforcement. Browsers match the prefix ASCII
  // case-INSENSITIVELY (Chromium/Firefox/WebKit all lowercase the name
  // before the prefix test), so `__host-` / `__SECURE-` get the same
  // browser enforcement as `__Host-` / `__Secure-`. Compare a lowercased
  // copy so a case-variant name can't dodge the invariant here and then
