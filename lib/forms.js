...
function validate(spec, body) {
  ...
  for (var i = 0; i < spec.fields.length; i++) {
    ...
    if (f.type === "number" || f.type === "range") {
      ...
      if (f.min !== undefined && coerced < _numericBound(f, "min")) {
        errors[f.name] = (f.label || f.name) + " must be ≥ " + f.min;
        continue;
      }
      if (f.max !== undefined && coerced > _numericBound(f, "max")) {
    ...
    if (typeof coerced === "string") {
      if (f.minlength !== undefined && coerced.length < _numericBound(f, "minlength")) {
        errors[f.name] = (f.label || f.name) + " must be at least " + f.minlength + " characters";
        continue;
      }
      if (f.maxlength !== undefined && coerced.length > _numericBound(f, "maxlength")) {
