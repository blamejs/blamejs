...
function _coerce(field, raw) {
  ...
  if (field.type === "checkbox") {
    // Unchecked is the ABSENCE of the field (undefined) for a urlencoded
    // form, and boolean false / numeric 0 (plus their string forms) for a
    // JSON body. A required checkbox must reject every one of these:
    // recognizing only the string forms let a JSON `false` / `0` fall through
    // to `true` and slip past the required-checkbox check.
    if (raw === undefined || raw === null || raw === "" ||
        raw === false || raw === 0 || raw === "false" || raw === "0") return false;
    return true;
  }
...
function _numericBound(f, key) {
  ...
  if (!Number.isFinite(n)) {
    throw new Error("forms.validate: field '" + f.name + "'." + key +
      " must be a finite number; got " + (typeof f[key]) +
      ". Fix the form spec.");
  }
  return n;
}
