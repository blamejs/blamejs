...
function _patternMatchesWholeValue(pattern, value) {
  // allow:dynamic-regex — anchors an operator pattern already screened by guardRegex.assertSafe (forms render/validate) to the whole value; ^(?:…)$ wrapping adds no backtracking
  var anchored = new RegExp("^(?:" + pattern.source + ")$", pattern.flags.replace(/[gym]/g, ""));
...
function validate(spec, body) {
  ...
  for (var i = 0; i < spec.fields.length; i++) {
    ...
    if (typeof coerced === "string") {
      ...
      if (f.pattern && !_patternMatchesWholeValue(f.pattern, coerced)) {
