...
//
// The fractional part is written `(?:\.\d*)?` rather than `\.?\d*` so that
// entering it requires a literal dot. Spelled the looser way, a run of digits
// could be split between the two quantifiers in as many places as it has
// digits, and a long run followed by a character that fails the anchor took
// time proportional to the SQUARE of the length. The two forms accept and
