...
function _detectIssues(flow, opts) {
  ...
  if (typeof flow.redirect_uri === "string" &&
    ...
    // broken is that the warning went nowhere: "medium" keeps it out of the
    // refusal set while putting it where an operator reads findings.
    //
    ...
    if (!Array.isArray(allowlist) || allowlist.length === 0) {
      issues.push({
        kind: "redirect-uri-allowlist-missing", severity: "medium",
