...
function parse(url, opts) {
  ...
  if (parsed.hostname && !guardDomain().aceLabelsCarryPayload(parsed.hostname)) {
    throw _makeError(errClass, "safe-url/malformed",
      "malformed URL: host carries an `xn--` label with no Punycode payload. " +
      "The URL parser accepts such a host on some Node versions and rejects it " +
      "on others, so the parser's verdict is not the framework's.");
  }
...
function format(url) {
  try {
    ...
    if (url instanceof URL) {
      ...
      u = new URL(url);   // allow:raw-new-url-parse-only — safeUrl.format wraps URL ctor for CVE-2026-21712; this IS the safe wrapper.
    } else {
      throw new SafeUrlError("safe-url/format-bad-input",
        "safeUrl.format: url must be a string or URL instance");
    }
    if (u.hostname && !guardDomain().aceLabelsCarryPayload(u.hostname)) {
      throw new SafeUrlError("safe-url/format-failed",
        "safeUrl.format refused: host carries an `xn--` label with no Punycode payload");
    }
    return u.href;
  } catch (e) {
