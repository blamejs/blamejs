...
// How far into an ASN.1 leaf to look for an SVG root element. A conformant
// SVG may put an XML declaration, a DOCTYPE and a comment ahead of `<svg`,
// which together run past 300 characters, so a magic-byte-sized window
// decides "not an SVG" on documents that plainly are one. Characters, not
// bytes: the leaf is decoded as UTF-8 before it is searched.
var SVG_PREFIX_SCAN_CHARS = 512;
...
function _scanForEmbeddedSvg(node, depthBudget) {
  ...
  if (!node.constructed) {
    ...
    // Decode before applying the window. Slicing the buffer first would count
    // bytes, and a preamble carrying non-ASCII text reaches a byte limit sooner
    // than its character count suggests — an XML comment of 300 accented
    // characters is 600 bytes, which would put a root element well inside a
    // 512-character window and outside a 512-byte one.
    var text = node.value.toString("utf8");
    // The `<?xml[\s\S]*<svg` alternative that used to stand beside this search
    // required `<svg` inside the same window the search already covers, so it
    // could never supply an answer the search had not; what it looked like it
    // was for — a root element behind a declaration — is the window's job.
    var at = text.indexOf("<svg");
    if (at !== -1 && at < SVG_PREFIX_SCAN_CHARS) return text;
    return null;
  }
