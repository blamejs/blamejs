...
// An XML namespace prefix: an NCName, so a letter or `_` first, then letters,
// digits, `.`, `-` and `_`, and no colon anywhere.
//
// XML's name grammar runs through most of Unicode, so each class below pairs
// the ASCII letters with a literal U+0080 to U+FFFF range: everything above
// ASCII is admitted rather than enumerated, and what is refused is the ASCII
// punctuation and the leading digit a malformed name uses. That range's lower
// bound does not render, so the pattern reads shorter than it is; the bytes are
// C2 80 and EF BF BF. Anchored at both ends with no nested quantifier, so it
// costs one pass over a short prefix.
...
function _svgRootFollowsPrologue(text) {
  ...
  if (parts.length === 2 && !NC_NAME_RE.test(parts[0])) return false;
