...
// compare: lowercase + strip trailing dot(s) + IDN A-label (punycode) via
// Node's UTS #46. Both sides of every comparison go through this so a U-label
// vs A-label, case, or trailing-dot difference does not decide authorization.
// Returns "" for an unparseable host (which then matches nothing).
function _canonBimiHost(host) {
  var s = String(host == null ? "" : host).toLowerCase().replace(/\.+$/, "");
