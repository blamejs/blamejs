...
function _certValidityReason(cert, now) {
  var notBefore = Date.parse(cert.validFrom);
  var notAfter  = Date.parse(cert.validTo);
  if (!isFinite(notBefore) || !isFinite(notAfter)) return "cert validity dates unparseable";
  if (now < notBefore) return "cert not-yet-valid (notBefore=" + cert.validFrom + ")";
  if (now > notAfter) return "cert expired (notAfter=" + cert.validTo + ")";
  return null;
}

function _verifyCertChain(leaf, intermediates, anchors) {
  var now = Date.now();
  var res = x509Chain.resolveChain(leaf, intermediates, anchors, {
    validAt: function (cert) { return _certValidityReason(cert, now) === null; },
    maxDepth: 8,
  });
  if (res.ok) return { ok: true };
  if (res.invalidCert) {
    return { ok: false, reason: _certValidityReason(res.invalidCert, now) };
  }
  if (res.reason === "pathlen") {
    return { ok: false, reason: "chain exceeds a CA basicConstraints pathLenConstraint" };
  }
  if (res.reason === "nameconstraint") {
    return { ok: false, reason: "chain violates a CA nameConstraints (RFC 5280 §4.2.1.10)" };
  }
  if (res.depthLimited) {
    return { ok: false, reason: "chain depth exceeded 8" };
  }
  if (leaf.checkIssued(leaf)) {
    return { ok: false, reason: "self-signed root not in trust-anchor bundle" };
  }
  return { ok: false, reason: "no issuer found for `" + leaf.subject + "` in chain or trust anchors" };
}
