...
function _scanForEmbeddedSvg(node, depthBudget) {
  ...
  if (!node.constructed) {
    ...
    var prefix = node.value.slice(0, Math.min(node.value.length, SVG_PREFIX_SCAN_CHARS)).toString("utf8");
    // The `<?xml[\s\S]*<svg` alternative that used to stand here required
    // `<svg` inside the same window this search already covers, so it could
    // never supply an answer the search had not; what it looked like it was
    // for — a root element behind a declaration — is handled by the window.
    if (prefix.indexOf("<svg") !== -1) {
