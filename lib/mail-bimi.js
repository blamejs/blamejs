...
function _svgRootFollowsPrologue(text) {
  ...
  // `<svgfoo` and `<svg.foo` are different elements, so the name has to END
  // here. Asked the other way round — "is the next character one that continues
  // a name?" — the answer is only as good as the character list, and XML's name
  // grammar runs well past the ASCII set any such list holds; `.` alone is
  // enough to let `<svg.foo` through. What may follow a FINISHED tag name is a
  // closed set: whitespace, the slash of an empty element, or the tag's end.
  var after = text.charAt(i + 4);
  return after === ">" || after === "/" ||
         markupTokenizer.isMarkupSpace(text.charCodeAt(i + 4));
}
