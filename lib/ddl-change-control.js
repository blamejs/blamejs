 ...
 *
 * SOX 404 ICFR + PCI DSS Req 6.5 / 10.7 require a documented change-
 * control process for any change that affects financial reporting or
 * cardholder-data systems - including database schema. Existing
 * b.audit emission on DDL only logs that a change happened; this
 * primitive enforces a multi-approver flow BEFORE the change applies:
 *
 *   1. propose(sql, opts) - captures the SQL with a SHA3-512 hash and
 *      optional signed payload. proposed audit fires.
 *   2. approve(changeId, approver, opts) - adds an approver signature
 *      to the change. approved audit fires.
 *   3. reject(changeId, reviewer, reason) - terminal rejection.
 *   4. applyApproved(changeId, runner) - runs the SQL via the operator-
 *      supplied runner ONLY when the change has minimum N approvals AND
 *      the window is open. applied audit fires (with sqlHash so the
 *      audit chain anchors the bytes that ran).
 *
 ...
 *   });
 *
 * Audit emissions (namespace "ddl"):
 *   ddl.change.proposed       { changeId, sqlHash, proposer, reason }
 *   ddl.change.approved       { changeId, approver, signaturesCount }
 *   ddl.change.rejected       { changeId, reviewer, reason }
 *   ddl.change.applied        { changeId, sqlHash, applier, durationMs }
 *   ddl.change.apply_refused  { changeId, reason } - too few approvers
 *                                                  OR window closed
 *                                                  OR sql tampered
 *
 * State is in-process by default. Operators wiring a durable backend
 * pass opts.store exposing { get(id), put(id, change), list() }.
 */
