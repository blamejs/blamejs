...
function create(opts) {
  ...
  async function _ensureCert(certManifest) {
    ...
    if (meta && certBuf && keyBuf && meta.expiresAt > Date.now() + minDaysBeforeExpiry * C.TIME.days(1)) {
    ...
    if (!match && names.length > 0) {
      ...
      for (var nj = 0; nj < names.length; nj += 1) {
        ...
        for (var sj = 0; sj < ctxJ.sniNames.length; sj += 1) {
          ...
          if (pattern.charAt(0) === "*" &&
              servername.endsWith(pattern.slice(1))) {
            match = ctxJ; break;
          }
        }
        if (match) break;
  ...
  async function refresh(name) {
    ...
    return _ensureCert(certsByName[name]);
  }
