...
function create(opts) {
  ...
  // Every window here is measured against this, so a caller that puts its other
  // budgets on a monotonic reading can put these on the same one. Without it a
  // clock step forward retired every recorded failure at once: an address at the
  // auth-failure cap was admitted again immediately, because the window is a
  // comparison against stored timestamps rather than a counter that decays.
  // The seam has to be inside the primitive — the timestamps are taken here at
  // write time and compared here at read time, so there is no point at which a
  // caller could substitute its own reading.
  validateOpts.optionalFunction(opts.clock,
    "b.mail.server.rateLimit.create: opts.clock",
    MailServerRateLimitError, "mail-server-rate-limit/bad-clock");
