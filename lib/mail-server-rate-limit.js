...
function create(opts) {
  ...
  //
  // Both listeners call it, so the policy lives here with the number rather
  // than in two copies that can drift. `bytes` and `elapsedMs` are what the
  // caller has already been tracking; nothing is stored here, so a connection
  // needs no teardown to release it.
 ...
 * `minBytesPerSecond`, `bodyRateStarved` — or resolve refuses at config time
...
var LIMITER_INTERFACE = Object.freeze([
  ...
  "minBytesPerSecond", "bodyRateStarved",
]);
