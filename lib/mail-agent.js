...
async function _expunge(ctx, args) {
  ...
  for (var i = 0; i < args.objectIds.length; i += 1) {
    ...
    if (floorMs > 0) {
      // An arrival time this cannot establish REFUSES the deletion. It read an
      // absent one as 0 — the epoch — which makes the message look 56 years
      // old and clears every retention floor there is, so the one input the
      // check could not verify was the one that always passed it. A retention
      // floor exists to prevent destruction, so the unknown case has to fall on
      // the preserving side.
      //
      // isNonNegativeSafeInt rather than a truthiness test: it refuses
      // undefined, null, NaN, Infinity, non-integers, strings and negatives,
      // and also the band above 2^53 where a BIGINT read out of a backend
      // collapses onto a shared Number and two different timestamps stop being
      // distinguishable.
      // `receivedAt` only. `internalDate` is chosen by whoever sent the
      // message -- RFC 9051 section 6.3.12 lets an APPEND carry a date-time,
      // and a backend that records it without stamping its own arrival time
      // produces a row whose only date came from the client. Reading that as
      // the arrival time hands the age, and so the delete, to the party the
      // retention floor exists to constrain: a row dated to the epoch clears
      // a floor of any size. It remains correct for display and for
      // INTERNALDATE responses; it is not an input to a destruction decision.
