...
function upgradeSocket(opts) {
  ...
  // RFC 8879 certificate compression, same as the HTTP listener. A mail
  // server's certificate is the same ML-DSA-87 chain and the same dominant
  // share of its handshake; the option is omitted rather than passed empty on
  // a runtime that does not implement the extension.
  var mailTlsOpts = {
    isServer:      true,
    secureContext: opts.secureContext,
  };
  var certCompression = C.TLS_CERT_COMPRESSION();
  if (certCompression.length > 0) mailTlsOpts.certificateCompression = certCompression;
  var tlsSocket = new nodeTls.TLSSocket(plainSocket, mailTlsOpts);
