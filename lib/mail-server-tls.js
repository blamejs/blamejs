...
function upgradeSocket(opts) {
  ...
  // The pre-upgrade plain-socket idle timer + its "timeout" listener wrote a
  // PLAINTEXT idle reply and closed the connection (every line-protocol server
  // arms socket.setTimeout + a "timeout" handler at connect). Left armed, it
  // fires on the wrapped socket AFTER the handshake and injects cleartext into
  // the now-encrypted stream — the peer sees a TLS decode error / reset instead
  // of the TLS-aware onTimeout replying encrypted. Strip the listener AND disarm
  // the plain timer; the TLS socket re-arms its own idle timer + a TLS-aware
  // onTimeout on "secure".
  plainSocket.removeAllListeners("timeout");
  if (typeof plainSocket.setTimeout === "function") {
    try { plainSocket.setTimeout(0); } catch (_e) { /* tolerate already-closed */ }
  }
