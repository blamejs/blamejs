 ...
 * `a2a/missing-field` / `a2a/bad-field` / `a2a/insecure-endpoint` when input is
...
function signCard(card, privateKeyPem, opts) {
  ...
  numericBounds.requirePositiveFiniteIntIfPresent(opts.ttlMs, "a2a.signCard: opts.ttlMs", errorClass, "a2a/bad-ttl");
...
function verifyCard(envelope, publicKeyPem, opts) {
  opts = opts || {};
  var errorClass = opts.errorClass || A2aError;
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxBytes, "a2a.verifyCard: opts.maxBytes", errorClass, "a2a/bad-max-bytes");
  numericBounds.requireNonNegativeFiniteIntIfPresent(opts.clockSkewMs, "a2a.verifyCard: opts.clockSkewMs", errorClass, "a2a/bad-skew");
