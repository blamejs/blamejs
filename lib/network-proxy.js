...
function _parseProxyUrl(value) {
  ...
  var u;
  try { u = new URL(value); } catch (_e) {
    throw new ProxyError("invalid proxy URL: " + value, "proxy/bad-url");
  }
  if (u.protocol !== "http:" && u.protocol !== "https:") {
    throw new ProxyError("proxy URL must be http:// or https://, got " + u.protocol, "proxy/bad-protocol");
  }
  return u;
}
...
function _matchCidr(cidr, host) {
  ...
  if (!isFinite(prefix) || prefix < 0 || prefix > 32) return false;
...
function shouldProxy(targetUrl) {
  var u = targetUrl instanceof URL ? targetUrl : new URL(String(targetUrl));
  var host = u.hostname;
  ...
  proxySocket.on(proxyUrl.protocol === "https:" ? "secureConnect" : "connect", function () {
    ...
    function onData(chunk) {
      ...
      if (!match || parseInt(match[1], 10) !== 200) {
        done(new ProxyError("proxy CONNECT to " + targetHost + ":" + targetPort + " failed: " + status,
          "proxy/connect-failed"));
        try { proxySocket.destroy(); } catch (_e) {}
        return;
      }
