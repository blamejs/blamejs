...
function _connectThroughTunnel(proxyUrl, targetHost, targetPort, callback) {
  ...
  function done(err, sock) {
    if (settled) return;
    settled = true;
    try { proxySocket.setTimeout(0); } catch (_e) { /* best-effort timer clear */ }
    callback(err, sock);
  }
  // Wall-clock bound on the proxy connect + CONNECT handshake: a proxy
  // that accepts the socket but never (or only trickles) replies must not
  // hang the tunnel forever. Destroy the socket and fail closed.
  proxySocket.setTimeout(TUNNEL_CONNECT_TIMEOUT_MS, function () {
    done(new ProxyError("proxy/connect-timeout",
      "proxy CONNECT to " + targetHost + ":" + targetPort + " timed out after " +
      TUNNEL_CONNECT_TIMEOUT_MS + "ms"));
