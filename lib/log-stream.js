 ...
 *     cloudwatch — PutLogEvents over HTTPS with SigV4; honors the
 ...
 * are dropped before redaction, so debug / info chatter on a
 * production deployment goes no further than the dispatcher.
 *
 * @opts
...
function init(opts) {
  ...
  if (opts.minLevel !== undefined && opts.minLevel !== null) {
    ...
    if (LEVELS.indexOf(topLevel) === -1) {
      throw _err("log-stream/invalid-level",
...
function emit(level, message, meta) {
  ...
  if (LEVELS.indexOf(level) === -1) {
    throw _err("log-stream/invalid-level", "log level must be one of " + LEVELS.join(", "), true);
  }
  var record = {
    ts:      Date.now(),
    level:   level,
...
function onIncoming(handler) {
  if (typeof handler !== "function") {
    throw _err("log-stream/invalid-handler", "onIncoming requires a function handler", true);
  }
 ...
 *   );
 *   // → one result per sink the command reached, each { ok, value }, and an
 *   //   empty array where no sink is registered under that name
 */
 ...
 *   // → one row per registered sink, each shaped
 ...
 *
 * Recognized variables: `BLAMEJS_LOG_STREAM_PROTOCOL` (`local` |
