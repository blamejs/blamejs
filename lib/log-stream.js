 ...
 * + protocol-specific options. v0.0.13 ships 'local' (append-only file
 * with rotation) and 'webhook' (generic HTTP POST — covers Splunk HEC,
 * Datadog, Sumo Logic, Loki, custom). Specialized adapters (syslog over
 * TLS, OpenTelemetry OTLP, AWS CloudWatch, Vector/Fluentd) follow.
 *
...
var DEFERRED_PROTOCOLS = {
  "syslog":     { since: "v0.0.14+", description: "RFC 5424 syslog over TLS" },
  "otlp":       { since: "v0.0.14+", description: "OpenTelemetry Logs OTLP/HTTP" },
  "cloudwatch": { since: "v0.0.14+", description: "AWS CloudWatch Logs (PutLogEvents)" },
};
...
function init(opts) {
  ...
  for (var name in opts.sinks) {
    ...
    if (DEFERRED_PROTOCOLS[cfg.protocol]) {
      ...
      throw _err("PROTOCOL_NOT_IMPLEMENTED",
        "log-stream protocol '" + cfg.protocol + "' is deferred to " + d.since +
        " (" + d.description + "). Use 'local' or 'webhook' for now.", true);
    }
    var proto = PROTOCOLS[cfg.protocol];
    if (!proto) throw _err("UNKNOWN_PROTOCOL", "unknown log-stream protocol: '" + cfg.protocol + "'", true);
    sinks[name] = {
      name:     name,
  ...
  Object.keys(sinks).forEach(function (name) {
    ...
    Promise.resolve()
      ...
      .catch(function (e) {
        ...
        } catch (_e) { /* audit unavailable — no-op */ }
      });
  });
}
