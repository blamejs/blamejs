...
function create(opts) {
  ...
  async function record(key, bytes) {
    ...
    try { await backend.account(key, bytes, nowMs); }
      ...
      // cache/unsupported at call time (the public update method exists on
      // every b.cache, so the get/set-only case can't be caught at
      // construction). Under the drop-silent policy below this would silently
      // disable the quota — surface it LOUD so the operator fixes the backend
      // rather than running an unenforced quota.
