...
function parse(headerValue) {
  ...
  var out = [];
  var members = structuredFields.splitTopLevel(headerValue, ",");
  for (var i = 0; i < members.length; i++) {
    ...
    var rel = [], params = Object.create(null);
    for (var p = 0; p < paramParts.length; p++) {
      ...
      if (name === "rel") { rel = value.split(/\s+/).filter(Boolean); continue; }
      // RFC 8288 §3.4: a repeated parameter keeps the FIRST occurrence.
