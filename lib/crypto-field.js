...
// allow:hand-rolled-sql — the canonical logical name of the per-row-key registry, which is the AAD-tuple table component and the frameworkSchema.tableName key
...
function sealRow(table, row, opts) {
  ...
  for (var i = 0; i < s.sealedFields.length; i++) {
    ...
    if (kRow) {
      ...
      // allow:seal-without-aad-by-design — plain-mode legacy table; the operator opts into AAD per table via registerTable({aad:true}), and a pinned posture floor refuses plain mode above
