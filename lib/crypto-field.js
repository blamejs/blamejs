 ...
 * are decrypted under the row's K_row: `dbHandle` (the db-query layer
...
function unsealRow(table, row, actor, dbHandle) {
  ...
  function _kRowOnce() {
    ...
    _kRowCache = null;
    if (!keyedTable || kRowId.length === 0 ||
        !dbHandle || typeof dbHandle.prepare !== "function") {
      return null;
    }
    var spec = perRowKeyTables[table];
    var wrap = dbHandle.prepare(
        ...
        if (typeof out[field] === "string" && isRowSealed(out[field])) {
          ...
          if (!kRow) {
            throw new CryptoFieldError("crypto-field/row-key-unavailable",
              ...
              "' is unavailable (shredded, never materialized, or no dbHandle)");
          }
          var cellAad = _aadBytes(_rowCellAad(s, table, field, kRowId));
