    ...
    if (out[field]) {
      ...
      try {
        ...
        if (typeof out[field] === "string" && isRowSealed(out[field])) {
          ...
          // An accepted unbound cell is recorded. The window exists so an
          // upgrade does not null a live column, and it is only useful if the
          // operator can see how many rows are still unbound -- otherwise
          // "migration finished" is a guess, and clearing the window on a guess
          // nulls whatever was left.
