...
function sealRow(table, row, opts) {
  ...
  for (var i = 0; i < s.sealedFields.length; i++) {
    ...
    // Skip null / undefined (nothing to seal) AND the empty string. The three
    // envelope branches disagree on "": vault.seal returns it unchanged (falsy
    // pass-through), the K_row branch encrypts an empty buffer, but
    // vault.aad.seal REFUSES an empty plaintext and throws vault-aad/bad-input
    // — so an aad-table write whose sealed column held "" crashed the insert
    // while the plain/K_row paths did not. An empty string carries no plaintext
    // to protect; skipping it (mirroring vault.seal's own falsy contract)
    // makes all three branches agree and round-trip "" back to "" on read
    // (unsealRow's `if (out[field])` leaves the empty cell untouched). The
    // guard is exact: only the literal empty string produces an empty plaintext
    // for vault.aad.seal (an empty Buffer / object / array encode to a
    // non-empty typed form), so nothing else is caught here.
