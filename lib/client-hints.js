...
function _scanControlBytes(s, headerName) {
  for (var i = 0; i < s.length; i += 1) {
    ...
    if (c === 9) continue;                                                                          // allow:raw-byte-literal — ASCII HT (folding whitespace)
    if (c < 32 || c === 127) {                                                                     // allow:raw-byte-literal — C0 + DEL codepoint range
      throw new ClientHintsError("client-hints/bad-header-value",
        "parse: " + headerName + " contains control characters");
    }
  }
}
...
function _parseSfBrandList(s) {
  ...
  if (t.length === 0) return [];
  var out = [];
  // Walk character-by-character respecting quoted-string boundaries
  // so a `, ` inside a brand name doesn't split the list.
  var depth = 0;
  var inQuote = false;
  var escape = false;
  var start = 0;
  for (var i = 0; i <= t.length; i += 1) {
    var c = i < t.length ? t.charAt(i) : ",";
    ...
    if (c === ")") { depth -= 1; continue; }
    if (c === "," && depth === 0) {
      var piece = t.slice(start, i).trim();
      if (piece.length > 0) {
...
function _parseBrandMember(piece) {
  ...
  // the parameter value. Use a quote-aware splitter that tracks
  // RFC 8941 §3.3.3 quoted-string boundaries with backslash-escape.
  var params = _splitTopLevelSemis(piece);
...
// Quote-aware top-level `;` splitter — same shape as
// _splitTopLevelCommas in cdn-cache-control.js. Tracks RFC 8941
// §3.3.3 quoted-string state with backslash-escape so a sf-string
// parameter value containing `;` doesn't split mid-value.
function _splitTopLevelSemis(s) {
  var out = [];
  var start = 0;
  var inQuote = false;
  var escape = false;
