 ...
 *   request headers carrying structured-fields data per RFC 9651:
 *
 ...
 *   factors strings — plus the raw RFC 9651 parsed shape for any
 ...
 *
 * @card
 *   RFC 9651 / W3C Client Hints — parse `Sec-CH-UA*` request headers
...
// RFC 9651 §3.3.3 sf-string — quoted-string with backslash-escape for
...
function _parseSfString(s) {
  ...
  if (t.charAt(0) === "\"") {
    ...
    // Single-pass RFC 9651 unescape — chained .replace() mis-decodes
...
// RFC 9651 §3.3.6 sf-boolean — `?1` (true) / `?0` (false). Returns
...
// RFC 9651 §3.1.1 sf-list — comma-separated members, each potentially
...
function _parseSfBrandList(s) {
  var t = s.trim();
  if (t.length === 0) return [];
  // Walk via the shared quote-aware top-level `,` splitter. RFC 9651
...
function _parseBrandMember(piece) {
  ...
  // RFC 9651 §4.1.1.4 — parameter values can be sf-string. A bare
