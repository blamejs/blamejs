...
// whether it uses CRLF or bare LF.
...
// RFC 2045 quoted-string body: a backslash escapes the character after it, and
// a trailing backslash with nothing after it stands for itself. Line
// terminators are ordinary characters here — a `.` in the pattern this
// replaced did not match one, so `\` before a newline was left in place.
...
// One left-to-right pass over the quoted-printable body: `=` followed by CRLF
// or LF is a soft line break and disappears; `=` followed by two hex digits is
// the byte they name; anything else stands. One pass matters here — the two
// pattern replaces this covers ran in sequence, so an `=` produced by the
// first was fed to the second.
var QP_HEX_RADIX = 16;
...
function _decodeQuotedPrintable(buf) {
  ...
  for (var i = 0; i < s.length; i += 1) {
    ...
    if (next === 0x0A) {                                               // soft break, bare LF
      out += s.slice(keepFrom, i);
      keepFrom = i + 2;
      i += 1;
      continue;
    }
    if (next === 0x0D && s.charCodeAt(i + 2) === 0x0A) {               // soft break, CRLF
      out += s.slice(keepFrom, i);
      keepFrom = i + 3;
      i += 2;
      continue;
    }
    if (_isHexDigit(next) && _isHexDigit(s.charCodeAt(i + 2))) {
      out += s.slice(keepFrom, i) +
  ...
  return Buffer.from(keepFrom === 0 ? s : out + s.slice(keepFrom), "binary");
}
...
function _filenameParamValue(cd) {
  var lower = String(cd).toLowerCase();
  var at = -1;
  var from = 0;
  while (from <= lower.length) {
    var hit = lower.indexOf("filename", from);
    if (hit === -1) return null;
    var after = hit + "filename".length;
    if (lower.charAt(after) === "*") after += 1;
    if (lower.charAt(after) === "=") { at = after + 1; break; }
    from = hit + 1;
  }
  if (at === -1) return null;
  var semi = cd.indexOf(";", at);
  var value = semi === -1 ? cd.slice(at) : cd.slice(at, semi);
