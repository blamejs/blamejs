...
function _displayEntry(part, path, index, parentSubtype) {
  ...
  return {
    ...
    kind:     _displayKindOf(cd, leafType, filename, index, parentSubtype),
  };
}

function _displayKindOf(cd, leafType, filename, index, parentSubtype) {
  ...
  var displayed = index === 0 || (parentSubtype !== "related" && !named);
...
function _selectInto(part, path, into, index, parentSubtype) {
...
function _parseHeaders(buf, ctx) {
  ...
  for (var i = 0; i < lines.length; i += 1) {
    ...
    var nameBad = _firstNonFtextOffset(name);
    if (nameBad !== -1) {
      throw new SafeMimeError("safe-mime/bad-header-name",
        "safeMime.parse: header name is not RFC 5322 3.6.8 ftext: byte 0x" +
        name.charCodeAt(nameBad).toString(16) + " at offset " + nameBad +
        " of " + name.length + " (name shown escaped: " + _previewBytes(name) + ")");
    }
