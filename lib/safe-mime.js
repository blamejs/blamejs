...
function _displayEntry(part, path, index, parentSubtype, siblingCount) {
  ...
  return {
    ...
    kind:     _displayKindOf(cd, leafType, filename, index, parentSubtype, siblingCount),
  };
}

function _displayKindOf(cd, leafType, filename, index, parentSubtype, siblingCount) {
  ...
  var alone = (siblingCount === undefined ? 1 : siblingCount) <= 1;
  var displayed;
  if (parentSubtype === "related") {
    displayed = index === 0;
  } else if (parentSubtype === "alternative") {
    displayed = index === 0 || !named;
  } else {
    displayed = !named || (index === 0 && alone);
  }
...
function _selectInto(part, path, into, index, parentSubtype, siblingCount) {
