...
function _expandUseGraph(graph, rendered) {
  ...
  function resolve(start) {
    ...
    while (stack.length > 0) {
      ...
      for (var i = 0; i < kids.length; i += 1) {
        ...
        if (kids[i].step === 1) participating += 1;
  ...
  return {
    ...
    depth: deepest,
    cyclic: cyclic,
    participating: participating + rendered.length,
  };
}
...
function _detectIssues(input, opts) {
  ...
  for (var i = 0; i < tokens.length; i += 1) {
    ...
    _recordUseReference(refGraph, openIds, defsBoundary, renderedRefs, defsDepth, tok);
    var ownId = _idOf(tok);
    if (ownId === null) {
    ...
    _recordFunctionalRefs(refGraph, openIds, defsBoundary, renderedRefs, defsDepth, tok, ownId);
    if (ownId !== null) {
