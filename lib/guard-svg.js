...
function _urlNormalize(s) {
  ...
  while (from < to && _isAsciiSpaceCode(stripped.charCodeAt(from))) from += 1;
  while (to > from && _isAsciiSpaceCode(stripped.charCodeAt(to - 1))) to -= 1;
  return stripped.slice(from, to);
}

function _isAsciiSpaceCode(c) {
...
function _useTargetsOf(tok) {
  ...
  for (var i = 0; i < attrs.length; i += 1) {
    ...
    if (v.charAt(0) === "#") out.push(_percentDecode(v.slice(1)));
  }
  return out;
}
...
function _recordUseReference(g, openIds, defsBoundary, rendered, defsDepth, tok) {
  if (!Object.prototype.hasOwnProperty.call(HREF_TEMPLATE_TAGS, tok.name)) return 0;
  var targets = _useTargetsOf(tok);
  if (targets.length === 0) return 0;
  var dormant = Object.prototype.hasOwnProperty.call(NON_RENDERING_CONTAINERS, tok.name);
  var owner = dormant ? null : _renderOwner(openIds, defsBoundary);
  var anonymous = _idOf(tok) === null;
  for (var i = 0; i < targets.length; i += 1) {
    if (owner !== null && anonymous) _addEdge(g.uses, owner, targets[i]);
    if (defsDepth === 0 && !dormant) rendered.push(targets[i]);
  }
...
function _styleRefsInto(out, value) {
  ...
  for (var d = 0; d < decls.length; d += 1) {
    ...
    _collectUrlRefs(out, parts.value);
  }
}

function _recordFunctionalRefs(g, openIds, defsBoundary, rendered, defsDepth, tok, ownId) {
  var attrs = tok.attrs || [];
  var refs = [];
  for (var i = 0; i < attrs.length; i += 1) {
    var attrName = attrs[i].name.toLowerCase();
    var takesRef = Object.prototype.hasOwnProperty.call(FUNC_IRI_ATTRS, attrName) ||
      Object.prototype.hasOwnProperty.call(ANIMATION_VALUE_ATTRS, attrName);
    if (!takesRef) continue;
    if (attrName === STYLE_ATTR) _styleRefsInto(refs, attrs[i].value);
    else _collectUrlRefs(refs, _refText(attrs[i].value));
  }
  var dormant = Object.prototype.hasOwnProperty.call(NON_RENDERING_CONTAINERS, tok.name);
  var owner = ownId !== null
    ? ownId
    : (dormant ? null : _renderOwner(openIds, defsBoundary));
  var renders = defsDepth === 0 && !dormant;
  for (var j = 0; j < refs.length; j += 1) {
    if (owner !== null) _addEdge(g.uses, owner, refs[j]);
    if (renders) rendered.push(refs[j]);
  }
  return refs.length;
}
...
function _expandUseGraph(graph, rendered) {
  ...
  function neighbours(id) {
    ...
    if (direct !== undefined) {
      for (var i = 0; i < direct.length; i += 1) out.push({ to: direct[i], step: 1 });
    }
    var inner = graph.contains.get(id);
    if (inner !== undefined) {
      for (var j = 0; j < inner.length; j += 1) out.push({ to: inner[j], step: 0 });
    }
  ...
  function resolve(start) {
    ...
    while (stack.length > 0) {
      ...
      var total = 0;
      var drawn = 1 + (graph.weight.get(top.id) || 0);
      var tallest = 0;
      for (var i = 0; i < kids.length; i += 1) {
        var to = kids[i].to;
        var sub = count.has(to) ? count.get(to) : MAX_USE_EXPANSION;
        total += kids[i].step === 0 ? sub : 1 + sub;
        drawn += elements.has(to) ? elements.get(to) : MAX_USE_EXPANSION;
        var h = height.has(to) ? height.get(to) : 0;
        if (h + kids[i].step > tallest) tallest = h + kids[i].step;
      }
      count.set(top.id, Math.min(total, MAX_USE_EXPANSION));
      elements.set(top.id, Math.min(drawn, MAX_USE_EXPANSION));
  ...
  for (var r = 0; r < rendered.length; r += 1) {
    var target = rendered[r];
    resolve(target);
    instances = Math.min(instances + 1 + (count.has(target) ? count.get(target) : 0),
                         MAX_USE_EXPANSION);
    drawnTotal = Math.min(drawnTotal + (elements.has(target) ? elements.get(target) : 0),
                          MAX_USE_EXPANSION);
    var d = 1 + (height.has(target) ? height.get(target) : 0);
    if (d > deepest) deepest = d;
  }
  return { instances: instances, elements: drawnTotal, depth: deepest, cyclic: cyclic };
}
...
function _detectIssues(input, opts) {
  ...
  var refGraph = { uses: new Map(), contains: new Map(), weight: new Map() };
  var openIds = [];
  var elemCarriedId = [];
  var renderedRefs = [];
  var defsDepth = 0;
  ...
  for (var i = 0; i < tokens.length; i += 1) {
    ...
    if (tok.type === "endTag") {
      if (tok.name === "use") {
        ...
        if (elemCarriedId.length > 0 && elemCarriedId.pop()) openIds.pop();
      }
      continue;
    }
    ...
    if (tok.name === "use") useCount += 1;
    var templated = _recordUseReference(
      refGraph, openIds, defsBoundary, renderedRefs, defsDepth, tok);
    if (tok.name !== "use") useCount += templated;
    var ownId = _idOf(tok);
    if (ownId === null) {
      var weightOwner = _renderOwner(openIds, defsBoundary);
      if (weightOwner !== null) {
        refGraph.weight.set(weightOwner, (refGraph.weight.get(weightOwner) || 0) + 1);
      }
    }
    useCount += _recordFunctionalRefs(
      refGraph, openIds, defsBoundary, renderedRefs, defsDepth, tok, ownId);
    if (ownId !== null) {
      var parent = Object.prototype.hasOwnProperty.call(
        NON_RENDERING_CONTAINERS, tok.name) ? null : _renderOwner(openIds, defsBoundary);
      if (parent !== null) _addEdge(refGraph.contains, parent, ownId);
      if (Object.prototype.hasOwnProperty.call(HREF_TEMPLATE_TAGS, tok.name)) {
        var selfTargets = _useTargetsOf(tok);
        for (var st = 0; st < selfTargets.length; st += 1) {
          _addEdge(refGraph.uses, ownId, selfTargets[st]);
        }
      }
    }
    if (!tok.selfClosing && tok.name !== "use") {
      var openBefore = openIds.length;
      elemCarriedId.push(ownId !== null);
      if (ownId !== null) openIds.push(ownId);
      if (Object.prototype.hasOwnProperty.call(NON_RENDERING_CONTAINERS, tok.name)) {
        defsDepth += 1;
  ...
  if (expansion.cyclic) {
    ...
    var budget = Math.min(MAX_USE_EXPANSION - 1,
      Math.max(USE_EXPANSION_FLOOR, useCount * USE_AMPLIFICATION_MAX));
