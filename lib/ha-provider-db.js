"use strict";
/**
 * Default HA provider — DB-row-based leader election.
 *
...
function create(config) {
  if (!config || !config.externalDbBackend) {
    throw _err("INVALID_CONFIG",
      "ha-provider-db requires { externalDbBackend: <name> }", true);
  }
  ...
  if (dialect !== "postgres" && dialect !== "sqlite") {
    throw _err("UNSUPPORTED_DIALECT",
      "ha-provider-db dialect must be 'postgres' or 'sqlite' (got: " + dialect + ")",
      true);
  }
