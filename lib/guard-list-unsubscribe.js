...
function _extractUris(raw, maxUris) {
  ...
  // bracket pairs via String.split on `,` then per-item trim +
  // bracket-strip; safer than a regex with side-effecting lastIndex
  // state.
  var items = raw.split(",");
  var uris = [];
  for (var i = 0; i < items.length; i += 1) {
    ...
    if (trimmed.charAt(0) !== "<" || trimmed.charAt(trimmed.length - 1) !== ">") {
      // RFC 2369 violation — non-bracketed item. Fall through to
      // empty list; caller's "no URIs" branch surfaces the refuse
      // verdict with the RFC §3.1 message.
      continue;
    }
    uris.push(trimmed.slice(1, -1).trim());
