...
// Short field names, matched by OCCURRENCE with a per-name list of the
// continuations that make the occurrence part of an innocent word.
//
// Each of these is short enough to appear inside ordinary words, and a plain
// substring pass scrubs fields that are not credentials and are exactly what an
// operator reads to find out what happened. Measured against the field names
// this framework emits: `auth` took 93 of them, including `authenticated`,
// `authservId` and `author` -- the RFC 5322 word for the `From` party; `sid`
// took 32, including `residency`, `sidecar` and `residentKey`; `pin` took 29,
// including `stepIndex` and `mapping`; and `ssn` took `className`.
//
// Every narrower rule tried first lost real credentials, which is why this one
// is written the way it is. Exact matching drops `authCode` and `preAuthCode`.
// Splitting on camel-case boundaries drops `authcode` from a caller that
// lowercases its keys. Requiring a name boundary drops `shippingpincode`, where
// the trigram has letters on both sides. What survives all three is: look at
// every occurrence, and clear it only when what FOLLOWS it continues a known
// innocent word.

// Segments alone are not enough, because a caller that lowercases its JSON keys
// hands over `authcode`, `ssnnumber` and `customercvv` as ONE segment -- and
// those are credentials. So each short name is looked for as an OCCURRENCE too,
// with two knobs decided per entry from what the framework's own field names
// actually contain rather than from imagination:
//
//   ordinary  what may follow the occurrence and make it an innocent word
//   anywhere  whether an occurrence mid-word counts, or only one at a boundary
//
// `auth` is distinctive enough to count anywhere, which is what catches
// `preauthcode`; its innocent continuations are a closed list of three English
// stems. Enumerating the NOUNS that make it a secret instead (code, header,
// response, key, blob, ...) is open-ended and fails silently on the one nobody
// listed, while the stems are finite.
//
// `pin` and `sid` are three letters and sit inside ordinary words constantly --
// measured here: `pin` appears in `mapping`, `grouping`, `stamping`,
// `stepIndex`, `sweepIntervalMs`; `sid` in `residency`, `sidecar`, `side`,
// `residentKey`, `isIdentifierChar`. They count only at a name boundary, plus
// the continuations that clear what still collides there (`ping`/`pinned`, and
// `side`/`sidecar`).
// The `auth` exemption is ANCHORED AT BOTH ENDS, so it clears a name that IS an
// ordinary word and not one that merely starts with one. `authentication` is
// readable; `authenticationCode` is a credential, and a prefix-shaped exemption
// cleared both. The two are lexically identical up to the noun, so no
// structural rule separates them -- which decides the direction: an unlisted
// compound redacts. A new readable name that lands here loses legibility until
// it is added; a new secret does not leak.
var AUTH_ORDINARY_REMAINDER_RE =
  /(?:entic|enticate|enticated|enticatedas|enticates|enticating|entication|enticationresults|enticator|enticators|enticity|or|ors|ored|oring|ority|orities|oritative|serv|servid)$/y;

// EVERY occurrence counts, and what clears one is what FOLLOWS it. A boundary
// rule is not enough: a caller that lowercases its keys sends
// `shippingpincode`, where `pin` has letters on both sides and is still a
// credential.
//
// Each `ordinary` pattern is the set of continuations that make the occurrence
// part of an innocent word, measured from the field names lib/ actually emits
// rather than imagined. `auth` is anchored at both ends because `authentication`
// is readable while `authenticationCode` is a secret, and the two are identical
// up to the noun; the rest are prefixes, since their innocent words are
// distinguished by the letters immediately after the trigram.
//
//   shippingpincode  pin@4 -> "gpincode" (innocent, `ping`)
//                    pin@8 -> "code"     -> SECRET
//   mapping          pin@2 -> "g"        (innocent)
//   stepindex        pin@3 -> "dex"      (innocent)
//   residency        sid@3 -> "ency"     (innocent)
//   sidecar          sid@0 -> "ecar"     (innocent)
//   classname        ssn@3 -> "ame"      (innocent)
//   customerssnnumber ssn@8 -> "number"  -> SECRET
//
// An unlisted innocent word is over-redacted, which costs legibility; an
// unlisted credential noun is redacted, which is the whole point. Both failure
// modes fall on the safe side.
// The patterns are STICKY rather than start-anchored, so the remainder is
// tested where it begins instead of being cut out of the name first. Slicing
// allocated a copy per occurrence, which is what a length cap was there to
// bound -- and a length cap on this scan is a hole rather than a bound, because
// these six names are matched here and nowhere else: skipping a long key stops
// looking for them at all, and the key is the part of a logged object a caller
// can make any length it likes. Sticky costs nothing and needs no cap.
var SHORT_SENSITIVE_FIELDS = [
  { name: "auth", ordinary: AUTH_ORDINARY_REMAINDER_RE },
  // A one- or two-letter continuation has to be the WHOLE remainder. As a
  // prefix it exempts almost anything: `s` clears `pins`, and it clears
  // `pinSalt` and `pinSecret` with it. Only the continuations long enough to
  // identify the word they belong to are matched as prefixes.
  //
  // ping/mappings/pinned/pinning/pinset/stepIndex/sweepInterval/
  // skipIntegrity/installed/spkiPinVerifier/pinIp
  //
  // `pins` is NOT exempt. It reads as the plural of a certificate pin and as
  // the plural of a credential, `pins` and `userPins` both being field names an
  // application writes, and only one of those two readings can be served. A
  // pin list scrubbed from a log costs legibility; a PIN printed into one does
  // not come back. `spkiPinVerifier` and the rest keep their own continuations.
  { name: "pin",  ordinary: /(?:(?:g(?:fn|ms|s)?|ip|set)$|ned|ning|dex|terval|tegrit|stalle|verifi)/y },
  // side/sides/sidecar/residency/resident/identifier/residual/residualRisks.
  // `ent` is spelled out as `entifier` and `entkey` because `entropy` starts
  // with it and is not an ordinary word here.
  { name: "sid",  ordinary: /(?:(?:e|es)$|ecar|ency|entifier|entkey|ual)/y },
  // className/classNames
  { name: "ssn",  ordinary: /ames?$/y },
  { name: "cvc",  ordinary: null },
  { name: "cvv",  ordinary: null },
];

function _hasShortSensitiveOccurrence(lk) {
  for (var e = 0; e < SHORT_SENSITIVE_FIELDS.length; e += 1) {
    var entry = SHORT_SENSITIVE_FIELDS[e];
    var i = lk.indexOf(entry.name);
    while (i !== -1) {
      if (!entry.ordinary) return true;
      // Set on every use: a sticky pattern carries `lastIndex` between calls.
      entry.ordinary.lastIndex = i + entry.name.length;
      if (!entry.ordinary.test(lk)) return true;                                                        // allow:regex-no-length-cap — every pattern is a fixed alternation with no quantifier, matched at one sticky offset, so each test is O(1) in the name length and the scan is linear overall
      i = lk.indexOf(entry.name, i + 1);
    }
  }
  return false;
}
...
var SENSITIVE_FIELDS = [
  ...
  "authorization", "creditcard", "credit_card",
  "card_number", "cardnumber",
  "privatekey", "private_key", "passphrase", "session",
  "_authtoken", "auth_token", "authtoken", "auth-token", "bearer", "cookie",
  "pincode", "pin_code", "pin-code", "sessionpin", "session_pin",
...
function _isSensitiveFieldName(key) {
  ...
  // Segment entries are checked here and NOT in the substring pass below: they
  // are short enough to sit inside ordinary words, so a substring pass over
  // them scrubs the fields an operator reads to find out what happened, while
  // a whole-name pass would miss `authCode` and `preAuthCode`.
  if (_hasShortSensitiveOccurrence(lk)) return true;
