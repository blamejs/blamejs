 ...
 *   errors? }` (operator-friendly at HTTP boundaries). A value nested
 *   more than 256 levels is refused with the issue code
 *   `safe-schema/value-too-deep`, which `parse` throws and `safeParse`
 *   reports.
 *
...
function isUuid(v) {
  ...
  if (v.charAt(14) < "1" || v.charAt(14) > "5") return false;
  return "89abAB".indexOf(v.charAt(19)) !== -1;
}
...
function _rootPath() {
  return { parent: null, key: null, graft: null, length: 0, children: null };
}

function _child(path, key) {
  if (path.children === null) path.children = new Map();
  var node = path.children.get(key);
  if (node === undefined) {
    node = { parent: path, key: key, graft: null, length: path.length + 1, children: null };
    path.children.set(key, node);
  }
  return node;
}

function _grafted(base, inner, skip) {
  return { parent: null, key: null, graft: { base: base, inner: inner, skip: skip },
    length: base.length + inner.length - skip, children: null };
}

function _lastKeys(path, count) {
  var keys = [];
  var node = path;
  while (count > 0 && node.graft === null) {
    keys.push(node.key);
    node = node.parent;
    count -= 1;
  }
  ...
  var graft = node.graft;
  var innerCount = graft.inner.length - graft.skip;
  var prefix = count <= innerCount
    ? _lastKeys(graft.inner, count)
    : _lastKeys(graft.base, count - innerCount).concat(_lastKeys(graft.inner, innerCount));
  return prefix.concat(keys);
}

function _issue(path, code, message) {
  return { at: path, code: code, message: message };
}

function _fail(path, code, message) {
  return { ok: false, issues: [_issue(path, code, message)] };
}

function _publicIssues(issues) {
  return issues.map(function (issue) {
    return { path: _lastKeys(issue.at, issue.at.length), code: issue.code, message: issue.message };
  });
}
...
var UNION_MAX_ISSUES = 100;
var MAX_VALUE_DEPTH = 512;
var RUN_LIMIT_FLOOR = 100000;
var RUN_LIMIT_PER_VALUE = 1000;
var NEGATIVE_ZERO_KEY = { negativeZero: true };
var _runMemo = null;
var _parseBudget = null;
var _unionNesting = 0;
var _probing = 0;
var _operatorCalls = 0;
var _lazyResolutions = 0;
var UNRESOLVED_LAZY = { unresolvedLazy: true };

function _withRunMemo(fn) {
  var outermost = _runMemo === null;
  if (outermost) _runMemo = new Map();
  try { return fn(); }
  finally { if (outermost) _runMemo = null; }
}

function _callOperator(fn, arg) {
  _operatorCalls += 1;
  return fn(arg);
}

function _runsOperatorCode(node) {
  return node._callbacks === true ||
    (node._hasDefault === true && typeof node._default === "function") ||
    (node._hasCatch === true && typeof node._catch === "function");
}

var _callbackFreeCache = new WeakMap();

function _callbackFree(root) {
  var known = _callbackFreeCache.get(root);
  if (known === true || known === false) return known;
  if (known && known.lazyResolutions === _lazyResolutions) return null;
  var visited = new Set();
  var stack = [root];
  ...
  while (stack.length > 0) {
    ...
    visited.add(node);
    if (_runsOperatorCode(node)) {
      _callbackFreeCache.set(root, false);
      return false;
    }
    if (typeof node._children !== "function") continue;
    var children = node._children();
    if (children === UNRESOLVED_LAZY) { unresolved = true; continue; }
    for (var i = 0; i < children.length; i += 1) stack.push(children[i]);
  }
  if (unresolved) {
    _callbackFreeCache.set(root, { lazyResolutions: _lazyResolutions });
    return null;
  }
  visited.forEach(function (node) { _callbackFreeCache.set(node, true); });
  return true;
}

function _withParse(input, fn) {
  var previousMemo = _runMemo;
  var previousBudget = _parseBudget;
  var previousNesting = _unionNesting;
  var previousProbing = _probing;
  _runMemo = null;
  _parseBudget = { runs: 0, limit: RUN_LIMIT_FLOOR, size: null, root: input };
  _unionNesting = 0;
  _probing = 0;
  try { return fn(); }
  finally {
    _runMemo = previousMemo;
    _parseBudget = previousBudget;
    _unionNesting = previousNesting;
    _probing = previousProbing;
  }
}

function _valueSize(root) {
  var seen = new Set();
  var count = 0;
  var stack = [root];
  while (stack.length > 0) {
    ...
    seen.add(v);
    var children = Array.isArray(v) ? v : Object.keys(v).map(function (k) { return v[k]; });
    for (var i = 0; i < children.length; i += 1) stack.push(children[i]);
  }
  return count;
}

function _countRun(count) {
  if (_parseBudget === null) return;
  _parseBudget.runs += count === undefined ? 1 : count;
  if (_parseBudget.runs <= _parseBudget.limit) return;
  if (_parseBudget.size === null) {
    _parseBudget.size = _valueSize(_parseBudget.root);
    _parseBudget.limit = Math.max(RUN_LIMIT_FLOOR, RUN_LIMIT_PER_VALUE * _parseBudget.size);
    if (_parseBudget.runs <= _parseBudget.limit) return;
  }
  var message = "validation ran more than " + _parseBudget.limit + " schema checks for a value of " +
    _parseBudget.size + " nodes; the value reaches the same schema through more paths than its size accounts for";
  var err = new SafeSchemaError("safe-schema/evaluation-limit", message);
  err.issues = [{ path: [], code: "safe-schema/evaluation-limit", message: message }];
  throw err;
}
...
function _runGuardingStack(run) {
  try {
    return run();
  } catch (e) {
    if (!(e instanceof RangeError) || !_isStackExhaustion(e)) throw e;
    var message = "value nests deeper than validation can walk";
    var err = new SafeSchemaError("safe-schema/value-too-deep", message);
    err.issues = [{ path: [], code: "safe-schema/value-too-deep", message: message }];
    throw err;
  }
}

function _refuseDepth(path) {
  var message = "value nests deeper than " + MAX_VALUE_DEPTH +
    " levels, which is more than validation can walk";
  var err = new SafeSchemaError("safe-schema/value-too-deep", message);
  err.issues = [{ path: _lastKeys(path, Math.min(path.length, 8)),
    code: "safe-schema/value-too-deep", message: message }];
  throw err;
}

function _memoizedRun(schema, value, path) {
  var byValue = boundedMap.getOrInsert(_runMemo, schema._run, function () { return new Map(); });
  var key = (value === 0 && 1 / value < 0) ? NEGATIVE_ZERO_KEY : value;
  var mode = (_probing > 0 ? 1 : 0) + (_unionNesting > 0 ? 2 : 0);
  var results = byValue.get(key);
  var known = results ? results[mode] : null;
  if (known && known.operatorCalls === _operatorCalls && (!known.ok || known.at === path)) {
    if (known.ok) {
      _countRun();
      return { ok: true, value: known.value };
    }
    _countRun(Math.max(1, known.issues.length));
    return { ok: false, issues: known.issues.map(function (issue) {
      return _issue(_grafted(path, issue.at, known.depth), issue.code, issue.message);
    }) };
  }
  _countRun();
  var callsBefore = _operatorCalls;
  var r = schema._run(value, path);
  if (_operatorCalls === callsBefore) {
    if (!results) {
      results = [null, null, null, null];
      byValue.set(key, results);
    }
    results[mode] = r.ok
      ? { ok: true, at: path, value: r.value, operatorCalls: callsBefore }
      : { ok: false, depth: path.length, issues: r.issues.slice(), operatorCalls: callsBefore };
  }
  return r;
}


function _runWithModifiers(schema, value, path, memoize) {
  if (path.length > MAX_VALUE_DEPTH) _refuseDepth(path);
  if (value === undefined) {
    if (schema._hasDefault) value = _modifierValue(schema._default);
  ...
  if (value === null) {
    ...
    if (schema._hasCatch)   return { ok: true, value: _modifierValue(schema._catch) };
    return _fail(path, "type", "must not be null");
  }
  var r;
  if (memoize === true && _runMemo) {
    r = _memoizedRun(schema, value, path);
  } else {
    _countRun();
    r = schema._run(value, path);
  }
  if (!r.ok && schema._hasCatch) {
    return { ok: true, value: _modifierValue(schema._catch) };
  }
  return r;
}

function _modifierValue(v) {
  return typeof v === "function" ? _callOperator(v) : v;
}
...
function _baseSchema(spec) {
  var s = {
    ...
    _pure:        spec.pure === true,
    _children:    spec.children,
    _callbacks:   spec.callbacks === true,
    _run:         spec.run,
  };

  s.parse = function (input) {
    var r = _runGuardingStack(function () {
      return _withParse(input, function () { return _runWithModifiers(s, input, _rootPath()); });
    });
    if (r.ok) return r.value;
    var issues = _publicIssues(r.issues);
    var err = new SafeSchemaError(
      "safe-schema/invalid",
      "validation failed: " + _formatIssues(issues)
    );
    err.issues = issues;
    throw err;
  };
  ...
  s.safeParse = function (input) {
    var r;
    try {
      r = _runGuardingStack(function () {
        return _withParse(input, function () { return _runWithModifiers(s, input, _rootPath()); });
      });
    } catch (e) {
      if (e instanceof SafeSchemaError &&
          (e.code === "safe-schema/evaluation-limit" || e.code === "safe-schema/value-too-deep")) {
        return { ok: false, errors: e.issues };
      }
      throw e;
    }
    if (r.ok) return { ok: true, value: r.value };
    return { ok: false, errors: _publicIssues(r.issues) };
  };
  ...
  s.default = function (v) {
    return _baseSchema(_extendSpec(spec, { isOptional: true, hasDefault: true, defaultValue: v,
      pure: typeof v === "function" ? false : spec.pure }));
  };
  s.catch = function (v) {
  ...
  s.refine = function (predicate, opts) {
    ...
    return _baseSchema(_extendSpec(spec, {
      ...
      run: function (value, path) {
        ...
        if (!r.ok) return r;
        try {
          if (!_callOperator(predicate, r.value)) return _fail(path, code, message);
        } catch (e) {
  ...
  s.transform = function (fn) {
    ...
    return _baseSchema(_extendSpec(spec, {
      ...
      run: function (value, path) {
        ...
        if (!r.ok) return r;
        try {
          return { ok: true, value: _callOperator(fn, r.value) };
        } catch (e) {
  ...
  s.pipe = function (next) {
    ...
    return _baseSchema(_extendSpec(spec, {
      pure: false,
      children: function () { return [inner, next]; },
      run: function (value, path) {
        var r = inner._run(value, path);
        if (!r.ok) return r;
...
function _extendSpec(spec, overrides) {
  return {
    ...
    pure:         overrides.pure         != null ? overrides.pure         : spec.pure,
    children:     overrides.children     != null ? overrides.children     : spec.children,
    callbacks:    overrides.callbacks    != null ? overrides.callbacks    : spec.callbacks,
 ...
 * long string.
 *
 * `.regex(re, msg?)` runs `re` against every value the schema validates.
 * When the schema is built it throws `SafeSchemaError` with
 * `safe-schema/bad-regex` when `re` is not a RegExp,
 * `safe-schema/unsafe-pattern` when `b.guardRegex.assertSafe` refuses it,
 * and `safe-schema/stateful-pattern` when it carries the `g` or `y` flag.
 *
 * @example
 *   var b = require("blamejs");
 *   var s = b.safeSchema;
...
function _stringMethods(schema, spec) {
  ...
  schema.regex = function (re, msg) {
    if (!(re instanceof RegExp)) {
      throw new SafeSchemaError("safe-schema/bad-regex",
        "string().regex(): pattern must be a RegExp; got " + typeof re);
    }
    guardRegex.assertSafe(re, "string().regex()", SafeSchemaError, "safe-schema/unsafe-pattern");
    guardRegex.assertStateless(re, "string().regex()", SafeSchemaError, "safe-schema/stateful-pattern");
    return chain(function (v, p) {
...
function _objectWithMode(shape, keys, mode) {
  var spec = {
    kind: "object",
    children: function () { return keys.map(function (k) { return shape[k]; }); },
    run: function (value, path) {
      if (typeof value !== "object" || Array.isArray(value)) {
        return _fail(path, "type", "must be an object");
      }
      var fast = _probing > 0 && _callbackFree(schema) === true;
      var prechecked = null;
      function extraKeyIssue(ik) {
        if (pick.isPoisonedKey(ik)) {
          return _issue(_child(path, ik), "object/poisoned-key",
            "key '" + ik + "' is forbidden (prototype-pollution defense)");
        }
        if (mode === "passthrough") return null;
        return _issue(_child(path, ik), "object/unknown-key",
          "unknown key '" + ik + "' (use .passthrough() to allow extra keys)");
      }
      if (fast) {
        prechecked = {};
        var earlyKeys = Object.keys(value);
        for (var pi = 0; pi < earlyKeys.length; pi++) {
          if (Object.prototype.hasOwnProperty.call(shape, earlyKeys[pi])) continue;
          var extra = extraKeyIssue(earlyKeys[pi]);
          if (extra) return { ok: false, issues: [extra] };
        }
        for (var pk = 0; pk < keys.length; pk++) {
          if (shape[keys[pk]]._pure !== true) continue;
          var pr = _runWithModifiers(shape[keys[pk]], value[keys[pk]], _child(path, keys[pk]));
          if (!pr.ok) return pr;
          prechecked[pk] = pr;
        }
      }
      ...
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        var sub = shape[key];
        var childPath = _child(path, key);
        var r = (prechecked && prechecked[i]) ? prechecked[i] : _runWithModifiers(sub, value[key], childPath);
        if (!r.ok) {
          if (fast) return r;
      ...
      for (var ii = 0; ii < inputKeys.length; ii++) {
        ...
        var issue = extraKeyIssue(ik);
        if (issue) issues.push(issue);
        else out[ik] = value[ik];
      }
...
function array(itemSchema) {
  ...
  var spec = {
    kind: "array",
    children: function () { return [itemSchema]; },
    run: function (value, path) {
      if (!Array.isArray(value)) return _fail(path, "type", "must be an array");
      var fast = _probing > 0 && _callbackFree(itemSchema) === true;
      var issues = [];
      var out = [];
      for (var i = 0; i < value.length; i++) {
        var childPath = _child(path, i);
...
function _tupleWithRest(items, restSchema) {
  var schema = _baseSchema({
    kind: "tuple",
    children: function () { return restSchema ? items.concat([restSchema]) : items.slice(); },
    run: function (value, path) {
      ...
      var out = [];
      for (var i = 0; i < items.length; i++) {
        var r = _runWithModifiers(items[i], value[i], _child(path, i));
        if (!r.ok) {
          if (fast) return r;
          for (var j = 0; j < r.issues.length; j++) issues.push(r.issues[j]);
          continue;
      ...
      if (restSchema !== null) {
        for (var k = items.length; k < value.length; k++) {
          var rr = _runWithModifiers(restSchema, value[k], _child(path, k));
          if (!rr.ok) {
            if (fast) return rr;
 ...
 * the failure carries a parent-level `union` issue summarizing the
 * miss, followed by the first 100 issues the options reported and,
 * when there were more, one `union/issues-omitted` issue giving the
 * count left out. A union evaluated inside another union's option checks
 * each of its options' literal, enum, string, number and boolean keys
 * first, stops an option at its first failure, and reports that failure,
 * for every object, array, tuple and record whose schema holds no `refine`,
 * `transform` or `preprocess` callback, no `default` or `catch` factory, and
 * no `lazy` that has not been resolved yet; the others are validated field by
 * field in order, as at the top level.
 * While a union or a lazy target is being validated, a schema that has
 * already failed for a value is not run again for that value: the earlier
 * failure is reported at the new position, and each of its issues counts
 * toward the evaluation limit. A schema that has already accepted a value at
 * the same position in the input returns that result again. A result is
 * reused only when no `refine`, `transform` or `preprocess` callback, `lazy`
 * getter, or `default` or `catch` factory ran while it was computed or has
 * run since. A recursive schema whose options descend into the same child
 * costs the size of the value, whether or not its `lazy` references have
 * been resolved by an earlier parse, and a `transform` or `default` factory
 * still runs for every value it produces. For tagged-variant shapes prefer
...
function union(options) {
  ...
  return _baseSchema({
    kind: "union",
    children: function () { return options.slice(); },
    run: function (value, path) {
      return _withRunMemo(function () { return _runUnionOptions(options, value, path); });
    },
  });
}

function _runUnionOptions(options, value, path) {
  var collected = [];
  var omitted = 0;
  var probeOptions = _unionNesting > 0;
  _unionNesting += 1;
  try {
    for (var i = 0; i < options.length; i++) {
      if (probeOptions) _probing += 1;
      var r;
      try { r = _runWithModifiers(options[i], value, path, true); }
      finally { if (probeOptions) _probing -= 1; }
      if (r.ok) return r;
      for (var j = 0; j < r.issues.length; j++) {
        if (collected.length < UNION_MAX_ISSUES) collected.push(r.issues[j]);
        else omitted += 1;
      }
    }
  } finally {
  ...
  if (omitted > 0) {
    summary.issues.push(_issue(path, "union/issues-omitted",
      omitted + " further issues from the options were omitted"));
  }
  return summary;
}
...
function record(a, b) {
  ...
  return _baseSchema({
    kind: "record",
    children: function () { return keySchema ? [keySchema, valueSchema] : [valueSchema]; },
    run: function (value, path) {
      if (typeof value !== "object" || value === null || Array.isArray(value)) {
        return _fail(path, "type", "must be a plain object (record)");
      }
      var fast = _probing > 0 && _callbackFree(valueSchema) === true &&
        (!keySchema || _callbackFree(keySchema) === true);
      var issues = [];
      var out = {};
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        if (pick.isPoisonedKey(k)) {
          var poisoned = _issue(_child(path, k), "record/poisoned-key",
            "key '" + k + "' is forbidden (prototype-pollution defense)");
          if (fast) return { ok: false, issues: [poisoned] };
          issues.push(poisoned);
          continue;
        }
        if (keySchema) {
          var kr = _runWithModifiers(keySchema, k, _child(path, k));
          if (!kr.ok) {
            if (fast) return kr;
        ...
        var r = _runWithModifiers(valueSchema, value[k], _child(path, k));
        if (!r.ok) {
          if (fast) return r;
          for (var j = 0; j < r.issues.length; j++) issues.push(r.issues[j]);
          continue;
  ...
  return _baseSchema({
    ...
    run: function (value, path) {
      ...
      for (var i = 0; i < options.length; i++) {
        ...
        var dr = d._run(disc, _child(path, discriminator));
        if (dr.ok) {
          return _runWithModifiers(opt, value, path);
        }
      }
      return _fail(_child(path, discriminator), "discriminated-union/no-match",
...
function preprocess(fn, inner) {
  ...
  return _baseSchema({
    ...
    run: function (value, path) {
      var preprocessed;
      try {
        preprocessed = _callOperator(fn, value);
      } catch (e) {
...
function lazy(getter) {
  ...
  function resolve() {
    if (!cached) {
      var got = _callOperator(getter);
      if (!got || typeof got._run !== "function") return null;
      cached = got;
      _lazyResolutions += 1;
    }
    return cached;
  ...
  return _baseSchema({
    kind: "lazy",
    children: function () { return cached ? [cached] : UNRESOLVED_LAZY; },
    run: function (value, path) {
      var target = resolve();
      if (target === null) {
      ...
      return _withRunMemo(function () { return _runWithModifiers(target, value, path, true); });
    },
  });
}
