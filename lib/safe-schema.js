...
// Maximum URL length per RFC 9112 §3 guidance — also reused as the
...
function _stringMethods(schema, spec) {
  ...
  schema.url = function () {
    return chain(function (v, p) {
      // RFC 9110 doesn't set a hard URL length, but RFC 9112 §3
      ...
      if (v.length > URL_MAX_LEN) return _fail(p, "string/url-too-long",
        "must be a valid URL (max " + URL_MAX_LEN + " chars per RFC 9112 §3 guidance)");
