...
function _callbackFree(root) {
  ...
  if (unresolved) {
    root._callbackFree = { lazyResolutions: _lazyResolutions };
    return null;
  }
  visited.forEach(function (node) { node._callbackFree = true; });
  return true;
}
...
function _memoizedRun(schema, value, path) {
  ...
  var failures = byValue.get(key);
  var failure = failures ? failures[mode] : null;
  if (failure && failure.operatorCalls === _operatorCalls) {
    _countRun(Math.max(1, failure.issues.length));
    return { ok: false, issues: failure.issues.map(function (issue) {
      return Object.assign({}, issue, { path: path.concat(issue.path.slice(failure.depth)) });
    }) };
  }
  _countRun();
  var callsBefore = _operatorCalls;
  var r = schema._run(value, path);
  if (!r.ok && _operatorCalls === callsBefore) {
    if (!failures) {
      failures = [null, null, null, null];
      byValue.set(key, failures);
    }
    failures[mode] = { depth: path.length, issues: r.issues.slice(), operatorCalls: callsBefore };
  }
  return r;
}
...
function _baseSchema(spec) {
  ...
  s.parse = function (input) {
    var r = _withParse(input, function () { return _runWithModifiers(s, input, []); });
  ...
  s.safeParse = function (input) {
    var r;
    try {
      r = _withParse(input, function () { return _runWithModifiers(s, input, []); });
    } catch (e) {
    ...
    run: function (value, path) {
      ...
      function extraKeyIssue(ik) {
        if (pick.isPoisonedKey(ik)) {
          return { path: path.concat([ik]), code: "object/poisoned-key",
            message: "key '" + ik + "' is forbidden (prototype-pollution defense)" };
        }
        if (mode === "passthrough") return null;
        return { path: path.concat([ik]), code: "object/unknown-key",
      ...
      if (fast) {
        ...
        for (var pk = 0; pk < keys.length; pk++) {
          ...
          var pr = _runWithModifiers(shape[keys[pk]], value[keys[pk]], path.concat([keys[pk]]));
          if (!pr.ok) return pr;
          prechecked[pk] = pr;
        }
      }
 ...
 * toward the evaluation limit. A failure is reused only when no `refine`,
 * `transform` or `preprocess` callback, `lazy` getter, or `default` or
 * `catch` factory ran while it was computed or has run since. A recursive
 * schema whose options descend into the same child costs the size of the
 * value, and a `transform` or `default` factory still runs for every value
 * it produces. For tagged-variant shapes prefer
...
function _runUnionOptions(options, value, path) {
  ...
  if (omitted > 0) {
    summary.issues.push({ path: path.slice(), code: "union/issues-omitted",
      message: omitted + " further issues from the options were omitted" });
  }
  return summary;
}
