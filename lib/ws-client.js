...
class WsClient extends EventEmitter {
  ...
  _dial() {
    ...
    if (parsed.protocol === "wss:") {
      ...
      }, C.TLS_OUTBOUND_POSTURE(), dialTlsOpts || {});
      ...
      // A runtime `setKeyShares()` narrows or reorders the framework default,
      // so prefer the live list over the static posture value. The option is
      // `ecdhCurve`: node:tls has no `curves` option — it accepts the key and
      // ignores it, which silently dropped this preference from the
      // handshake, while a bad `ecdhCurve` throws. An operator value in
      // dialTlsOpts still wins.
      try {
        var pqcShares = networkTls().pqc.getKeyShares();
        if (Array.isArray(pqcShares) && pqcShares.length > 0 &&
            !(dialTlsOpts && dialTlsOpts.ecdhCurve)) {
          tlsOpts.ecdhCurve = pqcShares.join(":");
        }
