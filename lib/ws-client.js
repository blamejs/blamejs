...
class WsClient extends EventEmitter {
  ...
  _dial() {
    ...
    function _onError(err) {
      // A wss:// handshake refused by the outbound posture arrives as a bare
      // OpenSSL alert; name the cause before it reaches the caller's handler.
      networkTls().annotateOutboundFailure(err, {
        host: host, port: Number(parsed.port) || (parsed.protocol === "wss:" ? 443 : 80),
      });
