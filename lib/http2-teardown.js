...
// drainH2Session(session, graceMs, absoluteMaxMs) — retire a session WITHOUT
// cutting off the
...
// Ceiling on a drain regardless of observed progress — see the deadline
// note in drainH2Session. Generous enough for a genuinely long transfer,
// finite so a stalled stream on a chatty connection still ends.
var DRAIN_ABSOLUTE_MAX_MS = C.TIME.minutes(10);

function drainH2Session(session, graceMs, absoluteMaxMs) {
  if (!session) return;
  try { if (typeof session.close === "function") session.close(); }
  ...
  // Progress alone is not a sufficient bound. The byte counters are the
  // SOCKET's, so connection-level traffic a peer generates on its own —
  // PING frames, WINDOW_UPDATE, SETTINGS — keeps them moving even when the
  // response stream itself is stalled, and the watchdog would then never
  // fire. Keep an absolute ceiling alongside it so a session cannot linger
  // indefinitely on control traffic.
  var ceiling = typeof absoluteMaxMs === "number" && absoluteMaxMs > 0
    ? absoluteMaxMs : DRAIN_ABSOLUTE_MAX_MS;
  ...
  var watch = safeAsync.repeating(function () {
    ...
    if (moved !== lastMoved && Date.now() < deadline) {
      lastMoved = moved;                                       // still progressing
      return;
    }
    watch.stop();
    try { if (typeof session.destroy === "function") session.destroy(); }
