...
// Mandatory-security-assessment thresholds from the Measures for Security
// Assessment of Outbound Data Transfers (CAC, effective 2022-09-01).
// Crossing ANY of these forces the security-assessment mechanism — the
// operator cannot fall back to the standard contract or certification.
//   - CIIO exporter, or "important data" in scope: always mandatory.
//   - PI of more than 1,000,000 individuals handled by the exporter.
//   - cumulative outbound PI of more than 100,000 individuals, OR
//     cumulative outbound SENSITIVE PI of more than 10,000 individuals,
//     since 1 January of the preceding year.
var SECURITY_ASSESSMENT_PI_VOLUME_THRESHOLD = 1000000;
var SECURITY_ASSESSMENT_CUMULATIVE_PI_THRESHOLD = 100000;
var SECURITY_ASSESSMENT_CUMULATIVE_SENSITIVE_PI_THRESHOLD = 10000;
...
function sccFilingAssessment(opts) {
  ...
  // Mandatory-security-assessment determination (CAC Measures). Crossing
  // ANY trigger forces the security-assessment mechanism regardless of the
  // operator's declared legalBasis — the operator cannot self-downgrade to
  // the standard contract or certification once a trigger is present.
  var triggers = [];
  if (ciio) triggers.push("ciio");
  if (importantData) triggers.push("important-data");
  if (opts.volume > SECURITY_ASSESSMENT_PI_VOLUME_THRESHOLD) triggers.push("pi-volume");
  if (cumulativePI > SECURITY_ASSESSMENT_CUMULATIVE_PI_THRESHOLD) triggers.push("cumulative-pi");
  if (cumulativeSensitivePI > SECURITY_ASSESSMENT_CUMULATIVE_SENSITIVE_PI_THRESHOLD) triggers.push("cumulative-sensitive-pi");
