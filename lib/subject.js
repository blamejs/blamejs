 ...
 *   });
 *   Object.keys(dump);
 *   // → one key per declared table holding a row for that subject,
 *   //   ["users", "orders", "audit_log"] for the schema above
 *
 ...
 *
 * Identical behavior to `b.subject.export`. Shipped as a non-reserved
 * alias because some downstream toolchains (older bundlers, TypeScript
 * `import { export }` parsing, JSON-serialized method lists) trip on
...
function _writeAudit(action, subjectId, outcome, metadata) {
  // Drop-silent: a throw from audit.emit does not fail the caller.
  try {
    audit.emit({
      actor:    {},
      action:   action,
