 ...
 *
 * Cryptographic erasure (destroying per-subject derivation keys) is
 * structurally deferred to Phase 5+ — v0.0.4 ships physical row deletion
 * with the audit chain preserved (the subject's data rows are gone; the
 * audit_log entries about them remain hash-linked). This satisfies GDPR
 * Art. 17 in the strict sense (subject data is erased) at the cost of
 * losing the granular cryptographic-erasure property the spec aspires to.
 */
...
function _getDerivedFieldName(tableName, sourceField) {
  var schema = require("./field-crypto").getSchema(tableName);
...
function _resetForTest() { _db = null; }
