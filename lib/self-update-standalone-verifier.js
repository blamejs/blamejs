...
function verify(assetPath, signaturePath, pubkeyPem, opts) {
  ...
  // createVerify (incremental). For Ed25519 / ML-DSA we accumulate to
  // a buffer because node:crypto's one-shot verify needs the full
  // payload. 64 KiB chunks match the framework's hash-while-streaming
  ...
  var bufferedChunks = (verifier === null) ? [] : null;

  var fd = nodeFs.openSync(assetPath, "r");
  try {
    var chunk = Buffer.allocUnsafe(64 * 1024);   // allow:raw-byte-literal — module is zero-dep by contract; cannot import C.BYTES
    while (true) {
      var n = nodeFs.readSync(fd, chunk, 0, chunk.length, null);
      if (n === 0) break;
      var slice = chunk.subarray(0, n);
      sha256.update(slice);
      sha3.update(slice);
      if (verifier) verifier.update(slice);
      if (bufferedChunks) bufferedChunks.push(Buffer.from(slice));
    }
  } finally {
    nodeFs.closeSync(fd);
  }
  ...
  if (alg === "ecdsa-p384") {
    ...
    // 96 = P-384 IEEE-P1363 signature length (48-byte r || 48-byte s);
    // DER-encoded P-384 sigs are variable-length (~100-104). Protocol
    // constant, not a byte-size.
    ...
    var full = Buffer.concat(bufferedChunks);
    ok = nodeCrypto.verify(null, full, key, signature);
  } else if (alg === "ml-dsa-87") {
    var full2 = Buffer.concat(bufferedChunks);
    ok = nodeCrypto.verify(null, full2, key, signature);
  }
