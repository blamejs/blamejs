 ...
 *     IANA / RFC catalog (unless `allowExperimental: true`, which
 *     itself audits).
 *
...
var JMAP_METHODS_WITHOUT_ACCOUNT = Object.freeze({ "Core/echo": 1 });

/**
 * @primitive b.mail.serverRegistry.jmapMethodTakesAccount
 * @signature b.mail.serverRegistry.jmapMethodTakesAccount(name)
 * @since     0.20.32
 * @status    stable
 * @related   b.mail.serverRegistry.create, b.mail.server.jmap.create
 *
 * Report whether a JMAP method the registry knows takes an `accountId`.
 * RFC 8620 section 1.6.2 gives most methods a mandatory `accountId`, and
 * `Core/echo` is the one method in the registry's RFC 8620 and RFC 8621
 * list defined without one. `b.mail.server.jmap` asks this before it
 * dispatches, and answers `invalidArguments` when an account-scoped method
 * names no account. A name the registry does not know reads false: an
 * experimental method declares its own arguments.
 *
 * @example
 *   b.mail.serverRegistry.jmapMethodTakesAccount("Email/get");   // → true
 *   b.mail.serverRegistry.jmapMethodTakesAccount("Core/echo");   // → false
 */
function jmapMethodTakesAccount(name) {
  if (typeof name !== "string") return false;
  if (Object.prototype.hasOwnProperty.call(JMAP_METHODS_WITHOUT_ACCOUNT, name)) return false;
  return Object.prototype.hasOwnProperty.call(JMAP_METHODS, name);
}

/**
 * @primitive b.mail.serverRegistry.jmapMethodIsAccountFree
 * @signature b.mail.serverRegistry.jmapMethodIsAccountFree(name)
 * @since     0.20.32
 * @status    stable
 * @related   b.mail.serverRegistry.jmapMethodTakesAccount, b.mail.server.jmap.create
 *
 * Report whether this is a method the registry knows to take no account at
 * all. `Core/echo` is the one such method in the registry's RFC 8620 and RFC
 * 8621 list, and RFC 8620 section 4 returns its arguments unchanged, so a
 * member of them named like an account is data rather than a target.
 *
 * This is not the negation of `jmapMethodTakesAccount`, which also reads
 * false for a name the registry has never heard of. An operator's own method
 * is not known to be account-free, so it keeps every check.
 *
 * @example
 *   b.mail.serverRegistry.jmapMethodIsAccountFree("Core/echo");    // → true
 *   b.mail.serverRegistry.jmapMethodIsAccountFree("Custom/run");   // → false
 */
function jmapMethodIsAccountFree(name) {
  if (typeof name !== "string") return false;
  return Object.prototype.hasOwnProperty.call(JMAP_METHODS_WITHOUT_ACCOUNT, name);
}
...
module.exports = {
  ...
  JMAP_METHODS_WITHOUT_ACCOUNT: Object.keys(JMAP_METHODS_WITHOUT_ACCOUNT),
  jmapMethodTakesAccount:  jmapMethodTakesAccount,
  jmapMethodIsAccountFree: jmapMethodIsAccountFree,
