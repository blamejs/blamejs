...
function create(opts) {
  ...
  function dispatch(name) {
    ...
    // Tenant scope check — pre-dispatch, pre-guard, pre-audit. argsArr[0]
    // is the per-listener `state` object (which carries `state.actor`).
    // When agent is global-scope (no agentTenantId), the check no-ops.
    if (tenantScope && argsArr.length > 0 && argsArr[0] && argsArr[0].actor) {
      // tenantScope.check throws AgentTenantError on cross-tenant; we
      // let the typed error propagate so the listener's catch-path
      // converts it to the protocol's refusal reply.
      tenantScope.check(argsArr[0].actor, agentTenantId);
    }
