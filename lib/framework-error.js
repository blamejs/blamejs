...
// AuditChainOriginError covers an append refused because the purge anchor —
// the only statement that licenses missing audit rows — is present and cannot
// be believed. Permanent: retrying appends the same unlinkable row, and the
// condition is resolved by an operator investigating the anchor, not by time.
...
// "http-client/<reason>" shape: hash-mismatch, dest-not-writable,
