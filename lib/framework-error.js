...
// layer (lib/auth/*). Always permanent — auth failures are not
// transient ("retry might work"); they're "this credential doesn't
// match" or "this input was malformed".
class AuthError extends FrameworkError {
...
class JobsError extends FrameworkError {
  constructor(code, message, permanent) {
    super(message, code);
    this.name = "JobsError";
    this.permanent = !!permanent;
...
class SchedulerError extends FrameworkError {
  constructor(code, message, permanent) {
    super(message, code);
    this.name = "SchedulerError";
    this.permanent = !!permanent;
    this.isSchedulerError = true;
  }
}
