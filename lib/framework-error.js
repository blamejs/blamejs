...
function defineClass(name, opts) {
  ...
  var GeneratedError = class extends FrameworkError {
    constructor(code, message, arg3, arg4) {
      ...
      // when a metrics registry is active. Lazy require avoids a circular
      // load (metrics.js requires framework-error at module load).
      try { require("./metrics").tap("error.construct", 1, { class: name }); }
