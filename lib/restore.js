 ...
 * `rollbackRoot` on another filesystem, and `restore/bad-staging-root` or
 * `restore/bad-rollback-root` for one inside the data directory. A failure
...
function create(opts) {
  ...
  function _requireSwappableLayout() {
    var resolvedDataDir = nodePath.resolve(dataDir);
    var inside = [["stagingRoot", stagingRoot], ["rollbackRoot", rollbackRoot]];
    for (var n = 0; n < inside.length; n++) {
      var relative = nodePath.relative(resolvedDataDir, nodePath.resolve(inside[n][1]));
      if (relative === "" || (relative.split(nodePath.sep)[0] !== ".." && !nodePath.isAbsolute(relative))) {
    ...
    if (live !== null && live.dev !== dataParent.dev) {
