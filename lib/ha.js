"use strict";
/**
 * High-availability coordination — leader election + fencing tokens.
 *
 * Opt-in via `b.ha.init(...)`. When init is never called, the local
 * process behaves as a permanent single leader: `isLeader()` always
 * returns true, `fencingToken()` returns 0, no heartbeat thread runs,
 ...
 *
 * Public API:
 *   await ha.init(opts)                  one-time bootstrap
 *   ha.isLeader()                        sync; true on leader (or single-node)
 *   ha.currentNodeId()                   sync; configured nodeId
 *   ha.fencingToken()                    sync; current monotonic token
 *   ha.requireLeader()                   sync; throws NotLeaderError
 *   ha.currentLeader()                   async; { nodeId, leaseExpiresAt,
 *                                                 fencingToken } | null
 *   ha.onTransition(fn)                  register transition handler
 *   await ha.shutdown()                  releases lease, stops heartbeat
 */
...
function log(msg)    { console.log("[blamejs:ha] " + msg); }
function logErr(msg) { console.error("[blamejs:ha] " + msg); }

class NotLeaderError extends Error {
  constructor(message) {
    super(message || "not leader: write rejected by HA gate");
    this.name = "NotLeaderError";
    this.code = "NOT_LEADER";
    this.statusCode = 503;            // operator's LB should retry on the leader
    this.isHaError = true;
...
async function init(opts) {
  if (initialized) {
    throw _err("ALREADY_INITIALIZED", "ha.init() called twice", true);
  }
  opts = opts || {};
  if (!opts.nodeId) {
    throw _err("INVALID_CONFIG", "ha.init({ nodeId }) is required", true);
  }
  ...
  if (opts.provider) {
    ...
    if (!opts.externalDbBackend) {
      throw _err("INVALID_CONFIG",
        "ha.init requires either { provider } or { externalDbBackend }", true);
    }
    var dbProvider = require("./ha-provider-db");
...
function requireLeader() {
  if (!isLeader()) {
    throw new NotLeaderError(
      ...
      (initialized ? "" : " (ha not initialized)")
    );
  }
}
