...
function tokenize(text) {
  ...
  for (var i = 0; i < rawTokens.length && out.length < MAX_TOKENS_PER_FIELD; i++) {
    ...
    // The leading trim is anchored and costs one pass; the trailing one is
    // not, and as a regex it re-scanned the run from every position.
    t = codepointClass.trimTrailingChars(t.replace(/^[']+/, ""), "'");
    if (!t) continue;
