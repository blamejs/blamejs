...
function _verifyNsec3Denial(opts, qtypeNum) {
  ...
  if (!findCover("*." + ceName) && !findMatch("*." + ceName)) {
    throw new DnssecError("dnssec/denial-not-proven", "dnssec.verifyDenial: the wildcard at the closest encloser is not covered");
  }
...
function _verifyNsecDenial(opts, qtypeNum) {
  ...
  var wildcard = "*." + ce;
  if (!findCover(wildcard) && !findMatch(wildcard)) {
    throw new DnssecError("dnssec/denial-not-proven", "dnssec.verifyDenial: the wildcard at the closest encloser is neither covered nor matched");
  }
