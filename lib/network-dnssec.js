...
// the total signature-validation work across the whole chain walk. Matches
// the BIND `max-key-tag-collisions` + Unbound validation-budget mitigations.
var MAX_COLLIDING_KEYS            = 4;                       // allow:raw-byte-literal — same-tag DNSKEY candidates tried per RRSIG
var MAX_SIG_VALIDATIONS_PER_CHAIN = 16;                      // allow:raw-byte-literal — total pubkey signature-verifies per verifyChain response
    ...
    if (budget) {
      if (budget.remaining <= 0) {
        throw new DnssecError("dnssec/validation-budget-exceeded",
          "dnssec.verifyChain: signature-validation budget (" +
          MAX_SIG_VALIDATIONS_PER_CHAIN + ") exhausted — refused as a KeyTrap " +
          "(CVE-2023-50387) amplification vector");
      }
      budget.remaining -= 1;
    }
    try {
 ...
 * and the whole response is held to a 16 signature-validation budget — a
 * hostile zone publishing many colliding keys / signatures is refused with
 * <code>dnssec/too-many-colliding-keys</code> /
...
function verifyChain(opts) {
  ...
  // response — a hostile chain cannot exceed it no matter how the keys /
  // sigs / links are arranged.
  var budget = { remaining: MAX_SIG_VALIDATIONS_PER_CHAIN };
