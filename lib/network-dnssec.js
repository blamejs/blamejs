...
function _findKeyByTag(dnskeys, tag) {
  for (var i = 0; i < dnskeys.length; i++) if (keyTag(dnskeys[i]) === tag) return dnskeys[i];
  return null;
}
...
function verifyChain(opts) {
  ...
  for (var i = 0; i < opts.links.length; i++) {
    ...
    // 1. The DNSKEY RRset is self-signed by one of its own keys.
    var signer = _findKeyByTag(link.dnskeys, link.dnskeyRrsig.keyTag);
    if (!signer) throw new DnssecError("dnssec/chain-no-signing-key", "dnssec.verifyChain: no DNSKEY in '" + link.zone + "' matches the DNSKEY RRSIG key tag");
    var sp = _dnskeyParts(signer);
    verifyRrset({ name: link.zone, type: "DNSKEY", rdatas: link.dnskeys, rrsig: link.dnskeyRrsig, dnskey: { algorithm: sp.algorithm, publicKey: sp.publicKey }, at: opts.at });
    ...
    if (i === 0) {
      ...
      var dsSigner = _findKeyByTag(trustedKeys, link.dsRrsig.keyTag);
      if (!dsSigner) throw new DnssecError("dnssec/chain-no-parent-key", "dnssec.verifyChain: no trusted parent key matches the DS RRSIG key tag for '" + link.zone + "'");
      var dp = _dnskeyParts(dsSigner);
      verifyRrset({ name: link.zone, type: "DS", rdatas: link.dsRdatas, rrsig: link.dsRrsig, dnskey: { algorithm: dp.algorithm, publicKey: dp.publicKey }, at: opts.at });
      var dsMatched = false;
