 ...
 *
 * @example
 *   var parts = b.apiKey.parseFormat("bk_live_00000001_0000000000000002");
 *   // → { prefix: "bk", namespace: "live", idHex: "00000001",
 *   //     secretHex: "0000000000000002" }
 *
...
function create(opts) {
  ...
  async function verify(token, verifyOpts) {
    ...
    if (row.expiresAt != null && numericBounds.finiteTimestamp(row.expiresAt) < nowMs) {
      if (auditFailures) {
        _emit("apikey.verify", {
          actor:    _actor(verifyOpts, rowOwnerId),
          resource: { kind: "apikey", id: compositeId },
