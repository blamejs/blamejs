...
// The columns the framework schema seals. Asked of `cryptoField` rather than
// written out a second time, so a schema change cannot leave this list behind:
// a column added to `sealedFields` and forgotten here would be skipped by the
// re-seal pass and stay under the old envelope with nothing reporting it.
...
function create(opts) {
  ...
  // Rewrite every row's sealed columns under the current envelope. `ownerId`,
  // `scopes` and `metadata` are bound to the row they sit on, and a row
  // written before that binding existed carries the unbound form, which a
  // bound column refuses on read.
  //
  // Nothing in the ordinary lifecycle rewrites those columns -- `verify`
  // touches `lastUsedAt` and, on an algorithm upgrade, `secretHash` -- so a
  // deployment cannot reach the bound form by being used. This is the step
  // that gets there: open the window with `db.init({ allowPlainSealMigration:
  // ["_blamejs_api_keys"] })`, run this once, then restart without it. Reading
  // an unbound cell records `system.crypto_field.plain_cell_accepted`, so the
  // count reaching zero is what says the window can close.
  //
  // Re-sealing an already-bound row produces fresh ciphertext and is
  // otherwise a no-op, so a second run is safe and an interrupted one resumes.
  async function resealSealedColumns(resealOpts) {
    ...
    for (var i = 0; i < rows.length; i += 1) {
      ...
      // A cell that could not be read comes back null, and writing that back
      // would destroy the very rows this pass exists to save. The read only
      // fails for the reason this pass is run -- the row predates the binding
      // and the window is not open -- so refuse the whole pass and name the
      // option rather than rewriting anything.
      ...
      // Seal the whole row and write back only the sealed columns; every other
      // column is left exactly as it is, so a concurrent `verify` touching
      // `lastUsedAt` is not rolled back by this pass.
