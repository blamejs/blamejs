...
function create(opts) {
  ...
  async function verify(token) {
    ...
    // controls the verification algorithm. Fall back to legacy raw-hex
    // SHA3-512 for any rows issued by v0.2.27 (pre-envelope); those
    // get rehashed transparently below on success.
    var primaryMatch = await credentialHash.verifyAllowingLegacy(
      parsed.secretHex, row.secretHash);
    ...
    if (!primaryMatch && secondaryActive) {
      secondaryMatch = await credentialHash.verifyAllowingLegacy(
        parsed.secretHex, row.secondarySecretHash);
    }
