...
var ARCH_WRAP_HEADER_BYTES = C.BYTES.bytes(6);
var ARCH_PASSPHRASE_MAGIC = "BAWPP";
var ARCH_PASSPHRASE_VERSION = 0x01;
var ARCH_PASSPHRASE_HEADER_BYTES = C.BYTES.bytes(7);
...
/**
 * @primitive b.archive.canUnwrapFor
 * @signature b.archive.canUnwrapFor(recipient)
 * @since     0.20.31
 * @status    stable
 * @related   b.archive.unwrap, b.archive.wrap
 *
 * Report whether `unwrap` can read an archive written for this recipient
 * with the material the recipient object carries. True for the static-key
 * pair (`privateKey` + `ecPrivateKey`), for the peer-certificate pair
 * (`certPrivateKey` + `kemSecret`), and for `"tenant"`, whose key the host
 * holds in its own vault. A recipient carrying only public material (the
 * shape `wrap` needs) reads false: encrypting for someone else is what it
 * is for.
 *
 * @example
 *   var keys = b.crypto.generateEncryptionKeyPair();
 *   b.archive.canUnwrapFor(keys);                                  // → true
 *   b.archive.canUnwrapFor({ publicKey: keys.publicKey,
 *                            ecPublicKey: keys.ecPublicKey });     // → false
 */
function canUnwrapFor(recipient) {
  if (recipient === "tenant") return true;
  if (!recipient || typeof recipient !== "object") return false;
  if (recipient.certPrivateKey && recipient.kemSecret) return true;
  return !!(recipient.privateKey && recipient.ecPrivateKey);
}

module.exports = {
  wrap:                  wrap,
  unwrap:                unwrap,
  canUnwrapFor:          canUnwrapFor,
