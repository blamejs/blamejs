...
async function wrapWithPassphrase(bytes, opts) {
  ...
  var minEntropy = typeof opts.minEntropyBits === "number" ? opts.minEntropyBits : 80;  // allow:raw-byte-literal — entropy-bits constant, not byte count
...
function _estimatePassphraseEntropyBits(passphrase) {
  var s = typeof passphrase === "string" ? passphrase : passphrase.toString("utf-8");
  if (s.length === 0) return 0;
  // Observed-alphabet count. log2(alphabetSize) bits per character is
  // the upper bound for uniformly-random selection over that alphabet.
  // Operators picking a dictionary phrase score lower in reality;
  // this approximation is the standard NIST/OWASP "estimate by
  // character classes" approach and matches what b.backup uses
  // elsewhere.
