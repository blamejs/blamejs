...
class Router {
  ...
  async handle(req, res) {
    ...
    // and writing the normalized target back onto req.url, keeps it a pure path
    // for the route matcher AND every downstream req.url reader.
    if (reqTarget.charAt(1) === "/") {
      reqTarget = reqTarget.replace(/^\/+/, "/");
      req.url = reqTarget;
    }
