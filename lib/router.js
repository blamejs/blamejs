...
class Router {
  ...
  async handle(req, res) {
    ...
    // Only an origin-form request target — a single leading "/" then the path
    // (RFC 9112 §3.2.1) — is routable by a path router. Reject an absolute-form
    // (`http://host/path`), authority-form (`host:port`), asterisk-form
    // (`OPTIONS *`), or network-path reference (`//host/path`) target with 400
    // rather than coercing it into a routable path by prefixing: prefixing
    // would turn `http://evil/admin` into `/http://evil/admin`, which a `/:x`
    // or catch-all route would still match — the opposite of failing closed.
    if (reqTarget.charAt(0) !== "/" || reqTarget.charAt(1) === "/") {
      res.statusCode = 400;
