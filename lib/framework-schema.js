...
async function withDeleteTriggersSuspended(opts, fn) {
  ...
  if (dialect === "postgres") {
    throw new FrameworkSchemaError(
      "withDeleteTriggersSuspended does not support Postgres: its guard is a " +
        "trigger FUNCTION shared by every WORM table, so dropping the triggers " +
        "and putting them back is not the same operation there. Nothing in the " +
        "framework needs this on any dialect — the guard is anchor-bounded and " +
        "a purge leaves it installed; see wormGuardIsAnchorBounded",
      "framework-schema/invalid-config"
    );
  }
  var dropped = [];
 ...
 *
 * Postgres and MySQL both express the condition in the trigger body. MySQL's
 * needs `IF … THEN SIGNAL …; END IF`, whose internal semicolon makes the body
 * compound — a protocol driver sends that as one message, and only a CLI needs
 * DELIMITER to keep from splitting it.
 *
 * @example
 *   b.frameworkSchema.wormGuardIsAnchorBounded("postgres");  // → true
 *   b.frameworkSchema.wormGuardIsAnchorBounded("sqlite");    // → false
 */
