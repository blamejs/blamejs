...
function _auditPurgeAnchorDDL(dialect) {
  ...
  return _table(tableName("_blamejs_audit_purge_anchor"), dialect, [
    ...
    { col: "fencingToken",         def: t.INT + " NOT NULL DEFAULT 0" },
    { raw: "CHECK (" + safeSql.quoteIdentifier("scope", _qd(dialect)) + " = 'audit')" },
  ], [], ["firstPurgedCounter", "signature", "publicKeyFingerprint", "fencingToken"]);
}
...
async function _installWormTriggers(backend, dialect) {
  ...
  for (var i = 0; i < wormTables.length; i++) {
    ...
    if (dialect === "postgres") {
      ...
      // The guard reads a SESSION setting rather than being taken off for a
      // sanctioned deletion. Dropping the trigger would lift the protection
      // for the whole database, so every other connection could delete audit
      // rows for as long as the purge ran — a window that exists for no one's
      // benefit and that a concurrent purge could also leave open by
      // recreating the guard underneath another one.
      //
      // `SET LOCAL` scopes the permission to one transaction on one
      // connection, so it cannot outlive the statement that needed it and no
      // other session sees it. A DELETE is allowed only where the purge put
      // the flag; everything else still meets the exception. `true` as the
      // second argument to current_setting makes an unset value NULL rather
      // than an error, which is the state every ordinary connection is in.
      await externalDb.query(
        "CREATE OR REPLACE FUNCTION " + fnName + "() RETURNS trigger AS $$ " +
        "BEGIN " +
        "IF TG_OP = 'DELETE' AND " +
        "current_setting('blamejs.audit_purge', true) = 'on' THEN RETURN OLD; END IF; " +
  ...
  if (unrestored.length > 0) {
    ...
    throw new FrameworkSchemaError(
      "the append-only DELETE guard could not be restored on " +
        ...
        (bodyErr ? ". The purge itself failed with: " + ((bodyErr && bodyErr.message) || String(bodyErr)) : ""),
...
/**
 * @primitive b.frameworkSchema.purgePermissionSql
 * @signature b.frameworkSchema.purgePermissionSql(dialect)
 * @since     0.18.58
 * @status    stable
 * @related   b.frameworkSchema.ensureSchema, b.auditTools.purge
 *
 * The statements that grant and withdraw one connection's permission to delete
 * from the append-only audit tables, or `null` on a dialect whose guard cannot
 * read one — in which case the caller suspends the guard instead. Returning a
 * grant a trigger does not inspect would be worse than returning nothing: the
 * deletion would take the permission path and still be refused.
 *
 * The append-only guard stays installed. Taking it off for a sanctioned
 * deletion would lift the protection for the WHOLE database and for as long as
 * the purge ran, so every other connection could delete audit rows in that
 * window — and a second purge could restore the guard underneath the first.
 * Instead the trigger asks whether THIS connection was granted permission, and
 * only the purge grants it.
 *
 * Both statements must run on the same connection as the deletion, which means
 * inside one transaction. On Postgres `SET LOCAL` ends with that transaction
 * by itself; MySQL's user variable does not, so it is cleared explicitly —
 * a pooled connection handed on with the permission still set would carry it
 * to whatever borrowed it next.
 *
 * The caller runs `grant`, then its deletion, then `revoke` if there is one —
 * all inside one `b.clusterStorage.transaction`, so they share a connection.
 * `b.db.purgeAuditChain` is the only caller.
 *
 * @example
 *   b.frameworkSchema.purgePermissionSql("postgres");
 *   // → { grant: "SET LOCAL blamejs.audit_purge = 'on'", revoke: null }
 *
 *   b.frameworkSchema.purgePermissionSql("mysql");
 *   // → null — its guard cannot read a session value, so the caller
 *   //   suspends the guard instead
 */
function purgePermissionSql(dialect) {
  var d = (dialect || "").toLowerCase();
  if (d === "postgres") {
    return { grant: "SET LOCAL blamejs.audit_purge = 'on'", revoke: null };
  }
  // MySQL deliberately returns null. Its DELETE guard is unconditional —
  // making it read a session variable needs `IF … THEN SIGNAL …; END IF`,
  // whose internal semicolon stops it being a single sendable statement — so
  // granting a permission it never inspects would send every MySQL purge down
  // this path to be refused by the trigger anyway. Null routes MySQL to
  // withDeleteTriggersSuspended, which is what its guard actually supports.
  return null;
}
...
module.exports = {
  ...
  purgePermissionSql:     purgePermissionSql,
  tableName:              tableName,
