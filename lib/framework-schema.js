...
function _auditPurgeAnchorDDL(dialect) {
  ...
  return _table(tableName("_blamejs_audit_purge_anchor"), dialect, [
    ...
    // And the covering checkpoint, separately: an archive is refused without
    // one, so a bundle whose checkpoint payload was replaced is unusable even
    // though its rows still match. Checked against the manifest alone, that
    // bundle reports as producible.
    { col: "archiveCheckpointDigest", def: "TEXT" },
    // And the manifest that says how to OPEN those payloads. The two digests
    // above cover the encrypted bytes; the manifest carries the salts the key
    // is derived from, and nothing signed covered those. An attacker able to
    // edit the archive directory could change salts.rows alone — payloads and
    // their signed digests untouched — and the bundle still reported as
    // producible while no longer decrypting at all.
    ...
    for (var j = 0; j < d.indexes.length; j++) {
      ...
      if (dialect === "mysql") {
        try {
          ...
          // The driver's own errno, not its wording: a message test for
          // `exist` also matches "Table 'db.X' doesn't exist", so a CREATE
          // INDEX that failed because its table was missing was swallowed and
          // the schema below reported as created without the index.
...
// The WHEN clause that makes a SQLite delete guard anchor-bounded, or "" for
// a table no purge ever touches (which is then refused unconditionally).
...
async function _installDeleteTrigger(backend, dialect, t, boundedBy) {
  ...
  if (dialect === "mysql") {
    ...
    // Same rule as Postgres: permitted only for a row the purge anchor already
    // licenses. Expressing it needs `IF … THEN SIGNAL …; END IF`, whose
    // internal semicolon makes the body compound — which is why this used to
    // be unconditional and the purge suspended the guard instead, opening a
    // window in which any other session could delete audit rows. A protocol
    // driver sends the whole CREATE TRIGGER as one message; only a CLI needs
    // DELIMITER to keep from splitting it.
    var deny = "SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '" +
      t + " is append-only — DELETE prohibited'";
    //
    // COALESCE(..., FALSE) is load-bearing. With no anchor row the subquery is
    // NULL, the comparison is NULL, and `IF NOT NULL THEN` is not true — so a
    // bare condition would let the DELETE through on exactly the volume that
    // has never authorized one. Deny unless explicitly permitted.
  ...
  // SQLite: DROP then CREATE, not CREATE IF NOT EXISTS. A volume upgraded from
  // a build whose guard was unconditional already HAS a trigger by this name,
  // and IF NOT EXISTS would leave that old body in place — which, now that the
  // purge no longer takes the guard off, means every purge on an upgraded
  // volume is refused by a guard that cannot be satisfied. Replacing it is the
  // migration. The drop and the create are two statements, but the SQLite
  // backends here are a single shared connection (cluster) or a single process
  // holding the file (local), so there is no second session to slip between
  // them.
  await externalDb.query(
    'DROP TRIGGER IF EXISTS "no_delete_' + t + '"', [], { backend: backend });
  // The same anchor-bounded rule as the other two, carried by a WHEN clause —
  // SQLite evaluates a subquery there, so the guard can read the boundary
  // without the purge ever taking it off. COALESCE for the same reason: with
  // no anchor the comparison is NULL, and a WHEN that is not true would skip
  // the RAISE and let the delete through on the volume that has authorized
  // nothing.
  await externalDb.query(
...
async function _installWormTriggers(backend, dialect) {
  ...
  for (var i = 0; i < wormTables.length; i++) {
    ...
    if (dialect === "postgres") {
      ...
      // COALESCE for the same reason the MySQL body has it, and spelled the
      // same way so the rule cannot drift between the two: with no anchor row
      // the subquery is NULL and the comparison is NULL. Here that already
      // falls through to the RAISE below — the permit branch requires TRUE —
      // but a later edit that inverted the branch would silently start letting
      // those deletes through, and the volume with no anchor is precisely the
      // one that has never authorized a deletion.
      ...
      var qt = "`" + t + "`";
      // Anchor-bounded, the same rule as Postgres. It used to be
      // unconditional, because the condition needs `IF … THEN SIGNAL …; END
      // IF` and the semicolon inside makes the body compound — so the purge
      // suspended the guard instead, leaving a window in which any other
      // session could delete audit rows. A protocol driver sends a compound
      // CREATE TRIGGER as one message; DELIMITER is a CLI construct, and
      // bending the guard's design around a test harness that needed it was
      // the wrong trade.
      //
      // Through the shared helper, so any other caller installs exactly this
      // trigger rather than a second spelling of it.
      ...
      // SQLite cluster path. Through the shared helper so this is the same
      // trigger the restore and the local path install, rather than a second
      // spelling that could drift from the anchor-bounded rule.
      await _installDeleteTrigger(backend, dialect, t, boundedBy);
      await externalDb.query(
 ...
 */
// The dialects whose delete guard can be dropped and rebuilt as the SAME
// guard. Postgres is absent deliberately — see the refusal below.
...
async function withDeleteTriggersSuspended(opts, fn) {
  ...
  // No default. It defaulted to postgres — the one dialect this refuses — so
  // an omitted dialect could only throw, and the message blamed a choice the
  // caller never made. There is no dialect it would be safe to guess: dropping
  // and restoring a guard is dialect-specific DDL against an operator's audit
  // tables.
  ...
  // The guard genuinely comes off, and that is a real cost: a DROP TRIGGER is
  // global, so for as long as `fn` runs, any other connection holding the
  // framework role can delete audit rows too. Nothing on the framework's own
  // path takes this route any more — every dialect's guard is anchor-bounded
  // and a purge leaves it installed — so the window belongs to whoever calls
  // this deliberately, and they own the decision.
  //
  ...
  var result;
  try {
    // Paired with the logical name so the restore can rebuild the SAME guard.
    // Reinstalling without the boundary column would put back an
    // unconditional trigger where an anchor-bounded one had been — a
    // different guard wearing the same name, and one that refuses the very
    // deletion the anchor licenses.
