...
// Columns added to a table that has already shipped.
//
// Every DDL here is CREATE TABLE IF NOT EXISTS, which does exactly nothing to a
// table that already exists — so a column added after a deployment's first boot
// never arrives on its own, and the first write naming it fails on a database
// the operator believes they just upgraded. Re-running schema setup has to be
// enough.
//
// Idempotence differs by dialect: Postgres has ADD COLUMN IF NOT EXISTS, while
// SQLite and MySQL do not and report a duplicate column instead. That duplicate
// IS the intended end state, so it is swallowed where it is issued; any other
// error still propagates.
var ADDITIVE_COLUMNS = [
  // 0.18.56 — the purge anchor is signed, so it carries the signature and the
  // fingerprint of the key that made it. Without these a purge cannot record
  // which rows it removed, and the rows are what would be lost.
  { table: "_blamejs_audit_purge_anchor", col: "signature",            type: "BLOB" },
  { table: "_blamejs_audit_purge_anchor", col: "publicKeyFingerprint", type: "TEXT" },
];

function _additiveColumnDDL(dialect) {
  var t = _types(dialect);
  var qd = _qd(dialect);
  return ADDITIVE_COLUMNS.map(function (c) {
    return "ALTER TABLE " + tableName(c.table) + " ADD COLUMN " +
      (dialect === "postgres" ? "IF NOT EXISTS " : "") +
      safeSql.quoteIdentifier(c.col, qd, { allowReserved: true }) + " " +
      (c.type === "BLOB" ? t.BLOB : c.type);
  });
}
...
function _auditPurgeAnchorDDL(dialect) {
  ...
  return _table(tableName("_blamejs_audit_purge_anchor"), dialect, [
    ...
    // The anchor decides which rows are allowed to be missing, so it carries
    // the same standing as the chain it excuses: signed, and checked before
    // any gap is honoured. These MUST mirror the local declaration in db.js —
    // the two describe one table, and a purge against an external backend
    // whose columns lagged would delete the rows and then fail to write the
    // anchor explaining them.
    //
    // Nullable, because a volume purged by an earlier version has an anchor
    // with no signature. Such an anchor is no longer honoured, but it has to
    // remain readable to say so.
...
async function ensureSchema(opts) {
  ...
  // Bring an already-existing table up to the current column set. The CREATE
  // above is a no-op for it, so this is the only thing that reaches a database
  // that was set up by an earlier version.
  var alters = _additiveColumnDDL(dialect);
  for (var ai = 0; ai < alters.length; ai += 1) {
    try {
      await externalDb.query(alters[ai], [], { backend: opts.externalDbBackend });
    } catch (e) {
      // The column already being there is the intended end state, not a
      // failure. Postgres says so with IF NOT EXISTS and never lands here;
      // SQLite reports "duplicate column name" and MySQL error 1060.
      if (!/duplicate column|already exists|1060/i.test((e && e.message) || "")) throw e;
    }
  }
...
module.exports = {
  ...
  _additiveColumnDDLForTest: _additiveColumnDDL,
};
