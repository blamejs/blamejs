...
function _sanitize(input, opts) {
  ...
  if (_hasAdsSuffix(name)) {
    throw _err("filename.ntfs-ads",
      "filename contains NTFS alternate data stream syntax — always refused " +
      "here; adsPolicy \"allow\" applies only to verifyExtractionPath");
  }
