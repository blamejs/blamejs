  ...
  if (nodeFs.existsSync(rootResolved)) {
    ...
    while (probe.length >= rootResolved.length && safetyCounter < SAFETY_LIMIT) {
      ...
      if (nodeFs.existsSync(probe)) {
        ...
        if (realProbe !== realRoot && realProbe.indexOf(realRoot + sep) !== 0) {
        ...
        if (!opts.followSymlinks) {
          // Detect symlink in the resolved chain — followSymlinks: false
          // means a symlink ANYWHERE in the resolved path refuses.
          if (realProbe !== probe) {
            throw new GuardFilenameError("filename.extraction-realpath-symlink",
              "verifyExtractionPath: probe " + JSON.stringify(probe) +
              " is a symlink to " + JSON.stringify(realProbe) +
              " — refuse extraction across symlink (set opts.followSymlinks=true to opt in)");
          }
        }
        break;
      }
