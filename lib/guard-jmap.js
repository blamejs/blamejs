 ...
 * server enforces. Throws `GuardJmapError` with code
 * `guard-jmap/bad-profile` when the profile is not one of the three.
 *
 * @opts
...
function limitsFor(opts) {
  ...
  if (!Object.prototype.hasOwnProperty.call(PROFILES, profileName)) {
    throw new GuardJmapError("guard-jmap/bad-profile",
      "guardJmap.limitsFor: unknown profile '" + profileName + "'");
  }
...
function validate(rawBody, opts) {
  opts = opts || {};
  var caps;
  try { caps = limitsFor(opts); }
  catch (e) {
    throw new GuardJmapError("guard-jmap/bad-profile",
      "guardJmap.validate: " + ((e && e.message) || "unknown profile")
        .replace("guardJmap.limitsFor: ", ""));
  }
