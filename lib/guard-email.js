...
function _hasSmuggledVerb(text) {
  var len = text.length;
  // One backward pass. Walking the whitespace run forward from every bare line
  // ending re-read the same characters once per position, and a carriage
  // return is itself whitespace, so a value that is nothing but bare carriage
  // returns cost time proportional to its SQUARE: 243ms for a single
  // 16k-character call. A guard is handed hostile input by definition, so that
  // shape is not hypothetical.
  //
  // Going backwards, the end of the whitespace run at a position is known from
  // the position after it, so two scalars carry it and no table is built. The
  // scan accepts up to maxBytes, which is 128 MiB on the permissive profile,
  // and one entry per character there would allocate 512 MiB beside a message
  // that passed the size gate.
  var afterOne = len;                    // first non-whitespace at or after i+1
  var afterTwo = len;                    // first non-whitespace at or after i+2
  for (var i = len - 1; i >= 0; i -= 1) {
    var cc = text.charCodeAt(i);
    var bare = (cc === 0x0D && text.charCodeAt(i + 1) !== 0x0A) ||
               (cc === 0x0A && text.charCodeAt(i - 1) !== 0x0D);
    // Read the run boundary before the scalars move past this position: an
    // optional "." sits between the line ending and the whitespace, so a
    // dotted line ending skips one further.
