...
function _hasSmuggledVerb(text) {
  var len = text.length;
  // Where the whitespace run starting at each index ends, worked out once in a
  // single backward pass.
  //
  // Walking that run forward from every bare line ending re-read the same
  // characters once per position, and a carriage return is itself whitespace,
  // so a value that is nothing but bare carriage returns cost time
  // proportional to its SQUARE: 243ms for a single 16k-character call. A guard
  // is handed hostile input by definition, so that shape is not hypothetical.
  var nextNonWs = new Int32Array(len + 1);
  nextNonWs[len] = len;
  for (var k = len - 1; k >= 0; k -= 1) {
    nextNonWs[k] =
      codepointClass.inRanges(text.charCodeAt(k), codepointClass.WHITESPACE_RANGES)
        ? nextNonWs[k + 1]
        : k;
  }
