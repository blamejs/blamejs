...
/**
 * @primitive b.cliHelpers.resolveSignatureOptions
 * @signature b.cliHelpers.resolveSignatureOptions(args, report)
 * @since     0.20.31
 * @status    stable
 * @related   b.cliHelpers.resolvePassphrase, b.restore.create, b.restoreBundle.extract
 *
 * Read the backup-signature flags every command that decrypts a bundle
 * accepts, and return them in the shape `b.restore.create` and
 * `b.restoreBundle.extract` take: `{ requireSignature, expectedFingerprint,
 * verifySignature }`. The flags are `--expected-fingerprint <hex>`,
 * `--require-signature` and `--no-verify-signature`.
 *
 * Returns `null` after writing a usage error through `report` when
 * `--expected-fingerprint` carries no value, or when
 * `--no-verify-signature` is combined with either of the others; the caller
 * returns the reporter's exit code.
 *
 * @example
 *   var report = b.cliHelpers.makeReporter(ctx, "blamejs backup");
 *   var sig = b.cliHelpers.resolveSignatureOptions({ flags: { "require-signature": true } }, report);
 *   sig.requireSignature;      // → true
 *   sig.verifySignature;       // → undefined
 */
function resolveSignatureOptions(args, report) {
  var flags = (args && args.flags) || {};
  var expected = flags["expected-fingerprint"];
  if (expected !== undefined && (expected === true || String(expected).length === 0)) {
    report.error("--expected-fingerprint requires a fingerprint value", 2);
    return null;
  }
  var skip = flags["no-verify-signature"] === true;
  var required = flags["require-signature"] === true;
  if (skip && (required || expected !== undefined)) {
    report.error("--no-verify-signature cannot be combined with --require-signature or --expected-fingerprint", 2);
    return null;
  }
  return {
    requireSignature:    required,
    expectedFingerprint: expected !== undefined ? String(expected) : undefined,
    verifySignature:     skip ? false : undefined,
  };
}
...
module.exports = {
  ...
  resolveSignatureOptions: resolveSignatureOptions,
  bootApp:            bootApp,
};
