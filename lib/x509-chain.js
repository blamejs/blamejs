...
function _constraintName(node) {
  ...
  if (node.tag === 2) return node.constructed ? { type: "malformed" } : { type: "dns", value: node.value.toString("latin1") };
  if (node.tag === 7) return node.constructed ? { type: "malformed" } : { type: "ip", value: Buffer.from(node.value) };
  if (node.tag === 0 || node.tag === 1 || node.tag === 3 || node.tag === 4 ||
      node.tag === 5 || node.tag === 6 || node.tag === 8) return { type: "other" };
...
function _subjectAltConstraintNames(cert) {
  var out = [];
  var extnValue;
  try { extnValue = _certExtensionValue(cert.raw, OID_SUBJECT_ALT_NAME); } catch (_e) { return { names: out, malformed: true }; }
...
function _nameConstraintsOf(cert) {
  var extnValue;
  try { extnValue = _certExtensionValue(cert.raw, OID_NAME_CONSTRAINTS); } catch (_e) { return { error: true }; }
  if (!extnValue) return null;
  try {
    var seq = asn1.readNodeStrict(extnValue);
