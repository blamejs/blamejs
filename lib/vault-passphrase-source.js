 ...
 *
 *   passphraseSource.getPassphrase({
 *     envVars: {
...
var envSafe = require("./parsers/safe-env");
...
async function fromEnv(opts) {
  ...
  // envSafe.readVar handles: missing/empty (required), size cap, utf8→Buffer
  // coercion, and process.env strip-after-read in one call.
  return envSafe.readVar(envVars.value, {
    type:     "buffer",
    required: true,
    ...
    var onData = function (chunk) {
      for (var i = 0; i < chunk.length; i++) {
        ...
        if (b === 0x0A || b === 0x0D) { // enter
          ...
          for (var ci = 0; ci < chunks.length; ci++) bufferSafe.secureZero(chunks[ci]);
          try {
        ...
        if (b === 0x7F || b === 0x08) { // backspace / DEL
          if (chunks.length > 0) bufferSafe.secureZero(chunks.pop());
          continue;
        }
