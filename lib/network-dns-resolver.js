...
function create(opts) {
  ...
  async function query(name, type, qopts) {
    ...
    if (inflight.has(key)) {
      try { await inflight.get(key); } catch (_e) { /* winner failed — re-check cache, else fetch ourselves below */ }
    ...
    var fillPromise = _fetchFill(name, qtype, key, hit, now);
    inflight.set(key, fillPromise);
    var filled;
    try { filled = await fillPromise; }
    finally { if (inflight.get(key) === fillPromise) inflight.delete(key); }
