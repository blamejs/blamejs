...
async function init(opts) {
  ...
  if (!auditResult.ok) {
    ...
    logErr("Recovery is manual: restore from backup, or rebuild the audit chain from a verified earlier snapshot.");
    process.exit(1);
  }
...
function close() {
  ...
  // checkpoint; followers (and post-HA-shutdown nodes) skip silently.
  if (ha.isLeader()) {
    try {
      audit.checkpoint({ skipIfUnchanged: true });
    } catch (e) {
...
function _checkRollback(dataDirPath) {
  ...
  var tip;
  try {
    tip = jsonSafe.parse(fs.readFileSync(tipPath), { schema: AUDIT_TIP_SCHEMA });
  } catch (e) {
    logErr("FATAL: audit.tip unreadable or schema-invalid at " + tipPath + " — " + e.message);
