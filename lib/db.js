...
async function init(opts) {
  ...
  // Operator-driven retention purge (Phase 7) will drop these triggers
  // explicitly inside a transaction, perform the purge, and recreate them.
  ...
  var auditSigningMode = (opts.auditSigning && opts.auditSigning.mode)
    ...
    : (process.env.BLAMEJS_AUDIT_SIGNING_MODE || "wrapped");
  await auditSign.init({ dataDir: dataDir, mode: auditSigningMode });
  ...
  var audit = require("./audit");  // re-require — already required above; lazy-cache makes it a no-op
  var ckptResult = audit.verifyCheckpoints();
  if (!ckptResult.ok) {
    logErr("FATAL: audit checkpoint verification failed at row " +
      ckptResult.breakAt + " (" + ckptResult.reason + ")");
    logErr("  checkpoint _id: " + ckptResult.checkpointId);
    logErr("Refusing to boot. Either the audit-signing key was rotated " +
  ...
  audit.checkpoint({ skipIfUnchanged: true });
...
function close() {
  ...
  // anchors the most recent state.
  try {
    var audit = require("./audit");
    audit.checkpoint({ skipIfUnchanged: true });
  } catch (e) {
    logErr("close: final checkpoint failed: " + e.message);
  }
...
function _checkRollback(dataDirPath) {
  ...
  var raw, tip;
  try {
    raw = fs.readFileSync(tipPath, "utf8");
    tip = JSON.parse(raw);
  } catch (e) {
    logErr("FATAL: audit.tip is unreadable: " + e.message);
    logErr("Either delete it (forfeits rollback protection until next checkpoint) " +
      "or restore from operator backup.");
    process.exit(1);
  }
  if (!tip || typeof tip.atMonotonicCounter !== "number") {
    logErr("FATAL: audit.tip has malformed format (expected { atMonotonicCounter, ... })");
    process.exit(1);
  }
  ...
  if (currentMax < tip.atMonotonicCounter) {
    logErr("FATAL: audit-log rollback detected.");
    logErr("  audit.tip recorded counter: " + tip.atMonotonicCounter);
    logErr("  current DB max counter:     " + currentMax);
    logErr("Either the DB was restored from an older snapshot, or audit_log " +
...
async function _runNtpBootCheck(opts) {
  if (process.env.BLAMEJS_SKIP_NTP_CHECK === "1") return;
  var ntpCheck;
  ...
  try {
    ...
    logErr("ntp boot check threw unexpectedly: " + e.message + " (continuing)");
    return;
  }
  ...
  if (result.severity === "info") {
    ...
    logErr("ntp warning: " + result.message);
  } else if (result.severity === "fatal") {
    logErr("FATAL: ntp clock drift exceeds threshold: " + result.message);
    if (process.env.BLAMEJS_NTP_STRICT !== "0") {
      logErr("Refuse to boot. Investigate NTP / RTC / container time sync.");
      logErr("Override: BLAMEJS_NTP_STRICT=0 to continue (NOT recommended for production).");
      process.exit(1);
    }
  }
}
...
module.exports = {
  ...
  _writeAuditTip: function (tip) {
    ...
    fs.writeFileSync(tipPath, JSON.stringify(tip, null, 2), { mode: 0o600 });
  },
};
