...
async function init(opts) {
  ...
  if (frameworkTablesEnabled) {
    ...
    if (auditSigningEnabled && opts.acceptUnsignedPurgeAnchor === true) {
      var pinned = await auditTools().signExistingPurgeAnchor();
      if (pinned.signed) {
        log.warn("signed the existing unsigned audit purge anchor at counter=" +
          pinned.lastPurgedCounter + " under key " + pinned.publicKeyFingerprint +
          " — the boundary it claims is now pinned and cannot be moved. " +
          "This pins the boundary rather than proving it; verify the retained " +
          "archive against the chain if you need proof. " +
          "Remove acceptUnsignedPurgeAnchor from db.init — it does nothing now.");
      }
    }
    ...
    // Turning signing off does not make an anchor written while it was ON
    // unverifiable either — but it does mean no key is loaded, so the key
    // comes from the volume's own unsealed public-key history instead. That
    // reads public material only: no passphrase, and nothing is written to a
    // volume the operator opened without signing.
    var anchorPolicy = audit.setPurgeAnchorPolicy({
      allowUnsigned: !auditSigningEnabled,
      // A key that was never rotated is not in that history, so it can come
      // back empty for a key that exists. Throwing rather than returning null
      // is the difference between "this could not be checked" and "this names
      // a key nothing knows" — the second is a tampering alarm, and raising
      // one because the operator turned signing off would be a false one.
      resolvePublicKey: auditSigningEnabled ? undefined : function (fp) {
        var pem = auditSign.publicKeyFromHistory(dataDir, fp);
        if (!pem) {
          throw _dbErr("db/no-public-key-on-record",
            "audit signing is off and no public key is on record for " + fp);
        }
        return pem;
      },
...
module.exports = {
  ...
  purgeAuditChain:     async function (args) {
    ...
    if (cluster.isClusterMode()) {
      ...
      var perm = frameworkSchema.purgePermissionSql(dialect);
      if (perm) {
        // The guard stays installed and asks whether THIS connection may
        // delete. Permission and deletion therefore have to be the same
        // connection, which is what the transaction is for — and on Postgres
        // `SET LOCAL` ends with it, so the permission cannot outlive the
        // statements it was granted for.
        return await cs.transaction(async function (tx) {
          ...
          try {
            ...
            if (perm.revoke) await tx.execute(perm.revoke, []);
          }
        });
      }
      // SQLite has no session state a trigger can read, so there the guard
      // comes off for the deletion and goes back after.
      return await frameworkSchema.withDeleteTriggersSuspended({
        externalDbBackend: cluster.externalDbBackend(),
        dialect:           dialect,
      }, async function () {
        return await _clusterPurgeDelete(cs, lastPurgedCounter, dialect);
      });
    }
