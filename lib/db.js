...
var FRAMEWORK_SCHEMA = [
  ...
  },
  {
    ...
    columns: {
      ...
      // This row decides which rows are allowed to be missing, so it has to
      // carry the same standing as the rows it excuses. Unsigned, it was the
      // one thing in the chain an attacker could write freely: delete the rows,
      // delete the checkpoints covering them, insert an anchor naming any hash
      // and counter, and verification adopted that hash as the chain origin and
      // reported the volume intact. That is a cheaper attack than the relink-
      // and-forge one the chain already refuses, so it was not a partial
      // defense.
      //
      // Nullable because a volume purged by an earlier version has an anchor
      // with no signature. Such an anchor is no longer honored — see
      // auditChain.purgeAnchorPayload — but it must still be readable to say so.
      signature:            "BLOB",
...
async function init(opts) {
  ...
  // Audit-signing bootstrap. Runs BEFORE the chain verification below, which
  // needs the key to check a purge anchor's signature. Operator opt-out via
  // `auditSigning: false` skips it entirely; also implicitly skipped when
  // frameworkTables are off (no audit_log to sign checkpoints over).
  ...
  if (frameworkTablesEnabled) {
    // A purge anchor is signed, and checking that signature needs the signing
    // key — so signing is initialized ABOVE this rather than below it. With the
    // old order any previously purged volume failed its own boot verification:
    // the key was not loaded yet, the anchor could not be checked, and the
    // chain refused to verify.
    //
    // Where the operator has turned signing off, there is no key and never will
    // be. That deployment has opted out of signature-based tamper evidence
    // altogether — it has no checkpoints either — so the anchor is adopted as
    // it was before, with a warning saying the gap is being taken on trust.
    // Refusing to boot would punish them for a protection they declined, and
    // pretending the anchor was verified would be the false claim this release
    // exists to remove.
    var auditResult = await audit.verify(
      auditSigningEnabled ? {} : { allowUnsignedPurgeAnchor: true });
    if (!auditResult.ok) {
    ...
    // Say so when a missing prefix was accepted on trust rather than checked.
    // Otherwise the only thing an operator sees is "audit chain ok", which
    // reads identically whether the boundary was proved by a signature or
    // taken from a row anyone with write access could have authored — and the
    // rows below it are gone either way.
    if (auditResult.purgeAnchor && auditResult.purgeAnchor.honored &&
        auditResult.purgeAnchor.signatureVerified === false) {
      log.warn("WARNING: rows at or below audit counter " +
        auditResult.purgeAnchor.belowCounter + " are absent, on a purge anchor that " +
        "carries NO verifiable signature. It was accepted because audit signing is " +
        "disabled on this volume, so nothing here can establish who wrote that " +
        "boundary — an unsigned anchor can be authored by anyone able to write the " +
        "database. Turning auditSigning on does not settle THIS boundary: it cannot " +
        "sign an anchor written in the past, and a volume booted with signing on will " +
        "refuse an unsigned one. Re-run the purge against the retained archive to " +
        "write a signed anchor, then enable signing so later purges are proved.");
    }
  ...
  // (Signing is initialized above, before the chain verification that needs it
  // to check a purge anchor's signature.)
