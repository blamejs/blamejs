...
var FRAMEWORK_SCHEMA = [
  ...
  },
  {
    ...
    columns: {
      ...
      // And its covering checkpoint, which an archive is refused without.
      archiveCheckpointDigest: "TEXT",
      // And the manifest, which is how those payloads are opened. The two
      // digests above cover the encrypted bytes and say nothing about the
      // salts the decryption key is derived from, so editing salts.rows alone
      // left every signed value intact while the bundle stopped decrypting.
  ...
  },
  {
    ...
    // AAD-bound to (table, row, column). `ownerId` and `scopes` are what
    // `verify()` returns for a consumer to decide what a credential may do, so
    // a cell that can be moved between rows makes database write access an
    // escalation rather than a disclosure -- one credential's scope transplanted
    // onto another credential's row -- and sealing the column was supposed to
    // mean write access was not enough. Under the plain envelope the binding
    // arrived only when a compliance posture happened to be pinned, which is a
    // decision about a whole deployment taken for unrelated reasons.
    aad:           true,
    rowIdField:    "id",
  },
  {
...
function _tmpDirResidencyIssue(tmpDir, platform, realpath, readMountInfo) {
  ...
  // The kernel is asked what is mounted here, rather than the path's NAME being
  // compared against a list of places tmpfs is usually found. The name cannot
  // answer this, and it was wrong in both directions:
  //
  //   /tmpdata, /dev/shmx, /run/shm-backup   passed — an unanchored prefix
  //                                          matches any name that starts with
  //                                          one of those strings
  //   a /tmp that is ordinary disk           passed SILENTLY — no refusal, no
  //                                          warning — and that is an entirely
  //                                          normal host
  //
  // The second is the one that mattered: the whole decrypted database was
  // written where backups, snapshots, replicas and disk images reach it, which
  // is the threat atRest: "encrypted" exists to address, and the operator was
  // told the residency was fine. The paragraph above, on the non-Linux branch,
  // already says a name comparison cannot see whether a path is disk-backed.
  var entries = null;
  ...
  // bestMatch is the longest mount point that is a path-prefix of realTmp, and
  // it already requires a path separator at the boundary — so /tmpdata does not
  // match the /tmp mount. A directory somebody created by hand is not a mount
  // point, so it resolves to whatever filesystem contains it, which is the
...
async function _initReleasingClaimOnFailure(opts) {
  try {
    ...
    //
    if (initialized) {
      // The handle goes live BEFORE the boot verifications, so audit and
      // consent can read the chain through the public surface — which means a
      // refusal throws from here with the database open. `init` never
      // returned, so the caller holds nothing to close it with: leaving it
      // would keep a database this boot just refused reachable through the
      // public API, and under `atRest: "encrypted"` would leave its decrypted
      // working copy in tmpDir. A full close() is the right shutdown for
      // that — it re-seals and then removes the plaintext, where deleting the
      // files out from under a live handle would turn a refusal into a
      // corruption.
      //
      // Without its final checkpoint, though. A checkpoint is a signed
      // statement about the state it anchors, and this state was just
      // refused: writing one here would leave the next boot a fresh anchor
      // over the tampering that stopped this one. Measured, the attempt
      // currently writes nothing on this path anyway — but that is a race
      // with the synchronous teardown below, not a decision, and it is one
      // refactor away from landing.
      try { _shutdown(false); } catch (_e) { /* the caller's error is the refusal, not this */ }
      // The working copy is NOT force-removed here when that shutdown could
      // not re-encrypt, and the distinction is deliberate. A refusal does not
      // mean nothing was written: migrations and the schema reconcile commit
      // BEFORE the verifications run, so on this path the copy can hold work
      // that db.enc does not, and discarding it would lose committed
      // migrations while leaving the stale encrypted volume in place — the
      // next boot would then re-run migration steps it has no record of having
      // applied. Data outranks the cleanup.
      //
      // The plaintext that remains is not orphaned, only unclaimable by THIS
      // process: its owner record names a live pid, so the sweep leaves it
      // alone until the process exits, after which a later boot reclaims it or
      // decryptToTmp adopts it as the newer copy. `_shutdown` logs the failed
      // re-encryption, so the window is announced rather than silent. That is
      // the same posture a close() that cannot re-encrypt already takes.
    } else if (workingCopyClaimed) {
      // Never finished, so nothing above applies — but a claimed working copy
      // is a decrypted database with nothing left to remove it.
...
async function init(opts) {
  ...
  // Tables whose sealed columns may still be read under the unbound envelope
  // while an upgrade re-seals them. Empty by default: every table is read
  // strictly unless the operator names it here for the length of a migration.
  ...
  for (var i = 0; i < fullSchema.length; i++) {
    ...
    cryptoField.registerTable(t.name, {
      ...
      //
      // A FRAMEWORK table cannot declare the window for itself. The window is
      // a hole for as long as it is open -- an unbound cell is exactly what an
      // attacker with write access would place -- so how long it stays open is
      // the operator's decision about their own data, taken once during an
      // upgrade and then closed, not a default this framework ships open
      // forever. `db.init({ allowPlainSealMigration: [...] })` names the tables
      // it applies to.
  ...
  // call db.prepare() through the public surface. A verify that refuses the
  // boot throws from here with the handle already live, and the caller never
  // receives it — so the throw is caught at the boundary (see
  // _initReleasingClaimOnFailure), which closes what this init opened rather
  // than leaving a refused database reachable.
  initialized = true;
...
// The shutdown body. `anchorCheckpoint` is false for exactly one caller: the
// boot that just REFUSED this database. A checkpoint is a signed statement
// about the state it anchors, and writing one over a state the verify rejected
// would hand the next boot a fresh anchor for the tampering that stopped this
// one — laundering the refusal instead of preserving it.
...
// A numeric clock setting read from the environment is refused when it is not
// one, rather than passed on as whatever parseInt returned. `parseInt("abc")`
// is NaN, and NaN reached the boot check as a timeout — where the opts schema
// refuses it, so the ENTIRE clock check was skipped and the boot continued. The
// two drift thresholds were quieter still: setThresholds' failure is caught and
// logged at debug, so a mistyped threshold left the registered value in place
// and said nothing an operator would see. A typo must not change clock policy
// in silence.
//
// Bounded rather than open-ended: fifteen digits is more milliseconds than any
// threshold means, and a value that long is a mistake rather than a policy.
...
async function _runNtpBootCheck(opts) {
  // Read the requirement FIRST, because two early returns sit above the place
  // it used to be consulted: the skip flag, and a checker that will not load.
  // Either one returned before the requirement was reached, so a deployment
  // that asked for authenticated time booted without it and said nothing. A
  // requirement any earlier return can step over is not a requirement.
  ...
  if (safeEnv.readVar("BLAMEJS_SKIP_NTP_CHECK", { default: "" }) === "1") {
    // Two settings that contradict each other, resolved toward the one asking
    // for a guarantee: a skip cannot satisfy a requirement to authenticate.
  ...
  // The two NTS knobs. Both were declared in the shipped compose files and read
  // by nothing at all — an operator who set BLAMEJS_NTS_SERVERS got exactly the
  // unauthenticated check they were trying to replace, and no signal that the
  // setting had done nothing. Same shape as the NTP message that named a
  // variable no code consulted.
  // envNtsRequire is read at the top of this function, because two early
  // returns above would otherwise skip the requirement it carries.
  ...
  if (envWarn || envFatal) {
    // Zero is meaningful on both thresholds: it disables that one.
    var thr = {};
    ...
    // Each value is already a whole number of milliseconds, so a throw here is
    // the primitive refusing the COMBINATION — a warn threshold above the fatal
    // one, which is a policy that cannot be satisfied. Surfaced rather than
    // absorbed at debug, which is how a mistyped threshold used to vanish while
    // the registered defaults quietly stayed in force.
  ...
  try {
    ...
    // Continuing is right for a check that merely could not answer — but not
    // when the operator asked to be refused, and BOTH refusals live below this
    // point. A throw here returned before either was reached, so a malformed
    // BLAMEJS_NTS_SERVERS entry silently downgraded a deployment that had
    // required authenticated time into booting without it. A check that did
    // not complete has not satisfied a requirement; it is the same answer as
  ...
  // Asked BEFORE severity is considered, because "I require authenticated time"
  // is a different question from "how far off is the clock". Disabling the
  // unauthenticated fallback was not enough on its own: with every NTS query
  // failed, bootCheck reports an unreachable WARNING with ok:true, so a
  // deployment that explicitly required authenticated time booted anyway
  // unless a second, unrelated variable happened to be set as well. That is
  // the same defect as a message naming a setting nothing reads — the knob
  // existed, was documented as fail-closed, and did not close.
  ...
  if (result.severity === "info") {
    ...
    // An unreachable server leaves the clock unchecked, and this is the only
    // place that can refuse the boot for it. The message used to name
    // BLAMEJS_NTP_STRICT, which is read on the FATAL branch below and already
    // defaults to on — so an operator following the advice set a variable to
    // the value it held, on a branch that never consults it, and a host with
    // UDP/123 blocked went on booting unchecked indefinitely while being told
    // it had a way to prevent exactly that.
    //
    // Opt-in, not default-on: a network without outbound NTP is ordinary, and
    // refusing every such host would be a boot failure nobody asked for. Only
    // `driftMs === null` qualifies — a drift reading inside the thresholds is
    // also a warning, and the clock WAS checked there.
