...
async function init(opts) {
  ...
  if (immutableOpen) {
    var walBytes = 0;
    try { walBytes = nodeFs.statSync(dbPath + "-wal").size; }
    catch (_e) { walBytes = 0; }
    if (walBytes > 0) {
      throw _dbErr("db/immutable-pending-wal",
        "db.init: immutable was requested but '" + dbPath + "-wal' holds " + walBytes +
        " byte(s). An immutable open does not read the write-ahead log, so any " +
        "transaction committed only there would be silently missing. A clean close " +
        "removes this file, so the volume was not closed cleanly. Open it once " +
        "writable to checkpoint and remove the log, or drop immutable and open it " +
        "read-only on a mount that permits the -wal / -shm pair.");
    }
  }
