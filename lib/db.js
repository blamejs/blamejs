...
//   null   — no ownership record, or one that cannot be read.
//
// The liveness question itself belongs to b.pidProbe, which owns the
// EPERM-means-alive / ESRCH-means-dead classification and the capped,
// symlink-refusing read of the sidecar. A process id from another id namespace
// that matches nothing here reads as dead, and that is the one wrong answer
// this can give: it costs a leaked file in a temporary directory, where the
// opposite mistake destroyed a running deployment's data.
function _tmpDbOwnerAlive(workingCopyPath) {
  var pid = pidProbe.readPidFile(workingCopyPath + OWNER_SUFFIX);
  if (pid === null) return null;                       // no record; owner unknowable
...
async function init(opts) {
  ...
  if (atRest === "encrypted") {
    ...
    atomicFile.writeSync(dbPath + OWNER_SUFFIX, Buffer.from(process.pid.toString(10), "utf8"));
    decryptToTmp();
  } else {
    // plain mode
