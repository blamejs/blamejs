...
module.exports = {
  ...
  purgeAuditChain:     async function (args) {
    ...
    if (cluster.isClusterMode()) {
      ...
      if (frameworkSchema.wormGuardIsAnchorBounded(dialect)) {
        // The guard stays installed and decides per row against the boundary
        // the anchor already records — and the anchor is written before this
        // runs, so it permits exactly these rows and nothing else. There is
        // no permission to grant and no window where the table is unguarded.
        //
        // Still one transaction: the rows and the checkpoints covering them
        // are two statements, and a failure between them would leave
        // checkpoints attesting to rows that no longer exist.
        return await cs.transaction(async function (tx) {
          return await _clusterPurgeDelete(tx, lastPurgedCounter, dialect);
        });
      }
      // The other dialects cannot express that condition in a trigger body, so
      // there the guard comes off for the deletion and goes back after.
      return await frameworkSchema.withDeleteTriggersSuspended({
        externalDbBackend: cluster.externalDbBackend(),
        dialect:           dialect,
      }, async function () {
        // One transaction here for the same reason as the branch above: the
        // rows and the checkpoints covering them are two statements, and a
        // failure between them leaves checkpoints attesting to rows that are
        // gone — with the anchor already advanced, so re-running the same
        // archive is refused by the contiguity guard and the state is awkward
        // to repair. The suspension around it is DDL, which commits on its own
        // and is unaffected by the transaction inside it.
        return await cs.transaction(async function (tx) {
          return await _clusterPurgeDelete(tx, lastPurgedCounter, dialect);
        });
      });
