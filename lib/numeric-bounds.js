...
// Non-negative integer that JavaScript can still tell apart from its
// neighbours. Distinct from isNonNegativeFiniteInt, which accepts the range
// above 2^53 where a BIGINT read out of a database collapses onto a Number
// shared with other values.
//
// Use this wherever the number is rendered into bytes that are then signed,
// hashed, or compared for identity — a chain counter, a purge boundary, an
// anchored tip. There, two inputs landing on one Number means two different
// statements producing one signature, so a value in that range has to be
// refused rather than approximated. A plain magnitude bound (a size, a
// timeout, a retry count) does not need this and should keep using
// isNonNegativeFiniteInt.
function isNonNegativeSafeInt(value) {
  return typeof value === "number" && Number.isSafeInteger(value) && value >= 0;
}

// The same bound for a value the caller is about to COUNT PAST: a purge
// boundary, a resolved chain origin, any floor whose whole purpose is that the
// next number comes after it. MAX_SAFE_INTEGER itself passes the check above
// and then produces an unsafe successor, so the boundary has to stop one short
// — otherwise the first counter after it aliases another and the chain reuses
// or skips one, which is the failure isNonNegativeSafeInt exists to prevent,
