...
function _bufferedClamVerdict(collector) {
  ...
  return _classifyClamReply(reply);
}
 ...
 *   - `errorCode` / `errorMessage`: present on `verdict: "error"`, naming
 *     which error it was. On the clamav-instream backend,
  ...
  function _scanClamavInstream(messageBytes, scanOpts) {
    return new Promise(function (resolve, reject) {
      ...
      sock.on("end", function () {
        ...
        resolve(classified !== null ? classified : {
          verdict:      "error",
          threats:      [],
