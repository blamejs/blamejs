...
var PROFILES = Object.freeze({
  strict:     { timeoutMs: C.TIME.seconds(30),  maxMessageBytes: C.BYTES.mib(25) },                // allow:raw-byte-literal — operator-facing default mailbox cap
  balanced:   { timeoutMs: C.TIME.seconds(60),  maxMessageBytes: C.BYTES.mib(50) },                // allow:raw-byte-literal — operator-facing default mailbox cap
  permissive: { timeoutMs: C.TIME.seconds(120), maxMessageBytes: C.BYTES.mib(150) },               // allow:raw-byte-literal — operator-facing default mailbox cap
});
...
function create(opts) {
  ...
  function _scanIcap(messageBytes, scanOpts) {
    return new Promise(function (resolve, reject) {
      var sock = (scanOpts._socket && typeof scanOpts._socket.write === "function")
        ? scanOpts._socket
        : nodeNet.createConnection({ host: opts.host, port: opts.port });
  ...
  function _scanClamavInstream(messageBytes) {
    return new Promise(function (resolve, reject) {
      var sock = nodeNet.createConnection({ host: opts.host, port: opts.port });
      var chunks = [];
      var done = false;
      var to = setTimeout(function () {
        if (done) return;
      ...
      sock.on("end", function () {
        ...
        clearTimeout(to);
        var reply = Buffer.concat(chunks).toString("utf8").replace(/[\r\n\0]+$/g, "");             // allow:regex-no-length-cap — trailing-trim anchored
