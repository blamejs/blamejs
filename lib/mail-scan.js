...
var CLAMAV_REPLY_EXCERPT_CHARS = 200;

var CLAMAV_REPLY_ELLIPSIS = "...";

function _clamReplyExcerpt(reply) {
  var s = reply === undefined || reply === null ? "" : String(reply);
  if (s.length <= CLAMAV_REPLY_EXCERPT_CHARS) return s;
  return s.slice(0, CLAMAV_REPLY_EXCERPT_CHARS - CLAMAV_REPLY_ELLIPSIS.length) +
    CLAMAV_REPLY_ELLIPSIS;
}
...
function _clamErrorVerdict(reply) {
  ...
  return {
    ...
    errorMessage: _clamReplyExcerpt(reply),
  };
}
...
function _icapVerdictFrom(parsed) {
  ...
  return {
    verdict:      parsed.threatFound ? "infected"
 ...
 *   - `errorCode` / `errorMessage`: naming which error it was, on a
 *     clamav-instream error verdict and on any scan the framework itself
 *     failed. An ICAP error verdict carries neither: it comes from the
 *     response status, which `icapResponse` already reports. On the
 *     clamav-instream backend,
