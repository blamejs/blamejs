...
async function deriveWrappingKey(passphrase, salt, argonParams) {
  ...
  if (pwBuf.length === 0) {
    if (weOwnPwBuf) bufferSafe.secureZero(pwBuf);
...
async function wrap(plaintext, passphrase, opts) {
  ...
  try {
    ...
    bufferSafe.secureZero(wrappingKey);
    if (weOwnPlaintextBuf) bufferSafe.secureZero(plaintextBuf);
  }
}
