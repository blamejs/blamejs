...
// dualEmitter(opts) — an emitter that writes the framework trail AND the
// operator's own sink, for a primitive that documents `audit` as an ADDITIONAL
// reader rather than a replacement.
//
// The deduplication lives here rather than in emitToSink, because it is only a
// duplicate when both writes happen. `audit: b.audit` names the module emit()
// already writes through, so a caller doing both appends every event twice --
// two chain rows, two of whatever an operator alerts on, and nothing
// downstream can tell the pair from a genuine repeat. But a caller that emits
// ONLY to the sink (the archive readers) has that same option as its single
// path to the trail, and suppressing it there loses the event rather than
// deduplicating it. Same expression, opposite meaning, decided by whether the
// caller also emits.
//
// Identity, not shape: a wrapper an operator writes around b.audit is a
// distinct sink and still receives its events.
