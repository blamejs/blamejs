...
var _RAW_INSERT_RE = /^\s*(?:INSERT|REPLACE)\s+(?:OR\s+[A-Za-z]+\s+)?INTO\s+["'`]?([A-Za-z_]\w*)["'`]?\s*\(([^)]+)\)\s*VALUES\s*\(([\s\S]+)\)\s*;?\s*$/i;
var _RAW_UPDATE_RE = /^\s*UPDATE\s+["'`]?([A-Za-z_]\w*)["'`]?\s+SET\s+([\s\S]+?)(?:\s+WHERE\b[\s\S]*)?\s*;?\s*$/i;
var _RAW_TABLE_RE = /^\s*(?:INSERT|REPLACE)\s+(?:OR\s+[A-Za-z]+\s+)?INTO\s+["'`]?([A-Za-z_]\w*)|^\s*UPDATE\s+["'`]?([A-Za-z_]\w*)/i;
...
function _assertRawWriteResidency(sql, boundParams) {
  ...
  if (mi) {
    ...
    var assigns = _splitTopLevelCommas(mu[2]);
