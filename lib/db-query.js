...
class Query {
  ...
  _addCondition(field, op, value) {
    ...
    if (this._isSealedField(field)) {
      var lookup = fieldCrypto.lookupHash(this._table, field, value);
      if (!lookup) {
        throw new Error(
    ...
    fieldCrypto && _validateField(field);
  ...
  _isSealedField(field) {
    var sealed = fieldCrypto.getSealedFields(this._table);
  ...
  first() {
    ...
    return row ? fieldCrypto.unsealRow(this._table, row) : null;
  }
  ...
  all() {
    ...
    for (var i = 0; i < rows.length; i++) {
      out[i] = fieldCrypto.unsealRow(this._table, rows[i]);
    }
    return out;
  }
  ...
  insertOne(row) {
    ...
    var sealed = fieldCrypto.sealRow(this._table, withId);
  ...
  _update(changes, single) {
    ...
    var sealed = fieldCrypto.sealRow(this._table, changes);
...
function _validateField(field) {
  if (typeof field !== "string" || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(field)) {
    throw new Error("invalid field name: '" + field + "' (must match [A-Za-z_][A-Za-z0-9_]*)");
  }
}
