...
function _dearmorMessage(armored) {
  var matchResult = armored.match(/-----BEGIN PGP MESSAGE-----\r?\n(?:[^\r\n]+\r?\n)*\r?\n([\s\S]+?)\r?\n-----END PGP MESSAGE-----/);
  if (!matchResult) {
    throw new MailCryptoError("mail-crypto/pgp/bad-armor",
      "dearmor: envelope is not BEGIN PGP MESSAGE armored");
  }
  return Buffer.from(matchResult[1].replace(/\r?\n/g, ""), "base64");
}
 ...
 * @since      0.10.16
 * @status     experimental
 * @related    b.mail.crypto.pgp.experimental.wkd.computeUrl
 *
...
function wkdFetch(email, opts) {
  ...
  var maxBytes = (typeof opts.maxKeyBytes === "number" && opts.maxKeyBytes > 0)
    ? opts.maxKeyBytes : (256 * 1024);                                                                // allow:raw-byte-literal — 256 KiB default key cap
