 ...
 *       signatures. Keys < 2048 bits are refused at sign and verify
 *       time: `b.mail.dkim.RSA_MIN_BITS`, the framework RSA floor, which
 *       follows the RFC 8301 §3.2 recommendation and applies across the
 *       mail surface.
 *
 *   Threat model:
 ...
 * 2048 bits, the size RFC 8301 §3.2 recommends, are refused.
 *
 * @example
...
function sign(opts) {
  ...
  var keyType = privateKey.asymmetricKeyType;
...
function wkdFetch(email, opts) {
  ...
  return Promise.resolve(opts.httpsGet(urls.direct)).then(function (resp) {
    if (resp && resp.status === C.HTTP.STATUS.OK && Buffer.isBuffer(resp.body) && resp.body.length > 0) {
    ...
    return Promise.resolve(opts.httpsGet(urls.advanced)).then(function (resp2) {
      if (resp2 && resp2.status === C.HTTP.STATUS.OK && Buffer.isBuffer(resp2.body) && resp2.body.length > 0) {
