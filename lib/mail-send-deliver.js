...
// The retry ramp, and with it the give-up time. RFC 5321 §4.5.4.1: "the
// give-up time generally needs to be at least 4-5 days" — a receiver may be
// down for a maintenance weekend, and a sender that bounces before then loses
// mail that would have been delivered.
//
// The ten waits below sum to 97h21m — four days and an hour — and the attempt
// count is the ramp's length PLUS ONE, so all ten are used. It used to stop
// after the first four (81 minutes), because the attempt count defaulted to
// this array's LENGTH, and N attempts have only N-1 waits between them, so the
// longest rung was unreachable and the ramp described a schedule the queue
// never ran. The total is asserted against the RFC floor in
// test/layer-0-primitives/mail-send-deliver.test.js rather than left to be
// re-added by hand from the rungs.
// Frequent early (a transient blip clears in minutes), then daily (a receiver
// down for a weekend is not helped by hammering it).
...
function _classifySmtpOutcome(err, response) {
  ...
  if (err) {
    ...
    //
    // Both of these read the framework's own error CODE and never the message.
    // A message carries the peer's reply text, so matching words in it lets the
    // remote side choose its own classification: "451 4.7.0 REQUIRETLS policy
    // service temporarily unavailable" is a request to retry, and reading
    // REQUIRETLS out of it turned that into "this host declines the extension".
    // The permanent arm below is the dangerous direction — a transient refusal
    // whose text happens to name the policy would bounce deliverable mail. A
    // code is assigned here, at the point the condition is actually detected.
...
// The DSN `Status:` field for a peer reply. RFC 3464 §2.3.4 makes it an
// ENHANCED status — `class.subject.detail` — not the three-digit SMTP reply,
// which belongs in Diagnostic-Code and is already carried there inside the
// transport's reason string. Putting the bare reply in Status produces
// `Status: 550`, which a conforming parser rejects or misreads, so the reply's
// CLASS becomes the enhanced code and the detail stays "other or undefined"
// (§3.3) rather than being invented.
...
function _buildDsnMessage(opts) {
  ...
  // `originalHeaders` takes either the quoted text or `{ text, charset }`. A
  // message off the wire is not guaranteed to be valid UTF-8, and the part has
  // to declare which reading it was quoted under: text/* defaults to us-ascii
  // (RFC 2046 §4.1.2), so an undeclared 8-bit header block is read wrongly by
  // whatever opens the report.
...
async function _deliverOne(envelope, recipient, ctx) {
  ...
  for (var i = 0; i < mxs.length; i += 1) {
    ...
    try {
      ...
      // Only this framework's own code settles the REQUIRETLS question, and
      // only when the envelope actually asked for the extension. Scanning the
      // message instead let a peer's own wording decide: a transient "451
      // REQUIRETLS policy service temporarily unavailable" counted as the host
      // declining to offer it, and for a single-MX domain that was the whole
      // evidence set, so the exhaustion branch below promoted a retryable
      // failure to a permanent 5.7.30 bounce.
...
function create(opts) {
  ...
  // The ramp is resolved FIRST, because the attempt count is derived from it.
  // It was derived from the default constant's length regardless of what the
  // caller supplied, so an operator who passed a two-rung ramp still got five
  // attempts and had their last wait repeated three times.
  ...
  // One more attempt than there are waits: N attempts are separated by N-1
  // waits, so a count equal to the ramp's length leaves its last rung — the
  // longest one, and the whole reason the ramp reaches days — unreachable.
    ...
    for (var i = 0; i < envelope.to.length; i += 1) {
      ...
      if (res.outcome === "transient") {
        ...
        if (attempts >= maxAttempts) {
          ...
          // The status has to convert with it. RFC 3463 §3.1 makes class 4 a
          // PERSISTENT TRANSIENT failure — "try again later" — and this DSN
          // says Action: failed. A bounce carrying `Status: 4.x.y` tells the
          // reader's parser the opposite of what the report is for. Only the
          // class changes: what went wrong is still what the last host said,
          // and the subject/detail were never more specific than "other".
  ...
  // The resolved retry policy, so an operator can see when this queue gives up
  // rather than deriving it from two options and an off-by-one. `giveUpMs` is
  // the sum of the waits actually used: N attempts have N-1 waits between them.
  deliver.retryPolicy     = function () {
    // Summed the way the queue actually waits: N attempts have N-1 waits, and
    // the schedule CLAMPS to the last rung once it runs past the ramp
    // (Math.min(attempts - 1, backoffMs.length - 1) below). Slicing the ramp
    // instead reported only the rungs it holds, so an operator who set
    // maxAttempts above the ramp's length — nine waits against a two-rung
    // ramp — was told a give-up time far shorter than the one they had
    // configured, from the very surface added to make it legible.
    var total = 0;
...
// Extract the header block (everything before the blank line that ends it) for
// inclusion in the DSN per RFC 3462 §3. Returns `{ text, charset }`.
//
// The separator's width is carried out of the match rather than assumed: it is
// two octets when the message uses CRLF and one when it uses bare LF, so a
// fixed `+ 2` made the two spellings of one message disagree about whether the
// terminating blank line is part of the block.
//
// The search runs over BYTES for the same reason it is not decoded first.
// `toString("utf8")` substitutes U+FFFD for every sequence it cannot read
// rather than reporting one, and a message off the wire is not guaranteed to be
// valid UTF-8: a legacy 8-bit header, or a raw byte inside an unencoded
// parameter, survives transmission. A bounce quoting a header the sender never
// wrote is worse than one quoting an awkward encoding, because the quoted
// header is the part a person reads to work out what happened.
var HEADER_SEP_CRLF = Buffer.from("\r\n\r\n", "ascii");
var HEADER_SEP_LF   = Buffer.from("\n\n", "ascii");
var BASE64_LINE_CHARS = 76;                                                                          // RFC 2045 §6.8 line length
...
function _extractHeaderBlock(raw) {
  ...
  // The report is assembled as a string and written out as UTF-8, so a block
  // that is not UTF-8 cannot travel through it as text: decoding each octet to
  // its own code point preserves the value inside this process and then emits
  // two bytes for every one above 0x7F, which is neither the original octets
  // nor what a `charset=iso-8859-1` label would promise. Base64 (RFC 2045 §6.8)
  // carries the bytes exactly through a US-ASCII pipeline, and the part says so
