...
function _isNullReversePath(from) {
  if (typeof from !== "string") return true;
  var trimmed = from.trim();
  return trimmed === "" || trimmed === "<>";
}

async function _applyMtaStsPolicy(domain, mxs, policyMode, auditEmit, resolver) {
  if (policyMode === "off") return mxs;
  var sts;
  try {
    sts = await smtpPolicy().mtaSts.fetch(domain, { resolver: resolver || undefined });   // allow:raw-outbound-http-framework-internal — method call on b.network.smtp.policy wrapper, not a raw `fetch(`
  } catch (e) {
...
async function _deliverOne(envelope, recipient, ctx) {
  ...
  try {
    mxs = await _applyMtaStsPolicy(domain, mxs, ctx.policy.mtaSts, ctx.auditEmit, ctx.resolver);
  } catch (e) {
    return { recipient: recipient, outcome: "permanent",
 ...
 *                                          //   Every lookup made on the sender's behalf goes through it: the MX
 *                                          //   records, the `_mta-sts` TXT record and the DANE TLSA records
 *
 *   policy: {
...
function create(opts) {
  ...
  async function deliver(envelope) {
    ...
    for (var i = 0; i < envelope.to.length; i += 1) {
      ...
      var dsnSent = false;
      if (dsnOpts && !_isNullReversePath(envelope.from)) {
        try {
