...
// Anything outside ASCII alphanumeric becomes a separator, on the default
// path. Written as a scan: the question is one character wide, so a pattern
// buys nothing, and `codepointClass.isAsciiAlnum` is the same test every
// other module in the framework asks.
//
// The two Unicode classes above stay as patterns. They resolve general
// categories from the tables the engine ships, which track its Unicode
// version; a hand-kept copy would be a snapshot that drifts, and a slug that
// silently keeps a punctuation mark or drops a letter is worse than one
// character class. Neither can backtrack: both are a single class with a
// repeat, matched in one pass.
function _asciiAlnumToSeparator(s, sep) {
  ...
  for (var i = 0; i < s.length; i += 1) {
    if (codepointClass.isAsciiAlnum(s.charCodeAt(i))) {
      ...
      // A run of non-alphanumerics collapses to one separator, which is what
      // the `+` did.
      out += sep;
      inRun = true;
    }
