...
function readNode(buf, offset) {
  ...
  if ((lenByte & 0x80) === 0) {
    ...
    if (length < 128 || buf[offset + headerLen] === 0) {
...
function readNodeStrict(buf) {
  var node = readNode(buf, 0);
  if (node.totalLength !== buf.length) {
    throw new Asn1Error("asn1/trailing-data",
      "trailing bytes after the top-level DER node (" + node.totalLength +
      " of " + buf.length + " bytes consumed)");
  }
  return node;
}

function readCertificateTbsFields(certDer) {
  var top = readNode(certDer, 0);
  if (top.tag !== TAG.SEQUENCE) {
    throw new Asn1Error("asn1/not-a-certificate", "certificate is not a SEQUENCE");
  }
  var certKids = readSequence(top.value);
  if (certKids.length === 0) {
    throw new Asn1Error("asn1/not-a-certificate", "certificate has no tbsCertificate");
  }
  var tbsKids = readSequence(certKids[0].value);
  var fieldBase = 0;
  if (tbsKids.length > 0 && tbsKids[0].tagClass === TAG_CLASS.CONTEXT_SPECIFIC &&
      tbsKids[0].tag === 0) {
    fieldBase = 1;
  }
  return {
    tbsKids:      tbsKids,
    fieldBase:    fieldBase,
    serialNumber: tbsKids[fieldBase],
    issuer:       tbsKids[fieldBase + 2],
    subject:      tbsKids[fieldBase + 4],
    spki:         tbsKids[fieldBase + 5],
  };
}
...
module.exports = {
  ...
  TAG:                 TAG,
  readNode:            readNode,
  readNodeStrict:      readNodeStrict,
  readCertificateTbsFields: readCertificateTbsFields,
