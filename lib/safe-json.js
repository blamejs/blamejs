...
// True iff `value` is a plain JSON object — not null, not an array, not a
// scalar. safeJson.parse accepts the literal `null` and scalars/arrays (all
// valid JSON documents), so a parsed header / claims set / document must be
// re-checked before its fields are dereferenced; this is that check, shared so
// the `!x || typeof x !== "object" || Array.isArray(x)` idiom isn't re-rolled.
