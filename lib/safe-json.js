...
function _patternMatcher(pattern) {
  ...
  try {
    ...
    try {
      ...
      // Compiled so that assertSafe can screen the COMPILED form, per the
      // comment above: a source-only screen misreads `(a|A)+` under `i`. The
      // pattern is the operator's schema `pattern` keyword — it is the input
      // being validated, not the implementation of a screen — and it is refused
      // before it ever runs against a value.
