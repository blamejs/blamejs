 ...
 * already-decoded plain object, returned by identity once `measureBytes`
 * confirms its JSON form fits `maxBytes` and `maxDepth`. This is the recurring
...
function parseStringOrObject(input, opts) {
  ...
  if (input !== null && typeof input === "object" &&
    ...
    var sized;
    try {
      sized = measureBytes(input, {
        limit: maxBytes, maxDepth: opts.maxDepth, followToJson: false,
      });
    } catch (e) {
      if (typeof opts.errorClass === "function") {
        throw new opts.errorClass(opts.jsonCode, label + ": invalid JSON — " + (e && e.message));
    ...
    if (sized.exceeded) {
      var tooLarge = new SafeJsonError(
        "input exceeds maxBytes (" + maxBytes + ")", "json/too-large");
      if (typeof opts.errorClass === "function") {
        throw new opts.errorClass(opts.jsonCode, label + ": invalid JSON — " + tooLarge.message);
      }
      throw tooLarge;
    }
    return input;
...
function _resolveForJson(value, key, followToJson) {
  var t = typeof value;
  if (followToJson &&
      ((value !== null && (t === "object" || t === "function")) || t === "bigint")) {
    var hook = value.toJSON;
    if (typeof hook === "function") value = hook.call(value, key);
  }
  if (followToJson && value !== null && typeof value === "object") {
...
function _accumulate(value, state, depth, stack) {
  ...
  if (Array.isArray(value)) {
    ...
    for (i = 0; i < length; i += 1) {
      ...
      if (!_accumulate(_resolveForJson(value[i], String(i), state.followToJson),
                       state, depth + 1, stack)) {
        _addBytes(state, 4);
      }
    }
    ...
    for (i = 0; i < keys.length; i += 1) {
      var member = _resolveForJson(value[keys[i]], keys[i], state.followToJson);
 ...
 * `json/too-deep` past `opts.maxDepth`, and `json/wrong-input-type` on a
 * BigInt, matching what `JSON.stringify` refuses. A `toJSON` hook or a
 * wrapper coercion that throws propagates its own error, as it does
 * through `JSON.stringify`.
 *
 * A caller bounding what it is about to WORK on rather than what it is
 * about to write passes `followToJson: false`. `toJSON` changes what
 * `JSON.stringify` emits and changes nothing about the value itself, so
 * a hook returning `{}` reports two bytes for an object that still holds
 * megabytes, and a cap read off that admits every one of them.
 *
 * @opts
 *   limit:        number,   // stop counting past this; default 64 MiB, no ceiling
 *   maxDepth:     number,   // default 100; capped at 1000
 *   followToJson: boolean,  // default true: call toJSON as JSON.stringify does
 *
 * @example
...
function measureBytes(value, opts) {
  ...
  var state = {
    ...
    followToJson: opts.followToJson !== false,
  };
  try {
    if (!_accumulate(_resolveForJson(value, "", state.followToJson), state, 0, [])) {
