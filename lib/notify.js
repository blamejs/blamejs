...
function _validateTransport(name, t) {
  if (!t || typeof t !== "object") {
    throw _err("notify/bad-opt", "notify: transport for channel '" + name +
...
function _validateChannelEntry(name, entry) {
  if (typeof entry !== "object" || entry === null) {
    throw _err("notify/bad-opt", "notify: channel '" + name + "' must map to a transport or { transport, ... } object");
  }
  // Two accepted shapes:
  ...
  if (entry.transport === undefined) {
    throw _err("notify/bad-opt", "notify: channel '" + name +
  ...
  if (entry.retry !== undefined && (typeof entry.retry !== "object" || entry.retry === null)) {
    throw _err("notify/bad-opt", "notify: channel '" + name + "' retry must be a b.retry.withRetry opts object");
  }
  if (entry.breaker !== undefined && entry.breaker !== null && typeof entry.breaker !== "object") {
    throw _err("notify/bad-opt", "notify: channel '" + name + "' breaker must be a b.retry.CircuitBreaker opts object or null");
  }
  if (entry.timeoutMs !== undefined && entry.timeoutMs !== 0 && !_isFiniteNonNegative(entry.timeoutMs)) {
    throw _err("notify/bad-opt", "notify: channel '" + name + "' timeoutMs must be a non-negative finite number (0 disables)");
  }
  if (entry.serialize !== undefined && typeof entry.serialize !== "boolean") {
    throw _err("notify/bad-opt", "notify: channel '" + name + "' serialize must be a boolean");
  }
  return {
...
function _validateCreateOpts(opts) {
  ...
  if (!opts.channels || typeof opts.channels !== "object") {
    throw _err("notify/bad-opt", "notify.create: channels must be an object mapping channel names to transports");
  }
  var keys = Object.keys(opts.channels);
  if (keys.length === 0) {
    throw _err("notify/bad-opt", "notify.create: channels object must have at least one channel");
  }
  ...
  if (opts.redact !== undefined && opts.redact !== null && typeof opts.redact !== "function") {
    throw _err("notify/bad-opt", "notify.create: redact must be a function returning a redacted message");
  }
  if (opts.defaultTimeoutMs !== undefined &&
      opts.defaultTimeoutMs !== 0 &&
      !_isFiniteNonNegative(opts.defaultTimeoutMs)) {
    throw _err("notify/bad-opt", "notify.create: defaultTimeoutMs must be a non-negative finite number (0 disables)");
  }
  if (opts.defaultRetry !== undefined && opts.defaultRetry !== null &&
      (typeof opts.defaultRetry !== "object")) {
    throw _err("notify/bad-opt", "notify.create: defaultRetry must be a b.retry.withRetry opts object");
  }
  if (opts.defaultBreaker !== undefined && opts.defaultBreaker !== null &&
      typeof opts.defaultBreaker !== "object") {
    throw _err("notify/bad-opt", "notify.create: defaultBreaker must be a b.retry.CircuitBreaker opts object");
  }
  validateOpts.optionalObjectWithMethod(opts.queue, "enqueue",
    "notify.create: queue", NotifyError, "notify/bad-opt",
...
function httpJson(opts) {
  if (!opts || typeof opts !== "object") {
    throw _err("notify/bad-opt", "notify.transports.httpJson: opts must be { url, ... }");
  }
  validateOpts.requireNonEmptyString(opts.url, "notify.transports.httpJson: url", NotifyError, "notify/bad-opt");
  ...
  if (bodyFormat !== "json" && bodyFormat !== "form") {
    throw _err("notify/bad-opt", "notify.transports.httpJson: bodyFormat must be 'json' or 'form'");
  }
  var staticHeaders = opts.headers || {};
  var signing = opts.signing || null;
  if (signing && typeof signing.sign !== "function") {
    throw _err("notify/bad-opt", "notify.transports.httpJson: signing must have a sign(body) function");
  }
  ...
  return {
    ...
    send: async function (message, sendOpts) {
      ...
      if (!successStatus(status)) {
        ...
        httpErr.code = "notify/http-failure";
...
function create(opts) {
  ...
  async function send(input) {
    if (!input || typeof input !== "object") {
      throw _err("notify/bad-opt", "notify.send: input must be { channel, message, ... }");
    }
    if (typeof input.channel !== "string" || input.channel.length === 0) {
      throw _err("notify/bad-opt", "notify.send: channel must be a non-empty string");
    }
    if (input.message === undefined || input.message === null) {
      throw _err("notify/missing-message", "notify.send: message is required");
    }
    var channel = input.channel;
    var entry = channels[channel];
    if (!entry) {
      throw _err("notify/no-channel", "notify.send: unknown channel '" + channel + "'");
    }
    ...
    return observability().tap("notify.send", { channel: channel }, async function () {
      try {
        ...
        var wrapped = _err("notify/send-failed",
  ...
  async function sendBatch(inputs) {
    if (!Array.isArray(inputs)) {
      throw _err("notify/bad-opt", "notify.sendBatch: inputs must be an array");
    }
  ...
  async function queue(input, queueOpts) {
    ...
    if (!q || typeof q.enqueue !== "function") {
      throw _err("notify/no-queue", "notify.queue: requires a b.queue handle (pass at create or per-call)");
    }
    if (!input || typeof input !== "object" || typeof input.channel !== "string") {
      throw _err("notify/bad-opt", "notify.queue: input must be { channel, message, ... }");
    }
  ...
  function addChannel(name, transport, channelOpts) {
    if (channels[name]) {
      throw _err("notify/channel-exists", "notify.addChannel: channel '" + name + "' already exists");
    }
