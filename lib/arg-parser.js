"use strict";
/**
 * arg-parser — reusable command-line argument parser for tools that
 * sit on top of the framework. Operators describe a parser declaratively
 * (commands + per-command flags + top-level flags) and call .parse(argv)
 * to get a typed result, or .help() to render usage.
 *
 ...
 *         flags: [
 *           { name: "db",  type: "string", required: true,  description: "Path to sqlite file" },
 ...
 *   });
 *
 *   var parsed = ap.parse(process.argv.slice(2));
 *   //   { command: "migrate", flags: { db: "./app.db", dir: "./migrations" }, positionals: [] }
 *
 * Design notes:
 *
 *   - Throws ArgParserError on any invalid input (unknown flag, missing
 *     required, type-coercion failure, prototype-pollution attempt).
 *     Config-time tier: bad input fails fast at parse so the CLI exits
 *     with a clear message instead of running a subcommand with garbage.
 *
 *   - `--` ends flag parsing. Everything after `--` is positional, even
 *     if it begins with `-` or `--`.
 *
 *   - `--help` / `-h` is reserved. Top-level `--help` prints the program
 *     usage; `<cmd> --help` prints that command's usage. When the parser
 *     was created with `{ exit: true }` (default when invoked via the
 *     `bin/` shim — opt-in by passing `{ exit: true }` to .parse), the
 *     parser writes help to stdout and calls process.exit(0). Programmatic
 *     callers pass `{ exit: false }` (the default for .parse) and inspect
 *     parsed.helpRequested instead.
 *
 *   - Refuses operator-input flag names of `__proto__` / `constructor` /
 *     `prototype` (prototype-pollution defense — the parsed flags object
 *     uses Object.create(null) as well, this is belt-and-suspenders so
 *     downstream code that copies fields into a normal object is also
 *     safe).
 *
 *   - Type coercion:
 *       string  — value as-is
 *       number  — Number(value), throws on NaN
 *       boolean — true if the flag was present (no value form), or the
 *                 strings "true" / "false" / "1" / "0" / "yes" / "no"
 *       list    — repeated --flag x --flag y → ["x","y"]; or a single
 *                 occurrence with comma-separated values → ["x","y"]
 *
 *   - Defaults apply when the flag is absent from argv. `required: true`
 *     fires after defaults, so a flag with both required and default is
 *     always satisfied.
 *
 *   - Aliases: a single-letter shortcut. `-d ./app.db` is equivalent to
 *     `--db ./app.db`. Aliases must be a single character; longer aliases
 *     are flag names, not aliases.
 */
...
// parseRaw — minimal positional + flag splitter used by lib/cli.js's
// internal subcommand handlers. Each handler historically consumed
// `args.flags` directly with hand-written validation; the framework
// keeps that surface stable while routing through this primitive so
// behavior is consistent across the CLI and the public argParser.
//
// Returns { pos, flags } where flags is an Object.create(null) bag.
// Refuses operator-input prototype-pollution names. Treats `-x` as a
// boolean shortcut; `--key value` and `--key=value` and `--bool` are
// all supported.
