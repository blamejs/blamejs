...
function _validateFlagSpec(flagKey, spec) {
  ...
  var validatedRules = null;
  if (spec.rules != null) {
    validatedRules = targeting.validateRules(spec.rules, flagKey + ".rules");
  ...
  return validatedRules === null ? Object.assign({}, spec) : Object.assign({}, spec, { rules: validatedRules });
}

function _validateFlagMap(flagMap) {
  var validated = {};
  for (var key in flagMap) {
    if (!Object.prototype.hasOwnProperty.call(flagMap, key)) continue;
    validated[key] = _validateFlagSpec(key, flagMap[key]);
  }
  return validated;
}
...
function memory(opts) {
  ...
  return _makeProvider("memory", _validateFlagMap(opts.flags));
}
...
function localFile(opts) {
  ...
  var provider = _makeProvider("local-file", _validateFlagMap(parsed.flags));
    ...
    var reload = function () {
      try {
        ...
        if (nextParsed && nextParsed.flags) {
          provider._replace(_validateFlagMap(nextParsed.flags));
        }
...
function environmentVariable(opts) {
  ...
  for (var key in opts.flags) {
    ...
    var clone = _validateFlagSpec(key, opts.flags[key]);
  ...
  var provider = {
    ...
    evaluate: function (flagKey, ctx) {
      ...
      if (targetingResult.reason === "targeting_match") {
        return _buildResult(kind, flagKey, spec, targetingResult.variant,
        ...
        if (tk.length > 0) {
          ...
          for (var i = 0; i < spec.rollout.length; i += 1) {
            ...
            if (bucket < cumulative) {
              return _buildResult(kind, flagKey, spec, spec.rollout[i].variant,
                                  "split", { bucket: bucket });
            }
          }
        }
      }
      return _buildResult(kind, flagKey, spec, spec.default, "default", {});
    },
  };
  return provider;
}

function _buildResult(kind, flagKey, spec, variantName, reason, metaAdd) {
  ...
  var metadata = { flagKey: flagKey, provider: kind };
  if (metaAdd) {
