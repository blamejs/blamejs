...
function localFile(opts) {
  ...
  if (opts.watch === true) {
    try {
      // Stat-poll the config file (StatWatcher), NOT fs.watch: fs.watch aborts
      // libuv — an uncatchable src/win/fs-event.c assertion — when opts.path
      // resolves through a Windows 8.3 short-name component (e.g. a path under
      // C:\Users\SOMEUS~1\...). StatWatcher never opens a directory-change
      // handle, so it is immune to the path form; a ~1s poll is ample latency
      // for a feature-flag hot reload.
      nodeFs.watchFile(opts.path, { persistent: false, interval: C.TIME.seconds(1) }, function () {
        try {
