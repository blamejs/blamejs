...
function fsyncDir(dirPath) {
  try {
    ...
    // framework data directory (vault, audit chain, session store) — never
    // an os.tmpdir-rooted attacker-reachable path. fsyncDir is invoked
    // post-rename to durably commit the parent inode; the fd is read-only
...
function _readSyncCore(filepath, opts) {
  ...
  // TOCTOU defense: open the fd FIRST, then fstat + read off the fd.
  // A separate existsSync/statSync + readFileSync sequence opens a window
  // where an attacker can swap the file (symlink flip, rename) between
  ...
  try {
    ...
    while (read < stat.size) {
      var n = nodeFs.readSync(fd, buf, read, stat.size - read, null);
      if (n === 0) break;
      read += n;
    }
    if (read !== stat.size) {
      throw new AtomicFileError(
        "short read: " + read + " of " + stat.size + " bytes",
        "atomic-file/short-read"
      );
    }
  } finally {
    try { nodeFs.closeSync(fd); } catch (_c) { /* best-effort fd close */ }
  }
