 ...
 * `opts.prefix` followed by a `pathTimestamp` stamp, and that stamp is more
 * than `opts.olderThanMs` before now. A name listed in `opts.keep` is never
 * removed, and a directory that cannot be removed is left for a later call.
 * Returns the names removed. Throws `atomic-file/bad-opts` when `prefix` is
 * not a non-empty string or `olderThanMs` is not a positive integer.
 *
 * @opts
