...
frameworkError.messageFirstFactory(AtomicFileError);
 ...
 *   );
 *   // → what it copied, { fileCount: <n>, byteCount: <bytes> }
 */
...
var PATH_TIMESTAMP_LENGTH = 24;

function _pathTimestampMs(stamp) {
  if (typeof stamp !== "string" || stamp.length !== PATH_TIMESTAMP_LENGTH) return NaN;
  var ms = Date.parse(stamp.slice(0, 13) + ":" + stamp.slice(14, 16) + ":" + stamp.slice(17, 19) + "." + stamp.slice(20));
  return isFinite(ms) && pathTimestamp(new Date(ms)) === stamp ? ms : NaN;
}

var STALE_ACTIVITY_SCAN_ENTRIES = 4096;

function _heartbeatMs(dirPath) {
  try { return nodeFs.statSync(dirPath + HEARTBEAT_SUFFIX).mtimeMs; }
  catch (e) {
...
function _lastActivityMs(dirPath) {
  var newest = 0;
  var pending = [dirPath];
  var seen = 0;
  while (pending.length > 0) {
    if (seen >= STALE_ACTIVITY_SCAN_ENTRIES) return Infinity;
    var current = pending.pop();
    var stat;
    try { stat = nodeFs.statSync(current); }
    catch (e) {
    ...
    var children;
    try { children = nodeFs.readdirSync(current); }
    catch (e2) {
      if (e2 && e2.code === "ENOENT") continue;
      return Infinity;
    }
    for (var i = 0; i < children.length; i += 1) pending.push(nodePath.join(current, children[i]));
  }
  return newest;
}

/**
 * @primitive b.atomicFile.removeStaleDirs
 * @signature b.atomicFile.removeStaleDirs(parentDir, opts)
 * @since     0.20.31
 * @status    stable
 * @related   b.atomicFile.pathTimestamp
 *
 * Remove working directories that a process left in `parentDir` when it
 * stopped before cleaning up. A directory is removed when its name is
 * `opts.prefix` followed by a `pathTimestamp` stamp, that stamp is more than
 * `opts.olderThanMs` before now, AND nothing inside it has been written in
 * that window: a copy or restore still running writes as it goes, so its
 * directory is left alone however long it has been running. A phase that
 * reads without writing, an upload of what was staged, holds
 * `b.atomicFile.heartbeat(dir)` open so the directory keeps reporting
 * activity. The activity scan reads at most 4,096 entries and gives up on
 * an unreadable one; a scan it could not finish keeps the directory rather
 * than guessing that the part it did not read is idle. A name listed in
 * `opts.keep` is never removed, and a directory that cannot be removed is
 * left for a later call. Returns the names removed. Throws
 * `atomic-file/bad-opts` when `prefix` is not a non-empty string or
 * `olderThanMs` is not a positive integer.
 *
 * @opts
 *   prefix:      string,          // required; the part of the name before the stamp
 *   olderThanMs: number,          // required; positive integer
 *   keep:        Array<string>,   // names never removed
 *
 * @example
 *   var removed = b.atomicFile.removeStaleDirs(require("node:os").tmpdir(), {
 *     prefix:      "myapp-staging-",
 *     olderThanMs: b.constants.TIME.hours(24),
 *   });
 *   Array.isArray(removed);   // → true
 */
var HEARTBEAT_SUFFIX = ".active";
var HEARTBEAT_INTERVAL_MS = C.TIME.minutes(1);

/**
 * @primitive b.atomicFile.heartbeat
 * @signature b.atomicFile.heartbeat(dirPath, opts?)
 * @since     0.20.31
 * @status    stable
 * @related   b.atomicFile.removeStaleDirs
 *
 * Report that work in `dirPath` is still going, for the benefit of
 * `removeStaleDirs`, which reads a directory's contents to decide whether
 * the process that created it has stopped. A phase that reads without
 * writing (uploading what was staged, hashing it, waiting on a remote) does
 * not move any modification time, so it holds a heartbeat until it is done.
 * The marker is written next to the directory, as `<dirPath>.active`, so it
 * never joins the bytes a caller is copying or comparing. It is created
 * exclusively and refreshed through the descriptor that created it, never
 * reopened by name, so a link planted at that path in a shared temp
 * directory is not followed and not truncated; a path already taken leaves
 * the heartbeat silent rather than writing through it. Returns `{ stop }`,
 * which removes it. The timer does not hold the event loop open, and a
 * write that fails is ignored: this reports progress, it does not guard
 * anything.
 *
 * @opts
 ...
 *
 * @example
 *   // requires: write access to the directory below
 *   var beat = b.atomicFile.heartbeat("/var/lib/blamejs/.staging-1");
 *   try { await uploadEverything(); } finally { beat.stop(); }
 */
function heartbeat(dirPath, opts) {
  opts = opts || {};
  var intervalMs = numericBounds.isPositiveFiniteInt(opts.intervalMs)
    ? opts.intervalMs
    : HEARTBEAT_INTERVAL_MS;
  var marker = nodePath.resolve(dirPath) + HEARTBEAT_SUFFIX;
  var fd = null;
  try {
    fd = nodeFs.openSync(marker,
      nodeFs.constants.O_WRONLY | nodeFs.constants.O_CREAT | nodeFs.constants.O_EXCL |
        (nodeFs.constants.O_NOFOLLOW || 0),
      0o600);
    nodeFs.writeSync(fd, pathTimestamp() + "\n");
  } catch (_e) {
    if (fd !== null) {
      try { nodeFs.closeSync(fd); } catch (_e1) { /* best-effort */ }
      try { nodeFs.rmSync(marker, { force: true }); } catch (_e2) { /* best-effort */ }
    }
    fd = null;
  }
  function beat() {
    if (fd === null) return;
    var now = new Date();
    try { nodeFs.futimesSync(fd, now, now); }
    catch (_e2) { /* the file may be gone; the sweep is best-effort either way */ }
  }
  var timer = fd === null ? null : setInterval(beat, intervalMs);
  ...
  return {
    stop: function () {
      ...
      if (fd !== null) {
        try { nodeFs.closeSync(fd); } catch (_e3) { /* best-effort */ }
        try { nodeFs.rmSync(marker, { force: true }); } catch (_e4) { /* best-effort */ }
        fd = null;
      }
    },
  };
...
function removeStaleDirs(parentDir, opts) {
  opts = opts || {};
  validateOpts.requireNonEmptyString(opts.prefix, "removeStaleDirs: opts.prefix",
    AtomicFileError, "atomic-file/bad-opts");
  numericBounds.requirePositiveFiniteInt(opts.olderThanMs, "removeStaleDirs: opts.olderThanMs",
    AtomicFileError, "atomic-file/bad-opts");
  var keep = Array.isArray(opts.keep) ? opts.keep : [];
  var entries;
  try { entries = nodeFs.readdirSync(parentDir, { withFileTypes: true }); }
  catch (_e) { return []; }
  var cutoff = Date.now() - opts.olderThanMs;
  var removed = [];
  for (var i = 0; i < entries.length; i++) {
    var name = entries[i].name;
    if (!entries[i].isDirectory() || name.indexOf(opts.prefix) !== 0 || keep.indexOf(name) !== -1) continue;
    var stampMs = _pathTimestampMs(name.slice(opts.prefix.length, opts.prefix.length + PATH_TIMESTAMP_LENGTH));
    if (!isFinite(stampMs) || stampMs >= cutoff) continue;
    if (_heartbeatMs(nodePath.join(parentDir, name)) >= cutoff) continue;
    if (_lastActivityMs(nodePath.join(parentDir, name)) >= cutoff) continue;
    try {
      nodeFs.rmSync(nodePath.join(parentDir, name) + HEARTBEAT_SUFFIX, { force: true });
    } catch (_e) { /* best-effort: the marker is a hint, not a lock */ }
    try {
      nodeFs.rmSync(nodePath.join(parentDir, name), { recursive: true, force: true });
      removed.push(name);
    } catch (_e) { /* left for a later call: another process may still hold it open */ }
  }
  return removed;
}
 ...
 *
 * @example
 *   // The stamp is the moment of the call, so each of these names the file
 *   // it was asked about at the time it was asked.
 *   var p = b.atomicFile.conflictPath("/srv/notes.md");
 *   // → "/srv/notes.conflict-<timestamp>.md"
 *
 *   var withSuffix = b.atomicFile.conflictPath("/srv/notes.md", {
 *     tag: "merge", suffix: "abc123",
 *   });
 *   // → "/srv/notes.merge-<timestamp>.abc123.md"
 */
 ...
 * normalizes the answer for callers that already require this module
...
module.exports = {
  ...
  removeStaleDirs:   removeStaleDirs,
  heartbeat:         heartbeat,
