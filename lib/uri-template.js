...
function _expandExpr(expr, vars) {
  ...
  expr.specs.forEach(function (spec) {
    ...
    if (typeof value !== "object") {
      ...
      if (!spec.explode) {
        var joined = value.map(function (m) { return _pctEncode(_toStr(m), o.reserved); }).join(",");
        pieces.push(_named(o, spec.name, joined, value.length === 0));
      } else {
      ...
      if (!spec.explode) {
        ...
        pieces.push(_named(o, spec.name, pairs.join(","), keys.length === 0));
      } else {
