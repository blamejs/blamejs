...
function _decodeCaptureName(spelling) {
  ...
  while (i < spelling.length) {
    ...
    if (spelling.charAt(i + 1) !== "u") return null;
...
function _isCaptureName(name) {
  ...
  for (var i = 0; i < name.length; i += 1) {
    ...
    var isNameSign = code === 0x24 || code === 0x5F;
 ...
 * are honored. `g` is not, because the matcher returns one match and the
