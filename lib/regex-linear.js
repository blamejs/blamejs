...
function _union(a, b) { return _norm(a.concat(b).map(function (r) { return [r[0], r[1]]; })); }

function _negate(ranges) {
  var sorted = _norm(ranges.map(function (r) { return [r[0], r[1]]; }));
  var out = [];
  var at = 0;
...
var DIGIT = [[0x30, 0x39]];
var WORD = [[0x30, 0x39], [0x41, 0x5A], [0x5F, 0x5F], [0x61, 0x7A]];
var SPACE = _norm([
  [0x09, 0x0D], [0x20, 0x20], [0xA0, 0xA0], [0x1680, 0x1680], [0x2000, 0x200A],
  [0x2028, 0x2029], [0x202F, 0x202F], [0x205F, 0x205F], [0x3000, 0x3000],
  [0xFEFF, 0xFEFF],
]);
var LINE_TERMINATORS = [[0x0A, 0x0A], [0x0D, 0x0D], [0x2028, 0x2029]];
...
function _parse(src, flags) {
  ...
  function parseQuantifier() {
    var plain = { min: 1, max: 1, lazy: false };
    var ch = peek();
    var min, max;
    ...
    return { min: min, max: max, lazy: lazy };
  }
  ...
  function parseGroup() {
    ...
    if (peek() === "?") {
      ...
      if (rest.charAt(1) === "<") {                       // named group
        ...
        if (!_isCaptureName(name)) {
          _fail("capture group name " + JSON.stringify(name) + " — a name here is " +
  ...
  function parseEscape() {
    ...
    return { t: "set", r: _one(_escapedCodePoint()), neg: false };
  }
  ...
  // The code point an escape stands for, having consumed it.
  function _escapedCodePoint() {
    ...
    if (ch === "x") {
      ...
      if (hex === null) _fail("invalid \\x escape", "regex/bad-escape");
      pos = hex.end;
      return hex.value;
    }
    if (ch === "u") {
    ...
    if (ch === "c") {
      ...
      if (!isLetter) _fail("invalid \\c escape", "regex/bad-escape");
      pos += 2;
  ...
  function parseClass() {
    ...
    while (pos < src.length) {
      ...
      if (lo.set !== null) {
        ...
        if (unicode && peek() === "-" && src.charAt(pos + 1) !== "]" && pos + 1 < src.length) {
  ...
  function classMember() {
    if (peek() === "\\") {
      ...
      if (shorthand !== null) {
        pos += 1;
        return { cp: -1, set: shorthand.neg ? _negate(shorthand.r) : shorthand.r };
      }
      return { cp: _escapedCodePoint(), set: null };
    }
...
function _run(compiled, subject, startAt, opts) {
  ...
  function add(list, marks, startPc, at, startCaps) {
    ...
    while (stack.length !== 0) {
      ...
      if (instruction.op === "v") {
        var copy = caps.slice();
        copy[instruction.i] = at;
        stack.push({ pc: pc + 1, caps: copy });
        continue;
      }
      if (instruction.op === "a") {
      ...
      if (instruction.op === "clr") {
        var cleared = caps.slice();
        for (var g = instruction.lo; g <= instruction.hi; g += 1) {
          cleared[g * 2] = -1;
          cleared[g * 2 + 1] = -1;
        }
        stack.push({ pc: pc + 1, caps: cleared });
        continue;
      }
  ...
  var at = startAt;
  var empty = new Array(slots);
  for (var s = 0; s < slots; s += 1) empty[s] = -1;
  ...
  for (;;) {
    ...
    if (matched === null && (!sticky || at === startAt)) add(clist, marksC, 0, at, empty);
    // An empty list means nothing survived FROM HERE — `\b` refusing at the
    // first position, say. The search still moves on; only a match already in
    // hand makes further positions pointless, because none of them could be
    // further left.
 ...
 * `RegExp` whose source and flags are used. Flags `i`, `m`, `s`, `u`, `v` and
 * `y` are honoured; `g` is not, because the matcher returns one match and the
 ...
 *
 * Three more are refused for narrower reasons, each under its own code and each
 ...
 * (`regex/bad-group`); and an inline flag modifier such as `(?i:...)`
 * (`regex/unsupported-group`), whose scoped flags this does not yet carry.
 ...
 * accepted here returns exactly what `RegExp` returns.
 *
 * @opts
...
function compile(source, flags) {
  ...
  function exec(subject, from) {
    ...
    // Under `u` a start offset in the middle of a surrogate pair is stepped
    // back to the character it splits, as the platform does with `lastIndex`.
    // Left alone, the low half would be read as a character of its own and the
    // match would contain half a character.
    if (runOpts.unicode && startAt > 0 && startAt < subject.length) {
      ...
      if (high >= 0xD800 && high <= 0xDBFF && low >= 0xDC00 && low <= 0xDFFF) startAt -= 1;
    }
