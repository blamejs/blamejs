...
function _triggerAt(text, value, quote, delims) {
  ...
  while (i < text.length) {
    var c = text.charAt(i);
    if (inQuotes && c === quote) {
      if (text.charAt(i + 1) === quote) return -1;
      inQuotes = false;
      i += 1;
      continue;
    }
    if (FORMULA_LEADING_SPACE.indexOf(c) === -1) break;
    if (!inQuotes && (_isLineTerminator(c) || delims.indexOf(c) !== -1)) return -1;
    i += 1;
  }
  if (i >= text.length) return -1;
  var ch = text.charAt(i);
  if (!inQuotes && delims.indexOf(ch) !== -1) return -1;
  return FORMULA_PREFIXES.indexOf(ch) === -1 ? -1 : i;
}
...
function _formulaAt(text, at, quote, delims, formulaPolicy, allowlist) {
  ...
  if (formulaPolicy === "allowlist" && Array.isArray(allowlist)) {
    var name = _leadingFunctionName(text.slice(trigger));
    if (name !== null && allowlist.some(function (fn) {
      return String(fn).toUpperCase() === name.toUpperCase();
    })) return null;
  }
  return { index: trigger, char: text.charAt(trigger) };
}
...
function _eachTriggeredWord(text, quote, delimiter, visit) {
  ...
  _readings(delimiter).forEach(function (delims) {
    ...
    _eachCellStart(text, quote, delims, function (start) {
      ...
      var at = v.index;
      if (text.charAt(at) === QUOTE_PREFIX) v = { index: at + 1, inQuotes: v.inQuotes };
      var trigger = _triggerAt(text, v, quote, delims);
      if (trigger === -1 || seen.has(trigger)) return false;
      if (!_isNameStart(text.charAt(trigger + 1))) return false;
      seen.add(trigger);
      var w = trigger + 1;
      while (w < text.length && _isWordChar(text.charAt(w))) w += 1;
      stopped = visit(trigger, text.slice(trigger + 1, w)) === true;
      return stopped;
    });
  });
}
...
function _detectIssues(text, opts) {
  ...
  if (Array.isArray(opts.dangerousFunctions) && opts.dangerousFunctions.length > 0) {
    _eachTriggeredWord(text, opts.quote || "\"", opts.delimiter,
                       function (at, name) {
  ...
  if (str.length > 0 && FORMULA_PREFIXES.indexOf(str.charAt(0)) !== -1) {
    ...
    if (policy === "reject") {
      ...
      function _isAllowed(name) {
        return opts.formulasAllowlist.some(function (fn) {
          return String(fn).toUpperCase() === String(name).toUpperCase();
        });
      }
      var allowed = firstWord !== null && _isAllowed(firstWord) &&
                    called.length > 0 && called.every(_isAllowed);
 ...
 * throw with the finding's rule id. Refuses pathological expansion:
 * when the character-level output exceeds `sanitizeAmplificationCap`
 * (default 1.5x) the function throws
 ...
 * the document for a dialect the guard is not emitting. A document
...
var _stripSanitize = module.exports.sanitize;
module.exports.sanitize = function sanitize(input, opts) {
  var resolved = module.exports.resolveOpts(opts);
  return _disarmFormulas(_stripSanitize(input, opts), resolved);
};
