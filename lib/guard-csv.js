...
function _cellValueStart(text, at, quote, delims) {
  ...
  while (i < text.length) {
    ...
    if (delims.indexOf(c) !== -1 && FORMULA_PREFIXES.indexOf(c) === -1) return null;
...
function _findFormulaCell(text, quote, delims, formulaPolicy) {
  ...
  _eachCellStart(text, quote, delims, function (at) {
    ...
    var ch = text.charAt(v.index);
    if (ch === "") return false;
    if (FORMULA_PREFIXES.indexOf(ch) === -1) return false;
    if (!v.inQuotes && _isLineTerminator(ch)) return false;
    if (v.inQuotes && ch === "\t" && tabIsMitigation) return false;
    hit = { index: v.index, char: ch };
    return true;
  });
  return hit;
}
