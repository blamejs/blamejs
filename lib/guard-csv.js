...
var TAG_RE_G      = codepointClass.TAG_RE_G;
...
function _eachCellStart(text, visit) {
  if (visit(0)) return;
  var i = 0;
  while (i < text.length) {
    if (text.charAt(i) === "\"") {
    ...
    while (i < text.length &&
           CELL_DELIMITERS.indexOf(text.charAt(i)) === -1 &&
...
//
// A quoted cell whose first character is TAB is skipped: that is the shipped
// mitigation (OWASP's Excel-resistant form is a TAB inside the quoted field),
// and a scanner that flags it reports every correctly-escaped document as
// hostile. A BARE leading TAB is still a finding — unquoted it is a delimiter
// under tab-separated import and is dropped by consumers that trim leading
// whitespace on unquoted fields, either of which puts the next character first
// again.
function _findFormulaCell(text) {
  if (typeof text !== "string") return null;
  var hit = null;
  _eachCellStart(text, function (at) {
    var quoted = text.charAt(at) === "\"";
...
// and keeps only what it recognises, so nothing accumulates for the ordinary
...
// spell around.
function _eachTriggeredWord(text, visit) {
  if (typeof text !== "string") return;
  _eachCellStart(text, function (start) {
    var at = text.charAt(start) === "\"" ? start + 1 : start;
...
function _detectIssues(text, opts) {
  ...
  if (opts.formulaInjectionPolicy !== "audit-only" && opts.formulaInjectionPolicy !== "allow") {
    ...
    // (BOM) followed by `=` puts a codepoint between the cell start and
    // the trigger, so the cell-start check reads the invisible character
    // rather than the `=`, and the formula reaches
    ...
    var formulaMatch = _findFormulaCell(stripped);
    if (formulaMatch) {
      issues.push({
  ...
  if (Array.isArray(opts.dangerousFunctions) && opts.dangerousFunctions.length > 0) {
    _eachTriggeredWord(text, function (at, name) {
