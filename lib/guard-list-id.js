...
function validate(headerValue, opts) {
  ...
  // label=team.example / ns=com). Validate the full list-id
  // shape and surface BOTH a heuristic label+namespace split (last
  // 2 dot-segments → namespace) AND the raw list-id for consumers
  // that need the literal.
  if (listId.indexOf(".") === -1) {
  ...
  if (caps.requireFqdn && parts.length < 2) {                                                            // allow:raw-byte-literal — FQDN requires ≥ 2 labels
    return _refuse("list-id has < 2 labels (FQDN required under '" + (opts.profile || DEFAULT_PROFILE) + "')");
  }

  // Heuristic split — namespace = last 2 labels (matches typical
  // DNS delegation); label = the rest. Consumer code that needs
  // PSL-accurate org-domain extraction composes
  // `b.publicSuffix.organizationalDomain`.
  var namespace;
  var label;
  if (parts.length >= 2) {                                                                               // allow:raw-byte-literal — heuristic split point
    namespace = parts.slice(-2).join(".");                                                               // allow:raw-byte-literal — last-2 split
    label     = parts.slice(0, -2).join(".");                                                            // allow:raw-byte-literal — last-2 split
  } else {
    namespace = listId;
    label     = "";
  }
  ...
  if (namespace.toLowerCase() === "localhost" || listId.toLowerCase().endsWith(".localhost")) {
