 ...
 *   <code>opts.store</code> adapter whose <code>incrBy</code> /
 *   <code>decrBy</code> are atomic on the shared backend (Redis
 *   <code>INCRBY</code>, a SQL <code>UPDATE ... RETURNING</code>,
 *   etc.). The framework records the active cluster node id on
 *   every breach event so a denial-of-wallet spike is attributable.
 *
...
//
// Single-threaded JS makes the read-modify-write below atomic within
// a process. Entries self-expire at the window boundary; reads past
// expiry return 0 (a fresh window). _keysWithPrefix backs the
// reset(tenant) enumeration the default store can satisfy without an
// external scan.

function _memoryStore() {
  var m = new Map();                                     // key -> { value, expiresAt }
  return {
    incrBy: function (key, amount, windowMs) {
      var now = Date.now();
      var e = m.get(key);
 ...
 *     store?:           object,    // { incrBy, decrBy, get, reset };
...
function create(opts) {
  ...
  function _result(tenantId, model, used, limit, windowStart, resetsAt, allowed, exceeded) {
  ...
  function consume(tenantId, model, amount, consumeOpts) {
    ...
    // Atomic charge: add first, then test. Under "hard" an
    // over-budget charge is refunded (decrBy) before refusal so a
    // rejected call leaves the counter untouched. This incr-test-
    // refund shape stays correct on an atomic shared store, where a
    // plain read-then-add would race across nodes.
    var used = store.incrBy(key, amount, windowMs);
    ...
    if (!exceeded) {
      ...
      return _result(tenantId, model, used, limit, windowStart, resetsAt, true, false);
    }

    // Over budget.
    var nodeId = _nodeId();
    _emitAudit("ai/quota-exceeded", mode === "hard" ? "denied" : "allowed", {
      tenantId: tenantId, model: model, dimension: dimension,
      period: period, amount: amount, used: used, limit: limit,
      enforcement: mode, nodeId: nodeId,
    });
    _emitMetric("ai.quota.exceeded", 1);

    if (mode === "hard") {
      store.decrBy(key, amount);                         // refund the rejected reservation
      var attempted = used;                              // pre-refund total, for the message
      throw new AiQuotaError("aiQuota/exceeded",
        "ai.quota.consume: tenant '" + tenantId + "' model '" + model +
        "' would reach " + attempted + " of " + limit + " " + dimension +
        " this " + period + "; call refused");
    }

    // soft / warn: the charge stands; the window stays over budget
    // until it rolls. soft reports allowed:false so the caller can
    // choose to honor it; warn reports allowed:true (advisory only).
    var allowed = mode === "warn";
    return _result(tenantId, model, used, limit, windowStart, resetsAt, allowed, true);
  }
  ...
  function check(tenantId, model) {
    ...
    return _result(tenantId, model, used, limit, windowStart, resetsAt, used < limit, used >= limit);
  }
...
function _validateStore(store) {
  if (!store || typeof store !== "object" ||
      typeof store.incrBy !== "function" ||
      typeof store.decrBy !== "function" ||
      typeof store.get !== "function" ||
      typeof store.reset !== "function") {
    throw new AiQuotaError("aiQuota/bad-store",
      "ai.quota.create: store must expose incrBy / decrBy / get / reset functions");
  }
}
