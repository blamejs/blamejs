...
function _validateTarget(t, idx) {
  if (!t || typeof t !== "object") {
    throw new HeartbeatError("targets[" + idx + "] must be an object", "heartbeat/bad-target");
  }
  if (typeof t.name !== "string" || t.name.length === 0) {
    throw new HeartbeatError("targets[" + idx + "].name required (string)", "heartbeat/bad-name");
  }
  if (t.type !== "http" && t.type !== "tcp" && t.type !== "ntp") {
    throw new HeartbeatError("targets[" + idx + "].type must be 'http' | 'tcp' | 'ntp', got " + JSON.stringify(t.type),
      "heartbeat/bad-type");
  }
  if (t.type === "http" && (typeof t.url !== "string" || t.url.length === 0)) {
    throw new HeartbeatError("targets[" + idx + "].url required for http heartbeat", "heartbeat/bad-url");
  }
  if (t.type === "tcp" && (typeof t.host !== "string" || typeof t.port !== "number")) {
    throw new HeartbeatError("targets[" + idx + "] tcp heartbeat requires host (string) + port (number)",
      "heartbeat/bad-tcp");
  }
  if (t.type === "ntp" && typeof t.server !== "string") {
    throw new HeartbeatError("targets[" + idx + "] ntp heartbeat requires server (string)", "heartbeat/bad-ntp");
  }
  if (t.intervalMs !== undefined && (typeof t.intervalMs !== "number" || !isFinite(t.intervalMs) || t.intervalMs <= 0)) {
    throw new HeartbeatError("targets[" + idx + "].intervalMs must be a positive number", "heartbeat/bad-interval");
  }
  if (t.timeoutMs !== undefined && (typeof t.timeoutMs !== "number" || !isFinite(t.timeoutMs) || t.timeoutMs <= 0)) {
    throw new HeartbeatError("targets[" + idx + "].timeoutMs must be a positive number", "heartbeat/bad-timeout");
  }
  if (t.threshold !== undefined && (typeof t.threshold !== "number" || t.threshold < 1 || Math.floor(t.threshold) !== t.threshold)) {
    throw new HeartbeatError("targets[" + idx + "].threshold must be a positive integer", "heartbeat/bad-threshold");
  }
}
...
async function _runProbe(entry) {
  ...
  if (typeof entry.onResult === "function") {
    try { entry.onResult({ name: entry.target.name, state: entry.state, result: result }); } catch (_e) {}
  }
}
...
function start(opts) {
  ...
  if (!Array.isArray(opts.targets) || opts.targets.length === 0) {
    throw new HeartbeatError("heartbeat.start: targets required (non-empty array)", "heartbeat/no-targets");
  }
  ...
  for (var j = 0; j < opts.targets.length; j++) {
    ...
    if (TARGETS.has(t.name)) {
      throw new HeartbeatError("heartbeat target '" + t.name + "' already started", "heartbeat/duplicate");
    }
    var entry = {
      target:              t,
...
function stop(name) {
  ...
  entry.stopped = true;
  if (entry.timer) { try { clearTimeout(entry.timer); } catch (_e) {} }
  TARGETS.delete(name);
  return true;
}
