...
// hashStream(readable, algorithm?) — stream a Readable through
// createHash(algorithm) and resolve with the raw digest Buffer.
// Default algorithm is the framework's SHA3-512. Algorithm is
// validated against STREAM_HASH_ALGORITHMS so a typo / weak choice
// throws at config time rather than producing a digest under a
// different algorithm than the operator expected. Read-only — no
// audit emit (see hashFile docstring).
...
// hashFile(path, algorithm?) — open `path` as a Readable and stream
// it through hashStream. Resolves with the raw digest Buffer. Default
// algorithm is sha3-512. Read-only / no operator-state change → no
// audit emit (the path is operator-supplied; the digest is the only
// observable side-effect, returned to the caller).
