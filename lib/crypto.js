...
function decrypt(ciphertext, privateKeys, opts) {
  ...
  if (packed[0] === 0xE1) {                                                       // allow:raw-byte-literal — legacy envelope magic
    ...
    // visible. Inline-require for the audit module per the circular-
    // load defense pattern used elsewhere in this file.
    setImmediate(function () {
      try {
        var auditMod = require("./audit");                                        // allow:inline-require — circular-load defense (audit imports crypto)
        auditMod.safeEmit({
          ...
          outcome:  "success",
          metadata: { magic: "0xE1", kemId: packed[1] },
        });
      } catch (_e) { /* drop-silent — audit best-effort */ }
    });
    return decryptEnvelope(packed, privateKeys, { omitFixedInfo: true });
  }
