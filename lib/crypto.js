...
function spkiPinVerifier(opts) {
  ...
  if (!Array.isArray(opts.pins) || opts.pins.length < 2) {
    throw new TypeError(
      "crypto.spkiPinVerifier: opts.pins must be an array of at least two " +
  ...
  var pinBufs = new Array(opts.pins.length);
  for (var pi = 0; pi < opts.pins.length; pi += 1) {
    pinBufs[pi] = _decodeSpkiPin(opts.pins[pi], "crypto.spkiPinVerifier");
  }
