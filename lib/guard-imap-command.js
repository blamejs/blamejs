 ...
 *
 * `maxSequenceSetItems` bounds how many messages a set names on the wire,
 * which this guard can count without a mailbox: `1:20000` names 20000 and
 * is refused at strict. A range written against `*`, which RFC 9051
 * section 9 defines as the largest number in use, names a count only the
 * selected mailbox knows, so `FETCH 1:*` is not counted here and is not
 * refused here. What bounds it is the mailbox itself and the per-handler
 * response-byte and wall-clock budgets `b.mail.server.imap` applies.
 *
 * @opts
