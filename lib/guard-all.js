...
function _validateProfileAndPosture(opts) {
  ...
  if (opts.posture != null) {
    throw _err("guard-all/bad-opt",
      "option \"posture\" is not read here; the shared compliance regime is " +
      "named \"compliancePosture\"");
  }
 ...
 * rather than served Content-Type. Honors the same `exceptFor` /
 ...
 *   //    "email","text","filename","domain","uuid","cidr","country","time",
 *   //    "mime","jwt","oauth","graphql","shell","regex","jsonpath",
 *   //    "template","image","pdf","auth","smtpCommand","sql"]
 */
function allGuards() {
