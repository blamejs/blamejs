 ...
 *     - **CNAME chain depth** (default 8 — RFC 1034 §3.6.2 advises
 *       against long chains; we cap to defend RFC 9156 §3.1
 *       amplification + redirection-loop classes).
 ...
 * @since     0.9.31
 * @status    stable
 * @related   b.network.dns.resolver, b.safeDns.boundEdns0
 *
...
function checkCnameChainDepth(depth, opts) {
  ...
  if (depth > caps.maxCnameDepth) {
    throw new SafeDnsError("safe-dns/oversize-cname-depth",
      ...
      caps.maxCnameDepth + " (RFC 1034 §3.6.2 chain-loop defense)");
  }
}
