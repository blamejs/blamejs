 ...
 *
 * HMAC-SHA1 with 30-second step and 6-digit codes is the default,
 * matching the universal authenticator-app interop set (Google
 * Authenticator, Authy, 1Password, Bitwarden, FreeOTP, …). SHA-256
 * and SHA-512 are supported for operators with stronger HMAC posture
 * AND control over the authenticator. The framework's PQC stance is
 * about long-term cryptographic hardness; TOTP is a short-window
 * preimage check (the code is one-time within a 30s step), so even
 * SHA-1 here is fit for purpose — HMAC's security depends on PRF
 * strength, not collision resistance, and HMAC-SHA1 is still
 ...
 *   stepSeconds:   30
 *   digits:        6
 *   driftSteps:    1   (accept ±1 step from current time = ±30s)
 *   algorithm:     "sha1"
 *
 * The defaults are also what every authenticator app expects without
 * additional configuration. Operators who change them MUST configure
 * the authenticator the same way; the otpauth URI fields surface the
 * non-default values for QR-code provisioning.
 */
...
var SUPPORTED_ALGORITHMS = Object.freeze(["sha1", "sha256", "sha512"]);
