...
function _tsigVariables(keyName, algName, timeSigned, fudge, error, otherData) {
  ...
  return Buffer.concat([_encodeName(keyName), head, _encodeName(algName), time, tail, otherData]);
}
...
function verify(message, opts) {
  ...
  // Resolve the key for this RR's owner name.
  var keyEntry = null;
  if (opts.keys && typeof opts.keys === "object") {
    keyEntry = opts.keys[rr.keyName] || opts.keys[rr.keyName.replace(/\.$/, "")];
  } else if (opts.keyName != null) {
    if (opts.keyName === rr.keyName || opts.keyName.replace(/\.$/, "") === rr.keyName.replace(/\.$/, "")) {
