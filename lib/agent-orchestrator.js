 ...
 * @since     0.9.21
 * @status    stable
 * @related   b.mail.agent, b.cluster, b.appShutdown
 *
...
function _spawnConsumers(ctx, args) {
  ...
  if (!isFinite(shards) || shards < 1 || shards > 256) {                                              // allow:raw-byte-literal — shard cap
    throw new AgentOrchestratorError("agent-orchestrator/bad-shard-count",
      "spawnConsumers: shards must be in 1..256");
  }
...
// Consistent-hash router — operator passes `shardKey(envelope)` to
// pick a shard; orchestrator picks the topic suffix. Composes with
// queue.enqueue; the queue itself stays unmodified.
