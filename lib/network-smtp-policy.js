...
var POLICY_RECORD_VERSION_GRAMMAR = Object.freeze({ requireNextField: true });
...
async function _fetchStsTxt(domain, dnsLookup) {
  ...
  for (var i = 0; i < records.length; i += 1) {
    ...
    if (structuredFields.recordVersionMatches(rec, "v", "STSv1", POLICY_RECORD_VERSION_GRAMMAR)) matches.push(rec);
  }
...
async function _fetchStsPolicy(lcDomain, id) {
...
async function mtaStsFetch(domain, opts) {
  ...
  var txt;
  try { txt = await _fetchStsTxt(lcDomain, opts.dnsLookup); }
  catch (e) {
  ...
  try {
    fresh = await store.wrap("fetch|" + lcDomain + "|" + txt.id, function () {
      return _fetchStsPolicy(lcDomain, txt.id);
    }, { ttlMs: 0 });
  } catch (e2) {
    if (cached) return cached;
    throw e2;
...
async function tlsRptFetchPolicy(domain, opts) {
  ...
  for (var i = 0; i < (records || []).length; i += 1) {
    ...
    if (structuredFields.recordVersionMatches(s, "v", "TLSRPTv1", POLICY_RECORD_VERSION_GRAMMAR)) matches.push(s);
  }
  if (matches.length !== 1) return null;
