...
function create(config) {
  ...
  function list(prefix, opts) {
    ...
    return _request("GET", url, headers, null, { timeoutMs: config.timeoutMs }).then(function (res) {
      ...
      var items = [];
      // Parse <Contents>...</Contents> blocks via String.matchAll (no regex.exec).
      var blockMatches = xml.matchAll(/<Contents>([\s\S]*?)<\/Contents>/g);
      for (var m of blockMatches) {
        var inner = m[1];
        var keyMatch = inner.match(/<Key>([\s\S]*?)<\/Key>/);
        var sizeMatch = inner.match(/<Size>(\d+)<\/Size>/);
        var lmMatch = inner.match(/<LastModified>([\s\S]*?)<\/LastModified>/);
        if (keyMatch) {
          items.push({
            key:          keyMatch[1],
      ...
      var truncated = /<IsTruncated>true<\/IsTruncated>/.test(xml);
      var tokenMatch = xml.match(/<NextContinuationToken>([\s\S]*?)<\/NextContinuationToken>/);
      return {
        items:             items,
        truncated:         truncated,
