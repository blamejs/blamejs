...
// XSD dateTime (VCDM 2.0 §4.9): [-]CCYY-MM-DDThh:mm:ss with an optional
// fractional second and an optional timezone (Z or +/-hh:mm). Date.parse
// alone is far too lenient — it accepts a bare year ("2020"), a slash date
// ("2020/01/01"), a date with no time ("2020-01-01"), and locale forms
// ("March 5, 2020") — so a non-conformant value would be silently accepted
// and given a heuristic instant. The shape is asserted here BEFORE Date.parse
// so a malformed value fails closed at both issue and verify; Date.parse then
// rejects the shape-valid-but-impossible dates (month 13, day 32) that a
// regex can't catch.
...
function _parseValidityField(cred, name) {
  ...
  if (typeof cred[name] !== "string" || !_XSD_DATETIME.test(cred[name])) {
