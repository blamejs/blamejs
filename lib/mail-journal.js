 ...
 *     - `b.cryptoField.sealRow` — every journaled message is sealed at
 *       rest with the operator's vault key. Plaintext columns kept for
 *       forensic queryability (`journalId`, `direction`, `archivedAt`,
 *       `actorId`, `messageId`, `sizeBytes`, `regimes[]`); recipient
 *       list / subject / from / body all sealed.
 ...
 * @since      0.9.57
 * @status     stable
 * @compliance hipaa, sox, soc2, dora
...
function create(opts) {
  ...
  // Route the table-name through safeSql.quoteIdentifier so the
  // identifier validation + dialect-correct quoting happen at the
  // single shared substrate. `namespace` is already constrained to
  // `[a-zA-Z0-9_-]{1,64}` at create-time, but the additional gate
  // ensures any future relaxation of the namespace charset doesn't
  // open an injection vector.
  var qTable = safeSql.quoteIdentifier("_mail_journal_" + namespace.replace(/-/g, "_"));
  opts.db.runSql(
    ...
    ");" +
    "CREATE INDEX IF NOT EXISTS " + qTable + "_archived_at_idx ON " + qTable + " (archived_at);" +
    "CREATE INDEX IF NOT EXISTS " + qTable + "_message_id_idx  ON " + qTable + " (message_id);"
  );
  ...
  async function record(req) {
    ...
    var sealed = cryptoField().sealRow(payload, opts.vault, {
      table:  qTable,
      schema: { direction: "plain", messageId: "plain" },
    });

    await opts.storage.putObject(storageKey, sealed.blob, {
    ...
    opts.db.runSql(
      ...
      [journalId, req.direction, req.actorId, req.messageId,
       archivedAt, sizeBytes, regimesJson, floorUntil, storageKey, sealed.blob]
    );
  ...
  async function getById(journalId) {
    ...
    var r = rows[0];
    var unsealed = cryptoField().unsealRow(r.sealed_payload, opts.vault);
