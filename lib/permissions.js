...
function _validateScopePattern(scope, ctx) {
  if (typeof scope !== "string" || scope.length === 0) {
    throw _err("permissions/bad-scope", ctx + ": scope must be a non-empty string, got " + typeof scope);
  }
  ...
  if (scope.length > SCOPE_MAX_LENGTH || !SCOPE_RE.test(scope)) {
    throw _err("permissions/bad-scope", ctx + ": scope '" + scope +
...
function _normalizeRoleEntry(name, entry) {
  ...
  if (entry && typeof entry === "object") {
    ...
    if (!Array.isArray(ext)) {
      throw _err("permissions/bad-role", "role '" + name + "': extends must be an array of role names");
    }
    if (!Array.isArray(perms)) {
      throw _err("permissions/bad-role", "role '" + name + "': permissions must be an array of scope strings");
    }
    var dbRole = null;
    if (entry.dbRole !== undefined && entry.dbRole !== null) {
      if (typeof entry.dbRole !== "string" || entry.dbRole.length === 0) {
        throw _err("permissions/bad-role",
  ...
  throw _err("permissions/bad-role", "role '" + name + "' must be an array of scopes or { extends?, permissions, dbRole?, requireMfa?, mfaWindowMs? }");
}

function _validateRoles(roles) {
  if (!roles || typeof roles !== "object" || Array.isArray(roles)) {
    throw _err("permissions/bad-opt", "permissions.create: roles must be an object map of name → spec");
  }
  var names = Object.keys(roles);
  if (names.length === 0) {
    throw _err("permissions/bad-opt", "permissions.create: roles map must have at least one role");
  }
  var normalized = {};
  for (var i = 0; i < names.length; i++) {
    var name = names[i];
    if (typeof name !== "string" || name.length === 0) {
      throw _err("permissions/bad-role", "role name must be a non-empty string");
    }
    ...
    for (var k = 0; k < spec.extends.length; k++) {
      if (typeof spec.extends[k] !== "string" || spec.extends[k].length === 0) {
        throw _err("permissions/bad-role", "role '" + name + "': extends entry must be a non-empty string");
      }
    }
  ...
  for (var n = 0; n < names.length; n++) {
    ...
    for (var m = 0; m < spec2.extends.length; m++) {
      if (!Object.prototype.hasOwnProperty.call(normalized, spec2.extends[m])) {
        throw _err("permissions/unknown-role", "role '" + names[n] + "': extends references unknown role '" +
          spec2.extends[m] + "'");
      }
    }
  }
...
function _detectCycle(roleName, table, stack) {
  if (stack.indexOf(roleName) !== -1) {
    throw _err("permissions/cycle", "permissions.create: cycle in extends chain: " +
...
function _validateCreateOpts(opts) {
  ...
  if (opts.denyStatus !== undefined &&
    ...
    throw _err("permissions/bad-opt", "permissions.create: denyStatus must be an HTTP status code (100-599)");
  }
  if (opts.missingActorStatus !== undefined &&
    ...
    throw _err("permissions/bad-opt", "permissions.create: missingActorStatus must be an HTTP status code (100-599)");
  }
...
function create(opts) {
  ...
  function policy(scope, predicate) {
    ...
    if (typeof predicate !== "function") {
      throw _err("permissions/bad-opt", "permissions.policy: predicate must be a function (actor, context) -> bool");
    }
    if (policies[scope]) {
      throw _err("permissions/duplicate-policy", "permissions.policy: '" + scope + "' is already registered");
    }
  ...
  function _middleware(mode, requested, mwOpts) {
    if (mode === "single") {
      ...
      if (!Array.isArray(requested) || requested.length === 0) {
        throw _err("permissions/bad-opt", "permissions." + (mode === "all" ? "requireAll" : "requireAny") +
    ...
    if (mwOpts.mfaWindowMs !== undefined && mwOpts.mfaWindowMs !== null) {
      if (typeof mwOpts.mfaWindowMs !== "number" || !isFinite(mwOpts.mfaWindowMs) || mwOpts.mfaWindowMs <= 0) {
        throw _err("permissions/bad-opt", "permissions middleware: mfaWindowMs must be a positive finite number");
      }
    ...
    if (contextProvider !== undefined && typeof contextProvider !== "function") {
      throw _err("permissions/bad-opt", "permissions middleware: context must be a function (req) -> object");
    }
