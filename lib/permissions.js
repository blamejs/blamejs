 ...
 * (feedback_security_defaults_on_by_default.md): `auditFailures: true`
...
function _actorAuditShape(actor) {
  if (!actor) return {};
  // Audit chain expects { actor: { userId, ... } }; map common shapes
  // without leaking the entire actor object.
  var out = {};
  if (actor.userId)                       out.userId  = actor.userId;
  if (Array.isArray(actor.roles))         out.roles   = actor.roles.slice();
  if (Array.isArray(actor.scopes))        out.scopes  = actor.scopes.slice();
  return out;
}
