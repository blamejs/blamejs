...
function create(opts) {
  ...
  function _handleAppend(state, socket, tag, args, literalBody) {
    ...
    if (catenateMatch) {
      ...
      // Parse the parts list. Each part is either `TEXT {<n>}<literal>`
      // or `URL "imap://..."`. Literals were already collected by the
      // upstream literal-aware parser; we receive them in
      // `literalBody.parts` (Array<Buffer>) when CATENATE is used.
      var partsBody = catenateMatch[4].replace(/^\(/, "").replace(/\)\s*$/, "");
      var parts = [];
      var urlMatchAll = partsBody.match(/URL\s+"([^"]+)"/gi) || [];
      for (var u = 0; u < urlMatchAll.length; u += 1) {
        var urlM = urlMatchAll[u].match(/URL\s+"([^"]+)"/i);
        if (urlM) parts.push({ kind: "URL", url: urlM[1] });
      }
      // The literal-aware command parser delivers TEXT-part bodies as
      // `literalBody` when a single literal lives in the command, OR
      // as `literalBody.parts[]` when multiple literals were collected
      // across the CATENATE list. Backends accept either shape.
      if (Buffer.isBuffer(literalBody) && literalBody.length > 0) {
        parts.push({ kind: "TEXT", bytes: literalBody });
      } else if (literalBody && Array.isArray(literalBody.parts)) {
        for (var pp = 0; pp < literalBody.parts.length; pp += 1) {
          if (Buffer.isBuffer(literalBody.parts[pp])) {
            parts.push({ kind: "TEXT", bytes: literalBody.parts[pp] });
          }
        }
      }
      Promise.resolve()
