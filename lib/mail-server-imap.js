...
function _listMatcher(patterns, reference) {
  var patternChars = 0;
  for (var p = 0; p < patterns.length; p += 1) patternChars += patterns[p].length;
...
function _validateMailboxName(name, opts) {
  ...
  if (safeBuffer.byteLengthOf(name) > MAX_MAILBOX_NAME_BYTES) return false;
...
function create(opts) {
  ...
  function _resyncAfterRefusedLiteral(state, socket, line) {
    ...
    if (announced.size > maxLiteralBytes) {
      _emit("mail.server.imap.refused_literal_unreadable",
        { connectionId: state.id, announced: announced.size, cap: maxLiteralBytes }, "denied");
      _writeUntagged(socket, "BYE Refused literal is larger than the cap; closing");
      _close(socket, state);
      return;
    }
    _emit("mail.server.imap.refused_literal_discarded",
      { connectionId: state.id, size: announced.size }, "denied");
    state.pendingLiteral = {
      tag:           null,
      verb:          null,
      line:          line,
      ...
      .then(function (folders) {
        ...
        return Promise.resolve()
          .then(function () {
            ...
            return mailStore.listSubscriptions(state.actor);
          })
          .then(function (subscribed) {
            var list = Array.isArray(subscribed) ? subscribed : [];
  ...
  function _sameAccount(a, b2) {
    ...
    for (var i = 0; i < fields.length; i += 1) {
      var left = _identityValue(a[fields[i]]);
      var right = _identityValue(b2[fields[i]]);
  ...
  function _stillSelected(state, mailbox) {
    return state.selectedMailbox === mailbox && !state.closed;
  }
  ...
  function _releaseSelections(name, renamingState, renamedTo) {
    ...
    sessions.forEach(function (other) {
      ...
      var isDescendant = renamedTo !== undefined && renamedTo !== null &&
  ...
  function _handleMailboxVerb(state, socket, tag, args, spec) {
    ...
    for (var i = 0; i < operands.length; i += 1) {
      if (!_validateMailboxName(operands[i], { allowLegacyMUtf7: allowLegacyMUtf7 })) {
        _writeTagged(socket, tag, "BAD Mailbox name refused");
        return;
      }
  ...
  function _handleExpunge(state, socket, tag) {
    ...
    var expungedFrom = state.selectedMailbox;
    return Promise.resolve()
      .then(function () {
      ...
      .then(function (info) {
        if (!_stillSelected(state, expungedFrom)) {
  ...
  function _handleFetch(state, socket, tag, args, useUid) {
    ...
    var fetchedFrom = state.selectedMailbox;
    return Promise.resolve()
      .then(function () {
      ...
      .then(function (rows) {
        if (!_stillSelected(state, fetchedFrom)) {
          _refuseStaleSelection(socket, tag, "closed");
          return;
        }
        var rs = rows || [];
  ...
  function _handleStore(state, socket, tag, args, useUid) {
    ...
    var storedIn = state.selectedMailbox;
    return Promise.resolve()
      .then(function () {
      ...
      .then(function (result) {
        if (!_stillSelected(state, storedIn)) {
          _refuseStaleSelection(socket, tag, "closed");
          return;
        }
        var rs, modifiedSet;
