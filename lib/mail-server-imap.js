...
function create(opts) {
  ...
  function _handleConnection(rawSocket) {
    ...
    mailServerNet.wireLineSocket(socket, {
      ...
      // every wire byte once and gates a pending literal's payload before the
      // buffer grows.
  ...
  // Would this line open a literal if it were taken now? The guard parses it
  // and applies the profile, and the declared size has to be one this listener
  // would collect. A tail-matching answer would hand `BOGUS {N+}` an exemption
  // for a payload no handler will ever read.
  //
  // The session's stage is deliberately NOT consulted, though `_handleLine`
  // refuses an AUTHENTICATED-only verb's literal before opening one. A peer
  // may pipeline `LOGIN` and an `APPEND` in a single write, which RFC 9051
  // allows and LITERAL+ exists to make worth doing; judging the APPEND against
  // the state as it stands BEFORE the login refuses a command that would be
  // valid by the time the reader reaches it, and closes the connection for it.
  // Nothing is given away: the reader takes the line on the same turn, and a
  // literal that is refused there is never opened, so its announced octets
  // stop being exempt as soon as the opener leaves the buffer and the bytes
  // still queued behind it are charged on the next read.
  // `authenticated: true` for the same reason, one level down: the guard
  // refuses LITERAL+ to an unauthenticated peer, so asking it in the session's
  // current state refuses the exemption for the very shape a pipelined LOGIN
  // makes legal. The probe asks whether the line COULD open a literal here,
  // and the reader still asks the real question when it takes it.
  ...
  // The bound, asked wherever the buffer's composition can have changed.
  //
  // An exemption is granted for an opener the reader has not taken yet, so it
  // is a prediction. When the reader takes that line and REFUSES the literal --
  // an AUTHENTICATED-only verb before login, an announcement over the cap --
  // the prediction was wrong and those octets are ordinary queue. Asking only
  // on a socket read left them uncharged for as long as the pump kept going,
  // so one coalesced write could hold and dispatch a literal cap's worth of
  // commands against a much smaller pipeline allowance.
  //
  // The length test comes first and costs nothing. Only a buffer already over
  // the allowance is worth scanning for an opener, which keeps this off the
  // hot path when the answer is obviously no.
  ...
  function _drainBuffer(state, socket) {
    ...
    function step() {
      ...
      // Re-asked per command, because the exemption the last check granted was
      // a prediction about a line the reader had not taken yet. With a literal
      // actually open there is nothing to predict and nothing to re-ask.
      ...
      // the previous one absorbed stop counting against this one -- both the
      // payload total and what their write-backs added to the line.
  ...
  function _completeLiteralCommand(state, socket, rest) {
    ...
    if (tail !== "") {
      ...
      //
      // Which means the expansions of EARLIER literals come off the count too.
      // A command may carry several, and each completed one is written back
      // into the line as a quoted string, so by the third literal `lineNoLit`
      // carries the first two payloads and charges them here -- refusing a
      // command whose own syntax is well inside the cap and whose literals are
      // inside theirs. `literalSyntaxBytes` is what those write-backs added.
      ...
      // What this write-back adds to the line, so the NEXT literal's syntax
      // check can take it back off again.
  ...
  function _close(socket, state) {
    // The reader's guard was dead before this: _close never wrote a marker, so
    // the reader kept taking commands after the socket was destroyed.
    //
    // The marker is its own field rather than a `stage` value. `stage` is the
    // protocol position and handlers assign it — "authenticated" when a login
    // resolves, "selected" when a mailbox opens — so a close that landed while
    // one of those was in flight was overwritten by the continuation, and the
    // reader went back to work on the buffer the close had refused. This one is
    ...
    // The marker above is set synchronously so the reader stops at once; only
    // the socket teardown waits for the reply to flush.
    // Left in `connections` until the socket actually closes, which
    // `trackConnection` already watches for. Removing it here dropped a socket
    // from the count while the teardown was still holding it open to flush, so
    // `close()` and `connectionCount()` reported none while a live,
    // process-referring socket remained.
