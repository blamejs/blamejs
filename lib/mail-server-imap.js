...
function create(opts) {
  ...
  function _handleFetch(state, socket, tag, args, useUid) {
    ...
    // RFC 7162 §3.1.4.1 — when CONDSTORE is enabled (or the client
    // requested MODSEQ explicitly), every untagged FETCH response
    // includes the MODSEQ attribute. Pass-through to backend so it
    // can include `modseq` in row.payload OR a separate row.modseq.
    var includeModseq = state.enabledCondStore === true || /\bMODSEQ\b/i.test(partsSpec);
    Promise.resolve()
      .then(function () {
    ...
    Promise.resolve()
      ...
      .then(function (result) {
        ...
        if (!silent) {
          for (var i = 0; i < rs.length; i += 1) {
            var r = rs[i];
            var payload = "FLAGS (" + (r.flags || []).join(" ") + ")";
