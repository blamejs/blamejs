...
function create(opts) {
  ...
  function _handleLine(state, socket, line) {
    ...
    // Literal-opener: stash + emit continuation
    if (parsed.literalSize !== null && parsed.literalSize > 0) {
  ...
  function _capabilityLine(state) {
    var caps = ["IMAP4rev2", "AUTH=PLAIN"];
    if (!state.tls) caps.unshift("STARTTLS");
  ...
  function _handleFetch(state, socket, tag, args) {
    if (!state.selectedMailbox) {
      _writeTagged(socket, tag, "NO No mailbox selected");
      return;
    }
  ...
  function _handleStore(state, socket, tag, args) {
    if (!state.selectedMailbox) {
      _writeTagged(socket, tag, "NO No mailbox selected");
      return;
    }
  ...
  function _handleUid(state, socket, tag, args) {
    ...
    // wrapping the same handlers but tells the backend to interpret
    // the sequence-set as UIDs rather than msg-numbers. Backend is
    // responsible for the actual UID semantics.
    ...
    var subArgs = sub[2];
    // Delegate by composing the sub-verb handler with a `useUid: true`
    // marker the backend can read off mailStore.fetchRange / storeFlags.
    // Since v1 backends may not honor it yet, the response just rides
    // the same path.
    if (subVerb === "FETCH") return _handleFetch(state, socket, tag, subArgs);
    if (subVerb === "STORE") return _handleStore(state, socket, tag, subArgs);
