    ...
    Promise.resolve()
      .then(function () {
        ...
        if (typeof mailStore.quota === "function") {
          return Promise.resolve(mailStore.quota(state.actor, name))
            .then(function (q) {
              if (q && typeof q.usedBytes === "number" &&
                  typeof q.limitBytes === "number" &&
                  q.limitBytes > 0 &&
                  q.usedBytes + literalBody.length > q.limitBytes) {
                var err = new Error("APPEND would exceed quota (used " + q.usedBytes +
                  " + " + literalBody.length + " > limit " + q.limitBytes + ")");
                err.code = "mail-server-imap/overquota";
                err.overquota = true;
                err.limit = q.limitBytes;
                throw err;
              }
