...
async function purge(opts) {
  ...
  var result = await audit().withChainLock(async function () {
    ...
    // way to complete it at all. Those anchors get the end-only match they
    // were written under; every anchor that records a start is held to it.
    ...
    var isRetryOfAnchoredRange = !!anchor &&
      ...
      (anchorFirst === 0 || anchorFirst === Number(v.range.firstCounter));
