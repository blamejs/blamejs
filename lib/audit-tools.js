...
// The lowest counter still present at or below a boundary — where a purge that
// recorded its boundary but never deleted its rows actually has to restart.
// null when nothing is left under the boundary, which says the delete already
// happened and there is no interrupted purge to finish.
...
async function verifyBundle(opts) {
  ...
  // The COUNTER the manifest claims to start at, not only the hash. A bundle
  // rewritten to keep a valid chain over a shorter slice — rows 51..100 under
  // a manifest still claiming to begin at 1 — satisfies every check above,
  // because each one asks about the rows that ARE there. What it then buys is
  // a purge: the boundary is authorized from the claimed start, so rows 1..50
  // are deleted with no copy of them anywhere. The claim has to be bound to
  // the archived rows, and this is the one place every caller reads it
  // through.
  ...
  // The slice row at lastCounter has to exist for the same reason from the
  // other end; a manifest naming a counter no archived row carries would
  // otherwise be caught only by the hash comparison below, which cannot run
  // without it.
  if (!sliceLastRow) {
    return {
      ok: false, kind: read.manifest.kind, rowsVerified: read.rows.length,
...
async function purge(opts) {
  ...
  // The digests the anchor is about to sign have to EXIST. Removing a checksum
  // from a manifest is the cheapest way to defeat a comparison against it: the
  // bundle reader skips a member it has no expectation for, the archive still
  // verifies, and an absent digest would be signed as the empty string. What
  ...
  // signature exists to replace. Every bundle this framework writes records
  // one for each member it includes, so an absent expectation is a failure
  // rather than a pass, and it is refused BEFORE any row is deleted.
  ...
  var applyArgs = {
    ...
    // Taken from the bundle THIS purge verified, and signed with the rest. The
    // manifest states the same digest, but it travels with the payload: an
    // attacker who rewrites rows.enc rewrites the manifest's checksum too, and
    // a reader comparing the two only learns the bundle is consistent with
    // itself. Signed here, it says which bytes the archive has to be, in a
    // claim only the signing key can make.
    archiveRowsDigest:    String(checksums.rowsSha3_512),
    // The covering checkpoint, for the same reason and separately: verifying
    // an archive REQUIRES it, so a bundle whose checkpoint payload has been
    // replaced is unusable however well its rows still match. Bound only to
    // the manifest, that bundle answers "still producible" to a reader asking
    // whether the rows this anchor licensed away can still be produced.
    archiveCheckpointDigest: String(checksums.checkpointSha3_512),
    // And the manifest itself, which is how those payloads are OPENED. The two
    // digests above cover the encrypted bytes; the salts the decryption key is
    // derived from live in the manifest and nothing signed covered them, so
    // editing salts.rows alone left every payload and every signed digest
    // intact while the bundle stopped decrypting — and a reader comparing
    // checksums still called it producible.
    //
    // Over the CANONICAL form rather than the file's raw bytes: the manifest is
    // written canonically, so the two agree, and a reformat is then not a
    // refusal while any change to a value it carries still is.
  ...
  var result = await audit().withChainLock(async function () {
    ...
    // The WHOLE range has to match, not just where it ended. An archive that
    // ends at the same counter but starts later is a different, smaller
    // archive: treating it as a retry would let it delete everything through
    // the boundary while retaining only its own tail, and the rows between
    // the two starts would be gone with no copy anywhere. That is the same
    // unaccounted deletion the anchor exists to prevent, reached through the
    // recovery path instead of around it.
    //
    // A stored start of 0 means the anchor predates the field: chain counters
    // begin at 1, so no real range starts there and the value is unambiguously
    // "unrecorded" rather than a claim. Requiring it to equal the archive's
    // start would reject the very archive that could finish an interrupted
    // purge on such a volume — and the contiguity check would then refuse it
    // too, because the anchor already ends where that archive ends, leaving no
    // way to complete it at all.
    //
    // But "unrecorded" cannot mean "unchecked": accepting any archive that
    // merely ENDS in the right place would let a smaller one — 51..100 under a
    // boundary of 100 — pass as the retry, and the delete that follows removes
    // 1..100. Rows 1..50 would be gone with no copy anywhere, which is the
    // deletion the anchor exists to prevent, reached through the recovery path.
    //
    // The volume answers the question the anchor did not record. An
    // interrupted purge is one that wrote its boundary and did NOT delete, so
    // the rows it failed to remove are still there and the lowest of them is
    // where its archive has to start. When nothing remains under the boundary
    // the delete already happened: there is no interrupted purge to finish, so
    // a "retry" would only replace the anchor with whatever archive was
    // presented — narrower included — and it is refused.
    // Read from storage, never from an option. This value decides whether a
    // DELETE is authorized, so a caller-supplied one would let the check be
    // satisfied by the caller it is checking: hand back the tail archive's own
    // first counter and the narrower archive passes as the retry, which is the
    ...
    if (anchor && anchorFirst === 0) {
      expectedRetryFirst = await _defaultReadLowestLiveCounter(
        Number(anchor.lastPurgedCounter));   // null when the rows are already gone
    }
...
async function signExistingPurgeAnchor(opts) {
  opts = opts || {};
  // Which repairs this call is authorized to make. An operator invoking this
  // directly is asserting both; boot passes only what its flags asserted, so
  // one flag cannot quietly buy the other's repair — they are different
  // claims. "This anchor predates signing, pin what it says" has no
  // cryptographic evidence behind it at all; "I rotated the key myself" is
  // backed by a signature that verifies under the outgoing key.
  ...
  return await audit().withChainLock(async function () {
    ...
    // `rotated-key` joins `unsigned` as re-signable, and it has to: rotating
    // the audit-signing key leaves every existing anchor named after the old
    // one, and refusing that here would make the only repair available the one
    // thing this function turns away — the same trap as an interrupted purge.
    // The signature is checked under the rotated key first, so what gets
    // re-signed is an anchor that key really did produce, not whatever the row
    // happens to say now. That check reads the unsealed history, which cannot
    // AUTHORIZE anything — but it is still evidence, and it is strictly more
    // than the unsigned path has ever had.
    // Not repairable at all — corrupt, forged, a signature that does not
    // verify — throws, as it always has. Signing one of those would convert a
    // detectable problem into a permanent one, and that is true no matter what
    ...
    // Repairable, but this caller did not assert THIS repair.
    ...
    if (verdict.status === "rotated-key") {
      ...
      // The same layouts the read path accepts, through the same function. An
      // anchor written before a signed field joined the payload AND since
      // rotated is exactly the anchor this repair exists for; recognising
      // fewer layouts here than verifyPurgeAnchor does would refuse to fix it
      // and leave the volume unable to boot.
      var genuine = false;
      try {
        genuine = !!rotatedPem &&
    ...
    var args = {
      ...
      // Carried as whatever the row already holds — 0 on an anchor written
      // before the field existed, which is the honest claim: nothing recorded
      // where that purge began, so it is pinned as "from the beginning".
      firstPurgedCounter: Number(anchor.firstPurgedCounter || 0),
      // Same principle: pin what the row records, including nothing. This
      // re-signs an existing anchor; it does not have the archive in hand and
      // must not invent a digest for one.
      ...
      // Pinned from the row for the same reason: this re-signs an existing
      // anchor and does not have the archive in hand, so it must not invent a
      // manifest digest for one it has not read.
    ...
    return {
      signed:               true,
      // Which situation was repaired, so a caller reporting the repair can name
      // the right one rather than assuming the older of the two.
...
// Is the anchor on disk the record THIS purge wrote, rather than another one
// that happens to end at the same place?
function _isSameAnchorRecord(surviving, args) {
  ...
  // Unsigned deployment: compare what the signature would have covered.
...
async function _defaultApplyPurge(args) {
  ...
  // Deliberately the DEFAULT reader, never opts.readAnchor. That option lets a
  // caller say what the PRIOR anchor was; this read asks what is on disk right
  // now, which is the only question whose answer may authorize a delete. A
  // caller-supplied answer here would let the check be satisfied by the caller
  // it is checking.
  ...
  // The boundary alone does not identify an anchor. Two leaders archiving the
  // same range write the same counter and row hash while naming DIFFERENT
  // archives — each with its own bundle id and payload digests, each encrypted
  // under its own key. Comparing only the boundary lets the superseded call
  // mistake the replacement for its own, delete the rows, and report ITS
  // archive id: the operator keeps that bundle, discards nothing else, and the
  // next boot asks the surviving anchor for an archive nobody retained.
  //
  // The signature settles it in one comparison, because it covers every field
  // the anchor carries — bundle id, both digests, the fencing token. Where
  // there is no signature to compare (`auditSigning: false`), the same
  // question is asked of the fields themselves.
...
async function _writePurgeAnchor(args) {
  ...
  var fence = await clusterStorage.fencedUpsert({
    ...
    values: {
      ...
      // Empty string rather than null when absent, so the column stores what
      // the signed payload was built from — the payload renders a missing
      // digest as "", and a row holding null would rebuild it the same way but
      // read back as a different value to anything comparing the two.
