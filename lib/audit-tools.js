...
async function _defaultApplyPurge(args) {
  // Sign the anchor BEFORE anything is deleted. It is the row that decides
  // which rows are allowed to be missing, so it needs the standing of the chain
  // it excuses — unsigned, it was the one record an attacker could write
  // freely, and writing it discarded everything below the counter it named
  // while verification called the volume intact.
  //
  // Every field it covers comes from the caller's own arguments, so nothing
  // here needs the delete to have happened first. Signing afterwards would mean
  // an uninitialized or unavailable signing key fails only once the rows are
  // permanently gone, leaving an unexplained gap and no anchor to explain it —
  // strictly worse than refusing the purge outright.
  var anchorFields = {
    scope:             "audit",
  ...
  // A deployment running `auditSigning: false` has no key, on purpose. Its
  // anchor is written unsigned, exactly as before, and the verifier adopts it
  // on the same terms — reporting the gap as trusted rather than verified. The
  // writer has to allow what the verifier allows, or purge would throw for a
  // configuration the framework supports.
  //
  // Only not-initialized takes that path. A signing key that exists and fails
  // is a real failure and must not quietly downgrade to an unsigned anchor,
  // because an unsigned anchor is precisely what an attacker would write.
  try {
    anchorFields.signature = auditSign.sign(auditChain.purgeAnchorPayload(anchorFields));
    anchorFields.publicKeyFingerprint = auditSign.getPublicKeyFingerprint();
  } catch (err) {
    if (!/not-initialized/.test((err && (err.code || err.message)) || "")) throw err;
    anchorFields.signature = null;
    anchorFields.publicKeyFingerprint = null;
  }

  // Prove the anchor can actually be stored before deleting anything. On an
  // external backend the anchor table is created by framework-schema's DDL,
  // which only ever issues CREATE TABLE IF NOT EXISTS — a volume whose table
  // predates the signature columns would delete the rows and only then fail on
  // an UPSERT naming columns that are not there, leaving a chain gap with no
  // anchor to explain it.
  //
  // The order stays delete-then-anchor rather than the reverse. An anchor
  // written before a delete that then fails would tell verification to skip
  // rows that are still present, hiding live history; a delete that succeeds
  // with no anchor reports a break the operator can see and investigate against
  // the archive they just made. Loud beats quiet, so the probe is what closes
  // the window rather than a reordering.
  var probeBuilt = sql.select("_blamejs_audit_purge_anchor", _sqlOpts())   // allow:hand-rolled-sql
    .columns(["signature", "publicKeyFingerprint"])
    .where("scope", "audit")
    .toSql();
  try {
    await clusterStorage.executeOne(probeBuilt.sql, probeBuilt.params);
  } catch (e) {
    throw new AuditToolsError("audit-tools/anchor-not-signable",
      "purge: the audit purge anchor cannot record a signature on this backend (" +
      ((e && e.message) || String(e)) + "). Nothing has been deleted. The anchor " +
      "table needs its `signature` and `publicKeyFingerprint` columns — re-run " +
      "schema setup against this database, or add them by hand — before a purge " +
      "can record which rows it removed.");
  }
  ...
  var built = sql.upsert("_blamejs_audit_purge_anchor", _sqlOpts())   // allow:hand-rolled-sql
    .columns(["scope", "lastPurgedCounter", "lastPurgedRowHash", "archiveBundleId",
              "purgedAt", "signature", "publicKeyFingerprint"])
    .values(anchorFields)
    .onConflict(["scope"])
    .doUpdateFromExcluded(["lastPurgedCounter", "lastPurgedRowHash", "archiveBundleId",
                           "purgedAt", "signature", "publicKeyFingerprint"])
    .toSql();
