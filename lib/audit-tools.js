...
async function _defaultReadPredecessorRowHash(firstCounter) {
  ...
  if (!row) {
    // allow:hand-rolled-sql — the purge anchor's logical name is its bare `_blamejs_`-prefixed name, which clusterStorage rewrites
 ...
 *   // → { signed: true, lastPurgedCounter: <counter>, publicKeyFingerprint: <hex> }
 *   //   where audit signing is configured, and { signed: false, reason } where
 *   //   it is not
 */
...
async function _writePurgeAnchor(args) {
  // allow:hand-rolled-sql — the purge anchor's logical name is its bare `_blamejs_`-prefixed name, which clusterStorage rewrites and placeholderizes while b.sql quotes the columns and binds the scope
