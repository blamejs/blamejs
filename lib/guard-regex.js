...
/**
 * @primitive  b.guardRegex.assertStateless
 * @signature  b.guardRegex.assertStateless(input, label?, ErrorClass?, code?)
 * @since      0.20.30
 * @status     stable
 * @related    b.guardRegex.assertSafe
 *
 * Refuse a <code>RegExp</code> that carries the <code>g</code> or <code>y</code>
 * flag. With either flag, <code>test()</code> and <code>exec()</code> start at the
 * <code>lastIndex</code> the previous call left. A pattern kept in configuration
 * and run once per request, row or file then gives an answer that depends on the
 * value before it: a <code>/^alice$/g</code> pattern fails on every second
 * <code>"alice"</code>, and a sticky pattern matches only at the offset where the
 * previous match ended. Every framework option that keeps an operator's RegExp
 * and runs it on later calls passes it through this check when the option is
 * accepted.
 *
 * On a flagged RegExp it throws <code>ErrorClass(code, ...)</code> when an error
 * class is supplied, and otherwise a <code>GuardRegexError</code> with code
 * <code>regex/stateful-pattern</code>. It returns a pattern string, or a RegExp
 * without either flag, unchanged.
 *
 * @example
 *   b.guardRegex.assertStateless(/^[a-z]+$/i);           // returns the RegExp
 *   try { b.guardRegex.assertStateless(/^[a-z]+$/g); }   // throws: g flag
 *   catch (e) { e.code; }                                // → "regex/stateful-pattern"
 */
function assertStateless(input, label, ErrorClass, code) {
  if (!(input instanceof RegExp) || (!input.global && !input.sticky)) return input;
  var message = (label || "regex") + ": the " + (input.global ? "g" : "y") +
    " flag makes each test() start where the previous call stopped; remove the flag";
  if (ErrorClass) throw new ErrorClass(code || "regex/stateful-pattern", message);
  throw new GuardRegexError("regex/stateful-pattern", message);
}
...
_guard.assertStateless = assertStateless;
