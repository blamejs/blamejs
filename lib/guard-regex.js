...
function _parsePattern(src, flags, budget) {
  ...
  function parseAtom(depth, activeFlags) {
    ...
    if (c === "^" || c === "$") { pos += 1; return { type: "anchor", flags: activeFlags }; }
    if (c === ".") {
      pos += 1;
    ...
    if (c === "\\") {
      ...
      if (esc === "") return fail();
      pos += 2;
      if (esc === "b" || esc === "B") return { type: "anchor", flags: activeFlags };
      var set = _escapeSet(esc);
      if (set === null) return { type: "opaque", flags: activeFlags };
      if (!spend(_setSize(set))) return fail();
      return { type: "set", set: _applyFold(set, activeFlags), flags: activeFlags };
    }
    if (c === "*" || c === "+" || c === "?" || c === ")") return fail();         // a quantifier with nothing to quantify
    pos += 1;
    return { type: "set", set: _applyFold(_mkSet([c], false), activeFlags), flags: activeFlags };
  }
  ...
  function parseClass(activeFlags) {
    ...
    while (pos < src.length) {
      ...
      if (src.charAt(pos) === "-" && src.charAt(pos + 1) !== "]" && pos + 1 < src.length) {
        pos += 1;
        var hi = _classMember();
        if (hi === null || lo.length !== 1 || hi.length !== 1) { characterised = false; continue; }
      ...
      if (typeof lo === "string") {
        ...
        lo.chars.forEach(function (m) { members.add(m); });                      // a shorthand inside the class
        if (lo.negated !== negated) characterised = false;
  ...
  function parseGroup(depth, activeFlags) {
    ...
    if (src.charAt(pos) === "?") {
      ...
      if (/^\?(?:=|!|<=|<!)/.test(rest.slice(0, 3))) {
        var close = _skipToGroupEnd(open);
        if (close === -1) return fail();
        pos = close + 1;
        return { type: "look", flags: activeFlags };
      }
      ...
      if (named) pos += named[0].length;
        ...
        if (mod[1] && mod[1].indexOf("i") !== -1) innerFlags = _withFlag(activeFlags, "i", true);
        if (mod[2] && mod[2].indexOf("i") !== -1) innerFlags = _withFlag(activeFlags, "i", false);
        pos += mod[0].length;
      }
    }
...
function _applyFold(set, flags) {
  return flags.indexOf("i") !== -1 ? _foldSet(set) : set;
}
...
function _firstSet(node) {
  ...
  if (node.type === "opaque" || node.type === "look") return _anySet();
...
function _allSet(node) {
  if (node.type === "set") return node.set;
  if (node.type === "anchor") return _mkSet([], false);
  if (node.type === "opaque" || node.type === "look") return _anySet();
...
function _waysToMatch(node) {
  if (node.type === "set" || node.type === "anchor") return 1;
  if (node.type === "opaque" || node.type === "look") return Infinity;
...
function _isVariableLength(node) {
  if (node.type === "set" || node.type === "anchor") return false;
  if (node.type === "opaque" || node.type === "look") return true;
...
function _fixedLength(node) {
  ...
  if (node.type === "opaque" || node.type === "look") return null;
...
function _adjacentAmbiguity(seq) {
  for (var i = 0; i < seq.terms.length; i += 1) {
    var left = seq.terms[i];
    if (!_repeatsVariably(left)) continue;
    // A position with a countable number of ways to match contributes a
    // constant, not a factor of the input length — two of them side by side
    // still divide a run a fixed number of ways.
    if (_repetitionIsEnumerable(left)) continue;
    ...
    for (var j = i + 1; j < seq.terms.length; j += 1) {
      ...
      if (_repeatsVariably(right) && !_repetitionIsEnumerable(right) &&
          _setsIntersect(leftSet, rightHead)) {
        if (_hasFallibleTail(seq, j + 1)) return true;
        break;
      }
...
function _hasFallibleTail(seq, from) {
  for (var i = from; i < seq.terms.length; i += 1) {
    var t = seq.terms[i];
    if (t.node.type === "anchor") continue;
    if (t.min === 0) continue;                                     // can match nothing — never fails
    return true;                                                   // must match, so the input can lack it
  }
  return false;
}
...
function _findAmbiguity(node, out) {
  if (node === null || typeof node !== "object") return;
  if (node.type === "alt") {
    for (var b = 0; b < node.branches.length; b += 1) _findAmbiguity(node.branches[b], out);
    return;
  }
  if (node.type === "group") { _findAmbiguity(node.body, out); return; }
  ...
  if (!forced && _adjacentAmbiguity(node)) out.nested = true;
  for (var i = 0; i < node.terms.length; i += 1) {
    var term = node.terms[i];
    _findAmbiguity(term.node, out);
...
function _ambiguityFindings(src, flags) {
  var out = { nested: false, alternation: false };
  var text = String(src);
...
function _detectIssues(input, opts) {
  ...
  var ambiguity = (opts.nestedQuantPolicy !== "allow" ||
    ...
    : { nested: false, alternation: false };
