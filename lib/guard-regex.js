...
function _parsePattern(src, flags, budget) {
  var pos = 0;
  var foldGroups = flags.indexOf("i") === -1 ? new Map() : _engineFoldGroups(src, flags);
  ...
  function parseAtom(depth, activeFlags) {
    ...
    if (c === "^" || c === "$") {
      pos += 1;
      return { type: "anchor", edge: true, flags: activeFlags };
    }
    if (c === ".") {
      pos += 1;
    ...
    if (c === "\\") {
      ...
      if (esc === "b" || esc === "B") return { type: "anchor", edge: false, flags: activeFlags };
