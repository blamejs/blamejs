  ...
  while (i < n) {
    ...
    if (c === ")") {
      ...
      var closeUnbounded = qm !== null && _repeatsMoreThanOnce(qm[0]) &&
        !(grp.open >= 0 &&
          _boundedRepeatPathsSmall(src.slice(grp.open + 1, i), qm[0], budget));
...
function _stripGroupPrefix(body) {
  ...
  var named = /^\?<[A-Za-z_$][A-Za-z0-9_$]*>/.exec(body);
...
// The case-folding in force for a group's CONTENTS. A modifier group changes
// the flags for what it encloses, so `(?i:a|A)` is two branches the engine
// reads as one even when the pattern itself carries no flag — and `(?-i:...)`
// inside a case-insensitive pattern is the reverse. Reading the group's body
// under the pattern's own flags would prove those branches distinct and
// suppress the finding they exist to raise.
var MODIFIER_PREFIX_RE = /^\?([dgimsuvy]{0,8})(?:-([dgimsuvy]{1,8}))?:/;

function _prefixFold(body, fold) {
  // The longest prefix it can match is nineteen characters.
  var m = MODIFIER_PREFIX_RE.exec(body.slice(0, 20));
  if (m === null) return fold;
  if (m[1] && m[1].indexOf("i") !== -1) return true;
  if (m[2] && m[2].indexOf("i") !== -1) return false;
  return fold;
}
...
// How many lengths an atom's quantifier admits, or null when there is no
// ceiling on them.
function _quantWays(q) {
  if (q === "") return 1;
  if (q === "?" || q === "??") return 2;
  if (q === "*" || q === "+" || q === "*?" || q === "+?") return null;
  var m = /^\{(\d*)(?:,(\d*))?\}\??$/.exec(q);
  if (m === null) return null;
  if (m[2] === undefined) return 1;                   // {n} — exactly n
  if (m[2] === "") return null;                       // {n,} — no ceiling
  var span = Number(m[2]) - Number(m[1]) + 1;
  return span > 0 ? span : null;
}

// A repeat whose ceiling AND whose body's own variation are both finite
// explores a fixed number of ways to match, however long the input is. Three
// dotted octets, `(?:\d{1,3}\.){3}`, is twenty-seven of them — which is why a
// dotted quad with a port or a prefix length after it is an ordinary pattern
// and not a catastrophic one. A quantifier without a ceiling anywhere in the
// body makes the count unbounded again, so `(a+){2,30}` is not covered.
var MAX_BOUNDED_PATHS = 4096;

function _boundedRepeatPathsSmall(bodySrc, quant, budget) {
  var qm = /^\{(\d*)(?:,(\d*))?\}\??$/.exec(quant);
  if (qm === null) return false;                      // * or + — no ceiling
  var maxRepeats = qm[2] === undefined ? Number(qm[1])
                 : (qm[2] === "" ? Infinity : Number(qm[2]));
  if (!isFinite(maxRepeats)) return false;
  var stripped = _stripGroupPrefix(bodySrc);
  ...
  for (var b = 0; b < branches.length; b += 1) {
    var flat = _inlinePlainGroups(branches[b], 0);
    if (flat === null) return false;
    var atoms = _parseAtoms(flat, budget);
    if (atoms === null) return false;
    var ways = 1;
    for (var i = 0; i < atoms.length; i += 1) {
      var w = _quantWays(atoms[i].quant);
      if (w === null) return false;
      ways *= w;
      if (ways > MAX_BOUNDED_PATHS) return false;
    }
    perRepetition += ways;
  }
  if (perRepetition <= 1) return true;                // one way per repetition
  var total = Math.pow(perRepetition, maxRepeats);
...
function _hasQuantifiedAlternation(src) {
  ...
  _walkGroups(src, function (body, quant) {
    ...
    if (_boundedRepeatPathsSmall(body, quant, _budget())) return;
...
function _topLevelTerms(src, budget, flags) {
  ...
  var termFold = foldsCase;          // reset per group from its own modifier prefix
  function foldSet(set) { return termFold ? _foldCase(set) : set; }
  while (i < src.length) {
    var c = src.charAt(i);
    ...
    if (c === "(") {
      ...
      if (close === -1) return null;
      var qm = /^(?:[*+?]\??|\{\d*(?:,\d*)?\}\??)/.exec(src.slice(close + 1));
      var gq = qm ? qm[0] : "";
      termFold = _prefixFold(src.slice(i + 1, close), foldsCase);
      ...
      terms.push({
        variable: bodyVaries || gq === "*" || gq === "+" || gq === "*?" ||
                  gq === "+?" || /^\{\d*,\}\??$/.test(gq) || _isVariableQuant(gq),
    ...
    termFold = foldsCase;                                    // a bare atom is under the pattern's own flags
    ...
    var aq = /^(?:[*+?]\??|\{\d*(?:,\d*)?\}\??)/.exec(src.slice(i + consumed));
    var quant = aq ? aq[0] : "";
...
function _alternationBranchesProvablyDisjoint(src, flags) {
  ...
  _walkGroups(src, function (body, quant) {
    ...
    for (var i = 0; i < branches.length; i += 1) {
      ...
      firsts.push(groupFold ? _foldCase(first) : first);
    }
