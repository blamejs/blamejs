...
function _hasNestedQuantifier(src, flags) {
  ...
  var justOpenedGroup = false;   // previous token was an unescaped `(`
  var justQuantified = false;    // previous token was a quantifier a `?` could make lazy
  var i = 0;
  var n = src.length;
  // Any quantifier here, then judged by whether it can repeat more than once.
  var AFTER_GROUP_QUANT = /^(?:[*+?]\??|\{\d{0,19}(?:,\d{0,19})?\}\??)/;
  ...
  while (i < n) {
    var c = src.charAt(i);
    if (c === "\\") { justOpenedGroup = false; justQuantified = false; i += 2; continue; }
    if (inClass) { if (c === "]") inClass = false; i += 1; continue; }
    if (c === "[") { inClass = true; justOpenedGroup = false; justQuantified = false; i += 1; continue; }
    if (c === "(") {
      stack.push({ quant: false, open: i });
      justOpenedGroup = true; justQuantified = false;
      i += 1; continue;
    }
    if (c === ")") {
      var grp = stack.pop() || { quant: false, open: -1 };
      var qm = AFTER_GROUP_QUANT.exec(src.slice(i + 1));           // allow:regex-no-length-cap — bounded slice of a maxPatternBytes-capped input
      var closeUnbounded = qm !== null && _repeatsMoreThanOnce(qm[0]);
    ...
    // marker on a quantifier already counted. Which of those applies is a
    // property of the PREVIOUS TOKEN, not of the previous source character —
    // reading the character called `a\*?` a lazy marker, because after the
    // two-character escape the character behind the `?` is the `*` itself,
    // and dropped the length variation the optional actually contributes.
    if (c === "?") {
      if (!justOpenedGroup && !justQuantified) {
        if (stack.length) stack[stack.length - 1].quant = true;
        justQuantified = true;
      } else {
        justQuantified = false;                                    // lazy marker consumed
      }
      justOpenedGroup = false;
      i += 1; continue;
    }
    if (c === "{") {
      ...
      if (open) {
        if (stack.length) stack[stack.length - 1].quant = true;
        justOpenedGroup = false; justQuantified = true;
      ...
      justOpenedGroup = false; justQuantified = false;
...
function _parseAtoms(src, budget) {
  ...
  while (i < src.length) {
    ...
    if (q === "*" || q === "+" || q === "?") {
      ...
      var m = /^\{\d{1,19}(?:,\d{0,19})?\}/.exec(src.slice(i));
      if (!m) return null;
      quant = m[0];
      i += m[0].length;
    }
...
function _stripGroupPrefix(body) {
  ...
  // The name bound is a backstop on this matcher, not a rule about names —
  // ECMAScript sets no limit. A name past it used to fall through to "not a
  // group I read", and the alternation check treated that as out of scope, so
  // a long enough name hid the pattern it was wrapped around.
  var named = /^\?<[A-Za-z_$][A-Za-z0-9_$]{0,255}>/.exec(body);
  if (named) return body.slice(named[0].length);
  // Modifier groups — `(?i:`, `(?-i:`, `(?im-sx:`. Non-capturing groups that
  // change flags for their contents; nothing about repetition differs.
  var modifier = /^\?[dgimsuvy]{0,8}(?:-[dgimsuvy]{1,8})?:/.exec(body);
  if (modifier) return body.slice(modifier[0].length);
  return null;
}

// A group prefix that means "look, do not consume". Those have their own
// detector, and are the only reason an unreadable prefix is not a refusal.
var LOOKAROUND_PREFIX_RE = /^\?(?:=|!|<=|<!)/;
...
function _matchingParen(src, open) {
  var depth = 0;
  var inClass = false;
...
var MAX_INLINE_DEPTH = 8;
var GROUP_QUANT_RE = /^(?:[*+?]\??|\{\d{0,19}(?:,\d{0,19})?\}\??)/;

// Rewrite a branch with its plain groups inlined. A group that neither repeats
// nor offers a choice — `(x)`, `(?:foo)` — matches exactly its contents, so
// for first-character and length purposes it IS its contents, and dropping the
// parentheses lets the atom parser read a branch it would otherwise refuse to
// touch. A group that quantifies or alternates inside is left alone: the parse
// then fails and the pattern stays unproven, which is the safe answer.
// Returns null when nothing can be said.
function _inlinePlainGroups(src, depth) {
  ...
  while (i < src.length) {
    ...
    if (inClass) { out += c; if (c === "]") inClass = false; i += 1; continue; }
    if (c === "[") { inClass = true; out += c; i += 1; continue; }
    if (c === ")") return null;                     // unbalanced
    if (c !== "(") { out += c; i += 1; continue; }
    var close = _matchingParen(src, i);
    if (close === -1) return null;
    var quantMatch = GROUP_QUANT_RE.exec(src.slice(close + 1));   // allow:regex-no-length-cap — bounded slice of a maxPatternBytes-capped input
    if (quantMatch && quantMatch[0].length > 0) return null;      // repeats — not plain
    var body = _stripGroupPrefix(src.slice(i + 1, close));
    if (body === null) return null;                 // lookaround — a different detector
    var branches = _splitTopLevelAlternation(body);
    if (branches === null || branches.length !== 1) return null;  // offers a choice
    var inner = _inlinePlainGroups(branches[0], depth + 1);
...
// Peel wrappers that neither repeat nor choose. `((a|b))+` repeats exactly what
// `(a|b)+` repeats — the extra parentheses change nothing about how the engine
// backtracks — so reading only the outermost body finds no alternation where
// the engine finds one, and the inner group carries no quantifier of its own
// for a second look to catch.
function _peelPlainWrappers(inner) {
  var current = inner;
  for (var d = 0; d < MAX_INLINE_DEPTH; d += 1) {
    if (current.charAt(0) !== "(") return current;
    // A lone wrapper spans the whole body, so nothing follows it to quantify.
    var close = _matchingParen(current, 0);
    if (close !== current.length - 1) return current;
    var body = _stripGroupPrefix(current.slice(1, close));
    if (body === null) return current;              // lookaround — a different detector
    current = body;
  }
  return current;
}

// The stretches of a fragment that contain no group, with each group and its
// own quantifier cut out. Two atoms separated by a group are not adjacent, and
// what is inside the group is examined on its own.
function _groupFreeSegments(text) {
  var segs = [];
  var current = "";
  var depth = 0;
  var inClass = false;
  ...
  while (i < text.length) {
    var c = text.charAt(i);
    if (c === "\\") { if (depth === 0) current += text.slice(i, i + 2); i += 2; continue; }
    if (inClass) { if (depth === 0) current += c; if (c === "]") inClass = false; i += 1; continue; }
    if (c === "[") { inClass = true; if (depth === 0) current += c; i += 1; continue; }
    if (c === "(") {
      if (depth === 0) { segs.push(current); current = ""; }
    ...
    if (c === ")") {
      ...
      if (depth <= 0) {
        depth = 0;
        var q = GROUP_QUANT_RE.exec(text.slice(i + 1));    // allow:regex-no-length-cap — bounded slice of a maxPatternBytes-capped input
        i += 1 + (q ? q[0].length : 0);
        continue;
      }
      i += 1; continue;
...
// Two variable-length quantifiers side by side over characters they share
// split a run of those characters between them in as many ways as the run is
// long, and k of them in as many ways as the run has compositions into k
// parts. `\w*\w*!` is quadratic and ten of them is degree ten. No group is
// involved in any of it, so every detector that walks groups sees nothing —
// `(?:a*a*-)*` was refused only because something happened to enclose it.
function _hasAdjacentOverlappingQuantifiers(src, flags) {
  var budget = _budget();
  var fold = _ignoresCase(flags);
  var found = false;

  function examine(fragment) {
    if (found) return;
    var segments = _groupFreeSegments(fragment);
    for (var s = 0; s < segments.length && !found; s += 1) {
      // Anchors carry no characters; `.` is every character but a newline,
      // which _parseAtoms declines to read but which intersects everything.
      var text = segments[s].replace(/[\^$]/g, "").replace(/\./g, "[^\\n]");
      var branches = _splitTopLevelAlternation(text);
      if (branches === null) continue;
      for (var br = 0; br < branches.length && !found; br += 1) {
        var atoms = _parseAtoms(branches[br], budget);
        if (atoms === null) continue;               // not a fragment this reads
        for (var a = 0; a + 1 < atoms.length; a += 1) {
          if (!_isVariableQuant(atoms[a].quant)) continue;
          if (!_isVariableQuant(atoms[a + 1].quant)) continue;
          var left = fold ? _foldCase(atoms[a].set) : atoms[a].set;
          var right = fold ? _foldCase(atoms[a + 1].set) : atoms[a + 1].set;
          if (_setsIntersect(left, right, budget)) { found = true; break; }
        }
      }
    }
  }
  ...
  examine(src);
  _walkGroups(src, function (body) {
    if (found) return;
    var stripped = _stripGroupPrefix(body);
    if (stripped !== null) examine(stripped);
  });
  return found;
}

// Can this quantifier take its group more than once? That, not whether it has
// a ceiling, is what lets a group's parts compete for the same input: `{2,30}`
// still admits every way of dividing the input among the repetitions, and an
// exact `{10}` is polynomial of degree nine. Only a quantifier that permits at
// most ONE repetition — `?`, `{0,1}`, `{1}` — cannot multiply the paths, which
// is why `(a+)?` stays linear while `(a+){1,2}` does not.
function _repeatsMoreThanOnce(quant) {
  if (quant === "" || quant === "?" || quant === "??") return false;
  if (quant.charAt(0) !== "{") return true;                    // * + *? +?
  var m = /^\{(\d{0,19})(,(\d{0,19}))?\}\??$/.exec(quant);
  if (m === null) return true;                                 // unrecognised — assume it repeats
  if (m[2] !== undefined && m[3] === "") return true;          // {n,} — no ceiling
  var max = Number(m[2] === undefined ? m[1] : m[3]);
  return !isFinite(max) || max > 1;
}

// Does the pattern contain an alternation repeated without a ceiling? The
// shape that makes branch overlap expensive, found the way the analysis that
// judges it walks the pattern — so the two cannot disagree about which
// patterns are in scope.
function _hasQuantifiedAlternation(src) {
  var found = false;
  _walkGroups(src, function (body, quant) {
    if (found || !_repeatsMoreThanOnce(quant)) return;
    var stripped = _stripGroupPrefix(body);
    if (stripped === null) {
      // A prefix this parser cannot read leaves the group's contents unread,
      // and under a quantifier that repeats, unread is unproven — which has to
      // refuse, not wave through. Wrapping the canonical overlapping
      // alternation in a modifier group silenced this check entirely.
      // The longest prefix it can match is three characters (`?<=`).
      if (!LOOKAROUND_PREFIX_RE.test(body.slice(0, 3))) found = true;
      return;
    }
    var branches = _splitTopLevelAlternation(_peelPlainWrappers(stripped));
    if (branches !== null && branches.length >= 2) found = true;
  });
  return found;
}
...
function _topLevelTerms(src, budget, flags) {
  var terms = [];
  var i = 0;
  // Fold each atom's set HERE, before any of them is combined. Case folding
  // does not survive a union: `[^ab]` unioned with `A` is everything-but-a-
  // and-b, and folding that result excludes `a` — which is precisely the
  // character the `A` was contributing under `i`. A consumer that folds the
  // union it is handed reads a term as unable to match a delimiter it plainly
  // can, and the boundary proof then admits a super-linear pattern. Fold the
  // parts, union the folded parts, and every consumer sees case-correct sets.
  var foldsCase = _ignoresCase(flags);
  function foldSet(set) { return foldsCase ? _foldCase(set) : set; }
  while (i < src.length) {
    var c = src.charAt(i);
    ...
    if (c === "(") {
      ...
      if (close === -1) return null;
      var qm = /^(?:[*+?]\??|\{\d{0,19}(?:,\d{0,19})?\}\??)/.exec(src.slice(close + 1));
      var gq = qm ? qm[0] : "";
      ...
      for (var bi = 0; bi < branches.length; bi += 1) {
        var flatBranch = _inlinePlainGroups(branches[bi], 0);
        if (flatBranch === null) return null;
        var atoms = _parseAtoms(flatBranch, budget);
        if (atoms === null || atoms.length === 0) return null;
        var f = _firstSet(atoms);
        if (f === null) return null;
        firsts.push(foldSet(f));
        if (branches.length > 1) bodyVaries = true;   // branches of differing length
        for (var ai = 0; ai < atoms.length; ai += 1) {
          everySet.push(foldSet(atoms[ai].set));
      ...
      terms.push({
        variable: bodyVaries || gq === "*" || gq === "+" || gq === "*?" ||
                  gq === "+?" || /^\{\d{0,19},\}\??$/.test(gq) || _isVariableQuant(gq),
    ...
    var aq = /^(?:[*+?]\??|\{\d{0,19}(?:,\d{0,19})?\}\??)/.exec(src.slice(i + consumed));
    var quant = aq ? aq[0] : "";
    terms.push({ variable: _isVariableQuant(quant), first: foldSet(one[0].set),
                 all: foldSet(one[0].set), delimiter: null });
    i += consumed + quant.length;
  }
  return terms;
}

// Union of character sets, complements included.
//
// A complement's `chars` are what it EXCLUDES, so folding them in alongside a
// positive set's members inverts the meaning: the union of `[^b]` and `a` came
// out as `[^ba]`, a set that excludes the very character it was supposed to
// gain. Read the wrong way round by the boundary proof, that says a later term
// cannot consume an earlier delimiter when it plainly can.
//
// With any complement present the union is itself a complement: it covers
// everything except the characters EVERY complement excludes and NO positive
// set supplies.
function _unionSets(sets) {
  var negs = [], poss = [];
  for (var i = 0; i < sets.length; i += 1) (sets[i].negated ? negs : poss).push(sets[i]);
  if (negs.length === 0) {
    var chars = [];
    for (var p = 0; p < poss.length; p += 1) {
      for (var j = 0; j < poss[p].chars.length; j += 1) _pushChar(chars, poss[p].chars[j]);
    }
    return _set(chars, false);
  }
  var excluded = [];
  for (var k = 0; k < negs[0].chars.length; k += 1) {
    var ch = negs[0].chars[k];
    var inEvery = true;
    for (var n = 1; n < negs.length && inEvery; n += 1) {
      if (negs[n].chars.indexOf(ch) === -1) inEvery = false;
    }
    if (!inEvery) continue;
    var supplied = false;
    for (var q = 0; q < poss.length && !supplied; q += 1) {
      if (poss[q].chars.indexOf(ch) !== -1) supplied = true;
    }
    if (!supplied) _pushChar(excluded, ch);
  }
  return _set(excluded, true);
}
...
function _concatenationBoundariesForced(src, budget, flags) {
  ...
  for (var v = 0; v < variable.length - 1; v += 1) {
    ...
    for (var later = idx + 1; later < terms.length; later += 1) {
      var reach = terms[later].all;                  // already folded per-atom
...
function _alternationBranchesProvablyDisjoint(src, flags) {
  ...
  _walkGroups(src, function (body, quant) {
    if (!proven) return;
    if (!_repeatsMoreThanOnce(quant)) return;
    var stripped = _stripGroupPrefix(body);
    ...
    for (var i = 0; i < branches.length; i += 1) {
      var flat = _inlinePlainGroups(branches[i], 0);
      if (flat === null) { proven = false; return; }
      var atoms = _parseAtoms(flat, budget);
...
function _detectIssues(input, opts) {
  ...
  if (opts.nestedQuantPolicy !== "allow" &&
      _hasAdjacentOverlappingQuantifiers(input, opts.regexFlags)) {
    issues.push({
      kind: "adjacent-quantifiers", severity: "critical",
      ruleId: "regex.adjacent-quantifiers",
      snippet: "pattern repeats the same characters at two adjacent positions " +
               "(e.g. `\\w*\\w*` / `a*a*b`) — a run of them can be split " +
               "between the quantifiers every possible way, which is " +
               "polynomial in how many of them there are; give each position " +
               "characters the next one cannot match, or use one quantifier",
    });
  }

  if (opts.alternationQuantPolicy !== "allow" &&
      _hasQuantifiedAlternation(input) &&
