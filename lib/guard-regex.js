...
function _headRunsAway(terms, multiline) {
  ...
  var head = terms[i];
  var reach = _maxLength(head.node);
  if (!(reach === Infinity || head.max === Infinity && reach > 0)) return false;
  for (var j = i + 1; j < terms.length; j += 1) {
    var later = terms[j];
    if (later.node.type === "anchor" || later.node.type === "look") continue;
    if (later.min > 0) return true;                        // must match, so an attempt can fail late
  }
  return false;
}
