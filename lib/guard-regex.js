  ...
  function parseGroup(depth, activeFlags) {
    ...
    if (src.charAt(pos) === "?") {
      ...
      if (look) {
        ...
        var negatedLook = look[0].indexOf("!") !== -1;
        pos += look[0].length;
        var lookBody = parseAlt(depth + 1, innerFlags);
        if (lookBody === null) return fail();
        if (src.charAt(pos) !== ")") return fail();
        pos += 1;
        return { type: "look", body: lookBody, negated: negatedLook, flags: activeFlags };
      }
...
// Can this node match some string made only of characters from `alphabet`?
//
// Asked of a lookahead's body, with the run's own characters as the alphabet:
// backtracking hands the assertion a position that begins with run characters,
// as many of them as the run is long, so an assertion satisfiable out of them
// succeeds at the first backtrack deep enough. Knowing only what it can START
// with is not enough — `(?=a[^a])` begins with an `a` a run of `a` supplies and
// then demands something that run can never provide, and fails at every depth.
//
// Anything it cannot read — a nested assertion, an unparsed construct — answers
// no, which classes the enclosing pattern as a repeated scan rather than
// vouching for it.
function _matchableWithin(node, alphabet) {
  if (node.type === "set") return _setsIntersect(node.set, alphabet);
  if (node.type === "group") return _matchableWithin(node.body, alphabet);
  if (node.type === "anchor" || node.type === "look" || node.type === "opaque") return false;
  if (node.type === "alt") {
    for (var b = 0; b < node.branches.length; b += 1) {
      if (_matchableWithin(node.branches[b], alphabet)) return true;
    }
    return false;
  }
  for (var i = 0; i < node.terms.length; i += 1) {
    ...
    if (!_matchableWithin(t.node, alphabet)) return false;
  }
  return true;
}
...
function _varyingPartsCannotTrade(terms) {
  ...
  for (var v = 0; v < varying.length; v += 1) {
    for (var w = v + 1; w < varying.length; w += 1) {
      ...
      // They can also trade THROUGH a fixed-width term between them: in
      ...
      for (var k = varying[v].at + 1; k < varying[w].at; k += 1) {
        var bridge = _allSet(terms[k].node);
        if (_setsIntersect(bridge, varying[v].set) &&
            _setsIntersect(bridge, varying[w].set)) return false;
      }
    }
  }
  return true;
...
function _branchScanIsQuadratic(seq, multiline) {
  if (seq.type !== "seq") return false;
  return _termsScanIsQuadratic(_inlineForScan(seq.terms), multiline);
}

function _termsScanIsQuadratic(terms, multiline) {
  // A choice at the head is several scans, not one: only one branch of
  // `(?:x|a+b)` runs away, and that is enough.
  for (var g = 0; g < terms.length; g += 1) {
    ...
    if (term.min === 1 && term.max === 1 && term.node.type === "group" &&
      ...
      for (var br = 0; br < term.node.body.branches.length; br += 1) {
        ...
        if (_termsScanIsQuadratic(terms.slice(0, g).concat(spliced), multiline)) return true;
      }
      return false;
    }
    break;
  }
  return _headRunsAway(terms, multiline);
}

function _headRunsAway(terms, multiline) {
  var i = 0;
  for (; i < terms.length; i += 1) {
    if (terms[i].node.type === "look") continue;
    if (terms[i].node.type === "anchor") {
      // A leading `^` pins the attempt to one position. Under `m` it matches at
      // every line start instead, which is fewer positions than characters but
      // still grows with the input.
      if (terms[i].node.edge === "start" && !multiline) return false;
      continue;
    }
    break;
  }
...
function _canFailAfterRun(terms, from) {
  ...
  for (var j = from + 1; j < terms.length; j += 1) {
    ...
    if (later.node.type === "look") {
      ...
      // whether it can even begin. When everything it forbids is something the
      // run eats — `a+(?!a)` — a greedy run leaves a character it could not
      // eat, so the assertion holds on the first try. When it forbids nothing
      // the run eats — `a+(?!b)` — one handed-back character is enough. Only
      // the partial overlap fails repeatedly: `a+(?![ab])` meets the forbidden
      // `b` past the run and then walks back through a run of forbidden `a`s,
      // refusing at every step.
      ...
      if (body) {
        ...
        } else if (_matchableWithin(body, reach)) continue;
      }
      return true;
    }
    ...
    var head = _firstSet(later.node);
    if (head === null) continue;                             // nullable — asks for nothing
    ...
    if (_setIsSubsetOf(reach, head)) continue;
    return true;
  }
  return false;
}
