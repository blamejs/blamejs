...
var GROUP_QUANT_RE = /^(?:[*+?]\??|\{\d{0,9}(?:,\d{0,9})?\}\??)/;
...
// The group quantifiers that repeat without a ceiling — the ones that let a
// group's branches compete for the same input. `{n,}` is `+` under another
// spelling and belongs here; `?` and `{n,m}` cap the repetition and do not.
function _isUnboundedGroupQuant(quant) {
  return quant === "*" || quant === "+" || quant === "*?" || quant === "+?" ||
         /^\{\d{0,9},\}\??$/.test(quant);
}
...
function _hasQuantifiedAlternation(src) {
  ...
  _walkGroups(src, function (body, quant) {
    if (found || !_isUnboundedGroupQuant(quant)) return;
...
function _alternationBranchesProvablyDisjoint(src, flags) {
  ...
  _walkGroups(src, function (body, quant) {
    if (!proven) return;
    if (!_isUnboundedGroupQuant(quant)) return;
    var stripped = _stripGroupPrefix(body);
    if (stripped === null) return;                  // lookaround — a different detector
    var branches = _splitTopLevelAlternation(_peelPlainWrappers(stripped));
