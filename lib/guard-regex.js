  ...
  while (i < n) {
    ...
    if (c === ")") {
      ...
      if (grp.quant && closeUnbounded && grp.open >= 0 &&
          _delimitedBodyForcesSplit(src.slice(grp.open + 1, i), budget)) {
...
function _isUnboundedQuant(q) {
  return q === "*" || q === "+" || /^\{\d{0,9},\}$/.test(q);
}

function _hasUnboundedQuant(atoms) {
  return atoms.some(function (a) { return _isUnboundedQuant(a.quant); });
}
...
function _delimitedBodyForcesSplit(body, budget) {
  ...
  for (var i = 0; i < atoms.length - 1; i += 1) {
    if (!_spend(budget, atoms[i].set.chars.length)) return false;
    if (_setContains(atoms[i].set, delimiter)) return false;
  }
  return true;
}
...
function _alternationBranchesProvablyDisjoint(src) {
  var budget = _budget();
  var proven = true;
  var examined = false;
  _walkGroups(src, function (body, quant) {
    ...
    for (var i = 0; i < branches.length; i += 1) {
      ...
      // An unbounded quantifier inside a branch would make the branch's LENGTH
      // ambiguous as well as its choice; that shape is the nested-quantifier
      // class and is refused by the detector above regardless.
      if (atoms === null || _hasUnboundedQuant(atoms)) { proven = false; return; }
...
function _detectIssues(input, opts) {
  ...
  if (opts.alternationQuantPolicy !== "allow" &&
      ...
      !_alternationBranchesProvablyDisjoint(input)) {
    issues.push({
