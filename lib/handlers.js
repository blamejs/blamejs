 ...
 * sink. Each historically used a fire-and-forget Promise pattern,
 * which has well-known failure modes:
 *
 ...
 *           backoff + jitter (asyncSafe.asyncRetry). Default 3 attempts.
...
function _err(code, message, cause) {
  ...
  e.code = code;
  e.isHandlerError = true;
  if (cause !== undefined) e.cause = cause;
  return e;
}

function create(opts) {
  if (!opts || typeof opts.flush !== "function") {
  ...
  var retryConfig       = Object.assign({}, DEFAULTS.retry, opts.retry || {});
  ...
  var onError           = (typeof opts.onError    === "function") ? opts.onError    : function (err) {
    console.error("[blamejs:handlers/" + name + "] " + (err && err.message ? err.message : String(err)));
  };
  ...
  var _shutdown = false;
  var _drainMutex = new asyncSafe.Mutex();
  var _breaker = new asyncSafe.CircuitBreaker("handler:" + name, breakerConfig);
  ...
  // a drain land in the same buffer and are processed in subsequent
  // iterations; concurrent drain() calls share the in-flight Promise.
  function drain(drainOpts) {
    ...
    _drainPromise = _drainMutex.runExclusive(async function () {
      ...
      while (_buffer.length > 0) {
        ...
        var batch = _buffer.splice(0, maxBatch);
        ...
        try {
          await _breaker.wrap(async function () {
            ...
            var attempts = 0;
            await asyncSafe.asyncRetry(async function () {
  ...
  async function shutdown(shutdownOpts) {
    ...
    _cancelAgeFlush();
    try {
      await asyncSafe.withTimeout(drain(), timeoutMs, { name: "handler:" + name + ".shutdown" });
    } catch (e) {
