...
function safeDecompress(input, opts) {
  ...
  try {
    ...
    //
    // Two different things arrive here and a caller has to tell them apart: a
    // stream that expanded past `maxOutputBytes`, which is amplification and
    // says something about the sender, and a stream that is malformed, which
    // says something about the transfer. They get separate codes, because the
    // alternative is every caller walking the `.cause` chain and matching on
    // zlib's wording -- and that wording has changed across Node releases
    // ("Output length exceeded the limit" and "Cannot create a Buffer larger
    // than N bytes" are the same refusal).
