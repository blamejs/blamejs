 ...
 * field. Existing pre-v0.1.54 deployments wrote no algorithm field;
 * those files are loaded as ML-DSA-87 (the previous implicit default)
 * and continue to verify their checkpoint history. Operators who
 * want to migrate rotate their audit-signing key.
 *
 * Design:
...
var passphraseSource = require("./vault-passphrase-source");
...
// their own algorithm via the on-disk `algorithm` field (legacy
// files without that field load as "ml-dsa-87" — the pre-v0.1.54
// implicit default).
...
var log = createLogger("audit-sign");
