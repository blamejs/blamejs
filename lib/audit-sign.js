...
// node:crypto from the PEM), smaller signatures (3,309 bytes vs 4,627
// at 87 / 49,856 at SLH-DSA-SHAKE-256f), faster verify. Operators with
 ...
 *   // -> "-----BEGIN PUBLIC KEY-----\n..." (or null)
 */
// Find a history entry for `fp` and RETURN IT ONLY IF THE KEY HASHES TO THAT
// FINGERPRINT. The label alone is not evidence: the history file is
// deliberately unsealed so a rotated-out key can be resolved without a
// passphrase, so anyone able to write it can put their own PEM under the
// expected fingerprint, sign a purge anchor with the matching private key, and
// have verification accept the forged boundary — never touching the wrapped
// key this mechanism exists to protect. The fingerprint IS the hash of the key
// text, so making the entry prove its own name costs one hash.
//
// Both callers route through here: two lookups over the same file answering
// the same question is exactly where one of them gets hardened and the other
// does not.
...
async function reSignAll(iter, opts) {
  ...
  for await (var entry of iter) {
    try {
      ...
      // run reports progress to whatever the operator is writing it into, which
      // is as likely to be an async sink as a console line.
...
async function rotateSigningKey(rotOpts) {
  ...
  //
  // The algorithm is settled FIRST, before either branch does any work with
  // the key. A caller who gets both wrong should hear about the algorithm:
  // it is the field they chose deliberately, and reporting the key instead
  ...
  if (typeof rotOpts.privateKeyPem === "string" && typeof rotOpts.publicKeyPem === "string") {
    // A fingerprint is the hash of the PEM TEXT, so the spelling of a
    // supplied key becomes its identity. The same key saved with CRLF line
    // endings would take a different fingerprint from the same key saved with
    // LF, and every later reader that re-serializes before hashing — an
    // offline verifier handed the key on the command line, say — would
    // compute the other one and report a healthy chain as signed by an
    // unknown key. Re-exporting through the parsed key at the point of
    // ingestion means one key has one spelling from here on, and validates
    // the supplied PEM while we are at it.
    var canonicalPublicPem;
    try {
      canonicalPublicPem = canonicalPublicKeyPem(rotOpts.publicKeyPem);
    } catch (e) {
      throw _err("ROTATE_BAD_KEY",
...
function _normalizeTip(tip, fnLabel) {
  ...
  // Safe-integer: the counter is rendered into the signed bytes as text, so
  // above 2^53 two distinct tips render identically and one signature would
  // cover both.
...
function _verifyOneAnchor(a) {
  ...
  // Safe-integer, not merely finite. The signed bytes carry the counter as
  // String(counter), so above 2^53 two distinct counters render identically
  // and one signature would verify for both — the anchor could be moved
  // between them without breaking anything.
