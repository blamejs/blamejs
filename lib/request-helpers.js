...
function requestProtocol(req, opts) {
  ...
  // The HTTP/2 `:scheme` pseudo-header, which is the scheme the client used
  // and comes from the connection framing the same way `:authority` does. It
  // is read AFTER the trusted forwarded header and BEFORE the socket: behind a
  // TLS-terminating proxy the backend leg can be cleartext h2, so `:scheme`
  // says `http` there while the browser used `https`, and the gated forwarded
  // header is the only one that knows. It is read at all because an h2 request
  // object -- an RFC 8441 extended CONNECT is built from headers alone -- may
  // carry no socket to ask.
