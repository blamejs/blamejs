...
function requestProtocol(req, opts) {
  ...
  // The socket answers next, and it answers before `:scheme`. Unlike
  // `:authority`, which node derives from the connection, `:scheme` is a value
  // the client writes: over cleartext h2c a direct client may send
  // `:scheme: https` on a socket that is not encrypted, and node passes it
  // through. Reading it first let such a request report HTTPS without passing
  // any trusted-proxy predicate, which reaches the `secure` attribute on a
  // session cookie and the DPoP `htu` a token is bound to. Where a socket
  // exists, its encryption state is not something the peer can assert.
  // Presence decides, not encryption: falling through an UNENCRYPTED socket to
  // `:scheme` is the same hole, since that is exactly the case a cleartext h2c
  // client claiming `https` produces.
  var sock = req.socket || req.connection || null;
  if (sock) return sock.encrypted ? "https" : "http";
  // `:scheme` last, for a request object that carries no socket to ask -- an
  // RFC 8441 extended CONNECT is built from headers alone. Behind a
  // TLS-terminating proxy the backend leg can be cleartext h2 and `:scheme`
  // says `http` where the browser used `https`; the gated forwarded header
  // above is the only one that knows, which is why it is read first.
...
function requestHost(req, opts) {
  ...
  // `:authority` BEFORE `Host`, and not merely as a fallback.
  //
  // Node does not synthesize one from the other. RFC 9113 section 8.3.1 makes
  // `:authority` the HTTP/2 form and says clients SHOULD NOT send `Host`, so
  // over h2 -- which this framework's TLS listeners negotiate with every
  // browser -- `headers.host` is absent and a reader that takes it alone gets
  // nothing at all.
  //
  // Where both arrive, the pseudo-header is the one to believe: it comes from
  // the connection, while `Host` is caller text that a non-browser client can
  // write freely. Preferring `Host` let such a client supply BOTH halves of an
  // origin comparison and have the check agree with it.
...
function captureResponseStatus(res, onEnd) {
  ...
  res.end = function () {
    ...
    // onEnd never breaks the response — a caller's instrumentation issue,
    // whether it throws or rejects.
