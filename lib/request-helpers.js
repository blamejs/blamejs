...
var _cdnCacheControl = lazyRequire(function () { return require("./cdn-cache-control"); });
 ...
 */
var ANONYMOUS_ACTOR_KEY = "anonymous";

var ACTOR_IDENTITY_FIELDS = Object.freeze([
  "id", "userId", "username", "sub", "principalId", "email",
]);

var ACTOR_DISPLAY_FIELDS = Object.freeze([
  "username", "email", "sub", "principalId", "id", "userId",
]);

function _ownField(actor, name) {
  if (!actor || typeof actor !== "object") return undefined;
  return Object.prototype.hasOwnProperty.call(actor, name) ? actor[name] : undefined;
}

function _identityPart(value, label) {
  if (typeof value === "string" && value.length > 0) {
    return label + ":s:" + value.length + ":" + value;
  }
  if (typeof value === "number" && isFinite(value)) return label + ":n:" + String(value);
  return null;
}

/**
 * @primitive b.requestHelpers.actorIdentityKey
 * @signature b.requestHelpers.actorIdentityKey(actor, opts?)
 * @since     0.20.32
 * @status    stable
 * @related   b.requestHelpers.extractActorContext
 *
 * Derive the key that separates one principal from another for a per-actor
 * bucket: an ownership record, a quota, a concurrency slot, a replay scope.
 * Answers `b.requestHelpers.ANONYMOUS_ACTOR_KEY` when there is no principal
 * at all, a key when the actor can be named, and `null` when an actor object
 * exists and carries none of the names this framework reads.
 *
 * A caller that receives `null` refuses the operation. Folding an unnameable
 * actor onto a shared literal is what lets two authenticated users share one
 * bucket, and a bucket is an ownership record as often as it is a counter.
 *
 * What an actor is called is the operator's choice: `verify` and `userLoader`
 * return whatever the deployment's tokens carry, so `opts.actorKey(actor)`
 * names the field when it is none of the ones read here (`id`, `userId`,
 * `username`, `sub`, `principalId`, `email`, in that order). Each key is
 * tagged with the field it came from and its length, so `{ id: "x" }` and
 * `{ userId: "x" }` are two principals rather than one, and `tenantId` scopes
 * the whole key.
 *
 * @opts
 *   actorKey: function,   // (actor) → string | number naming the principal
 *
 * @example
 *   var b = require("blamejs");
 *   b.requestHelpers.actorIdentityKey({ sub: "alice", tenantId: "t1" });
 *   // → "13:tenant:s:2:t1|sub:s:5:alice"
 *
 *   b.requestHelpers.actorIdentityKey({ role: "admin" });
 *   // → null   (an actor carrying no name this framework reads)
 */
function actorIdentityKey(actor, opts) {
  opts = opts || {};
  if (actor === null || actor === undefined) return ANONYMOUS_ACTOR_KEY;
  var identity;
  if (typeof opts.actorKey === "function") {
    identity = _identityPart(opts.actorKey(actor), "custom");
  } else if (typeof actor !== "object") {
    identity = _identityPart(actor, "actor");
  } else {
    identity = null;
    for (var i = 0; i < ACTOR_IDENTITY_FIELDS.length && identity === null; i += 1) {
      identity = _identityPart(_ownField(actor, ACTOR_IDENTITY_FIELDS[i]),
                               ACTOR_IDENTITY_FIELDS[i]);
    }
  }
  if (identity === null) return null;
  var tenant = "";
  if (actor && typeof actor === "object") {
    var declared = _ownField(actor, "tenantId");
    if (declared !== undefined && declared !== null) {
      tenant = _identityPart(declared, "tenant");
      if (tenant === null) return null;
    }
  }
  return tenant.length + ":" + tenant + "|" + identity;
}

/**
 * @primitive b.requestHelpers.legacyActorKey
 * @signature b.requestHelpers.legacyActorKey(actor)
 * @since     0.20.32
 * @status    deprecated
 * @related   b.requestHelpers.actorIdentityKey
 *
 * The identity derivation used before `b.requestHelpers.actorIdentityKey`:
 * `actor.id || actor.userId` as written, string or number, and the literal
 * `_anonymous` for no actor. Answers `null` for anything else, including
 * every actor the old derivation would have folded onto that literal.
 *
 * It exists so a primitive can recognize a record its earlier version wrote,
 * rather than stranding it, and it lives here because identity is derived in
 * one module. It is not an answer for a new record: the value it returns is
 * untagged, so `{ id: "x" }` and `{ userId: "x" }` share it, and a caller
 * compares against it only for a record that carries no format version of
 * its own. Comparing a new record against it would let an actor whose
 * literal id spells another owner's encoded key pass for that owner.
 *
 * @example
 *   var b = require("blamejs");
 *   b.requestHelpers.legacyActorKey({ id: "ada" });      // → "ada"
 *   b.requestHelpers.legacyActorKey({ sub: "ada" });     // → null
 */
function legacyActorKey(actor) {
  if (actor === null || actor === undefined) return "_anonymous";
  if (typeof actor !== "object") return null;
  var value = _ownField(actor, "id") || _ownField(actor, "userId");
...
/**
 * @primitive b.requestHelpers.actorDisplayName
 * @signature b.requestHelpers.actorDisplayName(actor, opts?)
 * @since     0.20.32
 * @status    stable
 * @related   b.requestHelpers.actorIdentityKey
 *
 * Answer the name to show a principal by, as the actor wrote it, or `null`
 * when the actor carries none. Reads the same fields as
 * `b.requestHelpers.actorIdentityKey`, so a field that names a principal to
 * one is a field that names it to the other, but in the order a reader wants
 * to see: the login name and address before an opaque id.
 *
 * This is the display answer, so it is the raw value rather than the tagged
 * key: it is not an identity for an access decision.
 *
 * @opts
 *   actorKey: function,   // (actor) → the name, when the shape is the operator's
 *
 * @example
 *   var b = require("blamejs");
 *   b.requestHelpers.actorDisplayName({ sub: "alice" });   // → "alice"
 *   b.requestHelpers.actorDisplayName({ role: "admin" });  // → null
 */
function actorDisplayName(actor, opts) {
  opts = opts || {};
  if (actor === null || actor === undefined) return null;
  if (typeof opts.actorKey === "function") {
    var supplied = opts.actorKey(actor);
    if (typeof supplied === "string" && supplied.length > 0) return supplied;
    if (typeof supplied === "number" && isFinite(supplied)) return String(supplied);
    return null;
  }
  if (!actor || typeof actor !== "object") return null;
  for (var i = 0; i < ACTOR_DISPLAY_FIELDS.length; i += 1) {
    var value = _ownField(actor, ACTOR_DISPLAY_FIELDS[i]);
    if (typeof value === "string" && value.length > 0) return value;
 ...
 * prefix RIRs allocate, RFC 4291 §2.5.4). Returns a
 * <code>"network/prefix"</code> string, with every IPv6 group written out
 * rather than <code>::</code>-compressed, or <code>""</code> for a non-string /
 ...
 *   b.requestHelpers.ipPrefix("2001:db8::1");     // → "2001:db8:0:0:0:0:0:0/64"
 */
...
function appendVary(res, value) {
  ...
  var merged = appendVaryValue(res.getHeader("Vary"), value);
  if (merged !== null) res.setHeader("Vary", merged);
}

/**
 * @primitive b.requestHelpers.appendVaryValue
 * @signature b.requestHelpers.appendVaryValue(existing, value)
 * @since     0.20.30
 * @status    stable
 * @related   b.requestHelpers.appendVary, b.requestHelpers.mergeVary
 *
 * Return the `Vary` value that carries both `existing` and `value`, or
 * `null` when `existing` already carries it. `existing` is a header value
 * (a string, an array of strings, `null` or `undefined`); the result is a
 * string. A `*` on either side is the whole result, because RFC 9110
 * section 12.5.5 gives the wildcard form its meaning only when it stands
 * alone: a cache reading `*, Accept` cannot apply the never-reuse that `*`
 * asks for. Token comparison ignores case.
 *
 * @example
 *   b.requestHelpers.appendVaryValue("Cookie", "Accept");   // → "Cookie, Accept"
 *   b.requestHelpers.appendVaryValue("cookie", "Cookie");   // → null
 *   b.requestHelpers.appendVaryValue("Cookie, *", "Accept"); // → "*"
 *   b.requestHelpers.appendVaryValue("Cookie", "*");        // → "*"
 */
function appendVaryValue(existing, value) {
  if (typeof value !== "string" || value === "") return null;
  var tokens = parseListHeader(Array.isArray(existing) ? existing.join(", ") : existing);
  var lower = value.toLowerCase();
  for (var i = 0; i < tokens.length; i += 1) {
    if (tokens[i] === "*") return existing === "*" ? null : "*";
  }
  if (lower === "*") return "*";
  for (var j = 0; j < tokens.length; j += 1) {
    if (tokens[j].toLowerCase() === lower) return null;
  }
  tokens.push(value);
  return tokens.join(", ");
}
...
/**
 * @primitive b.requestHelpers.finalizeHeaders
 * @signature b.requestHelpers.finalizeHeaders(res, headers)
 * @since     0.20.30
 * @status    stable
 * @related   b.requestHelpers.mergeVary, b.cdnCacheControl.keepNoStore
 *
 * Return the copy of a header object that `res.writeHead(status, headers)`
 * can send without dropping what is already on `res`. `Vary` is merged as
 * `mergeVary` describes, and a `Cache-Control` in the object (in any case) is
 * passed through `b.cdnCacheControl.keepNoStore`, so a `no-store` already set
 * on the response survives a value the caller supplied. `res` is not
 * modified.
 *
 * @example
 *   res.setHeader("Cache-Control", "no-store");
 *   res.setHeader("Vary", "Cookie");
 *   var headers = b.requestHelpers.finalizeHeaders(res, {
 ...
 */
function finalizeHeaders(res, headers) {
  var out = mergeVary(res, headers);
  var keys = Object.keys(out);
  for (var i = 0; i < keys.length; i += 1) {
    if (keys[i].toLowerCase() !== "cache-control") continue;
    var value = out[keys[i]];
    if (Array.isArray(value)) value = value.join(", ");
    if (typeof value !== "string") continue;
    out[keys[i]] = _cdnCacheControl().keepNoStore(res, value);
  }
  return out;
}

/**
 * @primitive b.requestHelpers.mergeVary
 * @signature b.requestHelpers.mergeVary(res, headers)
 * @since     0.20.30
 * @status    stable
 * @related   b.requestHelpers.appendVary
 *
 * Return a copy of a header object for `res.writeHead(status, headers)` in
 * which `Vary` lists the tokens already set on `res` followed by the tokens
 * the object carries, each once (compared case-insensitively). node:http lets
 * a header object passed to `writeHead` replace a header set with
 * `setHeader`, so without the merge a writer that sends its own `Vary` drops
 * the `Origin` or `Accept-Encoding` an earlier middleware added. A `*` on
 * either side makes the result `*`. An object with no `Vary` key (in any
 * case) is returned as a copy unchanged, and `res` is not modified.
 *
 * @example
 *   res.setHeader("Vary", "Origin");
 *   var headers = b.requestHelpers.mergeVary(res, { "Content-Type": "text/html", Vary: "Accept-Language" });
 *   headers.Vary;   // → "Origin, Accept-Language"
 *   res.writeHead(200, headers);
 */
function mergeVary(res, headers) {
  var out = validateOpts.assignOwnEnumerable({}, headers);
  var own = [];
  var keys = Object.keys(out);
  for (var i = 0; i < keys.length; i += 1) {
    if (keys[i].toLowerCase() === "vary") {
      own.push(out[keys[i]]);
      delete out[keys[i]];
    }
  }
  if (own.length === 0) return out;
  var existing = res && typeof res.getHeader === "function" ? res.getHeader("Vary") : undefined;
  var tokens = [];
  var seen = Object.create(null);
  var sources = [existing].concat(own);
  for (var s = 0; s < sources.length; s += 1) {
    var list = parseListHeader(Array.isArray(sources[s]) ? sources[s].join(", ") : sources[s]);
    for (var t = 0; t < list.length; t += 1) {
      var lower = list[t].toLowerCase();
      if (lower === "*") { out.Vary = "*"; return out; }
      if (seen[lower] === true) continue;
      seen[lower] = true;
      tokens.push(list[t]);
    }
  }
  if (tokens.length > 0) out.Vary = tokens.join(", ");
  return out;
}
...
function makeSkipMatcher(opts, label) {
  ...
  for (var i = 0; i < skipPaths.length; i++) {
    ...
    if (skipPaths[i] instanceof RegExp) {
      ...
      _guardRegex().assertStateless(skipPaths[i], label + ": skipPaths[" + i + "]");
    }
  }
 ...
 *   res.writeHead(204);
 *   res.end();
 *   // the hook writes: final status: 204
 */
...
function parseQualityList(headerValue, opts) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    var segs = structuredFields.splitTopLevel(p, ";");
...
module.exports = {
  ...
  actorIdentityKey:          actorIdentityKey,
  actorDisplayName:          actorDisplayName,
  legacyActorKey:            legacyActorKey,
  ANONYMOUS_ACTOR_KEY:       ANONYMOUS_ACTOR_KEY,
  ACTOR_IDENTITY_FIELDS:     ACTOR_IDENTITY_FIELDS,
  ACTOR_DISPLAY_FIELDS:      ACTOR_DISPLAY_FIELDS,
  ...
  appendVaryValue:           appendVaryValue,
  mergeVary:                 mergeVary,
