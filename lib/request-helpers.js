...
// extractBearer(req) — RFC 6750 §2.1 inbound bearer-token extractor.
//
// Reads the `Authorization` request header, validates the case-insensitive
// `Bearer ` scheme, and returns the trimmed token string. Returns null
// when the header is absent / malformed — this is a defensive request-
// shape reader (it runs on every authenticated request and must not
// throw on garbage input). Callers downstream THROW their own
// authentication-shape errors when null surfaces in a context that
// requires a token.
//
// Refusal cases (return null):
//   - missing Authorization header
//   - non-string Authorization value
//   - multiple Authorization headers (CWE-345 trust mismatch — a
//     malicious upstream that injects a second header would otherwise
//     race against operator middleware that picks a different one)
//   - scheme is not "Bearer" (case-insensitive per RFC 6750 §2.1)
//   - missing space + token after the scheme
//   - token contains CR / LF / NUL or other ASCII control bytes
//     (CRLF injection / response-splitting defense — the token
//     transits framework log lines + audit metadata)
//
// The framework's `b.authHeader.bearer(token)` is the OUTBOUND
// counterpart that constructs the `Authorization: Bearer <token>`
// header for outgoing requests; extractBearer is the symmetric
// inbound reader.
//
// Token shape past the scheme word is NOT validated against RFC 6750
// `b64token = 1*( ALPHA / DIGIT / "-" / "." / "_" / "~" / "+" / "/" ) *"="`
// here — operators sometimes carry opaque session ids or JWT-shaped
// payloads through Bearer, and the registered guard primitives
// (b.guardJwt / b.middleware.bearerAuth) own the format-specific
// validation. extractBearer's job is to safely surface the raw token
// string for downstream handlers.
