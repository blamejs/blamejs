...
function trustedClientIp(opts) {
  ...
  if (trustedProxies.length) {
    ...
    for (var i = 0; i < trustedProxies.length; i++) {
      // A CIDR is valid iff it contains its own network address. Reuse the
      // same matcher the resolver uses so format rules can't diverge.
      ...
      if (slash === -1 || !ssrfGuard.cidrContains(cidr, cidr.slice(0, slash))) {
        throw new TypeError("trustedClientIp: trustedProxies[" + i + "] is not a valid CIDR, got " + JSON.stringify(cidr));
      }
    }
    predicate = function (addr) {
      for (var j = 0; j < trustedProxies.length; j++) {
        if (ssrfGuard.cidrContains(trustedProxies[j], addr)) return true;
      }
      return false;
    };
  }
