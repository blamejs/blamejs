 ...
 * order — the first one PRESENT on the request is used, and it defaults to
...
function clientIp(req, opts) {
  ...
  if (trust && req.headers) {
    ...
    var names = _normForwardedHeaders(opts && opts.forwardedHeaders, "clientIp");
    var xff = null;
    for (var n = 0; n < names.length; n++) {
      var candidate = req.headers[names[n]];
      if (candidate !== undefined && candidate !== null && candidate !== "") { xff = candidate; break; }
    }
    if (xff) {
 ...
 *
 * `forwardedHeaders` names which header carries the address, in preference
 * order — the first one PRESENT on the request is used. It defaults to
