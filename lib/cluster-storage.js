...
// Ceiling for one fenced upsert statement. Matches the framework's other
// single-statement SQL timeouts — a fence is a keyed write, not a scan.
...
// missingRelationCode(e) — does the driver's own structured error say the
// relation is absent, independently of how it worded the message?
//
// Every caller that has to tell "the table is not there yet" from "the query
// failed" needs this, and each was answering it from the message alone. The
// codes are driver-stable and locale-independent, so they are the part that
// can be shared; what a caller does with the WORDING is its own business —
// b.cluster treats any "does not exist" as a missing table, while b.auditChain
// must not, because Postgres words a missing COLUMN the same way apart from
// the noun and a legacy volume's table is present without its newer columns.
//
...
//
// mysql2 exposes errno / code; the docker-exec shim and ANSI drivers surface
// the SQLSTATE on code or sqlState instead, so all three are read.
/**
 ...
 * recognise a driver carrying no code adds its own message test on
...
function missingRelationCode(e) {
  if (!e) return false;
  if (e.errno === 1146) return true;                                    // MySQL ER_NO_SUCH_TABLE
...
// missingColumnCode(e) — the column-level sibling, for a caller distinguishing
// "this table is older than the code reading it" from a real failure. Separate
// from missingRelationCode because the two mean opposite things to such a
// caller: an absent TABLE ends the read, an absent COLUMN sends it to an
// earlier projection.
//
//   SQLite:    "no such column: X"
//   Postgres:  column "X" does not exist       (SQLSTATE 42703)
//   MySQL:     Unknown column 'X' in ...       (errno 1054, SQLSTATE 42S22)
/**
...
function missingColumnCode(e) {
  if (!e) return false;
  if (e.errno === 1054) return true;                                    // MySQL ER_BAD_FIELD_ERROR
  var code     = (e.code != null) ? String(e.code) : "";
  var sqlState = (e.sqlState != null) ? String(e.sqlState) : "";
...
// duplicateIndexCode(e) — the third of the family, for the schema reconcilers.
// MySQL has no CREATE INDEX IF NOT EXISTS, so an idempotent pass re-issues the
// plain form and swallows the duplicate; Postgres and SQLite use the native
// form and never reach it.
//
//   MySQL: Duplicate key name 'X'   (errno 1061, SQLSTATE 42000)
//
// Unlike its two siblings this one does NOT accept the SQLSTATE on its own.
// 42000 is MySQL's catch-all for syntax and access errors, so accepting it
// would swallow a malformed statement as an already-present index. The errno
// and the driver's own code name it exactly; nothing else does.
/**
...
function duplicateIndexCode(e) {
  if (!e) return false;
  if (e.errno === 1061) return true;                                    // MySQL ER_DUP_KEYNAME
...
async function transaction(fn) {
  ...
  if (cluster.isClusterMode()) {
    ...
    return await externalDb.transaction(async function (txClient) {
      // Same coercion `execute` applies, for the same reason and at the same
      // point: node-postgres hands back BIGINT as a decimal string and BYTEA
      // as a Buffer, so without it a counter read inside a transaction is a
      // string while the identical read outside one is a number. That kind of
      // split is invisible on SQLite, where both are already numbers, and
      // shows up only against a real Postgres — which is precisely how it
      // would reach an operator rather than a test.
...
async function fencedUpsert(opts) {
  ...
  // Every named column must be non-decreasing for the write to apply. Usually
  // that is the fencing token alone, but a caller can name more: a value that
  // only ever moves one way — a purge boundary, a high-water mark — is safest
  // enforced by the database rather than by whoever happens to write next.
  ...
  // The fence compares the EXISTING row against the PROPOSED one. The existing
  // side is table-qualified; the proposed side is spelled per dialect, because
  // MySQL has no EXCLUDED. guardColumn tells the MySQL renderer which column
  // the fence protects, so the IF() wrap on every other assignment evaluates
  // against that column's pre-update value and it is assigned last.
  // Interpolated into the statement, so they answer the identifier rules at
  // the entry point rather than deeper in. Quoted with allowReserved for the
  // same reason b.db.from() does: a name in identifier position is safe once
  // quoted, and one rule for every path means no path ends up stricter than
  // the query builder without anyone noticing.
  ...
  if (d === "mysql") {
    // The write and the read that interprets it run in ONE transaction. They
    // have to: the readback is how MySQL's refusal is detected at all, and a
    // writer advancing a fenced column between the two would make this report
    // a refusal for a write that was accepted — telling a purge its boundary
    // was rejected after it had been recorded. Inside the transaction the
    // readback sees this statement's own effect and not a later commit.
    return await safeAsync.withTimeout(transaction(async function (tx) {
      ...
      // Any stored value above the one proposed means the guard held and
      // nothing was written — the whole statement is refused together, so one
      // column being above is enough to know that.
