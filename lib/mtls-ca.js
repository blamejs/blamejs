...
function parseGeneration(certPem) {
  ...
  try {
    ...
    // Anchor the OU=CAv{N} match to an RDN BOUNDARY (subject start, a newline / comma RDN
    // separator, or the " + " attribute separator node emits inside a MULTI-VALUED RDN, e.g.
    // "CN=x + OU=CAv7") so a CN or other attribute VALUE that literally contains the substring
    // "OU=CAv<k>" is not misread as the generation, and the FIRST real OU RDN is taken. The
    // unescaped-plus boundary uses a `(?<!\\)` lookbehind so an ESCAPED "\+" — a literal plus
    // inside a value ("CN=foo\+OU=CAv9") — is NOT treated as a separator. An embedded "OU=CAv"
    // inside a value never sits at a boundary (it follows "<Type>=").
