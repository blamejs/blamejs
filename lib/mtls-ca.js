...
function create(opts) {
  ...
  function _reconcileCommitJournalLocked() {
    ...
    var priorCertPem = (typeof manifest.cert === "string") ? Buffer.from(manifest.cert, "base64").toString("utf8") : null;
    // The rotation REPUBLISHED the cert (completed / roll-forward) iff the live cert
    // differs from the prior cert the journal recorded. Then the journal is spent —
    // the current key/cert/retained state is authoritative — so drop it WITHOUT
    // re-applying the old retained root (which would defeat a completed hard cutoff).
    if (priorCertPem !== null && curCertPem !== priorCertPem) {
      try { nodeFs.unlinkSync(keyJournal); } catch (_ue) { /* best-effort */ }
      return;
    }
    // The cert was never republished (crashed before the cert rename, or a failed
    // rotation whose in-memory rollback was partial): roll BACK to the prior state.
    // Restore the key only if the live pair no longer matches, and reconcile the
    // retained root even when the pair IS consistent — a partial rollback can leave
    // a consistent key/cert pair beside the WRONG retained root.
    ...
    if (manifest.prevAction === "restore" && typeof manifest.prevData === "string") {
      var want = Buffer.from(manifest.prevData, "base64");
      var have = nodeFs.existsSync(paths.caCertPrev)
        ? atomicFile.fdSafeReadSync(paths.caCertPrev, { maxBytes: C.BYTES.mib(1) }) : null;
      if (have === null || !Buffer.from(have).equals(want)) {
        atomicFile.writeSync(paths.caCertPrev, want, { fileMode: 0o644 });
        atomicFile.fsyncDir(nodePath.dirname(paths.caCertPrev));
      }
  ...
  function _journalRetainedRoot() {
    ...
    try {
      ...
      if (m && m.prevAction === "restore" && typeof m.prevData === "string" && typeof m.cert === "string") {
        ...
        var priorCert = Buffer.from(m.cert, "base64").toString("utf8");
        if (_readCurrentCert() === priorCert) {
