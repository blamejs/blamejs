...
function create(opts) {
  ...
  async function _rotateImpl(rotateOpts) {
    ...
    // Cross-handle / cross-process guard. _rotateChain serializes rotations on
    // THIS handle, but a separate handle over the same dataDir (or another
    // process) owns a different chain; it could have committed a new generation
    // while we awaited generateCa above. Re-read the on-disk generation NOW and
    // refuse if it moved, rather than overwriting that CA and snapshotting our
    // transient intermediate as ca.prev.crt (which would drop the root clients
    // still trust). The caller retries against the current generation.
    var nowSt = status();
