...
function parseGeneration(certPem) {
  ...
  try {
    ...
    // Anchor the OU=CAv{N} match to an RDN BOUNDARY (subject start, or after a newline /
    // comma RDN separator) so a CN or other attribute VALUE that literally contains the
    // substring "OU=CAv<k>" is not misread as the generation, and the FIRST real OU RDN
    // is taken. node emits one RDN per line (older builds comma-separate); an embedded
    // "OU=CAv" inside a value never sits at a boundary (it follows "<Type>=").
...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    return atomicFile.lock(paths.caCert, function () {
      ...
      } else if (!usesDefaultEngine && caAlgorithm !== undefined && opts2.algorithm !== undefined) {
        // A CUSTOM engine's label can't be inferred from the committed cert, so a pinned
        // handle migrating to a different-algorithm CA supplies the NEW effective label
        // explicitly (as rotate({ algorithm }) does). Without it the stale pin would be
        // passed to the newly committed issuer on the next issuance (reject / incompatible
        // leaf) even though commit() succeeded.
  ...
  function _reconcileCommitJournalLocked() {
    ...
    if (![manifest.key, manifest.newKey, manifest.cert, manifest.newCert, manifest.prevData]
  ...
  async function _adoptExistingCASnapshot() {
    ...
    // the rotation lock: reconcile, then re-read the pair under it.
    ...
    // Refuse a persistent mismatch, validate the pin, and capture the pin WITH the snapshot.
  ...
  async function generateCrl(opts3) {
    ...
    // serial once.
    // Scope the CRL to the CURRENT issuer. A CRL is signed by ONE CA, and X.509 serials are
    // unique only per issuer, so a revocation whose cert was issued by a DIFFERENT (superseded)
    // generation must NOT be published here — under a custom engine that restarts its serial
    // counter on rotation, an old generation's revoked serial would false-revoke the unrelated
    // current-generation certificate reusing it. Resolve each entry's generation from the
    // issuance ledger — by FINGERPRINT (unique; revokeGeneration/revoke({serial,fingerprint})
    // carry it) preferentially, else by serial when it maps to a single generation. An entry
    // whose generation cannot be determined (an out-of-band serial, or an undeterminable current
    // generation) stays in, best-effort.
    var currentGen = parseGeneration(ca.caCertPem);
    var _genByFingerprint = new Map();
    var _gensBySerial = new Map();
    issuanceStore.list().forEach(function (e) {
      if (!e || typeof e.generation !== "number") return;
      if (e.fingerprint != null) _genByFingerprint.set(e.fingerprint, e.generation);
      if (e.serialNumber != null) {
        if (!_gensBySerial.has(e.serialNumber)) _gensBySerial.set(e.serialNumber, new Set());
        _gensBySerial.get(e.serialNumber).add(e.generation);
      }
    });
    var _entryGeneration = function (r) {
      if (r.fingerprint != null && _genByFingerprint.has(r.fingerprint)) return _genByFingerprint.get(r.fingerprint);
      var gens = _gensBySerial.get(r.serialNumber);
      if (gens && gens.size === 1) return gens.values().next().value;
      return null;
    };
    var seenSerials = new Set();
    var revocations = allRevocations.filter(function (r) {
      if (!(r && r.serialNumber != null)) return false;
      if (typeof currentGen === "number" && currentGen > 0) {
        var eg = _entryGeneration(r);
        if (eg != null && eg !== currentGen) return false;   // belongs to another issuer's CRL
      }
    ...
    if (opts3.persist !== false) {
      ...
      await atomicFile.lock(paths.caCert, function () {
        ...
        // A revoke()/revokeGeneration() may have COMPLETED while we signed — the just-signed
        // CRL then omits it. For the default store, compare the store version under the
        // revocations lock so the check and the publish are atomic with revoke()'s write
        // (paths.revocations is only ever a leaf lock, so this caCert->revocations nesting
        // has no inverse and cannot deadlock). If it advanced, skip — the CRL is stale;
        // persisted=false signals the caller to regenerate (same contract as a rotation).
        if (usesDefaultRevocationStore) {
          return atomicFile.lock(paths.revocations, function () {
            if (revocationStore.version() === revSnapshotVersion) { _writeCrl(); }
          });
        }
        _writeCrl();
      });
