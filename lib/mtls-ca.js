...
function create(opts) {
  ...
  function _commitLocked(opts2) {
    ...
    try {
      ...
      // Invalidate a persisted CRL as a REQUIRED part of the commit, BEFORE the cert
      // rename (the point of no return): this commit republishes the CA cert, so a CRL
      // persisted under the OLD cert becomes signed by a superseded issuer, and the
      // release contract is that a rotation invalidates it. Removing it is therefore
      // part of establishing the requested state, not best-effort cleanup — a failure
      // (e.g. paths.crl in a separately-configured read-only directory) throws to the
      // outer catch, which rolls the whole commit back rather than publishing a new CA
      // while an unauthenticatable CRL stays served at the documented path. Doing it
      // here (before the cert rename) also means a COMPLETED rotation has always already
      // removed the stale CRL, so reconcile's roll-forward needs no CRL handling. A
      // consumer regenerates it under the new CA via generateCrl(). fsyncDir is best-
      // effort (it swallows platform errors); only the unlink can fail the commit.
