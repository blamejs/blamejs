...
function create(opts) {
  ...
  // custom engine's own labels / key curves.
  var usesDefaultEngine = opts.engine === undefined;
  ...
  function _leafEngineArgs(ca, opts2) {
    ...
    // leafAlg is the DEFAULT (the key is omitted entirely when none resolved, so
    // a custom engine never sees an inferred label); an explicit opts2.algorithm
    // overrides it via assign order; caCertPem/caKeyPem are forced last so opts2
    // can't shadow them.
    return Object.assign(leafAlg !== undefined ? { algorithm: leafAlg } : {},
      opts2, { caCertPem: ca.caCertPem, caKeyPem: ca.caKeyPem });
  }
