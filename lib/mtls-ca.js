...
function create(opts) {
  ...
  function _revIndexAdd(entry) {
    if (_revIndex === null) return;   // not built yet — the lazy build will read this entry from the store
    if (entry.serialNumber) _revIndex.add(entry.serialNumber);
    if (entry.fingerprint) _revIndex.add(entry.fingerprint);
  }
  ...
  function isRevoked(serialOrFingerprint) {
    ...
    return _revIndexEnsure().has(norm);
  }
