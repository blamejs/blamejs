...
function create(opts) {
  ...
  function revokeGeneration(n, opts3) {
    ...
    // Bump the watermark under its OWN cross-process lock (atomic for ALL stores —
    // the watermark is a separate file, so a custom revocation store that can only
    // list()/add() and cannot make its append+sweep atomic is still covered)
    // BEFORE sweeping, so an in-flight issuance that records after the sweep-read
    // still self-revokes (see _recordIssuance).
    return atomicFile.lock(paths.revokedGeneration, function () { _bumpRevokedWatermark(n); }).then(function () {
