...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    // Grace-window retention: snapshot the OUTGOING cert as the retained CA
    // BEFORE the new one overwrites it, so loadTrustBundle() keeps trusting it
    // while live clients re-enroll. No-op on a first commit (nothing to
    // retain) or when retainPrevious is unset.
    if (opts2.retainPrevious && nodeFs.existsSync(paths.caCert)) {
      var outgoing = atomicFile.fdSafeReadSync(paths.caCert, { maxBytes: C.BYTES.mib(1) });
      atomicFile.writeSync(paths.caCertPrev, outgoing, { mode: 0o644 });
    }
  ...
  // Record an issued leaf in the ledger. Best-effort: a ledger write failure
  // must never fail issuance (the cert is already minted) — it only means
  // revokeGeneration can't later find this serial, which we surface at debug.
  function _recordIssuance(caCertPem, id) {
    try {
      ...
      caLog.debug("issuance-ledger-write-failed", { error: (e && e.message) || String(e) });
    }
  }
  ...
  async function rotate(rotateOpts) {
    ...
    var newGen = (rotateOpts.generation !== undefined && rotateOpts.generation !== null)
      ? Math.floor(rotateOpts.generation) : curGen + 1;
