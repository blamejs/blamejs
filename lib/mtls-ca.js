...
function create(opts) {
  ...
  async function initCA() {
    if (exists()) {
      ...
      if (caAlgorithm !== undefined) {
        ...
        /* c8 ignore next -- the "" fallback is defensive: a parsed KeyObject always reports a non-empty asymmetricKeyType, so it is never reached */
        try { actualType = String(nodeCrypto.createPrivateKey(existingKeyPem).asymmetricKeyType || "").toLowerCase(); }
    ...
    var fresh = await engine.generateCa({ generation: generation, algorithm: caAlgorithm });
  ...
  async function generateClientCert(opts2) {
    opts2 = opts2 || {};
    var ca = await initCA();
    // The leaf follows the CA's algorithm: the pin when set (initCA already
    // verified it matches the stored CA), otherwise the stored CA's own
    // algorithm — never the engine's process default, which would sign an
    // ML-DSA leaf under an existing classical CA. An explicit opts2.algorithm
    // still wins (Object.assign order) for a caller that wants a specific leaf.
    var leafAlg = caAlgorithm !== undefined ? caAlgorithm : _labelForCaKeyType(ca.caKeyPem);
    var args = Object.assign({ algorithm: leafAlg }, opts2,
  ...
  async function generateClientP12(opts2) {
    ...
    var ca = await initCA();
    // Leaf (and its P12 MAC tier) follows the CA's algorithm — the pin when set,
    // else the stored CA's own algorithm, never the process default.
    var leafAlg = caAlgorithm !== undefined ? caAlgorithm : _labelForCaKeyType(ca.caKeyPem);
    var args = Object.assign({ algorithm: leafAlg }, opts2,
      { caCertPem: ca.caCertPem, caKeyPem: ca.caKeyPem });
