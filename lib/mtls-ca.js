...
function create(opts) {
  ...
  async function _freshCreateSerialized() {
    ...
    return atomicFile.lock(paths.caCert, function () {
      ...
      // Persist the create-time custom label as shared metadata BEFORE publishing the CA, so a
      // sibling handle over this dataDir adopts it (a custom label is not cert-derivable). It goes
      // FIRST because a fresh create writes no rollback journal (there is no prior key to restore),
      // so there is nothing to reconcile the label from after the fact: if the algorithm path is
      // unwritable, throw here — before _commitLocked publishes the cert LAST — leaving no CA
      // installed rather than a labelless CA a sibling would issue under its own stale pin. Since the
      // cert is published last, any handle that sees the CA also sees its label. Default-engine
      // labels are derived from the cert, not stored.
