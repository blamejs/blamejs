...
function create(opts) {
  ...
  function _normalizeSerial(s) {
    ...
    // Hex-only, lowercase. The peculiar/x509 generator emits hex
    // serials; operators looking up by openssl-printed serial paste
    // either hex or decimal. Accept both shapes by normalising to hex.
    var trimmed = s.replace(/^0x/i, "").replace(/[^0-9a-fA-F]/g, "");
    if (trimmed.length === 0) {
      throw new MtlsCaError("mtls-ca/bad-serial",
        "serial number contains no hex digits");
    }
