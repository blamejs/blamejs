...
function create(opts) {
  ...
  function _reconcileCommitJournalLocked() {
    ...
    // Make the journal deletion durable BEFORE returning: the journal's parent may
    // differ from ca.prev.crt's, so without this a power loss could resurrect the
    // journal while keeping the retained-root change durable — and a resurrected
    // interrupted journal would re-trust its saved root, undoing a completed cutoff.
    try {
      nodeFs.unlinkSync(keyJournal);
  ...
  async function _recordIssuance(caCertPem, id) {
    ...
    if (_trustRoots().indexOf(caCertPem) === -1) {
      await revoke({ serial: id.serialNumber || null, fingerprint: id.fingerprint || null, reason: "superseded" });
      throw new MtlsCaError("mtls-ca/issuance-superseded",
  ...
  // Lock-free trust-root snapshot used INTERNALLY (e.g. _recordIssuance's
  // root-drop check). The double-read makes the snapshot internally consistent;
  // the PUBLIC loadTrustBundle() wraps this in the rotation lock so a completed
  // dropRetained()/rotation cannot precede delivery of a stale bundle.
