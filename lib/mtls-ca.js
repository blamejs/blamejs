 ...
 * rotating to it); and revokeGeneration(n) revokes every cert the issuance ledger
...
function create(opts) {
  ...
  // Revoked-generation watermark (default stores only) — see DEFAULT_PATHS
  // .revokedGeneration. revokeGeneration() bumps it under the revocation lock;
  // issuance reads it after recording to catch a leaf whose signing straddled a
  // generation revocation.
  function _readRevokedWatermark() {
    try {
      var n = parseInt(atomicFile.fdSafeReadSync(paths.revokedGeneration, { maxBytes: 64, encoding: "utf8" }), 10);
  ...
  async function _recordIssuance(caCertPem, id) {
    ...
    // is caught by one side or the other. If its generation is already revoked,
    // revoke this leaf and refuse to hand back a credential the operator killed.
    if (usesDefaultRevocationStore && gen < _readRevokedWatermark()) {
  ...
  function revokeGeneration(n, opts3) {
    ...
    var sweep = function () {
      // Bump the watermark BEFORE sweeping so an in-flight issuance that records
      // after our sweep-read still catches itself (see _recordIssuance); one that
      // recorded before is caught by the sweep here. Uses _revokeCore directly —
      // we already hold the revocation lock, so calling revoke() would re-enter it.
      if (usesDefaultRevocationStore) _bumpRevokedWatermark(n);
    ...
    if (usesDefaultRevocationStore) {
      return atomicFile.lock(paths.revocations, sweep);
    }
