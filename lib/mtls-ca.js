...
function create(opts) {
    ...
    try {
      ...
      try {
        if (priorKeyExisted && priorKey !== null) {
          atomicFile.writeSync(keyDest, priorKey, { mode: 0o600 });
        }
      } catch (keyRbErr) {
  ...
  async function initCA() {
    if (exists()) {
      ...
      // Still inconsistent after re-reading — refuse rather than sign with a
  ...
  function importIssuance(entries) {
    ...
    // Read the watermark up front (fail-closed on a malformed value): an imported
    // leaf whose generation is already revoked must be revoked here, or it would
    // stay accepted — the sweep that set the watermark already read the ledger and
    // won't see this late append.
