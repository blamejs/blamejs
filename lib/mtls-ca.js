 ...
 *
 * The framework's CA-management primitive: storage, sealed-loading
 * dispatch, generation tagging, atomic commit. The actual cert
 * issuance (CA generation, client cert signing, PKCS#12 packaging)
 * is operator-pluggable via opts.engine — the primitive doesn't ship
 * a bundled X.509 engine because Node's built-in crypto can read
 * certificates but not issue them, and the bundled engine waits on
 * vendoring @peculiar/x509 + pkijs (its own framework slice).
 *
 ...
 *     engine:           myCertEngine,    // optional; required for issuance
 *   });
 *
 ...
 *     pair via engine.generateCa() and atomically commits it. Throws
 *     mtls-ca/no-engine when the engine isn't wired.
 *
 ...
 *
 * Engine contract (operator-supplied):
 *
 *   {
 ...
 *       returns { p12, fingerprint256, certPem, issuedAt, expiresAt },
 *   }
 */
...
function create(opts) {
  ...
  function _requireEngine(method) {
    if (!engine) {
      throw new MtlsCaError("mtls-ca/no-engine",
        "mtlsCa." + method + " requires opts.engine — the bundled cert " +
        "issuance engine ships in a future slice once @peculiar/x509 + " +
        "pkijs are vendored. Operators with mTLS needs today wire their " +
        "own engine implementation.");
    }
  }
  ...
  async function generateClientCert(opts2) {
    opts2 = opts2 || {};
    _requireEngine("generateClientCert");
    var ca = await initCA();
  ...
  async function generateClientP12(opts2) {
    opts2 = opts2 || {};
    _requireEngine("generateClientP12");
    ...
    if (!result || !Buffer.isBuffer(result.p12)) {
      throw new MtlsCaError("mtls-ca/bad-engine-output",
        "engine.packageP12 must return { p12: Buffer, fingerprint256, certPem, ... }");
    }
    return result;
  }
