...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    if (usesDefaultEngine) {
      var _commitCertParses = true, _commitKeyParses = true;
      try { new nodeCrypto.X509Certificate(opts2.caCertPem); } catch (_ce) { _commitCertParses = false; }
      try { nodeCrypto.createPrivateKey(opts2.caKeyPem); } catch (_ke) { _commitKeyParses = false; }
      if (!_commitCertParses || !_commitKeyParses) {
        throw new MtlsCaError("mtls-ca/bad-commit",
  ...
  async function generateCrl(opts3) {
    ...
    if (opts3.persist !== false) {
      ...
      await atomicFile.lock(paths.caCert, function () {
        ...
        var _writeIfIssuanceUnchanged = function () {
          if (usesDefaultIssuanceStore) {
            return atomicFile.lock(paths.issuance, function () {
              if (issuanceStore.version() === issuanceSnapshotVersion) { _writeCrl(); }
            });
          }
          _writeCrl();
        };
        if (usesDefaultRevocationStore) {
          return atomicFile.lock(paths.revocations, function () {
            if (revocationStore.version() === revSnapshotVersion) { return _writeIfIssuanceUnchanged(); }
          });
        }
