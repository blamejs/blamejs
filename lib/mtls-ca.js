...
function create(opts) {
    ...
    try {
      ...
      if (priorKeyExisted && priorKey !== null) {
        ...
        atomicFile.writeSync(keyJournal, JSON.stringify({
          ...
          crlMovedAside: crlExisted,   // did THIS commit move a CRL aside (so reconcile may restore it)?
      ...
      try {
        if (crlExisted && nodeFs.existsSync(crlRollback) && !nodeFs.existsSync(paths.crl)) {
  ...
  function _reconcileCommitJournalLocked() {
    ...
    var crlRollback = _crlRollbackPath();
    if (manifest.crlMovedAside && nodeFs.existsSync(crlRollback)) {
      if (completed) {
        nodeFs.unlinkSync(crlRollback);
        atomicFile.fsyncDir(nodePath.dirname(crlRollback));
