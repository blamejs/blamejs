...
function create(opts) {
  ...
  function _commitLocked(opts2) {
    ...
    try {
      ...
      // The CUSTOM-engine effective label to publish alongside the CA (the default engine derives
      // its label from the cert, so it is never persisted). opts2.algorithm carries it: an explicit
      // commit/rotate({ algorithm }), or the preserved persisted label a bare custom rotate resolves.
      var _customCommitLabel = (!usesDefaultEngine && typeof opts2.algorithm === "string" && opts2.algorithm.length > 0)
        ? opts2.algorithm : null;
      if (priorKeyExisted && priorKey !== null) {
      ...
      // can never be left stale (old) against the new CA. Default-engine labels are cert-derived.
      var _labelPersistDeferred = false;
      if (_customCommitLabel !== null) {
        try {
  ...
  function _readPersistedAlgorithm() {
    ...
    /* c8 ignore start -- defensive: existsSync just confirmed the file, so fdSafeReadSync only throws on a concurrent removal race (file unlinked between the check and the read), not reached in tests */
    } catch (_e) {
      return undefined;
    }
    /* c8 ignore stop */
  ...
  async function _freshCreateSerialized() {
    ...
    return atomicFile.lock(paths.caCert, function () {
      ...
      _commitLocked(fresh);
      // Persist the create-time custom label as shared metadata so a sibling handle over this dataDir
      // adopts it (a custom label is not cert-derivable). Default-engine labels are derived, not stored.
      if (!usesDefaultEngine && caAlgorithm !== undefined) _persistAlgorithm(caAlgorithm);
  ...
  async function generateCrl(opts3) {
    ...
    if (opts3.persist !== false) {
      ...
      await atomicFile.lock(paths.caCert, function () {
        ...
        // signed, making the CRL stale. Re-check BOTH store versions under their leaf locks
        // before publishing, so each check and the paths.crl write are atomic with that store's
        // write. Global lock order is caCert < revocations < issuance — each is only ever a leaf
        // lock elsewhere (revokeGeneration reads the ledger unlocked; _recordIssuance releases
        // the issuance lock before taking caCert), so this nesting has no inverse and cannot
        // deadlock. If either advanced, skip — persisted=false signals the caller to regenerate.
        var _writeIfIssuanceUnchanged = function () {
          if (usesDefaultIssuanceStore) {
            return atomicFile.lock(paths.issuance, function () {
              // Fast path: the ledger version is unchanged. Else re-check the CRL's ACTUAL scoped
              // serial set against the fresh ledger — a normal issuance (unrelated fresh serial)
              // must not spuriously skip; only an issuer-backfill of a revoked serial does.
              if (issuanceStore.version() === issuanceSnapshotVersion ||
                  _scopedCrlSerials(allRevocations, issuanceStore.list()) === signedCrlSerials) { _writeCrl(); }
            });
          }
          _writeCrl();
        };
        if (usesDefaultRevocationStore) {
          return atomicFile.lock(paths.revocations, function () {
            // Fast path: the registry version is unchanged. Else re-check the CRL's scoped serial set
            // against the fresh registry — a fingerprint-only revoke() (never in a serial CRL) must
            // not spuriously skip; only a new SERIAL revocation does.
            if (revocationStore.version() === revSnapshotVersion ||
                _scopedCrlSerials(revocationStore.list(), _issuanceEntries) === signedCrlSerials) { return _writeIfIssuanceUnchanged(); }
          });
        }
        return _writeIfIssuanceUnchanged();
      });
    }
  ...
  async function _rotateImpl(rotateOpts) {
    ...
    await atomicFile.lock(paths.caCert, function () {
      ...
      if (nowGen !== curGen || nowCertChanged) {
  ...
  async function canVerifyInTls(algorithm) {
    ...
    if (!usesDefaultEngine) {
      var _persistedLabel = _readPersistedAlgorithm();
