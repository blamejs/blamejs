...
function create(opts) {
  ...
  function _commitLocked(opts2) {
    ...
    try {
      ...
      // Invalidate a persisted CRL as part of the commit (UNDER the lock, so every
      // CA-change entry point — rotate() AND the public commit() — is covered): this
      // commit republished the CA cert, so a CRL persisted under the OLD cert is now
      // signed by a superseded issuer. Remove it (best-effort — the CA is already
      // published; a stale CRL is a published-artifact problem, not CA corruption)
      // so a consumer regenerates it under the new CA via generateCrl().
