...
function create(opts) {
  ...
  function _commitLocked(opts2) {
    ...
    var caCertChanged = priorCert === null || !priorCert.equals(Buffer.from(opts2.caCertPem));
  ...
  async function _adoptExistingCASnapshot() {
    ...
    // A stored CA is returned as-is (never a silent rotation), but a pin that DISAGREES with
    // it cannot be honored — refuse the mismatch. Capture the pin WITH the snapshot so
    // _leafEngineArgs binds the leaf to the pin as of this read (a later rotate({ algorithm })
    // refreshing the mutable caAlgorithm cannot retarget an in-flight issuance's leaf).
    _assertPinMatchesStoredCa(existingCertPem, existingKeyPem);
    return { caCertPem: existingCertPem, caKeyPem: existingKeyPem, algorithm: caAlgorithm };
  }
  ...
  async function _freshCreateSerialized() {
    ...
    return atomicFile.lock(paths.caCert, function () {
      if (exists()) {
        ...
        return { caCertPem: adoptedCert, caKeyPem: adoptedKey, algorithm: caAlgorithm };
      }
      _commitLocked(fresh);
