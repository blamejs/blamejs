...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    try {
      // Exclusive-create the .tmp staging files. If an attacker (or a
      // crashed prior run) pre-created either path, EEXIST surfaces and
      // we refuse the commit rather than overwriting unexpected bytes.
      var keyBytes = sealed
        ? (function () { _requireVault("sealed CA key commit"); return vault.seal(opts2.caKeyPem); }())
        : opts2.caKeyPem;
      var keyFd = nodeFs.openSync(keyTmp, "wx", 0o600);
      try { nodeFs.writeSync(keyFd, keyBytes); nodeFs.fsyncSync(keyFd); }
      finally { try { nodeFs.closeSync(keyFd); } catch (_c) { /* best-effort */ } }
      var certFd = nodeFs.openSync(certTmp, "wx", 0o644);
      try { nodeFs.writeSync(certFd, opts2.caCertPem); nodeFs.fsyncSync(certFd); }
      finally { try { nodeFs.closeSync(certFd); } catch (_c) { /* best-effort */ } }
