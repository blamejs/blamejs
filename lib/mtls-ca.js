...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    // Crash-recovery journal. The key and cert are two files, so the two renames
    // below cannot be a single atomic swap: if the process dies after the key
    // rename but before the cert rename (or during the retained-root fsyncs
    // between them), the in-memory catch rollback never runs and the prior key —
    // already overwritten on disk — is otherwise unrecoverable, stranding the CA
    // (new key beside old cert, mtls-ca/ca-pair-inconsistent). Persist the prior
    // key durably (fsync'd tmp+rename) BEFORE overwriting it; its presence is the
    // "rotation in progress" marker _recoverCommitJournal() reconciles from, and
    // a clean commit removes it once the new pair is durably consistent.
    ...
    try {
      ...
      // Persist the prior key durably before the rename overwrites it, so a crash
      // before the cert publishes leaves a recoverable copy (see keyJournal note).
      if (priorKeyExisted && priorKey !== null) {
        atomicFile.writeSync(keyJournal, priorKey, { fileMode: 0o600 });
      ...
      // trusted. Best-effort (own try): a prev write/clear failure degrades the
      // grace window but must not fail the rotation (loadTrustBundle/dropRetained
      // remain the recovery).
      try {
  ...
  function _reconcileCommitJournalLocked() {
    ...
    if (_caPairConsistent(curCertPem, curKeyPem)) {
      ...
      // nothing to restore; drop the spent journal.
      try { nodeFs.unlinkSync(keyJournal); } catch (_ue) { /* best-effort */ }
      return;
    }
    // The live key does not match the live cert: restore the prior key so the
    // old, consistent CA is recovered, then drop the journal.
    var priorKeyBytes;
    try {
      priorKeyBytes = atomicFile.fdSafeReadSync(keyJournal, { maxBytes: C.BYTES.mib(1) });
    } catch (_je) {
      // Unreadable journal — cannot recover; leave it and let the pair-
      // consistency check surface the inconsistency as a clear error.
      return;
    }
    atomicFile.writeSync(keyDest, priorKeyBytes, { fileMode: 0o600 });
    try { nodeFs.unlinkSync(keyJournal); } catch (_ue) { /* best-effort */ }
    caLog.warn("recovered-interrupted-rotation",
      { path: keyDest, detail: "restored prior CA key from the rollback journal after an interrupted rotation" });
  }
