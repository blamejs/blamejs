...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    return atomicFile.lock(paths.caCert, function () { return _commitLocked(opts2); });
  }
  ...
  async function _rotateImpl(rotateOpts) {
    ...
    // A persisted CRL is signed by the SUPERSEDED CA after a rotation — a consumer
    // serving paths.crl would keep publishing a CRL the NEW CA cannot authenticate,
    // so revocations for newly-issued certs are ineffective until it is regenerated.
    // Invalidate the stale CRL (remove it) so the operator regenerates it under the
    // current CA rather than serving a wrong-issuer one; generateCrl() re-signs.
    if (nodeFs.existsSync(paths.crl)) {
      try {
        nodeFs.unlinkSync(paths.crl);
        atomicFile.fsyncDir(nodePath.dirname(paths.crl));
        caLog.info("invalidated stale CRL on rotation (regenerate with generateCrl)", { path: paths.crl });
      } catch (crlErr) {
        caLog.error("stale-crl-removal-failed",
          { path: paths.crl, error: (crlErr && crlErr.message) || String(crlErr) });
      }
    }
  ...
  async function canVerifyInTls(algorithm) {
    ...
    var label = (typeof algorithm === "string" && algorithm.length > 0)
      ? algorithm : (st.algorithm || caAlgorithm);
