...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    // The CA commit landed — now settle the retained root as part of the same
    // logical transaction: snapshot the outgoing cert for the grace window, or
    // (explicit retainPrevious:false) clear a root a prior retained rotation left
    // behind so loadTrustBundle() stops trusting it. Both run only post-commit.
