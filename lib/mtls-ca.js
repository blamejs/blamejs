...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    try {
      ...
      // Persist the prior key AND the prior retained root durably before either is
      // mutated below, so a crash before the cert publishes leaves both recoverable
      // (see keyJournal note). prevAction: "restore" (prior prev bytes), "delete"
      // (no prior prev — any prev is this rotation's creation), "leave" (prior prev
      // existed but was unreadable — don't clobber it).
      ...
      // trusted. Establishing retention is a REQUIRED part of the commit: when a
      // rotation asks to retain (outgoingCaCert !== null) and the write fails (a
      // read-only ca.prev.crt dir, disk full), throwing here propagates to the
      // outer catch, which rolls the whole rotation back — the alternative
      // (publish the new cert but silently omit the outgoing root) breaks the
      // advertised no-outage migration by rejecting clients still enrolled under
      // the just-superseded CA. Clearing a stale root (retainPrevious:false) is
      // NOT an outage risk, so that stays best-effort (dropRetained is the
      // explicit cleanup).
      if (outgoingCaCert !== null) {
        ...
        try { nodeFs.unlinkSync(paths.caCertPrev); }
        catch (clearErr) {
          caLog.error("retained-root-clear-failed",
            { path: paths.caCertPrev, error: (clearErr && clearErr.message) || String(clearErr) });
        }
      }
      atomicFile.renameWithRetry(certTmp, paths.caCert);   // publish the new current LAST
      // renameSync alone is not crash-durable: without fsyncing the rename dirs a
      // power loss could persist the journal deletion below while LOSING the cert
      // rename, reintroducing the unrecoverable new-key/old-cert pair. Make both
      // renames durable BEFORE removing the recovery journal (the commit point),
      // then make the deletion durable too. keyDest and caCert may have distinct
      // operator-configured parents. fsyncDir is best-effort (Windows rejects
      // directory fsync), matching atomicFile's own durability contract.
      ...
      try {
        if (priorPrevExisted && priorPrev !== null) {
          atomicFile.writeSync(paths.caCertPrev, priorPrev, { mode: 0o644 });
      ...
      // The in-memory rollback restored the live key, so the journal is spent —
      // remove it. If the restore FAILED, keep the journal so the next
      // _reconcileCommitJournalLocked() completes the rollback from the durable
      // copy rather than stranding the CA on a new-key/old-cert pair.
      if (keyJournalWritten && keyRolledBack) {
  ...
  function _reconcileCommitJournalLocked() {
    ...
    var curCertPem, curKeyPem;
    try {
      curCertPem = loadCert().toString("utf8");
      curKeyPem  = loadKey().toString("utf8");   // unseals when caKeySealedMode is "required"
    } catch (_re) {
      // The live pair is unreadable right now (a genuinely broken store) — leave
      // the journal so a later reconcile can retry.
      return;
    }
    if (_caPairConsistent(curCertPem, curKeyPem)) {
      // Rotation completed (new pair) or never overwrote the key (old pair) —
      // the new retained root is correct; nothing to restore. Drop the spent
      // journal.
      try { nodeFs.unlinkSync(keyJournal); } catch (_ue) { /* best-effort */ }
      return;
    }
    // The live key does not match the live cert: an interrupted rotation. Restore
    // the prior key AND the prior retained root from the manifest so the old,
    // consistent CA — and the trust bundle clients still depend on — are recovered,
    ...
    try {
      ...
      // Unreadable / malformed journal — cannot recover; leave it and let the
      // pair-consistency check surface the inconsistency as a clear error.
      return;
    }
    if (!manifest || typeof manifest.key !== "string") return;   // not a rollback manifest — leave it
    atomicFile.writeSync(keyDest, Buffer.from(manifest.key, "base64"), { fileMode: 0o600 });
    // Reconcile the retained root exactly as the rotation would have rolled it back.
    if (manifest.prevAction === "restore" && typeof manifest.prevData === "string") {
      atomicFile.writeSync(paths.caCertPrev, Buffer.from(manifest.prevData, "base64"), { fileMode: 0o644 });
    } else if (manifest.prevAction === "delete") {
      try { nodeFs.unlinkSync(paths.caCertPrev); } catch (_pe) { /* already absent */ }
    } // "leave" (or unknown): don't touch the retained root
    try { nodeFs.unlinkSync(keyJournal); } catch (_ue) { /* best-effort */ }
    caLog.warn("recovered-interrupted-rotation",
      { path: keyDest, detail: "restored prior CA key and retained root from the rollback journal after an interrupted rotation" });
  }
  ...
  async function initCA() {
    if (exists()) {
      ...
      if (!_caPairConsistent(existingCertPem, existingKeyPem) || nodeFs.existsSync(_commitJournalPath())) {
        await atomicFile.lock(paths.caCert, function () { _reconcileCommitJournalLocked(); });
        existingCertPem = loadCert().toString("utf8");
        existingKeyPem  = loadKey().toString("utf8");
      }
  ...
  function loadTrustBundle() {
    ...
    // just means the grace window ended — return the still-valid current CA
    // rather than throwing on a benign race.
