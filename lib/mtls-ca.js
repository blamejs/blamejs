...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    // Abort if an existing key could not be captured. Without the prior key we
    // cannot write a rollback journal, so a later cert-publish failure (or a crash)
    // would strand the CA on a new-key/old-cert pair with no way to restore the old
    // key. Refuse to overwrite it rather than proceed unprotected — a transient read
    // fault must not silently disable the rollback path.
  ...
  function _journalRetainedRoot() {
    ...
    try {
      var m = safeJson.parse(atomicFile.fdSafeReadSync(keyJournal, { maxBytes: C.BYTES.mib(2), encoding: "utf8" }),
        { maxBytes: C.BYTES.mib(2) });
      if (m && m.prevAction === "restore" && typeof m.prevData === "string") {
