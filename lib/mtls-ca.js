...
function create(opts) {
  ...
  function commit(opts2) {
    ...
    try {
      ...
      // Roll back the retained-root change: the rotation FAILED (the new cert was
      // not published), so the prior retained root must survive. Restore its old
      // content, or remove a prev we created for this (failed) rotation.
      try {
