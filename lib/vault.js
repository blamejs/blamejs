...
function fsyncDir(dir) {
  ...
  } catch (_e) { /* fsync on directory not supported on all platforms; best effort */ }
}
...
function resolvePaths(dataDir) {
  return {
    ...
    sealedTmp:         path.join(dataDir, "vault.key.sealed.tmp"),
  };
}

// Pre-dispatch cleanup: a previous boot may have crashed mid-write of the
// .tmp file. Removing it is always safe (it's per-definition incomplete).
function cleanOrphanTmp(p) {
  if (fs.existsSync(p.sealedTmp)) {
    try {
      fs.unlinkSync(p.sealedTmp);
...
function initPlaintext() {
  if (fs.existsSync(paths.plaintext)) {
    var loaded;
    try {
      loaded = JSON.parse(fs.readFileSync(paths.plaintext, "utf8"));
    } catch (e) {
      logErr("FATAL: vault.key corrupted or unreadable at " + paths.plaintext + " — " + e.message);
      logErr("All sealed data requires the original key. Restore from backup, then restart.");
      process.exit(1);
    }
    if (!loaded || !loaded.publicKey || !loaded.privateKey || !loaded.ecPublicKey || !loaded.ecPrivateKey) {
      logErr("FATAL: vault.key is missing required fields (ML-KEM-1024 + P-384 hybrid expected).");
      process.exit(1);
    }
    keys = loaded;
    return;
  ...
  fs.writeFileSync(paths.plaintext, JSON.stringify(keys, null, 2), { mode: 0o600 });
...
async function initWrapped() {
  ...
  var sealedBytes;
  try {
    sealedBytes = fs.readFileSync(paths.sealed);
  } catch (e) {
  ...
  if (!keys || !keys.publicKey || !keys.privateKey || !keys.ecPublicKey || !keys.ecPrivateKey) {
...
async function initFirstRunWrapped() {
  ...
  // Atomic write via .tmp + rename. fsync is best-effort.
...
module.exports = {
  ...
  _resetForTest:         function () { keys = null; initialized = false; currentPassphrase = null; paths = null; currentMode = null; },
