...
function _smtpSend(message, cfg) {
  return new Promise(function (resolve, reject) {
    ...
    // `replyCode` is the peer's own numeric reply, where there was one. It
    // always travels on the error as `statusCode`, so a delivery layer can act
    // on the verdict instead of parsing prose.
    //
    // `envelopeScoped` says whether a 5yz makes it PERMANENT. RFC 5321 §4.2.1
    // makes the leading digit the verdict for the command that was refused —
    // and which command that was decides how far the verdict reaches:
    //
    //   - MAIL FROM / RCPT TO / DATA / BDAT are about this MESSAGE and this
    //     recipient. A 5yz there is a bounce; retrying it anywhere is the
    //     hours-long deferral of a "User unknown" that this release fixes.
    //   - A greeting, EHLO or STARTTLS refusal is about this SESSION with this
    //     HOST. The recipient has not been named yet, let alone judged. A
    //     backup MX may take the message happily, so marking it permanent
    //     would end the failover loop and bounce deliverable mail because one
    //     host was unwilling.
    //   - AUTH is the exception among the session-scoped steps, because a
    //     transport that authenticates is talking to ONE configured relay
    //     rather than a set of MX records. `535 5.7.8` on those credentials
    //     says the configuration is wrong, and retrying the same relay with
    //     the same credentials defers an unrecoverable error forever. A 4yz
    //     there — a rate-limited or temporarily-unavailable authenticator —
    //     is still transient, which the reply-code rule already gives.
    //
    ...
    function handleResponse(code) {
      if (step === SMTP_STEP_GREETING) {
        ...
        if (cfg.requireTls === true && !peerSupportsRequireTls) {
          ...
          // Host-scoped, so NOT permanent: whether REQUIRETLS is offered is a
          // property of THIS peer, and a backup MX may advertise it. Failing
          // over honours the flag rather than violating it — where refusing
          // permanently would bounce mail a host willing to carry it would
          // have taken. If no host offers it the message ends deferred, which
          // is the honest answer to "nobody here can promise what you asked".
        ...
        if (cfg.servername) tlsConnectOpts.servername = cfg.servername;
        // The peer's name, as the implicit-TLS path sets it. node checks the
        // certificate against `servername || host`, and `servername` is left
        // undefined on purpose for an address literal (an IP is not a valid
        // SNI name), so without this the check falls back to node's default
        // and an address-literal peer is verified against `localhost` -- one
        // presenting a correct IP SAN cannot be verified at all.
        tlsConnectOpts.host = cfg.host;
        // allow:outbound-tls-posture — _smtpTlsOpts merges the live posture
        ...
        // A RCPT reply speaks for ONE mailbox. With a single recipient it is
        // the verdict on this message, and a 5yz is a bounce. With several, it
        // is not: this transport aborts the whole transaction on the first
        // refusal, so calling that permanent would discard the recipients that
        // were never offered — a valid address bounced because a sibling in
        // the same envelope was unknown. Those stay retryable, and the caller
        // that wants per-recipient verdicts sends per recipient, which is what
        // b.mail.send.deliver does.
