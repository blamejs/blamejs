...
function smtpTransport(opts) {
  ...
  // The shared outbound posture first, so SMTP gets the same hybrid group
  // preference and certificate compression as every other outbound path; the
  // operator's own values then override it. minTlsVersion stays operator-
  // settable here, unlike the framework's other clients: an MX that only
  // speaks TLS 1.2 is common, and refusing it would mean not delivering mail.
  var tlsOpts = Object.assign(networkTls().outboundPosture(), {
