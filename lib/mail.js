...
function create(opts) {
  ...
  function _validateAddrDomain(addr, label) {
    if (!guardDomainProfile) return;
    if (typeof addr !== "string") return;
    var bracketMatch = addr.match(/<([^>]+)>/);
    var rawAddr = bracketMatch ? bracketMatch[1] : addr;
    ...
    var verdict = guardDomain.validate(domain, guardDomainProfile);
    if (!verdict.ok) {
