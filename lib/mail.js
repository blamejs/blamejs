 ...
 * Public API:
 *
 *   mail.create({ transport?, defaults?, audit?, commercial?, postalAddress?, footerSeparator?, footerHtml?, regulated? }) → instance
 *
 ...
 *                 (default true).
 *     commercial — when true, every send() refuses unless `postalAddress`
 *                 was supplied at create() AND the message exposes a
 *                 functional opt-out (List-Unsubscribe header or
 *                 unsubscribe.{url|mailto} on the message). 15 USC §7704
 *                 (CAN-SPAM Act §5) requires every commercial-content
 *                 message to (a) carry a valid physical postal address
 *                 of the sender and (b) provide a clear unsubscribe
 *                 mechanism. The address is auto-appended to text + html
 *                 bodies; the unsubscribe header is the existing RFC 8058
 *                 one-click integration.
 *     postalAddress — { street, city, region, postalCode, country }
 *                 (object) | non-empty string. Required when
 *                 commercial:true. Refused when missing / blank / shape-
 *                 incomplete. The shape validates BEFORE the first
 *                 send() — typo-surfacing throw at config time, not
 *                 silent shipping with a missing footer.
 *     footerSeparator — divider rendered between the body and the postal-
 *                 address footer. Defaults to "\n\n----\n" for text and
 *                 "<hr>" for html.
 *     footerHtml — operator-supplied HTML override for the html-part
 *                 footer (e.g. when the deploy needs a logo + branded
 *                 text alongside the address). Must include the postal
 *                 address; the framework refuses an override that does
 *                 not contain the country and postal-code values.
 *     regulated — alias for commercial:true; future hooks for additional
 *                 commercial-mail regimes (CASL §6 Canada, EU PEC) read
 *                 this flag.
 *
