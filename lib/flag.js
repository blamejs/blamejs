 ...
 * from its client address and User-Agent. That key decides which bucket the
 * caller lands in, so the address is resolved peer-gated: forwarded headers
 * are ignored unless `middleware` is given `trustedProxies` (CIDRs of your
 * reverse proxies), optionally with `forwardedHeaders` when the address
 * arrives as something other than `X-Forwarded-For`, or `clientIpResolver`
 * to own resolution outright. Behind a proxy without one of those, every
 * anonymous caller shares the proxy's address and therefore one bucket.
 *
