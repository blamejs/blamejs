...
function scanToTagEnd(s, from, len) {
  ...
  while (p < len) {
    ...
    if (inQuote) {
      ...
      if (afterEquals) inQuote = ch;
    } else if (ch === ">") {
      break;
    }
    if (!inQuote) {
      if (ch === "=") afterEquals = true;
      else if (!isMarkupSpace(s.charCodeAt(p))) afterEquals = false;
    }
    p += 1;
  }
  return p;
...
var ATTR_RECOVERY_ROUNDS = 64;

function parseAttrsRecovering(src) {
  var s = String(src).trim();
  var out = [];
  var at = 0;
  for (var round = 0; round < ATTR_RECOVERY_ROUNDS && at < s.length; round += 1) {
