...
function decodeCharRefs(s) {
  ...
  while (i < s.length) {
    ...
    if (body.charAt(0) === "#") {
      ...
      // The largest code point is six hex digits or seven decimal ones. Leading
      // zeros are legal but a reference padded past this denotes nothing new,
      // so the length is bounded before the digits are tested rather than
      // letting an attribute carry an arbitrarily long run into the check. A
      // refused reference is left exactly as written, so bounding it cannot
      // manufacture a match.
      var wellFormed = digits.length > 0 && digits.length <= MAX_CHAR_REF_DIGITS &&
                       (hex ? HEX_DIGITS_RE.test(digits) : DEC_DIGITS_RE.test(digits));
      if (wellFormed) {
        var cp = parseInt(digits, hex ? 16 : 10);
        // A lone surrogate is not a character; neither is anything past the
        // last plane. Either would throw out of fromCodePoint or produce a
        // value no document can contain.
