...
function scanToTagEnd(s, from, len) {
  ...
  var inUnquoted = false;
  while (p < len) {
    var ch = s.charAt(p);
    var space = isMarkupSpace(s.charCodeAt(p));
    if (inQuote) {
      if (ch === inQuote) inQuote = "";
    } else if (inUnquoted) {
      if (ch === ">") break;
      if (space) inUnquoted = false;
    } else if (ch === ">") {
      break;
    } else if (ch === '"' || ch === "'") {
      if (afterEquals) { inQuote = ch; afterEquals = false; }
    } else if (ch === "=") {
      afterEquals = true;
    } else if (!space) {
      if (afterEquals) { inUnquoted = true; afterEquals = false; }
    }
    p += 1;
  }
  return p;
