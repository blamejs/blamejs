...
function parse(xml) {
  ...
  // Walks the text once instead of running `/&([^;]+);/g` over it. That
  // pattern restarts at every `&`, so a value of nothing but ampersands cost
  // time proportional to its SQUARE (about 23ms for a single 8192-character
  // call) on text that arrives inside a signed document. An `&` with no `;`
  // after it is left alone, and so is an empty `&;`, which is what the
  // one-or-more body required.
  function _decodeEntities(s) {
    ...
    for (;;) {
      ...
      if (semi === -1) return out + s.slice(i);
      if (semi === amp + 1) { out += s.slice(i, amp + 1); i = amp + 1; continue; }
      out += s.slice(i, amp) + _decodeEntity(s.slice(amp + 1, semi));
      i = semi + 1;
    }
  }

  function _decodeEntity(name) {
    switch (name) {
      case "amp":  return "&";
      case "lt":   return "<";
      case "gt":   return ">";
