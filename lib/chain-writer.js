...
function create(opts) {
  ...
  // reports it.
  var resolveOrigin = opts.resolveOrigin || null;
  if (resolveOrigin !== null && typeof resolveOrigin !== "function") {
    throw new ChainWriterError(
      "resolveOrigin must be a function",
      "chain-writer/invalid-config"
    );
  }
    ...
    if (!numericBounds.isNonNegativeSafeInt(counter)) {
      throw new ChainWriterError(
        "resolveOrigin returned counter '" + String(origin.counter) +
          "', which is not a whole non-negative number below 2^53",
        "chain-writer/bad-origin"
      );
    }
  ...
  async function _cachedOrigin(k, keyValue) {
    ...
    boundedMap.getOrInsert(_originByKey, k, function () { return origin; });
