...
var backupBundle = require("./backup-bundle");
var lazyRequire = require("./lazy-require");
var audit = lazyRequire(function () { return require("./audit"); });
var { FrameworkError } = require("./framework-error");

class BackupError extends FrameworkError {
  constructor(code, message, permanent) {
    super(message, code);
    this.name = "BackupError";
    this.permanent = !!permanent;
    this.isBackupError = true;
  }
}

// "2026-04-27T14-00-00-123Z-a8f30b21" — ISO with ':'+'.' replaced by '-'
// to keep the id usable as a filesystem directory name on every platform.
...
function _generateBundleId() {
  // Date#toISOString → "YYYY-MM-DDTHH:MM:SS.sssZ" → swap : and . for -
  var iso = new Date().toISOString().replace(/[:.]/g, "-");
  return iso + "-" + nodeCrypto.randomBytes(4).toString("hex");
}
...
function _copyDirRecursive(src, dest) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    else if (entries[i].isFile()) fs.copyFileSync(s, d);
  }
}
...
function localStorage(opts) {
  ...
  return {
    ...
    async writeBundle(bundleId, sourceDir) {
      ...
      _copyDirRecursive(sourceDir, dest);
    },
