 ...
 * each with a protocol + protocol-specific options. v0.0.12 ships the
 * 'local' protocol (SQLite-backed, baked into the framework's main DB).
 * External protocols (redis, sqs, amqp, nats) land in subsequent releases.
 *
 * Public API:
...
var DEFERRED_PROTOCOLS = {
  "redis":  { since: "v0.0.13+", description: "Redis Streams (XADD/XREADGROUP/XACK/XCLAIM)" },
  "sqs":    { since: "v0.0.13+", description: "AWS SQS (and S3-compatible queue endpoints) via SigV4" },
  "amqp":   { since: "v0.0.13+", description: "AMQP 0-9-1 (RabbitMQ etc.)" },
  "nats":   { since: "v0.0.13+", description: "NATS JetStream" },
};
  ...
  for (var name in opts.backends) {
    ...
    if (DEFERRED_PROTOCOLS[cfg.protocol]) {
      ...
      throw _err(
        ...
        "queue protocol '" + cfg.protocol + "' is deferred to " + d.since +
        " (" + d.description + "). Use protocol: 'local' for now.",
        true
      );
    }
...
function consume(queueName, handler, opts) {
  ...
  var state = {
    ...
    cancelled:    false,
    inFlight:     new Set(),
    cancel:       function () { state.cancelled = true; },
  };
...
function _backoffDelay(attempt) {
  ...
  var ms = 1000 * Math.pow(2, attempt - 1);
