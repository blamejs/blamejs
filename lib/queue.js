...
function init(opts) {
  ...
  if (!opts || !opts.backends) {
    throw _err("queue/invalid-config", "queue.init({ backends }) is required", true);
  }
  ...
  if (sweepIntervalMs !== 0 && !numericChecks.isPositiveInt(sweepIntervalMs)) {
    throw _err("queue/invalid-config",
...
function _backendFor(opts) {
  ...
  var b = backends[name];
  if (!b) throw _err("queue/unknown-backend", "no backend named '" + name + "'", true);
  return b;
}
...
function enqueue(queueName, payload, opts) {
  _requireInit();
  if (!queueName) throw _err("queue/missing-queue", "enqueue requires queueName", true);
  opts = opts || {};
...
function consume(queueName, handler, opts) {
  _requireInit();
  if (!queueName) throw _err("queue/missing-queue", "consume requires queueName", true);
  if (typeof handler !== "function") throw _err("queue/invalid-handler", "handler must be a function", true);
  opts = opts || {};
  var b = _backendFor(opts);
  ...
  if (opts.rateLimit) {
    ...
    if (!numericChecks.isPositiveInt(rlMax) || !numericChecks.isPositiveFinite(rlPer)) {
      throw _err("queue/bad-rate-limit",
  ...
  return {
    extendLease: function (additionalMs) {
      if (typeof backend.extendLease !== "function") {
        throw _err("queue/extend-lease-unsupported",
...
async function tick(opts) {
  ...
  if (!queueName) throw _err("queue/missing-queue", "tick requires opts.queue", true);
  if (typeof opts.handler !== "function") {
    throw _err("queue/invalid-handler", "tick requires opts.handler to be a function", true);
  }
  var max = opts.max === undefined ? 10 : opts.max;
  if (!numericChecks.isPositiveInt(max)) {
    throw _err("queue/bad-max", "tick({ max }) must be a positive integer, got " + JSON.stringify(opts.max), true);
  }
  var leaseMs = opts.leaseMs === undefined ? C.TIME.seconds(30) : opts.leaseMs;
  if (!numericChecks.isPositiveInt(leaseMs)) {
    throw _err("queue/bad-lease", "tick({ leaseMs }) must be a positive integer, got " + JSON.stringify(opts.leaseMs), true);
  }
  var concurrency = opts.concurrency === undefined ? 1 : opts.concurrency;
  if (!numericChecks.isPositiveInt(concurrency)) {
    throw _err("queue/bad-concurrency", "tick({ concurrency }) must be a positive integer, got " + JSON.stringify(opts.concurrency), true);
  }

  var backend = _backendFor(opts);
  if (backend.protocol === "sqs") {
    throw _err("queue/tick-unsupported",
 ...
 * trace correlation id. Rejects with `queue/dlq-unsupported` when the
...
function dlqList(queueName, opts) {
  ...
  if (typeof b.dlqList !== "function") {
    return Promise.reject(_err("queue/dlq-unsupported",
 ...
 * surfaces before it becomes a backlog. Rejects with `queue/dlq-unsupported`
...
function _requireInit() {
  if (!initialized) throw _err("queue/not-initialized", "queue.init() must be called first", true);
}
 ...
 * runs at registration time — bad graphs reject with `queue/flow-cycle` /
 * `queue/flow-unknown-dep` before any row lands.
 *
...
function enqueueFlow(spec) {
  ...
  if (!spec || typeof spec !== "object") {
    return Promise.reject(_err("queue/bad-flow", "enqueueFlow requires an opts object", true));
  }
  if (typeof spec.queueName !== "string" || !spec.queueName) {
    return Promise.reject(_err("queue/bad-flow", "enqueueFlow requires queueName", true));
  }
  if (!Array.isArray(spec.children) || spec.children.length === 0) {
    return Promise.reject(_err("queue/bad-flow", "enqueueFlow requires children: [...]", true));
  }
  var byName = {};
  for (var i = 0; i < spec.children.length; i++) {
    var c = spec.children[i];
    if (!c || typeof c !== "object") {
      return Promise.reject(_err("queue/bad-flow", "children[" + i + "] must be an object", true));
    }
    if (typeof c.name !== "string" || !c.name) {
      return Promise.reject(_err("queue/bad-flow", "children[" + i + "].name must be a non-empty string", true));
    }
    if (byName[c.name]) {
      return Promise.reject(_err("queue/bad-flow", "duplicate child name '" + c.name + "'", true));
    }
    byName[c.name] = c;
    if (c.dependsOn !== undefined) {
      if (!Array.isArray(c.dependsOn)) {
        return Promise.reject(_err("queue/bad-flow",
  ...
  function _visit(name, stack) {
    if (stack.indexOf(name) !== -1) {
      throw _err("queue/flow-cycle", "flow cycle detected: " +
    ...
    for (var k = 0; k < child.dependsOn.length; k++) {
      ...
      if (!byName[dep]) {
        throw _err("queue/flow-unknown-dep",
  ...
  if (typeof flowBackend.patchFlowDeps !== "function") {
    return Promise.reject(_err("queue/flow-unsupported",
 ...
 *
 * Throws `queue/invalid-config` when `BLAMEJS_QUEUE_PROTOCOL` is unknown
...
function bootFromEnv(opts) {
  ...
  if (protocol === "local") {
    ...
    backendCfg = {
      ...
      tls:       tlsRaw !== undefined ? tls : undefined,
