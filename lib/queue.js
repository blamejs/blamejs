...
function enqueue(queueName, payload, opts) {
  ...
  return require("./tracing").tap("queue.enqueue",
    { "queue.name": queueName, "queue.backend": b.name },
    function () {
      return b.enqueue(queueName, payload, opts).then(function (result) {
        ...
        try { require("./metrics").tap("queue.enqueue", 1, { queueName: queueName }); }
  ...
  (async function loop() {
    ...
    while (!state.cancelled) {
      ...
      for (var i = 0; i < jobs.length; i++) {
        try { require("./metrics").tap("queue.lease", 1, { queueName: queueName }); }
        catch (_e) { /* boot-order tolerance */ }
        (function (job) {
          ...
          require("./tracing").tap("queue.consume",
            { "queue.name": queueName, "queue.backend": b.name, "queue.jobId": job.jobId, "queue.attempt": job.attempts },
            function () {
              return Promise.resolve()
                ...
                .then(function () {
                  return b.complete(job.jobId).then(function () {
                    ...
                    try { require("./metrics").tap("queue.complete", 1, { queueName: queueName }); }
                    catch (_e) { /* boot-order tolerance */ }
                  });
                }, function (err) {
              ...
              return b.fail(job.jobId, msg, { retryDelayMs: _backoffDelay(job.attempts) })
                .then(function () {
                  try { require("./metrics").tap("queue.fail", 1, { queueName: queueName, willRetry: willRetry }); }
                  catch (_e) { /* boot-order tolerance */ }
