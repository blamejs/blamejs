...
function enqueue(queueName, payload, opts) {
  ...
  return b.enqueue(queueName, payload, opts).then(function (result) {
    ...
    // metrics tap — no-op when no registry is active.
          ...
          var ctx = {
            extendLease: function (additionalMs) {
              if (typeof b.extendLease !== "function") {
                throw new Error("queue backend '" + b.name + "' does not support extendLease");
              }
          ...
          Promise.resolve()
            ...
            .then(function () {
              ...
              return b.fail(job.jobId, msg, { retryDelayMs: _backoffDelay(job.attempts) })
                .then(function () {
                  try { require("./metrics").tap("queue.fail", 1, { queueName: queueName }); }
                  catch (_e) { /* boot-order tolerance */ }
