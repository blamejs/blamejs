 ...
 *
 * Returns `{ leased, succeeded, failed, stale, movedToDlq, mayHaveMore,
 * queueDepth }`.
 *
 ...
 *   // → { leased: 50, succeeded: 48, failed: 2, stale: 0, movedToDlq: 1,
 *   //     mayHaveMore: true, queueDepth: 62 }
 */
  ...
  async function worker() {
    while (next < jobs.length) {
      ...
      try { settled = await _runJob(backend, queueName, job, opts.handler); }
        ...
        // _runJob settles the job before it resolves; a throw here is a
        // lifecycle failure (the backend rejecting complete/fail). Count it as
        // failed rather than losing the tally — the audit chain has the detail.
        settled = { status: "failed", movedToDlq: false };
      }
