 ...
 * locally so a leader transition picks up cleanly. This is best-effort
 * single-fire — apps needing strict at-most-once semantics should also
 * make their job handler idempotent (jobs.enqueue dedup keys, etc.).
 *
...
var logger = lazyRequire(function () { return require("./logger"); });

var DEFAULT_MAX_JOB_MS = 10 * 60 * 1000;
...
function create(opts) {
  ...
  function _err(code, message, permanent) {
    return new SchedulerError(code, message, permanent);
  }
  ...
  function _fireOnce(task) {
    ...
    if (task.running) {
      ...
      if (task.runningSince && (Date.now() - task.runningSince) > maxJobMs) {
        try {
          logger().warn("[scheduler] '" + task.name + "' exceeded " +
            (maxJobMs / 1000) + "s — forcing reset");
        } catch (_e) { /* logger best-effort */ }
        _emit("scheduler.task.watchdog", { name: task.name }, "warning");
        task.running = false;
      } else {
        task.misses++;
        _emit("scheduler.task.skipped",
          { name: task.name, reason: "previous-run-in-flight" }, "warning");
        return;
      }
    }
    ...
    Promise.resolve(promise).then(function (_v) {
      ...
      try {
        logger().error("[scheduler] '" + task.name + "' failed", { error: task.lastError });
      } catch (_e) { /* logger best-effort */ }
      _emit("scheduler.task.failure", {
        name:       task.name,
        kind:       task.kind,
        durationMs: Date.now() - startedAt,
        viaJob:     !!task.job,
