...
function create(opts) {
  ...
  function schedule(spec) {
    if (started) {
      throw _err("scheduler/already-started",
        "scheduler.schedule: cannot register '" + (spec && spec.name) +
        "' after start() — schedule all tasks before calling start()", true);
    }
    if (!spec || typeof spec !== "object") {
      throw _err("scheduler/invalid-spec", "scheduler.schedule requires a spec object", true);
    }
    if (typeof spec.name !== "string" || spec.name.length === 0) {
      throw _err("scheduler/invalid-name", "scheduler.schedule: spec.name is required", true);
    }
    boundedMap.requireAbsent(tasks, spec.name, function () {
      throw _err("scheduler/duplicate-name",
    ...
    if ((hasCron && hasEvery) || (!hasCron && !hasEvery)) {
      throw _err("scheduler/invalid-spec",
  ...
  function register(name, intervalMs, fn) {
    if (typeof name !== "string" || name.length === 0) {
      throw _err("scheduler/invalid-name", "scheduler.register: name must be a non-empty string", true);
    }
    ...
    if (typeof fn !== "function") {
      throw _err("scheduler/invalid-spec", "scheduler.register: fn must be a function", true);
    }
  ...
  var facade = {
    ...
    _fireOnce:     function (name) { // test hook
      ...
      if (!task) throw _err("scheduler/unknown-name", "no task '" + name + "'", true);
      _fireOnce(task);
    },
