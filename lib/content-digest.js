 ...
 *   or tampering in transit. The field is an RFC 9651 dictionary of
...
// Decode an RFC 9651 Byte Sequence payload as STRICT, canonical base64.
 ...
 * value over a message body (RFC 9530 §2): an RFC 9651 dictionary of
...
function create(body, opts) {
  ...
  var members = algos.map(function (a) {
    ...
    return name + "=:" + digest + ":";                       // RFC 9651 Byte Sequence value
  });
  return members.join(", ");
}
...
function verify(fieldValue, body, opts) {
  ...
  for (var i = 0; i < members.length; i++) {
    ...
    if (raw.length < 2 || raw.charAt(0) !== ":" || raw.charAt(raw.length - 1) !== ":") {
      throw new ContentDigestError("content-digest/bad-field", "contentDigest.verify: '" + name + "' value is not an RFC 9651 byte sequence (:base64:)");
    }
