 ...
 * land in subsequent releases as `b.externalDb.adapters.pg`,
 * `.libsqlHttp`, etc. — operators can plug those into a backend's
 * `connect`/`query`/`close` directly without writing the wire client.
 *
 * Public API:
 ...
 *     pool: { min: 1, max: 10, idleTimeoutMs: 60000 }
...
class Pool {
  constructor(name, config) {
    this.name = name;
    this.config = Object.assign({ min: 1, max: 10, idleTimeoutMs: 60000 }, config.pool || {});
    ...
    this._reaper = setInterval(this._reapIdle.bind(this), 10000);
    this._reaper.unref();
  }
...
function init(opts) {
  ...
  for (var name in opts.backends) {
    ...
    backends[name] = {
      ...
      breaker:         new retry.CircuitBreaker("externalDb:" + name, cfg.breaker),
...
async function query(sql, params, opts) {
  ...
  var t0 = Date.now();
  try {
    var result = await retry.withRetry(function () {
