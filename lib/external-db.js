...
function init(opts) {
  ...
  for (var name in opts.backends) {
    ...
    var applicationName = cfg.applicationName !== undefined ? cfg.applicationName : "blamejs";
    ...
    if (dialect === "postgres") {
      var quotedAppName = "'" + applicationName.replace(/'/g, "''") + "'";
      connectFn = async function () {
        var client = await rawConnect();
        try {
          await rawQuery(client, "SET application_name TO " + quotedAppName, []);
        } catch (e) {
          // Surface the failed init to the pool's catch — the partial
          // client gets destroyed, the operator sees a fail-fast at
          // first acquire instead of a silent missing-app-name.
          throw e;
        }
        return client;
      };
