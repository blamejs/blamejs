 ...
 *   rowResidencyTag?:   string,   // the row's residency region tag; required for DML to a residency-tagged backend under a cross-border regulated posture (pass "global"/"unrestricted" for region-neutral rows)
 *
 * @example
...
async function query(sql, params, opts) {
  ...
  // Per-row residency write gate — refuses cross-border DML before
...
// a top-level (outside string-literal) semicolon — the multi-statement
// shape that would let a trailing DML hide behind a harmless leading
// keyword. Quote-aware: a `;` inside a '...' or "..." literal is data.
function _hasTrailingStatement(sql) {
  ...
  for (var i = 0; i < sql.length; i += 1) {
    ...
    if (ch === ";") {
      // A trailing run that is only whitespace / SQL comments is not a
      // second statement (a single statement may end with `;`).
      var rest = sql.slice(i + 1)
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .replace(/--[^\n]*/g, "")
        .trim();
...
function _assertRowResidency(sql, opts, backend) {
  ...
  if (regulated && backendTag !== "unrestricted") {
    // _classifyStatement reads only the leading keyword, so a
    // multi-statement string ("SELECT 1; INSERT INTO eu_rows ...")
    // would classify as its harmless prefix and slip a trailing DML
    // past the gate ON THIS ENFORCED PATH. Refuse trailing statements
    // here so a residency-bound DML cannot hide behind a SELECT prefix.
    ...
    if (cls !== "DML") return null;
    if (!tag) {
      return {
        code: "RESIDENCY_GATE_REQUIRED",
        message: "DML write to backend '" + backend.name + "' (residencyTag='" +
  ...
  if (tag && _classifyStatement(sql) === "DML") {
    // Unregulated posture or untagged backend with a tag supplied on a
    // DML statement — pass, but surface the advisory so operators
    // staging a posture flip can see what WOULD be evaluated.
    _emit("db.residency.gate.advisory", "info", {
      backend: backend.name, backendTag: backendTag, rowTag: tag,
      posture: posture || null, statementClass: "DML", scope: "external",
    });
  }
  return null;
}
