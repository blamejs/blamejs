...
// db.statement is the sanitized statement text. Bound parameter VALUES
// are never folded into the SQL string here (the framework passes them
// out-of-band to the driver), so the captured text carries no literal
// argument values — but it is still truncated to the framework's
// log-truncation length to bound metadata size. No PII risk beyond the
// SQL literals an operator embeds inline; those are the operator's
// choice and bounded by the same 256-char cap db.js uses.
function _otelDbAttributes(b, sql) {
  var attrs = {
    "db.system":    _OTEL_DB_SYSTEM[b.dialect] || "other_sql",
    "db.name":      b.name,
    "db.statement": String(sql == null ? "" : sql).slice(0, 256),   // log-truncation length, not bytes
  };
...
async function query(sql, params, opts) {
  ...
  try {
    ...
    }, _otelDbAttributes(b, sql)), (e && e.message) || String(e));
    ...
    try {
      for (;;) {
        ...
        try {
          ...
          }, _otelDbAttributes(b, "BEGIN")), (txErr && txErr.message) || String(txErr));
