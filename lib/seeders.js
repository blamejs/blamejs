...
function _validateEnv(name, value) {
  if (typeof value !== "string" || value.length === 0) {
    throw _err("seeders/bad-env", name + " must be a non-empty string, got " +
      (typeof value) + " " + JSON.stringify(value));
  }
  if (!_isEnvName(value)) {
    throw _err("seeders/bad-env", name + " must match " + ENV_RE +
...
function _validateCreateOpts(opts) {
  ...
  validateOpts.requireNonEmptyString(opts.dir, "seeders.create: dir", SeederError, "seeders/bad-opt");
  validateOpts.optionalObjectWithMethod(opts.db, "prepare",
    "seeders.create: db", SeederError, "seeders/bad-opt",
...
function _resolveDb(opts) {
  ...
  if (typeof d.prepare !== "function") {
    throw _err("seeders/no-db", "seeders: no db handle: pass opts.db or initialize b.db before create()");
  }
  return d;
}
...
function _loadSeed(rootDir, env, file) {
  var mod = moduleLoader.requireFresh(nodePath.join(_envDir(rootDir, env), file), function (e) {
    return _err("seeders/load-failed",
      "seed '" + env + "/" + file + "' failed to load: " + ((e && e.message) || String(e)));
  });
  if (!mod || typeof mod.run !== "function") {
    throw _err("seeders/bad-seed",
  ...
  validateOpts.optionalNonEmptyStringArray(mod.dependsOn,
    "seed '" + env + "/" + file + "': dependsOn", SeederError, "seeders/bad-seed");
  ...
  function _dfs(node, stack) {
    ...
    for (var i = 0; i < deps.length; i++) {
      ...
      if (!Object.prototype.hasOwnProperty.call(inEnv, d)) {
        throw _err("seeders/missing-dep",
  ...
  async function run(callerOpts) {
    ...
    if (only !== undefined && only !== null) {
      if (typeof only !== "string" || only.length === 0) {
        throw _err("seeders/bad-opt", "seeders.run: only must be a non-empty string filename");
      }
    }
    ...
    try {
      ...
      if (failed) {
        var err = _err("seeders/run-failed",
