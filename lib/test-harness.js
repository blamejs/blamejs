 ...
 *
 * Boot an isolated test harness. Returns a handle exposing `dataDir`,
 * `dbPath`, `vaultDir`, `env` (the env-var overrides set), and an
 * async `stop()` that tears the harness down (closes vault, removes
 * the temp directory, restores env).
 *
 * @opts
 ...
 *
 * @example
 *   var h = b.testHarness.start({ envPrefix: "MYAPP", initVault: true });
 *   try {
 *     // ... operator's app boot reads process.env.MYAPP_DATA_DIR etc.
 *     // ... run tests ...
 *   } finally {
...
function start(opts) {
  ...
  if (initVault) {
    try {
      vault().init({ dataDir: vaultDir, mode: "plaintext" });
    } catch (e) {
  ...
  async function stop() {
    ...
    // primitive scrub. Best-effort: if vault was init'd by operator
    // outside the harness, the reset still runs.
    if (initVault) {
      try {
        var v = vault();
