...
function _validateTlsRptReport(raw, ctx) {
  ...
  for (var i = 0; i < policies.length; i += 1) {
    ...
    if (summary && typeof summary === "object") {
      var s = Number(summary["total-successful-session-count"]) || 0;
      var f = Number(summary["total-failure-session-count"])    || 0;
      totalSuccess += s;
      totalFailure += f;
    }
  }
 ...
 *
 * @opts
 *   trustedReporters:        string[],  // OPTIONAL — when set, refuses reports whose organization-name
 *                                       //   isn't in this list (RFC 8460 §5.3-class defense extended to HTTPS).
 *                                       //   Default: accept all; downstream operator filters by policy-domain.
...
function tlsRptIngestHttp(opts) {
  opts = opts || {};
  validateOpts(opts, ["trustedReporters", "maxCompressedBytes", "maxDecompressedBytes",
                       "maxRatio", "onAccept", "onRefuse", "audit", "compliance"],
