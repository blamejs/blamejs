...
function _domainOk(d) {
  ...
  for (var i = 0; i < d.length; i++) {
    ...
    if (codepointClass.isForbiddenControlChar(c, { forbidTab: true }) || c === 0x22) return false;
  }
  return true;
}
 ...
 *   rv.dnsTxtRecord;          // → "v=STSv1; id=<stamp of this publish>;"
...
function autoConfigXml(opts) {
  ...
  function _jmapServer(cfg) {
    ...
    for (var k = 0; k < cfg.url.length; k++) {
      ...
      if (codepointClass.isForbiddenControlChar(c, { forbidTab: true }) || c === 0x22) {
        throw new MailDeployError("mail-deploy/bad-jmap-url",
...
function tlsRptIngestHttp(opts) {
  ...
  return function tlsRptHandler(req, res) {
    ...
    if (ctRoot !== "application/tlsrpt+json" && ctRoot !== "application/tlsrpt+gzip") {
      ...
      safeAsync.safeApply(onRefuse,
        ["mail-tlsrpt/bad-content-type", "unexpected content-type " + ctRoot, req]);
    ...
    if (authenticate) {
      ...
      authPromise.then(function (ok) {
        if (!ok) {
          ...
          safeAsync.safeApply(onRefuse,
            ["mail-tlsrpt/unauthenticated", "authenticate(req) returned falsy", req]);
        ...
        safeAsync.safeApply(onRefuse,
          ["mail-tlsrpt/auth-error", (err && err.message) || String(err), req]);
    ...
    req.on("data", function (chunk) {
      ...
      try { collector.push(chunk); }
        ...
        safeAsync.safeApply(onRefuse,
          ["mail-tlsrpt/oversize-compressed", "body exceeded " + maxCompressed + " bytes", req]);
        if (!res.headersSent) {
    ...
    req.on("end", function () {
      ...
      try {
        ...
        safeAsync.safeApply(onRefuse, [code, (e && e.message) || String(e), req]);
      ...
      if (trusted && !trusted[report["organization-name"]]) {
        ...
        safeAsync.safeApply(onRefuse, ["mail-tlsrpt/untrusted-reporter",
        ...
        try {
          ...
          if (ret && typeof ret.then === "function") {
            ret.then(function () {
              ...
              if (!res.headersSent) {
                res.writeHead(C.HTTP.STATUS.INTERNAL_SERVER_ERROR, { "Content-Type": "text/plain" });
