 ...
 *   protocol primitives. Composes existing verifiers (`b.mail.mtaSts`
 *   inbound verifier shipped pre-v0.9.46, `b.mail.bimi` BIMI trust-
 *   anchor verifier shipped v0.9.x) so the publish-side helpers stay
 *   thin and the operator only learns one vocabulary.
 *
 *   Surface:
...
function _domainOk(d) {
  ...
  // Refuse CR/LF/NUL outright (header-injection class) + obvious
  // non-LDH bytes.
 ...
 * @since      0.9.56
 * @status     stable
 * @related    b.mail.mtaSts (inbound verifier)
 *
...
function autoConfigXml(opts) {
  ...
  var incoming = "";
  if (opts.imap) incoming += _server("incoming", opts.imap, "imap");
  if (opts.pop3) incoming += _server("incoming", opts.pop3, "pop3");
  if (!incoming) {
    throw new MailDeployError("mail-deploy/bad-opts",
      "autoConfigXml: at least one of opts.imap / opts.pop3 / opts.jmap required");
  }
  var outgoing = opts.smtp ? _server("outgoing", opts.smtp, "smtp") : "";
