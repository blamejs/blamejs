...
async function createApp(opts) {
  ...
  function listen(listenOpts) {
    ...
    return new Promise(function (resolve, reject) {
      // A bind failure is an `error` event on the server, not a throw: the
      // try/catch below sees a bad TLS option and nothing else. Without a
      // listener for the event, `EADDRINUSE` and `EACCES` end the process
      // instead of rejecting, and the port already being taken is the ordinary
      // way an operator meets this. Armed before the bind can settle and
      // removed once it succeeds, so a later server error is not swallowed by
