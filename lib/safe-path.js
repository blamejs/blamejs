...
function _resolveCore(base, rel, opts) {
  ...
  // The containment boundary separator MUST be the RUNTIME separator, because
  // baseResolved / joined were produced by nodePath.resolve, which always
  // resolves with runtime semantics regardless of opts.platform. Deriving it
  // from opts.platform (isWin) instead means a caller validating against a
  // NON-runtime platform's naming rules (the recommended cross-platform
  // pattern) gets every in-base path falsely refused, because the boundary
  // slice can never match the runtime-separated output. opts.platform / isWin
  // continues to gate only the per-segment naming rules above (reserved names,
  // trailing dot/space, ADS colon), never the lexical-containment separator.
  var sepChar = nodePath.sep;
