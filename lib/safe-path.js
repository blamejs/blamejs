 ...
 * and the one a server confining an OPERATOR-PLACED tree (b.staticServe)
 * composes directly — such a server must still serve a POSIX file whose
 * name legitimately carries a `:` (ISO-timestamp logs) or a Windows
 * reserved word, which `resolve` refuses by design.
 *
 * @opts
