...
function fakeClock(initialMs) {
  if (initialMs === undefined) initialMs = 1_000_000;
  if (typeof initialMs !== "number" || !isFinite(initialMs)) {
    throw _err("testing/bad-input", "fakeClock: initialMs must be a finite number, got " +
  ...
  function advance(ms) {
    if (typeof ms !== "number" || !isFinite(ms)) {
      throw _err("testing/bad-input", "fakeClock.advance: ms must be a finite number");
    }
    current += ms;
  }
  function set(ms) {
    if (typeof ms !== "number" || !isFinite(ms)) {
      throw _err("testing/bad-input", "fakeClock.set: ms must be a finite number");
    }
    current = ms;
  }
  return {
...
function fakeHttpClient(responder) {
  if (typeof responder !== "function") {
    throw _err("testing/bad-input", "fakeHttpClient: responder must be a function");
  }
  var calls = [];
  return {
    calls: calls,
...
async function runMiddleware(middleware, req, res, opts) {
  if (typeof middleware !== "function") {
    throw _err("testing/bad-input", "runMiddleware: middleware must be a 3-arg (req, res, next) function");
  }
  opts = opts || {};
  req = req || mockReq();
  res = res || mockRes();
  ...
  if (timeoutMs !== 0 && !_isFiniteNonNegative(timeoutMs)) {
    throw _err("testing/bad-input", "runMiddleware: timeoutMs must be a non-negative finite number (0 disables)");
  }
...
async function waitFor(predicate, opts) {
  if (typeof predicate !== "function") {
    throw _err("testing/bad-input", "waitFor: predicate must be a function returning truthy/falsy or Promise<bool>");
  }
  opts = opts || {};
  ...
  if (!_isFiniteNonNegative(timeoutMs) || timeoutMs <= 0) {
    throw _err("testing/bad-input", "waitFor: timeoutMs must be a positive finite number");
  }
  if (!_isPositiveInt(intervalMs)) {
    throw _err("testing/bad-input", "waitFor: intervalMs must be a positive integer");
  }
...
function tempDir(prefix) {
  ...
  if (typeof prefix !== "string" || prefix.length === 0) {
    throw _err("testing/bad-input", "tempDir: prefix must be a non-empty string");
  }
  if (prefix.indexOf("..") !== -1 || prefix.indexOf("/") !== -1 ||
      prefix.indexOf("\\") !== -1 || prefix.indexOf("\0") !== -1) {
    throw _err("testing/bad-input",
