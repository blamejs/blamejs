...
function create(opts) {
  ...
  function _handleConnection(socket) {
    ...
    //
    // A backstop, not the writer. The flag has to be set where the teardown is
    // DECIDED, because `destroy()` emits `close` a macrotask later while a
    // chunk already chained onto `pumpChain` resumes on a microtask -- so a
    // handler that refuses the connection is followed by the peer's queued
    // commands, which is the ordering the flag exists to stop. This still
    // catches a peer-initiated close.
    socket.on("close", function () { connClosed = true; });

    // Close the SESSION. Every refusal in this listener goes through here so
    // the queued-chunk guard is true before the next microtask runs.
    function _closeSession(sock) {
      connClosed = true;
      // connClosed above is set synchronously so the chunk queue stops at
      // once; only the socket teardown waits for the reply to flush.
    ...
    async function _handleMailFrom(state, socket, line) {
      ...
      // Two different faults, and they were answered with one sentence. A
      // second MAIL with a transaction already open is RFC 5321 section 4.1.4's
      // "sender already specified"; telling that client to issue EHLO sends a
      // reader looking for a session that lost its EHLO, and there is no such
      // session. `state.mailFrom` is the condition the RFC names, and the
      // guard tested `state.stage !== "mail"` -- a value nothing in this file
      // ever assigns, so that half never excluded anything.
      // `!== null`, not truthiness: `MAIL FROM:<>` is the null reverse path
      // (RFC 5321 section 4.5.5, the bounce sender) and stores as the EMPTY
      // STRING, which is falsy -- so a truthiness test would miss exactly the
      // case an MX sees most. `null` is what _resetTransaction writes.
