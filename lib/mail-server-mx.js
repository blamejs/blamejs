...
function create(opts) {
  ...
  function _handleConnection(socket) {
    ...
    socket.on("data", function (chunk) {
      // Chain each chunk behind the previous one's full (async) processing
      // so command handlers + their gates run strictly in arrival order.
      pumpChain = pumpChain.then(function () {
        if (connClosed) return undefined;
        return _ingestBytes(state, socket, chunk);
      }).catch(function (err) {
