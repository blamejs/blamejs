 ...
 *
 *   - **STARTTLS stripping (CVE-2021-38371 Exim)** — once STARTTLS
 *     is advertised + selected, subsequent commands MUST run over
 *     the negotiated TLS context. A pre-STARTTLS pipelining attempt
 *     to inject commands that take effect post-handshake is refused
 *     by clearing the command buffer at STARTTLS time and reading
 *     fresh from the TLS socket only.
 *
 ...
 *   - **8BITMIME / SMTPUTF8** — advertised via opts but the DATA
 *     parser path through `b.safeMime` is encoding-agnostic; the
 *     wire-protocol layer here doesn't transcode.
 *
...
function create(opts) {
  ...
  function _handleConnection(socket) {
    ...
    function _ingestBytes(state, socket, chunk) {
      if (inDataBody) {
        ...
        if (_detectSmugglingShape(collected)) {
        ...
        // Canonical \r\n.\r\n terminator?
        var endIdx = _findDotTerminator(collected);
        if (endIdx !== -1) {
    ...
    function _handleStartTls(state, socket) {
      ...
      // CVE-2021-38371 (Exim STARTTLS-injection) defense: clear the
      // command buffer + body collector at upgrade time. Any commands
      // pipelined BEFORE the TLS handshake are discarded — only
      // commands sent on the post-handshake TLS socket are honored.
      lineBuffer    = "";
      bodyCollector = null;
      inDataBody    = false;
  ...
  async function listen(listenOpts) {
    ...
    var port    = listenOpts.port    || 25;                                                           // allow:raw-byte-literal — SMTP MX port (IANA)
...
function _detectSmugglingShape(buf) {
  // CVE-2023-51764 SMTP smuggling: a bare-LF dot-terminator inside
  // the DATA body. RFC 5321 §2.3.8 mandates CRLF-only line termination;
  // the smuggling exploit relies on parsers that accept \n.\n as an
  // alternate terminator. This detector scans for the shape; the
  // listener refuses the transaction on match.
...
function _findDotTerminator(buf) {
  // Canonical \r\n.\r\n terminator. Returns the byte index where the
  // body ends (exclusive), or -1 if not yet seen.
...
function _dotUnstuff(buf) {
  // RFC 5321 §4.5.2 — reverse the client-side dot-stuffing of body
  // lines beginning with `.`. Scan + write to a fresh buffer.
  var out = Buffer.alloc(buf.length);
  var oi = 0;
  for (var i = 0; i < buf.length; i += 1) {
    out[oi++] = buf[i];
    // After \r\n, if the next byte is `.` followed by ANOTHER non-CR
    // byte (i.e., not the terminator), strip the dot.
...
module.exports = {
  ...
  _detectSmugglingShape: _detectSmugglingShape,
  _findDotTerminator:    _findDotTerminator,
  _dotUnstuff:           _dotUnstuff,
};
