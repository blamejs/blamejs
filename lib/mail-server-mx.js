...
function create(opts) {
  ...
  function _resolveLocalDomains() {
    try { return _resolveLocalDomainsInner(); }
      ...
      if (!_ldWarned.has("__resolver")) {
        _ldWarned.add("__resolver");
  ...
  function _resolveLocalDomainsInner() {
    ...
    try { raw = localDomainsFn(); }
      ...
      if (!_ldWarned.has("__threw")) {
        _ldWarned.add("__threw");
        _emit("mail.server.mx.local_domains_unavailable",
    ...
    if (!Array.isArray(raw)) {
      if (!_ldWarned.has("__shape")) {
        _ldWarned.add("__shape");
        _emit("mail.server.mx.local_domains_unavailable",
    ...
    // JSON rather than a join: it quotes and escapes every entry, so no two
    // different sets can collide on one key — a join can, whenever an entry
    // contains the separator. One pass to build it, against a b.guardDomain
    // call per entry per recipient when it lets the work be skipped.
    //
    // Guarded, because everything else in this resolver fails closed and a
    // throw here would not: JSON.stringify raises on a BigInt or a circular
    // reference, and this runs inside RCPT handling, so one malformed entry
    // would take down the connection that happened to arrive rather than
    // refusing its recipients.
    var contentKey = null;
    try { contentKey = JSON.stringify(raw); }
    catch (err) {
      if (!_ldWarned.has("__unserializable")) {
        _ldWarned.add("__unserializable");
        _emit("mail.server.mx.local_domains_unavailable",
    ...
    // Per ENTRY, not per operation. Coercion and validation are both reached
    // with operator-supplied runtime data — an object with a throwing toString,
    // a value the guard chokes on — and this runs inside RCPT handling, so
    // anything that escapes ends the connection that happened to arrive rather
    // than refusing its recipients. One bad entry drops and the rest of the set
    // still serves, which is the same answer the guard gives for one the guard
    // itself rejects.
    var out = [];
    for (var i = 0; i < raw.length; i += 1) {
      var d = null;
      try {
        d = String(raw[i]).toLowerCase();
        if (guardDomainProfile) {
          ...
          if (!verdict.ok) {
            if (!_ldWarned.has(d)) {
              ...
              _emit("mail.server.mx.local_domain_refused",
                { domain: d,
                  kind: (verdict.issues && verdict.issues[0] && verdict.issues[0].kind) || null,
        ...
        var label = "__entry" + i;
        if (!_ldWarned.has(label)) {
          _ldWarned.add(label);
          _emit("mail.server.mx.local_domain_refused",
            { domain: null, kind: "unreadable",
              remark: "localDomains() entry " + i + " could not be read (" +
                      String((entryErr && entryErr.message) || entryErr).slice(0, 160) +
