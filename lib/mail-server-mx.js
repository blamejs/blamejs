  ...
  function _handleConnection(socket) {
    ...
    async function _ingestBytes(state, socket, chunk) {
      if (inDataBody) {
        ...
        //
        // Slow-loris floor. idleTimeoutMs cuts a fully stalled connection; a
        // peer trickling a few bytes at a time resets that timer forever and
        // holds the connection — and its slot in the per-address cap — for as
        // long as it likes. The module has documented this floor since it
        // shipped and no listener enforced it.
