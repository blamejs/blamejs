 ...
 * @signature  b.guardCountry.isValid(input, opts?)
 * @since      0.18.44
 * @status     stable
 * @related    b.guardCountry.validate, b.guardCountry.sanitize
 *
 * Whether `input` is an officially assigned ISO 3166-1 alpha-2
 * code, as a boolean. Sugar for `validate(input, opts).ok`, for the
 * call site that wants a predicate rather than a result object.
 * Case-insensitive; never throws, so a non-string is `false` rather
 * than an exception.
 *
 * @opts
 *   profile:            "strict"|"balanced"|"permissive",
...
module.exports = gateContract.defineGuard({
  ...
  extra: {
    ...
    // Reads validate off module.exports rather than closing over the guard,
    // because the guard does not exist yet inside its own factory call.
    isValid: function isValid(input, opts) {
      try { return module.exports.validate(input, opts).ok === true; }
