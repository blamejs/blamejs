...
function _scanJsonShapes(text) {
  var found = {
    ...
    singleQuotedKey: -1, hexLiteral: -1, bigInteger: -1, pollutionKeys: [],
  };
  var i = 0;
  while (i < text.length) {
    ...
    if (cc === APOSTROPHE) {
      // A single-quoted run followed by a colon is a JSON5 key.
    ...
    if (_isDigit(cc)) {
      ...
      // Only an INTEGER: a digit run followed by a fraction or an exponent is
      // a double the operator already accepted as approximate.
      var terminator = text.charCodeAt(i);
      var isInteger = !(terminator === 0x2E || terminator === 0x65 || terminator === 0x45);
      if (isInteger && i - digitsStart >= UNSAFE_INTEGER_DIGITS && found.bigInteger === -1) {
