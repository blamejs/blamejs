...
function dispatcher(opts) {
  ...
  async function _attemptDelivery(deliveryId) {
    ...
    try {
      ...
      // as a WebhookDispatcherError (won't fix on retry) -> dead-letter. A raw
      // resolver / network fault from the host-resolution step (EAI_AGAIN,
      // ETIMEDOUT, SERVFAIL, ...) is TRANSIENT like every other transport error
      // below -> back off and retry (capped at maxAttempts) instead of
      // dead-lettering a delivery on a transient DNS blip.
      var permanent = err instanceof WebhookDispatcherError;
