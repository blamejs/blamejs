...
function dispatcher(opts) {
  ...
  async function declareSchema(xdb) {
    ...
    // Partial index on the due-pending pool the retry poller scans.
    var deliveriesIdx = sql.toExternalSql(sql.createIndex(deliveriesTable + "_pending_idx",
      deliveriesTable, ["next_attempt_at"],
      { dialect: dialect, where: "status = 'pending'" }), dialect);
  ...
  async function _attemptDelivery(deliveryId) {
    ...
    try {
      ...
      // (and, being permanent, dead-letters immediately rather than retrying).
      ...
      // An HTTP error status is transient — back off and retry.
      return await _onFailure(deliveryId, attemptNo, "delivery returned HTTP " + status, false);
    } catch (err) {
      // A permanent error (SSRF-refused, malformed URL) won't fix itself on
      // retry — dead-letter it now. Transient errors (network, TLS, timeout)
      // back off.
      return await _onFailure(deliveryId, attemptNo,
        (err && err.message) || String(err), !!(err && err.permanent));
    }
  }
