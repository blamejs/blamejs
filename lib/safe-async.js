...
function _routeCallbackError(onError, e) {
  ...
  try { returned.then(null, function () { /* nowhere left to report */ }); }
  catch (_e3) { /* a thenable whose then throws */ }
}
...
function containRejection(value, onError) {
  ...
  // A thenable whose own `then` throws is not a promise anything can attach
  // to, so its failure is routed here rather than left to escape.
  try { value.then(null, function (e) { _routeCallbackError(onError, e); }); }
