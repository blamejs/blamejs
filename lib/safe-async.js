...
//
// An `async` callback is as ordinary a thing for an operator to write as a
// synchronous one, and its rejection is the same failure arriving a turn
// later. A synchronous try/catch cannot see it, and a rejection nobody
// observes ends the process under Node's default — so the returned value is
// inspected and a rejection routed to onError exactly as a throw is. Without
// that, the drop-silent promise holds only for callbacks that happen not to
// be async, which is not a contract anyone can rely on.
//
...
// safeApply — safeInvoke for the hooks that take more than one argument.
// Roughly a third of the framework's operator hooks do: onEvict(key, value),
// onMissingKey(key, locale), onRefuse(code, detail, req), onExit(code, id).
// Without this each of those sites wraps the call in a closure, which is both
// noisier and one more place for the containment to be left out.
/**
...
// The fulfillment handler handed to a thenable whose value nothing here wants.
// Named rather than inline so both attachment sites pass the same thing, and
// so it is obvious that "we ignore the value" is deliberate.
function _noop() {}

function _routeCallbackError(onError, e) {
  if (typeof onError !== "function") return;
  var returned;
  try { returned = onError(e); } catch (_e2) { return; /* nested handler must not bubble */ }
  // An async error handler rejects like any other callback, and there is
  // nowhere left to report it — the reporter is the thing that failed. It is
  // swallowed rather than left to reach the process.
  //
  // Reading `then` is inside the guard for the same reason it is in
  // containRejection: on a Proxy or custom thenable the property is a GETTER,
  // and one that throws here escapes into a promise-rejection handler, where
  // it becomes the unhandled rejection this whole file exists to prevent.
  if (!returned) return;
  try {
...
// containRejection — the half of safeInvoke that a caller needing the
// callback's return value cannot get from safeInvoke itself. A hook whose
// result decides what happens next (did it commit the response?) has to be
// called directly, and then its rejection still has to go somewhere.
/**
...
function containRejection(value, onError) {
  ...
  // Reading `then` is itself the callback's code: on a custom thenable or a
  // Proxy the property is a GETTER, and one that throws would escape a guard
  // placed after the read — out of here, out of safeInvoke, and into whatever
  // request, timer or teardown path was told these helpers never throw. So the
  // read, the type check and the call are all inside the try, and a thenable
  // that cannot be attached to has its failure routed like any other.
  try {
    var then = value.then;
    if (typeof then !== "function") return value;
    // A no-op rather than null for the fulfillment handler. A native promise
    // ignores a non-callable one, but a hand-written thenable is usually
    // `then(resolve, reject) { ... resolve(v) }` — and calling `null(v)` is a
    // TypeError thrown on a later turn, outside this try and outside anything
    // that could catch it. The one shape that must never crash the process is
    // the one whose whole contract is that it does not.
