...
function validate(line, opts) {
  ...
  // codeql[js/user-controlled-bypass] — `verb` is sourced from the wire,
  // but `_validateAuth` only PARSES the AUTH command's syntax; it does
  // not perform authentication, grant any credential, or alter trust
  // state. The dispatch is the standard verb→parser table every SMTP
  // parser implements; refusing to dispatch on a user-controlled verb
  // would defeat the parser entirely. The "Auth" in the function name
  // is the cause of the CodeQL flag; renaming the helper to
  // `_validateAuthCommandSyntax` would be cosmetic — the dispatch
  // shape is identical for every verb.
