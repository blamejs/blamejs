 ...
 *   // → { verb: "MAIL", args: ["<alice@example.com>"], params: { SIZE: "12345" } }
 */
...
function _validatePath(verb, rest, caps, requiredPrefix) {
  ...
  var pathEnd = safeBuffer.unquotedIndexOf(after, ">", 1);
 ...
 *   // → { ok: true, action: "serve", issues: [], ... } — the gate-contract
 *   //   decision shape, whose other fields are null on an accepted line
 */
function gate(opts) {
  opts = opts || {};
 ...
 * @signature b.guardSmtpCommand.detectBodySmuggling(buf, isBodyStart?, precededByCr?, moreComing?)
 * @since     0.9.46
 * @status    stable
 ...
 * relies on parsers that accept a non-canonical body terminator
 * (`\n.\n`, `\r\n.\n`, `\n.\r\n`, `\r.\r`, `\r.\n`, `\n.\r`,
 * `\r\n.\r`) and then resume parsing the NEXT bytes as a new SMTP
 * transaction.
 *
 * Returns `true` if the buffer contains a `.` alone on its line whose
 * surrounding line endings are not both canonical CRLF, so at least
 * one is a bare LF or bare CR: `\n.\n`, `\r\n.\n`, `\n.\r\n`, `\r.\r`,
 * `\r.\n`, `\n.\r`, `\r\n.\r`. Only the canonical `\r\n.\r\n`
 * end-of-data marker (and an empty-body `.\r\n` at the start) returns
 * `false`, as does any lone bare LF or CR not forming such a
 * dot-line, so a binary BDAT body is not refused for its content
 * bytes. `isBodyStart` marks a buffer that begins the body (so a
 * leading `.` is a boundary); `precededByCr` carries whether the byte
 * before `buf[0]` was a CR, so a CRLF split across two chunks is read
 * as canonical. A trailing `.` then bare CR at the end of the buffer,
 * after a canonical `\r\n.`, is the ambiguous split of a canonical
 * `\r\n.\r\n` whose `\n` is in the next chunk, so it is deferred when
 * `moreComing` is omitted or `true`; pass `moreComing: false` to mark
 * a final buffer, where that trailing `.\r` is a bare-CR terminator.
 * Operators wiring an MX / submission listener call this on every DATA
 * chunk (`moreComing: true`) or the whole BDAT body, and refuse the
 * transaction on `true`.
 *
 * @example
 ...
 */
function detectBodySmuggling(buf, isBodyStart, precededByCr, moreComing) {
  ...
  var n = buf.length;
  for (var i = 0; i < n; i += 1) {
    if (buf[i] !== 0x2e) continue;
    var beforeCanonical;
    if (i === 0) {
      if (isBodyStart !== false) beforeCanonical = true;
      else if (precededByCr === true) beforeCanonical = false;
      else continue;
    } else if (buf[i - 1] === 0x0a) {
      beforeCanonical = i >= 2 ? buf[i - 2] === 0x0d : precededByCr === true;
    } else if (buf[i - 1] === 0x0d) {
      beforeCanonical = false;
    } else {
      continue;
    }
    var a1 = i + 1 < n ? buf[i + 1] : -1;
    var afterCanonical;
    if (a1 === 0x0a) {
      afterCanonical = false;
    } else if (a1 === 0x0d) {
      if (i + 2 < n) {
        afterCanonical = buf[i + 2] === 0x0a;
      } else if (beforeCanonical && moreComing !== false) {
        continue;
      } else {
        afterCanonical = false;
      }
