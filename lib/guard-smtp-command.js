...
function validate(line, opts) {
  ...
  // Verb→parser lookup. Using a Map (not a plain object) so the
  // dispatch is a Map.get on a string key, not a property access on
  // a user-controlled object key. The Map has no prototype-chain
  // surface (Object.create(null)-like safety by construction) and
  // CodeQL's js/unvalidated-dynamic-method-call tracker treats Map
  // lookups as data access rather than dynamic method dispatch.
  var parser = VERB_PARSERS.get(verb);
  if (parser) return parser(verb, rest, caps);
  return { verb: verb, args: [], params: {} };
}

var VERB_PARSERS = new Map([
  ["EHLO", function (verb, rest, caps) { return _validateGreeting(verb, rest, caps); }],
  ["HELO", function (verb, rest, caps) { return _validateGreeting(verb, rest, caps); }],
  ["MAIL", function (verb, rest, caps) { return _validatePath(verb, rest, caps, "FROM:"); }],
  ["RCPT", function (verb, rest, caps) { return _validatePath(verb, rest, caps, "TO:"); }],
  ["BDAT", function (_verb, rest)      { return _validateBdat(rest); }],
  ["VRFY", function (verb, rest, caps) { return _validateMailbox(verb, rest, caps); }],
  ["EXPN", function (verb, rest, caps) { return _validateMailbox(verb, rest, caps); }],
  ["AUTH", function (_verb, rest)      { return _parseAuthCommandSyntax(rest); }],
  ["NOOP", function (verb, rest)       { return { verb: verb, args: rest ? [rest] : [], params: {} }; }],
  ["HELP", function (verb, rest)       { return { verb: verb, args: rest ? [rest] : [], params: {} }; }],
]);
