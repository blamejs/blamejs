 ...
 * as canonical; `moreComing` marks a streaming chunk that a further
 * chunk may follow, so a trailing `.\r` is deferred rather than read
 * as a bare CR. Operators wiring an MX / submission listener call
 * this on every DATA chunk (`moreComing: true`) or the whole BDAT
 * body, and refuse the transaction on `true`.
 *
 * @example
