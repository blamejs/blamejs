...
function validate(line, opts) {
  ...
  // C0 controls (except SP=0x20 and HTAB=0x09 — neither valid in SMTP
  // commands anyway, both refused below) plus DEL.
...
function gate(opts) {
  opts = opts || {};
  var resolved = _resolveProfile(opts);
  ...
  // resolved captured to surface bad profile/posture early via _resolveProfile
