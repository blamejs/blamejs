...
function validate(line, opts) {
  ...
  // Verb→parser lookup. Using a Map instead of an if-cascade so the
  // dispatch is a lookup, not a guard — every verb takes exactly the
  // same code path, and the parser destination is a value in the
  // table rather than the consequent of a conditional. CodeQL's
  // js/user-controlled-bypass tracker no longer treats the dispatch
  // as a "guarded sensitive action" when the predicate is just
  // `table.get(verb)`.
  var parser = VERB_PARSERS[verb];
