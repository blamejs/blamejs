...
function _anyValueToProto(v, depth) {
  ...
  if (t === "number") {
    if (Number.isInteger(v)) {
      // int_value uses varint encoding (sint64 in proto, but the
      // encoder treats it as uint64 since OTLP only sends non-negative
      // counters / timestamps in practice).
      return v < 0
        ? pb.embeddedMessage(3, pb._writeVarint(v >>> 0))  // negative → bit-cast
        : pb.uint64(3, v);
    }
