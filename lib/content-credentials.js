...
function _verifyIdentityX509Chain(certChainPem, trustAnchorsPem) {
  ...
  var leaf, anchorCerts;
  try { leaf = new nodeCrypto.X509Certificate(chain[0]); }
  catch (_eLeaf) { return { ok: false, reason: "bad-leaf-cert" }; }
  ...
  var now = Date.now();
  if (now < leaf.validFromDate.getTime() || now > leaf.validToDate.getTime()) {
    return { ok: false, reason: "leaf-expired" };
  }
  for (var a = 0; a < anchorCerts.length; a += 1) {
    var anchor = anchorCerts[a];
    var chained = false;
    try { chained = leaf.checkIssued(anchor) && leaf.verify(anchor.publicKey); }
    catch (_eChk) { chained = false; }
    if (!chained && leaf.fingerprint256 === anchor.fingerprint256) chained = true;   // leaf IS the anchor
    if (chained) {
