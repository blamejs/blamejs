...
function encodeSignedData(opts) {
  ...
  if (Array.isArray(opts.certificates) && opts.certificates.length > 0) {
    ...
    certsBlock = _writeImplicit(0, concat);
  }
...
function _writeImplicit(tagNumber, payload) {
  // [N] IMPLICIT constructed context-specific tag.
...
function _recipientInfo(recipient, contentKey) {
  ...
  var kemRi = asn1.writeNode(TAG_SEQUENCE, Buffer.concat([
    ...
    _writeImplicit(0, recipient.recipientId),                                                         // [0] IMPLICIT rid
...
function _encryptedContentInfo(plaintext, contentKey) {
  ...
  return asn1.writeNode(TAG_SEQUENCE, Buffer.concat([
    ...
    _writeImplicitOctetString(0, ct),
  ]));
}

function _writeImplicitOctetString(tagNumber, value) {
  // [N] IMPLICIT OCTET STRING — primitive context-specific tag (no constructed bit).
