...
function decode(buf, opts) {
  ...
  var node;
  try { node = asn1.readNodeStrict(buf); }
  catch (e) {
    if (e && e.code === "asn1/trailing-data") {
      throw new CmsCodecError("cms/trailing-data",
        "decode: trailing bytes after the top-level ContentInfo DER structure");
    }
...
function _recipientInfo(recipient, contentKey) {
  ...
  var kemRi = asn1.writeNode(TAG_SEQUENCE, Buffer.concat([
    ...
    _algorithmIdentifier(OID.mlkem1024),
    asn1.writeOctetString(Buffer.from(encap.cipherText)),
    _algorithmIdentifier(OID.shake256),
    asn1.writeInteger(Buffer.from([32])),
    _algorithmIdentifier(OID.chacha20Poly1305),
    asn1.writeOctetString(wrapped),
  ]));
