...
// Default response-body cap. Control-plane (PUT/HEAD/DELETE) replies are
// status confirmations — tens of bytes typically. Capping at 16 MiB
// across all methods would break GET for stored files larger than the
// cap, so callers pass `maxResponseBytes` per-request: small for control-
// plane, large (default 1 GiB) for GET. Files larger than 1 GiB should
// use getStream(), which is the streaming entrypoint by design.
...
function _request(method, url, body, headers, opts) {
  return new Promise(function (resolve, reject) {
    ...
    // PQC TLS group preference for HTTPS endpoints
    if (u.protocol === "https:") {
    ...
    var maxResponseBytes = (opts && opts.maxResponseBytes) ||
    ...
    var req = lib.request(reqOpts, function (res) {
      ...
      res.on("end", function () {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          reject(_err("HTTP_ERROR", "HTTP " + res.statusCode + " " + (res.statusMessage || ""), permanent, res.statusCode));
        }
      });
      res.on("error", function (e) {
        if (capExceeded) return;
        reject(_err(e.code || "RES_ERROR", e.message, false));
      });
    });

    req.on("timeout", function () {
      req.destroy();
      reject(_err("ETIMEDOUT", "request timeout", false));
    });
    req.on("error", function (e) {
      reject(_err(e.code || "REQ_ERROR", e.message, false));
    });
