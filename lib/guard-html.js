...
var BIDI_RE       = new RegExp("[" + _charClass(BIDI_RANGES) + "]");
// allow:dynamic-regex — codepoints from BIDI_RANGES literal table
var BIDI_RE_G     = new RegExp("[" + _charClass(BIDI_RANGES) + "]", "g");
// allow:dynamic-regex — codepoints from C0_CTRL_RANGES literal table
var C0_CTRL_RE    = new RegExp("[" + _charClass(C0_CTRL_RANGES) + "]");
// allow:dynamic-regex — codepoints from C0_CTRL_RANGES literal table
var C0_CTRL_RE_G  = new RegExp("[" + _charClass(C0_CTRL_RANGES) + "]", "g");
// allow:dynamic-regex — codepoints from ZERO_WIDTH_RANGES literal table
var ZERO_WIDTH_RE = new RegExp("[" + _charClass(ZERO_WIDTH_RANGES) + "]");
// allow:dynamic-regex — codepoints from ZERO_WIDTH_RANGES literal table
var ZW_RE_G       = new RegExp("[" + _charClass(ZERO_WIDTH_RANGES) + "]", "g");
// allow:dynamic-regex — single literal codepoint U+0000
var NULL_RE_G     = new RegExp(_hex4(0x0000), "g");
...
function _resolveOpts(opts) {
  ...
  if (typeof opts.profile === "string") {
    if (!PROFILES[opts.profile]) {
      throw _err("html.bad-profile", "unknown profile " + JSON.stringify(opts.profile));
    }
    overlay = PROFILES[opts.profile];
  }
  if (typeof opts.compliancePosture === "string") {
    if (!COMPLIANCE_POSTURES[opts.compliancePosture]) {
      throw _err("html.bad-posture",
        "unknown compliancePosture " + JSON.stringify(opts.compliancePosture));
    }
    overlay = Object.assign({}, overlay, COMPLIANCE_POSTURES[opts.compliancePosture]);
  }
  return Object.assign({}, DEFAULTS, overlay, opts);
}
...
function _detectIssues(input, opts) {
  ...
  // 1. Whole-input bidi / null-byte / control char.
  var bidiMatch = s.match(BIDI_RE);
  if (bidiMatch && opts.bidiPolicy !== "allow") {
    issues.push({
      kind: "bidi-override", severity: "critical", ruleId: "html.bidi",
      location: bidiMatch.index,
      snippet: "Unicode bidi override (CVE-2021-42574 Trojan Source)",
    });
  }
  var nullIdx = s.indexOf(NULL_BYTE);
  if (nullIdx >= 0 && opts.nullBytePolicy !== "allow") {
    issues.push({
      kind: "null-byte", severity: "critical", ruleId: "html.null-byte",
      location: nullIdx, snippet: "null byte at byte " + nullIdx,
    });
  }
  var ctrlMatch = s.match(C0_CTRL_RE);
  if (ctrlMatch && opts.controlPolicy !== "allow") {
    issues.push({
      kind: "control-char", severity: "high", ruleId: "html.control",
...
function _sanitize(input, opts) {
  ...
  if (opts.bidiPolicy === "reject" && BIDI_RE.test(s)) {                  // allow:regex-no-length-cap — input length capped by maxBytes above
    throw _err("html.bidi", "input contains Unicode bidi override");
  }
  if (opts.nullBytePolicy === "reject" && s.indexOf(NULL_BYTE) !== -1) {
    throw _err("html.null-byte", "input contains null byte");
  }
  if (opts.controlPolicy === "reject" && C0_CTRL_RE.test(s)) {                // allow:regex-no-length-cap — input length capped by maxBytes above
    throw _err("html.control", "input contains C0 control character");
  }

  // Strip-pass for character-class threats.
  if (opts.bidiPolicy === "strip")     s = s.replace(BIDI_RE_G, "");
  if (opts.controlPolicy === "strip")  s = s.replace(C0_CTRL_RE_G, "");
  if (opts.nullBytePolicy === "strip") s = s.replace(NULL_RE_G, "");
  if (opts.zeroWidthPolicy === "strip") s = s.replace(ZW_RE_G, "");
...
function validate(input, opts) {
  ...
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxBytes,
    "guardHtml.validate: maxBytes", GuardHtmlError, "html.bad-opt");
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxAttrValueBytes,
    "guardHtml.validate: maxAttrValueBytes", GuardHtmlError, "html.bad-opt");
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxTagDepth,
    "guardHtml.validate: maxTagDepth", GuardHtmlError, "html.bad-opt");
  numericBounds.requirePositiveFiniteIntIfPresent(opts.maxAttrsPerTag,
    "guardHtml.validate: maxAttrsPerTag", GuardHtmlError, "html.bad-opt");
...
function loadRulePack(pack) {
  validateOpts.requireObject(pack, "guardHtml.loadRulePack", GuardHtmlError);
  validateOpts.requireNonEmptyString(pack.id,
    "guardHtml.loadRulePack: pack.id", GuardHtmlError, "html.bad-opt");
