...
var SIG_ALGS = {
  "1.2.840.113549.1.1.1":  { hash: null,     scheme: "rsa" },
  "1.2.840.113549.1.1.11": { hash: "sha256", scheme: "rsa" },
  "1.2.840.113549.1.1.12": { hash: "sha384", scheme: "rsa" },
  "1.2.840.113549.1.1.13": { hash: "sha512", scheme: "rsa" },
  "1.2.840.113549.1.1.10": { hash: null,     scheme: "pss" },
  "1.2.840.10045.2.1":     { hash: null,     scheme: "ecdsa" },
  "1.2.840.10045.4.3.2":   { hash: "sha256", scheme: "ecdsa" },
  "1.2.840.10045.4.3.3":   { hash: "sha384", scheme: "ecdsa" },
  "1.2.840.10045.4.3.4":   { hash: "sha512", scheme: "ecdsa" },
};
...
function buildRequest(data, opts) {
  ...
  var children = [asn1.writeInteger(Buffer.from([1])), messageImprint];
...
function parseResponse(der) {
  ...
  var root;
  try { root = asn1.readNodeStrict(buf); } catch (e) {
    if (e && e.code === "asn1/trailing-data") {
      throw new TsaError("tsa/trailing-data",
        "tsa.parseResponse: trailing bytes after the top-level TimeStampResp DER structure");
    }
...
function _parseTstInfo(eContent) {
  var root;
  try { root = asn1.readNodeStrict(eContent); }
  catch (e) {
    throw new TsaError("tsa/malformed", "tsa: TSTInfo is not valid DER: " + ((e && e.message) || e));
  }
...
function _certExtension(certDer, wantOid) {
  ...
  var top = asn1.readSequence(cert.value);
...
function _candidateSigners(sidRaw, certs) {
  ...
  try {
    ...
    if (sid.tag === asn1.TAG.SEQUENCE && sid.tagClass === asn1.TAG_CLASS.UNIVERSAL) {
      var parts = asn1.readSequence(sid.value);
...
function _verifyCmsSignature(si, eContent, signerCertDer) {
  ...
  if (!attrs.contentType) {
    throw new TsaError("tsa/no-content-type-attr",
      "tsa.verifyToken: signed attrs missing the contentType attribute (RFC 5652 §11.1)");
  }
  if (attrs.contentType !== OID_TST_INFO) {
...
function _parseSignedAttrs(signedAttrsRaw) {
  ...
  for (var i = 0; i < attrs.length; i += 1) {
    var a = asn1.readSequence(attrs[i].value);
...
function _verifyChain(signerCertDer, tokenCerts, trustAnchorsPem, at) {
  ...
  var res = x509Chain.resolveChain(current, pool, anchors, {
    validAt: function (cert) {
      return atTime >= cert.validFromDate.getTime() && atTime <= cert.validToDate.getTime();
    },
  });
  if (res.ok) return;
  if (res.invalidCert) {
    throw new TsaError("tsa/cert-expired",
      "tsa.verifyToken: certificate '" + res.invalidCert.subject + "' is not valid at the asserted time");
  }
  if (res.reason === "pathlen") {
    throw new TsaError("tsa/chain-pathlen-exceeded",
      "tsa.verifyToken: certificate chain exceeds a CA basicConstraints pathLenConstraint (RFC 5280 §4.2.1.9)");
  }
  if (res.reason === "nameconstraint") {
    throw new TsaError("tsa/name-constraint-violation",
      "tsa.verifyToken: certificate chain violates a CA nameConstraints (RFC 5280 §4.2.1.10)");
  }
...
function verifyToken(token, opts) {
  ...
  try { sd = cms.parseSignedData(_bytes(token, "token")); }
    ...
    if (e && e.code === "cms/trailing-data") {
      throw new TsaError("tsa/trailing-data",
        "tsa.verifyToken: trailing bytes after the top-level token DER structure");
    }
