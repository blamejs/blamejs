...
function _verifyChain(signerCertDer, tokenCerts, trustAnchorsPem, at) {
  ...
  var walked = [current];
  var pathLenFailed = false;
  var seen = 0;
  var atTime = at.getTime();
  while (seen <= pool.length + 1) {
    _assertValidAt(current, atTime);
    for (var a = 0; a < anchors.length; a += 1) {
      var isIssuer = _issued(anchors[a], current);
      var isSelf = current.fingerprint256 === anchors[a].fingerprint256;
      if (!isIssuer && !isSelf) continue;
      if (!x509Chain.pathLenSatisfied(isIssuer ? walked.concat([anchors[a]]) : walked)) {
        pathLenFailed = true; continue;
      }
      if (isIssuer) _assertValidAt(anchors[a], atTime);
      return;
    }
    var parent = null;
