 ...
 *     trustAnchorsPem: string,    // PEM roots (array) — enables chain + validity verification
 *     at:              Date,      // validity instant for chain check (default: genTime)
 *   }
 *
 * @example
...
function verifyToken(token, opts) {
  ...
  // (7) optional chain + validity.
  if (Array.isArray(opts.trustAnchorsPem) && opts.trustAnchorsPem.length) {
    _verifyChain(signerCertDer, sd.certificates, opts.trustAnchorsPem, opts.at instanceof Date ? opts.at : tst.genTime);
  }
