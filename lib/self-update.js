"use strict";
/**
 * b.selfUpdate — GitHub-releases poll + signed-asset download + atomic
 * binary swap + rollback.
 *
 * Four-step lifecycle for an operator-driven self-update:
 *
 *   1. await b.selfUpdate.poll({ releasesUrl, currentVersion })
 *        Fetches the releases JSON (a GitHub /releases or any compatible
 *        feed exposing { tag_name, assets[{ name, browser_download_url }] })
 *        and reports whether a newer tag is available. Returns
 *        { available, latestTag, currentVersion, asset, signature, etag? }.
 *
 *   2. b.httpClient.downloadStream({ url: result.asset.url, dest, hash, expected })
 *        Operator pulls both the asset bytes and the detached signature
 *        bytes. The framework-shipped downloader handles SSRF guard,
 *        TLS posture, hash-while-streaming, and atomic rename of the
 *        download tmp file.
 *
 *   3. await b.selfUpdate.verify({ assetPath, signaturePath, pubkeyPem,
 *                                  hashAlgo? })
 *        Verifies the detached signature over the asset bytes via
 *        b.crypto.verify (auto-detects ML-DSA-87 / Ed25519 / EC from the
 *        operator-supplied public key PEM). Returns
 *        { verified: true, hash } or throws SelfUpdateError on mismatch.
 *
 *   4. await b.selfUpdate.swap({ from, to, backupTo })
 *        Atomic install: copies the current `to` to `backupTo`, then
 *        renames `from` -> `to`, then dir-fsyncs both directories. On
 *        any failure the operation rolls back and the original binary
 *        is restored. Returns { ok: true, swappedAt, backupTo }.
 *
 *   await b.selfUpdate.rollback({ to, backupTo })
 *      Restores `backupTo` -> `to` via the same atomic flow. Used when
 *      a post-swap healthcheck reports the new binary is bad.
 *
 * Audit:
 *   selfupdate.poll.checked        releasesUrl + currentVersion + latestTag + available
 *   selfupdate.verify.passed       assetPath + signaturePath + alg + hash
 *   selfupdate.verify.failed       same surface, outcome=denied
 *   selfupdate.swap.completed      from + to + backupTo
 *   selfupdate.rollback.completed  to + backupTo
 *
 * Composition:
 *   - Outbound HTTP routes through b.httpClient.request — TLS posture,
 *     SSRF guard, allowedHosts, audit-on-host-deny apply. The detached
 *     signature + asset are typically two separate calls; this module
 *     does NOT bundle them into one because the operator may want to
 *     pull from different release channels per artifact.
 *   - Atomic file ops route through b.atomicFile (write + rename + fsync).
 *   - Signature verify routes through b.crypto.verify so the alg vocabulary
 *     stays uniform with the rest of the framework (ML-DSA-87 / Ed25519 /
 *     ECDSA P-384 — auto-detected from the public key PEM).
 */
