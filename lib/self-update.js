...
// asset only when the release ships exactly one AND that sidecar references the
// asset stem (the common one-asset-one-sig case); anything ambiguous OR a lone
// sidecar whose name is unrelated to the asset fails closed (null) rather than
// pairing a signature that may not sign the returned asset.
...
async function rollback(opts) {
  ...
  // touching either file. (resolve() canonicalizes ./ and separator differences.)
  if (nodePath.resolve(quarantine) === nodePath.resolve(backupTo)) {
    throw new SelfUpdateError("selfupdate/rollback-failed",
      "selfUpdate.rollback: backupTo must not be the reserved quarantine path " +
