...
// asset has an extension to strip, the stem plus each suffix (replace
// convention). Every derived name ends in a signature suffix, so the boundary
// after the stem is always that suffix's leading delimiter — a bare-prefix
// look-alike (application.sig for app.bin, stem `app`) never derives, because
// the derived name is exactly `app.sig`.
function _derivedSignatureNames(assetName) {
  var stem  = _assetStem(assetName);
  var names = [];
  for (var s = 0; s < _SIG_SUFFIXES.length; s++) {
    names.push(assetName + _SIG_SUFFIXES[s]);
    if (stem !== assetName) names.push(stem + _SIG_SUFFIXES[s]);
  }
  return names;
}
...
function _selectSignatureFor(assetName, entries, signaturePattern) {
  ...
  var derived = _derivedSignatureNames(assetName);
