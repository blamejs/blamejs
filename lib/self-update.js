...
async function swap(opts) {
  ...
  // Re-verify the bytes about to be installed match what selfUpdate.verify
  // checked, closing the verify -> swap window: an attacker who swaps `from`
  // after verify passes (or points verify at a different inode via a symlink)
  // installs different bytes than were signature-verified. Re-hash immediately
  // before the install so the residual is a sub-millisecond window rather than
  // the operator-controlled gap between the two calls. (A pure-Node rename
  // can't pin the verified inode; this is the strongest available binding.)
  var swapAlg = opts.hashAlgo || DEFAULT_HASH_ALG;
  var fromBytes;
  try {
    fromBytes = await atomicFile.read(from, {
      maxBytes: typeof opts.maxBytes === "number" ? opts.maxBytes : C.BYTES.gib(1),
    });
  } catch (e) {
    throw new SelfUpdateError("selfupdate/swap-read-failed",
      "selfUpdate.swap: failed to read from for the integrity re-check: " + ((e && e.message) || String(e)));
  }
