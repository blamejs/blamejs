...
async function poll(opts) {
  ...
  if (res.statusCode === C.HTTP.STATUS.NOT_MODIFIED) {                                                    // HTTP status code (RFC 9110 §15.4.5), not bytes
    ...
    return { available: false, latestTag: null, currentVersion: opts.currentVersion,
             asset: null, signature: null, etag: opts.etag, statusCode: C.HTTP.STATUS.NOT_MODIFIED };    // HTTP status code (RFC 9110 §15.4.5), not bytes
  }
