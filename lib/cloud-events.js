...
// JSON-formatted media type: */json or */*+json (after stripping params).
var JSON_MEDIA_RE = /^[^;]*\/(?:[^;]*\+)?json\s*(?:;|$)/i;
...
function _isJsonMedia(ct) { return ct == null || JSON_MEDIA_RE.test(ct); }   // allow:regex-no-length-cap — linear regex, input bounded by maxBytes
...
function encodeBinary(event) {
  ...
  var body;
  if (event.data_base64 != null) body = Buffer.from(event.data_base64, "base64");
