 ...
 *
 *   `dsn.parse(raw, opts?)` holds EVERY per-recipient group to RFC 3464
 *   §2.3, not only the first — a report is one document, and the groups
 *   past the first are exactly what a consumer reads through
 *   `event.raw.allRecipients` to handle a multi-recipient report. A
 *   group with an Action outside §2.3.3, or with neither
 *   Final-Recipient nor Original-Recipient, refuses the whole report.
 *   `Action` is required, because it is the field that says whether the
 *   message failed, was delayed, or was in fact delivered; real-world
 *   reports that omit it parse under `{ requireAction: false }`.
 *
 * @card
...
// One per-recipient group, held to RFC 3464 §2.3 whether or not it is the
// group the normalized event happens to be built from. `index` and `total`
// only shape the message: a report with five groups that names which one is
// wrong is a report an operator can act on.
function _validateRecipientGroup(recip, index, total, requireAction) {
  var where = total > 1 ? " (group " + (index + 1) + " of " + total + ")" : "";
  ...
  if (!finalRecipient) {
    throw _err("bounce/dsn-malformed",
      "mailBounce.dsn.parse: per-recipient group missing Final-Recipient" + where);
  }
  var action = (recip["action"] || "").toLowerCase();
  if (!action) {
    if (!requireAction) return;
    throw _err("bounce/dsn-malformed",
      "mailBounce.dsn.parse: per-recipient group has no Action, which RFC 3464 " +
      "§2.3.3 requires" + where + " — pass { requireAction: false } to accept a " +
      "report that states no verdict");
  }
  if (!Object.prototype.hasOwnProperty.call(DSN_ACTIONS, action)) {
    throw _err("bounce/dsn-malformed",
      "mailBounce.dsn.parse: Action token '" + action + "' is not RFC 3464 §2.3.3" + where);
  }
}

function _parseDsn(rawMessage, opts) {
  // RFC 3464 §2.3.3 makes Action mandatory in every per-recipient group, and
  // it is the field that says whether the message failed, was delayed, or was
  // in fact DELIVERED — so a report that states none of those is refused
  // rather than handed on with an empty verdict a consumer would then act on.
  // Real reports do omit it, so the leniency exists; it is something an
  // operator chooses knowingly, not what an unconfigured parser does.
  var requireAction = !(opts && opts.requireAction === false);
  ...
  // EVERY group, not just the one the normalized event is built from. RFC 3464
  // §2.3 defines the per-recipient group as repeating precisely because a
  // report describes several recipients, and privileges none of them — while a
  // consumer handling the general case has to iterate `raw.allRecipients`,
  // which is exactly the part that went unchecked. A report is one document,
  // and half of one is not a smaller true statement: a bad group refuses the
  // whole report rather than leaving the readable half to be trusted.
  for (var pri = 0; pri < status.perRecipients.length; pri += 1) {
    _validateRecipientGroup(status.perRecipients[pri], pri,
      status.perRecipients.length, requireAction);
  }
