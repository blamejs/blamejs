...
function destroySocketAfterFlush(socket, cfg) {
  ...
  // Not cleared when the flush wins the race: `torn` makes a late firing a
  // no-op, and the timer is unref'd so a pending one cannot hold the process
  // open. One flag, rather than a flag and a handle to keep in step.
  var timer = setTimeout(hardDestroy, graceMs);
