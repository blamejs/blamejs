...
function _normalizedMailbox(address, delimiter) {
  ...
  var folded = foldSubaddress(address, delimiter);
  var at = folded.lastIndexOf("@");
  if (at <= 0 || at === folded.length - 1) return null;
  var rawDomain = folded.slice(at + 1);
  var domain = _isAddressLiteral(rawDomain)
    ? rawDomain : mailModule().toAscii(rawDomain);
  if (domain === null) return null;
  var raw = folded.slice(0, at);
  var local = _unquotedLocalPart(raw);
  if (local === null) return null;
  return { local: local, quoted: raw.charAt(0) === "\"", domain: domain.toLowerCase() };
}
