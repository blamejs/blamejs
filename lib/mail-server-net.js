...
// announcedLiteralBytes(lineBuffer, cfg) — how much of a receive buffer is
// literal PAYLOAD the client has announced, rather than queued commands.
//
// A backlog bound has to exclude it, and `state.pendingLiteral` alone does not
// say: a non-synchronizing opener arrives WITH its body in one read, so at the
// moment the bound runs no literal is pending yet and the whole payload counts
// as queue.
//
// `cfg.openerBytes(line)` decides, and the caller supplies it because the
// answer is the listener's own: it returns the octet count only for a line
// that would open a literal if it were taken NOW, in this session's state,
// and null otherwise. Matching the `{N}` tail instead would exempt a payload
// behind any line shaped like a command, so `BOGUS {1048576+}` and a megabyte
// of bytes would pass a bound the peer never had to satisfy.
//
// One literal is exempted, the first the scan reaches. Only one can be in
// flight, since the reader takes one command at a time; anything behind it is
// waiting, which is what the bound counts.
//
// The scan gives up after `maxPipelinedBytes` of queued commands rather than
// after a fixed number of lines. That is the point past which the caller
// refuses the connection whatever this answers, so nothing is lost, and it
// terminates because every line consumes at least the two bytes of its own
// terminator. A line count would have to be guessed instead: the allowance is
// several tens of kilobytes and a client may legally queue hundreds of short
// commands inside it before the one that opens a literal.
//
//   cfg.pending           the literal already in progress, if any
//   cfg.maxLineBytes      longest line worth scanning for an opener
//   cfg.maxPipelinedBytes the caller's backlog allowance
//   cfg.openerBytes       (line) -> octets this line opens, or null
function announcedLiteralBytes(lineBuffer, cfg) {
  ...
  while (pos <= cfg.maxPipelinedBytes) {
    ...
    pos += crlf + 2;
    if (announced === null || announced === undefined) continue;   // an ordinary queued command
...
// wireLineSocket(socket, cfg) — the socket wiring every line-protocol listener
// repeats: an idle timeout that answers before closing, an error handler that
// records rather than throws, and a data handler that appends to the session's
// line buffer and hands it to the reader.
//
// The three listeners had a copy each. They were near-identical before, and
// became identical enough for the duplicate detector to say so once their close
// contracts converged, which is the point at which it stops being three
// similar functions and starts being one function written three times. What
// genuinely differs is passed in: the reply writer's wording, the audit event
// name, and which reader takes the bytes.
//
//   cfg.idleTimeoutMs   idle budget for the socket
//   cfg.onIdleTimeout   called before the close, to write the protocol's own
//                       "idle timeout" reply
//   cfg.onError         called with the socket error, to emit under the
//                       listener's own event name
//   cfg.state           the session, whose `lineBuffer` accumulates
//   cfg.drain           (state, socket) reader, run after each chunk. OMIT it
//                       to wire only the timeout and error halves: a listener
//                       that counts wire bytes or gates a literal's payload
//                       needs its own data handler, and adding a second one
//                       here would append every chunk twice.
//   cfg.close           () teardown, run after onIdleTimeout and on the
//                       socket's own close
// How long a refusal is given to reach the peer before the socket is torn down
// regardless. A peer that never reads cannot hold a connection open past this.
var CLOSE_FLUSH_GRACE_MS = C.TIME.seconds(5);

// destroySocketAfterFlush(socket) — end the connection so the reply that
// preceded it actually arrives.
//
// `end()` queues the FIN behind whatever is still in the write buffer, and a
// `destroy()` on the next line tears the socket down without waiting for that
// buffer to drain. Whether the reply reached the peer then depended on whether
// it happened to fit in the kernel socket buffer, which depends on how much the
// peer sent -- not on anything the server decided. Measured against an MX with
// a 1 MiB ceiling and an undeclared DATA body: 2 KiB over the limit delivered
// `552 5.3.4`, 64 KiB over delivered it and then a clean FIN, and 256 KiB over
// delivered nothing at all and reset the connection.
//
// The failure ran the wrong way round. A small overshoot, where retrying is
// cheap, got the permanent answer; a large one, where retrying is expensive for
// both ends, got the reset -- and a peer that receives nothing has no verdict,
// so it falls back to its retry schedule and re-sends for days (RFC 5321
// section 4.2.1 makes the 5xx-vs-nothing distinction load-bearing). Several of
// the refusals on this path are ones a hostile peer can trigger deliberately,
// so the reply most worth delivering was the one most likely to be lost.
//
// The destroy moves onto the flush callback, with the socket's own close and a
// timer as backstops so a peer that stops reading cannot hold the connection.
// Callers still mark the session closed SYNCHRONOUSLY before calling this: the
// reader has to stop at once, and only the socket teardown waits.
function destroySocketAfterFlush(socket, cfg) {
  var graceMs = (cfg && cfg.graceMs) || CLOSE_FLUSH_GRACE_MS;
  var torn = false;
  var timer = null;
  // The timer is CLEARED when the flush or the close wins, not merely made a
  // no-op by the flag. `unref()` stops a pending timer holding the process
  // open and does nothing about what it retains: the callback closes over the
  // socket, so an uncleared entry keeps that socket and everything it
  // references alive for the whole grace period. Listeners answer short-lived
  // connections at whatever rate a peer opens them, and most of those close
  // promptly, so the retained set is bounded by arrival rate rather than by
  // anything the server decided.
  ...
  try {
    ...
    // A socket already past the point of taking an end() is already gone.
    hardDestroy();
  }
}
...
function wireLineSocket(socket, cfg) {
  ...
  // A connection also ends without the listener deciding it: the peer hangs
  // up, or `close()` destroys a lingering socket at shutdown. Those paths run
  // no refusal, so nothing marked the session, and a reader waiting on an
  // asynchronous handler resumed when it settled and executed whatever the
  // peer had already pipelined -- a store write on a connection that no longer
  // exists. Teardown is idempotent, so running it again here costs nothing.
