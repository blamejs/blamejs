"use strict";
/**
 * b.acme — RFC 8555 ACME client + RFC 9773 ACME Renewal Information.
 *
 * Operator surface:
 *
 *   var acme = b.acme.create({
 *     directory:  "https://acme-staging-v02.api.letsencrypt.org/directory",
 *     accountKey: { privatePem, publicPem, jwk, kty: "EC", crv: "P-256" },
 *     audit:      b.audit,
 *     contact:    ["mailto:ops@example.com"],   // optional
 *   });
 *
 *   await acme.fetchDirectory();                            // RFC 8555 §7.1.1
 *   await acme.newAccount();                                // RFC 8555 §7.3
 *   var order = await acme.newOrder({                       // RFC 8555 §7.4
 *     identifiers: [{ type: "dns", value: "example.com" }],
 *   });
 *   // operator runs HTTP-01 / DNS-01 challenges on order.authorizations
 *   await acme.finalize(order, csrDer);                     // RFC 8555 §7.4
 *   var certPem = await acme.retrieveCert(order);           // RFC 8555 §7.4.2
 *   var verdict = await acme.renewIfDue({ certPem });       // RFC 9773
 *
 * RFC 9773 ARI — `renewIfDue` consults the directory's `renewalInfo`
 * endpoint with the ACMECertID derived from the cert's AKI + serial.
 * The CA returns `{ suggestedWindow: { start, end }, explanationURL? }`.
 * If `Date.now()` is before `suggestedWindow.start`, the call is a
 * no-op + audit `acme.cert.renew.skipped`. If `Date.now()` is at or
 * past `suggestedWindow.start`, the verdict is `{ shouldRenew: true }`
 * + audit `acme.cert.renewed.scheduled`. Operators wire this with
 * `b.network.tls.expiryMonitor` so the renewal trigger composes.
 *
 * Directory URL: NO default. Operator passes the production CA's
 * directory URL (Let's Encrypt prod / Pebble in tests / any RFC 8555-
 * compliant CA). The framework refuses to default to a single CA — the
 * operator's CA choice is policy, not framework decision.
 *
 * JWS algorithm: ES256 (P-256 + SHA-256) — RFC 8555 §6.2 mandates this
 * for account-key signatures. ACME predates the JOSE PQC algorithm
 * registry; until CAs publish PQC-capable directories, the wire format
 * is classical. The framework's audit chain stays PQC-signed regardless.
 *
