 ...
 * `lib/notify.js` (Phase 10.10) and the framework fires them on the
 * specific high-signal conditions documented in `EVENTS` below.
 *
 ...
 *   });
 *
 * For async fan-out (PagerDuty webhook, etc.), wire through `lib/notify.js`
 * once Phase 10.10 lands — its queue absorbs the async work without
 * blocking the framework's fail-fast path.
 */
