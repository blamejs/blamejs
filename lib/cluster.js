...
async function init(opts) {
  ...
  // in via opts.allowedProtocols (urlSafe.ALLOW_HTTP_ALL).
  if (opts.endpoint != null) {
    try {
      urlSafe.parse(opts.endpoint, {
  ...
  // Boot-time rollback detection on the audit chain. Runs regardless of
  // role — every node should refuse to participate in a cluster whose
  // shared audit log has been rolled back (a follower would face the
  // same chain integrity failure if it later took over). Skipped when
  // configuredExternalDbBackend is unset, which means a custom provider
  // is in use without externalDb-resident framework state — the
  // operator owns rollback detection in that case.
  if (configuredExternalDbBackend) {
    await _checkAuditTipRollback();
...
async function _checkAuditTipRollback() {
  ...
  try {
    tipRows = await externalDb.query(
      "SELECT atMonotonicCounter, rowHash FROM _blamejs_audit_tip " +
      "WHERE scope = 'audit'",
      [],
      { backend: configuredExternalDbBackend }
    );
  } catch (e) {
    // Table missing → operator is running cluster gates-only mode
    // (the v0.1.0 scope: cluster.requireLeader gates protect followers
    // but framework state still lives in per-node SQLite, no
    // frameworkSchema.ensureSchema). The check is a no-op in that
    // configuration — there's no shared audit-tip to compare against.
    // SQLite says "no such table"; Postgres says "does not exist".
    var msg = (e && e.message) || "";
    if (/no such table|does not exist|relation .* does not exist/i.test(msg)) {
      log("audit-tip table not present — skipping rollback check (cluster gates-only mode)");
      return;
    }
    throw e;
  }
  if (!tipRows.rows || tipRows.rows.length === 0) {
    log("no audit-tip row — skipping rollback check (first cluster boot or operator-cleared)");
    return;
  }
  ...
  var currentRows = await externalDb.query(
    "SELECT MAX(monotonicCounter) AS m FROM _blamejs_audit_log",
    [],
    { backend: configuredExternalDbBackend }
  );
  ...
  if (currentMax < tipCounter) {
    log.error("FATAL: cluster-mode audit-log rollback detected.");
    log.error("  audit-tip counter:        " + tipCounter);
    log.error("  current external-db max:  " + currentMax);
    log.error("Either external-db was restored from an older snapshot, or " +
      "_blamejs_audit_log rows have been deleted. Investigate before continuing.");
    process.exit(1);
  }

  if (tipHash) {
    var hashRows = await externalDb.query(
      "SELECT rowHash FROM _blamejs_audit_log WHERE monotonicCounter = " +
    ...
    if (hashRows.rows && hashRows.rows.length > 0) {
      ...
      if (rowAtTip !== tipHash) {
        log.error("FATAL: cluster-mode audit-log rollback detected (row-hash mismatch).");
        log.error("  audit-tip counter:    " + tipCounter);
        log.error("  audit-tip rowHash:    " + tipHash);
  ...
  log("cluster audit-tip rollback check ok (tip counter " + tipCounter +
    ", current " + currentMax + ")");
}
...
function _vaultKeyFingerprint() {
  ...
  var keys = jsonSafe.parse(keysJson);
