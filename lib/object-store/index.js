...
function _err(code, message, permanent) {
  return new ObjectStoreError(code, message, permanent);
}

// Reserved entries surface a clear "not yet implemented" message rather
// than a confusing "unknown protocol" trap. All currently advertised
// protocols are bundled — this is the hook for adding deferred ones later.
...
function buildBackend(config) {
  ...
  return {
    ...
    delete:          wrap("delete"),
    list:            wrap("list"),
    // presignedUploadUrl is a sync URL-builder (no network call), so it
    // bypasses retry + circuit-breaker — propagate any throw directly.
