...
function create(config) {
  if (!config) throw _err("objectstore/bad-opt", "gcs bucketOps: config required", true);
  ...
  if (!serviceAccount && config.serviceAccountFile) {
    try {
      ...
      throw _err("objectstore/bad-opt", "gcs bucketOps: failed to read serviceAccountFile '" +
  ...
  async function _ensureToken() {
    ...
    if (!tokenResp.access_token) {
      throw _err("objectstore/auth-failed",
