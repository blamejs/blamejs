...
function canonicalHeaders(headers) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    merged[names[i]] = byName[names[i]];
  }
  // `merged` is the same representation the signature commits to, for a caller
  // that must put these headers on the wire: sending the ORIGINAL keys would
  // transmit both spellings of a duplicated name, and both node:http and the
  // HTTP/2 header builder keep only the last — so the peer would receive a
