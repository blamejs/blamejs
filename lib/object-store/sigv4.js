...
function create(config) {
  ...
  function presignedUploadUrl(opts) {
    opts = opts || {};
    if (!opts.key || typeof opts.key !== "string") {
      throw _err("INVALID_KEY", "presignedUploadUrl: key is required", true);
    }
    if (opts.key.indexOf("\0") !== -1) {
      throw _err("INVALID_KEY", "null byte in key", true);
    }
    // expiresIn: seconds. SigV4 max is 7 days (604800).
    var expiresIn = opts.expiresIn != null ? opts.expiresIn : 900;
    if (typeof expiresIn !== "number" || expiresIn < 1 || expiresIn > 604800) {
      throw _err("INVALID_EXPIRES",
        "presignedUploadUrl: expiresIn must be a number of seconds between 1 and 604800 (7 days)",
        true);
    }
    ...
    // The set order doesn't matter — canonicalQueryString sorts.
    ...
    // Canonical request: sign only "host" so the client can attach
    // Content-Type / etc. without invalidating the signature. Payload
    // hash for query-string PUT presigning is the literal string
    // "UNSIGNED-PAYLOAD" — the body is not part of the signature.
    var headers = { host: url.host };
    var canon = canonicalRequest("PUT", url, headers, "UNSIGNED-PAYLOAD");
    var sts = stringToSign(amzDate, credentialScope, canon);
