  ...
  for (var i = 0; i < segments.length; i++) {
    if (segments[i] === "." || segments[i] === "..") {
      throw _err("objectstore/invalid-key",
        "object key has a '" + segments[i] + "' path segment; such a key resolves " +
        "outside the configured bucket or container and is refused", true);
    }
  }
  return segments.map(function (s) { return awsUriEncode(s, true); }).join("/");
}
