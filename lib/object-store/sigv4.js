...
function assertSafeObjectKey(key) {
  ...
  if (key.indexOf("\0") !== -1) {
    throw _err("objectstore/invalid-key", "null byte in object key", true);
  }
  var segments = key.split("/");
  for (var i = 0; i < segments.length; i++) {
    if (segments[i] === "." || segments[i] === "..") {
      throw _err("objectstore/invalid-key",
        "object key must not contain a '" + segments[i] + "' path segment", true);
    }
  }
  return segments;
}
...
function encodeObjectKeyPath(key) {
  return assertSafeObjectKey(key).map(function (s) { return awsUriEncode(s, true); }).join("/");
}
...
function _alignWireQueryToSigV4(url) {
  ...
  if (search === "") return;
  var aligned = search.split("+").join("%20").split("*").join("%2A").split("%7E").join("~");
  if (aligned !== search) url.search = aligned;
}
...
function create(config) {
  ...
  function _keyToUrl(key) {
    var encoded = encodeObjectKeyPath(key);
    if (pathStyle) {
  ...
  function _presign(method, opts) {
    ...
    sharedRequest.assertSafeContentType(opts.contentType, "presigned URL");
  ...
  function presignedUploadPolicy(opts) {
    ...
    assertSafeObjectKey(opts.key);
    sharedRequest.assertSafeContentType(opts.contentType, "presignedUploadPolicy");
...
module.exports = {
  ...
  encodeObjectKeyPath:  encodeObjectKeyPath,
  assertSafeObjectKey:  assertSafeObjectKey,
  sha256Hex:            sha256Hex,
