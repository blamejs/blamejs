...
function _now() {
  // The Date.now fallback that used to sit here is gone with the local
  // conversion. It was unreachable on any Node this framework runs on, and it
  // was the one path that could put a wall clock behind a duration — a host
  // corrected mid-request would have reported a negative or wildly inflated
  // timing to whoever reads the header.
...
function _formatDur(ms) {
  ...
  // Drop the trailing zeros toFixed pads with, then the point they leave
  // behind. Written as two steps rather than `/\.?0+$/`, which has no start
  // anchor and so retries the zero run from every position.
  var s = codepointClass.trimTrailingChars(ms.toFixed(3), "0");
