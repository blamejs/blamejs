...
var MAX_PARSE_BYTES       = C.BYTES.mib(4);
...
function create(opts) {
  ...
  return {
    get:        function (method, actorId, key)                       { return _get(store, method, actorId, key, auditImpl, ttlMs); },
...
async function _get(store, method, actorId, key, auditImpl, ttlMs) {
  ...
  var result;
  try {
    result = safeJson.parse(row.resultBlob, { maxBytes: MAX_PARSE_BYTES });
  } catch (e) {
...
function _fingerprintArgs(args) {
  ...
  var canonical;
  try { canonical = safeJson.stringify(argsClone); }
