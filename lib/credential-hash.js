...
// v0.2.27 issued raw-hex SHA3-512 strings. We need to reverse the
// hex digest at verify time to support those rows during the rollout
// window. New rows never use this path.
function _rawSha3_512(secret) {
  return Buffer.from(crypto.sha3Hash(secret), "hex");
}
...
async function hash(secret, opts) {
  ...
  if (algoId === C.CRED_HASH_IDS.SHAKE256) {
    ...
    return _envelope(algoId, _shake256(secret, length));
  }
  if (algoId === C.CRED_HASH_IDS.ARGON2ID) {
    ...
    return _envelope(algoId, Buffer.from(phc, "utf8"));
  }
...
async function verify(secret, envelope) {
  ...
  if (typeof secret !== "string" && !Buffer.isBuffer(secret)) return false;
  var len = Buffer.isBuffer(secret) ? secret.length : Buffer.byteLength(secret, "utf8");
  ...
  if (decoded.algoId === C.CRED_HASH_IDS.SHAKE256) {
    // The payload's actual byte length tells us how many bytes to
    // recompute from the secret — that's the XOF property in action.
    var expected = _shake256(secret, decoded.payload.length);
    return crypto.timingSafeEqual(expected, decoded.payload);
  }
  if (decoded.algoId === C.CRED_HASH_IDS.ARGON2ID) {
    var phc = decoded.payload.toString("utf8");
    var plain = Buffer.isBuffer(secret) ? secret.toString("utf8") : secret;
    try { return await passwordPrimitive().verify(phc, plain); }
...
//
// Pre-envelope (v0.2.27) api-key rows stored a raw SHA3-512 hex string
// in secretHash. The api-key.verify path uses this helper to detect
// such rows and decode them as if they were envelope-format SHA3_512.
// Once a row is rehashed under the envelope it never falls back here
// again. After enough release cadence, this can be deleted; until
// then it's the no-breaking-change shim.

function isLegacyRawHexSha3_512(stored) {
  return typeof stored === "string" &&
         stored.length === 128 &&
         /^[0-9a-fA-F]+$/.test(stored);
}

async function verifyAllowingLegacy(secret, stored) {
  if (await verify(secret, stored)) return true;
  if (isLegacyRawHexSha3_512(stored)) {
    var raw = _rawSha3_512(secret).toString("hex");
    return crypto.timingSafeEqual(raw, stored);
  }
  return false;
}
...
module.exports = {
  hash:                 hash,
  verify:               verify,
  verifyAllowingLegacy: verifyAllowingLegacy,
  inspect:              inspect,
  needsRehash:          needsRehash,
  isLegacyRawHexSha3_512: isLegacyRawHexSha3_512,
  ALGOS:                ALGOS,
  DEFAULTS:             DEFAULTS,
