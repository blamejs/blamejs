 ...
 *   one `b.middleware.noCache` sets, the helpers send that value instead of
 *   the default.
 *
 * @card
...
function _writeResponse(res, status, headers, body) {
  ...
  if (typeof res.writeHead === "function") {
    res.writeHead(status, requestHelpers().mergeVary(res, headers));
  } else {
    res.statusCode = status;
    if (typeof res.setHeader === "function") {
      var merged = requestHelpers().mergeVary(res, headers);
      for (var k in merged) {
 ...
 * default steps aside for that entry and for a `Cache-Control` with `no-store`
 * already on the response, and for nothing else. Nothing is buffered: a
...
async function stream(res, iterable, opts) {
  ...
  if (res.headersSent !== true && typeof res.writeHead === "function") {
    try {
      res.writeHead(_statusOr(opts, C.HTTP.STATUS.OK), requestHelpers().mergeVary(res, headers));
    } catch (e) {
      closed.dispose();
      failBeforeStreaming(e, source);
    }
