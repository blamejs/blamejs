 ...
 *   `opts.headers["Cache-Control"]` for CDN-cacheable responses. When the
 *   response already carries a `Cache-Control` with `no-store`, such as the
 *   one `b.middleware.noCache` sets, the helpers send that value instead,
 *   both in place of the default and in place of a `Cache-Control` the
 *   caller passes in `opts.headers`. A route that must stay cacheable is
 *   kept out of the middleware's scope with its `when` predicate.
 *
 * @card
...
function _writeResponse(res, status, headers, body) {
  ...
  if (typeof res.writeHead === "function") {
    ...
    if (typeof res.setHeader === "function") {
      var merged = requestHelpers().finalizeHeaders(res, headers);
      for (var k in merged) {
        if (Object.prototype.hasOwnProperty.call(merged, k)) res.setHeader(k, merged[k]);
      }
    }
  }
 ...
 * without losing Content-Type. A `no-store` already on the response
 * survives that merge. The Content-Type it sends is its own:
...
function json(res, body, opts) {
  ...
  var headers = _mergedHeaders({
    ...
    "Cache-Control":  cdnCacheControl.keepNoStore(res, DEFAULT_DYNAMIC_CACHE_CONTROL),
  }, opts.headers);
 ...
 * An explicit `opts.headers` entry still wins over both. The Cache-Control
 * default steps aside for that entry, and a `Cache-Control` with `no-store`
 * already on the response replaces both of them. Nothing is buffered: a
...
async function stream(res, iterable, opts) {
  ...
  if (res.headersSent !== true && typeof res.writeHead === "function") {
    try {
      res.writeHead(_statusOr(opts, C.HTTP.STATUS.OK), requestHelpers().finalizeHeaders(res, headers));
    } catch (e) {
      closed.dispose();
      failBeforeStreaming(e, source);
    }
