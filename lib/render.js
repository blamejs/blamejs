...
function _defaultContentTypeUnlessStated(res, defaults) {
  ...
  // A header set to nothing states nothing. Only a non-empty string is an
  // answer the route actually gave.
...
async function stream(res, iterable, opts) {
  ...
  if (!willCommitHere) pending = null;
    ...
    try {
      ...
      // A SYNC iterable may yield a PROMISE, and `for await` resolves it before
      // handing it on — so a rejected first one is another failure before any
      // chunk was produced, and it belongs in this window rather than in the
      // loop. Resolved here, the loop sees a settled value and writes it.
      if (!pending.done && !source.isAsync &&
          pending.value && typeof pending.value.then === "function") {
        pending = {
          done: false,
