...
function json(res, body, opts) {
  ...
  _writeResponse(res, opts.status || C.HTTP.STATUS.OK, headers, encoded);
}
...
async function stream(res, iterable, opts) {
  ...
  if (res.headersSent !== true && typeof res.writeHead === "function") {
    try {
      res.writeHead(opts.status || C.HTTP.STATUS.OK, headers);
    } catch (e) {
...
function redirect(res, location, opts) {
  ...
  var status = opts.status || C.HTTP.STATUS.FOUND;
