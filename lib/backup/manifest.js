...
function _validateFileEntry(f, idx, errors) {
  ...
  } else if (f.relativePath.indexOf("..") !== -1 || /^[/\\]/.test(f.relativePath) || /^[a-zA-Z]:/.test(f.relativePath)) {
    // No traversal — when restored, relativePath joins under dataDir. A
    // Windows drive prefix (C:\...) is absolute and would escape dataDir on
    // restore (path.resolve honors it), so it is refused alongside '..' and
    // a leading separator.
    errors.push("files[" + idx + "].relativePath: must be a relative path without '..', a drive letter, or a leading separator");
  }
  if (typeof f.encryptedPath !== "string" || f.encryptedPath.length === 0) {
    errors.push("files[" + idx + "].encryptedPath: required non-empty string");
  } else if (f.encryptedPath.indexOf("..") !== -1 || /^[/\\]/.test(f.encryptedPath) || /^[a-zA-Z]:/.test(f.encryptedPath)) {
    errors.push("files[" + idx + "].encryptedPath: must be a relative path without '..', a drive letter, or a leading separator");
  }
