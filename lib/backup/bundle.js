...
async function create(opts) {
  ...
  for (var i = 0; i < opts.files.length; i++) {
    ...
    // The previous existsSync → statSync → readFileSync sequence opened
    // a swap window between checks and the actual read.
    var srcFd;
    try {
      srcFd = nodeFs.openSync(srcPath, "r");
    } catch (openErr) {
    ...
    try {
      ...
      plain = Buffer.allocUnsafe(stat.size);
      var read = 0;
      while (read < stat.size) {
        var nRead = nodeFs.readSync(srcFd, plain, read, stat.size - read, null);
        if (nRead === 0) break;
        read += nRead;
      }
      if (read !== stat.size) {
        plain = plain.slice(0, read);
      }
    } finally {
      try { nodeFs.closeSync(srcFd); } catch (_c) { /* best-effort */ }
    }
