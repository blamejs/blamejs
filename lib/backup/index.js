...
bundleAdapterStorage.objectStoreAdapter = function (client, osOpts) {
  ...
  return {
    ...
    async listKeys(keyPrefix) {
      var scoped = _scopedKey(keyPrefix || "_");          // bypass empty-key guard with placeholder
      // _scopedKey rejects empty strings; for list(prefix) we want
      // to allow listing the whole bundle root. Compose the
      // prefix manually: strip the placeholder reinstated above.
      var realScoped = prefix + (keyPrefix || "");
      void scoped;
      var result = await client.list(realScoped, listOpts);
      // b.objectStore returns { items: [{ key, ... }], truncated }
      // — items[].key is the FULL backend key (carrying the prefix).
      // Strip the prefix to surface keys relative to the adapter's
      // contract.
      var out = [];
