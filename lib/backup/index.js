...
// verifyManifestSignature — read the manifest from a bundle directory
// (or a parsed manifest object) and verify its audit-sign signature.
// Operator-facing wrapper around backupManifest.verifySignature; takes
// either a bundleDir string or a parsed bundle reference.
//
//   var ok = await b.backupBundle.verifyManifestSignature("/restore/bundle-2026-...");
//   var ok = await b.backupBundle.verifyManifestSignature({ manifest: parsedManifest });
//   var ok = await b.backupBundle.verifyManifestSignature(bundleDir, {
//     expectedFingerprint: pinnedFingerprint,
//   });
//
// Returns { ok, fingerprint?, reason? }. Throws BackupError only on
// missing-manifest / unreadable-manifest; bad-signature returns
