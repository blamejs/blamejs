 ...
 *       canReadBundle,           // optional; false when this host cannot read back
 *       async verifyStoredContent(bundleId),   // optional; { ok, reason? }
 *     }
 *
 *   `listBundles` and `hasBundle` answer two different questions, and a
 *   backend has to answer both. `listBundles` reports a bundle only when it
 *   holds what a restore needs: a `manifest.json` that parses, and every
 *   encrypted file that manifest declares present as a regular file at the
 *   declared size. Retention keeps the newest bundles a backend lists, so a
 *   bundle listed but unrestorable takes a keep slot from one that could
 *   have been restored. `hasBundle` reports whether the bundle is stored at
 *   all, whatever state it is in, so that a restore aimed at a damaged
 *   bundle can say what is wrong with it rather than that no such bundle
 *   exists. `run()` calls
 *   `readBundle` on each bundle it writes and compares the copy with what it
 *   wrote before retention deletes anything. A backend that writes to a
 *   recipient whose private key this host does not hold sets
 *   `canReadBundle: false` and offers `verifyStoredContent(bundleId)`, which
 *   compares the stored bytes with what was written without decrypting
 *   them. The result's `verifiedBy` says which check ran: `"readback"`,
 *   `"stored-content"`, or `"presence"`. Retention does not run after a
 *   `"presence"` check, since a stored key proves nothing about the bytes
 *   under it; the result carries `retentionSkipped` saying so.
 *
...
var PARTIAL_BUNDLE_PREFIX = ".partial-";
var MAX_BUNDLE_ENTRIES = 65535;
var BACKUP_STAGING_PREFIX = "blamejs-backup-staging-";
var BACKUP_READBACK_PREFIX = "blamejs-backup-readback-";
var STALE_WORKING_DIR_MS = C.TIME.hours(24);

function _removeStaleWorkingDirs(parentDir, prefix, keepBundleId) {
  atomicFile.removeStaleDirs(parentDir, {
    prefix: prefix, olderThanMs: STALE_WORKING_DIR_MS, keep: [prefix + keepBundleId],
  });
}
 ...
 *
 * A directory is reported by `listBundles` and `hasBundle` only when it
 * holds a `manifest.json` and every encrypted file that manifest names is
 * present at the size it records, so a copy interrupted partway through
 * takes no retention slot from a bundle that restores. A manifest this
 * release cannot parse is reported on the strength of its presence alone,
 * since it says nothing about which files to expect.
 *
...
function diskStorage(opts) {
  ...
  function _bundleDirExists(p) {
    try {
      if (!nodeFs.statSync(p).isDirectory()) return false;
      return nodeFs.lstatSync(nodePath.join(p, "manifest.json")).isFile();
    } catch (_e) { return false; }
  }

  function _isCompleteBundleDir(p) {
    if (!_bundleDirExists(p)) return false;
    var manifest = null;
    try { manifest = backupManifest.readFile(nodePath.join(p, "manifest.json")); }
    catch (_e2) { return false; }
    return _manifestDeclaresRestorableFiles(manifest, function (encryptedPath) {
      var blob = safePath.resolveOrNull(p, encryptedPath);
      if (blob === null) return null;
      var stat;
      try { stat = nodeFs.lstatSync(blob); } catch (_e3) { return null; }
      return { regularFile: stat.isFile(), size: stat.size };
    });
  }
  ...
  return {
    ...
    async writeBundle(bundleId, sourceDir) {
      ...
      _removeStaleWorkingDirs(root, PARTIAL_BUNDLE_PREFIX, bundleId);
      var partial = nodePath.join(root, PARTIAL_BUNDLE_PREFIX + bundleId);
      try {
        atomicFile.copyDirRecursive(sourceDir, partial);
        atomicFile.renameWithRetry(partial, dest);
      } catch (e) {
        try { nodeFs.rmSync(partial, { recursive: true, force: true }); }
        catch (_e) { /* best-effort: a stale partial is removed by a later write */ }
        throw e;
      }
      atomicFile.fsyncDir(root);
    },
    async readBundle(bundleId, destDir) {
      var src = _bundlePath(bundleId);
      if (!_bundleDirExists(src)) {
        throw new BackupError("backup/bundle-not-found",
          "readBundle: '" + bundleId + "' is not a complete bundle in storage at " + root +
          " (a bundle counts once its manifest.json is in place)");
      }
    ...
    async listBundles() {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        if (!_isCompleteBundleDir(p)) continue;
        var stat;
    ...
    async hasBundle(bundleId) {
      try { return _bundleDirExists(_bundlePath(bundleId)); }
      catch (_e) { return false; }
    },
  };
}

function _manifestDeclaresRestorableFiles(manifest, factsFor) {
  ...
  for (var i = 0; i < manifest.files.length; i += 1) {
    ...
    if (!entry || typeof entry.encryptedPath !== "string") return false;
    if (!numericBounds.isPositiveFiniteInt(entry.encryptedSize)) return false;
    var facts = factsFor(entry.encryptedPath);
    if (facts === null) return false;
    if (facts.regularFile !== true) return false;
    if (facts.size !== entry.encryptedSize) return false;
  }
  return true;
}

function _relativeFiles(rootDir) {
  var out = [];
  var stack = [""];
  while (stack.length > 0) {
    ...
    for (var i = 0; i < entries.length; i++) {
      var childRel = rel ? nodePath.join(rel, entries[i].name) : entries[i].name;
      if (entries[i].isDirectory()) stack.push(childRel);
...
var READ_BACK_CHUNK_BYTES = C.BYTES.mib(1);

function _fileBytesDiffer(expectedPath, actualPath, size) {
  var expectedFd = nodeFs.openSync(expectedPath, "r", 0o600);
  var actualFd = null;
  var expectedChunk = Buffer.allocUnsafe(READ_BACK_CHUNK_BYTES);
  var actualChunk = Buffer.allocUnsafe(READ_BACK_CHUNK_BYTES);
  try {
    actualFd = nodeFs.openSync(actualPath, "r", 0o600);
    for (var at = 0; at < size; at += READ_BACK_CHUNK_BYTES) {
      var want = Math.min(READ_BACK_CHUNK_BYTES, size - at);
      var gotExpected = _readFully(expectedFd, expectedChunk, want, at);
      var gotActual = _readFully(actualFd, actualChunk, want, at);
      if (gotExpected !== gotActual) return true;
      if (expectedChunk.compare(actualChunk, 0, gotActual, 0, gotExpected) !== 0) return true;
      if (gotExpected < want) return false;
    }
    return false;
  } finally {
    nodeFs.closeSync(expectedFd);
    if (actualFd !== null) nodeFs.closeSync(actualFd);
  }
}

function _readFully(fd, into, want, at) {
  var filled = 0;
  while (filled < want) {
    var n = nodeFs.readSync(fd, into, filled, want - filled, at + filled);
    if (n === 0) break;
    filled += n;
  }
  return filled;
...
function _firstBundleDifference(stagedDir, readBackDir) {
  var staged = _relativeFiles(stagedDir);
  var readBack = _relativeFiles(readBackDir);
  var readBackSet = new Set(readBack);
  for (var i = 0; i < staged.length; i++) {
    if (!readBackSet.has(staged[i])) return "'" + staged[i] + "' is missing from the stored bundle";
    var expectedPath = nodePath.join(stagedDir, staged[i]);
    var actualPath = nodePath.join(readBackDir, staged[i]);
    var expectedSize = nodeFs.statSync(expectedPath).size;
    var actualSize = nodeFs.statSync(actualPath).size;
    if (expectedSize !== actualSize) {
      return "'" + staged[i] + "' is " + actualSize + " bytes in storage, " + expectedSize + " bytes as written";
    }
    if (_fileBytesDiffer(expectedPath, actualPath, expectedSize)) {
      return "'" + staged[i] + "' differs from the bytes written";
    }
  }
  if (readBack.length !== staged.length) {
    var stagedSet = new Set(staged);
    var extra = readBack.filter(function (f) { return !stagedSet.has(f); });
    return "the stored bundle holds files that were not written: " + extra.slice(0, 5).join(", ");
  }
  return null;
}

function _readManifestAt(bundleDir, noManifestCode, badManifestCode, who) {
  var manifestPath = nodePath.join(bundleDir, "manifest.json");
  try {
    return backupManifest.readFile(manifestPath, {
      errorFor: function (kind, detail) {
        if (kind === "enoent") return new BackupError(noManifestCode, who + ": manifest.json missing at " + manifestPath);
        if (kind === "too-large") {
          return new BackupError(badManifestCode, who + ": manifest.json is " + detail.size +
            " bytes, above the " + detail.max + "-byte limit");
        }
        return new BackupError(badManifestCode, who + ": manifest.json unreadable: " + kind);
      },
    });
  } catch (e) {
    if (e && e.isBackupManifestError) {
      throw new BackupError(badManifestCode, who + ": manifest.json is not a valid backup manifest: " + e.message);
    }
    throw e;
  }
}
 ...
 * `storage.writeBundle`, and reads the bundle back through
 * `storage.readBundle`. When the copy read back differs from the bundle
 * written, `run()` deletes that bundle from storage, skips retention and
 * rejects with `backup/verify-after-write-failed`. Otherwise it applies
 * retention: `keep` counts the complete bundles whose id sorts at or
 * before the one this run wrote, and deletes the rest of those. Audit
 * events `backup.success` / `backup.failure` / `backup.retention.swept`
 * land on `b.audit` when `opts.audit !== false`.
 *
 * A storage backend's `listBundles` and `hasBundle` report a bundle only
 * once it is complete: `diskStorage` copies into `.partial-<bundleId>`
 * and renames it into place, and every listing requires `manifest.json`.
 * A manifest larger than `b.backupManifest.MAX_MANIFEST_BYTES` (64 MiB)
 * is refused with `backup-manifest/too-large` before it reaches storage.
 *
 ...
 *   passphrase:        Buffer | string,              // required; one Argon2id bundle key, HKDF-SHA3-512 subkey per file
 *   files:             Array<{ relativePath, kind, required }>,
 *   vaultKeyJson:      string | () => string | Promise<string>,
 *   retention:         { keep: number },             // optional; keep is a positive integer; sweep older bundles after run()
...
function create(opts) {
  ...
  var retention = opts.retention === undefined || opts.retention === null ? null : opts.retention;
  if (retention !== null && typeof retention !== "object") {
    throw new BackupError("backup/bad-retention",
      "create: opts.retention must be { keep: <positive integer> }; got " + typeof retention);
  }
  if (retention !== null) {
    numericBounds.requirePositiveFiniteInt(retention.keep,
      "create: opts.retention.keep", BackupError, "backup/bad-retention");
  }
  if (retention !== null) retention = { keep: retention.keep };
  ...
  async function run(runOpts) {
    ...
    var stagingDir = nodePath.join(os.tmpdir(), BACKUP_STAGING_PREFIX + bundleId);
    var readBackDir = nodePath.join(os.tmpdir(), BACKUP_READBACK_PREFIX + bundleId);
    _removeStaleWorkingDirs(os.tmpdir(), BACKUP_STAGING_PREFIX, bundleId);
    _removeStaleWorkingDirs(os.tmpdir(), BACKUP_READBACK_PREFIX, bundleId);
    ...
    var beat = atomicFile.heartbeat(stagingDir);
    try {
    ...
    var difference;
    var readBackKind = "readback";
    if (storage.canReadBundle === false) {
      readBackKind = typeof storage.verifyStoredContent === "function" ? "stored-content" : "presence";
    }
    if (readBackKind === "stored-content") {
      try {
        var verdict = await storage.verifyStoredContent(bundleId);
        difference = verdict && verdict.ok === true ? null
          : "storage.verifyStoredContent: " + ((verdict && verdict.reason) || "no verdict");
      } catch (e) {
        difference = "storage.verifyStoredContent failed: " + ((e && e.message) || String(e));
      }
    } else if (readBackKind === "presence") {
      try {
        var stored = await storage.hasBundle(bundleId);
        difference = stored === true ? null
          : "storage does not report the bundle it just wrote as stored";
      } catch (e) {
        difference = "storage.hasBundle failed: " + ((e && e.message) || String(e));
      }
    } else {
      var readBeat = atomicFile.heartbeat(readBackDir);
      try {
        await storage.readBundle(bundleId, readBackDir);
        difference = _firstBundleDifference(stagingDir, readBackDir);
      } catch (e) {
        difference = "storage.readBundle failed: " + ((e && e.message) || String(e));
      } finally {
        readBeat.stop();
      }
    }
    beat.stop();
    try { nodeFs.rmSync(readBackDir, { recursive: true, force: true }); } catch (_e) { /* best-effort tmpdir cleanup */ }
    try { nodeFs.rmSync(stagingDir, { recursive: true, force: true }); } catch (_e) { /* best-effort tmpdir cleanup */ }
    if (difference !== null) {
      var removedFromStorage = true;
      try { await storage.deleteBundle(bundleId); } catch (_e) { removedFromStorage = false; }
      _emitAudit("backup.failure", {
        bundleId: bundleId, reason: "read-back after write: " + difference, removedFromStorage: removedFromStorage,
      }, "failure");
      throw new BackupError("backup/verify-after-write-failed",
        "the bundle read back from storage does not match the bundle written: " + difference +
        (removedFromStorage ? "; the bundle was deleted from storage" : "; deleting the bundle from storage also failed") +
        ", and retention did not run");
    }
    ...
    var summary = {
      ...
      verifiedBy: readBackKind,
    };
    _emitAudit("backup.success", summary);

    if (retention !== null && readBackKind === "presence") {
      summary.retentionSkipped = "the storage reported only that the bundle exists, which does " +
        "not prove its contents are intact; deleting an older bundle on that basis could leave " +
        "only a corrupt one. Give the storage a verifyStoredContent(bundleId) or a readBundle " +
        "this host can decrypt, or purge with b.backup.purgeOlder once a restore test has passed";
      _emitAudit("backup.retention.skipped",
        { bundleId: bundleId, reason: summary.retentionSkipped }, "warning");
    } else if (retention !== null) {
      try {
        var purged = await _purge(retention.keep, bundleId, true);
  ...
  async function purgeOlder(purgeOpts) {
    ...
    return _purge(keep, atomicFile.pathTimestamp() + "-ffffffff");
  }

  async function _purge(keep, ceilingBundleId, requireCeilingListed) {
    var bundles = await storage.listBundles();
    var eligible = bundles.filter(function (entry) {
      return entry && typeof entry.bundleId === "string" && entry.bundleId <= ceilingBundleId;
    }).sort(function (a, b) { return a.bundleId < b.bundleId ? 1 : -1; });
    if (requireCeilingListed && (eligible.length === 0 || eligible[0].bundleId !== ceilingBundleId)) {
      throw new BackupError("backup/retention-current-not-listed",
        "retention: storage.listBundles() does not list the bundle this run wrote (" + ceilingBundleId +
        "), so no bundle was deleted");
    }
    var toDelete = eligible.slice(keep);
    var deleted = [];
  ...
  function scheduleTest(testOpts) {
    ...
    schedInstance.schedule({
      ...
      run:      async function () {
        ...
        var manifest, sigVerification;
        try {
          await storage.readBundle(bundleId, stagingDir);
          manifest = _readManifestAt(stagingDir, "backup/test-no-manifest", "backup/test-bad-manifest",
            "scheduleTest bundle " + bundleId);
 ...
 *
 * The signing key must match `opts.expectedFingerprint`, or be the
 * active or a rotated `b.auditSign` key. A signature that verifies under
 * any other key returns `{ ok: false }`. A process that has not run
 * `b.auditSign.init()` passes `expectedFingerprint`.
 *
 * @opts
 *   expectedFingerprint: string,   // trust only the key with this SHA3-512 fingerprint
 *
 * @example
 *   var fs   = require("node:fs");
 *   var path = require("node:path");
...
function verifyManifestSignature(target, opts) {
  ...
  if (typeof target === "string") {
    manifest = _readManifestAt(target, "backup/no-manifest", "backup/bad-manifest", "verifyManifestSignature");
...
module.exports = {
  ...
  _firstBundleDifferenceForTest: _firstBundleDifference,
};
 ...
 *
 * `maxBundleBytes` bounds the archive this storage writes and reads back (8
 * GiB by default), the tar framing included: each entry costs a 512-byte
 * header and padding to the next 512-byte boundary, so a bundle of many
 * small files spends more on framing than on payload. `writeBundle` refuses
 * `backup/bundle-too-large` for the built archive, not only for the sum of
 * the file sizes, so a bundle this storage accepted is one it can read.
 *
 * @opts
...
function bundleAdapterStorage(opts) {
  ...
  var _tracksWrittenDigests = cryptoStrategy === "recipient" &&
    !archiveLazy().canUnwrapFor(recipient);
  var _writtenDigests = boundedMap.boundedMap({ maxEntries: 64, policy: "evict-oldest" });

  async function verifyStoredContent(bundleId) {
    var written = _writtenDigests.get(bundleId);
    _writtenDigests.delete(bundleId);
    if (written === undefined) {
      return { ok: false, reason: "this process did not write bundle '" + bundleId + "'" };
    }
    var stored;
    try { stored = await adapter.readFile(written.key); }
    catch (e) {
      return { ok: false, reason: "reading '" + written.key + "' back failed: " +
        ((e && e.message) || String(e)) };
    }
    if (!Buffer.isBuffer(stored) && !(stored instanceof Uint8Array)) {
      return { ok: false, reason: "'" + written.key + "' read back as " + typeof stored };
    }
    if (stored.length !== written.bytes) {
      return { ok: false, reason: "'" + written.key + "' is " + stored.length +
        " bytes in storage, " + written.bytes + " bytes were written" };
    }
    if (bCrypto.sha3Hash(Buffer.from(stored)) !== written.digest) {
      return { ok: false, reason: "'" + written.key + "' does not hash to the bytes written" };
    }
    return { ok: true };
  }

  function _readBackBombPolicy() {
    return {
      maxEntries:                MAX_BUNDLE_ENTRIES,
      maxEntryDecompressedBytes: maxBundleBytes,
      maxTotalDecompressedBytes: maxBundleBytes,
    };
  }
  ...
  function _manifestLast(paths, manifestPath) {
    var rest = paths.filter(function (p) { return p !== manifestPath; });
    return rest.length === paths.length ? rest : rest.concat([manifestPath]);
  }

  async function _deleteKeysBestEffort(bundleId) {
    try {
      var written = await adapter.listKeys(bundleId + "/");
      for (var i = 0; i < written.length; i += 1) {
        try { await adapter.deleteKey(written[i]); } catch (_e) { /* best-effort: an unlisted bundle holds no retention slot */ }
      }
    } catch (_e) { /* best-effort: an unlisted bundle holds no retention slot */ }
  }
  ...
  return {
    name: "adapter",
    canReadBundle: cryptoStrategy !== "recipient" ||
      archiveLazy().canUnwrapFor(recipient),
    verifyStoredContent: verifyStoredContent,
    async writeBundle(bundleId, sourceDir) {
      ...
      if (format === "tar" || format === "tar.gz") {
        ...
        if (relPaths.length > MAX_BUNDLE_ENTRIES) {
          throw new BackupError("backup/too-many-files",
            "writeBundle: the bundle holds " + relPaths.length + " entries, more than the " +
            MAX_BUNDLE_ENTRIES + " a tar bundle carries; split the source tree across " +
            "multiple bundles");
        }
        var tarBuffer = t.toBuffer();
        var framedBytes = tarBuffer.length;
        if (framedBytes > maxBundleBytes) {
          throw new BackupError("backup/bundle-too-large",
            "writeBundle: the tar carrying " + relPaths.length + " entries is " + framedBytes +
            " bytes, which exceeds maxBundleBytes=" + maxBundleBytes + ". Each entry costs a " +
            "512-byte header and padding to the next 512-byte boundary, so many small files " +
            "cost more framing than payload; split the source tree across multiple bundles " +
            "or raise the cap");
        }
        var keySuffix = format === "tar.gz" ? TAR_GZ_KEY_SUFFIX : TAR_KEY_SUFFIX;
        var payloadBytes = format === "tar.gz"
          ? archiveLazy().gz(tarBuffer).toBuffer()
          : tarBuffer;
        ...
        if (_tracksWrittenDigests) {
          _writtenDigests.set(bundleId, {
            key:    bundleId + keySuffix,
            digest: bCrypto.sha3Hash(payloadBytes),
            bytes:  payloadBytes.length,
          });
        }
        return;
      }
      var dirRelPaths = _manifestLast(_walkDirSync(sourceDir, []), "manifest.json");
      try {
        ...
        await _deleteKeysBestEffort(bundleId);
        throw e;
      }
    },
    async readBundle(bundleId, destDir) {
      ...
      if (hasTarGz) {
        ...
        var tarReader = gzReader.asTar({ bombPolicy: _readBackBombPolicy() });
      ...
      if (hasTar) {
        ...
        var reader = archiveLazy().read.tar(archiveAdaptersLazy().buffer(tarBytes),
          { bombPolicy: _readBackBombPolicy() });
    ...
    async listBundles(listOpts) {
      ...
      var presentKeys = new Set(allKeys);
      async function _directoryBundleIsComplete(bundleId) {
        if (typeof adapter.statKey !== "function") return true;
        var manifestKey = bundleId + "/manifest.json";
        var manifestStat = await adapter.statKey(manifestKey);
        if (!manifestStat || typeof manifestStat.size !== "number") return false;
        if (manifestStat.size > backupManifest.MAX_MANIFEST_BYTES) return false;
        var manifest;
        try {
          manifest = backupManifest.readBuffer(await adapter.readFile(manifestKey));
        } catch (_e) { return false; }
        if (!manifest || !Array.isArray(manifest.files)) return false;
        var sizes = new Map();
        for (var fi = 0; fi < manifest.files.length; fi += 1) {
          var declared = manifest.files[fi];
          if (!declared || typeof declared.encryptedPath !== "string") return false;
          var blobKey = bundleId + "/" + declared.encryptedPath;
          if (!presentKeys.has(blobKey)) return false;
          var seen = await adapter.statKey(blobKey);
          if (!seen || typeof seen.size !== "number") return false;
          sizes.set(declared.encryptedPath, seen.size);
        }
        return _manifestDeclaresRestorableFiles(manifest, function (encryptedPath) {
          if (!sizes.has(encryptedPath)) return null;
          return { regularFile: true, size: sizes.get(encryptedPath) };
        });
      }
      ...
      for (var i = 0; i < allKeys.length; i += 1) {
        ...
        var stats = boundedMap.getOrInsert(byBundle, bid, function () {
          return { count: 0, hasTar: false, hasTarGz: false, hasManifest: false };
        });
        stats.count += 1;
        var rest = key.slice(slash + 1);
        if (rest === "bundle.tar")         stats.hasTar = true;
        else if (rest === "bundle.tar.gz") stats.hasTarGz = true;
        else if (rest === "manifest.json") stats.hasManifest = true;
      }
      var out = [];
      ...
      for (var j = 0; j < entries.length; j += 1) {
        ...
        if (!statsJ.hasTar && !statsJ.hasTarGz && !statsJ.hasManifest) continue;
        var fmtJ;
        if (statsJ.hasTarGz)    fmtJ = "tar.gz";
        else if (statsJ.hasTar) fmtJ = "tar";
        else                    fmtJ = "directory";
        if (fmtJ === "directory" && !(await _directoryBundleIsComplete(bidJ))) continue;
        var entry = {
          bundleId:  bidJ,
          format:    fmtJ,
          createdAt: null,
    ...
    async verifyBundle(bundleId, vOpts) {
      ...
      try {
        ...
        if (info.format === "tar.gz") {
          ...
          tarReader = gzReader.asTar({ bombPolicy: _readBackBombPolicy() });
        } else {
    ...
    async cloneBundle(srcBundleId, dstBundleId, cloneOpts) {
      ...
      if (info.format === "tar" || info.format === "tar.gz") {
        ...
        var srcKeys = _manifestLast(await adapter.listKeys(srcBundleId + "/"), srcBundleId + "/manifest.json");
