...
bundleAdapterStorage.fsAdapter = function (fsOpts) {
  ...
  function _keyPath(key) {
    // Resolve the key THROUGH root with safePath — defense in depth even though
    // the storage layer also checks. safePath refuses traversal, an absolute /
    // drive-letter / UNC prefix, an NTFS alternate-data-stream marker, a
    // Windows reserved name, and NUL / control bytes, and contains the result
    // under root. A hand-rolled '..' + NUL check (the prior shape) missed the
    // drive-letter / ADS / reserved-name class the shared primitive handles.
    var resolved = safePath.resolveOrNull(root, key);
    if (resolved === null) {
      throw new BackupError("backup/bad-key",
