  ...
  return {
    ...
    async listBundles() {
      ...
      for (var i = 0; i < allKeys.length; i += 1) {
        ...
        if (!stats) {
          stats = { count: 0, format: null };
          byBundle.set(bid, stats);
        }
        stats.count += 1;
        var rest = key.slice(slash + 1);
        if (rest === "bundle.tar")        stats.format = "tar";
        else if (rest === "bundle.tar.gz") stats.format = "tar.gz";
        else if (stats.format === null)    stats.format = "directory";
      }
      var out = [];
    ...
    async bundleInfo(bundleId) {
      ...
      var sizeBytes = null;
      if (payloadKey !== null) {
        var payload = await adapter.readFile(payloadKey);
        sizeBytes = payload.length;
        envelopeKind = archiveLazy().sniffEnvelope(payload);
      }
      return {
        bundleId:     bundleId,
        format:       fmt,
