...
function bundleAdapterStorage(opts) {
  ...
  return {
    ...
    async verifyAllBundles(vOpts) {
      vOpts = vOpts || {};
      var concurrency = typeof vOpts.concurrency === "number" && vOpts.concurrency > 0
        ? Math.floor(vOpts.concurrency) : 4;                                          // allow:raw-byte-literal — default fan-out, not byte count
