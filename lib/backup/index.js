...
function _firstBundleDifference(stagedDir, readBackDir) {
  ...
  for (var i = 0; i < staged.length; i++) {
    ...
    var expected = nodeFs.readFileSync(nodePath.join(stagedDir, staged[i]));
    var actual = nodeFs.readFileSync(nodePath.join(readBackDir, staged[i]));
    if (expected.length !== actual.length) {
      return "'" + staged[i] + "' is " + actual.length + " bytes in storage, " + expected.length + " bytes as written";
    }
    if (!expected.equals(actual)) return "'" + staged[i] + "' differs from the bytes written";
  }
