...
function diskStorage(opts) {
  ...
  function _isCompleteBundleDir(p) {
    try {
      ...
      if (!nodeFs.statSync(nodePath.join(p, "manifest.json")).isFile()) return false;
    } catch (_e) { return false; }
    var manifest = null;
    try { manifest = backupManifest.readFile(nodePath.join(p, "manifest.json")); }
    catch (_e2) { manifest = null; }
    if (manifest === null || !Array.isArray(manifest.files)) return true;
    for (var i = 0; i < manifest.files.length; i += 1) {
      var entry = manifest.files[i];
      if (!entry || typeof entry.encryptedPath !== "string") return false;
      var blob = safePath.resolveOrNull(p, entry.encryptedPath);
      if (blob === null) return false;
      var stat;
      try { stat = nodeFs.statSync(blob); } catch (_e2) { return false; }
      if (!stat.isFile()) return false;
      if (numericBounds.isPositiveFiniteInt(entry.encryptedSize) && stat.size !== entry.encryptedSize) {
        return false;
      }
    }
    return true;
  ...
  return {
    ...
    async hasBundle(bundleId) {
      try { return _isCompleteBundleDir(_bundlePath(bundleId)); }
      catch (_e) { return false; }
    },
  };
}
