 ...
 *   `canReadBundle: false`; `run()` then confirms the bundle is stored
 *   instead of reading it back. The result's `verifiedBy` says which check
 *   ran, `"readback"` or `"presence"`.
 *
...
function create(opts) {
  ...
  async function run(runOpts) {
    ...
    var difference;
    var readBackKind = storage.canReadBundle === false ? "presence" : "readback";
    if (readBackKind === "presence") {
      try {
