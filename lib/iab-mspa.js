...
// The header's trailing section-ID list: the text after the first `.` that has
// only digits and dots after it and a `[A-Za-z0-9_-]` character before it.
//
// Written as two scans rather than the equivalent
// `/[A-Za-z0-9_-]+\.([0-9.]+)$/`, which has no start anchor and a prefix class
// that overlaps the digits the tail wants. On a header of digits with no dot,
// that pattern retried from every position and took time proportional to the
// SQUARE of the length: 25ms at the 8192-char cap, for a value that arrives on
// the request. The two forms select the same text.
function _numericTail(s) {
  var len = s.length;
  if (len < 3) return null;
  // Back over the trailing run of digits and dots.
  var i = len;
  while (i > 0) {
    var c = s.charCodeAt(i - 1);
    if (c === 46 || (c >= 48 && c <= 57)) i -= 1; else break;
  }
  // The first dot in that run whose preceding character can end the token.
