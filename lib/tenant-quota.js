...
function create(opts) {
  ...
  async function _computeBytesUsed(tenantId) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      // sealed columns sails under the cap. quoteIdentifier guards the table
      // and field names against injection; allowReserved matches the query
      // builder (db.from() quotes with it too) — quoting is exactly what makes
      // a reserved-word table or column name like `order` / `user` safe, so a
      // schema-valid identifier that works through b.db.from() must not be
      // rejected here. The tenant value is a bound parameter.
      var qi = function (name) { return safeSql.quoteIdentifier(name, "sqlite", { allowReserved: true }); };
      var sql = "SELECT * FROM " + qi(tables[i]) +
        " WHERE " + qi(tenantField) + " = ?";
      var rows = db.prepare(sql).all(tenantId);
