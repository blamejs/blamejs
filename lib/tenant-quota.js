...
function create(opts) {
  ...
  // quoteIdentifier guards table/field names against injection; allowReserved
  // matches the query builder (db.from() quotes with it too) — quoting is
  // exactly what makes a reserved-word table or column name like `order` /
  // `from` safe, so a schema-valid identifier that works through b.db.from()
  // must not be rejected here.
  function _qIdent(name) { return safeSql.quoteIdentifier(name, "sqlite", { allowReserved: true }); }
  ...
  async function _computeBytesUsed(tenantId) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      // Read the RAW stored rows via a prepared statement — NOT
      // db.from().all(), which routes every row through the ORM's
      // cryptoField.unsealRow and hands back DECRYPTED plaintext for sealed
      // columns. A storage cap must count what is actually on disk: the
      // (much larger) vault envelope of a sealed column, not the small
      // plaintext it unseals to — otherwise a tenant whose data lives in
      // sealed columns sails under the cap. The tenant value(s) are bound
      // parameters.
      var placeholders = whereVals.map(function () { return "?"; }).join(", ");
      var sql = "SELECT * FROM " + _qIdent(table) +
        " WHERE " + _qIdent(whereField) + " IN (" + placeholders + ")";
      var stmt = db.prepare(sql);
      var rows = stmt.all.apply(stmt, whereVals);
