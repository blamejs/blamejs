"use strict";
/**
 * tenant-quota — per-tenant DB storage caps + query budget + isolation
 * breach detection.
 *
 * Multi-tenant deployments need three things the framework's DB layer
 * doesn't natively provide:
 *
 *   1. Storage caps  — refuse INSERT when a tenant has consumed more
 *                      than its allowance (defaultBytesCap, or
 *                      perTenantBytesCap[tenantId] override).
 *   2. Query budgets — refuse SELECT when a tenant exceeds its rolling-
 *                      window QPS or rows-read totals.
 *   3. Isolation     — every row a query reads under a claimed tenantId
 *                      MUST belong to that tenant. Cross-tenant rows
 *                      surface as `db.tenant.crossover` events.
 *
 * Replaces the global `maxRowsPerQuery` knob for tenant-scoped scenarios
 * — operators were previously forced to pick one global cap that would
 * starve large tenants or under-cap small ones.
 *
 ...
 *   });
 *
 *   await quota.assert("tenant-acme");      // throws if over cap
 *   var snap = await quota.snapshot("tenant-acme");
 *     // → { tenantId, bytesUsed, bytesCap, percent }
 *   var all = await quota.list();
 *     // → [{ tenantId, bytesUsed, bytesCap, percent }, ...]
 *
 ...
 *     tenantField:              "tenantId",
 *     perTenantQpsCap:          100,
 *     perTenantTotalRowsRead:   b.constants.ROWS.thousand(50),
 *     window:                   b.constants.TIME.minutes(1),
 *   });
 *   budget.observe("tenant-acme", { rowsRead: 12 });   // throws on exceed
 *
 * Audit emissions:
 *   tenant.quota.exceeded   — assert() refused an insert/update
 *   tenant.budget.exceeded  — observe() refused a query
 *   db.tenant.crossover     — instrumentQuery saw rows belonging to the
 *                             wrong tenant under operator-claimed tid
 *
 * SOC 2 CC6.1 ("logical access controls") + ISO 27001 A.8.1.5
 * ("classification of information") map directly onto this primitive —
 * operators wire its emissions into the same audit chain auditors read.
 *
 * Storage-cap accounting:
 *   bytesUsed is computed by SUM(LENGTH(serialized-row)) across every
 *   table where the operator's schema declares the configured
 *   tenantField. The framework caches the per-tenant total for
 *   `cacheTtlMs` (default 30s) so a hot path doesn't pay the SUM on
 *   every assert.
 *
 * Query budget:
 *   Sliding-window counter keyed (tenantId, windowStart). Window size
 *   defaults to 60s. observe() rejects when either the QPS-equivalent
 *   call count exceeds perTenantQpsCap*window OR the rows-read total
 *   exceeds perTenantTotalRowsRead.
 */
