...
function create(opts) {
  ...
  async function _computeBytesUsed(tenantId) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      // No .select() → the query builder defaults to SELECT * (a literal
      // "*" is rejected by the column-name validator). We need every
      // column to sum the row's textual bytes, so take the full row.
      ...
      for (var r = 0; r < rows.length; r++) {
        ...
        for (var k = 0; k < keys.length; k++) {
          ...
          // that would refuse inserts well below the real storage cap.
