...
function trimTrailingChars(text, chars) {
  ...
  // Membership is by CODEPOINT, decided once per codepoint of `chars` rather
  // than once per character of `text`.
  //
  // Both halves of that matter. Asking `chars.indexOf` per text character
  // reads the whole set each time, which is quadratic when the set is long
  // and the text ends in a long run that matches it. And a set built from
  // UTF-16 code units holds the surrogate halves of an astral character
  // separately, so a DIFFERENT astral character in the text that happens to
  // share one half would be cut in two and leave a lone surrogate behind.
...
function firstDelimited(text, open, close, from) {
  ...
  // Identical delimiters have no unambiguous grouping: reading forward, the
  // closing character of one group is the opening character of the next,
  // while reading backward it is the other way round, so the two helpers
  // would disagree about the same text. Refused rather than answered
  ...
  for (;;) {
    ...
    i = start + 1;                       // empty group: the body needs one
  }
}
...
function lastDelimited(text, open, close) {
  if (typeof text !== "string") return null;
  // Single characters only. Walking back over a multi-character delimiter
  // would have to re-derive where each one begins, and no caller needs it.
  if (typeof open !== "string" || open.length !== 1) return null;
  if (typeof close !== "string" || close.length !== 1) return null;
  if (open === close) return null;       // no unambiguous grouping
  ...
  for (var i = end - 1; i >= 0; i -= 1) {
    var c = text.charAt(i);
    if (c === close) break;              // the body cannot contain one
    if (c === open) start = i;           // a greedy body keeps the leftmost
  }
