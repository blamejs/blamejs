 ...
 * must be an ampersand. Returns `{ text, next }`: the decoded text and the
 * index just after the reference. Returns `null` when no numeric or named
 * reference begins there. A caller walking a string one position at a time
 * gets the same decoding `decodeMarkupEntities` applies to the whole of it.
 *
 * @example
 ...
 * @since     0.20.0
 * @status    stable
 * @related   b.codepointClass.decodeEntityAt, b.codepointClass.decodeMarkupEntities
 *
 * Decode the one named entity reference that begins at index `at` of `s`,
 * which must be an ampersand, and nothing else: a numeric reference at `at`
 * returns `null`. Returns `{ text, next }` or `null`. `decodeMarkupEntities`
 * decodes numeric references in one pass and named references in a second
 * pass over the result, so the second pass sees a name whose letters, or
 * whose closing semicolon, were written as numeric references: `&co&#108;on;`
 * is `&colon;` to it. This reads the name the same way, so a caller walking
 * by position recognizes exactly the references the whole-string decoder
 * does. A numeric reference that produces an ampersand can likewise complete
 * a named reference with the text after it; the caller reproduces that by
 * decoding the numeric reference, then offering the ampersand it produced
 * plus the following text to this.
 *
 * @example
...
function decodeNamedEntityAt(s, at) {
  ...
  if (!Object.prototype.hasOwnProperty.call(NAMED_ENTITY_ASCII, name)) {
    return { text: text.slice(at, end), next: end };
  }
