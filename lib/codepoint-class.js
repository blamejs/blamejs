 ...
 * policy of its own.
 *
 * Every class scans on the same terms, and `severity` follows the resolved
 * policy — `reject` reports `critical`, `audit` reports `warn`. A caller
 * cannot ask for a class to be skipped or stamp a severity of its own: a
 * finding that refuses is the operator's `reject` and nothing else.
 *
