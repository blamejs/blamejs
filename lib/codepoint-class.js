 ...
 * @signature b.codepointClass.firstInRanges(text, ranges)
 * @since     0.18.29
 * @status    stable
 * @related   b.codepointClass.inRanges, b.codepointClass.stripRanges
 *
 * Index of the first codepoint of `text` that falls in `ranges`, or `-1`. The
 * index is a UTF-16 code-unit offset, matching a JS string index.
 *
 ...
 */
function firstInRanges(text, ranges) {
...
function indexOfAny(text, chars, from) {
  ...
  for (var i = from > 0 ? from : 0; i < text.length; i += 1) {
    if (chars.indexOf(text.charAt(i)) !== -1) return i;
  }
  return -1;
}
...
function replaceAny(text, chars, replacement) {
  ...
  for (var i = 0; i < text.length; i += 1) {
    if (chars.indexOf(text.charAt(i)) === -1) continue;
    out += text.slice(keepFrom, i) + replacement;
    keepFrom = i + 1;
  }
  return keepFrom === 0 ? text : out + text.slice(keepFrom);
...
function containsFolded(haystack, needle) {
  ...
  for (var start = 0; start <= limit; start += 1) {
    ...
    for (var j = 0; j < needle.length; j += 1) {
      if (_foldAscii(haystack.charCodeAt(start + j)) !== _foldAscii(needle.charCodeAt(j))) {
        hit = false;
        break;
      }
    }
