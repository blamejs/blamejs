...
function trimTrailingChars(text, chars) {
  ...
  // Membership is decided once per character of `chars`, not once per
  // character of `text`. Asking `chars.indexOf` per text character reads the
  // whole set each time, which is quadratic when a caller passes a long set
  // and the text ends in a long run that matches it.
  var set = Object.create(null);
  for (var c = 0; c < chars.length; c += 1) set[chars.charAt(c)] = true;
  while (end > 0 && set[text.charAt(end - 1)] === true) end -= 1;
  return end === text.length ? text : text.slice(0, end);
}
