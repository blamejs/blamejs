...
function trimTrailingChars(text, chars) {
  ...
  var end = text.length;
  while (end > 0 && chars.indexOf(text.charAt(end - 1)) !== -1) end -= 1;
 ...
 * index of `close`. Returns `null` when there is none. An empty group is
 * skipped and the search continues, which is what `[^close]+` requiring one
 * character does.
 *
...
function firstDelimited(text, open, close, from) {
  ...
  // An empty delimiter has no position to find: indexOf would answer with the
  // cursor itself and the search below would never advance.
  if (typeof open !== "string" || open.length === 0) return null;
  if (typeof close !== "string" || close.length === 0) return null;
  ...
  for (;;) {
    ...
    if (end > bodyStart) return { body: text.slice(bodyStart, end), start: start, end: end };
