 ...
 * @since     0.18.29
 * @status    stable
 * @related   b.codepointClass.indexOfAny, b.codepointClass.ASCII_ALNUM
 *
...
function isRunOf(text, chars, min, max) {
  if (typeof text !== "string" || typeof chars !== "string") return false;
  var lo = typeof min === "number" ? min : 1;
  if (text.length < lo) return false;
  if (typeof max === "number" && text.length > max) return false;
  for (var i = 0; i < text.length; i += 1) {
    if (chars.indexOf(text.charAt(i)) === -1) return false;
  }
  return true;
}
