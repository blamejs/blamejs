...
function detectCharThreats(text, opts, codePrefix, zeroWidthSeverity) {
    ...
    if (zwMatch) {
      issues.push({
        ...
        snippet: "zero-width / invisible-formatting char U+" +
                 zwMatch[0].charCodeAt(0).toString(HEX_RADIX) + " at offset " + zwMatch.index,
      });
    }
  }
  ...
  //
  // Severity matches bidi rather than zero-width: a Tags character carries an
  // invisible copy of ASCII text, so it is a channel for content that renders
  // as nothing, not merely a spacing artifact.
  if (_tagsPolicy(opts) && _tagsPolicy(opts) !== "allow") {
    var tagMatch = text.match(TAG_RE);
    if (tagMatch) {
      issues.push({
        kind: "unicode-tags", severity: "critical",
        ruleId: codePrefix + ".unicode-tags",
        location: tagMatch.index,
        snippet: "Unicode Tags block char U+" +
                 tagMatch[0].codePointAt(0).toString(HEX_RADIX).toUpperCase() +
...
function assertNoCharThreats(text, opts, errorFactory, codePrefix) {
  ...
  if (opts && opts.bidiPolicy === "reject" && BIDI_RE.test(text)) {           // allow:regex-no-length-cap — caller bounds length via assertWithinMaxBytes
  ...
  if (opts && opts.zeroWidthPolicy === "reject" && ZERO_WIDTH_RE.test(text)) { // allow:regex-no-length-cap — caller bounds length before invoking
    throw errorFactory(codePrefix + ".zero-width",
      "input contains zero-width / invisible-formatting character");
  }
  if (_tagsPolicy(opts) === "reject" && TAG_RE.test(text)) {                  // allow:regex-no-length-cap — caller bounds length before invoking
...
function applyCharStripPolicies(text, opts) {
  ...
  if (_tagsPolicy(opts) === "strip")            out = out.replace(TAG_RE_G, "");
  return out;
}
