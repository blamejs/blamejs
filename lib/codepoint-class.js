 ...
 *
 *   C1_CTRL_RANGES: the C1 control characters U+0080-U+009F. U+009B
 *     is the one-character form of ESC [ and U+0085 NEXT LINE is a line
 *     break under Unicode line breaking.
 *
 *   CTRL_RANGES: C0_CTRL_RANGES, DEL (U+007F) and C1_CTRL_RANGES. The
 *     text guards read this table for `controlPolicy`.
 *
...
var C1_CTRL_RANGES    = [[0x0080, 0x009F]];
var CTRL_RANGES       = C0_CTRL_RANGES.concat([0x007F], C1_CTRL_RANGES);
...
function assertNoCharThreats(text, opts, errorFactory, codePrefix) {
  ...
  if (opts && opts.controlPolicy === "reject" && firstInRanges(text, CTRL_RANGES) !== -1) {
    throw errorFactory(codePrefix + ".control",
      "input contains a control character");
  }
...
/**
 * @primitive b.codepointClass.isAtext
 * @signature b.codepointClass.isAtext(cc)
 * @since     0.20.32
 * @status    stable
 * @related   b.codepointClass.isDotAtomText, b.codepointClass.isAsciiAlnum
 *
 * Is this code unit RFC 5322 section 3.2.3 `atext`: an ASCII letter, an
 * ASCII digit, or one of ``!#$%&'*+-/=?^_`{|}~``? The dot is not atext;
 * it joins atoms, which is what `isDotAtomText` reads.
 *
 * @example
 *   b.codepointClass.isAtext("+".charCodeAt(0));                     // → true
 */
function isAtext(cc) {
...
/**
 * @primitive b.codepointClass.isDotAtomText
 * @signature b.codepointClass.isDotAtomText(text)
 * @since     0.20.32
 * @status    stable
 * @related   b.codepointClass.isAtext, b.guardEmail.validate
 *
 * Is this string RFC 5322 section 3.2.3 `dot-atom-text`: one or more
 * atoms of `atext` joined by single dots? An empty string, a leading or
 * trailing dot, a doubled dot, and any character outside `atext` all
 * read false. This is the shape of an unquoted local part and of the
 * left and right halves of a `msg-id`.
 *
 * @example
 *   b.codepointClass.isDotAtomText("first.last");                    // → true
 *   b.codepointClass.isDotAtomText("first..last");                   // → false
 */
function isDotAtomText(text) {
  return _isDotAtomOver(text, isAtext);
}

/**
 * @primitive b.codepointClass.isUtf8DotAtomText
 * @signature b.codepointClass.isUtf8DotAtomText(text)
 * @since     0.20.32
 * @status    stable
 * @related   b.codepointClass.isDotAtomText, b.codepointClass.isAtext
 *
 * Is this string RFC 6531 section 3.3 `dot-atom-text` for an
 * internationalized address: the RFC 5322 shape with `atext` extended by
 * every non-ASCII code unit. The dot rule is unchanged, so a leading,
 * trailing or doubled dot still reads false, as does an ASCII character
 * outside `atext` such as `@`, `;` or a space.
 *
 * @example
 *   b.codepointClass.isUtf8DotAtomText("üser");                 // → true
 *   b.codepointClass.isUtf8DotAtomText("üser@evil");            // → false
 */
function isUtf8DotAtomText(text) {
  return _isDotAtomOver(text, _isUtf8Atext);
}

function _isUtf8Atext(cc) {
  return cc >= 0x80 || isAtext(cc);
}

function _isDotAtomOver(text, isMember) {
 ...
 * (`0x7f`), any C0 control (`< 0x20`) other than TAB (`0x09`), and the C1
 * controls (`0x80`-`0x9f`); LF and CR are refused by default but can be
 * permitted per call (a reader that already split on CRLF, or a folding
 * grammar). It agrees with the `CTRL_RANGES` table. `C0_CTRL_RANGES` is a
 * different table: it always exempts LF/CR and never matches DEL or C1.
 *
 * `allowC1` permits `0x80`-`0x9f`. Pass it only when the string is a latin1
 * view of bytes, such as an HTTP field value, where those codes are the
 * continuation bytes of a UTF-8 sequence, or when the grammar being parsed
 * allows C1 (TOML 1.0 strings).
 *
 * @opts
 *   forbidTab: boolean,   // also forbid TAB
 *   allowC1:   boolean,   // permit the C1 controls 0x80-0x9f
 ...
 *   b.codepointClass.isForbiddenControlChar(0x9b);                 // true (C1 CSI)
 *   b.codepointClass.isForbiddenControlChar(0x85, { allowC1: true });   // false (latin1 byte view)
 */
function isForbiddenControlChar(code, opts) {
  if (code === 0x7f) return true;
  if (code >= 0x80 && code <= 0x9f) return !(opts && opts.allowC1 === true);
 ...
 *   allowC1:   boolean,   // permit the C1 controls 0x80-0x9f (latin1 byte views)
 *   allowLf:   boolean,   // permit LF (0x0a)
 *   allowCr:   boolean,   // permit CR (0x0d)
 *
 * @example
 ...
 *   partners("k".codePointAt(0), false);       // → [ 0x4b ], "K" and not the Kelvin sign
 */
...
module.exports = {
  ...
  isAtext:           isAtext,
  isDotAtomText:     isDotAtomText,
  isUtf8DotAtomText: isUtf8DotAtomText,
