 ...
 *
 * It is not "the upper case" and it is not "the lower case". The language folds
 * through upper case, keeps a character whose upper case is more than one
 * character, and refuses to fold a non-ASCII character onto an ASCII one — the
 * last of which is why `/k/i` does not match a Kelvin sign, and why converting
 * both characters and comparing gets the wrong answer. Under `u` the rule
 * changes to case FOLDING, the ASCII guard drops, and a long s does fold onto
 * an `s`.
 *
...
function canonicalizeForCase(cp, unicode) {
  ...
  // "More than one character" is counted in CODE POINTS. Counting UTF-16 units
  // would call every astral letter a multi-character mapping and leave it
  // unfolded, so Deseret capital and small letters would never meet.
  ...
  if (unicode) {
    ...
    return _oneCodePoint(folded) ? folded.codePointAt(0) : cp;
  }
 ...
 * needed.
 *
 * Most partners are the character's own upper and lower forms. A few dozen are
 * not reachable that way — a micro sign and a Greek mu, a final sigma and an
 * ordinary one, the title-case digraphs, and under `u` a Kelvin sign and a `k`
 * — and those come from a table derived from the running platform's own case
 * mappings rather than transcribed from a Unicode revision.
 *
 * @example
