...
function _isDuplicateCheckpointCounter(e) {
  if (!e) return false;
  var text = [
    e.message, e.detail, e.constraint, e.sqlMessage, e.table, e.code, e.errno,
    (typeof e.toString === "function" ? e.toString() : ""),
  ].map(function (v) { return v == null ? "" : String(v); }).join(" | ").toLowerCase();
  var namesCounter = text.indexOf("atmonotoniccounter") !== -1 ||   // sqlite column
                     text.indexOf("chkpt_counter") !== -1;          // idx_chkpt_counter (pg/mysql index)
  if (!namesCounter) return false;
  return text.indexOf("unique constraint failed") !== -1 ||   // sqlite text
         text.indexOf("sqlite_constraint") !== -1 ||          // sqlite code
         text.indexOf("duplicate key") !== -1 ||              // postgres text
         text.indexOf("23505") !== -1 ||                      // postgres SQLSTATE unique_violation
         text.indexOf("duplicate entry") !== -1 ||            // mysql text
         text.indexOf("er_dup_entry") !== -1 ||               // mysql code name
         text.indexOf("1062") !== -1;                         // mysql errno ER_DUP_ENTRY
}
...
async function checkpoint(opts) {
  ...
  try {
    ...
    // Lost the INSERT race to a concurrent anchor of this same tip counter: the
    // counter is already anchored (both anchors sign the identical payload), so
    // return null like the skipIfUnchanged "already anchored" path and let the
    // winner own the sidecar / audit-tip writes below. Any other error rethrows.
    if (_isDuplicateCheckpointCounter(e)) return null;
    throw e;
  }
