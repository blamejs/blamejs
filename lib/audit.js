...
function safeEmit(event) {
  ...
  try {
    ...
    // The try/catch around this block only ever caught a SYNCHRONOUS throw.
    // emit() is async, so a rejection from it escaped as an unhandled
    // rejection and took the process down — the exact opposite of what a
    // drop-silent sink promises, and reachable from any request that emits an
    // audit event. Attach a handler to the promise as well; there is nothing
    // useful to do with the failure here, because reporting it would be
    // another audit emission.
