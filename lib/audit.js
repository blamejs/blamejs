  ...
  return observability.tap("audit.record",
    ...
    async function () {
      ...
      if (_externalStore && typeof _externalStore.record === "function") {
        try {
          await _externalStore.record(appended);
        } catch (e) {
          try {
            observability.event("audit.shadow_failed", {
 ...
 *
 * Failure posture: if the operator's `record` throws or rejects, the
 * shadow failure is surfaced via `b.observability` as the
 * `audit.shadow_failed` event with `{ action, monotonicCounter,
 * error }` metadata, and the framework chain append still succeeds
 * (the row is durable in the framework's own table; the shadow is a
 * best-effort archival). This is the rule §5 drop-silent posture for
 * hot-path observability sinks — an unreachable shadow MUST NOT
 * crash the request path that triggered the audit attempt.
 *
