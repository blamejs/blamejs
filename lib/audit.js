...
async function record(event) {
  ...
  var written = await _chainWriter.append(logical);
  // metrics tap — no-op when no registry is active. Routes into
  // framework_audit_events_total{action, outcome} when an operator
  // has wired b.metrics.create().
  try { require("./metrics").tap("audit.record", 1, { action: event.action, outcome: event.outcome }); }
