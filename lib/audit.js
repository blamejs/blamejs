...
// Mutex serializes record() and checkpoint() calls within this process.
// The audit chain reads the previous tip and writes a successor; without
// serialization, two concurrent awaiting record() calls would hash
// against the same prev-tip and produce a forked chain. Cross-process
// coordination is the cluster module's leader election; this Mutex
...
// ---- Resilience-wrapped SQL operations ----
//
// Each named wrapper enforces a timeout. Idempotent reads also retry
// with backoff (transient external-db errors don't fail the framework
// outright). Non-idempotent writes do NOT retry — a partially-applied
// INSERT followed by a retry could double-write, and the audit chain
// must hold a strict 1:1 between record() calls and inserted rows.
...
async function _readChainTipRow() {
  return await asyncSafe.withTimeout(
    ...
    { name: "audit.readChainTip" }
  );
}

async function _readMaxCounter() {
  return await asyncSafe.withTimeout(
    asyncSafe.asyncRetry(function () {
      return clusterStorage.executeOne(
        "SELECT MAX(monotonicCounter) AS m FROM audit_log"
      );
    }),
    FRAMEWORK_SQL_TIMEOUT_MS,
    { name: "audit.readMaxCounter" }
  );
}
...
// at first record(). The Once wrapper ensures concurrent first-callers
// share one in-flight init Promise rather than each starting their own.
var _nextCounter = 1;
var _counterInit = null;     // asyncSafe.Once instance; rebuilt by _resetForTest

function _ensureCounterInit() {
  if (!_counterInit) {
    _counterInit = new asyncSafe.Once(async function () {
      var row = await _readMaxCounter();
...
async function record(event) {
  ...
  // Mutex serializes the read-prev-tip + compute-hash + insert sequence.
  // Without this, two concurrent record() calls awaiting the SELECT for
  // prev tip would each see the same row and produce sibling rows that
  // both claim the same prevHash — forking the chain.
  return await _chainMutex.runExclusive(async function () {
    return await _recordWithLock(event);
  });
}

async function _recordWithLock(event) {
  ...
  // Cluster-mode rows include the current fencing token; single-node always
  // writes 0. The DB-level fencing-token guard (an UPDATE on the audit-tip
  // row that filters on token >= stored) is wired in v0.1.11; for now the
  // application-layer requireLeader() gate above is the sole protection.
  ...
  var values = allCols.map(function (c) {
    if (c === "fencingToken") return fencingToken;
    return c in sealed ? sealed[c] : null;
  });
  await _insertAuditRow(allCols, values);
...
// Fire-and-forget variant for callers (middleware, log-stream sinks,
// background hooks) that want best-effort audit emission without
// awaiting. Errors are swallowed silently — the audit chain is
// observability, not the load-bearing part of the request.
//
// Returns the underlying Promise so callers who DO want to await
// (and handle errors themselves) still can — they just lose the
// silent-swallow behaviour by branching on the return value.
function recordSafe(event) {
  var p = record(event);
  p.catch(function () { /* intentional swallow — best-effort emission */ });
  return p;
}
