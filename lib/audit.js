...
async function _upsertAuditTip(counter, rowHash, signedAt, fencingToken) {
  ...
  // The dialect-specific half of this — the fence expression, and the fact
  // that MySQL cannot report a refusal and has to be asked afterwards — lives
  // in b.clusterStorage.fencedUpsert, because the purge anchor needs exactly
  // the same thing and a second hand-rolled copy is a second chance to get
  // MySQL's silent no-op wrong.
...
var FRAMEWORK_NAMESPACES = [
  ...
  "mailbimi",   // b.mail.bimi (mail.bimi.vmc.fetched / verified — BIMI VMC chain validation)
...
// Where the audit chain resumes when audit_log holds no rows. Usually that
// means nothing has ever been recorded, and the chain starts at ZERO_HASH from
// counter 1. After a sanctioned purge it means something else: the rows are
// gone because the framework removed them, the purge anchor recorded the hash
// and counter they ended on, and the next row has to continue from THOSE. Link
// to ZERO_HASH instead and the chain reads as broken at the exact point the
// framework itself edited it; restart the counter instead and the new rows sit
// at or below the purge boundary, where every verifier skips them.
//
// An anchor that is PRESENT and cannot be believed stops the append. Starting
// a fresh chain instead looks like the cautious choice and is the opposite of
// one: the new rows link to ZERO_HASH and, on an emptied table, take counters
// at or below the boundary the anchor claims — where a verifier skips them
// entirely. Every write between the tampering and the next verification would
// land there, unlinked to anything and invisible to the check meant to catch
// it. Refusing is loud, immediate, and costs an audit row rather than the
// chain's meaning. Boot already refuses to start on an anchor it cannot
// verify, so this is the same answer for tampering that happens afterwards.
// How this deployment may believe a purge anchor: whether unsigned ones are
// expected, whether one that could not be checked at all is acceptable, and
// where a public key comes from when signing is not loaded.
//
// It lives here, set once at boot, because THREE places ask the same question —
// the boot verify, the append-time chain origin, and the rollback guard — and
// each one that worked it out for itself ended up with a different answer.
// Every disagreement showed up the same way: a volume one of them accepted
// that another then refused, on a configuration the framework documents as
// supported. Defaults are the strict ones, so a consumer that never sets a
// policy gets the signing-on behaviour.
...
async function _resolveChainOrigin() {
  ...
  var built = sql.select("_blamejs_audit_purge_anchor", _sqlOpts())   // allow:hand-rolled-sql
    .where("scope", "audit")
    .toSql();
  // Deliberately not caught. A timeout, a dropped connection or a permission
  // error says nothing about whether rows are missing, and answering it with
  // "the chain starts here" writes rows at counter 1 — at or below any purge
  // boundary that exists, where verification skips them. A read that did not
  // happen must stop the append, not stand in for one that found nothing.
  // The anchor table is created by db.init alongside audit_log, so a volume
  // with rows to append to has it.
  anchor = await clusterStorage.executeOne(built.sql, built.params);
  // No anchor row: likewise nothing licenses an absence, and `null` tells the
  // writer to start at ZERO_HASH from counter 1.
...
function _resetForTest() {
  ...
  _externalStore = null;
  _externalStoreMode = "shadow";
  // Back to the strict defaults. A policy left over from a test that opened a
  // volume with signing off would silently relax the next one.
  _purgeAnchorPolicy = {
    resolvePublicKey: undefined,
    allowUnsigned:    false,
    allowUnchecked:   false,
...
// The outcome vocabulary. `record()` holds callers to it; the table below maps
// the spellings safeEmit accepts onto it. Read at call time, not at load, so
// its position after `record` in the file does not matter.
...
var OUTCOME_NORMALIZE = {
  ...
  skip:      "success",
  skipped:   "success",
  // `warning` is its own outcome, not a synonym for success. They are the two
  // answers a PARTIAL result needs to tell apart: an ingest that stored 490 of
  // 500 records is neither a success nor a denial. Mapped to success, that row
  // read exactly like one with nothing wrong in it, so the query an operator
  // would run to find partial results returned nothing -- and every consumer
  // that needed the distinction invented its own metadata convention for a
  // word the vocabulary already appeared to have.
  warn:      "warning",
  warning:   "warning",
  // Advisory and deadline-miss outcomes: something a reader should look at,
...
// An outcome this does not recognize is a programming error, and on a trail
// that exists to be read after something went wrong the safe direction is not
// "fine". `record()` refuses it outright; safeEmit cannot, because it is
// drop-silent by contract and throwing would lose the row entirely -- so it
// records the loudest value in the vocabulary instead. A typo now shows up as
// a failure row somebody investigates rather than a success nobody reads.
function _normalizeOutcome(o) {
  // OMITTED is not the same as unrecognized. A caller that leaves `outcome`
  // off has not classified the event, and the documented default for that has
  // always been success -- `system.log.incoming` and `system.ws.publish` are
  // two of the callers that rely on it. Turning those into failures would fill
  // the trail with rows that never happened, which is the same kind of damage
  // as the wrong-way default this fix is about, pointed the other way.
  //
  // An unrecognized VALUE is different: somebody wrote a classification and
  // got it wrong, and on a trail read after something went wrong the safe
  // answer is not "fine".
...
// The table's targets are held to the vocabulary at load, so the two cannot
// drift into disagreeing: a normalizer entry pointing at a value `record()`
// refuses would turn a recognized outcome into a dropped row, and the drop is
// silent by design. Checked here rather than trusted, because the two lists
// are written by hand and one of them is long.
(function () {
...
function safeEmit(event) {
  ...
  try {
    ...
    // The try/catch around this block catches only a SYNCHRONOUS throw. The
    // buffering handler's emit() returns nothing today, so there is no
    // rejection to escape — but this is a drop-silent sink on the request
    // path, and if emit() ever starts returning a promise, an unhandled
    // rejection from it would take the process down: the exact opposite of
    // what the sink promises. Contain whatever comes back. There is nothing
    // useful to do with a failure here, because reporting it would be another
    // audit emission.
    ...
    // Through the primitive rather than a hand-rolled attach, so the
    // containment does not have to be re-derived correctly at each site.
...
module.exports = {
  // Run `fn` holding the audit chain's append lock. A sanctioned deletion
  // needs it: removing rows and recording the anchor that accounts for them
  // are two writes, and an append landing between them links to a hash the
  // anchor is about to replace — a break the next verify reports as tampering,
  // caused by the framework's own bookkeeping rather than by anything an
  // operator did. Excludes appends made through this process's chain writer,
  // which is the only path that writes audit_log.
  withChainLock:        function (fn) { return _chainWriter.withChainLock(null, fn); },
  // Declare how a purge anchor may be believed on this deployment. Called by
  // b.db.init once its signing posture is settled, so the boot verify, the
  // append path and the rollback guard all answer that question the same way.
  // Read it back, for the operations that run AFTER boot and have to agree
  // with the decision boot made — extending an anchor, grounding the next
  // archive's proof. Each of those asking on its own behalf is how a volume
  // ends up accepted at startup and refused an hour later.
  ...
  // Tell the chain writer that the purge anchor changed, so the next append to
  // an emptied audit_log asks for the boundary again instead of using the one
  // it cached. Called by whoever wrote the new anchor, while it still holds
  // the lock above.
