...
// corruption. Recognize it (backend-agnostic: the message must name the
// counter column AND a uniqueness violation) so the loser returns null like
// the skipIfUnchanged "already anchored" path instead of throwing. Anything
...
function _isDuplicateCheckpointCounter(e) {
  ...
  if (msg.indexOf("atmonotoniccounter") === -1) return false;
  return msg.indexOf("unique constraint failed") !== -1 ||   // sqlite
         msg.indexOf("duplicate key") !== -1 ||               // postgres
         msg.indexOf("duplicate entry") !== -1;               // mysql
}
