 ...
 *
 * Streaming shape (multi-GB exports without OOM):
 *
 *   var archive = b.archive.zip();
 *   archive.addFile("logs/2026-q1.ndjson", fs.createReadStream("…"));   // Readable
 *   archive.addFile("logs/2026-q2.ndjson", fs.createReadStream("…"));
 *   await archive.toStream(res);              // pipe directly to HTTP response
 *
 * The stream form deflates each entry through a piped zlib transform
 * (no whole-entry Buffer.concat in the hot path), writes the central
 * directory only after every entry finalizes, and aborts the destination
 * with `archive/aborted` if any entry source errors. Operators reading
 * the docs for the "download my data" shape continue using `toBuffer()`
 * — `toStream()` is the path for archives larger than process RSS.
 *
 * Out of scope:
