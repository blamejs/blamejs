...
function _mustEscape(code) {
  return codepointClass.isForbiddenControlChar(code) ||
...
function _needsQuoting(str) {
  ...
  for (var i = 0; i < str.length; i += 1) {
    if (_mustEscape(str.charCodeAt(i))) return true;
  }
  return false;
}
...
function _quoteString(str) {
  ...
  for (var i = 0; i < str.length; i += 1) {
    ...
    else if (_mustEscape(code)) out += "\\u" + code.toString(16).padStart(4, "0");
    else                    out += ch;
  }
  out += '"';
  return out;
