...
// The first "]" at or after a position, or -1 when none remains. Both scans
// below ask only about positions that move forward, so one pointer that never
// goes backwards answers every question in a single pass over the text.
//
// Searching per opening bracket is what makes either scan quadratic: text that
// is a run of "[" with no "]" re-reads the whole suffix from each one. A table
// of one entry per character fixes the time and spends memory several times
// the size of the input to do it, which is the same amplifier pointed at a
// different resource.
...
function _findBracketUrls(text, wantBang, emit) {
  var len = text.length;
  var i = 0;
  // Openings are visited left to right, so the closing bracket each one needs
  // is never behind the last one found.
  var nextClose = null;
  while (i < len) {
    ...
    if (close === -1) return;                         // nothing closes from here on
...
function _findRefUrls(text, emit) {
  ...
  var lineStart = 0;
  var consumedTo = 0;
  // Lines are visited in order, so the closing bracket each one needs is
  // never behind the last one found. Searching per line re-read the whole
  // suffix from each, so output that is a run of lines beginning "[" with no
  // "]" anywhere cost time proportional to its square.
  var nextClose = null;
  for (;;) {
    var nl = -1;
...
// Call onUrl(url) for each URL an extractor sees and, when onUrl returns a
// replacement string, splice it in. Returns the possibly-rewritten text.
//
// `scan(text, emit)` reports each URL as `emit(url, start)`, in increasing
// position order, and this splices each one as it arrives rather than holding
// them. Model output is accepted up to a `maxBytes` the caller may raise, and
// output made of many short links produces a match for every one of them, so
// collecting them first costs a multiple of a response that was accepted.
//
// Splicing uses `start` rather than searching the matched text for the URL:
// when a markdown alt text equals its target URL, a search would hit the alt
// copy and leave the real URL intact (EchoLeak).
function _rewriteUrls(text, scan, onUrl) {
  ...
  scan(text, function (url, start) {
    ...
    if (start < last) return;                       // already rewritten
...
// `>` that ends the value and does not run away. This reports through the same
// emit the scans above use.
