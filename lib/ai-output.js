...
function _findBracketUrls(text, wantBang) {
  ...
  while (i < len) {
    ...
    if (nextClose === null) {
      nextClose = new Int32Array(len + 1);
      nextClose[len] = -1;
      for (var back = len - 1; back >= 0; back -= 1) {
        nextClose[back] = text.charCodeAt(back) === 0x5D ? back : nextClose[back + 1];
      }
    }
