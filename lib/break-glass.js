...
var DEFAULT_MAX_ROWS        = 1;
var DEFAULT_REASON_MIN_LEN  = 12;
var DEFAULT_LOCKED_BEHAVIOR = "throw";
var DEFAULT_AUDIT_REASON    = "cleartext";
var ALLOWED_FACTORS         = ["totp", "passkey"];
var ALLOWED_REASON_STORAGE  = ["cleartext", "hmac", "both"];

// allow:hand-rolled-sql — these constants are cryptoField registry keys, not SQL text; SQL composed via b.sql passes the same bare logical name for clusterStorage to rewrite
...
async function unsealRow(grantHandle, table, rowId, opts) {
  ...
  if (grantRow.revokedAt) {
    var revokedMs = numericBounds.finiteTimestamp(grantRow.revokedAt);
    throw new BreakGlassError("breakglass/grant-revoked",
      "unsealRow: grant " + grantHandle.id + " was revoked at " +
      (isFinite(revokedMs) ? new Date(revokedMs).toISOString() : "an unreadable time"), true);
  }

  var expiresAtMs = numericBounds.finiteTimestamp(grantRow.expiresAt);
  if (expiresAtMs <= Date.now()) {
    ...
    throw new BreakGlassError("breakglass/grant-expired",
      ...
      (isFinite(expiresAtMs) ? new Date(expiresAtMs).toISOString() : "an unreadable time"), true);
  }
