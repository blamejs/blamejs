...
function _verifyTotpFactor(factor, lastUsedStep) {
  ...
  // Thread lastUsedStep (highest previously-accepted TOTP step for this
  // credential) + factor.now (deterministic test clocking) into totp.verify.
  // Without lastUsedStep, totp.verify accepts any code valid in the drift
  // window — so the same code replays until the window passes. Passing the
  // stored step makes verify reject any step at-or-below it.
  var vopts = {};
  if (typeof lastUsedStep === "number") vopts.lastUsedStep = lastUsedStep;
  if (factor && typeof factor.now === "number") vopts.now = factor.now;
...
// Read the highest TOTP step previously accepted for (actorId, secret).
// Returns a number, or null when none recorded. Drop-silent on cache
// faults — a missing prior step degrades to "no replay floor", which the
// atomic commit-Math.max below still tightens on success.
async function _readLastTotpStep(actorId, secret) {
  _ensureFactorLockout();
  try {
    var v = await _factorLockoutCache.get(_replayStepKey(actorId, secret));
    return typeof v === "number" ? v : null;
  } catch (_e) { return null; }
}

// Commit Math.max(prior, newStep) atomically so a concurrent grant for the
// same credential cannot lower the replay floor (lost-update race). The
// TTL outlives the verify drift window many times over so a replayed code
// is still floored when it arrives.
async function _commitTotpStep(actorId, secret, newStep) {
  _ensureFactorLockout();
  if (typeof newStep !== "number") return;
  try {
    await _factorLockoutCache.update(_replayStepKey(actorId, secret), function (prior) {
      var floor = typeof prior === "number" ? Math.max(prior, newStep) : newStep;
      return { value: floor };
    }, { ttlMs: REPLAY_STEP_TTL_MS });
  } catch (_e) { /* drop-silent — cache fault degrades replay floor, not auth */ }
}
...
async function grant(opts) {
  ...
  var factorOk = false;
  var acceptedTotpStep = null;
  var totpSecret = null;
  if (factorType === "totp") {
    totpSecret = opts.factor && opts.factor.secret;
    // Replay defense — read the highest step previously accepted for this
    // (actor, secret) pair and pass it as the verify floor so a code
    // already redeemed inside the drift window is rejected.
    var lastStep = (typeof totpSecret === "string" && totpSecret.length > 0)
      ? await _readLastTotpStep(actorId, totpSecret)
      : null;
    var totpResult = _verifyTotpFactor(opts.factor, lastStep);
    factorOk = totpResult.ok;
    acceptedTotpStep = totpResult.step;
  ...
  // Commit the accepted TOTP step (keyed by actor + secret fingerprint) so
  // the same code cannot be replayed inside the drift window on a later
  // grant. Math.max under an atomic update — a concurrent grant for the
  // same credential can only raise the floor, never lower it.
  if (factorType === "totp" && typeof acceptedTotpStep === "number" &&
      typeof totpSecret === "string" && totpSecret.length > 0) {
    await _commitTotpStep(actorId, totpSecret, acceptedTotpStep);
  }
