...
function dotStuff(buf) {
  ...
  if (dots === 0) return buf;
  var out = Buffer.alloc(n + dots);
  var oi = 0;
  if (buf[0] === 0x2e) out[oi++] = 0x2e;
  out[oi++] = buf[0];
  for (var j = 1; j < n; j += 1) {
    out[oi++] = buf[j];
    if (buf[j - 1] === 0x0d && buf[j] === 0x0a &&
        j + 1 < n && buf[j + 1] === 0x2e) {
      out[oi++] = 0x2e;
    }
  }
...
/**
 * @primitive b.safeSmtp.canonicalizeEol
 * @signature b.safeSmtp.canonicalizeEol(buf)
 * @since     0.20.9
 * @status    stable
 * @related   b.safeSmtp.dotStuffForWire, b.safeSmtp.dotStuff
 *
 * Rewrite every line ending in a buffer to canonical CRLF: a bare LF
 * and a bare CR each become CRLF, an existing CRLF is unchanged. It
 * brings a stored message body to the wire form a POP3 or SMTP
 * multiline response requires before dot-stuffing, and gives the octet
 * count the peer receives, which is what `RETR` advertises rather than
 * the stored, possibly non-canonical, byte length.
 *
 * @example
 *   b.safeSmtp.canonicalizeEol(Buffer.from("a\nb\rc\r\nd")).toString("utf8");
 *   // → "a\r\nb\r\nc\r\nd"
 */
function canonicalizeEol(buf) {
  if (!Buffer.isBuffer(buf)) {
    throw new SafeSmtpError("safe-smtp/bad-input",
      "canonicalizeEol: input must be a Buffer");
  }
  var n = buf.length;
  var extra = 0;
  for (var i = 0; i < n; i += 1) {
    if (buf[i] === 0x0a) {
      if (i === 0 || buf[i - 1] !== 0x0d) extra += 1;
    } else if (buf[i] === 0x0d) {
      if (i + 1 >= n || buf[i + 1] !== 0x0a) extra += 1;
    }
  }
  if (extra === 0) return buf;
  var out = Buffer.alloc(n + extra);
  var oi = 0;
  for (var j = 0; j < n; j += 1) {
    var b = buf[j];
    if (b === 0x0d) {
...
/**
 * @primitive b.safeSmtp.dotStuffForWire
 * @signature b.safeSmtp.dotStuffForWire(buf)
 * @since     0.20.9
 * @status    stable
 * @related   b.safeSmtp.dotStuff, b.guardSmtpCommand.detectBodySmuggling
 *
 * Prepare a stored message body for a POP3 `RETR` / `TOP` or an SMTP
 * `DATA` multiline response: canonicalize every line ending to CRLF,
 * then apply RFC 5321 §4.5.2 / RFC 1939 §3 dot-stuffing.
 *
 * `dotStuff` on its own doubles a line-initial `.` only at a canonical
 * CRLF boundary, so a body stored with bare-LF or bare-CR line endings
 * (a Maildir / mbox artifact) emits a lone `.` line un-stuffed. A peer
 * that treats a bare LF or CR as a line boundary then reads that `.` as
 * the multiline terminator and the bytes after it as a fresh message or
 * response. This runs the canonicalization that `dotStuff` documents its
 * caller must do first, so every line ending is CRLF before stuffing and
 * every line-initial `.` is doubled.
 *
 * @example
 *   b.safeSmtp.dotStuffForWire(Buffer.from("hello\n.\nrest")).toString("utf8");
 *   // → "hello\r\n..\r\nrest"
 */
function dotStuffForWire(buf) {
  if (!Buffer.isBuffer(buf)) {
    throw new SafeSmtpError("safe-smtp/bad-input",
      "dotStuffForWire: input must be a Buffer");
  }
  var n = buf.length;
  var outLen = 0;
  ...
  for (i = 0; i < n; i += 1) {
    ...
    if (b === 0x0d) {
      outLen += 2;
      if (i + 1 < n && buf[i + 1] === 0x0a) i += 1;
      atLineStart = true;
    } else if (b === 0x0a) {
      outLen += 2;
      atLineStart = true;
    } else {
      if (atLineStart && b === 0x2e) outLen += 1;
      outLen += 1;
      atLineStart = false;
    }
  }
  if (outLen === n) return buf;
  var out = Buffer.alloc(outLen);
  var oi = 0;
  atLineStart = true;
  for (i = 0; i < n; i += 1) {
    var c = buf[i];
    if (c === 0x0d) {
      ...
      if (atLineStart && c === 0x2e) out[oi++] = 0x2e;
      out[oi++] = c;
      atLineStart = false;
    }
  }
...
module.exports = {
  ...
  dotStuff:          dotStuff,
  dotStuffForWire:   dotStuffForWire,
  canonicalizeEol:   canonicalizeEol,
