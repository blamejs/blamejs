 ...
 * `transform()` returns a `stream.Transform` that consumes from the
 * shared bucket; multiple transforms returned from the same bucket
 * share the rate budget. `state()` returns
 * `{ bytesPerSec, burstBytes, tokens, lastRefillNs }` for observation.
 *
 * @opts
