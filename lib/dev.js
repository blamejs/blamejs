 ...
 * `graceMs` / `killTimeoutMs`, an `ignore` entry that is neither a RegExp nor
 * a string (`dev/bad-ignore`), an `ignore` RegExp with the `g` or `y` flag
 * (`dev/stateful-pattern`), or an attempt to load with
...
function create(opts) {
  ...
  for (var ii = DEFAULT_IGNORE.length; ii < ignore.length; ii += 1) {
    var ignoreLabel = "dev.create: opts.ignore[" + (ii - DEFAULT_IGNORE.length) + "]";
    if (typeof ignore[ii] !== "string" && !(ignore[ii] instanceof RegExp)) {
      throw new DevError("dev/bad-ignore", ignoreLabel + " must be a RegExp or a string; got " + typeof ignore[ii]);
    }
    guardRegex.assertStateless(ignore[ii], ignoreLabel, DevError, "dev/stateful-pattern");
  }
