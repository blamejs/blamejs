...
function create(opts) {
  ...
  function _killChild() {
    return new Promise(function (resolve) {
      ...
      // A normally-terminating child fires 'exit'; a child that failed to
      // spawn (ENOENT) fires 'error' + 'close' but NEVER 'exit'. Settle on
      // any of them — otherwise stop() would await an 'exit' that never comes
      // and hang the supervisor (a ref'd keep-alive would never release, so a
      // single Ctrl-C could not terminate a bad-command dev run).
      c.once("exit",  done);
      c.once("close", done);
      c.once("error", done);
      var sent;
      ...
      // kill() returns false when there is no live OS process to signal (the
      // child failed to spawn, or already exited): settle now, don't wait for
