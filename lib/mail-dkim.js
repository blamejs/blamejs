...
function _parseDkimTagList(value) {
  ...
  //
  // `lowerKey: false` because §3.2 says tags are interpreted case-sensitively.
  // Folding them accepts a selector record published as `V=DKIM1; K=rsa;
  // P=...` that a conforming verifier reads as carrying no `p=` tag, so it has
  // no public key and the signature fails. The operator is told their DKIM is
  // correct while the receivers they send to treat their mail as unsigned, and
  // nothing on the sending side can see it: the only party that reports the
  // truth is the receiver. Tag VALUES have their own case rules, tag by tag,
