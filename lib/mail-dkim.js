...
// ARC reuse channel. b.mail.arc.verify validates an ARC-Message-Signature by
// renaming it to DKIM-Signature and running it through this verifier (the
// cryptographic shape is identical). But an AMS carries i=<instance> — an
// RFC 8617 §4.1.2 instance number (1..50), NOT a DKIM AUID — so the §3.5
// AUID-subdomain check (which an AMS would always fail, since "1" is not a
// subdomain of d=) must be skipped for that one reuse. The signal travels on
// a non-enumerable Symbol key so it is genuinely framework-internal: an
// operator calling b.mail.dkim.verify with a string opts object cannot reach
// it (validate-opts only walks string keys), so the AUID check stays a
// non-opt-out security default on the public DKIM path.
var ARC_AMS_REUSE = Symbol("blamejs.mail.dkim.arcAmsReuse");
...
async function verify(rfc822, opts) {
  ...
  // Framework-internal ARC reuse: when set (Symbol key, unreachable from the
  // public string-opts surface), the i= tag is an RFC 8617 instance number,
  // not a DKIM AUID — so the §3.5 AUID-subdomain check is skipped for it.
...
module.exports = {
  ...
  bootstrap:   bootstrap,
  verify:      verify,
  ARC_AMS_REUSE: ARC_AMS_REUSE,   // framework-internal: b.mail.arc reuse channel (see verify())
