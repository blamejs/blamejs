 ...
 *
 * An observed-remove map: keys follow observed-remove-set semantics (so a key
 * can be set, removed, and set again), and each key's value is a last-write-
 * wins register, so concurrent writes to the same key converge by timestamp.
...
function orMap(opts) {
  ...
  return {
    ...
    remove: function (key) { keys.remove(_mapKey(key)); return this; },
...
function _decodeKeys(set) {
  ...
  }).sort(function (a, b) { return String(a) < String(b) ? -1 : (String(a) > String(b) ? 1 : 0); });
}
