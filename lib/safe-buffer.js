 ...
 *
 * Public API:
 *   bufferSafe.normalizeText(input, { maxBytes, stripBom, errorClass })
 ...
 *
 *   bufferSafe.toBuffer(data, { maxBytes, errorClass })
 *     Accept Buffer | Uint8Array | string → returns Buffer. Throws
 *     errorClass on type mismatch or oversize.
 *
 *   bufferSafe.boundedChunkCollector({ maxBytes, errorClass })
 ...
 *   e.code === "xml/too-large" etc.). A default BufferSafeError is used
 *   if the caller doesn't pass one.
 */

var { FrameworkError } = require("./framework-error");

class BufferSafeError extends FrameworkError {
  constructor(message, code) {
    super(message);
    this.name = "BufferSafeError";
    this.code = code || "buffer/invalid";
    this.isBufferSafeError = true;
  }
}

function _throw(errorClass, message, code) {
  var Cls = errorClass || BufferSafeError;
...
function boundedChunkCollector(opts) {
  ...
  var maxBytes = (typeof opts.maxBytes === "number" && opts.maxBytes > 0) ? opts.maxBytes : null;
  if (maxBytes === null) {
    throw new BufferSafeError("boundedChunkCollector requires maxBytes", "buffer/bad-arg");
  }
  var errClass = opts.errorClass;
  var sizeCode = opts.sizeCode  || "buffer/too-large";
...
module.exports = {
  ...
  BufferSafeError:       BufferSafeError,
};
