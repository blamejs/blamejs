...
var SafeBufferError = frameworkError.defineMessageFirstClass(
  "SafeBufferError", "buffer/invalid");

var PREVIEW_TEXT_MAX_CHARS = 64;
...
function normalizeText(input, opts) {
  ...
  var stripBom  = opts.stripBom !== false;
...
function toBuffer(data, opts) {
  ...
  var allowString = opts.allowString !== false;
  var typeCode = opts.typeCode  || "buffer/wrong-input-type";
  var sizeCode = opts.sizeCode  || "buffer/too-large";
 ...
 *   // → 20  (just past the '>' of <body class=x>)
 *
...
/**
 * @primitive b.safeBuffer.hasCrlfOrNul
 * @signature b.safeBuffer.hasCrlfOrNul(s)
 * @since     0.19.0
 * @related   b.safeBuffer.hasCrlf, b.safeBuffer.stripCrlf
 *
 * Detect CR, LF or NUL in a string. A header field carries all three as
 * one class of problem: CR and LF end the field, and NUL truncates it
 * in any consumer that hands the value on to C. A multipart part header
 * parsed out of a body, and one about to be serialized into one, both
 * refuse on this predicate.
 *
 * Non-string input returns `false`, so the predicate chains without a
 * pre-typecheck, exactly as `hasCrlf` does — a caller that will COERCE
 * the value into a header must refuse a non-string itself first.
 *
 * @example
 *   var b = require("blamejs");
 *   b.safeBuffer.hasCrlfOrNul("filename.txt");
 *   // → false
 *
 *   b.safeBuffer.hasCrlfOrNul("ok\r\nX-Injected: bad");
 *   // → true
 *
 *   // A NUL truncates the value wherever it reaches C.
 *   b.safeBuffer.hasCrlfOrNul("ok" + String.fromCharCode(0) + "hidden");
 *   // → true
 */
function hasCrlfOrNul(s) {
  return codepointClass.indexOfAny(s, CRLF_NUL_CHARS) !== -1;
}
...
/**
 * @primitive b.safeBuffer.previewText
 * @signature b.safeBuffer.previewText(value, opts?)
 * @since     0.20.32
 * @status    stable
 * @related   b.safeBuffer.quoteString
 *
 * Render a piece of untrusted input for a refusal message: printable
 * US-ASCII passes through, every other code point becomes `\u{...}`, and the
 * result is capped (64 characters by default) with `...` when it was cut.
 *
 * A parser that quotes the document it refused is quoting bytes a stranger
 * chose. `ESC [ 2 K` is erase-in-line, so a consumer writing the message to
 * a terminal loses the line it printed beside, and U+202E reverses the
 * remainder of the sentence an operator is reading. A consumer cannot screen
 * the message afterwards, because by then the parser has decided what the
 * sentence says.
 *
 * @opts
 *   maxChars: number,   // default: 64
 *
 * @example
 *   var b = require("blamejs");
 *   var esc = String.fromCharCode(0x1b);
 *   b.safeBuffer.previewText("X-Probe" + esc + "[2K");
 *   // → "X-Probe\\u{1b}[2K"
 */
function previewText(value, opts) {
  var text = typeof value === "string" ? value : String(value);
  var maxChars = opts && numericBounds.isPositiveFiniteInt(opts.maxChars)
    ? opts.maxChars : PREVIEW_TEXT_MAX_CHARS;
  var capped = text.length > maxChars ? text.slice(0, maxChars) : text;
  var out = "";
  for (var i = 0; i < capped.length; i += 1) {
    var code = capped.charCodeAt(i);
    if (code >= 0x20 && code <= 0x7e) { out += capped.charAt(i); continue; }
    out += "\\u{" + code.toString(16) + "}";
  }
  return text.length > maxChars ? out + "..." : out;
}

/**
 * @primitive b.safeBuffer.readQuotedString
 * @signature b.safeBuffer.readQuotedString(input, at?)
 * @since     0.20.32
 * @status    stable
 * @related   b.safeBuffer.quoteString
 *
 * Read one operand of the quoted-string grammar `b.safeBuffer.quoteString`
 * writes: a DQUOTE-wrapped run whose backslash escapes are undone, or a bare
 * run up to the next space. Leading spaces are skipped. Returns
 * `{ value, next }`, where `next` is the offset just past the operand, so a
 * caller reads a line of operands by feeding `next` back in.
 *
 * Returns `null` when there is no operand to read, and when a quoted run is
 * unterminated or ends on a dangling backslash. An unterminated quote means
 ...
 *
 * The reader lives beside the writer so the two cannot drift apart: a
 * consumer that unquotes differently from the code that quoted disagrees
 * with it about what an argument says.
 *
 * @example
 *   var b = require("blamejs");
 *   b.safeBuffer.readQuotedString('"two words" after');
 *   // → { value: "two words", next: 11 }
 *
 *   b.safeBuffer.readQuotedString("INBOX rest");
 *   // → { value: "INBOX", next: 5 }
 */
function readQuotedString(input, at) {
  var s = typeof input === "string" ? input : String(input == null ? "" : input);
  var i = typeof at === "number" && isFinite(at) && at > 0 ? Math.floor(at) : 0;
...
/**
 * @primitive b.safeBuffer.unquotedIndexOf
 * @signature b.safeBuffer.unquotedIndexOf(input, character, from?)
 * @since     0.20.32
 * @status    stable
 * @related   b.safeBuffer.readQuotedString, b.safeBuffer.quoteString
 *
 * Report the offset of the first `character` that is not inside a quoted
 * string, starting at `from` (default 0), or -1 when there is none. A
 * backslash escapes the character after it, inside a quoted run and outside
 * one alike.
 *
 * A delimiter that also appears inside quotes is the shape behind the header
 * and command readers that cut an address short: `<"a>b"@example.com>` ends
 * at the second `>`, not the first. Searching for the raw character finds
 * the one the sender wrote as data.
 *
 * Returns -1 for a `character` that is empty, a DQUOTE or a backslash, since
 * those are the grammar itself rather than a delimiter within it.
 *
 * @example
 *   var b = require("blamejs");
 *   b.safeBuffer.unquotedIndexOf('"a>b"@example.com>', ">");
 *   // → 17
 */
function unquotedIndexOf(input, character, from) {
  var s = typeof input === "string" ? input : String(input == null ? "" : input);
  var target = typeof character === "string" && character.length > 0 ? character.charAt(0) : "";
  if (target === "" || target === "\"" || target === "\\") return -1;
  var i = typeof from === "number" && isFinite(from) && from > 0 ? Math.floor(from) : 0;
  while (i < s.length) {
    var ch = s.charAt(i);
    if (ch === "\\") { i += 2; continue; }
    if (ch === "\"") {
...
module.exports = {
  ...
  unquotedIndexOf:       unquotedIndexOf,
  toBuffer:              toBuffer,
