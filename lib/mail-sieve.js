...
var MATCH_GAS_CHARS_PER_UNIT = 1024;
...
function _matchWork(matchType, valueLen, needleLen) {
  if (matchType === "is" || matchType === "contains") return valueLen + needleLen + 1;
  return (valueLen + 1) * (needleLen + 1);
}

function _anyOfMatches(haystackList, needleList, matchType, comparator, ctx) {
  for (var i = 0; i < haystackList.length; i++) {
    var valueLen = haystackList[i].length;
    for (var j = 0; j < needleList.length; j++) {
      if (ctx) {
        _chargeGas(ctx, 1 + Math.floor(
          _matchWork(matchType, valueLen, needleList[j].length) / MATCH_GAS_CHARS_PER_UNIT));
      }
...
var IMPLEMENTED_TESTS = Object.freeze([
  "true", "false", "not", "allof", "anyof",
  "exists", "size", "header", "address", "envelope",
]);

var IMPLEMENTED_ACTIONS = Object.freeze([
  "keep", "fileinto", "discard", "redirect", "stop",
]);

/**
 * @primitive  b.mail.sieve.implementedNames
 * @signature  b.mail.sieve.implementedNames()
 * @since      0.20.32
 * @status     stable
 * @related    b.safeSieve.validate, b.mail.sieve.run
 *
 * The test and action names this interpreter resolves. `b.safeSieve.parse`
 * reads them so a script naming an action the interpreter does not implement
 * is refused at upload, where the author can still fix it, rather than at
 * delivery, where the throw costs the whole script.
 *
 * A consumer that needs the same answer reads it here instead of keeping its
 * own copy, which would rot at the next extension.
 *
 * @example
 *   b.mail.sieve.implementedNames().actions;
 *   // → ["keep", "fileinto", "discard", "redirect", "stop"]
 */
function implementedNames() {
  return { actions: IMPLEMENTED_ACTIONS.slice(), tests: IMPLEMENTED_TESTS.slice() };
}

function _evalTest(test, env, ctx) {
  ctx.depth += 1;
  if (ctx.depth > MAX_NESTING_DEPTH) {
    throw new MailSieveError("mail-sieve/nesting-too-deep",
      "Sieve test nesting exceeds " + MAX_NESTING_DEPTH + " levels");
  }
  try {
    return _evalTestInner(test, env, ctx);
  } finally {
    ctx.depth -= 1;
  }
}

function _evalTestInner(test, env, ctx) {
  ctx.gas++;
  if (ctx.gas > ctx.maxGas) {
    throw new MailSieveError("mail-sieve/gas-exhausted",
      "Sieve gas exhausted (cap " + ctx.maxGas + ")");
  ...
  if (name === "header") {
    ...
    return _anyOfMatches(allValues, keys, matchType, comparator, ctx);
  }
  ...
  if (name === "address" || name === "envelope") {
    ...
    return _anyOfMatches(allAddrs, keys2, matchType2, comparator2, ctx);
  }
...
function _runCommand(cmd, env, ctx) {
  ...
  if (cmd.kind === "action") {
    ...
    if (n === "redirect") {
      ...
      var ctrlAt = codepointClass.firstControlCharOffset(addr, { forbidTab: true });
      if (ctrlAt !== -1) {
        throw new MailSieveError("mail-sieve/bad-redirect",
          "redirect address carries control byte 0x" +
          addr.charCodeAt(ctrlAt).toString(16) + " at offset " + ctrlAt +
          ", and a CR or LF in it would end the SMTP command that carries it");
      }
      ctx.implicitKeepCancelled = true;
...
function _runCommands(commands, env, ctx) {
...
function _runBlock(commands, env, ctx) {
  ...
  if (ctx.depth > MAX_NESTING_DEPTH) {
    throw new MailSieveError("mail-sieve/nesting-too-deep",
      "Sieve block nesting exceeds " + MAX_NESTING_DEPTH + " levels");
  }
  try {
    _runCommands(commands, env, ctx);
  } finally {
    ctx.depth -= 1;
  }
}
 ...
 *     sizeBytes:  1024,
 *   });
 *   // → { actions: [{ kind: "fileinto", folder: "Junk" }], gas: 5, stopped: false }
 */
function run(ast, env, opts) {
  ...
  _runCommands(ast.commands, env || {}, ctx);
