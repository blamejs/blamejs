...
async function _subscribe(topics, pubsub, name, handler, sOpts, permissions, auditImpl) {
  ...
  // Cross-tenant subscription refused at registry — when tenantScope
  // is set, subscriber's actor must declare a matching tenantId (and
  // the publish-side wrapping carries the publisher's tenantId; the
  // delivery wrapper below filters).
  ...
  function _wrappedHandler(wrapped, evMeta) {
    ...
    if (entry.tenantScope && subscriberTenant && wrapped._tenantId &&
        wrapped._tenantId !== subscriberTenant) {
      _safeAudit(auditImpl, "agent.event_bus.cross_tenant_drop", sOpts.actor, {
        topic: name, publisherTenant: wrapped._tenantId, subscriberTenant: subscriberTenant,
      });
      return;
    }
    ...
    try { handler(wrapped.payload, { topic: name, publishedAt: wrapped._publishedAt, source: evMeta && evMeta.source }); }
    catch (e) {
