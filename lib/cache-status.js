...
var CACHE_NAME_RE   = /^[\x21-\x7E]{1,128}$/;                                                      // allow:raw-byte-literal — printable ASCII + length cap
...
function entryString(entry) {
  ...
  if (entry.cache.length > 128 || !CACHE_NAME_RE.test(entry.cache)) {                              // allow:raw-byte-literal — cache-name length cap, not bytes
    throw new CacheStatusError("cache-status/bad-cache-name",
      "entry.cache '" + entry.cache + "' must be printable ASCII <= 128 chars");
  }
  ...
  if (entry.ttl !== undefined && entry.ttl !== null) {
    if (typeof entry.ttl !== "number" || !Number.isInteger(entry.ttl) || entry.ttl < 0) {
      throw new CacheStatusError("cache-status/bad-ttl",
        "entry.ttl must be a non-negative integer");
    }
