...
// (RFC 9651 §3.3.4) OR sf-string. We accept sf-token shape bare; an
...
// per RFC 9651: starts with ALPHA or "*", continues with tchar / ":"
// / "/". tchar excludes `, ; " \ space and all controls.
var CACHE_NAME_RE   = /^[A-Za-z*][!#$%&'*+\-.^_`|~0-9A-Za-z:/]*$/;                                 // allow:duplicate-regex — sf-token shape per RFC 9651 §3.3.4
...
function entryString(entry) {
  ...
  if (entry.cache.length > CACHE_NAME_MAX || !CACHE_NAME_RE.test(entry.cache)) {
    throw new CacheStatusError("cache-status/bad-cache-name",
      ...
      "(RFC 9651 §3.3.4: starts with ALPHA or '*', uses tchar / ':' / '/' only — " +
