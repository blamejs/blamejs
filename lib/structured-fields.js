...
//
// Value model:
//   bare item  → number | string | boolean | SfToken | SfByteSequence
...
function _parseByteSeq(cx, E) {
  ...
  // Canonical base64 round trip rejects stray characters, bad padding,
  // and non-zero trailing bits (Node's decoder is otherwise permissive).
  if (buf.toString("base64") !== b64) throw E("structured-fields/parse", "byte sequence is not canonical base64");
