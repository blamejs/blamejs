...
/**
 * @primitive b.structuredFields.endsInsideQuotedString
 * @signature b.structuredFields.endsInsideQuotedString(s)
 * @since     0.20.32
 * @status    stable
 * @related   b.structuredFields.splitTopLevel, b.structuredFields.unquoteSfString
 *
 * Report whether `s` ends with a quoted string still open, counting RFC 9651
 * §3.3.3 backslash escapes. `splitTopLevel` drops the piece it cannot
 * terminate, so a caller reading that array cannot tell an absent parameter
 * from one an unterminated quote swallowed. Ask this before treating a
 * missing piece as unwritten: a grammar whose default for an absent value is
 * weaker than the value being smuggled past has to refuse the field instead.
 *
 * @example
 *   b.structuredFields.endsInsideQuotedString('a="1"; b=2');
 *   // → false
 *
 *   b.structuredFields.endsInsideQuotedString('a="1; b=2');
 *   // → true
 *
 *   b.structuredFields.endsInsideQuotedString('a="1\\"');
 *   // → true
 */
function endsInsideQuotedString(s) {
 ...
 * honors HTTP structured-field backslash-quote escapes; the two are NOT
 ...
 *
 * A RegExp `sep` is screened with `b.guardRegex.assertSafe` on each call,
 * which throws `GuardRegexError` `regex/unsafe-pattern` for a pattern it
 * refuses.
 *
 * @opts
...
function parseTagList(input, opts) {
  ...
  if (sep instanceof RegExp) {
    guardRegex.assertSafe(sep, "structuredFields.parseTagList: opts.sep",
      guardRegex.GuardRegexError, "regex/unsafe-pattern");
  }
 ...
 * @signature b.structuredFields.recordVersionMatches(record, name, value, opts?)
 * @since     0.20.30
 * @status    stable
 * @related   b.structuredFields.parseTagList
 *
 * Does a DNS TXT policy record begin with the version field `name=value`,
 * ended where its grammar ends it? Mail policy records select themselves this
 * way before any other field is read: SPF (RFC 7208 §4.5) takes a record whose
 * version section is exactly `v=spf1`, ended by a space or the end of the
 * record; DMARC (RFC 9989 §4.8), MTA-STS (RFC 8461 §3.1) and TLS-RPT
 * (RFC 8460 §3) end the version at `*WSP ";"`. A record that fails the check
 * is discarded, so `v=spf10`, `v=DMARC1x` and `x v=STSv1` are not policy
 * records.
 *
 * Case is per record type, because their ABNF differs and a uniform rule is
 * wrong in one direction or the other. A quoted ABNF literal folds (RFC 5234
 * §2.3); a value spelled in hex does not. SPF writes the whole version as
 * `"v=spf1"`, so both halves fold. DMARC and MTA-STS write the name as `"v"`
 * and the value in hex, so the name folds and the value is exact. TLS-RPT
 * writes even the name as `%x76`, so neither folds. The caller states which,
 * rather than this function guessing from the value.
 *
 * @opts
 *   end:              "space" | "semicolon", // default "semicolon"; "space" ends the version at SP or the end of the record
 *   wspAroundEquals:  boolean,  // default false; allow *WSP on both sides of "=" (DMARC)
 *   nameIgnoresCase:  boolean,  // default false; compare `name` case-insensitively (the quoted `"v"` of DMARC / MTA-STS / SPF)
 *   valueIgnoresCase: boolean,  // default false; compare `value` case-insensitively (SPF's quoted `"v=spf1"` only)
 *   requireNextField: boolean,  // default false; refuse a record that ends after the version (MTA-STS, TLS-RPT)
 *
 * @example
 *   b.structuredFields.recordVersionMatches("v=spf1 -all", "v", "spf1", { end: "space" });   // → true
 *   b.structuredFields.recordVersionMatches("v=spf10 +all", "v", "spf1", { end: "space" });  // → false
 *   b.structuredFields.recordVersionMatches("v = DMARC1; p=reject", "v", "DMARC1",
 *     { wspAroundEquals: true, nameIgnoresCase: true });                                    // → true
 */
function recordVersionMatches(record, name, value, opts) {
  opts = opts || {};
  if (typeof record !== "string" || typeof name !== "string" || typeof value !== "string") return false;
  var head = record.slice(0, name.length);
  if (opts.nameIgnoresCase === true ? head.toLowerCase() !== name.toLowerCase() : head !== name) return false;
  var i = name.length;
  if (opts.wspAroundEquals === true) while (_isWsp(record.charAt(i))) i += 1;
  if (record.charAt(i) !== "=") return false;
  i += 1;
  if (opts.wspAroundEquals === true) while (_isWsp(record.charAt(i))) i += 1;
  var seen = record.slice(i, i + value.length);
  if (opts.valueIgnoresCase === true
        ? seen.toLowerCase() !== value.toLowerCase()
        : seen !== value) return false;
  i += value.length;
  if (opts.end === "space") return i === record.length || record.charAt(i) === " ";
  while (_isWsp(record.charAt(i))) i += 1;
  if (i === record.length) return opts.requireNextField !== true;
  return record.charAt(i) === ";";
}
 ...
 * grammar. `splitTopLevel` honors quotes/parens for RFC 9651 lists, while a
 ...
 * `.trim()` then absorbs. Codes `0x80`-`0xff` are permitted as
 * obs-text (RFC 9110 §5.5): Node exposes a header value as a latin1
 * string, so those codes are the bytes of a UTF-8 sequence.
 *
...
function refuseControlBytes(value, opts) {
  ...
  if (codepointClass.firstControlCharOffset(value, { forbidTab: !allowHt, allowC1: true }) !== -1) {
 ...
 * default as folding-whitespace). Codes `0x80`-`0xff` are permitted
 * as obs-text, the same as `refuseControlBytes`.
 *
 * @opts
...
function containsControlBytes(value, opts) {
  ...
  for (var i = 0; i < value.length; i += 1) {
    ...
    if (codepointClass.isForbiddenControlChar(cc, { forbidTab: true, allowC1: true })) return true;
  }
  return false;
}
...
function _parseByteSeq(cx, E) {
  ...
  var b64 = cx.s.slice(start, cx.i); cx.i += 1;
...
function serialize(value, type, opts) {
  ...
  if (type === "dictionary") {
    ...
    return entries.map(function (e) {
      ...
      if (m && !Array.isArray(m.items) && m.value === true) return key + _serParams(m.params, E);
