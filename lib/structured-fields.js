 ...
 * ended where its grammar ends it? `value` is compared case-sensitively.
 * Mail policy records select themselves this way before any other field is
 * read: SPF (RFC 7208 §4.5) takes a record whose version section is exactly
 * `v=spf1`, ended by a space or the end of the record; DMARC (RFC 9989 §4.8),
 * MTA-STS (RFC 8461 §3.1) and TLS-RPT (RFC 8460 §3) end the version at
 * `*WSP ";"`. A record that fails the check is discarded, so `v=spf10`,
 * `v=DMARC1x` and `x v=STSv1` are not policy records.
 *
 * @opts
 ...
 *   nameIgnoresCase:  boolean,  // default false; compare `name` case-insensitively (DMARC's "v")
...
function recordVersionMatches(record, name, value, opts) {
  ...
  i += 1;
  if (opts.wspAroundEquals === true) while (_isWsp(record.charAt(i))) i += 1;
  if (record.slice(i, i + value.length) !== value) return false;
  i += value.length;
