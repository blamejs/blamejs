...
function _startSpan(tracing, name, sopts) {
  ...
  // tracing.span() wraps a function in a span; for the agent
  // primitive we want a manual span so the caller controls lifetime.
  // Use the lower-level startSpan if available; fall back to span().
  if (typeof tracing.startSpan === "function") {
    return tracing.startSpan(name, sopts);
  }
  // Stub-shape span — operator's tracing doesn't have manual startSpan.
  // Returns a minimal lifecycle object so callers don't blow up.
  var startedAt = Date.now();
  return {
    name: name,
    startedAt: startedAt,
    setAttribute: function () { /* no-op stub */ },
    setStatus:    function () { /* no-op stub */ },
    end:          function () { /* no-op stub */ },
    _stub:        true,
  };
}
...
function _recordResult(span, result, error) {
  ...
  if (error) {
    ...
    if (typeof span.setStatus === "function") {
      try { span.setStatus({ code: 2, message: (error && error.message) || String(error) }); }
