 ...
 *       inherits RFC 9112 §2.2's CRLF-only rule. Bare-LF terminators are the
...
function parse(buf, opts) {
  ...
  for (var i = 1; i < lines.length; i += 1) {
    var line = lines[i];
    if (line.length === 0) continue;                                                              // RFC 9112 §5 — blank header lines refused below as bad-header anyway
    headerCount += 1;
...
function _refuseBadHeaderBytes(buf, headerEnd) {
  // RFC 3507 §4.3.1 inherits RFC 9112 §2.2's CRLF-only rule. Bare-CR /
...
function _parseHeaderLine(line, maxValueBytes) {
  // RFC 9112 §5 — field-name ":" OWS field-value OWS. ICAP inherits.
  ...
  // RFC 9110 §5.6.2 — field-name token chars (RFC 5234 ALPHA / DIGIT
  ...
  for (var i = 0; i < name.length; i += 1) {
    ...
    if (!ok) {
      throw new SafeIcapError("safe-icap/bad-status-line",
        "safeIcap.parse: invalid char in header name '" + name + "' (RFC 9110 §5.6.2 tchar)");
    }
  }
