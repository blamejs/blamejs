 ...
 * @since     0.9.81
 * @status    stable
 * @related   b.safeIcap.compliancePosture, b.mail.scan
 *
...
function _parseStatusLine(line) {
  ...
  if (!ALLOWED_STATUS.hasOwnProperty(statusCode)) {
...
function _parseHeaderLine(line, maxValueBytes) {
  ...
  var value = line.slice(colon + 1).replace(/^\s+|\s+$/g, "");                                     // allow:regex-no-length-cap — bounded by line length already capped
