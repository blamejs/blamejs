...
function createDeadlineClock(opts) {
  ...
  function _notify(payload) {
    if (!notify) return;
    // Drop-silent — escalation is best-effort and never crashes a tick — and
    // that includes a send that REJECTS rather than throws. Composed rather
    // than hand-rolled so the containment is the one definition every other
    // consumer hook uses.
