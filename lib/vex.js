...
// CSAF 2.1 §3.2.2.1 — TLP (Traffic Light Protocol) labels.
var TLP_LABELS = Object.freeze(["WHITE", "GREEN", "AMBER", "RED", "CLEAR"]);
 ...
 * statement.
 *
 * Required: `cveId` (or `cwe` for non-CVE vulnerabilities; one MUST
 * be supplied), `status` (one of STATUS_VALUES), `productIds` (array
 ...
 *
 * @opts
 *   cveId:        string,    // CVE-YYYY-NNNN (one of cveId / cwe required)
 *   cwe:          string,    // CWE-NNN
...
function statement(opts) {
  ...
  if (!opts.cveId && !opts.cwe) {
    throw new VexError("vex/missing-vuln-id",
      "statement: one of cveId or cwe is required");
  }
  ...
  if (opts.cwe !== undefined) {
    if (typeof opts.cwe !== "string" || !/^CWE-\d+$/.test(opts.cwe)) {
      throw new VexError("vex/bad-cwe",
        "statement: cwe must match `CWE-NNN` (got '" + opts.cwe + "')");
    }
  }
  ...
  if (opts.cwe) vuln.cwe = { id: opts.cwe, name: opts.cwe };
