 ...
 *   sampling lets an attacker distinguish neighboring datasets with
...
function _frLte(a, b) { return a.num * b.den <= b.num * a.den; }
function _frGt(a, b)  { return a.num * b.den >  b.num * a.den; }
...
function _uniformOpen() {
  var v = Number(_uniformBelow(9007199254740992n));
  return (v + 1) / TWO_POW_53;
}
...
function _bernoulli(p) {
  return _uniformBelow(p.den) < p.num ? 1 : 0;
}
function _bernoulliExp1(x) {
  var k = 1n;
  for (;;) {
...
function _sampleDLaplace(scaleNum, scaleDen) {
  var invScale = _fr(scaleDen, scaleNum);
  for (;;) {
...
function _sampleDGauss(sigma2) {
  var t = _floorSqrtFrac(sigma2) + 1n;
  var two_sigma2 = _fr(2n * sigma2.num, sigma2.den);
  var sigma2_over_t = _fr(sigma2.num, sigma2.den * t);
  for (;;) {
    var candidate = _sampleDLaplace(t, 1n);
    var absC = candidate < 0n ? -candidate : candidate;
    var diff = _frSub(_fr(absC, 1n), sigma2_over_t);
...
function budget(opts) {
  ...
  var spentEpsilon = 0;
  var spentDelta = 0;
  var rdp = RDP_ORDERS.map(function () { return 0; });
