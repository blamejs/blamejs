...
The `Dockerfile` is multi-stage (deps build + slim runtime), runs as the unprivileged `node` user, and uses a `node:24-slim` base (the engine pin in `package.json` requires `>=24.21.0`, matching the framework's own floor). Argon2id routes through Node's built-in `crypto.argon2*` API, so the image carries no native-module prebuild.
